#!/usr/bin/env python3
"""Chroma1-HD sampler artifact manifest guard.

This is a no-CUDA metadata/header gate for Chroma sampler parity evidence. It
does not run the text encoder, transformer, VAE, or image comparison. It defines
the paired SerenityTrainer/Mojo artifact bundle required before a Chroma "sampler
parity" or "sampler speed parity" claim can be accepted.
"""
...
DEFAULT_MANIFEST = Path("/home/alex/serenity_trainer-mojo/parity/chroma_sampler_parity_manifest.json")
EXPECTED_MODEL_TYPE = "CHROMA_1"
EXPECTED_SCHEDULER = "FlowMatchEulerDiscreteScheduler"
EXPECTED_LATENT_CHANNELS = 16
EXPECTED_PACKED_CHANNELS = 64
EXPECTED_TEXT_WIDTH = 4096
EXPECTED_CFG_BATCH = 2
TEMPLATE_PROMPT_ID = "chroma1_hd_sampler_parity_todo"
TEMPLATE_PROMPT_POSITIVE = "TODO_FILL_EXACT_PROMPT"
TEMPLATE_PROMPT_NEGATIVE = "TODO_FILL_EXACT_NEGATIVE_PROMPT"
TEMPLATE_SEED = 42
TEMPLATE_WIDTH = 1024
...
REQUIRED_ARTIFACT_KEYS = (
    "serenity_trainer_initial_noise_raw_nchw",
    "serenity_trainer_initial_noise_packed",
    "serenity_trainer_latent_trajectory",
    "mojo_latent_trajectory",
    "serenity_trainer_cfg_prediction_trajectory",
    "mojo_cfg_prediction_trajectory",
    "serenity_trainer_prompt_embedding",
    "serenity_trainer_text_attention_mask",
    "serenity_trainer_attention_mask",
    "serenity_trainer_text_ids",
    "serenity_trainer_image_ids",
    "serenity_trainer_final_packed_latent",
    "serenity_trainer_final_unpacked_latent",
    "serenity_trainer_vae_input_latent",
    "serenity_trainer_vae_decoded_tensor",
    ...
    "serenity_trainer_png",
    "mojo_png",
)
REQUIRED_PNG_ARTIFACT_KEYS = ("serenity_trainer_png", "mojo_png")
...
def check_metric_pair(metrics: dict[str, Any], key: str) -> Check:
    reference = as_dict(metrics, "serenity_trainer")
    mojo = as_dict(metrics, "mojo")
    serenity_value = reference.get(key)
    mojo_value = mojo.get(key)
    if not isinstance(serenity_value, (int, float)) or not isinstance(mojo_value, (int, float)):
        return Check(False, f"metric {key}", "serenity_trainer and mojo values must be numeric")
    if float(serenity_value) <= 0.0 or float(mojo_value) <= 0.0:
        return Check(False, f"metric {key}", f"values must be positive: reference={serenity_value} mojo={mojo_value}")
    return Check(True, f"metric {key}", f"reference={serenity_value} mojo={mojo_value}")
...
def _template_artifact_groups() -> dict[str, Any]:
    return {
        "serenity_trainer_seed_replay_inputs": [
            "serenity_trainer_initial_noise_raw_nchw",
            "serenity_trainer_initial_noise_packed",
        ],
        "paired_cfg_prediction": [
            {
                "serenity_trainer": "serenity_trainer_cfg_prediction_trajectory",
                "mojo": "mojo_cfg_prediction_trajectory",
            }
        ],
        "paired_text_conditioning": [
            {"serenity_trainer": "serenity_trainer_prompt_embedding", "mojo": "mojo_prompt_embedding"},
            {
                "serenity_trainer": "serenity_trainer_text_attention_mask",
                "mojo": "mojo_text_attention_mask",
            },
            {"serenity_trainer": "serenity_trainer_attention_mask", "mojo": "mojo_attention_mask"},
            {"serenity_trainer": "serenity_trainer_text_ids", "mojo": "mojo_text_ids"},
            {"serenity_trainer": "serenity_trainer_image_ids", "mojo": "mojo_image_ids"},
        ],
        "paired_latent_trajectory": [
            {"serenity_trainer": "serenity_trainer_latent_trajectory", "mojo": "mojo_latent_trajectory"},
            {"serenity_trainer": "serenity_trainer_final_packed_latent", "mojo": "mojo_final_packed_latent"},
            {"serenity_trainer": "serenity_trainer_final_unpacked_latent", "mojo": "mojo_final_unpacked_latent"},
            {"serenity_trainer": "serenity_trainer_vae_input_latent", "mojo": "mojo_vae_input_latent"},
        ],
        "paired_decode_and_png": [
            {"serenity_trainer": "serenity_trainer_vae_decoded_tensor", "mojo": "mojo_vae_decoded_tensor"},
            {"serenity_trainer": "serenity_trainer_png", "mojo": "mojo_png"},
        ],
    }
...
def required_manifest_fields() -> dict[str, Any]:
    return {
        ...
        "metrics": [f"serenity_trainer.{key}" for key in REQUIRED_METRIC_KEYS]
...
def required_paired_sampler_fields(target_manifest: Path = DEFAULT_MANIFEST) -> dict[str, Any]:
    return {
        "serenity_trainer_reference": "/home/alex/SerenityTrainer ChromaSampler.py, same prompt/negative/seed/resolution/steps/cfg/dtype",
        "mojo_reference": "serenitymojo Chroma sampler/product path, same prompt/negative/seed/resolution/steps/cfg/dtype",
        "strict_check_command": (
            "python3 scripts/check_chroma_sampler_artifact_manifest.py "
            f"--manifest {target_manifest} --strict"
        ),
        "template_command": (
            "python3 scripts/check_chroma_sampler_artifact_manifest.py "
            f"--manifest {target_manifest} "
            "--write-template /tmp/chroma_sampler_parity_manifest.template.json"
        ),
        "readiness_command": (
            "python3 scripts/check_chroma_sampler_artifact_manifest.py "
            f"--manifest {target_manifest} "
            "--write-readiness /tmp/chroma_sampler_manifest_readiness.json"
        ),
        "parity_acceptance_note": (
            "All tensor paths must point to real paired SerenityTrainer/Mojo artifacts; "
...
def build_template_manifest(
    *,
    ...
    artifact_root: str = "/tmp/chroma_sampler_parity_artifacts",
    ...
    prediction_shape = (steps, 1, n_img, EXPECTED_PACKED_CHANNELS)
    prompt_embedding_shape = (EXPECTED_CFG_BATCH, text_tokens, EXPECTED_TEXT_WIDTH)
    text_mask_shape = (EXPECTED_CFG_BATCH, text_tokens)
    attention_mask_shape = (EXPECTED_CFG_BATCH, text_tokens + n_img)
    text_ids_shape = (text_tokens, 3)
    image_ids_shape = (n_img, 3)
    decoded_shape = (1, 3, height, width)

    artifacts: dict[str, Any] = {
        "serenity_trainer_initial_noise_raw_nchw": _template_tensor_artifact(
            artifact_root, "serenity_trainer_initial_noise_raw_nchw", raw_shape, dtype="F32"
        ),
        "serenity_trainer_initial_noise_packed": _template_tensor_artifact(
            artifact_root, "serenity_trainer_initial_noise_packed", packed_shape, dtype="F32"
        ),
        "serenity_trainer_latent_trajectory": _template_tensor_artifact(
            artifact_root, "serenity_trainer_latent_trajectory", trajectory_shape, dtype="F32"
        ),
        ...
        "serenity_trainer_cfg_prediction_trajectory": _template_tensor_artifact(
            artifact_root, "serenity_trainer_cfg_prediction_trajectory", prediction_shape, dtype="F32"
        ),
        "mojo_cfg_prediction_trajectory": _template_tensor_artifact(
            artifact_root, "mojo_cfg_prediction_trajectory", prediction_shape, dtype="TODO_FILL_STORAGE_DTYPE"
        ),
    }
    for prefix in ("serenity_trainer", "mojo"):
        ...
        artifacts[f"{prefix}_text_attention_mask"] = _template_tensor_artifact(
            artifact_root, f"{prefix}_text_attention_mask", text_mask_shape, dtype=dtype
        )
        artifacts[f"{prefix}_attention_mask"] = _template_tensor_artifact(
            artifact_root, f"{prefix}_attention_mask", attention_mask_shape, dtype=dtype
        )
    ...
    artifacts["serenity_trainer_png"] = _template_png_artifact(artifact_root, "serenity_trainer_png", width, height)
    artifacts["mojo_png"] = _template_png_artifact(artifact_root, "mojo_png", width, height)

    return {
        "schema_version": 1,
        "producer": "scripts/check_chroma_sampler_artifact_manifest.py --write-template",
        "template": True,
        "template_note": "Fill every TODO path/value with real paired SerenityTrainer/Mojo evidence before strict use.",
        ...
        "artifacts": artifacts,
        "metrics": {
            "serenity_trainer": {key: 0.0 for key in REQUIRED_METRIC_KEYS},
...
def write_template_manifest(path: Path, target_manifest: Path) -> None:
    ...
    print(f"[chroma-sampler-manifest] INFO wrote template manifest: {path}")
...
def inspect_manifest(path: Path) -> list[Check]:
    if not path.exists():
        return [
            Check(
                False,
                "manifest",
                f"missing {path}; expected paired reference trainer/Mojo Chroma sampler evidence manifest",
            )
        ]
    ...
    checks.append(Check(prompt.get("random_seed") is False, "prompt.random_seed", "must be false for deterministic reference trainer replay"))
    checks.append(Check(isinstance(prompt.get("cfg_scale"), (int, float)), "prompt.cfg_scale", str(prompt.get("cfg_scale"))))
    checks.append(Check(prompt.get("cfg_batch_size") == EXPECTED_CFG_BATCH, "prompt.cfg_batch_size", "ChromaSampler always runs prompt+negative branches"))
    ...
    checks.append(Check(width % 64 == 0 and height % 64 == 0, "prompt.resolution_quantized", f"{width}x{height}"))

    latent_h = height // 8 if height > 0 else 0
    latent_w = width // 8 if width > 0 else 0
    packed_h = latent_h // 2 if latent_h > 0 else 0
    packed_w = latent_w // 2 if latent_w > 0 else 0
    n_img = packed_h * packed_w
    raw_shape = (1, EXPECTED_LATENT_CHANNELS, latent_h, latent_w)
    packed_shape = (1, n_img, EXPECTED_PACKED_CHANNELS)
    ...
    prediction_shapes = (
        (steps, 1, n_img, EXPECTED_PACKED_CHANNELS),
        (steps, n_img, EXPECTED_PACKED_CHANNELS),
    )
    prompt_embedding_shape = (EXPECTED_CFG_BATCH, text_tokens, EXPECTED_TEXT_WIDTH)
    text_mask_shape = (EXPECTED_CFG_BATCH, text_tokens)
    attention_mask_shape = (EXPECTED_CFG_BATCH, text_tokens + n_img)
    ...
    checks.append(Check(isinstance(scheduler.get("shift"), (int, float)), "scheduler.shift", str(scheduler.get("shift"))))
    checks.append(Check(scheduler.get("dynamic_shift") is False, "scheduler.dynamic_shift", "ChromaSampler does not pass dynamic mu"))
    ...
    checks.extend(
        check_tensor_artifact(
            ...
            "serenity_trainer_cfg_prediction_trajectory",
            expected_dtype="F32",
            allowed_shapes=prediction_shapes,
        )
    )
    checks.extend(
        check_tensor_artifact(
    ...
    for prefix in ("serenity_trainer", "mojo"):
        ...
        checks.extend(
            check_tensor_artifact(
                artifacts,
                f"{prefix}_text_attention_mask",
                allowed_shapes=(text_mask_shape,),
            )
        )
        checks.extend(
            check_tensor_artifact(
                ...
                allowed_shapes=(attention_mask_shape,),
            )
        )
        checks.extend(
            check_tensor_artifact(
    ...
    for image_key in ("serenity_trainer_png", "mojo_png"):
        try:
...
def print_report(path: Path, checks: list[Check]) -> None:
    print("[chroma-sampler-manifest] scope=no-CUDA metadata/header gate")
    print(f"[chroma-sampler-manifest] manifest={path}")
    for check in checks:
        status = "PASS" if check.ok else "BLOCKED"
        print(f"[chroma-sampler-manifest] {status} {check.label}: {check.detail}")
    blockers = [check for check in checks if not check.ok]
    print(f"[chroma-sampler-manifest] blockers={len(blockers)}")
...
def build_readiness_report(path: Path, checks: list[Check]) -> dict[str, Any]:
    ...
    return {
        "schema_version": 1,
        "producer": "scripts/check_chroma_sampler_artifact_manifest.py",
        "scope": "Chroma1-HD sampler manifest readiness; no CUDA, no denoise, no VAE run, no image comparison",
        "manifest": str(path),
...
def write_readiness_report(path: Path, manifest: Path, checks: list[Check]) -> None:
    ...
    print(f"[chroma-sampler-manifest] INFO wrote readiness report: {path}")
...
def main() -> int:
    ...
    parser.add_argument(
        "--write-template",
        type=Path,
        help="Write a fill-in skeleton for the paired Chroma1-HD sampler parity manifest.",
    )
