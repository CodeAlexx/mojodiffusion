...
- Z-Image backend executes its internal Euler-like flow-match schedule, bounded
  ...
  512x512 only. It rejects unsupported sampler controls, supports proven
  multi-LoRA runtime stacking for accepted formats, records requested/executed
  sampler fields plus DPM++/UniPC trace fields, and records sampler parity as
  not accepted.
