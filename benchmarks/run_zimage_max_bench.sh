...
# MAX's Z-Image attention dlopens cuDNN. Prefer an explicit override, then the
# active Pixi/Conda environment. Do not assume a developer-home installation.
if [[ -z ${CUDNN_DIR:-} ]]; then
  if [[ -z ${CONDA_PREFIX:-} ]]; then
    echo "error: run through Pixi or set CUDNN_DIR" >&2
    exit 1
  fi
  CUDNN_DIR="$CONDA_PREFIX/lib"
fi
...
: > "$SERVE_LOG"
setsid pixi run bash -c "export LD_LIBRARY_PATH=$CUDNN_DIR:\$LD_LIBRARY_PATH; MAX_SERVE_API_TYPES='[\"responses\"]' max serve --model $MODEL --devices gpu --no-enable-overlap-scheduler" >"$SERVE_LOG" 2>&1 &
SERVE_PID=$!
echo "serve PID $SERVE_PID"
cleanup() { echo "=== teardown ==="; kill -- -"$SERVE_PID" 2>/dev/null; wait "$SERVE_PID" 2>/dev/null; }
trap cleanup EXIT
