...
# feeds the EXACT fixed image the oracle dumped (parity/ldmenc_img_64x64.bin,
# [1,3,64,64] in [-1,1], fed as BF16), and compares:
#   * MOMENTS  (NHWC [1,8,8,8], mean|logvar) vs parity/ldmenc_moments_64x64.bin
#   * MODE     (NCHW [1,4,8,8], the mean)    vs parity/ldmenc_mode_64x64.bin
...
def main() raises:
    ...
    var imgvals = _read_f32_bin(String(PARITY_DIR) + "/ldmenc_img_64x64.bin")
    var ish = List[Int]()
    ish.append(1); ish.append(3); ish.append(64); ish.append(64)
    ...
    # MOMENTS (the gate).
    var ref_moments = _read_f32_bin(String(PARITY_DIR) + "/ldmenc_moments_64x64.bin")
    ...
    var ref_mode = _read_f32_bin(String(PARITY_DIR) + "/ldmenc_mode_64x64.bin")
    ...
    var ref_moments_f32 = _read_f32_bin(
        String(PARITY_DIR) + "/ldmenc_moments_f32_64x64.bin"
    )
