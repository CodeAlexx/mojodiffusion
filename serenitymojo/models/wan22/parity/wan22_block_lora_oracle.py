...
def make_weights(seed):
    ...
    for pfx in ("sa", "ca"):
        ...
        # Wan2.2 qk_norm = "rms_norm_across_heads" -> RMSNorm over the FULL inner
        # dim (heads*head_dim = DIM), not per-head Dh (diffusers transformer_wan.py
        # RMSNorm(dim_head*heads)); matches wan22_block.mojo rms_norm(q_flat[S,DIM]).
        w[pfx + "_qn"] = rnd(DIM) * 0.1 + 1.0
        w[pfx + "_kn"] = rnd(DIM) * 0.1 + 1.0
...
def main():
    ...
    # qk-norm over the FULL DIM (rms_norm_across_heads): norm the flat [S,DIM]
    # linear output, THEN reshape to heads — matches wan22_block.mojo:453-455.
    q = rms_norm_lastdim(lin_lora(sa_in, w["sa_wq"], w["sa_bq"], lo["sa_q_A"], lo["sa_q_B"]), w["sa_qn"]).reshape(1, S, H, Dh)
    k = rms_norm_lastdim(lin_lora(sa_in, w["sa_wk"], w["sa_bk"], lo["sa_k_A"], lo["sa_k_B"]), w["sa_kn"]).reshape(1, S, H, Dh)
    ...
    # cross-attn qk-norm over full DIM (rms_norm_across_heads), then reshape.
    caq = rms_norm_lastdim(lin_lora(n3, w["ca_wq"], w["ca_bq"], lo["ca_q_A"], lo["ca_q_B"]), w["ca_qn"]).reshape(1, S, H, Dh)
    cak = rms_norm_lastdim(lin_lora(context, w["ca_wk"], w["ca_bk"], lo["ca_k_A"], lo["ca_k_B"]), w["ca_kn"]).reshape(1, TXT, H, Dh)
