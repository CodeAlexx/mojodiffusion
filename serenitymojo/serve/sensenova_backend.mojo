...
comptime WEIGHTS_DIR = "models/sensenova-u1"
comptime VOCAB_JSON = "models/sensenova-u1/vocab.json"
comptime MERGES_TXT = "models/sensenova-u1/merges.txt"
comptime ADDED_TOKENS_JSON = "models/sensenova-u1/added_tokens.json"
...
struct SensenovaBackendShape[
    ...
    def start(mut self, params: JobParams) raises:
        ...
        # MJ-1055: the flow-match Euler schedule in _prepare_job divides by
        # params.steps (t = i/steps). Reject a non-positive step count here at
        # admission so it fails LOUD instead of hitting a divide-by-zero (or an
        # empty schedule) deep in the denoise prep.
        if params.steps < 1:
            raise Error(
                String("sensenova: steps must be >= 1 (got ") + String(params.steps)
                + String("); the flow-match Euler schedule divides by steps")
            )
