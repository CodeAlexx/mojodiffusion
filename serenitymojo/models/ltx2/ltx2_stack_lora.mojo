...
# (training/lora_adamw_plain_fused.mojo; host-vs-device class = ±1 bf16 quantum
# on params + ulp-level F32 moment drift, per MJ-1017 / the zimage
# ZIMAGE_FUSED_ADAMW pattern). Gate:
# models/ltx2/parity/ltx2_gpu_adamw_update_parity.mojo. Host loop kept below
# (ltx2_lora_adamw_step_unfused) as the fallback + gate reference. Defaulted
# hyperparams pinned to the _lora_adamw defaults this step always used.
comptime LTX2_GPU_ADAMW = True
...
def ltx2_lora_adamw_step(
    ...
    comptime if LTX2_GPU_ADAMW:
        fused_lora_adamw_plain_step(
            lora.ad, grads.d_a, grads.d_b, 0, total_ltx2_adapters(lora),
            t, lr, beta1, beta2, eps, weight_decay, ctx,
        )
    else:
        ltx2_lora_adamw_step_unfused(
            lora, grads, t, lr, ctx, beta1, beta2, eps, weight_decay
        )


# The pre-2026-07 host scalar loop, UNCHANGED math (parity/gate reference and
# LTX2_GPU_ADAMW=False fallback).
def ltx2_lora_adamw_step_unfused(
    mut lora: LTX2LoraSet, grads: LTX2LoraGradSet, t: Int, lr: Float32, ctx: DeviceContext,
    beta1: Float32 = Float32(0.9), beta2: Float32 = Float32(0.999),
    eps: Float32 = Float32(1.0e-8), weight_decay: Float32 = Float32(0.01),
) raises:
    ...
    for i in range(n):
        ...
        _lora_adamw(lora.ad[i], g, t, lr, ctx, beta1, beta2, eps, weight_decay)


# ── MJ-1085 resident host-grad AdamW (persistent device P/M/V) ────────────────
# Sibling of ltx2_lora_adamw_step (LTX2_GPU_ADAMW per-step fused arm): the
...
# uploads P/M/V once (ltx2_lora_adamw_state_init) then per step only grads go up
# and params come back — same kernel, bit-identical values.
def ltx2_lora_adamw_state_init(
    lora: LTX2LoraSet, ctx: DeviceContext
) raises -> LoraAdamWPlainDeviceState:
    return lora_adamw_plain_device_state_init(
        lora.ad, 0, total_ltx2_adapters(lora), ctx
    )


def ltx2_lora_adamw_step_resident(
    mut state: LoraAdamWPlainDeviceState,
    mut lora: LTX2LoraSet, grads: LTX2LoraGradSet, t: Int, lr: Float32,
    ctx: DeviceContext,
    beta1: Float32 = Float32(0.9), beta2: Float32 = Float32(0.999),
    eps: Float32 = Float32(1.0e-8), weight_decay: Float32 = Float32(0.01),
) raises:
...
def _ltx2_lora_prefixes(num_layers: Int) -> List[String]:
    ...
    for bi in range(num_layers):
        for s in range(LTX2_SLOTS):
            out.append(String("transformer_blocks.") + String(bi) + String(".") + slot_names[s])
    return out^


def save_ltx2_lora_state(
    lora: LTX2LoraSet, path: String, ctx: DeviceContext,
    var meta: List[Float32] = List[Float32](),
) raises -> Int:
    """Write the trainer-only RESUME state (A/B + AdamW moments) for the 4×num_layers
    plain-LoRA adapters, in the SAME block×slot order as save_ltx2_lora. `meta`
    (optional) is the resume-scope guard vector (e.g. [step, seed])."""
    var prefixes = _ltx2_lora_prefixes(lora.num_layers)
    var named = List[NamedLora]()
    var n = total_ltx2_adapters(lora)
    for i in range(n):
...
def load_ltx2_lora_state(
    num_layers: Int, D: Int, rank: Int, alpha: Float32,
    path: String, ctx: DeviceContext,
) raises -> LTX2LoraSet:
    """Reload the LoRA set (A/B + AdamW moments) written by save_ltx2_lora_state.
    The restored moments ride the LoraAdapter host lists into the resident device
    AdamW state (ltx2_lora_adamw_state_init seeds dev_m/dev_v from them)."""
    var scale = alpha / Float32(rank)
    var prefixes = _ltx2_lora_prefixes(num_layers)
...
def load_ltx2_lora_resume(
    num_layers: Int, D: Int, rank: Int, alpha: Float32,
    path: String, ctx: DeviceContext,
) raises -> LTX2LoraSet:
    """WARM resume: reload A/B weights only (from a PEFT file that carries no AdamW
    moments). Moments are ZEROED — the caller must warn loudly. Same prefix order as
    save_ltx2_lora / load_ltx2_lora_state."""
    var scale = alpha / Float32(rank)
    var prefixes = _ltx2_lora_prefixes(num_layers)
