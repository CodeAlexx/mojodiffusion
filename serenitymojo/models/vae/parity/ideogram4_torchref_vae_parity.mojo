# Parity: Ideogram-4 VAE *encoder* (training data path) vs **torchref** oracle.
#
# Oracle = ideogram4_torchref_vae_oracle.py (torchref encode_images:556-578):
...
# torchref runs the WHOLE encode (incl the norm divide) in bf16 (vae dtype).
...
#   pixi run mojo run -I . serenitymojo/models/vae/parity/ideogram4_torchref_vae_parity.mojo
...
comptime FX = "/home/alex/mojodiffusion/serenitymojo/models/vae/parity/ideogram4_torchref_vae.safetensors"
...
def main() raises:
    ...
    # PROD reference (torchref bf16-norm) — the gate that matters.
    ...
    print("latents vs PROD (torchref bf16-norm):", r_prod)
    ...
    print("STD  mojo:", lat_std, " torchref prod:", lat_prod_std)
    ...
    # used) and the final normalized latents vs torchref's bf16-norm production.
    ...
    if not r_prod.passed:
        raise Error(
            "ideogram4 torchref VAE encode parity FAILED: cos="
            + String(r_prod.cos)
            + " < "
            + String(COS_BAR)
        )

    print("ideogram4 torchref VAE encode parity OK")
