# sampling/vae_encoder_contract.mojo - SerenityTrainer VAE encoder/cache contracts.
#
# This is the encode/precache companion to vae_postprocess_contract.mojo. It does
# not run a VAE. It records the SerenityTrainer data-loader contract before denoise:
...
from serenitymojo.sampling.serenity_trainer_sampler_contract import (
    ...
    SERENITY_SAMPLER_UNKNOWN,
    SERENITY_SAMPLER_ZIMAGE,
    serenity_sampler_family,
    serenity_sampler_family_name,
)


comptime SERENITY_VAE_ENCODE_INPUT_IMAGE_MINUS1_TO_1 = 0

comptime SERENITY_VAE_CACHE_SAMPLE_MEAN = 0

comptime SERENITY_VAE_CACHE_LAYOUT_NCHW = 0
comptime SERENITY_VAE_CACHE_LAYOUT_NCDHW_SINGLE_FRAME = 1

comptime SERENITY_VAE_PREP_LAYOUT_NCHW = 0
comptime SERENITY_VAE_PREP_LAYOUT_NCDHW_SINGLE_FRAME = 1

comptime SERENITY_VAE_PREP_SCALE_MUL_SCALING = 0
comptime SERENITY_VAE_PREP_SCALE_SUB_SHIFT_MUL_SCALING = 1
comptime SERENITY_VAE_PREP_SCALE_QWEN_MEAN_STD = 2
comptime SERENITY_VAE_PREP_SCALE_PATCH_BN = 3

comptime SERENITY_VAE_ENCODER_UNSUPPORTED = 0
comptime SERENITY_VAE_ENCODER_PARTIAL_PREPARED_ONLY = 1
comptime SERENITY_VAE_ENCODER_RAW_CACHE_READY = 2
...
@fieldwise_init
struct SerenityTrainerVaeEncoderContract(Copyable, Movable):
    var family: Int
    var name: String
...
def default_serenity_vae_encoder_contract(model_type: String) raises -> SerenityTrainerVaeEncoderContract:
    var family = serenity_sampler_family(model_type)
    if family == SERENITY_SAMPLER_UNKNOWN:
        raise Error(String("SerenityTrainer VAE encoder contract: unsupported model_type=") + model_type)

    var quant = 64
    var scale_factor = 8
    var image_channels = 3
    var image_range_mode = SERENITY_VAE_ENCODE_INPUT_IMAGE_MINUS1_TO_1
    var cache_sample_mode = SERENITY_VAE_CACHE_SAMPLE_MEAN
    var cache_layout_mode = SERENITY_VAE_CACHE_LAYOUT_NCHW
    var cache_latent_channels = 16
    var cache_has_frame_axis = False
    var prepared_layout_mode = SERENITY_VAE_PREP_LAYOUT_NCHW
    var prepared_latent_channels = 16
    var prepared_has_frame_axis = False
    var cache_to_prepared_patch_size = 1
    var prepared_scale_mode = SERENITY_VAE_PREP_SCALE_SUB_SHIFT_MUL_SCALING
    var transformer_pack_patch_size = 0
    var transformer_token_channels = 0
    var raw_status = SERENITY_VAE_ENCODER_UNSUPPORTED
    var prepared_status = SERENITY_VAE_ENCODER_UNSUPPORTED
    var module = String("")

    if family == SERENITY_SAMPLER_SDXL:
        cache_latent_channels = 4
        prepared_latent_channels = 4
        prepared_scale_mode = SERENITY_VAE_PREP_SCALE_MUL_SCALING
        raw_status = SERENITY_VAE_ENCODER_RAW_CACHE_READY
        prepared_status = SERENITY_VAE_ENCODER_RAW_CACHE_READY
        ...
        prepared_scale_mode = SERENITY_VAE_PREP_SCALE_SUB_SHIFT_MUL_SCALING
        raw_status = SERENITY_VAE_ENCODER_RAW_CACHE_READY
        prepared_status = SERENITY_VAE_ENCODER_RAW_CACHE_READY
        module = String("serenitymojo.models.vae.ldm_encoder.load_sd3_embedded_ldm_encoder")
    elif family == SERENITY_SAMPLER_QWEN:
        cache_layout_mode = SERENITY_VAE_CACHE_LAYOUT_NCDHW_SINGLE_FRAME
        cache_has_frame_axis = True
        prepared_layout_mode = SERENITY_VAE_PREP_LAYOUT_NCDHW_SINGLE_FRAME
        prepared_has_frame_axis = True
        prepared_scale_mode = SERENITY_VAE_PREP_SCALE_QWEN_MEAN_STD
        ...
        prepared_scale_mode = SERENITY_VAE_PREP_SCALE_PATCH_BN
        ...
        transformer_pack_patch_size = 2
        transformer_token_channels = 64
    elif family == SERENITY_SAMPLER_FLUX2_DEV or family == SERENITY_SAMPLER_FLUX2_KLEIN:
        cache_latent_channels = 32
        prepared_latent_channels = 128
        cache_to_prepared_patch_size = 2
        prepared_scale_mode = SERENITY_VAE_PREP_SCALE_PATCH_BN
        ...
        prepared_status = SERENITY_VAE_ENCODER_PARTIAL_PREPARED_ONLY
    ...
    return SerenityTrainerVaeEncoderContract(
        family,
...
def validate_serenity_vae_encoder_contract(c: SerenityTrainerVaeEncoderContract) raises:
    if c.family == SERENITY_SAMPLER_UNKNOWN:
        raise Error("SerenityTrainer VAE encoder contract: unknown family")
    if c.quantization <= 0:
        raise Error("SerenityTrainer VAE encoder contract: quantization must be positive")
    if c.vae_scale_factor <= 0:
        raise Error("SerenityTrainer VAE encoder contract: vae scale factor must be positive")
    if c.image_channels != 3:
        raise Error("SerenityTrainer VAE encoder contract: expected RGB image input")
    if c.cache_sample_mode != SERENITY_VAE_CACHE_SAMPLE_MEAN:
        raise Error("SerenityTrainer VAE encoder contract: target cache must use posterior mean")
    if c.cache_latent_channels <= 0 or c.prepared_latent_channels <= 0:
        raise Error("SerenityTrainer VAE encoder contract: latent channel counts must be positive")
    if c.cache_to_prepared_patch_size <= 0:
        raise Error("SerenityTrainer VAE encoder contract: patch size must be positive")
    if (
        ...
        raise Error("SerenityTrainer VAE encoder contract: prepared channel count mismatch")
    if c.cache_layout_mode == SERENITY_VAE_CACHE_LAYOUT_NCDHW_SINGLE_FRAME and not c.cache_has_frame_axis:
        raise Error("SerenityTrainer VAE encoder contract: cache layout requires frame axis")
    if c.prepared_layout_mode == SERENITY_VAE_PREP_LAYOUT_NCDHW_SINGLE_FRAME and not c.prepared_has_frame_axis:
        raise Error("SerenityTrainer VAE encoder contract: prepared layout requires frame axis")
    if c.prepared_scale_mode == SERENITY_VAE_PREP_SCALE_PATCH_BN and c.cache_to_prepared_patch_size != 2:
        raise Error("SerenityTrainer VAE encoder contract: BN scale is defined on 2x2 patchified latents")
    if c.transformer_pack_patch_size > 1:
        if c.transformer_token_channels != (
            ...
            raise Error("SerenityTrainer VAE encoder contract: transformer packed channel mismatch")
    if c.raw_cache_encode_status == SERENITY_VAE_ENCODER_RAW_CACHE_READY and c.mojo_real_encode_module == String(""):
        raise Error("SerenityTrainer VAE encoder contract: ready raw encoder must name a module")
    if not c.decode_postprocess_is_separate:
        raise Error("SerenityTrainer VAE encoder contract: decode/postprocess must be tracked separately")


def require_raw_cache_encode_ready(c: SerenityTrainerVaeEncoderContract) raises:
    if c.raw_cache_encode_status != SERENITY_VAE_ENCODER_RAW_CACHE_READY:
        raise Error(
            c.name
...
def quantized_encoder_sample_size(
    c: SerenityTrainerVaeEncoderContract, width: Int, height: Int
) -> EncoderPixelSize:
...
def _latent_dim(pixel_dim: Int, c: SerenityTrainerVaeEncoderContract) raises -> Int:
    if pixel_dim % c.vae_scale_factor != 0:
        raise Error("SerenityTrainer VAE encoder contract: pixel dim is not VAE aligned")
    return pixel_dim // c.vae_scale_factor


def _prepared_dim(pixel_dim: Int, c: SerenityTrainerVaeEncoderContract) raises -> Int:
    var latent = _latent_dim(pixel_dim, c)
    if latent % c.cache_to_prepared_patch_size != 0:
        raise Error("SerenityTrainer VAE encoder contract: latent dim is not patch aligned")
    return latent // c.cache_to_prepared_patch_size


def raw_cache_latent_shape(
    c: SerenityTrainerVaeEncoderContract, pixel_width: Int, pixel_height: Int
...
def transformer_latent_shape(
    ...
    if ph % c.transformer_pack_patch_size != 0 or pw % c.transformer_pack_patch_size != 0:
        raise Error("SerenityTrainer VAE encoder contract: transformer pack dim mismatch")
...
def vae_encoder_contract_summary(c: SerenityTrainerVaeEncoderContract) -> String:
    return (
        c.name
        + String(" cache_ch=")
