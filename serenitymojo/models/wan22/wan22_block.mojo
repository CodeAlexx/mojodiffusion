...
def wan22_block_lora_backward[
    ...
    # ca_qn/ca_kn frozen (LoRA path) -> d_x-only backward; d_g discarded (MJ-1035 class).
    ...
    # n3 affine frozen (LoRA path) -> d_x-only backward; d_g/d_b discarded.
    ...
    # sa_qn/sa_kn frozen (LoRA path) -> d_x-only backward; d_g discarded (MJ-1035 class).
...
def wan22_block_direct_lycoris_backward[
    ...
    # ca_qn/ca_kn frozen (lycoris path) -> d_x-only backward; d_g never captured (MJ-1035 class).
    var rb_caq_dx = rms_norm_backward_dx(d_caq_rms_flat, lsv_ca_q_pre, w.ca_qn[], eps, ctx)
    var lsv_ca_k_pre = _tbf16(saved.ca_k_pre.copy(), [TXT, dim], ctx)
    var rb_cak_dx = rms_norm_backward_dx(d_cak_rms_flat, lsv_ca_k_pre, w.ca_kn[], eps, ctx)
    var d_cav_flat = reshape(csb.d_v, [TXT, dim], ctx)
    ...
    var ca_q_g = _direct_proj_bwd_device(
        direct, WDIR_CA_Q, rb_caq_dx, sv_direct_ca_n3, w.ca_wq[],
        direct_w.ca_wq, S, ctx,
    )
    var ca_k_g = _direct_proj_bwd_device(
        direct, WDIR_CA_K, rb_cak_dx, sv_direct_context, w.ca_wk[],
    ...
    var lsv_x_sa = _tbf16(saved.x_sa.copy(), [S, dim], ctx)
    # n3 affine frozen (lycoris path) -> d_x-only backward; d_g/d_b never captured.
    var lnb_n3_dx = layer_norm_backward_dx(d_n3_in, lsv_x_sa, w.n3_w[], eps, ctx)
    var d_x_sa = TArc(add(lnb_n3_dx, d_x_ca[], ctx))
    ...
    var d_q_rms_flat = reshape(d_q_rms_b, [S, dim], ctx)
    var lsv_sa_q_pre = _tbf16(saved.sa_q_pre.copy(), [S, dim], ctx)
    # sa_qn/sa_kn frozen (lycoris path) -> d_x-only backward; d_g never captured (MJ-1035 class).
    var rb_saq_dx = rms_norm_backward_dx(d_q_rms_flat, lsv_sa_q_pre, w.sa_qn[], eps, ctx)
    var d_k_rms_b = cast_tensor(d_k_rms, STDtype.BF16, ctx)
    var d_k_rms_flat = reshape(d_k_rms_b, [S, dim], ctx)
    var lsv_sa_k_pre = _tbf16(saved.sa_k_pre.copy(), [S, dim], ctx)
    ...
    var sa_q_g = _direct_proj_bwd_device(
        direct, WDIR_SA_Q, rb_saq_dx, sv_direct_sa_in, w.sa_wq[],
        direct_w.sa_wq, S, ctx,
    )
    var sa_k_g = _direct_proj_bwd_device(
        direct, WDIR_SA_K, rb_sak_dx, sv_direct_sa_in, w.sa_wk[],
