...
def load_mxfp4_expert_dequant(
    st: ShardedSafeTensors,
    weight_name: String,
    ctx: DeviceContext,
) raises -> Tensor:
    """Read an MXFP4 expert stack `<weight_name>_blocks` (U8 [E,M,G,16]) +
    `<weight_name>_scales` (U8 [E,M,G]) from a (sharded) safetensors and return
    the dequantized BF16 weight [E,M,G*32]=[E,M,K]. Sidecar suffixes `_blocks`/
    `_scales` == the HF GPT-OSS convention
    (scripts/lingbot_quantize_mxfp4_experts.py). The dequant groups the last
    (contiguous) K-dim into 32-element blocks, matching the export."""
    var b_info = st.tensor_info(weight_name + "_blocks")
    var b_bytes = st.tensor_bytes(weight_name + "_blocks")
    var b_view = from_parts(b_info.dtype, b_info.shape.copy(), b_bytes)
    var blocks = Tensor.from_view_raw(b_view, ctx)
    var s_info = st.tensor_info(weight_name + "_scales")
    var s_bytes = st.tensor_bytes(weight_name + "_scales")
    var s_view = from_parts(s_info.dtype, s_info.shape.copy(), s_bytes)
    var scales = Tensor.from_view_raw(s_view, ctx)
    return mxfp4_dequant_to_bf16(blocks^, scales^, ctx)
