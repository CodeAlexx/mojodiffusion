...
#     bf16 image of the masters (the SerenityTrainer fine-tune master/weight
...
# documented delta vs SerenityTrainer ZImageFineTuneSetup (which trains all
...
# ── device-Adafactor arm (-D ZIMAGE_FT_ADAFACTOR=1) state builders ────────────
# Placeholder ZImageFullFTOpt for the device-Adafactor arm: no masters, no
# 8-bit moments, no staging (the arm never calls zimage_full_ft_step /
# zimage_full_ft_opt_init — and so does NOT run the adam8bit equivalence gate,
# which stays wired unchanged on the default arm).
def zimage_full_ft_opt_empty(ctx: DeviceContext) raises -> ZImageFullFTOpt:
    return ZImageFullFTOpt(
        List[List[Float32]](), List[Adam8bitState](),
        List[Float32](), List[Float32](),
        List[Int](), 0,
        ctx.enqueue_create_host_buffer[DType.uint8](2),
    )


# One AdafactorDeviceState per trained matrix (num_main*ZIMAGE_SLOTS slots;
# rows/cols from the RESIDENT weight shapes — the same inventory the host
# masters were built from). Factored F32 row+col vars only: ~20MB device.
def zimage_full_ft_afd_states_init(
    main_blocks: List[ZImageBlockWeights], D: Int, F: Int, ctx: DeviceContext,
    surf: Bool,
    main_mod_w: List[TArc],
    main_mod_b: List[TArc],
    ...
    for bi in range(num_main):
        for s in range(ZIMAGE_SLOTS):
            var w_t = zimage_fullft_slot_tensor(main_blocks[bi], s)
            if w_t[].dtype() != STDtype.BF16:
                raise Error("fullft afd init: base weight must be BF16-resident")
            var sh = w_t[].shape()
            if len(sh) != 2:
                raise Error("fullft afd init: trained weight is not 2-D")
            if sh[0] * sh[1] != slot_numels[s]:
                raise Error("fullft afd init: slot numel mismatch")
            states.append(AdafactorDeviceState(sh[0], sh[1], ctx))
            state_elems += sh[0] + sh[1]
    var n_factored = len(states)
    var n_unfactored = 0
    # Phase B full surface: 6 rms scales/block (1D unfactored) + per-block
    # adaLN_modulation.0 weight (2D factored) + bias (1D unfactored). Order MUST
    # match the driver's index layout: [210,390) norms bi-major j-minor,
    # [390,420) adaLN weights, [420,450) adaLN biases.
    if surf:
        if len(main_mod_w) != num_main or len(main_mod_b) != num_main:
            raise Error("fullft afd init SURF: main_mod_w/b length != num_main")
        for bi in range(num_main):
            ref wb = main_blocks[bi]
            # n1, q_norm, k_norm, n2, fn1, fn2 — resident BF16 1D scales
            var norms = List[TArc]()
            norms.append(wb.n1.copy()); norms.append(wb.q_norm.copy())
            norms.append(wb.k_norm.copy())
            norms.append(wb.n2.copy()); norms.append(wb.fn1.copy())
            norms.append(wb.fn2.copy())
            for j in range(6):
                if norms[j][].dtype() != STDtype.BF16:
                    raise Error("fullft afd init SURF: norm scale must be BF16")
                var n = norms[j][].numel()
                states.append(AdafactorDeviceState(n, ctx))
                state_elems += n
                n_unfactored += 1
        for bi in range(num_main):
            var wsh = main_mod_w[bi][].shape()
            if len(wsh) != 2:
                raise Error("fullft afd init SURF: adaLN weight not 2-D")
            states.append(AdafactorDeviceState(wsh[0], wsh[1], ctx))
            state_elems += wsh[0] + wsh[1]
            n_factored += 1
        for bi in range(num_main):
            var nb = main_mod_b[bi][].numel()
            states.append(AdafactorDeviceState(nb, ctx))
            state_elems += nb
            n_unfactored += 1
    print(
        "[fullft-afd] adafactor states: ", len(states), " (",
        n_factored, " factored 2D / ", n_unfactored,
        " unfactored 1D), ",
        Float32(Float64(state_elems) * 4.0 / 1.0e6),
        " MB F32 states (device-resident)",
    )
    return states^
...
# checkpoint layout (header JSON + per-key data offsets, sorted-name order) —
# shared by the host-master and device-resident save arms.
@fieldwise_init
struct _FullFTCkptLayout(Movable):
    var header: String
    var offsets: List[Int]
    var total_data: Int


def _fullft_ckpt_layout(
    st: ShardedSafeTensors, names: List[String]
) raises -> _FullFTCkptLayout:
    var header = String("{")
    var offsets = List[Int]()
    var cursor = 0
    for i in range(len(names)):
        ...
        cursor += nbytes
    header += String("}")
    return _FullFTCkptLayout(header^, offsets^, cursor)


# open out_dir/diffusion_pytorch_model.safetensors and write the u64 LE header
# length + header JSON; returns the fd (data_base = 8 + header.byte_length()).
def _fullft_open_ckpt_fd(out_dir: String, header: String) raises -> Int:
...
def _fullft_write_index_and_config(
    src_dir: String, out_dir: String, names: List[String], total_data: Int
) raises:
...
def zimage_full_ft_save_checkpoint(
    ...
    var layout = _fullft_ckpt_layout(st, names)
    var header = layout.header.copy()
    var offsets = layout.offsets.copy()
    var total_data = layout.total_data
    var out_path = out_dir + String("/diffusion_pytorch_model.safetensors")
    var fd = _fullft_open_ckpt_fd(out_dir, header)
    var data_base = 8 + header.byte_length()
    ...
    _fullft_write_index_and_config(src_dir, out_dir, names, total_data)
...
# Device-Adafactor arm (-D ZIMAGE_FT_ADAFACTOR=1) save: there are NO host
# masters — the RESIDENT device bf16 weights ARE the live model (the fused
# adafactor+SR step mutates them in place). Same output contract as
# zimage_full_ft_save_checkpoint (ALL source keys; trained keys = D2H of the
# resident tensors through pinned host staging — 12.31GB at save frequency is
# fine; frozen keys byte-copied from the source shards) + the same delta print.
# RESUME from this checkpoint is fail-loud at the trainer (start_step != 0
# raises): the adafactor row/col-var sidecar is the fleet-standard TODO.
# The trained-key resident map: matmul slots always, + (SURF) the 6 rms scales
# and per-block adaLN_modulation.0 weight/bias, all read from the RESIDENT device
# tensors so the checkpoint carries the trained values (resume reloads them).
def _zimage_afd_trained_resident_map(
    main_blocks: List[ZImageBlockWeights],
    surf: Bool, main_mod_w: List[TArc], main_mod_b: List[TArc],
) raises -> Dict[String, TArc]:
    var num_main = len(main_blocks)
    var m = Dict[String, TArc]()
    for bi in range(num_main):
        for s in range(ZIMAGE_SLOTS):
            m[zimage_fullft_slot_key(bi, s)] = zimage_fullft_slot_tensor(
                main_blocks[bi], s)
        if surf:
            var p = String("layers.") + String(bi)
            m[p + String(".attention_norm1.weight")] = main_blocks[bi].n1.copy()
            m[p + String(".attention.norm_q.weight")] = main_blocks[bi].q_norm.copy()
            m[p + String(".attention.norm_k.weight")] = main_blocks[bi].k_norm.copy()
            m[p + String(".attention_norm2.weight")] = main_blocks[bi].n2.copy()
            m[p + String(".ffn_norm1.weight")] = main_blocks[bi].fn1.copy()
            m[p + String(".ffn_norm2.weight")] = main_blocks[bi].fn2.copy()
            m[p + String(".adaLN_modulation.0.weight")] = main_mod_w[bi].copy()
            m[p + String(".adaLN_modulation.0.bias")] = main_mod_b[bi].copy()
    return m^


def zimage_full_ft_save_checkpoint_device(
    src_dir: String, out_dir: String,
    ...
    var trained = _zimage_afd_trained_resident_map(
        main_blocks, surf, main_mod_w, main_mod_b)
    var expected_trained = len(trained)
    var n_trained_found = 0
    ...
    # delta stats vs source (trained keys only) — whole-tensor bf16 diff (the
    # SR statistic; never sample a handful of values).
    var delta_l1 = 0.0
    var delta_max = Float32(0.0)
    var delta_nonzero = 0
    var trained_elems = 0

    # staging sized to the largest trained tensor. The matmul slots (F*D) far
    # exceed any Phase-B surface tensor (adaLN weight 4D*t_dim, norms <= D), so
    # the max matmul slot bounds the staging buffer.
    var slot_numels = zimage_fullft_slot_numels(D, F)
    var max_n = 0
    for s in range(ZIMAGE_SLOTS):
        if slot_numels[s] > max_n:
            max_n = slot_numels[s]
    var stage = ctx.enqueue_create_host_buffer[DType.uint8](max_n * 2)
    ...
    for i in range(len(names)):
        ...
        if name in trained:
            n_trained_found += 1
            var w_t = trained[name]
            if w_t[].dtype() != STDtype.BF16 or w_t[].nbytes() != nbytes:
                raise Error(String("fullft save: resident tensor mismatch: ") + name)
            var n = w_t[].numel()
            var dst = stage.create_sub_buffer[DType.uint8](0, nbytes)
            ctx.enqueue_copy(dst_buf=dst, src_buf=w_t[].buf)   # D2H
            ctx.synchronize()
            var sp = stage.unsafe_ptr().bitcast[BFloat16]()
            var op = src.unsafe_ptr().bitcast[BFloat16]()
            for e in range(n):
                var d = sp[e].cast[DType.float32]() - op[e].cast[DType.float32]()
                if d < Float32(0.0):
                    d = -d
                delta_l1 += Float64(d)
                if d > delta_max:
                ...
                if d > Float32(0.0):
                    delta_nonzero += 1
            trained_elems += n
            _pwrite_all(fd, Int(stage.unsafe_ptr()), nbytes, data_base + offsets[i])
        else:
            _pwrite_all(
                fd, Int(src.unsafe_ptr()), nbytes, data_base + offsets[i]
            )
    _ = sys_close(fd)
    if n_trained_found != expected_trained:
        raise Error("fullft save: not every trained key found in source schema")
    ...
    var secs = Float64(perf_counter_ns() - t0) / 1.0e9
    print(
        "[fullft-afd] checkpoint saved (from RESIDENT device weights): ",
        out_path, " (", len(names), " keys, ", total_data, " data bytes, ",
        Float32(secs), " s)",
    )
    print(
        "[fullft-afd] trained-weight delta vs source: nonzero_elems=",
        delta_nonzero, " /", trained_elems, " |Δ|_1=", Float32(delta_l1),
        " max|Δ|=", delta_max,
    )
