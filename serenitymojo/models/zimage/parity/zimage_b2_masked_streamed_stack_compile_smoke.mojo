# ZImage streamed B2 masked stack compile smoke.
#
# Evidence level: compile/runtime wiring smoke for the streamed non-graph masked
# B2 stack APIs. Uses zero transformer blocks and tiny tensors; not model parity,
# not SerenityTrainer replay, and not adapter-gradient parity.
#
# Run:
...
def _check(cond: Bool, msg: String) raises:
    if not cond:
        raise Error(String("zimage_b2_masked_streamed_stack_compile_smoke FAILED: ") + msg)
...
def _empty_opt_state(ctx: DeviceContext) raises -> LoraAdamWPlainDeviceState:
    var seg_len = List[Int]()
    return LoraAdamWPlainDeviceState(
        ctx.enqueue_create_buffer[DType.uint8](1),
        ctx.enqueue_create_buffer[DType.uint8](1),
        ctx.enqueue_create_buffer[DType.uint8](1),
        ctx.enqueue_create_buffer[DType.uint8](1),
        ctx.enqueue_create_host_buffer[DType.uint8](1),
        ctx.enqueue_create_host_buffer[DType.uint8](1),
        ctx.enqueue_create_host_buffer[DType.uint8](1),
        seg_len^,
        0,
...
def main() raises:
    ...
    var fwd = zimage_stack_lora_forward_main_device_b2_masked_streamed[H, Dh, N_IMG, N_TXT, S](
        st,
    ...
    var grads = zimage_stack_lora_backward_main_device_b2_masked_streamed[H, Dh, N_IMG, N_TXT, S](
        st,
    ...
    var fwd2 = zimage_stack_lora_forward_main_device_b2_masked_streamed[H, Dh, N_IMG, N_TXT, S](
        st,
        _fill(N_IMG * D, Float32(0.03), Float32(0.1)),
        _fill(N_TXT * D, Float32(0.05), Float32(0.2)),
        _fill(N_IMG * D, Float32(0.07), Float32(0.3)),
    ...
    var opt_state = _empty_opt_state(ctx)
    ...
    _check(wrote.grad_count == 0, "zero-block device grad count")
    _check(wrote.streaming_sync_count == 0, "zero-block device grad sync count")
    print(
