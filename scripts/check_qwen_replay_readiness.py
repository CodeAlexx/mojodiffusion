#!/usr/bin/env python3
"""Qwen-Image SerenityTrainer one-step replay readiness guard.

This is intentionally bounded: it inspects local SerenityTrainer dump artifacts and
...
PARITY = Path("/home/alex/serenity_trainer-mojo/parity")
...
MOJO_GATES: tuple[tuple[Path, str, str], ...] = (
    (
        ...
        "artifact-header",
        "opens qwen_train_ref_step000 and adapter dumps and validates the SerenityTrainer tensor contract",
    ),
    (
        REPO / "trainer/smoke/qwen_train_control_wiring_smoke.mojo",
        "source-only",
    ...
    (
        ...
        "block-oracle",
        "consumes local block oracle .bin files, not the SerenityTrainer one-step dump",
    ),
    (
    ...
    (
        ...
        "save-resume",
        "exercises synthetic LoRA save/resume state, not the SerenityTrainer dump",
    ),
    (
        REPO / "serenitymojo/models/qwenimage/parity/qwen_lora_serenity_save_key_smoke.mojo",
        "save-key",
        "emits reduced SerenityTrainer-format LoRA keys, not the one-step replay dump",
    ),
)
...
NEGATIVE_CLAIM_MARKERS = (
    ...
    "does not accept",
    "evidence only",
    "reference timing evidence only",
)

QWEN_TRAIN_SPEED_EVIDENCE_GROUPS = (
    (
        "SerenityTrainer profile artifact",
        ("qwen_train_profile_meta.json", "serenity_trainer train timing", "reference train timing"),
    ),
    ("SerenityTrainer predict timing", ("predict_seconds", "serenity_predict_seconds")),
    ("SerenityTrainer loss timing", ("loss_seconds", "serenity_loss_seconds")),
    ("SerenityTrainer backward timing", ("backward_seconds", "serenity_backward_seconds")),
    ("SerenityTrainer optimizer timing", ("optimizer_update_seconds", "serenity_optimizer_update_seconds")),
    ...
    (
        "SerenityTrainer train peak VRAM",
        ("serenity_train_peak_vram_mib", "serenity_trainer train peak vram", "serenity_trainer peak vram"),
    ),
    (
...
TRAIN_SPEED_CLAIM_SCAN_FILES: tuple[Path, ...] = (
    REPO / "SERENITY_MOJO_PORT_REMAINING.md",
    REPO / "trainer/src/serenity_trainer/trainer/train_qwenimage_real.mojo",
    REPO / "trainer/smoke/qwen_train_control_wiring_smoke.mojo",
    REPO / "serenitymojo/training/serenity_trainer_product_run.mojo",
    REPO / "serenitymojo/training/serenity_trainer_product_run_smoke.mojo",
    REPO / "serenitymojo/models/qwenimage/parity/qwen_train_ref_artifact_smoke.mojo",
    REPO / "serenitymojo/models/qwenimage/parity/qwenimage_real_smoke.mojo",
)
...
def require_contract(contract: dict, meta: dict, profile: dict) -> None:
    if Path(contract.get("reference_repo", "")) != SERENITY_ROOT:
        die(f"contract reference_repo is not {SERENITY_ROOT}: {contract.get('reference_repo')!r}")
    if meta.get("serenity_trainer") != str(SERENITY_ROOT):
        die(f"metadata serenity_trainer path is not {SERENITY_ROOT}: {meta.get('serenity_trainer')!r}")
    if profile.get("serenity_trainer") != str(SERENITY_ROOT):
        die(f"profile metadata serenity_trainer path is not {SERENITY_ROOT}: {profile.get('serenity_trainer')!r}")
    ...
    print(
        "[qwen-replay-readiness] PASS SerenityTrainer baseline "
...
def check_adapter_dump(header: dict) -> None:
    ...
    if dtypes != Counter({"F32": EXPECTED_ADAPTER_KEYS}):
        die(f"adapter dtypes {dict(dtypes)}, expected all F32 from reference trainer lora_weight_dtype")
...
def check_meta(meta: dict, profile: dict) -> None:
    ...
    print(
        "[qwen-replay-readiness] PASS SerenityTrainer loss scalar "
    ...
    print(
        "[qwen-replay-readiness] INFO this bucket proves the dumped reference trainer scalar "
...
def check_update_bearing_evidence(meta: dict) -> None:
    ...
    if has_candidate:
        ...
        print(
            "[qwen-replay-readiness] BLOCKED missing later SerenityTrainer step with "
...
def check_train_speed_evidence(profile: dict) -> None:
    ...
    print(
        "[qwen-replay-readiness] PASS SerenityTrainer train timing evidence only "
    ...
    print(
        ...
        "SerenityTrainer/Mojo run identity: batch=2 dtype=BFLOAT_16 optimizer=ADAMW "
