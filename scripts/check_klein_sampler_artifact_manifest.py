...
the required SerenityTrainer/Mojo sampler artifact bundle explicit enough that a
...
DEFAULT_MANIFEST = Path("/home/alex/serenity_trainer-mojo/parity/klein_sampler_parity_manifest.json")
SERENITY_TRAINER_PRODUCER = "scripts/run_klein_serenity_trainer_sampler_parity.py"
...
SERENITY_TRAINER_PRODUCER_ARTIFACT_KEYS = (
    "serenity_trainer_initial_noise_raw_nchw",
    "serenity_trainer_initial_noise_post_patch_nchw",
    "serenity_trainer_initial_noise_post_pack",
    "serenity_trainer_latent_trajectory",
    "serenity_trainer_final_packed_latent",
    "serenity_trainer_final_unpacked_latent",
    "serenity_trainer_final_unscaled_unpatchified_latent",
...
def check_metric_pair(metrics: dict[str, Any], key: str) -> Check:
    ...
    if not finite_number(serenity_value) or not finite_number(mojo_value):
        return Check(False, f"metric {key}", "serenity_trainer and mojo values must be finite numeric values")
...
def required_manifest_fields() -> list[dict[str, Any]]:
    return [
        ...
        },
        {
            ...
            "fields": [
                {
                    "field": f"metrics.serenity_trainer.{key}",
...
def required_artifacts() -> list[dict[str, Any]]:
    return [
        {
            "group": "serenity_trainer_seed_replay_inputs",
            "artifacts": [
                {
                    "name": "serenity_trainer_initial_noise_raw_nchw",
                    "dtype": "F32",
                    "shape": "[1, 32, prompt.height / 8, prompt.width / 8]",
                },
                {
                    "name": "serenity_trainer_initial_noise_post_patch_nchw",
                    "dtype": "F32",
                    "shape": "[1, 128, prompt.height / 16, prompt.width / 16]",
                },
                {
                    "name": "serenity_trainer_initial_noise_post_pack",
                    "dtype": "F32",
        ...
        },
        {
            ...
            "pairs": [
                {
                    "serenity_trainer": "serenity_trainer_latent_trajectory",
                    "mojo": "mojo_latent_trajectory",
                    "shape": "[prompt.steps + 1, tokens, 128] or [prompt.steps + 1, 1, 128, h16, w16]",
                },
                {
                    "serenity_trainer": "serenity_trainer_final_packed_latent",
                    "mojo": "mojo_final_packed_latent",
                    "shape": "[1, 128, h16, w16] or [tokens, 128]",
                },
                {
                    "serenity_trainer": "serenity_trainer_final_unpacked_latent",
                    "mojo": "mojo_final_unpacked_latent",
                    "shape": "[1, 128, h16, w16]",
                },
                {
                    "serenity_trainer": "serenity_trainer_final_unscaled_unpatchified_latent",
        ...
        },
        {
            ...
            "pairs": [
                {
                    "serenity_trainer": "serenity_trainer_vae_decoded_tensor",
                    "mojo": "mojo_vae_decoded_tensor",
                    "shape": "[1, 3, prompt.height, prompt.width]",
                },
                {
                    "serenity_trainer": "serenity_trainer_png",
                    "mojo": "mojo_png",
    ...
    return [
        {
            ...
            "required": "same SerenityTrainer/Mojo sample configuration",
            "fields": [
                "model_type",
                "prompt.id",
                "prompt.positive",
        ...
        },
        {
            ...
            "required": "positive numeric SerenityTrainer and Mojo values",
            "fields": [
...
def current_split_validation_evidence() -> dict[str, Any]:
    ...
    return {
        ...
        "note": (
            ...
            "They do not replace the paired SerenityTrainer/Mojo sampler manifest, "
        ...
        "remaining_sampler_parity_requirements": [
            "real paired SerenityTrainer/Mojo sampler manifest",
            "shared prompt, seed, resolution, steps, cfg, scheduler, and dtype identity",
            "SerenityTrainer-equivalent raw/post-patch/post-pack initial-noise artifacts",
...
def check_current_split_validation_evidence(data: dict[str, Any]) -> list[Check]:
    ...
    checks.append(
        Check(
            ...
            and any("paired SerenityTrainer/Mojo sampler manifest" in str(item) for item in requirements),
            "current_split_validation_evidence.remaining_sampler_parity_requirements",
            "must retain real paired reference trainer/Mojo manifest blocker",
        )
    )
    return checks


def is_serenity_trainer_producer_manifest(data: dict[str, Any]) -> bool:
    producer = data.get("producer")
    scope = data.get("scope")
    return producer == SERENITY_TRAINER_PRODUCER or (
        isinstance(scope, str) and "SerenityTrainer-only Klein/Flux2 sampler artifact producer" in scope
    )


def check_serenity_trainer_producer_fragment(data: dict[str, Any]) -> list[Check]:
    """Validate the reference trainer-only fragment without promoting it to paired parity."""
    if not is_serenity_trainer_producer_manifest(data):
        return []

    checks: list[Check] = [
        Check(
            True,
            "serenity_trainer producer fragment",
            "SerenityTrainer-only producer output detected; validating fragment readiness and non-parity flags.",
        )
    ]
    ...
    for key in ("accepted", "parity_claimed", "sampler_parity_accepted", "mojo_comparison_present"):
        checks.append(
            Check(
                ...
                f"serenity_trainer producer flag {key}",
    ...
    if not isinstance(artifacts, dict):
        checks.append(Check(False, "serenity_trainer producer artifacts", "artifacts must be an object"))
    else:
        missing = [key for key in SERENITY_TRAINER_PRODUCER_ARTIFACT_KEYS if key not in artifacts]
        checks.append(
            Check(
                not missing,
                "serenity_trainer producer artifacts.present",
                f"missing={missing}",
            )
        )
        ...
        checks.append(
            Check(
                not mojo_present,
                "serenity_trainer producer artifacts.mojo_absent",
                f"mojo_artifacts_present={mojo_present}; producer output must stay reference trainer-only",
            )
        )
        for key in SERENITY_TRAINER_PRODUCER_ARTIFACT_KEYS:
            value = artifacts.get(key)
            if not isinstance(value, dict):
                checks.append(Check(False, f"serenity_trainer producer artifact {key}", "must be an object"))
                continue
            path = value.get("path")
            checks.append(
                Check(
                    isinstance(path, str) and bool(path),
                    f"serenity_trainer producer artifact {key}.path",
                    str(path),
                )
            )
            if key.endswith("_png"):
                checks.append(
                    Check(
                        ...
                        f"serenity_trainer producer artifact {key}.dimensions",
                ...
                checks.append(
                    Check(
                        value.get("dtype") == "F32",
                        f"serenity_trainer producer artifact {key}.dtype",
                ...
                checks.append(
                    Check(
                        ...
                        f"serenity_trainer producer artifact {key}.shape",
                        str(shape),
                    )
                )
    ...
    if not isinstance(groups, dict):
        checks.append(Check(False, "serenity_trainer producer artifact_groups", "artifact_groups must be an object"))
    else:
        ...
        checks.append(
            Check(
                not missing,
                "serenity_trainer producer artifact_groups.missing_for_sampler_parity",
                f"missing={missing}",
            )
        )
    ...
    if not isinstance(metrics, dict):
        checks.append(Check(False, "serenity_trainer producer metrics", "metrics must be an object"))
    else:
        serenity_trainer = metrics.get("serenity_trainer")
        mojo = metrics.get("mojo")
        for namespace, values in (("serenity_trainer", serenity_trainer), ("mojo", mojo)):
            if not isinstance(values, dict):
                checks.append(Check(False, f"serenity_trainer producer metrics.{namespace}", "must be an object"))
                continue
            for key in METRIC_KEYS:
                value = values.get(key)
                if namespace == "serenity_trainer":
                    checks.append(
                        Check(
                            finite_number(value) and float(value) > 0.0,
                            f"serenity_trainer producer metric {namespace}.{key}",
                            f"value={value!r}",
                        )
                    )
                else:
    ...
    if not isinstance(comparisons, dict):
        checks.append(Check(False, "serenity_trainer producer comparisons", "comparisons must be an object"))
    else:
        for key in COMPARISON_KEYS:
            ...
            checks.append(
                Check(
                    ...
                    f"serenity_trainer producer comparison {key}",
                    str(value),
                )
            )
...
def build_template_manifest(
    *,
    ...
    artifacts = {
        ...
        "serenity_trainer_initial_noise_post_patch_nchw": _template_tensor_artifact(
            artifact_root,
            "serenity_trainer_initial_noise_post_patch_nchw",
            unpacked_shape,
            dtype="F32",
        ),
        "serenity_trainer_initial_noise_post_pack": _template_tensor_artifact(
            artifact_root,
            "serenity_trainer_initial_noise_post_pack",
            packed_shape,
            dtype="F32",
        ...
        "serenity_trainer_final_packed_latent": _template_tensor_artifact(
            artifact_root,
            "serenity_trainer_final_packed_latent",
            packed_shape,
            dtype="F32",
        ...
        "serenity_trainer_final_unpacked_latent": _template_tensor_artifact(
            artifact_root,
            "serenity_trainer_final_unpacked_latent",
            unpacked_shape,
            dtype="F32",
        ...
        "serenity_trainer_final_unscaled_unpatchified_latent": _template_tensor_artifact(
            artifact_root,
            "serenity_trainer_final_unscaled_unpatchified_latent",
            unscaled_shape,
            dtype="F32",
        ...
        "serenity_trainer_vae_decoded_tensor": _template_tensor_artifact(
            artifact_root,
            "serenity_trainer_vae_decoded_tensor",
            decoded_shape,
            dtype="F32",
        ...
        "serenity_trainer_png": _template_png_artifact(artifact_root, "serenity_trainer_png", width, height),
    ...
    return {
        ...
        "artifacts": artifacts,
        "metrics": {
            "serenity_trainer": {key: 0.0 for key in METRIC_KEYS},
...
def inspect_manifest(path: Path) -> list[Check]:
    if not path.exists():
        return [
            Check(
                False,
                "manifest",
                f"missing {path}; expected paired reference trainer/Mojo sampler evidence manifest",
            )
        ]
    ...
    checks.extend(check_serenity_trainer_producer_fragment(data))
...
def is_producer_fragment_check(check: Check) -> bool:
    return check.label.startswith("serenity_trainer producer")
...
def build_readiness_report(path: Path, checks: list[Check]) -> dict[str, Any]:
    ...
    return {
        ...
        "serenity_trainer_producer_fragment_ready": bool(producer_checks) and all(check.ok for check in producer_checks),
        "parity_claimed": False,
        ...
        "required_paired_sampler_fields": required_paired_sampler_fields(),
        "required_paired_sampler_artifacts": required_artifacts(),
        "serenity_trainer_producer_fragment_checks": [check_as_json(check) for check in producer_checks],
