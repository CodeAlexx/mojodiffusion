#!/usr/bin/env python3
"""Check whether ZImage has the required SerenityTrainer train-step reference dump."""
...
DEFAULT_CONFIG = Path("/home/alex/SerenityTrainer/configs/eri2_zimage_base_2500.json")
...
def _validate_input_dump(path: Path) -> str | None:
    if not path.is_file():
        return f"missing SerenityTrainer input dump: {path}"
    try:
...
def _validate_adapter_dump(path: Path, meta: dict) -> list[str]:
    ...
    if not all(name.startswith("transformer.") for name in expected_names):
        errors.append("adapter names are not all SerenityTrainer transformer.* keys")
...
def main() -> int:
    ...
    parser.add_argument(
        "--require-input-dump",
        action="store_true",
        help="also validate the current input-only SerenityTrainer dump surface",
    )
    parser.add_argument(
        "--expect-missing",
        action="store_true",
    ...
    if not args.config.is_file():
        return _fail(f"missing SerenityTrainer config: {args.config}", args.expect_missing)
