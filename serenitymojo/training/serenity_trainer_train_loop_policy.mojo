# serenity_trainer_train_loop_policy.mojo
#
# Shared SerenityTrainer policy helpers for real model train loops. Model files still
...
comptime SERENITY_GRAD_POLICY_ON_ONLY = 0
comptime SERENITY_GRAD_POLICY_ON_OR_CPU_OFFLOADED = 1
comptime SERENITY_GRAD_POLICY_ON_OR_OFF = 2


def serenity_full_finetune_unsupported_message(
    trainer_name: String,
) -> String:
    return (
        trainer_name
        + String(" received training_method=FINE_TUNE/full_finetune. ")
        + String("SerenityTrainer registers full finetune for this model, but the ")
        ...
        + String("SerenityTrainer parity artifacts.")
    )


def validate_serenity_lora_only_or_fail_full_finetune(
    cfg: TrainConfig, trainer_name: String,
) raises:
    cfg.validate_training_method_config()
    if cfg.training_method == TRAINING_METHOD_FINE_TUNE:
        raise Error(serenity_full_finetune_unsupported_message(trainer_name))
...
def validate_serenity_lora_adamw_loop_policy(
    cfg: TrainConfig, trainer_name: String,
) raises:
    cfg.validate_serenity_trainer_policy_config()
    validate_serenity_lora_only_or_fail_full_finetune(cfg, trainer_name)
...
def validate_serenity_train_math_policy(
    cfg: TrainConfig, trainer_name: String,
) raises:
    """Validate shared train math before expensive model setup.

    This binds parsed SerenityTrainer LR/AdamW/masked-loss fields to the product loop
    policy. Unsupported masked/prior modes fail loud for now instead of being
    silently ignored by a model-specific loss path.
    """
    validate_serenity_lora_adamw_loop_policy(cfg, trainer_name)
    ...
    if cfg.masked_training:
        raise Error(
            trainer_name
            + String(" does not implement SerenityTrainer masked_training loss yet")
        )
...
def serenity_lr_for_optimizer_step(cfg: TrainConfig, optimizer_step: Int) -> Float32:
    return lr_for_step(
        cfg.lr,
        optimizer_step,
        cfg.lr_warmup_steps,
...
def validate_serenity_gradient_checkpointing_policy(
    cfg: TrainConfig, trainer_name: String, policy: Int,
) raises:
    cfg.validate_offload_checkpoint_config()
    if policy == SERENITY_GRAD_POLICY_ON_ONLY:
        ...
        return
    if policy == SERENITY_GRAD_POLICY_ON_OR_CPU_OFFLOADED:
        if (
        ...
        return
    if policy == SERENITY_GRAD_POLICY_ON_OR_OFF:
    ...
    raise Error(
        trainer_name
        + String(": unknown SerenityTrainer gradient checkpoint policy ")
        + String(policy)
    )


def serenity_cache_dir_from_train_config(cfg: TrainConfig, default_cache_dir: String) -> String:
    if cfg.dataset_cache_dir != String(""):
        return cfg.dataset_cache_dir.copy()
    return default_cache_dir.copy()


def serenity_stage_dir_from_train_config(cfg: TrainConfig, default_cache_dir: String) raises -> String:
    ...
    var cache_dir = serenity_cache_dir_from_train_config(cfg, default_cache_dir)
    if cache_dir == String(""):
        raise Error("serenity_stage_dir_from_train_config: config has no cache_dir/dataset_cache_dir")
    return cache_dir + String("_stage")


def serenity_dataset_path_from_train_config(cfg: TrainConfig) raises -> String:
...
def serenity_output_lora_path_from_train_config(
...
def serenity_output_lora_path_for_stream_from_train_config(
    cfg: TrainConfig,
...
def serenity_fixed_output_lora_path_from_train_config(
...
def serenity_state_path_for_lora(lora_path: String) -> String:
    return lora_path + String(".state.safetensors")


def serenity_step_lora_path(base_path: String, step: Int) -> String:
    var suffix = String(".safetensors")
...
def serenity_final_or_step_lora_path(
    base_path: String,
    default_lora_dir: String,
...
def serenity_sample_cadence_from_train_config(
...
def serenity_sampling_enabled(cadence: SampleCadence) -> Bool:
    return cadence.sample_after_unit != SAMPLE_UNIT_NEVER


def serenity_sampling_enabled_checked(cadence: SampleCadence) raises -> Bool:
    validate_step_sample_cadence(cadence)
    return serenity_sampling_enabled(cadence)


def serenity_should_save_checkpoint(cfg: TrainConfig, completed_step: Int) -> Bool:
    return completed_step > 0 and cfg.save_every > 0 and completed_step % cfg.save_every == 0


def serenity_should_save_before_sample(
