# offload_checkpoint_config_smoke.mojo — SerenityTrainer offload/checkpoint config
...
def _gate_defaults() raises:
    print("--- defaults mirror SerenityTrainer scalar policy ---")
    var c = _read_json(String("/tmp/serenity_offload_defaults.json"), String(""))
...
def _gate_cpu_offloaded() raises:
    print("--- CPU_OFFLOADED maps to active SerenityTrainer offload policy ---")
    var tail = String(",")
    ...
    var c = _read_json(String("/tmp/serenity_offload_cpu.json"), tail)
...
def _gate_legacy_bool() raises:
    print("--- legacy bool migration matches SerenityTrainer True->ON / False->OFF ---")
    var on_cfg = _read_json(
        String("/tmp/serenity_offload_bool_on.json"),
        String(',"gradient_checkpointing":true'),
    )
    var off_cfg = _read_json(
        String("/tmp/serenity_offload_bool_off.json"),
...
def _gate_invalids() raises:
    ...
    _expect_reader_raises(
        String("/tmp/serenity_offload_bad_enum.json"),
        String(',"gradient_checkpointing":"GPU_MAGIC"'),
    )
    _expect_reader_raises(
        String("/tmp/serenity_offload_bad_fraction.json"),
        String(',"layer_offload_fraction":1.25'),
    )


def main() raises:
    print("=== SerenityTrainer offload/checkpoint config smoke ===")
    _gate_defaults()
    _gate_cpu_offloaded()
    _gate_legacy_bool()
    _gate_invalids()
