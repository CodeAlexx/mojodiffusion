...
# The 1024 VAE decode runs in a fresh DeviceContext and uses tiled decode so the
# UNet/embedding phase cannot keep allocator residency alive across decode.
#
...
def main() raises:
    ...
    # --- Staged denoise: final latent returns on host, model residency drops. ---
    var latent_h = denoise_cached(denoiser_path, emb_path)

    # --- Fresh VAE stage: re-upload latent, trim best-effort, tiled decode + PNG. ---
    var ctx = DeviceContext()
    ctx.synchronize()
