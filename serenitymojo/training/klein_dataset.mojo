...
# ── shared cache-read primitives (audit item 8, 2026-07-07) ───────────────────
# The ONE implementation of the per-sample cache reads that the klein/zimage/l2p
# reader (KleinCache/L2PCache below) AND the ernie/anima product trainers now all
# route through. Extracted VERBATIM from KleinCache._load_tensor and the Klein
# file-enumeration so every existing cache stays byte-identical; the trainers'
# former inline _read_cache_tensor / _cache_tensor / _list_cache / _cache_dims /
# _cache_has now call these directly. This is the layer the cache-schema gate
# (training/parity/cache_schema_gate.mojo) guards against the silent-zero-
# conditioning bug class — a missing key must FAIL LOUD, never zero-fill.


def load_cache_tensor(st: SafeTensors, name: String, ctx: DeviceContext) raises -> Tensor:
    """Read one safetensors tensor into a device Tensor at its STORED dtype (no
    cast; the caller stages/casts). The single shared per-sample cache read."""
...
def list_sorted_safetensors(dir: String) raises -> List[String]:
    """Enumerate `.safetensors` in `dir` -> absolute paths, insertion-sorted for a
    reproducible order (mirrors LatentDataset::new / the Rust cache_files.sort()).
    Does NOT raise on empty: each caller keeps its own empty-cache message."""
    var raw = listdir(dir)
    var fs = List[String]()
...
def cache_has_key(st: SafeTensors, name: String) -> Bool:
    """True iff `name` is one of the tensors in this cache sample (header scan)."""
    var ns = st.names()
...
def cache_tensor_dims(st: SafeTensors, name: String) raises -> List[Int]:
    """Header-only shape of `name` as List[Int] (no tensor-data copy)."""
    var info = st.tensor_info(name)
    var out = List[Int]()
...
def _load_tensor(st: SafeTensors, name: String, ctx: DeviceContext) raises -> Tensor:
    # Thin call-through to the shared primitive (kept so KleinCache/L2PCache
    # internal call sites stay textually unchanged).
    return load_cache_tensor(st, name, ctx)
...
struct KleinCache(Movable):
    ...
    def __init__(out self, dir: String) raises:
        ...
        var fs = list_sorted_safetensors(dir)
        if len(fs) == 0:
    ...
    def load_control_batch(
        self, indices: List[Int], ctx: DeviceContext
    ) raises -> Tensor:
        """T2.E / image-EDIT: load the control (reference) latents for `indices`
        and stack along dim 0 -> [B,128,H,W]. Mirrors load_batch but for the
        additive CONTROL_KEY only; the non-control load/load_batch path is
        untouched. Every index MUST carry a control_latent (load_control raises
        loud otherwise) and the caller passes SAME-bucket indices (identical
        latent shapes), matching the bucket sampler contract."""
        if len(indices) == 0:
            raise Error("load_control_batch: empty indices")
        if len(indices) == 1:
            return self.load_control(indices[0], ctx)
        var ctrls = List[ArcPointer[Tensor]]()
        for i in range(len(indices)):
            ctrls.append(ArcPointer[Tensor](self.load_control(indices[i], ctx)))
        var out = _concat_dim0(ctrls^, ctx)
        return out^
