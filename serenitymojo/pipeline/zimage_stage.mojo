...
#
# Product build: `pixi run build-krea-stage`. The Rust trainer supervisor runs
...
from serenitymojo.training.serenity_trainer_train_loop_policy import (
    serenity_stage_dir_from_train_config,
    serenity_dataset_path_from_train_config,
)
...
def main() raises:
    ...
    var data_dir = serenity_dataset_path_from_train_config(cfg)
    var stage_dir = serenity_stage_dir_from_train_config(cfg, String(""))
    ...
    if cfg.caption_dropout_prob > 0.0:
        var empty = List[UInt8]()
        _write_text(stage_dir + String("/uncond.txt"), empty)
        print("  caption dropout enabled: staged deterministic empty caption")
