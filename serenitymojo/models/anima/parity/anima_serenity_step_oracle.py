#!/usr/bin/env python3
# serenitymojo/models/anima/parity/anima_serenity_step_oracle.py
#
# SerenityTrainer-FAITHFUL recipe oracle for the ANIMA LoRA training STEP (Chunk A gate).
#
# WHAT THIS PROVES (the Chunk A recipe delta — NOT the transformer, already green):
#   predicted_flow + MSE-loss computed by the reference trainer recipe
...
#   t_embedder), and drives the reference trainer recipe end-to-end. Parity then isolates the recipe.
...
#   python3 serenitymojo/models/anima/parity/anima_serenity_step_oracle.py
#
...
def main():
    ...
    # single-axis table (theta=10000 over the flat token index) was reference trainer-UNFAITHFUL
    ...
    W("serenity_scaled_bthwc", scaled.reshape(-1))      # [B,1,Hd,Wd,C] flat
    W("serenity_noise_bthwc", noise.reshape(-1))
    W("serenity_context", context.reshape(-1))          # [B,512,1024]
    # references
    W("serenity_pred", pred)                            # [B*N,64] predicted_flow (patch layout)
    W("serenity_target_patches", target_patches)        # [B*N,64]
    with open(os.path.join(REF_DIR, "serenity_loss.bin"), "wb") as f:
        f.write(struct.pack("<f", float(loss)))
    with open(os.path.join(REF_DIR, "serenity_meta.txt"), "w") as f:
        f.write(f"sigma={sigma}\nt_in={t_in}\nL={L}\nS_IMG={S_IMG}\nS_TXT={S_TXT}\n")
        f.write(f"FIXED_TIMESTEP={FIXED_TIMESTEP}\nNUM_TRAIN_TIMESTEPS={NUM_TRAIN_TIMESTEPS}\n")

    print("=== Anima reference trainer-recipe step oracle ===")
