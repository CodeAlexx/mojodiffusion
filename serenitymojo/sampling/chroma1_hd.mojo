# sampling/chroma1_hd.mojo - Chroma1-HD SerenityTrainer sampler contract.
#
# Local SerenityTrainer references:
...
#   * Chroma1-HD scheduler_config.json loaded by the SerenityTrainer config
#
...
def chroma1_hd_shifted_sigma(
    ...
    """Static shifted FlowMatch sigma from SerenityTrainer's Chroma scheduler config."""
    if num_steps <= 0:
...
def chroma1_hd_scheduler_timestep_from_sigma(sigma: Float32) -> Float32:
    """Diffusers FlowMatch scheduler timestep value before SerenityTrainer `/ 1000`."""
    return sigma * 1000.0


def chroma1_hd_model_timestep_from_scheduler_timestep(timestep: Float32) -> Float32:
    """SerenityTrainer ChromaSampler transformer input: `expanded_timestep / 1000`."""
...
def chroma1_hd_cfg_batch_size() -> Int:
    """SerenityTrainer ChromaSampler always runs positive and negative branches."""
    return 2


def chroma1_hd_uses_guidance_embedding() -> Bool:
    """Chroma has no Flux-style guidance embedding in SerenityTrainer's sampler."""
    return False
