...
# gate_proj/up_proj/linear_fc2 — see the reference trainer/diffusers key map in zimage_stack_lora.
#
# Z-Image has TWO block flavors, BOTH with these 7 Linear projections (so BOTH get
# LoRA per SerenityTrainer's default — LoRAModuleWrapper adapts EVERY nn.Linear child of
...
#   the SerenityTrainer forward (LoRAModule.py:328-329: orig_forward(x) + up(down(x)) *
#   alpha/rank).
#
...
from serenitymojo.ops.norm_backward import (
    rms_norm_backward, rms_norm_backward_dx, rms_norm_backward_dg,
)
...
def zimage_lora_adapter_to_device(
    ...
    # ZImage trainer storage boundaries are BF16. F32 belongs inside compute
    # kernels/reductions or host oracle comparison, not device LoRA carriers.
...
def _zimage_oft_vec_tensor(
    ...
    # ZImage direct adapter vectors follow the same product boundary as LoRA:
    # store/upload BF16, then let oft_serenity_rotate_b4 cast internally for F32 math.
    return Tensor.from_host(sl.vec.copy(), [sl.r, 6], STDtype.BF16, ctx)
...
def zimage_block_lora_predict_device_tensor_moddev[
    ...
    var att = _zimage_sdpa_product_fwd[1, S, H, Dh](q_rope, k_rope, v, scale, ctx)
    var att_flat = reshape_owned(att^, [S, D])
...
def zimage_refiner_lora_backward[
    ...
    # fn2 frozen here (the only consumer, zimage_stack_lora_backward, discards
    # refiner norm grads) -> d_x-only backward; d_fn2 left empty.
    var d_ff = rms_norm_backward_dx(d_out_t, saved.ff[], w.fn2[], eps, ctx)
    var d_fn2 = List[Float32]()

    # ff = linear(act, w2)[+LoRA(w2)]  W [D, F]
    var act_h = saved.act[].to_host(ctx)
    var lb_w2 = _proj_bwd_with_lora(
        d_ff, saved.act[], w.w2[], act_h,
        lora.w2, SLOT_W2, S, F, D, d_a_slots, d_b_slots, ctx,
    )
    var d_w2 = List[Float32]()
    ...
    # xfn1 = rms_norm(h, fn1)  (no modulation between; fn1 frozen -> d_x only)
    var d_h_norm = rms_norm_backward_dx(d_xfn1, saved.h[], w.fn1[], eps, ctx)
    var d_fn1 = List[Float32]()
    var d_h = add(d_out_t, d_h_norm, ctx)

    # --- attention sub-block backward ---
    # h = x + attn_n2 (plain residual); attn_n2 = rms_norm(att_o, n2)
    # n2 frozen -> d_x-only backward; d_n2 left empty.
    var d_att_o = rms_norm_backward_dx(d_h, saved.att_o[], w.n2[], eps, ctx)
    var d_n2 = List[Float32]()

    # att_o = linear(att_flat, wo)[+LoRA(to_out)]  W [D, D]
    var att_flat_h = saved.att_flat[].to_host(ctx)
    var lb_o = _proj_bwd_with_lora(
        d_att_o, saved.att_flat[], w.wo[], att_flat_h,
        lora.to_out, SLOT_O, S, D, D, d_a_slots, d_b_slots, ctx,
    )
    var d_wo = List[Float32]()
    ...
    # xn1 = rms_norm(x, n1)  (no modulation between; n1 frozen -> d_x only)
    var d_xn1_norm = rms_norm_backward_dx(d_xn1, saved.x[], w.n1[], eps, ctx)
    var d_n1 = List[Float32]()

    var d_x_res = d_h.to_host(ctx)
    var d_x_norm = d_xn1_norm.to_host(ctx)
    var d_x = _add_lists(d_x_res, d_x_norm)
...
def zimage_block_lora_forward_device_tensor_batch_masked[
    B: Int, H: Int, Dh: Int, S: Int
](
    x_arc: TArc,
    w: ZImageBlockWeights, mv: ZImageModVecsDevice, lora: ZImageBlockLoraDevice,
    ...
    var att = sdpa[B, S, H, Dh](q_rope, k_rope, v, attn_mask, scale, ctx)
    var att_flat = reshape_owned(att^, [ROWS, D])
...
def zimage_block_lora_backward_device_tensors_batch_masked[
    B: Int, H: Int, Dh: Int, S: Int
](
    d_out: Tensor,
    w: ZImageBlockWeights, mv: ZImageModVecsDevice, lora: ZImageBlockLoraDevice,
    ...
    reshape_in_place(lb_o.d_x, [B, S, H, Dh])
    var sb = training_sdpa_backward_masked_batched_strict[B, S, H, Dh](
        saved.q_rope[], saved.k_rope[], saved.v[], attn_mask_f32, lb_o.d_x, scale, ctx,
    )
    var d_q_rms = rope_backward(sb.d_q, cos, sin, True, ctx)
    var d_k_rms = rope_backward(sb.d_k, cos, sin, True, ctx)
...
struct ZImageBlockFullFTBackward(Movable):
    ...
    # Phase B full-surface param grads (populated ONLY when SURF=True; else
    # [1] zero placeholders that are never read). F32.
    var d_n1: TArc        # [D]
    ...
    var d_fn1: TArc       # [D]
    var d_fn2: TArc       # [D]
    var d_scale_msa: TArc # [D]  raw adaLN chunk 0 grad
    var d_gate_msa: TArc  # [D]  raw adaLN chunk 1 grad (post-tanh_backward)
    var d_scale_mlp: TArc # [D]  raw adaLN chunk 2 grad
    var d_gate_mlp: TArc  # [D]  raw adaLN chunk 3 grad (post-tanh_backward)

    def __init__(
        out self, var d_x: TArc, var d_w: List[TArc],
        var d_n1: TArc, var d_q_norm: TArc, var d_k_norm: TArc,
        var d_n2: TArc, var d_fn1: TArc, var d_fn2: TArc,
        var d_scale_msa: TArc, var d_gate_msa: TArc,
        var d_scale_mlp: TArc, var d_gate_mlp: TArc,
    ):
        self.d_x = d_x^
        self.d_w = d_w^
        self.d_n1 = d_n1^
...
# One [1]-element F32 zero placeholder TArc (unread when SURF=False).
def _zimage_ft_zph(ctx: DeviceContext) raises -> TArc:
    var z = List[Float32]()
    z.append(Float32(0.0))
    return TArc(_t(z^, [1], ctx))


# SURF gates the full-surface param grads. When False the body is BYTE-IDENTICAL
# to the v1 (matmul-only) backward: dx-only norm/mod chains, and the 10 param
# grad fields are [1] zero placeholders. When True the norm chains switch to
# rms_norm_backward (d_x + d_g), the modulate chains compute d_scale, and the
# gate residuals switch to the 3-output gate_residual_backward (+ tanh_backward
# vs the RAW modvec) so the per-block adaLN_modulation.0 Linear + 6 rms scales
# get trained. d_x and the 7 matmul d_W in the SURF path are numerically the
# same chain (different kernel instances, gated at parity, not vs v1).
def zimage_block_backward_device_tensors_fullft[
    ...
    var d_n1 = _zimage_ft_zph(ctx)
    var d_q_norm = _zimage_ft_zph(ctx)
    var d_k_norm = _zimage_ft_zph(ctx)
    var d_n2 = _zimage_ft_zph(ctx)
    var d_fn1 = _zimage_ft_zph(ctx)
    var d_fn2 = _zimage_ft_zph(ctx)
    var d_scale_msa = _zimage_ft_zph(ctx)
    var d_gate_msa = _zimage_ft_zph(ctx)
    var d_scale_mlp = _zimage_ft_zph(ctx)
    var d_gate_mlp = _zimage_ft_zph(ctx)

    # ── mlp gate-residual + ffn_norm2 ───────────────────────────────────────
    # gate residual d_x = d_out (identity passthrough) — use d_out directly for
    # the residual grad rather than materialising it (byte-identical to v1).
    var d_ff: Tensor
    @parameter
    if SURF:
        var ff_n2_y = rms_norm(saved.ff[], w.fn2[], eps, ctx)
        ...
        d_gate_mlp = TArc(tanh_backward(grg2.d_g, mv.gate_mlp[], ctx))
        # d_x via the dx-only kernel (F32, identical to v1); d_g via the
        # weight-free F32 rms_norm_backward_dg (BF16 scale would round d_g).
        d_ff = rms_norm_backward_dx(grg2.d_y, saved.ff[], w.fn2[], eps, ctx)
        d_fn2 = TArc(rms_norm_backward_dg(grg2.d_y, saved.ff[], eps, ctx))
    else:
        var grg2 = gate_residual_backward_dxdy(d_out, saved.gate_mlp_t[], ctx)
        d_ff = rms_norm_backward_dx(grg2.d_y, saved.ff[], w.fn2[], eps, ctx)
    ...
    # ── mlp modulate (scale_mlp) + ffn_norm1 ────────────────────────────────
    var d_h_norm: Tensor
    @parameter
    if SURF:
        var mb_mlp = modulate_backward(d_xfn1s, saved.xfn1[], mv.scale_mlp[], ctx)
        d_scale_mlp = TArc(mb_mlp.d_scale.clone(ctx))
        d_h_norm = rms_norm_backward_dx(mb_mlp.d_x, saved.h[], w.fn1[], eps, ctx)
        d_fn1 = TArc(rms_norm_backward_dg(mb_mlp.d_x, saved.h[], eps, ctx))
    else:
        var mb_mlp = modulate_backward(
            d_xfn1s, saved.xfn1[], mv.scale_mlp[], ctx, compute_param_grads=False,
        )
        d_h_norm = rms_norm_backward_dx(mb_mlp.d_x, saved.h[], w.fn1[], eps, ctx)
    var d_h = add(d_out, d_h_norm, ctx)

    # ── attention gate-residual + attention_norm2 (d_x = d_h passthrough) ────
    var d_att_o: Tensor
    @parameter
    if SURF:
        var attn_n2_y = rms_norm(saved.att_o[], w.n2[], eps, ctx)
        ...
        d_gate_msa = TArc(tanh_backward(grg1.d_g, mv.gate_msa[], ctx))
        d_att_o = rms_norm_backward_dx(grg1.d_y, saved.att_o[], w.n2[], eps, ctx)
        d_n2 = TArc(rms_norm_backward_dg(grg1.d_y, saved.att_o[], eps, ctx))
    else:
        var grg1 = gate_residual_backward_dxdy(d_h, saved.gate_msa_t[], ctx)
        d_att_o = rms_norm_backward_dx(grg1.d_y, saved.att_o[], w.n2[], eps, ctx)
    ...
    # ── q/k per-head rms scales ─────────────────────────────────────────────
    var d_q_pre: Tensor
    var d_k_pre: Tensor
    @parameter
    if SURF:
        d_q_pre = rms_norm_backward_dx(d_q_rms, saved.q_pre[], w.q_norm[], eps, ctx)
        d_q_norm = TArc(rms_norm_backward_dg(d_q_rms, saved.q_pre[], eps, ctx))
        d_k_pre = rms_norm_backward_dx(d_k_rms, saved.k_pre[], w.k_norm[], eps, ctx)
        d_k_norm = TArc(rms_norm_backward_dg(d_k_rms, saved.k_pre[], eps, ctx))
    else:
        d_q_pre = rms_norm_backward_dx(d_q_rms, saved.q_pre[], w.q_norm[], eps, ctx)
        d_k_pre = rms_norm_backward_dx(d_k_rms, saved.k_pre[], w.k_norm[], eps, ctx)
    ...
    # ── attention modulate (scale_msa) + attention_norm1 ────────────────────
    var d_x_norm: Tensor
    @parameter
    if SURF:
        var mb_sa = modulate_backward(d_xn1s, saved.xn1[], mv.scale_msa[], ctx)
        d_scale_msa = TArc(mb_sa.d_scale.clone(ctx))
        d_x_norm = rms_norm_backward_dx(mb_sa.d_x, saved.x[], w.n1[], eps, ctx)
        d_n1 = TArc(rms_norm_backward_dg(mb_sa.d_x, saved.x[], eps, ctx))
    else:
        var mb_sa = modulate_backward(
            d_xn1s, saved.xn1[], mv.scale_msa[], ctx, compute_param_grads=False,
        )
    ...
    var d_x_t = add(d_h, d_x_norm, ctx)
    ...
    return ZImageBlockFullFTBackward(
        TArc(d_x_t^), d_w_slots^,
        d_n1^, d_q_norm^, d_k_norm^, d_n2^, d_fn1^, d_fn2^,
        d_scale_msa^, d_gate_msa^, d_scale_mlp^, d_gate_mlp^,
    )
