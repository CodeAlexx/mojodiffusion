...
from serenitymojo.ops.norm_backward import (
    ...
    LayerNormBackward, RmsNormBackward,
)
...
def _direct_proj_bwd_host(
    ...
    if direct.algo == SD35_DIRECT_ALGO_OFT:
        ...
        var g = oft_serenity_rotate_backward_b4(lb.d_x^, x, vec, ctx)
        var dx = g.d_x.to_host(ctx)
        return _SD35DirectProjectionBack(
            dx^,
            SD35DirectProjectionGrad(
...
def _layer_norm_backward_dx(
    ...
    # d_x-only kernel: skips the d_g/d_b parameter reductions entirely
    # (weight is ones, grads were discarded). Host-List carrier stays.
