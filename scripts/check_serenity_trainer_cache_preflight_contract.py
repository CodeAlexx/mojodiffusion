#!/usr/bin/env python3
"""Static guard for the SerenityTrainer product cache preflight contract.

This guard is intentionally narrow. It checks that the no-CUDA product preflight
is wired to the shared SerenityTrainer text-conditioning and VAE encoder/cache
...
CACHE_PREFLIGHT = REPO / "serenitymojo/training/serenity_trainer_cache_preflight.mojo"
CACHE_PREFLIGHT_SMOKE = REPO / "serenitymojo/training/serenity_trainer_cache_preflight_smoke.mojo"
PRODUCT_RUN = REPO / "serenitymojo/training/serenity_trainer_product_run.mojo"
PRODUCT_RUN_SMOKE = REPO / "serenitymojo/training/serenity_trainer_product_run_smoke.mojo"
TRAIN_DRY_RUN = REPO / "serenitymojo/training/serenity_trainer_train_dry_run.mojo"

TEXT_CONTRACT = REPO / "serenitymojo/models/text_encoder/serenity_trainer_conditioning_contract.mojo"
VAE_CONTRACT = REPO / "serenitymojo/sampling/vae_encoder_contract.mojo"

ALLOWED_SOURCE_ROOTS = (
    "/home/alex/SerenityTrainer",
    "/home/alex/SerenityTrainer-anima-ref",
)
...
def require_local_reference_scope(paths: list[Path]) -> None:
    """Forbid external SerenityTrainer/Anima reference paths in this product preflight.

    The module does not need to open SerenityTrainer sources; those are already
    ...
    for path in paths:
        ...
        for match in url_re.findall(text):
            lowered = match.lower()
            if "serenity_trainer" in lowered or "anima" in lowered:
                bad_refs.append(f"{path}: external URL {match}")
        for match in path_re.findall(text):
            if "SerenityTrainer" not in match:
                continue
            if not any(match.startswith(root) for root in ALLOWED_SOURCE_ROOTS):
                bad_refs.append(f"{path}: non-local SerenityTrainer path {match}")
...
def main() -> int:
    ...
    require(
        ...
        [
            "struct SerenityTrainerTextCacheReadinessContract",
    ...
    require(
        ...
        [
            "struct SerenityTrainerVaeEncoderContract",
    ...
    require(
        ...
        [
            "struct SerenityCachePreflightPlan",
    ...
    require(
        ...
        [
            ...
            "SerenityTrainer only_cache requires raw VAE cache encode readiness",
            "raw_status=",
            "prepared_status=",
        ],
    )
    ...
    require(
        ...
        [
            ...
            "serenity_trainer_cache_preflight_smoke PASS",
        ],
    )
    ...
    require(
        ...
        [
            ...
            "serenity_trainer_product_run_smoke PASS",
        ],
    )

    print("SerenityTrainer cache preflight contract static guard PASS")
...
if __name__ == "__main__":
    try:
        ...
        print(f"SerenityTrainer cache preflight contract static guard FAIL: {exc}")
        raise SystemExit(1)
