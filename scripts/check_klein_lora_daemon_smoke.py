#!/usr/bin/env python3
"""Run a bounded real Klein 9B LoRA txt2img request through the Mojo daemon.

This checker starts `serenity_daemon dispatch` by default, posts the flat
Serenity/Comfy-style LoRA request that the graph importer lowers to, then
verifies the produced PNG genparams, Klein result manifest, and loader log.
...
DEFAULT_REPORT = REPO / "output/checks/klein9b_lora_daemon_smoke.json"
DEFAULT_PROMPT = (
...
def make_request(args: argparse.Namespace) -> dict[str, Any]:
    return {
        ...
        "lora": [{"name": str(args.lora), "weight": 1.0}],
    }
...
def run(args: argparse.Namespace) -> dict[str, Any]:
    ...
    report: dict[str, Any] = {
        "schema": "serenity.klein9b_lora_daemon_smoke.v1",
        "case": "klein9b_lora_txt2img",
    ...
    try:
        ...
        print(f"[klein-lora-daemon-smoke] wrote report: {args.write_report}")
    return report
...
def main() -> int:
    ...
    args.daemon = args.daemon.expanduser()
    args.lora = args.lora.expanduser()
    if not args.daemon.is_absolute():
        args.daemon = (REPO / args.daemon).resolve()
    if not args.lora.is_absolute():
        args.lora = (Path.cwd() / args.lora).resolve()
    args.write_report = args.write_report.expanduser()
    if not args.write_report.is_absolute():
        args.write_report = (REPO / args.write_report).resolve()
    if args.log is not None:
        args.log = args.log.expanduser()
        if not args.log.is_absolute():
            args.log = (REPO / args.log).resolve()
    ...
    if args.json:
        ...
        print(
            f"[klein-lora-daemon-smoke] {status} "
            f"job={dict_or_empty(report.get('job')).get('id')} "
            f"output={report.get('output_path')}"
        )
        for blocker in report.get("blockers") or []:
            print(f"[klein-lora-daemon-smoke] blocker: {blocker}")
