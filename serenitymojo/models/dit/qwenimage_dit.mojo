...
struct QwenImageDitOffloaded(Movable):
    ...
    def load(dir: String, ctx: DeviceContext) raises -> QwenImageDitOffloaded:
        ...
        # fill_block_store=False — HOST-RAM BOMB FIX (job-0088, box froze):
        # the default persistent pinned host block store copies EVERY
        # transformer tensor into CUDA-pinned RAM at open(). Measured for this
        # DiT: 38.05 GiB store (1933 tensors) + 2x648 MiB slot slabs = ~39.3
        # GiB of UNRECLAIMABLE pinned memory, and the populate's sequential
        # 38 GiB mmap read balloons page cache on top — the kernel OOM-killed
        # the worker at shmem-rss ~53.5 GiB on the 60 GB box. Without the
        # store, each prefetch fills the bounded 2-slot pinned slabs straight
        # from the mmap (turbo_loader.mojo's documented low-RAM mode, same
        # bytes, same math) — peak pinned host stays ~1.3 GiB.
