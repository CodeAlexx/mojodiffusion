# chroma_train_ref_artifact_smoke.mojo -- Chroma SerenityTrainer one-step artifact gate.
#
# Header-plus-payload, no-CUDA gate. It opens the real local SerenityTrainer Chroma
...
# for the real update path. This smoke consumes SerenityTrainer update-delta
...
comptime STEP_DUMP = "/home/alex/serenity_trainer-mojo/parity/chroma_train_ref_step000.safetensors"
comptime ADAPTER_DUMP = "/home/alex/serenity_trainer-mojo/parity/chroma_train_ref_step000_adapters.safetensors"
comptime META_JSON = "/home/alex/serenity_trainer-mojo/parity/chroma_train_ref_meta.json"
...
def _read_utf8_file(path: String) raises -> String:
    ...
    if fd < 0:
        raise Error(String("Chroma reference trainer smoke cannot open ") + path)
    var n = file_size(fd)
    ...
    if done != n:
        buf.free()
        raise Error(String("Chroma reference trainer smoke short read from ") + path)
...
def _require_contains(text: String, needle: String, label: String) raises:
    _require(
        ...
        String("Chroma reference trainer meta missing ") + label + String(": ") + needle,
    )
...
def _require_tensor(
    ...
    _require(key in st.tensors, String("missing Chroma reference trainer tensor ") + key)
    var info = st.tensor_info(key)
    _require(
        info.dtype == dtype,
