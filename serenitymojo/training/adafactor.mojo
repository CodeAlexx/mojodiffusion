...
struct AdafactorState1D(Copyable, Movable):
    """Per-parameter UNFACTORED second-moment state for a 1D [n] vector:
    v [n] (EMA of g^2) and the 1-based step count. Mirrors torch state
    "variance" for params with grad.dim() <= 1 (_adafactor.py:110-113:
    `state["variance"] = torch.zeros_like(p.grad, ...)`). The factored/
    unfactored split is dimensionality-only: torch branches on
    `p.grad.dim() > 1` (:100) / `grad.dim() > 1` (:387); the transformers
    Adafactor SerenityTrainer runs agrees (`factored = len(param_shape) >= 2`,
    transformers/optimization.py:1186)."""
...
def adafactor_step_1d(
    mut p: List[Float32],
    g: List[Float32],
    mut state: AdafactorState1D,
    lr: Float64,
    beta2_decay: Float64,
    eps1_in: Float64,
    eps2: Float64,
    ...
    """One Adafactor step on one 1D [state.n] param — the UNFACTORED branch of
    torch/optim/_adafactor.py::_single_tensor_adafactor (torch 2.10.0, the
    SerenityTrainer venv's copy), line-by-line:

        one_minus_beta2_t = step_float**beta2_decay                     # :379
        rho_t  = min(lr, 1 / (step_float**0.5))                         # :380
        alpha  = max(eps2, param.norm(2).item()/(numel**0.5)) * rho_t   # :381
        if weight_decay != 0: param.mul_(1 - lr * weight_decay)         # :384-385
        grad_squared = grad * grad          # NO eps added to g^2       # :407
        variance.lerp_(grad_squared, one_minus_beta2_t)                 # :408
        var_estimate = variance.clone()     # v itself is NOT clamped   # :410
        update = var_estimate.clamp_(min=eps1*eps1).rsqrt_()            # :413
        update.mul_(grad)                                               # :414
        denom  = max(1.0, update.norm(2).item()/((numel**0.5)*d))       # :415
        param.add_(update, alpha=-alpha/denom)                          # :416

    eps1 handling is IDENTICAL to 2D: eps1 appears only at the clamp before
    rsqrt (:413) — torch does NOT add eps inside the lerp (that is the
    transformers variant, adafactor_extensions.py:62 `(grad**2) + eps[0]`,
    which SimpleTuner's "torch-adafactor" key does not use). lr enters only
    through rho_t (:380) and stepweight decay (:385) — no <2D special case.
    eps1_in <= 0 selects the torch F32 default (ADAFACTOR_EPS1_F32, :372-373).
    SerenityTrainer full-FT config note: weight decay applies UNIFORMLY to all
    params (no per-group / per-dim exclusion for norm scales or biases —
    reference trainer modelSetup defines no weight_decay overrides; Adafactor default wd is
    0.0, optimizer_util.py:102). Mutates p and state in place."""
    var n = state.n
    if len(p) != n or len(g) != n:
        raise Error("adafactor_step_1d: p/g length != n")
    var eps1 = eps1_in
    if eps1 <= 0.0:
        eps1 = ADAFACTOR_EPS1_F32
    ...
    state.step += 1
    var t = Float64(state.step)
    var one_minus_beta2_t = t**beta2_decay  # :379
    var rho_t = min(lr, 1.0 / sqrt(t))  # :380
    ...
    # v.lerp_(g*g, one_minus_beta2_t) i.e. v + w*(g^2 - v) (:407-408), all
    # F32 elementwise like torch's tensors (no mean — nothing to accumulate).
    var w = Float32(one_minus_beta2_t)
    for i in range(n):
        var g2 = g[i] * g[i]
        state.v[i] = state.v[i] + w * (g2 - state.v[i])

    # update = rsqrt(clamp(v, min=eps1^2)) * g (:413-414 — clamp on a COPY,
    # v stays unclamped per :410) with the d-threshold RMS clip (:415-416).
    var eps1_sq = Float32(eps1 * eps1)
    var update = List[Float32](capacity=n)
    var u_sq = Float64(0.0)
    for i in range(n):
