...
def zimage_tiled_decode_with_decoder[
    ...
    # Rectangular tiles need independent row/column strides. Reusing the
    # height stride for columns only happened to work for square canvases.
    var half_h = TILE_H // 2
    var half_w = TILE_W // 2
    ...
    var b = cast_tensor(dec.decode(cast_tensor(slice(r, 3, half_w, TILE_W, ctx), STDtype.BF16, ctx), ctx), STDtype.F32, ctx)
    ...
    # row 1 crop [half_h:half_h+TILE_H] (reassign a/b/c → prior tiles freed)
    ...
    b = cast_tensor(dec.decode(cast_tensor(slice(r, 3, half_w, TILE_W, ctx), STDtype.BF16, ctx), ctx), STDtype.F32, ctx)
