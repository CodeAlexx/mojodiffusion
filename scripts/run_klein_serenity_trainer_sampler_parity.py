#!/usr/bin/env python3
"""Produce SerenityTrainer Klein/Flux2 sampler artifacts for parity infrastructure.

This is a development/parity helper, not product runtime. It follows
SerenityTrainer's Flux2Sampler/Flux2Model path and writes the SerenityTrainer-side
...
DEFAULT_OUT_DIR = Path("/home/alex/serenity_trainer-mojo/parity/klein_sampler_parity")
DEFAULT_MANIFEST = DEFAULT_OUT_DIR / "manifest_fragment.json"
DEFAULT_CHECKER_MANIFEST = Path("/home/alex/serenity_trainer-mojo/parity/klein_sampler_parity_manifest.json")
...
def _install_serenity_trainer_imports() -> None:
    scripts_dir = SERENITY_TRAINER / "scripts"
    sys.path.insert(0, str(scripts_dir))
    from util.import_util import script_imports

    os.chdir(SERENITY_TRAINER)
    script_imports()
...
def _resolve_sample(args: argparse.Namespace) -> dict[str, Any]:
    ...
    if width <= 0 or height <= 0 or steps <= 0:
        raise ValueError("width/height/steps must be positive after SerenityTrainer-style quantization")
...
def _dtype_to_serenity_trainer(dtype_name: str) -> Any:
...
def _build_weight_dtypes(dtype_name: str) -> Any:
    ...
    train_dtype = _dtype_to_serenity_trainer(dtype_name)
...
def _preflight_model_args(args: argparse.Namespace) -> None:
    ...
    if base_path.exists() and base_path.is_file():
        raise ValueError(
            "SerenityTrainer Flux2ModelLoader does not load a single-file Flux2 base model. "
...
def _load_model(args: argparse.Namespace, device: Any, temp_device: Any) -> Any:
    ...
    if model_loader is None or model_setup is None:
        raise RuntimeError("could not create SerenityTrainer Flux2 loader/setup")

    dtype = _dtype_to_serenity_trainer(args.dtype)
...
def _artifact_groups() -> dict[str, Any]:
    return {
        ...
        "serenity_trainer_decode_and_png": [
...
def _checker_static_sections() -> dict[str, Any]:
    ...
    except Exception as exc:  # noqa: BLE001 - this helper should still write the reference trainer fragment.
        return {
            "current_split_validation_evidence": {
...
def _write_manifest(path: Path, manifest: dict[str, Any]) -> None:
    ...
    print(f"[reference-klein-sampler] wrote manifest: {path}")
...
def _write_checker_template(path: Path, args: argparse.Namespace, sample: dict[str, Any]) -> None:
    ...
    path.parent.mkdir(parents=True, exist_ok=True)
    path.write_text(json.dumps(manifest, indent=2, sort_keys=True) + "\n", encoding="utf-8")
    print(f"[reference-klein-sampler] wrote checker template: {path} parity_claimed=false")
...
def _run_serenity_trainer_sample(
    model: Any,
    sample: dict[str, Any],
    *,
    out_dir: Path,
    ...
    with torch.no_grad(), model.autocast_context:
        ...
        artifacts["serenity_trainer_initial_noise_raw_nchw"] = _save_tensor(
            out_dir / "serenity_trainer_initial_noise_raw_nchw.safetensors",
            latent_raw,
            note="SerenityTrainer Flux2Sampler raw PyTorch F32 noise before patchify_latents.",
        )

        latent_patch = model.patchify_latents(latent_raw)
        artifacts["serenity_trainer_initial_noise_post_patch_nchw"] = _save_tensor(
            out_dir / "serenity_trainer_initial_noise_post_patch_nchw.safetensors",
            latent_patch,
            note="SerenityTrainer Flux2Model.patchify_latents output.",
        )
        image_ids = model.prepare_latent_image_ids(latent_patch)

        latent_packed = model.pack_latents(latent_patch)
        artifacts["serenity_trainer_initial_noise_post_pack"] = _save_tensor(
            out_dir / "serenity_trainer_initial_noise_post_pack.safetensors",
            latent_packed.squeeze(0),
            note="SerenityTrainer Flux2Model.pack_latents output, squeezed to [tokens, 128] for the manifest guard.",
        )
        ...
        with _timed_stage(torch) as denoise_timer:
            for index, timestep in enumerate(timesteps):
                ...
                print(f"[reference-klein-sampler] denoise step={index + 1}/{len(timesteps)}")
        ...
        artifacts["serenity_trainer_latent_trajectory"] = _save_tensor(
            out_dir / "serenity_trainer_latent_trajectory.safetensors",
            trajectory,
            note="SerenityTrainer packed latent trajectory including step 0, shape [steps + 1, tokens, 128].",
        )
        artifacts["serenity_trainer_final_packed_latent"] = _save_tensor(
            out_dir / "serenity_trainer_final_packed_latent.safetensors",
            latent_packed.squeeze(0),
            note="SerenityTrainer final packed latent, squeezed to [tokens, 128] for the manifest guard.",
        )
        ...
        artifacts["serenity_trainer_final_unpacked_latent"] = _save_tensor(
            out_dir / "serenity_trainer_final_unpacked_latent.safetensors",
            final_unpacked,
            note="SerenityTrainer Flux2Model.unpack_latents output before VAE batch-norm unscale.",
        )
        artifacts["serenity_trainer_final_unscaled_unpatchified_latent"] = _save_tensor(
            out_dir / "serenity_trainer_final_unscaled_unpatchified_latent.safetensors",
            final_unscaled_unpatchified,
            note="SerenityTrainer unscale_latents then unpatchify_latents output passed to the VAE decoder.",
        )
        artifacts["serenity_trainer_vae_decoded_tensor"] = _save_tensor(
            out_dir / "serenity_trainer_vae_decoded_tensor.safetensors",
            decoded,
            note="Raw SerenityTrainer VAE decoded tensor before image_processor.postprocess.",
        )
        png_path = out_dir / "serenity_trainer_png.png"
        image[0].save(png_path)
        artifacts["serenity_trainer_png"] = _png_artifact(png_path, width, height)
...
def _build_manifest(
    *,
    ...
    manifest = {
        "schema_version": 1,
        "producer": "scripts/run_klein_serenity_trainer_sampler_parity.py",
        "scope": "SerenityTrainer-only Klein/Flux2 sampler artifact producer; not product runtime and not sampler parity.",
        "model_type": "FLUX_2",
        "variant": "klein",
        "accepted": False,
        ...
        "parity_note": (
            "This manifest records SerenityTrainer-side artifacts only. Accepted/parity flags stay false "
        ...
        "metrics": {
            "serenity_trainer": serenity_trainer_metrics,
            "mojo": {
        ...
        "measurement": {
            "source": "SerenityTrainer Flux2Sampler/Flux2Model via scripts/run_klein_serenity_trainer_sampler_parity.py",
            "command": command,
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Produce SerenityTrainer-only Klein/Flux2 sampler artifacts for the manifest guard."
    )
    ...
    parser.add_argument(
        ...
        type=Path,
        default=None,
        help="Write a paired-manifest template matching this sample without loading SerenityTrainer/CUDA.",
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--no-run",
        action="store_true",
        help="Only write requested checker template/readiness outputs; do not import SerenityTrainer or load CUDA.",
    )
...
def main() -> int:
    ...
    if args.no_run:
        if args.write_checker_template is None and args.write_checker_readiness is None:
            print("[reference-klein-sampler] ERROR --no-run requires --write-checker-template or --write-checker-readiness")
            return 2
        if args.write_checker_readiness is not None:
            _write_checker_readiness(args.write_checker_readiness, args.manifest)
        print("[reference-klein-sampler] INFO no-run complete; no SerenityTrainer/CUDA execution; parity_claimed=false")
        return 0

    _install_serenity_trainer_imports()
    import torch
    ...
    try:
        ...
        run = _run_serenity_trainer_sample(
            model,
            sample,
    ...
    print(
        "[reference-klein-sampler] PASS "
        f"artifacts={args.out_dir} manifest={args.manifest} "
        f"steps={sample['steps']} denoise_s_per_step={manifest['metrics']['serenity_trainer']['denoise_seconds_per_step']:.6f} "
        f"vae_s={manifest['metrics']['serenity_trainer']['vae_decode_seconds']:.6f} "
        f"peak_vram_mib={manifest['metrics']['serenity_trainer']['peak_vram_mib']:.1f} "
        "parity_claimed=false"
    )
    return 0
