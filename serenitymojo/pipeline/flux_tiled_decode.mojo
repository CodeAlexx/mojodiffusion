...
def flux_tiled_decode[
    ...
    # Height and width strides are independent.  The old square-only path used
    # TILE_H for both axes, which silently selected the wrong column crops for
    # rectangular latent grids even though this generic accepts LATENT_H/W.
    comptime HALF_H = TILE_H // 2
    comptime HALF_W = TILE_W // 2
...
def flux_tiled_decode_5x5_lowmem[
    ...
    var dec = load_flux1_ldm_decoder[TILE_H, TILE_W](vae_path, ctx)
    # Keep rectangular crop offsets axis-correct here too.  Using the
    # height-derived stride for columns only works when TILE_H == TILE_W.
    comptime STRIDE_H = (TILE_H * 3) // 4
    comptime STRIDE_W = (TILE_W * 3) // 4
