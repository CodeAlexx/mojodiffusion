...
def main():
    ...
    # Construct the creator model directly in BF16. Building the default F32
    # parameters and then converting would transiently require about 30 GB.
    ...
    # Stream the pinned Diffusers F32 shards into the creator-named BF16 model.
    # This is a development oracle adapter only; product loading remains Mojo.
    ...
    for shard in sorted(
        ...
        assert not extra, f"unexpected mapped keys in {shard}: {sorted(extra)[:8]}"
        overlap = loaded.intersection(mapped)
        assert not overlap, f"duplicate keys across shards: {sorted(overlap)[:8]}"
        model.load_state_dict(mapped, strict=False)
        loaded.update(mapped)
        del mapped, source
    missing = expected - loaded
    assert not missing, f"missing weights: {sorted(missing)[:8]}"
    print("loaded", len(loaded), "pinned tensors from", HF_REVISION)
