...
def ltx2_block_backward[
    ...
    # norm_ff = rms(hs)  (no affine -> d_x-only backward, d_g never needed)
    var d_hs_from_ff = rms_norm_backward_dx(d_norm_ff, sv_hs, ones_t, eps, ctx)  # [S,D] bf16
    ...
    # q_norm/k_norm frozen (LoRA path) -> d_x-only backward; d_g discarded (MJ-1035 class).
    var d_q_dx = rms_norm_backward_dx(d_q_rms_flat, q_flat, w.q_norm[], eps, ctx)
    var d_k_dx = rms_norm_backward_dx(d_k_rms_flat, k_flat, w.k_norm[], eps, ctx)
    var d_q_norm = _empty()
    var d_k_norm = _empty()
    var d_q = d_q_dx.clone(ctx)   # [S,D] bf16
    var d_k = d_k_dx.clone(ctx)
    ...
    # norm_h = rms(hidden) (no affine -> d_x-only backward, d_g never needed)
    var d_hidden_from_norm = rms_norm_backward_dx(d_norm_h, sv_hidden, ones_t, eps, ctx)  # bf16
