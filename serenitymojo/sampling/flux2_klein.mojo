...
#
# SerenityTrainer uses one Flux2Sampler for both Flux2 dev and Flux2/Klein. The
...
def compute_empirical_mu(image_seq_len: Int, num_steps: Int) -> Float64:
    ...
    SerenityTrainer Flux2Sampler imports `compute_empirical_mu` and passes the result
...
def build_flux2_sigma_schedule(
    ...
    SerenityTrainer Flux2Sampler supplies `np.linspace(1.0, 1/num_steps, num_steps)`
...
def build_flux2_fixed_shift_schedule(
    ...
    This is not the SerenityTrainer Flux2Sampler path, which uses empirical `mu` plus
...
def flux2_model_timestep_from_scheduler_timestep(timestep: Float32) -> Float32:
    """SerenityTrainer Flux2Sampler transformer input: `expanded_timestep / 1000`."""
...
def flux2_cfg_batch_size(guidance_scale: Float32, guidance_embeds: Bool) -> Int:
    """SerenityTrainer Flux2Sampler true-CFG batch rule."""
