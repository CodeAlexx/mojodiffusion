...
# sdpa-flash-signoff; HANDOFF_2026-06-11_OVERNIGHT_SERENITY_PARITY.md §3.5).
#
...
def sdpa_flash_infer_fwd_dynamic(
    ...
    if rc != 0:
        raise Error(
            String("sdpa_flash_infer_fwd: shim rc=") + String(rc)
...
def sdpa_flash_infer_fwd_causal_padmask[
    ...
    """Causal BF16 inference SDPA with a valid-prefix length.

    Gemma/LTX-2 prompt conditioning compacts real tokens into the prefix of a
    128-aligned compile bucket.  The cuDNN training-forward ABI is reused here
    because it already exposes both causal masking and real Q/KV lengths; its
...
def sdpa_flash_infer_fwd_causal_padmask_dynamic(
    ...
    var rc = Int(external_call["flame_cudnn_sdpa_bf16_train_fwd", Int32](
        ...
        Int32(1),  # causal
        Int32(real_len), Int32(real_len),
        stream,
    ))
    qs.free(); ks.free(); vs.free(); os_.free()
    if rc != 0:
        raise Error(
            String("sdpa_flash_infer_fwd_causal_padmask: shim rc=") + String(rc)
            + String(" (B=") + String(B) + String(" S=") + String(S)
...
def sdpa_flash_infer_fwd_rect[
    ...
    """Dedicated rectangular cuDNN inference SDPA with no padding or stats."""
    if q.dtype() != STDtype.BF16 or k.dtype() != STDtype.BF16 or v.dtype() != STDtype.BF16:
        raise Error("sdpa_flash_infer_fwd_rect: q/k/v must be BF16")
    var o_buf = ctx.enqueue_create_buffer[DType.uint8](B * Sq * H * Dh * 2)
    ctx.enqueue_memset[DType.uint8](o_buf, 0)
    var o_shape: List[Int] = [B, Sq, H, Dh]
    var o = Tensor(o_buf^, o_shape^, STDtype.BF16)
    var qs = _strides_bhnd(Sq, H, Dh)
    var ks = _strides_bhnd(Skv, H, Dh)
    var vs = _strides_bhnd(Skv, H, Dh)
    var os_ = _strides_bhnd(Sq, H, Dh)
    var stream = CUDA(ctx.stream())
    var rc = Int(external_call["flame_cudnn_sdpa_bf16", Int32](
        _dev_ptr(q), _dev_ptr(k), _dev_ptr(v), _dev_ptr(o),
        Int32(B), Int32(H), Int32(Sq), Int32(Skv), Int32(Dh), scale,
        qs, ks, vs, os_,
        Int64(0), Int64(0), Int64(0), Int64(0),
        Int32(0), stream,
    ))
    ...
    if rc != 0:
        raise Error(
            String("sdpa_flash_infer_fwd_rect: shim rc=") + String(rc)
            + String(" (B=") + String(B) + String(" Sq=") + String(Sq)
            + String(" Skv=") + String(Skv) + String(" H=") + String(H)
            + String(" Dh=") + String(Dh) + String(")")
        )
    return o^


def sdpa_flash_infer_fwd_rect_padmask[
    B: Int, Sq: Int, Skv: Int, H: Int, Dh: Int
](
    q: Tensor, k: Tensor, v: Tensor,
    real_kv_len: Int,
    ...
    """Rectangular inference with a real K/V length inside a padded buffer.

    Bernini's UMT5 producer stores 512 rows plus the true token count. Creator
    inference passes that count to varlen cuDNN attention, so padded text keys
    must not participate in softmax. This uses the existing training-forward
    cshim solely because that admitted graph already accepts real Q/KV lengths;
    saved tensors are local and discarded before return.
    """
    if q.dtype() != STDtype.BF16 or k.dtype() != STDtype.BF16 or v.dtype() != STDtype.BF16:
        raise Error("sdpa_flash_infer_fwd_rect_padmask: q/k/v must be BF16")
    if real_kv_len < 1 or real_kv_len > Skv:
        raise Error("sdpa_flash_infer_fwd_rect_padmask: real_kv_len out of range")
    ...
    var rc = Int(external_call["flame_cudnn_sdpa_bf16_train_fwd", Int32](
        ...
        Int64(0), Int64(0), Int64(0), Int64(0), Int64(0),
        Int32(0),
        Int32(Sq), Int32(real_kv_len),
        stream,
    ))
    qs.free(); ks.free(); vs.free(); os_.free()
    if rc != 0:
        raise Error(
            String("sdpa_flash_infer_fwd_rect_padmask: shim rc=") + String(rc)
            + String(" (Sq=") + String(Sq) + String(" Skv=") + String(Skv)
            + String(" real_kv_len=") + String(real_kv_len) + String(")")
        )
    var o = _unpad_seq[B, Sq, SQ_PAD](o_pad, H, Dh, ctx)
    # B=1 unpadding is a sub-buffer view into o_pad. Re-box it as an owning
    # exact-size buffer because this inference helper does not return o_pad.
    comptime if B == 1 and SQ_PAD != Sq:
        var exact_buf = ctx.enqueue_create_buffer[DType.uint8](B * Sq * H * Dh * 2)
        ctx.enqueue_copy(dst_buf=exact_buf, src_buf=o.buf)
        var exact_shape: List[Int] = [B, Sq, H, Dh]
        return Tensor(exact_buf^, exact_shape^, STDtype.BF16)
    return o^
...
# boundary, runs cuDNN flash, and casts O / dQ/dK/dV back to F32. Forward uses
# sdpa_flash_train_fwd's proven 128-row padding + real-length mask, so inference
# shapes such as Klein's rectangular S=4544/4672 are valid. Backward remains
# aligned-only and fails loudly below until its saved-tape strides are taught
# the padded length. The bf16 q/k/v/o + stats are returned as TArcs for the
# saved tape; aligned backward consumes them without re-casting.
