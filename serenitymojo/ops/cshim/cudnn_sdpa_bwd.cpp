...
struct SdpaBwdEntry {
    ...
    // MJ-1031 follow-up (2026-07-11, MEASURED): the per-call SYNC 4-byte seq_len
    // uploads were ~60/step on krea2, each draining the stream (~20ms) => ~1.2s/
    // step of host blocking. Cache one (seq_q, seq_kv) device pair PER UNIQUE
    // (rN_q, rN_kv): upload once on first sight (under g_cache_mutex), zero
    // copies steady-state. Lengths are bucketed per model => tiny map.
    std::unordered_map<uint64_t, std::pair<void*, void*>> seq_len_cache;
};
...
extern "C" int flame_cudnn_sdpa_bwd_bf16(
    ...
    int64_t ws_sz  = 0;
    {
        ...
        (void)ws_sz;
        // Per-length seq buffers: cached upload (once per unique length pair).
        if (it->second.seq_len_q_buf && it->second.seq_len_kv_buf) {
            uint64_t lk = ((uint64_t)(uint32_t)rN_q << 32) | (uint64_t)(uint32_t)rN_kv;
            auto& slc = it->second.seq_len_cache;
            auto sit = slc.find(lk);
            if (sit == slc.end()) {
                void* qb = nullptr; void* kb = nullptr;
                cudaError_t e = cudaMalloc(&qb, (size_t)B * sizeof(int32_t));
                if (e != cudaSuccess) { fprintf(stderr, "[flame_cudnn_sdpa_bwd] seq cache cudaMalloc failed: %s\n", cudaGetErrorString(e)); return -40; }
                e = cudaMalloc(&kb, (size_t)B * sizeof(int32_t));
                if (e != cudaSuccess) { fprintf(stderr, "[flame_cudnn_sdpa_bwd] seq cache cudaMalloc failed: %s\n", cudaGetErrorString(e)); return -41; }
                std::vector<int32_t> sq((size_t)B, rN_q);
                std::vector<int32_t> sk((size_t)B, rN_kv);
                e = cudaMemcpy(qb, sq.data(), (size_t)B * sizeof(int32_t), cudaMemcpyHostToDevice);
                if (e != cudaSuccess) { fprintf(stderr, "[flame_cudnn_sdpa_bwd] seq cache upload failed: %s\n", cudaGetErrorString(e)); return -42; }
                e = cudaMemcpy(kb, sk.data(), (size_t)B * sizeof(int32_t), cudaMemcpyHostToDevice);
                if (e != cudaSuccess) { fprintf(stderr, "[flame_cudnn_sdpa_bwd] seq cache upload failed: %s\n", cudaGetErrorString(e)); return -43; }
                sit = slc.emplace(lk, std::make_pair(qb, kb)).first;
            }
            seq_q_buf  = sit->second.first;
            seq_kv_buf = sit->second.second;
        }
    }

    // Seq-len buffers are pre-filled per unique length pair (cache above) — NO
    // per-call upload (was 2 sync 4-byte cudaMemcpy per execute, MEASURED ~60/step
    // on krea2, each a full stream drain => ~1.2s/step host blocking).
