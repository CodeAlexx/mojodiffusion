...
# reflection, so — mirroring SerenityTrainer BaseConfig.from_dict — we start from
...
def _set_part_dropout(mut cfg: TrainConfig, part: String, value: Float32):
    # SerenityTrainer per-text-encoder caption dropout (text_encoder.dropout_probability
...
def _loss_fn_int(s: String) raises -> Int:
    ...
    if s == "mse" or s == "MSE":
        ...
        return 2
    elif s == "mae" or s == "MAE" or s == "l1" or s == "L1":
        return 3
    raise Error(
        String("JSON config: unknown loss_fn '") + s
        + "' (expected mse|mae|l1|huber|smooth_l1)"
    )
...
def _timestep_distribution_int(s: String) raises -> Int:
    ...
    # SerenityTrainer carries additional distribution tags; preserve them as policy
...
def _training_method_int(s: String) raises -> Int:
    # SerenityTrainer TrainingMethod values plus common full-finetune shorthands.
...
def _gradient_checkpointing_int(s: String) raises -> Int:
    # SerenityTrainer GradientCheckpointingMethod values.
    if s == "OFF" or s == "off":
...
def read_model_config(json_path: String) raises -> TrainConfig:
    ...
    while True:
        ...
        if key == "model_type":
            ...
            # at config-load time (SerenityTrainer fail-fast discipline).
            # MJ-1065 (2026-07-03): "streamed_base_opt_in" is the explicit slow
            # arm (per-step bf16 disk stream) for anchor re-runs / A-Bs; the
            # per-trainer gate fail-louds on empty/OFF so no run silently streams.
            var sc = _read_scalar(cur)
            if sc.is_string:
                if (
        ...
        elif key == "save_max_keep" or key == "max_step_saves_to_keep":
            cfg.save_max_keep = Int(_read_scalar(cur).num)
        # ── LTX2 IC-LoRA / v2v (P5 (d)); absent -> struct defaults -> C13 ────────
        elif key == "ic_lora_strategy":
            var sc = _read_scalar(cur)
            if sc.is_string:
                cfg.ic_lora_strategy = sc.s
        elif key == "reference_downscale":
            cfg.reference_downscale = Int(_read_scalar(cur).num)
        elif key == "first_frame_conditioning_p" or key == "ltx2_first_frame_conditioning_p":
            cfg.first_frame_conditioning_p = Float32(_read_scalar(cur).num)
        elif key == "reference_cache_dir" or key == "reference_cache_directory":
            var sc = _read_scalar(cur)
            if sc.is_string:
                cfg.reference_cache_dir = sc.s
        elif key == "val_reference_cache_dir" or key == "val_reference_cache_directory":
            var sc = _read_scalar(cur)
            if sc.is_string:
                cfg.val_reference_cache_dir = sc.s
        # ── LTX2 intrinsic-conditioning set (P5.5); absent -> defaults -> C13 ────
        elif key == "prefix_conditioning_p":
            cfg.prefix_conditioning_p = Float32(_read_scalar(cur).num)
        elif key == "suffix_conditioning_p":
            cfg.suffix_conditioning_p = Float32(_read_scalar(cur).num)
        elif key == "spatial_crop_conditioning_p":
            cfg.spatial_crop_conditioning_p = Float32(_read_scalar(cur).num)
        elif key == "temporal_boundary":
            cfg.temporal_boundary = Int(_read_scalar(cur).num)
        elif key == "spatial_crop_y1":
            cfg.spatial_crop_y1 = Int(_read_scalar(cur).num)
        elif key == "spatial_crop_x1":
            cfg.spatial_crop_x1 = Int(_read_scalar(cur).num)
        elif key == "spatial_crop_y2":
            cfg.spatial_crop_y2 = Int(_read_scalar(cur).num)
        elif key == "spatial_crop_x2":
            cfg.spatial_crop_x2 = Int(_read_scalar(cur).num)
        elif key == "mask_conditioning_p":
            cfg.mask_conditioning_p = Float32(_read_scalar(cur).num)
        elif key == "mask_cache_dir" or key == "mask_cache_directory":
            var sc = _read_scalar(cur)
            if sc.is_string:
        ...
        # ── LTX2 P6 AV (audio) arm (P6.0); absent -> musubi defaults -> C13 ──────
            ...
            var sc = _read_scalar(cur)
            if sc.is_string:
                cfg.audio_loss_balance_mode = sc.s
        elif key == "audio_loss_balance_beta":
            cfg.audio_loss_balance_beta = Float32(_read_scalar(cur).num)
        elif key == "audio_loss_balance_eps":
            cfg.audio_loss_balance_eps = Float32(_read_scalar(cur).num)
        elif key == "audio_loss_balance_min":
            cfg.audio_loss_balance_min = Float32(_read_scalar(cur).num)
        elif key == "audio_loss_balance_max":
            cfg.audio_loss_balance_max = Float32(_read_scalar(cur).num)
        elif key == "audio_loss_balance_ema_init":
            cfg.audio_loss_balance_ema_init = Float32(_read_scalar(cur).num)
        elif key == "audio_loss_balance_target_ratio":
            cfg.audio_loss_balance_target_ratio = Float32(_read_scalar(cur).num)
        elif key == "audio_loss_balance_ema_decay":
            cfg.audio_loss_balance_ema_decay = Float32(_read_scalar(cur).num)
        elif key == "uncertainty_lr":
            cfg.uncertainty_lr = Float32(_read_scalar(cur).num)
        elif key == "video_caption_dropout_rate":
            cfg.video_caption_dropout_rate = Float32(_read_scalar(cur).num)
        elif key == "audio_caption_dropout_rate":
            cfg.audio_caption_dropout_rate = Float32(_read_scalar(cur).num)
        elif key == "separate_audio_buckets":
            cfg.separate_audio_buckets = _read_scalar(cur).num != 0.0
        elif key == "audio_bucket_strategy":
            var scb = _read_scalar(cur)
            if scb.is_string:
                cfg.audio_bucket_strategy = scb.s
        elif key == "audio_bucket_interval":
            cfg.audio_bucket_interval = Float32(_read_scalar(cur).num)
        elif key == "min_audio_batches_per_accum":
            cfg.min_audio_batches_per_accum = Int(_read_scalar(cur).num)
        elif key == "audio_batch_probability":
            cfg.audio_batch_probability = Float32(_read_scalar(cur).num)
        elif key == "use_audio_length_mask":
            cfg.use_audio_length_mask = _read_scalar(cur).num != 0.0
        elif key == "audio_ref_use_negative_positions":
            cfg.audio_ref_use_negative_positions = _read_scalar(cur).num != 0.0
        elif key == "audio_ref_mask_cross_attention_to_reference":
            cfg.audio_ref_mask_cross_attention_to_reference = _read_scalar(cur).num != 0.0
        elif key == "audio_ref_mask_reference_from_text_attention":
            cfg.audio_ref_mask_reference_from_text_attention = _read_scalar(cur).num != 0.0
        ...
        # ── SerenityTrainer offload/checkpoint policy ──
            ...
            if sc.is_string:
                ...
                # Mirrors SerenityTrainer migration of legacy bool True -> ON.
