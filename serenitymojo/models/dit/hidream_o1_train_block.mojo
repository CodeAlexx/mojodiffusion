...
from serenitymojo.ops.tensor_algebra import reshape, add, slice, concat
...
# ══════════════════════════════════════════════════════════════════════════════
# TRUE BATCH-2 (row-stacked) plain-LoRA block forward/backward.
#
# Mirrors the b1 pair above but processes TWO samples as one row-stacked [1,2S,D]
# hidden = [seq0 | seq1]. HiDream-O1 has NO adaLN/modulation, so EVERY op except
# attention is a pure token-parallel GEMM/elementwise that row-stacks trivially at
# M=2S (the compute win + true-batch semantics). ATTENTION is per-sample: the block
# runs SDPA TWICE (one per [1,S,...] slice, each with its own prefix-causal mask),
# then concats the two attention outputs. The 7 LoRA dA/dB naturally accumulate over
# BOTH sample halves (a single M=2S GEMM sums the two rows-blocks) → the batch grad.
#
# RoPE tables are PRE-TILED by the caller to 2S rows: cos/sin = concat(sample0 rows,
# sample1 rows) along dim 0 — the same row order as hidden [seq0|seq1]. The 0.5 joint
# 2N-mean loss scale lives in the TRAINER's d_out (per-sample d scaled by 0.5 before
# this backward); the block is agnostic — it processes whatever [1,2S,D] d_out it is
# given, so b2 LoRA grad = 0.5*g0 + 0.5*g1 = mean(g0,g1) = the grad-accum=2 oracle.
# ══════════════════════════════════════════════════════════════════════════════
def hidream_o1_block_lora_forward_b2[
    S: Int, H: Int, HKV: Int, Dh: Int
](
    hidden_in: TArc,                 # [1, 2S, D]  row-stacked [seq0 | seq1]
    w: HiDreamO1BlockWeights,
    lora: HiDreamO1BlockLora,
    cos_q: Tensor, sin_q: Tensor,    # [2S*H*half]  PRE-TILED (sample0 rows then sample1)
    cos_k: Tensor, sin_k: Tensor,    # [2S*HKV*half]
    mask0: Tensor, mask1: Tensor,    # each [1,H,S,S] additive (block dtype) — per sample
    D: Int, F: Int, eps: Float32,
    ctx: DeviceContext,
) raises -> HiDreamO1BlockForward:
    """Row-stacked batch-2 sibling of hidream_o1_block_lora_forward. Same op order
    at M=2S, but the single SDPA becomes TWO per-sample calls (own mask) concatenated
    back to [1,2S,H,Dh]. Saved tape fields are all [1,2S,...]."""
    comptime n_rep = H // HKV
    comptime S2 = 2 * S
    var scale = Float32(1.0) / sqrt(Float32(Dh))
    ...
    var q_flat = _maybe_lora_apply(
        linear(normed[], w.qw[], None, ctx), normed[], lora.q, S2, ctx)
    var k_flat = _maybe_lora_apply(
        linear(normed[], w.kw[], None, ctx), normed[], lora.k, S2, ctx)
    var v_flat = _maybe_lora_apply(
        linear(normed[], w.vw[], None, ctx), normed[], lora.v, S2, ctx)

    var q4 = reshape(q_flat, [1, S2, H, Dh], ctx)
    var k4 = reshape(k_flat, [1, S2, HKV, Dh], ctx)
    var v4 = reshape(v_flat, [1, S2, HKV, Dh], ctx)
    var q_pre = TArc(q4^)
    var k_pre = TArc(k4^)
    var v = TArc(v4^)
    ...
    var k_rep = TArc(_repeat_kv(k_rope[], S2, HKV, n_rep, Dh, ctx))   # [1,2S,H,Dh]
    var v_rep = TArc(_repeat_kv(v[], S2, HKV, n_rep, Dh, ctx))

    # ── ATTENTION is PER-SAMPLE ──────────────────────────────────────────────
    # Slice [1,2S,H,Dh] into two [1,S,H,Dh] halves, attend each with its own mask,
    # concat the outputs back. Never a B=2 attention kernel (no cross-sample keys).
    var q0 = slice(q_rope[], 1, 0, S, ctx)
    var q1 = slice(q_rope[], 1, S, S, ctx)
    var k0 = slice(k_rep[], 1, 0, S, ctx)
    var k1 = slice(k_rep[], 1, S, S, ctx)
    var v0 = slice(v_rep[], 1, 0, S, ctx)
    var v1 = slice(v_rep[], 1, S, S, ctx)
    var att0 = sdpa[1, S, H, Dh](q0, k0, v0, mask0, scale, ctx)
    var att1 = sdpa[1, S, H, Dh](q1, k1, v1, mask1, scale, ctx)
    var attn = concat(1, ctx, att0, att1)                            # [1,2S,H,Dh]
    var attn_flat_t = reshape(attn, [1, S2, H * Dh], ctx)
    var attn_flat = TArc(attn_flat_t^)

    var attn_out = _maybe_lora_apply(
        linear(attn_flat[], w.ow[], None, ctx), attn_flat[], lora.o, S2, ctx)
    var hidden2 = TArc(add(hidden_in[], attn_out, ctx))

    var normed2 = TArc(rms_norm(hidden2[], w.post_ln[], eps, ctx))
    var gate_pre = TArc(_maybe_lora_apply(
        linear(normed2[], w.gw[], None, ctx), normed2[], lora.gate, S2, ctx))
    var up_pre = TArc(_maybe_lora_apply(
        linear(normed2[], w.uw[], None, ctx), normed2[], lora.up, S2, ctx))
    var act = TArc(swiglu(gate_pre[], up_pre[], ctx))
    var mlp_out = _maybe_lora_apply(
        linear(act[], w.dw[], None, ctx), act[], lora.down, S2, ctx)
    var out = TArc(add(hidden2[], mlp_out, ctx))
...
def hidream_o1_block_lora_backward_b2[
    S: Int, H: Int, HKV: Int, Dh: Int
](
    d_out: Tensor,                   # [1, 2S, D]  (per-sample 0.5-scaled grads stacked upstream)
    w: HiDreamO1BlockWeights,
    lora: HiDreamO1BlockLora,
    saved: HiDreamO1BlockSaved,      # from hidream_o1_block_lora_forward_b2 (all [1,2S,...])
    cos_q: Tensor, sin_q: Tensor,    # [2S*H*half]  PRE-TILED (same as forward)
    cos_k: Tensor, sin_k: Tensor,    # [2S*HKV*half]
    mask_f32_0: Tensor, mask_f32_1: Tensor,   # each [H*S, S] F32 — per sample
    D: Int, F: Int, eps: Float32,
    ctx: DeviceContext,
) raises -> HiDreamO1BlockGrads:
    """Row-stacked batch-2 sibling of hidream_o1_block_lora_backward. Every arm is
    the same parity-proven backward at M=2S (so the 7 LoRA dA/dB accumulate over BOTH
    sample halves = the batch grad); the single SDPA backward becomes TWO per-sample
    sdpa_backward_masked calls on the sliced saved tape (own mask), concatenating
    dQ/dK/dV back to [1,2S,H,Dh]. Slot order q,k,v,o,gate,up,down."""
    comptime n_rep = H // HKV
    comptime S2 = 2 * S
    var scale = Float32(1.0) / sqrt(Float32(Dh))
    ...
    # ── MLP branch (token-parallel at M=2S; LoRA accumulates BOTH halves) ──────
    var lb_down = _proj_lora_bwd(d_out, saved.act[], w.dw[], lora.down, S2, F, D, ctx)
    if lb_down.d_a:
        d_a[6] = lb_down.d_a.value().copy()
        d_b[6] = lb_down.d_b.value().copy()
    var sg = swiglu_backward(lb_down.d_x, saved.gate_pre[], saved.up_pre[], ctx)
    var lb_gate = _proj_lora_bwd(sg.d_gate, saved.normed2[], w.gw[], lora.gate, S2, D, F, ctx)
    if lb_gate.d_a:
        d_a[4] = lb_gate.d_a.value().copy()
        d_b[4] = lb_gate.d_b.value().copy()
    var lb_up = _proj_lora_bwd(sg.d_up, saved.normed2[], w.uw[], lora.up, S2, D, F, ctx)
    if lb_up.d_a:
        d_a[5] = lb_up.d_a.value().copy()
        d_b[5] = lb_up.d_b.value().copy()
    var d_normed2 = add(lb_gate.d_x, lb_up.d_x, ctx)
    ...
    # ── attention branch ──────────────────────────────────────────────────────
    var lb_o = _proj_lora_bwd(
        d_hidden2, saved.attn_flat[], w.ow[], lora.o, S2, H * Dh, D, ctx)
    if lb_o.d_a:
        d_a[3] = lb_o.d_a.value().copy()
        d_b[3] = lb_o.d_b.value().copy()
    var d_attn4 = reshape(lb_o.d_x, [1, S2, H, Dh], ctx)

    # SDPA backward is PER-SAMPLE: slice saved q_rope/k_rep/v_rep + d_attn4 into two
    # [1,S,...] halves, run sdpa_backward_masked per sample (own mask), then concat
    # dQ/dK/dV back to [1,2S,H,Dh]. No cross-sample mixing (per-sample keys).
    var d_att0 = slice(d_attn4, 1, 0, S, ctx)
    var d_att1 = slice(d_attn4, 1, S, S, ctx)
    var sb0 = sdpa_backward_masked[1, S, H, Dh](
        slice(saved.q_rope[], 1, 0, S, ctx), slice(saved.k_rep[], 1, 0, S, ctx),
        slice(saved.v_rep[], 1, 0, S, ctx), mask_f32_0, d_att0, scale, ctx)
    var sb1 = sdpa_backward_masked[1, S, H, Dh](
        slice(saved.q_rope[], 1, S, S, ctx), slice(saved.k_rep[], 1, S, S, ctx),
        slice(saved.v_rep[], 1, S, S, ctx), mask_f32_1, d_att1, scale, ctx)
    ...
    var d_k_rope = repeat_kv_backward(d_k_sb, S2, HKV, n_rep, Dh, ctx)
    var d_v4 = repeat_kv_backward(d_v_sb, S2, HKV, n_rep, Dh, ctx)

    var d_q_rms = rope_backward(d_q_sb, cos_q, sin_q, False, ctx)
    var d_k_rms = rope_backward(d_k_rope, cos_k, sin_k, False, ctx)

    var d_q_pre = rms_norm_backward_dx(d_q_rms, saved.q_pre[], w.q_norm[], eps, ctx)
    var d_k_pre = rms_norm_backward_dx(d_k_rms, saved.k_pre[], w.k_norm[], eps, ctx)

    var d_q_flat = reshape(d_q_pre, [1, S2, H * Dh], ctx)
    var d_k_flat = reshape(d_k_pre, [1, S2, HKV * Dh], ctx)
    var d_v_flat = reshape(d_v4, [1, S2, HKV * Dh], ctx)

    var lb_q = _proj_lora_bwd(d_q_flat, saved.normed[], w.qw[], lora.q, S2, D, H * Dh, ctx)
    if lb_q.d_a:
        d_a[0] = lb_q.d_a.value().copy()
        d_b[0] = lb_q.d_b.value().copy()
    var lb_k = _proj_lora_bwd(d_k_flat, saved.normed[], w.kw[], lora.k, S2, D, HKV * Dh, ctx)
    if lb_k.d_a:
        d_a[1] = lb_k.d_a.value().copy()
        d_b[1] = lb_k.d_b.value().copy()
    var lb_v = _proj_lora_bwd(d_v_flat, saved.normed[], w.vw[], lora.v, S2, D, HKV * Dh, ctx)
    if lb_v.d_a:
        d_a[2] = lb_v.d_a.value().copy()
        d_b[2] = lb_v.d_b.value().copy()
