#!/usr/bin/env python3
# SerenityTrainer DoRA oracle (per-INPUT magnitude axis = DoRAModule default
# decompose_output_axis=False; MJ-1023). Runs SerenityTrainer's OWN DoRAModule.forward
# and dumps the exact A,B,m,W,x it used + the output y, for the Mojo gate
# (dora_serenity_trainer_parity.mojo) to reproduce with wd_on_out=False.
#
# Run (system python3 has torch; SerenityTrainer gguf-quant import is stubbed):
#   python3 serenitymojo/training/tests/gen_dora_serenity_trainer_oracle.py
import sys, types
sys.path.insert(0, "/home/alex/SerenityTrainer")
class _Any:
...
# Grad-enabled forward + backward with a FIXED d_y so SerenityTrainer's autograd gives
# the detached-norm gradient the Mojo dora_backward must match (W frozen).
lin.weight.requires_grad_(False)
y = m.forward(x)                                        # SerenityTrainer's OWN DoRA forward [M,OUT]
...
save_file(out, "/tmp/dora_serenity_oracle.safetensors")
print("WROTE /tmp/dora_serenity_oracle.safetensors  (per-INPUT axis, alpha=2 rank=4 eps=0, +autograd grads)")
