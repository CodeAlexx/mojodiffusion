...
from serenitymojo.io.ffi import BytePtr, sys_mkdir
...
# Saved K/V of the FIXED prompt prefix (system prompt + "<|im_start|>user\n").
# ~1GB; primed once (~4.4min), then every run loads it instead (MEASURED 263s of
# prompt priming removed). Self-healing: stored prefix_ids are verified against
# the current tokenization; any mismatch (edited system prompt) → re-prime+save.
comptime PREFIX_CACHE = "/home/alex/.serenity/cache/qwen3magic_prefix_cache.safetensors"
...
def magic_expand(plain: String, aspect: String, ctx: DeviceContext) raises -> String:
    ...
    # FIXED prefix (identical across captions) vs VARIABLE tail. The split point
    # is the end of "<|im_start|>user\n" — everything before it never changes.
    var prefix_str = (
    ...
    # BPE seam check: prefix-cache reuse is only valid if the full chat's ids
    # literally START WITH the prefix's ids (a BPE merge across the boundary
    # would change them). If the seam doesn't hold, fall back to the full prime.
    var seam_ok = len(ids) > len(prefix_ids)
    if seam_ok:
        for i in range(len(prefix_ids)):
            if ids[i] != prefix_ids[i]:
                ...
                break
    if not seam_ok:
        print("[magic] prefix seam mismatch — full prime (no prefix cache)")
        var gen_full = generate_greedy_cached(qwen, ids, 1700, EOS, ctx)
        return _normalize_caption(tok.decode(gen_full))

    # Try the saved prefix K/V; verify it covers EXACTLY our prefix_ids.
    var have_cache = False
    var cache = KVCache(qwen.config.num_layers)
    try:
        var stored_ids = List[Int]()
        var lcache = load_prefix_cache(
            PREFIX_CACHE, qwen.config.num_layers, ctx, stored_ids
        )
        var ids_match = len(stored_ids) == len(prefix_ids)
        if ids_match:
            for i in range(len(prefix_ids)):
                if stored_ids[i] != prefix_ids[i]:
                    ids_match = False
                    break
        if ids_match:
            cache = lcache^
            ...
            print("[magic] prefix cache stale (prompt changed) — re-priming")
    except:
        print("[magic] no prefix cache — priming once and saving")

    if not have_cache:
        # Prime the fixed prefix once (logits discarded) and persist it.
        for pos in range(len(prefix_ids)):
            _ = decode_step(qwen, cache, prefix_ids[pos], pos, ctx, want_logits=False)
        _ = sys_mkdir(String("/home/alex/.serenity/cache"))  # -1/EEXIST is fine
        save_prefix_cache(cache, prefix_ids, PREFIX_CACHE, ctx)
        print("[magic] prefix cache saved: " + PREFIX_CACHE)

    # Variable tail = user line + assistant header; resume from the prefix.
    var tail = List[Int]()
    for i in range(len(prefix_ids), len(ids)):
        tail.append(ids[i])
    var gen = generate_greedy_cached_resume(
        qwen, cache, len(prefix_ids), tail, 1700, EOS, ctx
    )
