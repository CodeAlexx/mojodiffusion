...
from serenitymojo.serve.zimage_decode_subprocess import (
    decode_child_run as zimage_decode_child_run,
)
...
def main() raises:
    var args = argv()
    if len(args) < 2:
        print("usage: serenity_worker_zimage <fd> | encode-child <prefix> <prompt> <negative> | decode-child <latent> <rgb> <latent_h> <latent_w>")
        return
    ...
    # P?: WHOLE-image VAE decode-in-a-child-process. The parent zimage worker
    # fork+execv's THIS same binary as `decode-child` so the 1024² whole decode runs
    # in a fresh CUDA context (tiled decode degrades output — MJ-1054), then this
    # process exits and the OS reclaims the decode VRAM. See
    # serve/zimage_decode_subprocess.mojo. Routed BEFORE the fd parse because
    # Int("decode-child") would raise.
    if String(args[1]) == "decode-child":
        if len(args) < 6:
            print("usage: serenity_worker_zimage decode-child <latent_path> <rgb_out_path> <latent_h> <latent_w>")
            return
        zimage_decode_child_run(
            String(args[2]), String(args[3]), Int(String(args[4])), Int(String(args[5]))
        )
