#!/usr/bin/env python3
"""Static/header guard for Anima SerenityTrainer raw LoRA key parity."""
...
SERENITY_TRAINER_ANIMA = Path("/home/alex/SerenityTrainer-anima-ref")

SERENITY_SETUP = SERENITY_TRAINER_ANIMA / "modules/modelSetup/AnimaLoRASetup.py"
SERENITY_MODEL = SERENITY_TRAINER_ANIMA / "modules/model/AnimaModel.py"
SERENITY_SAVER = SERENITY_TRAINER_ANIMA / "modules/modelSaver/anima/AnimaLoRASaver.py"
SERENITY_LORA_MODULE = SERENITY_TRAINER_ANIMA / "modules/module/LoRAModule.py"
SERENITY_MODULE_FILTER = SERENITY_TRAINER_ANIMA / "modules/util/ModuleFilter.py"
SERENITY_CONFIG_CLASS = SERENITY_TRAINER_ANIMA / "modules/util/config/TrainConfig.py"
SERENITY_CONFIG = SERENITY_TRAINER_ANIMA / "configs/anima_100step_baseline.json"
SERENITY_BASELINE = SERENITY_TRAINER_ANIMA / "output/anima_100step_baseline/lora.safetensors"
SERENITY_METRICS_WITH_GRAD = SERENITY_TRAINER_ANIMA / "output/anima_100step_baseline/metrics_with_grad.json"
SERENITY_CACHE = SERENITY_TRAINER_ANIMA / "workspace-cache/anima_100step_baseline"
SERENITY_STEP_PARITY = REPO / "serenitymojo/models/anima/parity/anima_serenity_step_parity.mojo"
SERENITY_ARTIFACT_SMOKE = REPO / "serenitymojo/models/anima/parity/anima_train_ref_artifact_smoke.mojo"
TRAIN_REF_DUMP = Path("/home/alex/serenity_trainer-mojo/parity/anima_train_ref_step000.safetensors")
TRAIN_REF_ADAPTER_DUMP = Path("/home/alex/serenity_trainer-mojo/parity/anima_train_ref_step000_adapters.safetensors")
TRAIN_REF_META = Path("/home/alex/serenity_trainer-mojo/parity/anima_train_ref_meta.json")
...
SERENITY_MODULES = tuple(module for module, _, _ in IMPLEMENTED_MODULES)
...
class InventoryContract:
    ...
    def adapters(self) -> int:
        return self.num_blocks * len(SERENITY_MODULES)
...
def expected_key_set(contract: InventoryContract) -> set[str]:
    return {
        ...
        for module in SERENITY_MODULES
...
def _require_under_anima_ref(label: str, path: Path) -> None:
    root = SERENITY_TRAINER_ANIMA.resolve(strict=False)
    resolved = path.resolve(strict=False)
    if resolved != root and root not in resolved.parents:
        die(f"{label} must stay under {SERENITY_TRAINER_ANIMA}, got {path}")
...
def _first_scalar(events: list[dict], tag: str) -> dict:
    for event in events:
        if event.get("tag") == tag:
            return event
    die(f"missing scalar event {tag!r} in {SERENITY_METRICS_WITH_GRAD}")
...
def _require_finite_positive_scalar(event: dict, label: str) -> None:
    ...
    if event.get("global_step") != 0:
        die(f"{label} must pin the first SerenityTrainer optimizer step at global_step=0, got {event.get('global_step')!r}")


def _count_pt_files(root: Path) -> int:
    if not root.exists():
        die(f"missing SerenityTrainer cache directory: {root}")
...
def _mojo_artifact_gate_consumes_baseline() -> bool:
    smoke = read_text(SERENITY_ARTIFACT_SMOKE)
    return str(SERENITY_BASELINE) in smoke and "SafeTensors.open(path)" in smoke
...
def report_train_ref_update_status(meta: dict | None) -> None:
    if meta is None:
        print(f"[anima-lora-keys] WARN: SerenityTrainer-anima train-ref meta: MISSING {TRAIN_REF_META}")
        print("[anima-lora-keys] SerenityTrainer-anima zero-lr optimizer-state-init oracle: NOT PROVEN")
        print("[anima-lora-keys] SerenityTrainer-anima update-bearing dump: MISSING")
    ...
    print(
        "[anima-lora-keys] SerenityTrainer-anima zero-lr optimizer-state-init oracle: PRESENT "
    ...
    if _adapter_update_gate_wired():
        print(
            "[anima-lora-keys] SerenityTrainer-anima adapter update oracle gate: PRESENT "
            f"{TRAIN_REF_ADAPTER_UPDATE_GATE.relative_to(REPO)}"
        )
    else:
        print(
            "[anima-lora-keys] WARN: SerenityTrainer-anima adapter update oracle gate: MISSING "
            f"{TRAIN_REF_ADAPTER_UPDATE_GATE}"
        )

    if _has_update_bearing_step(meta):
        print("[anima-lora-keys] SerenityTrainer-anima update-bearing dump: PRESENT")
    else:
        print(
            "[anima-lora-keys] SerenityTrainer-anima update-bearing dump: MISSING "
            "(current local dump is first optimizer step with lr_before=0.0; "
            "capture a later SerenityTrainer step with lr_before > 0 and nonzero "
...
def check_reference_training_dump(strict_consumption: bool) -> None:
    cfg = read_json(SERENITY_CONFIG)
    metrics = read_json(SERENITY_METRICS_WITH_GRAD)
    ...
    if output_path != SERENITY_BASELINE:
        die(f"training dump output path {output_path} != {SERENITY_BASELINE}")
    if cache_path != SERENITY_CACHE:
        die(f"training dump cache path {cache_path} != {SERENITY_CACHE}")
    if not SERENITY_BASELINE.exists() or SERENITY_BASELINE.stat().st_size <= 8:
        die(f"missing or empty SerenityTrainer Anima LoRA dump: {SERENITY_BASELINE}")
    ...
    _require_finite_positive_scalar(first_loss, "first SerenityTrainer loss/train_step")
    _require_finite_positive_scalar(first_grad, "first SerenityTrainer grad_norm")

    image_cache = SERENITY_CACHE / "image"
    text_cache = SERENITY_CACHE / "text"
    ...
    parity = read_text(SERENITY_STEP_PARITY)
    synthetic_markers = (
        'REF_DIR = "/home/alex/mojodiffusion/serenitymojo/models/anima/parity/"',
        '_in("serenity_scaled_bthwc")',
        '_in("serenity_noise_bthwc")',
        '_in("serenity_context")',
        '_in("serenity_pred")',
        '_in("serenity_loss")',
    )
    ...
    print(
        "[anima-lora-keys] SerenityTrainer training dump evidence: PASS "
        ...
        f"final_lora={SERENITY_BASELINE.name}"
    )
    if _mojo_artifact_gate_consumes_baseline():
        print(
            "[anima-lora-keys] SerenityTrainer-anima final LoRA artifact consumption: PASS "
            f"({SERENITY_ARTIFACT_SMOKE.relative_to(REPO)} opens the final SerenityTrainer LoRA safetensors)"
        )
    else:
        print(
            "[anima-lora-keys] WARN: SerenityTrainer-anima final LoRA artifact consumption: NOT PROVEN "
            f"{SERENITY_ARTIFACT_SMOKE.relative_to(REPO)} does not open {SERENITY_BASELINE}"
        )

    if train_ref_artifacts_present:
        print(
            "[anima-lora-keys] SerenityTrainer train-ref first-step safetensors: PRESENT "
        ...
        print(
            "[anima-lora-keys] WARN: SerenityTrainer train-ref first-step safetensors: MISSING "
            f"step={TRAIN_REF_DUMP} adapters={TRAIN_REF_ADAPTER_DUMP}"
        )

    if train_ref_artifacts_present and _mojo_step_artifact_gate_consumes_train_ref():
        print(
            "[anima-lora-keys] SerenityTrainer-anima first-step artifact consumption: PASS "
        ...
        print(
            "[anima-lora-keys] WARN: SerenityTrainer-anima first-step artifact consumption: NOT PROVEN "
    ...
    if uses_repo_oracle:
        print(
            "[anima-lora-keys] WARN: current "
            f"{SERENITY_STEP_PARITY.relative_to(REPO)} still consumes repo-local serenity_*.bin "
            "oracle files generated by anima_serenity_step_oracle.py"
        )
...
def check_reference_surface(model_text: str) -> None:
    ...
    if tuple(lora_surface) != SERENITY_MODULES:
        die(f"implemented Anima LoRA surface {lora_surface!r} != {list(SERENITY_MODULES)!r}")


def check_serenity_trainer_source() -> None:
    setup = read_text(SERENITY_SETUP)
    model = read_text(SERENITY_MODEL)
    saver = read_text(SERENITY_SAVER)
    lora_module = read_text(SERENITY_LORA_MODULE)
    module_filter = read_text(SERENITY_MODULE_FILTER)
    config_class = read_text(SERENITY_CONFIG_CLASS)

    check_reference_surface(model)

    require(setup, "model.transformer_lora = LoRAModuleWrapper(", "reference trainer transformer LoRA wrapper")
    require(setup, 'model.transformer, "transformer", config, config.layer_filter.split(",")', "reference trainer transformer LoRA prefix/filter")
    require(setup, "model.transformer_lora.set_dropout", "reference trainer LoRA dropout setup")
    require(setup, "config.lora_weight_dtype.torch_dtype()", "reference trainer LoRA dtype setup")
    require(setup, 'raise NotImplementedError("Embeddings not implemented for Anima")', "reference trainer Anima embedding rejection")
    ...
    require(saver, "state_dict |= model.transformer_lora.state_dict()", "reference trainer Anima raw transformer state_dict save")
    require(saver, "state_dict |= model.lora_state_dict", "reference trainer Anima prior LoRA merge save")
    require(saver, "self._save(model, output_model_format, output_model_destination, dtype)", "reference trainer Anima saver delegates dtype/output handling")

    require(lora_module, "self.prefix = prefix + '.'", "reference trainer PeftBase dotted prefix")
    require(lora_module, "case nn.Linear():", "reference trainer Linear LoRA support")
    require(lora_module, "case nn.Conv2d():", "reference trainer Conv2d LoRA support")
    require(lora_module, "Only Linear and Conv2d are supported layers", "reference trainer unsupported module rejection")
    require(lora_module, "self.register_buffer(\"alpha\", torch.tensor(alpha))", "reference trainer alpha buffer")
    require(lora_module, "self.lora_down, self.lora_up = self.create_layer()", "reference trainer down/up layer creation")
    require(lora_module, "nn.init.kaiming_uniform_(self.lora_down.weight", "reference trainer lora_down init")
    require(lora_module, "nn.init.zeros_(self.lora_up.weight)", "reference trainer lora_up init")
    require(lora_module, "return self.orig_forward(x) + ld * (self.alpha / self.rank)", "reference trainer alpha/rank forward scale")
    require(lora_module, "prefixed_name = (self.prefix + \".\" + name)", "reference trainer wrapper prefixed module names")
    require(lora_module, "if not isinstance(child_module, Linear | Conv2d):", "reference trainer wrapper Linear/Conv2d surface filter")
    require(lora_module, "module.state_dict(prefix=module.prefix)", "reference trainer module-prefixed state_dict")
    require(lora_module, 'PeftType.LORA: ".lora_down.weight"', "reference trainer LORA rank-check suffix")
    require(lora_module, 'if name.endswith(".alpha"):', "reference trainer raw alpha dummy-module load path")

    require(module_filter, "self._pattern in module_name", "reference trainer non-regex substring layer filter")
    require(config_class, 'data.append(("peft_type", PeftType.LORA', "reference trainer default PEFT type")
    require(config_class, 'data.append(("lora_rank", 16', "reference trainer default LoRA rank")
    require(config_class, 'data.append(("lora_alpha", 1.0', "reference trainer default LoRA alpha")
    require(config_class, 'data.append(("lora_decompose", False', "reference trainer default DoRA disabled")

    if SERENITY_CONFIG.exists():
        cfg = json.loads(SERENITY_CONFIG.read_text(encoding="utf-8"))
        expected = {
            "model_type": "ANIMA",
            "training_method": "LORA",
            "train_dtype": "BFLOAT_16",
    ...
    print("[anima-lora-keys] SerenityTrainer source/save contract: PASS")
...
def check_reference_artifact_gate_source() -> None:
    smoke = read_text(SERENITY_ARTIFACT_SMOKE)
    require(smoke, str(SERENITY_BASELINE), "Mojo real SerenityTrainer Anima baseline path")
    ...
    if "DeviceContext" in smoke or "std.gpu" in smoke:
        die(f"{SERENITY_ARTIFACT_SMOKE.relative_to(REPO)} must remain a no-CUDA artifact gate")
...
def check_mojo_sources(strict_port: bool) -> None:
    ...
    serenity_module_body = _between(stack, "def _anima_serenity_module", "def _anima_serenity_prefix")
    mojo_modules = tuple(re.findall(r'return String\("([^"]+)"\)', serenity_module_body))
    if mojo_modules != SERENITY_MODULES:
        die(f"Mojo _anima_serenity_module order {list(mojo_modules)!r} != {list(SERENITY_MODULES)!r}")

    require(stack, "def save_anima_lora", "Mojo Anima product save hook")
    require(stack, "def save_anima_lora_serenity", "Mojo Anima Serenity save alias")
    require(stack, "def load_anima_lora_resume", "Mojo Anima resume loader")
    require(stack, "save_lora_serenity_trainer", "Mojo shared SerenityTrainer save call")
    require(stack, "return save_lora_serenity_trainer(_anima_serenity_named(set), path, ctx)", "Mojo raw reference trainer product save")
    require(stack, "return save_anima_lora(set, path, ctx)", "Mojo reference trainer save alias")
    require(stack, "var prefixes = anima_serenity_prefixes(num_blocks)", "Mojo raw reference trainer resume prefixes")
    require(stack, 'String("transformer.transformer_blocks.")', "Mojo reference trainer wrapper prefix")

    require(lora_save, "def save_lora_serenity_trainer", "shared SerenityTrainer saver")
    require(lora_save, 'names.append(nl.prefix + ".alpha")', "shared reference trainer alpha suffix")
    require(lora_save, 'names.append(nl.prefix + ".lora_down.weight")', "shared reference trainer down suffix")
    require(lora_save, 'names.append(nl.prefix + ".lora_up.weight")', "shared reference trainer up suffix")
    ...
    gaps: list[str] = []
    save_body = _between(stack, "def save_anima_lora", "def save_anima_lora_serenity")
    ...
    if "Tensor.from_host(al_v" in stack:
        gaps.append("Mojo Anima reference trainer save still writes F32 shaped alpha directly")
...
def _key_context(key: str, contract: InventoryContract) -> str:
    ...
    if module not in SERENITY_MODULES:
...
def main() -> int:
    ...
    parser.add_argument(
        "--use-baseline",
        action="store_true",
        help="validate the SerenityTrainer Anima 100-step baseline if present, including first-step dump evidence",
    )
    parser.add_argument(
        "--check-training-dump",
        action="store_true",
        help="validate existing SerenityTrainer Anima dump metrics/cache and report whether the Mojo reference trainer step gate consumes them",
    )
    parser.add_argument(
    ...
    check_serenity_trainer_source()
    check_mojo_sources(args.strict_port)
    if args.check_training_dump or args.strict_dump_consumption or (args.use_baseline and SERENITY_BASELINE.exists()):
    ...
    if header_path is None and args.use_baseline:
        if SERENITY_BASELINE.exists():
            header_path = SERENITY_BASELINE
        else:
            print(f"[anima-lora-keys] saved inventory: SKIP (missing {SERENITY_BASELINE})")
