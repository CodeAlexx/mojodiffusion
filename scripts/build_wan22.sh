...
pixi run scripts/mem_safe.sh mojo build --optimization-level 2 -j 1 \
    -I . -I vendor/mojo-libs \
    serenitymojo/pipeline/wan22_encode_prompt.mojo \
    -o output/bin/wan22_encode_prompt

for profile in 1280x704 704x1280 1248x704 704x1248; do
    IFS=x read -r profile_width profile_height <<<"$profile"
    pixi run scripts/mem_safe.sh mojo build --optimization-level 2 -j 1 \
        -D WAN22_WIDTH="$profile_width" \
        -D WAN22_HEIGHT="$profile_height" \
        -I . -I vendor/mojo-libs \
        -Xlinker -lm \
        serenitymojo/pipeline/wan22_encode_first_frame.mojo \
        -o "output/bin/wan22_encode_first_frame_${profile}"
done
...
pixi run scripts/mem_safe.sh mojo build --optimization-level 2 -j 1 \
    ...
    -Xlinker '$ORIGIN/../../.pixi/envs/default/lib' \
    serenitymojo/pipeline/wan22_t2v.mojo \
    -o output/bin/wan22_t2v_1280x704
...
pixi run scripts/mem_safe.sh mojo build --optimization-level 2 -j 1 \
    ...
    -Xlinker '$ORIGIN/../../.pixi/envs/default/lib' \
    serenitymojo/pipeline/wan22_t2v.mojo \
    -o output/bin/wan22_t2v_704x1280
...
pixi run scripts/mem_safe.sh mojo build --optimization-level 2 -j 1 \
    ...
    -Xlinker '$ORIGIN/../../.pixi/envs/default/lib' \
    serenitymojo/pipeline/wan22_t2v.mojo \
    -o output/bin/wan22_t2v_1248x704
...
pixi run scripts/mem_safe.sh mojo build --optimization-level 2 -j 1 \
    ...
    -Xlinker '$ORIGIN/../../.pixi/envs/default/lib' \
    serenitymojo/pipeline/wan22_t2v.mojo \
    -o output/bin/wan22_t2v_704x1248

printf '%s\n' \
    "Built output/bin/wan22_encode_prompt" \
    "Built output/bin/wan22_encode_first_frame_{1280x704,704x1280,1248x704,704x1248}" \
    "Built output/bin/wan22_prepare_fp8_cache" \
    "Built output/bin/wan22_t2v" \
    "Built output/bin/wan22_t2v_480x832" \
    "Built output/bin/wan22_t2v_1280x704" \
    "Built output/bin/wan22_t2v_704x1280" \
    "Built output/bin/wan22_t2v_1248x704" \
    "Built output/bin/wan22_t2v_704x1248"
