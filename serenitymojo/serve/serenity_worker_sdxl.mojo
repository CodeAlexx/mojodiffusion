# serenitymojo.serve.serenity_worker_sdxl — STANDALONE per-kind SDXL worker.
#
# Per-kind sibling of serenity_worker_qwenimage.mojo / serenity_worker_zimage.mojo /
# serenity_worker_ideogram4.mojo: the SAME AF_UNIX newline-JSON IPC loop, but driving
# the REAL SdxlBackend (GPU/MAX SDXL pipeline — runtime CLIP-L+G text encode + SDXL
# UNet 30-step Euler CFG denoise + LDM VAE decode). The Rust control plane
# (serenity-server) spawns one of these per resident model, identically to the
# zimage/qwenimage/ideogram4 workers: `serenity_worker_sdxl <fd>`.
#
# !!! DO NOT BUILD THIS FILE INTERACTIVELY !!!
# Like the other per-kind workers, this entrypoint imports the full SDXL GPU+MAX
...
# `pixi run build-worker-sdxl-raw` (--optimization-level 2); never build it from an
# interactive session.
#
# Invoked:  serenity_worker_sdxl <fd>
...
def _sdxl_worker_loop(mut backend: SdxlBackend, fd: Int32) raises:
    """Identical control flow to the zimage/qwenimage/ideogram4 worker loops,
    specialized to SdxlBackend. One job at a time (single-GPU contract)."""
    set_nonblock(fd)
    ...
    while True:
        ...
        while True:
            ...
            if terminal:
                ...
                # (mirrors the zimage / qwenimage / ideogram4 worker trim).
                try:
