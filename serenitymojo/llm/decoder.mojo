...
from serenitymojo.llm.sqa import sqa_gpu, sqa_device, sqa_device_par
...
def decode_step(
    ...
    `want_logits=False` fills the K/V cache but SKIPS the final norm + lm_head GEMM
    (vocab=151936-wide) and returns a [1,1,1] dummy — for PROMPT PRIMING, where all
    but the last token's logits are discarded. The cache (and thus generation) is
    bit-identical either way; only the discarded logits are skipped. Cuts the huge
    per-prompt-token lm_head GEMM (6443× for the ~6.4k-token captioner prompt)."""
    var cfg = enc.config
    var H = cfg.num_heads
    ...
    # RoPE for the SINGLE absolute position `pos` — O(half) transcendentals, not
    # the O(pos*H*half) _build_rope_tables(pos+1) full-table build that was
    # discarded except for row `pos` (the O(L²)-total host hot spot; MEASURED).
    # inv_freq is head-independent, so compute half unique cos/sin once and
    # replicate across heads. Bit-identical to the old sliced-row values (same
    # log/exp/cos/sin arithmetic, t = pos).
    var log_theta = flog(Float32(cfg.rope_theta))
    var cvals = List[Float32](); var svals = List[Float32]()
    for i in range(half):
        var exponent = -log_theta * Float32(2 * i) / Float32(dh)
        var inv_freq = fexp(exponent)
        var angle = Float32(pos) * inv_freq
        cvals.append(fcos(angle)); svals.append(fsin(angle))
    var cosq = List[Float32](); var sinq = List[Float32]()
    for _h in range(H):
        for i in range(half):
            cosq.append(cvals[i]); sinq.append(svals[i])
    var cosk = List[Float32](); var sink = List[Float32]()
    for _h in range(H_kv):
        for i in range(half):
            cosk.append(cvals[i]); sink.append(svals[i])
    var cos_q = Tensor.from_host(cosq, [H * half], dtype, ctx)
    var sin_q = Tensor.from_host(sinq, [H * half], dtype, ctx)
    var cos_k = Tensor.from_host(cosk, [H_kv * half], dtype, ctx)
    ...
    for layer in range(cfg.num_layers):
        ...
        # Device-resident single-query attention: reads the K/V cache IN PLACE,
        # no host round-trip (was _sqa_tensor, the O(L)-host-pull hot spot). L is
        # the current cache depth (cache k is [1, L, H_kv, dh]).
        var cur_L = cache.k[layer][].shape()[1]
        var attn = sqa_device_par(q, cache.k[layer][], cache.v[layer][], H, H_kv, cur_L, dh, ctx)
    ...
    # Priming (want_logits=False): cache is filled; skip the final norm + the big
    # lm_head [hidden, vocab] GEMM whose logits would be discarded. Dummy return.
    if not want_logits:
