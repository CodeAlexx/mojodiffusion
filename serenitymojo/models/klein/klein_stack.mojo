...
struct KleinStackGrads(Copyable, Movable):
    ...
    # ONLY new trained param's grad. EMPTY when no ref supplied (text-to-image
    # path) — the C13 flags-off contract keeps the whole pass byte-identical.
...
def klein_stack_forward[
    ...
    ctx: DeviceContext,
    # OPT-IN img-EDIT reference branch (models/klein/img_in_ref.mojo). When BOTH
    # ref_tokens [N_IMG,in_ch] and img_in_ref_w [D,in_ch] are supplied, the input
    # image projection becomes img = linear(noise,img_in) + linear(ref,img_in_ref)
    # — the SAME summed projection img_in_ref_forward computes (identical ops:
    # _linear_fwd + _add_lists). When absent (or img_in_ref_w==0) this is the
    # EXACT existing text-to-image path, byte-for-byte (C13 flags-off contract).
    ref_tokens: Optional[List[Float32]] = None,
    img_in_ref_w: Optional[List[Float32]] = None,
    ...
    # additive reference projection (img-EDIT). Byte-identical to img_in_ref_forward's
    # ref branch (_linear_fwd + _add_lists); skipped entirely when no ref.
    if Bool(ref_tokens) and Bool(img_in_ref_w):
        var ref_proj = _linear_fwd(
            ref_tokens.value(), img_in_ref_w.value(), N_IMG, in_ch, D, ctx
        )
        img = _add_lists(img, ref_proj)
...
def klein_stack_backward[
    ...
    D: Int, F: Int, in_ch: Int, txt_ch: Int, out_ch: Int, eps: Float32,
    ctx: DeviceContext,
    # OPT-IN img-EDIT reference branch (see klein_stack_forward). When supplied,
    # d_img_in_ref [D,in_ch] is emitted (the SAME grad img_in_ref_backward gives:
    # a plain linear_backward on the ref branch with grad_y = d_io, the grad wrt
    # the input image projection output). Absent => d_img_in_ref stays EMPTY and
    # the pass is byte-identical to the text-to-image backward.
    ref_tokens: Optional[List[Float32]] = None,
    img_in_ref_w: Optional[List[Float32]] = None,
    ...
    # d_io here IS dL/d(img input-projection output) [N_IMG,D] — the SAME
    # `d_img_act` img_in_ref_backward consumes. linear_backward on the ref branch
    # yields d_img_in_ref (the only new trained param's grad). Identical op to
    # img_in_ref_backward; skipped when no ref (d_img_in_ref stays empty).
    var d_img_in_ref = List[Float32]()
    if Bool(ref_tokens) and Bool(img_in_ref_w):
        var lref = linear_backward(
            _t(d_io, [N_IMG, D], ctx),
            _t(ref_tokens.value(), [N_IMG, in_ch], ctx),
            _t(img_in_ref_w.value(), [D, in_ch], ctx),
            N_IMG, in_ch, D, ctx,
        )
        d_img_in_ref = lref.d_w.to_host(ctx)
