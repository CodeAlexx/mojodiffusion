...
# per-adapter GPU upload/readback churn while matching PyTorch/SerenityTrainer AdamW:
...
def _adamw_host_list_f32(
    ...
    var n = len(p)
    if len(g) != n or len(m) != n or len(v) != n:
        raise Error("_adamw_host_list_f32: param/grad/m/v len mismatch")
    if t < 1:
        raise Error("_adamw_host_list_f32: t must be >= 1")
