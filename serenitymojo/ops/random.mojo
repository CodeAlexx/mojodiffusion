...
# ── uniform draw (same ChaCha stream as randn, pre-Box-Muller) ────────────────
# Returns the two Standard-f32 uniforms in [0,1) that randn feeds into Box-Muller,
# so rand_uniform shares randn's exact generator (Rust rand 0.8.5 StdRng stream).
def _std_rng_uniform_pair(seed: UInt64, pair: UInt64) -> _NormalPair:
    var p = _pcg32(seed)
    var k0 = p.word
    p = _pcg32(p.state); var k1 = p.word
    p = _pcg32(p.state); var k2 = p.word
    p = _pcg32(p.state); var k3 = p.word
    p = _pcg32(p.state); var k4 = p.word
    p = _pcg32(p.state); var k5 = p.word
    p = _pcg32(p.state); var k6 = p.word
    p = _pcg32(p.state); var k7 = p.word
    var word_pos = pair * 2
    var block = word_pos // 16
    var offset = Int(word_pos % 16)
    var w0 = _chacha12_word_from_key(k0, k1, k2, k3, k4, k5, k6, k7, block, offset)
    var w1 = _chacha12_word_from_key(k0, k1, k2, k3, k4, k5, k6, k7, block, offset + 1)
    return _NormalPair(_standard_f32(w0), _standard_f32(w1))
...
def _rand_uniform_kernel_f32(
    ...
    var i = pair_i * 2
    if i < n:
        var u = _std_rng_uniform_pair(seed, UInt64(pair_i))
        o[i] = rebind[o.element_type](lo + span * u.z0)
        if i + 1 < n:
            o[i + 1] = rebind[o.element_type](lo + span * u.z1)
...
def _rand_uniform_kernel_bf16(
    ...
    if i < n:
        var u = _std_rng_uniform_pair(seed, UInt64(pair_i))
        o[i] = rebind[o.element_type]((lo + span * u.z0).cast[DType.bfloat16]())
        if i + 1 < n:
            o[i + 1] = rebind[o.element_type]((lo + span * u.z1).cast[DType.bfloat16]())
...
def _rand_uniform_kernel_f16(
    ...
    if i < n:
        var u = _std_rng_uniform_pair(seed, UInt64(pair_i))
        o[i] = rebind[o.element_type]((lo + span * u.z0).cast[DType.float16]())
        if i + 1 < n:
            o[i + 1] = rebind[o.element_type]((lo + span * u.z1).cast[DType.float16]())


def rand_uniform(
    var shape: List[Int], seed: UInt64, dtype: STDtype,
    lo: Float32, hi: Float32, ctx: DeviceContext,
) raises -> Tensor:
    """Device Tensor of deterministic Uniform[lo, hi) samples (same ChaCha stream
    as randn). Used for PEFT kaiming_uniform LoRA init: a ~ U(-1/sqrt(in), +1/sqrt(in))
    == kaiming_uniform_(a=sqrt(5)) distributionally (torch's exact RNG isn't
    reproducible cross-runtime, but the distribution + bound match)."""
    var n = 1
    for i in range(len(shape)):
        if shape[i] <= 0:
            raise Error("rand_uniform: shape dimensions must be positive")
        n *= shape[i]
    var span = hi - lo
    var out_buf = ctx.enqueue_create_buffer[DType.uint8](n * dtype.byte_size())
    var rl = RuntimeLayout[_DYN1].row_major(IndexList[1](n))
