...
#   * models/checkpoints/*.safetensors  -> models
...
def byte_substr(s: String, start: Int, end: Int) -> String:
    ...
    return String(StringSlice(unsafe_from_utf8=Span(ptr=base, length=n)))

comptime MODEL_ROOT = "models"
comptime CHECKPOINTS_DIR = "models/checkpoints"
comptime LORAS_DIR = "models/loras"
...
def _read_text_file(path: String) raises -> String:
    ...
    _ = sys_close(fd)
    var s = String(StringSlice(unsafe_from_utf8=Span(ptr=buf, length=n)))
    buf.free()
    return s^
...
def _header_text(path: String) raises -> String:
    ...
    _ = sys_close(fd)
    var s = String(StringSlice(unsafe_from_utf8=Span(ptr=buf, length=done)))
    buf.free()
    return s^
...
def scan_checkpoints() raises -> List[ScanEntry]:
    ...
    for i in range(len(known_names)):
        var dir = String(MODEL_ROOT) + "/" + known_names[i]
        if _dir_exists(dir):
