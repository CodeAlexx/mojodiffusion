# parity_forward_64.mojo — per-stage + final parity for the Z-Image NextDiT
# forward at REAL latent resolution (HL=WL=64) vs the diffusers oracle.
#
# Run: pixi run mojo run -I . serenitymojo/models/dit/parity_forward_64.mojo
#
# Oracle dumps live in parity/parity64/*.bin (+ .shape), generated via:
#   /tmp/vae_oracle_venv/bin/python parity/gen_oracle.py 64 64 32 parity64
# DEV-ONLY: Python never runs in the shipped path. Compile-time HL/WL/CAPLEN
# must match that oracle invocation (64 64 32).
#
# 64x64 dims: img_tokens=1024 (=img_padded, no pad), cap_padded=32,
...
comptime PARITY_DIR = "/home/alex/mojodiffusion/serenitymojo/models/dit/parity/parity64"
...
def main() raises:
    ...
    print("[parity64] loading NextDiT from", XFMR_DIR)
    var model = NextDiT[HL, WL, CAPLEN].load(String(XFMR_DIR), ctx)
    print("[parity64] loaded")
