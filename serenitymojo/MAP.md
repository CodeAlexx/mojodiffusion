...
| `sampling/ltx2_request_cli.mojo` + `configs/ltx2_request_profiles.json` + `configs/ltx2_feature_adapters.json` | Pure-Mojo request-driven LTX2 adapter for `serenity.genparams.v1`: validates prompt and pre/post-connector conditioning sidecars, resolves every requested LoRA and scale, accepts the request's `model_quant`, preserves an exact request copy, and dispatches one of 21 exact AOT width/height/frame-count/FPS runners. The feature registry is the single stable-ID contract: Cinemagraph validates its I2V trigger/weight; Foley/V2A validates frozen-video/generated-audio stream strengths; reference-token IC-LoRAs remain fail-closed until their feature runner lands. Emits atomic `status.json`, hands final latents to a clean decode process, and consumes no Python product runtime. | 🟠 experimental; all 21 runners rebuilt from one source on 2026-07-25; real Cinemagraph `video-0484` and Foley/V2A `video-0481` passed, while the 20-second `video-0451` peak still does not pass the 16GB 5080 gate |
...
| `pipeline/ltx2_t2v_av_hq.mojo` | LTX2 single/staged/RefHQ runners plus the request-profile execution surface. The request path consumes runtime seed/FPS/steps/conditioning/audio/LoRA selection/quant/guidance mode and fail-loud validates Creator distilled (`euler` + `ltx2_distilled`, exactly 8 stage-1 evaluations) or dev (`res2s` + `ltx2`, 1-20 steps) before model load. I2V/V2V apply exact clean-latent masks and per-token timesteps; painted V2V guide masks preserve unpainted tokens and edit painted tokens. Stage 2 uses the official 3-step distilled schedule. Fresh request decode uses the Desktop tiled contract and streams finalized PNG chunks instead of allocating the complete movie tensor. Audio policy is explicit (`generated`, `source`, or `none`); Foley/V2A attaches only its admitted cross-modal streams and preserves the source video bitstream while muxing generated audio. Writes atomic progress and result manifests with executed sampler/scheduler, timings, geometry, frame count, duration, dtype contract, quant mode, and sampled peak VRAM. | 🟠 experimental; optimized 121-frame source encode is ~1.5 s, exact-source-video Foley/V2A passed, and stage-2 denoise still exceeds the 16GB 5080 target |
...
**Open:** real AV training data needs an audio VAE **encoder** —
models/vae/ltx2_audio_vae.mojo exposes only `LTX2AudioVaeDecoderWeights` + `decode`
(inference needs decode, training needs encode). P6.3 (independent sigma + 3 balance
modes) and P6.4 (buckets + XOR sampler) contracts are pinned and ready.
