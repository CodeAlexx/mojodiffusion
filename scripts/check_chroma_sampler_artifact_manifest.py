...
the paired OneTrainer/Mojo artifact bundle required before a Chroma "sampler
...
DEFAULT_MANIFEST = Path("/home/alex/onetrainer-mojo/parity/chroma_sampler_parity_manifest.json")
...
def _template_artifact_groups() -> dict[str, Any]:
    return {
        ...
        "paired_cfg_prediction": [
            {
                "onetrainer": "onetrainer_cfg_prediction_trajectory",
        ...
        "paired_text_conditioning": [
            ...
            {
                "onetrainer": "onetrainer_text_attention_mask",
                "mojo": "mojo_text_attention_mask",
            },
            {"onetrainer": "onetrainer_attention_mask", "mojo": "mojo_attention_mask"},
...
def required_paired_sampler_fields(target_manifest: Path = DEFAULT_MANIFEST) -> dict[str, Any]:
    return {
        "one_trainer_reference": "/home/alex/OneTrainer ChromaSampler.py, same prompt/negative/seed/resolution/steps/cfg/dtype",
...
def build_template_manifest(
    *,
    ...
    artifacts: dict[str, Any] = {
        ...
        "onetrainer_cfg_prediction_trajectory": _template_tensor_artifact(
            artifact_root, "onetrainer_cfg_prediction_trajectory", prediction_shape, dtype="F32"
        ),
    ...
    if not path.exists():
        return [
            Check(
                False,
                "manifest",
                f"missing {path}; expected paired OT/Mojo Chroma sampler evidence manifest",
            )
        ]
