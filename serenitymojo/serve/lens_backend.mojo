...
# Verbatim from inference-flame/src/bin/lens_infer.rs:246-247 (MJ-1056).
...
# degrade output (MJ-1054). BUT the Lens whole decode goes through
# KleinVaeDecoder[64,64] (the Flux2 VAE), whose monolithic 1024^2 decode is
# MEASURED to peak ~22 GB and OOM a 24 GB card at the post-DiT high-water mark
# (klein_tiled_decode.mojo / lens_tiled_decode.mojo headers) — the very reason the
# klein backend defers whole decode. So the threshold here is set ABOVE that
# measured peak (23 GiB, not the 14 GiB used for the lighter LDM VAEs): whole is
# attempted only when nearly the whole card is free, otherwise the (safe) tiled
# path runs. Tighten by measurement; lens likely belongs with the deferred
# klein-class VAEs. See MJ-1054.
comptime WHOLE_DECODE_MIN_FREE_BYTES = 23 * 1024 * 1024 * 1024  # 23 GiB (> ~22 GiB Klein peak)
...
struct LensBackend(GenBackend, Movable):
    ...
    def _decode_and_save(mut self) raises -> String:
        ...
        # Klein whole decode peaks ~22 GB (see threshold note above) so this only
        # fires on a near-empty card; otherwise the tiled path runs.
        var mem = cu_mem_get_info()
        var free_gib = Float64(mem.free_bytes) / 1073741824.0
        if mem.free_bytes > WHOLE_DECODE_MIN_FREE_BYTES:
            print("[lens] WHOLE-image Klein decode (free=", free_gib,
                  "GiB) — tiled measured to degrade output (MJ-1054)")
            var dec = KleinVaeDecoder[LH, LW].load(String(FLUX2_VAE_PATH), self.ctx)
            var wimg = dec.decode(nchw, self.ctx)  # [1,3,1024,1024]
            _save_rgb_png_with_text(wimg, png_path, self.params.params_json, self.ctx)
            return png_path
        print("[lens] FLUX.2 (Klein) VAE TILED decode FALLBACK — free=", free_gib,
