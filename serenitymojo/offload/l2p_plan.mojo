...
# context_refiner has no adaLN so it is excluded from the reference trainer LoRA filter (same
...
def build_l2p_block_plan() -> BlockPlan:
    ...
    NOT trained (no adaLN => excluded from reference trainer LoRA filter); their LoRA
