#!/usr/bin/env python3
"""SerenityTrainer adapter phase/update replay evidence gate.

This is a CPU/no-CUDA support gate. It consumes an existing SerenityTrainer adapter
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Inspect SerenityTrainer adapter before/pre/post/after update dumps."
    )
...
def inspect(args: argparse.Namespace) -> dict[str, Any]:
    ...
    report = {
        ...
        "parity_scope": (
            "SerenityTrainer adapter phase/update oracle only; not Mojo backward or "
...
def main() -> int:
    ...
    if args.require_update_bearing and not bool(report["has_update"]):
        print(
            ...
            "later SerenityTrainer step with lr_before > 0 for a nonzero update oracle.",
            file=sys.stderr,
        )
        return 2
    return 0
