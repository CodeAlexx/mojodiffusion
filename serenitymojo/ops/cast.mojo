...
def cast_tensor(
    ...
    if src == DType.bfloat16 and dst == DType.float32:
        ...
        ctx.enqueue_function[_bf16_to_f32](
        ...
        ctx.enqueue_function[_f32_to_bf16](
            X, O, Int64(n), grid_dim=grid, block_dim=_BLOCK
        )
        ...
        ctx.enqueue_function[_f16_to_f32](
            X, O, Int64(n), grid_dim=grid, block_dim=_BLOCK
        )
        ...
        ctx.enqueue_function[_f32_to_f16](
            X, O, Int64(n), grid_dim=grid, block_dim=_BLOCK
        )
        ...
        unsafe_ptr=Pointer[Scalar[DType.float32], MutAnyOrigin](
            unsafe_from_address=Int(mid_buf.unsafe_ptr().bitcast[Float32]())
        ),
        runtime_layout=mid_rl,
    )
        ctx.enqueue_function[_bf16_to_f32](
            X, M, Int64(n), grid_dim=grid, block_dim=_BLOCK
        )
        var O = LayoutTensor[DType.float16, _DYN1, MutAnyOrigin](
        unsafe_ptr=Pointer[Scalar[DType.float16], MutAnyOrigin](
            unsafe_from_address=Int(out_buf.unsafe_ptr().bitcast[Float16]())
        ...
        ctx.enqueue_function[_f32_to_f16](
            M, O, Int64(n), grid_dim=grid, block_dim=_BLOCK
        )
...
def cast_tensor_slab(
    ...
    if src == DType.bfloat16 and dst == DType.float32:
        ...
        raise Error("cast_tensor_slab: unsupported dtype pair")
    if synchronize:
        ctx.synchronize()
    return Tensor(out_buf^, x.shape(), dtype)
