# qwen25vl_parity.mojo — GPU parity driver for the Qwen2.5-VL text encoder.
#
# Loads the REAL Qwen-Image-2512 text_encoder weights, runs the Mojo encoder for
# a FIXED token-id list (must match parity/qwen25vl_oracle.py TOKEN_IDS), and
# compares per-layer hidden states + the final last_hidden_state against the
# numpy oracle dump (parity/qwen25vl_ref.jsonl) using ParityHarness (cos +
# max_abs in F64).
#
# Per-layer GPU streaming comparison (Mojo side is BF16-storage/F32-accum; the
# reference is clean-f32 numpy). Run AFTER qwen25vl_oracle.py.
#
# Run: pixi run mojo run -I . \
#        serenitymojo/models/text_encoder/parity/qwen25vl_parity.mojo
...
comptime TE_DIR = (
    "/home/alex/.cache/huggingface/hub/models--Qwen--Qwen-Image-2512/"
    "snapshots/25468b98e3276ca6700de15c6628e51b7de54a26/text_encoder"
)
comptime REF_PATH = (
...
def main() raises:
    ...
    # FIXED token ids — MUST match parity/qwen25vl_oracle.py TOKEN_IDS.
    var ids = List[Int]()
    for t in [9707, 11, 1879, 0, 358, 1079, 264, 1467]:
        ids.append(t)

    print("loading Qwen2.5-VL text_encoder weights (text path)...")
    var enc = Qwen25VLEncoder.load(String(TE_DIR), cfg, ctx)
    print("  loaded.")

    var refs = _load_refs()

    print("running encoder forward (28 layers, seq=", len(ids), ")...")
    ...
    # per-layer (the oracle emitted layer0..3 + layer27)
    var check_layers = List[Int]()
    for li in [0, 1, 2, 3, 27]:
