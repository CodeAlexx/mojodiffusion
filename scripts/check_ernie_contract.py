...
TRAIN_REF_DIR = Path("/home/alex/serenity_trainer-mojo/parity")
TRAIN_REF_PREFIX = "ernie_train_ref"

SERENITY_CONFIG = SERENITY_TRAINER / "configs/ernie_eri2_100step_baseline.json"
SERENITY_BASELINE_LORA = SERENITY_TRAINER / "output/ernie_eri2_100step_baseline/lora.safetensors"
SERENITY_METRICS = SERENITY_TRAINER / "output/ernie_eri2_100step_baseline/metrics_nocompile.json"
SERENITY_COMPILE_METRICS = SERENITY_TRAINER / "output/ernie_eri2_100step_baseline/metrics.json"
...
TRAIN = REPO / "trainer/src/serenity_trainer/trainer/train_ernie_real.mojo"

MOJO_GATES: tuple[tuple[Path, str, str], ...] = (
    (
        REPO / "trainer/smoke/ernie_train_control_wiring_smoke.mojo",
        "source-only",
    ...
    (
        ...
        "torch-oracle",
        "consumes synthetic stack_oracle .bin files, not the SerenityTrainer one-step dump",
    ),
    (
        REPO / "serenitymojo/models/ernie/parity/lora_step_smoke.mojo",
        "synthetic-step",
        "runs a tiny deterministic LoRA step from stack_oracle inputs, not the SerenityTrainer dump",
    ),
    (
        REPO / "serenitymojo/models/ernie/parity/lora_stack_parity.mojo",
        "torch-oracle",
        "consumes local lora_stack_oracle .bin files, not the SerenityTrainer one-step dump",
    ),
    (
        REPO / "serenitymojo/models/ernie/parity/ernie_lora_serenity_save_key_smoke.mojo",
        "save-key",
    ...
    (
        ...
        "opens the real SerenityTrainer Ernie step000 and adapter dumps for header and scalar-payload checks",
    ),
    (
        REPO / "serenitymojo/models/ernie/parity/stack_real_smoke.mojo",
        "real-weight-finite",
        "loads real Ernie weights for finite/OOM coverage, not the SerenityTrainer one-step dump",
    ),
    (
        Path("/home/alex/serenity_trainer-mojo/smoke/ernie_train_ref_loss_replay.mojo"),
        "external-loss-replay",
        "loss-only replay of the existing SerenityTrainer Ernie step000 artifact",
    ),
)
...
def check_serenity_baseline_status() -> None:
    ...
    if mismatches:
        print(f"[ernie-train-ref] FAIL SerenityTrainer Ernie baseline config: {SERENITY_CONFIG}")
        for mismatch in mismatches:
            print(f"  {mismatch}")
        raise SystemExit(1)

    require_file(SERENITY_BASELINE_LORA)
    metrics = read_json(SERENITY_METRICS)
    ...
    print(
        "[ernie-train-ref] PASS SerenityTrainer 100-step baseline "
        f"steps={metrics['global_steps_seen']} loss={last_loss.get('value')} "
        f"grad_norm={last_grad_norm.get('value')} lora={SERENITY_BASELINE_LORA}"
    )

    if SERENITY_COMPILE_METRICS.exists():
        compile_metrics = read_json(SERENITY_COMPILE_METRICS)
...
def check_train_ref_json_contract() -> None:
    ...
    if dry.get("serenity_trainer") != str(SERENITY_TRAINER):
        raise SystemExit(f"[ernie-train-ref] FAIL blocker serenity_trainer path: {dry.get('serenity_trainer')!r}")
...
def check_train_ref_meta() -> TrainRefStepStatus:
    ...
    if meta.get("serenity_trainer") != str(SERENITY_TRAINER):
        raise SystemExit(f"[ernie-train-ref] FAIL meta serenity_trainer path: {meta.get('serenity_trainer')!r}")
...
def check_train_ref_artifacts(
    ...
    check_serenity_baseline_status()
...
def main() -> int:
    ...
    parser.add_argument(
        ...
        action="store_true",
        help="skip local SerenityTrainer one-step dump artifact/header checks",
    )
    parser.add_argument(
