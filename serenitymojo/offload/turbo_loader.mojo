...
struct TurboBlockLoader(Movable):
    ...
    # Whether the persistent pinned block store was filled at open. False when
    # the caller intends FULL device residency (pin_residents_fp8 of every
    # block, MJ-1065): the store would be ~whole-DiT bytes of PINNED host RAM
    # that is never read again once every block is resident — two concurrent
    # 17 GB stores OOM-killed the user session (systemd-oomd, 2026-07-04).
    # When False, prefetch falls back to mmap -> pinned slot slab.
    var use_block_store: Bool
    ...
    def open_with_copy_mode(
        ...
        var use_block_store = TURBO_USE_PERSISTENT_BLOCK_STORE and fill_block_store
        var block_store: HostBuffer[DType.uint8]
        if use_block_store:
    ...
    def __init__(
        ...
        self.use_block_store = use_block_store
