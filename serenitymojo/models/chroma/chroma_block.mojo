...
#   Chroma's SerenityTrainer/diffusers target set (chroma.rs:44-64). So the Chroma
...
# FULL-FINETUNE device block backwards (full-FT rollout item 3, 2026-07-07).
# Like the host backwards above, the FT math is 100% flux-shared, so the _dev
# implementations live FLUX-side (models/flux/flux_block_ft.mojo — which is
# simultaneously flux P1) and chroma re-exports. The trained surface is GATED
# by the SURFACE_V2 comptime param (gate-don't-fork): False = the v1 matmul-
# only surface (flux's current arm), True = matmuls + ALL linear biases + q/k
# rms-norm scales (FULL_SURFACE_PLAN_2026-07-08 Phase B row 3 — chroma's v2
# arm; the FT_*_SLOT_* constants below fix the dw/store/af-state order).
# Chroma's mod vecs stay frozen under BOTH (genuinely frozen: distilled
# approximator rows — a Phase C candidate). Gated by
# models/chroma/parity/chroma_block_ft_parity.mojo vs the base-only oracle mode
# (CHROMA_FT_ORACLE=1 chroma_block_oracle.py).
from serenitymojo.models.flux.flux_block_ft import (
    FluxDoubleBlockFTGrads as ChromaDoubleBlockFTGrads,
    FluxSingleBlockFTGrads as ChromaSingleBlockFTGrads,
    flux_double_block_ft_backward_dev as chroma_double_block_ft_backward_dev,
    flux_single_block_ft_backward_dev as chroma_single_block_ft_backward_dev,
    # v2 full-surface slot indices (shared flux/chroma order)
