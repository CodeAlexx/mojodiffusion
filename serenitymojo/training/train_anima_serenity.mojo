# serenitymojo/training/train_anima_serenity.mojo
#
# ANIMA LoRA TRAINING — SerenityTrainer-FAITHFUL recipe (Chunk A).
#
# This is train_anima_real.mojo RE-TARGETED to the SerenityTrainer reference
# (/home/alex/SerenityTrainer-anima-ref), changing EXACTLY the 5 recipe deltas, each
# cited to reference trainer source. The Anima block/stack/attn-mlp-LoRA math is REUSED unchanged
# (parity-gated cos>=0.99999999). The original train_anima_real.mojo is left intact.
#
# THE 5 reference trainer DELTAS (vs the inference-flame-anchored train_anima_real.mojo):
...
#     (proven to compile+run at 512 by parity/anima_serenity_step_parity.mojo).
...
#  3. reference trainer discrete timestep + sigma.  _get_timestep_discrete for the LOGIT_NORMAL
...
#     the old trainer passed RAW sigma; reference trainer passes timestep/1000).
...
#       serenitymojo/training/train_anima_serenity.mojo -o /tmp/train_anima_serenity
#   /tmp/train_anima_serenity
...
from serenitymojo.models.anima.anima_stack_lora import (
    ...
    anima_lora_adamw_step, save_anima_lora, save_anima_lora_serenity,
...
# DELTA 1: reference trainer context length 256 -> 512 (AnimaModel.py:23 PROMPT_MAX_LENGTH=512).
comptime S_TXT = 512
...
# DELTA 3: reference trainer discrete timestep schedule constants.
...
# at any grid). The real cache (anima_prepare_serenity) stores a [1,16,32,32] latent.
# 512px training: latent 64x64 -> S_IMG = (64/2)^2 = 1024 (the reference trainer 512px target).
...
# fixed target). The REAL reference trainer schedule (per-step logit-normal discrete timestep) runs
# when this is False.
# FIXED_STEP_SMOKE: hold (timestep, noise) constant so the flow target is identical
# every step — isolates the LoRA learning signal (loss MUST fall as adapters fit a
# fixed target). False = the REAL reference trainer per-step logit-normal discrete-timestep schedule
...
comptime LORA_OUT = "/home/alex/mojodiffusion/output/anima_lora_serenity_real.safetensors"
comptime LORA_OUT_SERENITY = "/home/alex/mojodiffusion/output/anima_lora_serenity_real_keys.safetensors"
...
# ── DELTA 3: reference trainer discrete timestep for the LOGIT_NORMAL distribution ───────────
...
def _serenity_timestep_discrete(seed: UInt64) -> Int:
...
# token index) was reference trainer-UNFAITHFUL (cos≈0.71 vs the real 3-axis table); replaced.
...
def _zero_b_set(set: AnimaLoraSet) -> AnimaLoraSet:
    ...
    for i in range(len(set.ad)):
        ...
        for j in range(len(src.b)):
            src.b[j] = BFloat16(0.0)  # zero the up-projection -> scale·B·A == 0
        ad.append(src^)
...
def _anima_sample_shift(
    ...
    while i < n_steps:
        ...
        # discrete ts for the t-embedder: ts/1000 == sigma-ish; reference trainer passes ts/1000.
...
def main() raises:
    ...
    # Schedule mode: "real" (default) = reference trainer per-step logit-normal; "fixed" = fixed-σ
    ...
    print("==== train_anima_serenity — ANIMA LoRA Serenity recipe training run ====")
    print("schedule:", "FIXED-sigma smoke" if fixed_step else "REAL reference trainer logit-normal",
    ...
    var ctx_n = len(context_raw)
    # The captured sidecar may be [1,256,1024]; reference trainer needs [1,512,1024]. If shorter,
    ...
    for step in range(start_step, run_steps):
        ...
        # ── DELTA 3: reference trainer discrete timestep -> sigma = (ts+1)/N ──
        var ts = FIXED_TIMESTEP
        if not fixed_step:
            var ts_seed = SEED * UInt64(7919) + UInt64(step) * UInt64(2654435761)
            ts = _serenity_timestep_discrete(ts_seed)
        ...
        if dataset_mode and ((step + 1) % ckpt_every == 0):
            ...
            var _no = save_anima_lora_serenity(lora, ALPHA, ckp[1], ctx)
            var _ns = save_anima_lora_state(lora, ckp[2], ctx)
            print("CKPT step=", step + 1, " saved PEFT=", ckp[0],
                  " reference trainer=", ckp[1], " opt=", ckp[2],
    ...
    # ── FINAL giger3 checkpoint (dataset mode): PEFT + reference trainer + Adam-state sidecar at
