# Smoke gate for the SerenityTrainer sampler intent map.
...
from serenitymojo.sampling.serenity_trainer_sampler_contract import (
    SERENITY_CFG_GUIDANCE_EMBED,
    SERENITY_CFG_GUIDANCE_EMBED_OR_TEXTBOOK,
    SERENITY_CFG_TEXTBOOK,
    SERENITY_SCHEDULER_DIFFUSERS_CREATE,
    SERENITY_SCHEDULER_FLUX2_MU_CUSTOM_SIGMAS,
    SERENITY_SCHEDULER_MODEL_COPY_CUSTOM_SIGMAS,
    SERENITY_SCHEDULER_MODEL_COPY_MU,
    SERENITY_SCHEDULER_MODEL_COPY_SET_TIMESTEPS,
    ...
    SERENITY_TIMESTEP_DIV_1000,
    SERENITY_TIMESTEP_RAW,
    default_serenity_sampler_plan,
    serenity_sampler_family,
    serenity_sampler_plan_with_sample_overrides,
...
def _check(cond: Bool, msg: String) raises:
    if not cond:
        raise Error(String("serenity_trainer sampler contract smoke FAILED: ") + msg)
...
def _check_plan(
    ...
    validate_serenity_sampler_plan(plan)
...
def _expect_unsupported(model_type: String) raises:
    ...
    if not raised:
        raise Error(String("serenity_trainer sampler contract smoke expected block for ") + model_type)


def main() raises:
    print("==== SerenityTrainer sampler contract smoke ====")
    ...
    var over = serenity_sampler_plan_with_sample_overrides(
    ...
    _check(serenity_sampler_family(String("klein9b")) == SERENITY_SAMPLER_FLUX2_KLEIN, "klein alias")
    _check(serenity_sampler_family(String("flux2_dev")) == SERENITY_SAMPLER_FLUX2_DEV, "Flux2 dev alias")

    print(sampler_contract_summary(default_serenity_sampler_plan(String("FLUX_2_DEV"))))
    print(sampler_contract_summary(default_serenity_sampler_plan(String("FLUX_2"))))
    print("SerenityTrainer sampler contract smoke PASS")
