...
`sdpa_backward_scratch` is the SerenityTrainer-style allocator variant. It returns
...
modeled after the local SerenityTrainer static activation/layer allocator pattern
(`SerenityTrainer/docs/RamOffloading.md` and
`SerenityTrainer/modules/util/LayerOffloadConductor.py`):
