# sampling/qwenimage_sampling.mojo - Qwen-Image SerenityTrainer sampler helpers.
#
# SerenityTrainer's Qwen sampler calls FlowMatchEulerDiscreteScheduler.set_timesteps(
...
def qwenimage_packed_seq_len(latent_width: Int, latent_height: Int) raises -> Int:
    """Qwen latent token count used by SerenityTrainer calculate_timestep_shift."""
...
def qwenimage_dynamic_shift_value(seq_len: Float32) -> Float32:
    """SerenityTrainer QwenModel.calculate_timestep_shift return value: exp(mu)."""
...
def build_qwenimage_serenity_trainer_sigmas(
    num_steps: Int, seq_len: Float32
) raises -> List[Float32]:
    """SerenityTrainer Qwen-Image set_timesteps(num_steps, mu=...) sigma schedule."""
    if num_steps <= 0:
        raise Error("build_qwenimage_serenity_trainer_sigmas: num_steps must be > 0")
...
struct QwenImageFlowMatchScheduler(Movable):
    ...
    def __init__(out self, num_steps: Int, seq_len: Float32) raises:
        self._sigmas = build_qwenimage_serenity_trainer_sigmas(num_steps, seq_len)
