...
def run(
    ...
    # NEW right-padded dumps (video_len key present): real tokens occupy the
    # HEAD -> slice [0, N_TXT) and pass valid_len so pads become the
    # connector's learnable registers (reference semantics). Legacy left-padded
    # dumps (no key): real tokens occupy the TAIL -> keep the tail slice with
    # the legacy no-replacement connector behavior.
    var ctx_valid = -1
    var slice0 = 1024 - N_TXT
    if _st_has(dump, String("video_len")):
        var lh = Tensor.from_view(dump.tensor_view("video_len"), ctx).to_host(ctx)
        ctx_valid = Int(lh[0])
        if ctx_valid > N_TXT:
            ctx_valid = N_TXT
        slice0 = 0
        print("  [ctx] right-padded dump, valid rows:", ctx_valid, "/", N_TXT)
    var video_pre = slice(vctx_full, 1, slice0, N_TXT, ctx)  # [1,N_TXT,4096]
    var audio_pre = slice(actx_full, 1, slice0, N_TXT, ctx)  # [1,N_TXT,2048] REAL
