# flux_lora_serenity_save_key_smoke.mojo -- Flux SerenityTrainer save-key contract.
#
...
# safetensors header and raw reference trainer alpha/down/up keys. No model weights or denoise.
...
from serenitymojo.models.flux.flux_stack_lora import (
    build_flux_lora_set,
    flux_lora_missing_serenity_transformer_prefixes,
    flux_lora_serenity_transformer_prefixes,
    flux_lora_prefixes,
    load_flux_lora_resume,
    require_flux_lora_serenity_transformer_complete,
...
comptime OUT = "/tmp/flux_lora_serenity_save_key_smoke.safetensors"
...
def _require_raises_transformer_complete() raises:
    var raised = False
    try:
        require_flux_lora_serenity_transformer_complete(1, 1)
    except:
        raised = True
    _require(raised, String("expected incomplete Flux reference trainer transformer surface to raise"))
...
def main() raises:
    ...
    var serenity_full = flux_lora_serenity_transformer_prefixes(1, 1)
    _require(len(serenity_full) == 30, String("expected 30 reference trainer transformer prefixes for 1 double + 1 single"))
    var missing = flux_lora_missing_serenity_transformer_prefixes(1, 1)
    _require(len(missing) == 13, String("expected 13 fail-loud missing Flux reference trainer prefixes"))
    _require_exact_missing_prefixes(missing)
    for i in range(len(supported)):
        _require_contains(serenity_full, supported[i], "full reference trainer transformer inventory")
        _require_not_contains(missing, supported[i], "fail-loud missing inventory")
    for i in range(len(missing)):
        _require_contains(serenity_full, missing[i], "full reference trainer transformer inventory")
    ...
    print("[flux-lora-reference-save-key] PASS:", OUT)
