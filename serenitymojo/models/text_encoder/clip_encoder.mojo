...
def _quick_gelu(x: Tensor, ctx: DeviceContext) raises -> Tensor:
    ...
    if dt == DType.float32:
        ...
        ctx.enqueue_function[_quick_gelu_kernel_f32](
        ...
        ctx.enqueue_function[_quick_gelu_kernel_bf16](
            X, O, Int64(n), grid_dim=grid, block_dim=_BLOCK
        )
    else:
        ...
        ctx.enqueue_function[_quick_gelu_kernel_f16](
            X, O, Int64(n), grid_dim=grid, block_dim=_BLOCK
        )
    ctx.synchronize()
...
struct ClipEncoder:
    ...
    def _embed(self, ids: List[Int], ctx: DeviceContext) raises -> Tensor:
        ...
        unsafe_ptr=Pointer[Scalar[DType.float32], MutAnyOrigin](
            unsafe_from_address=Int(tok_table.buf.unsafe_ptr().bitcast[Float32]())
        ),
        runtime_layout=tok_rl,
    )
            var PO = LayoutTensor[DType.float32, _DYN1, MutAnyOrigin](
        unsafe_ptr=Pointer[Scalar[DType.float32], MutAnyOrigin](
            unsafe_from_address=Int(pos_table.buf.unsafe_ptr().bitcast[Float32]())
        ),
        runtime_layout=pos_rl,
    )
            var O = LayoutTensor[DType.float32, _DYN1, MutAnyOrigin](
        unsafe_ptr=Pointer[Scalar[DType.float32], MutAnyOrigin](
            unsafe_from_address=Int(out_buf.unsafe_ptr().bitcast[Float32]())
        ...
        runtime_layout=out_rl,
            ...
            ctx.enqueue_function[_embed_kernel_f32](
                TK, PO, IDS, O, Int32(seq), Int32(hidden), grid_dim=grid, block_dim=_BLOCK
            )
        elif dt == DType.bfloat16:
            var TK = LayoutTensor[DType.bfloat16, _DYN1, MutAnyOrigin](
        unsafe_ptr=Pointer[Scalar[DType.bfloat16], MutAnyOrigin](
            unsafe_from_address=Int(tok_table.buf.unsafe_ptr().bitcast[BFloat16]())
        ),
        runtime_layout=tok_rl,
    )
            var PO = LayoutTensor[DType.bfloat16, _DYN1, MutAnyOrigin](
        unsafe_ptr=Pointer[Scalar[DType.bfloat16], MutAnyOrigin](
            unsafe_from_address=Int(pos_table.buf.unsafe_ptr().bitcast[BFloat16]())
        ),
        runtime_layout=pos_rl,
    )
            var O = LayoutTensor[DType.bfloat16, _DYN1, MutAnyOrigin](
        ...
        unsafe_ptr=Pointer[Scalar[DType.float16], MutAnyOrigin](
            unsafe_from_address=Int(tok_table.buf.unsafe_ptr().bitcast[Float16]())
        ),
        runtime_layout=tok_rl,
    )
            var PO = LayoutTensor[DType.float16, _DYN1, MutAnyOrigin](
        unsafe_ptr=Pointer[Scalar[DType.float16], MutAnyOrigin](
            unsafe_from_address=Int(pos_table.buf.unsafe_ptr().bitcast[Float16]())
        ),
        runtime_layout=pos_rl,
    )
            var O = LayoutTensor[DType.float16, _DYN1, MutAnyOrigin](
    ...
    def encode_sdxl[
        ...
        penultimate_context: Bool = False,
    ) raises -> Tuple[Tensor, Tensor]:
        """penultimate_context=True is the diffusers-SDXL convention (MJ-1061):
        context = hidden_states[-2] (output of layer num_layers-2, NO final LN)
        and a CAUSAL-ONLY mask (diffusers passes no attention_mask, so pad rows
        carry real EOS-propagated activations that the UNet cross-attends).
        Measured 2026-07-02: the old post-final-LN context is 0.727/0.680 row-cos
        off the HF penultimate reference. Default False preserves the historical
        behavior for flux/sd3/parity-gate consumers. Pooled is unchanged either
        way (full stack + final LN @ EOS; measured correct at cos 1.000)."""
        var cfg = self.config
        ...
        # combined causal + key-padding mask [1, H, S, S]. In penultimate/diffusers
        # mode the key-padding half is dropped (valid_key_end=S -> causal-only).
        var mask_valid_end = S if penultimate_context else real_eos
        var mask_data = _build_pad_mask(S, cfg.num_heads, mask_valid_end)
        var msh = List[Int]()
        msh.append(1)
        ...
        var penult = List[ArcPointer[Tensor]]()
        for i in range(cfg.num_layers):
            hidden = self._layer[S](i, hidden, mask, ctx)
            if penultimate_context and i == cfg.num_layers - 2:
                penult.append(ArcPointer(_clone(hidden, ctx)))
        ...
        if penultimate_context:
            var context = _clone(penult[0][], ctx)
