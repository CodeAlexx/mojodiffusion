...
- `models/vae/wan22_decoder.mojo` `Wan22VaeImageDecoder.load(path, ctx, f32: Bool=False)` + `compute_dtype` field — rank-5 conv3d weights now load FCQRS (`_load_conv3d_fcqrs(tv, out_dtype, ctx)`, same F16/F32/BF16→F32→out_dtype repack, no transpose) and the 4 causal-conv sites call `conv3d_fcqrs_cudnn` (cuDNN) → **7.3× decode** (513s→71s/13f), cos 0.99998/PSNR 51dB vs the naive `conv3d`/`_load_conv3d_qrscf` path; resample conv2d stays naive (`_load_conv2d_qrscf`). Decode latent-cast follows `compute_dtype`; default BF16 keeps the Lance T2V path. Gate cos 0.99999992 (F32, max_abs 0.0025) vs 0.99998 (bf16, 0.046).
...
Per-block + multi-block LoRA forward/backward for the Z-Image NextDiT trainer (7 slots: to_q/k/v/out + SwiGLU w1/w3/w2; base projection weights frozen). Math is **measured-authoritative vs the REAL diffusers `ZImageTransformerBlock`** (the class SerenityTrainer `ZImageModel` uses), not just self-consistent.
- `zimage_block_lora_forward[H,Dh,S](x, w, mv, lora: ZImageBlockLora, cos, sin, D,F,eps, ctx) -> ZImageBlockForwardLora` (.out, .saved); `zimage_block_lora_backward[...](d_out, ...) -> ZImageBlockLoraBackward` (.base: ZImageBlockGrads, .lora: ZImageBlockLoraGrads with `d_a`/`d_b` List[List[F32]] indexed by SLOT_Q..SLOT_W2). LoRA delta == SerenityTrainer `LoRAModule.forward`: `orig + scale·(x@Aᵀ)@Bᵀ`, A=[rank,in] B=[out,rank], scale=alpha/rank.
...
FLUX.2 Klein DiT scaffold. Reference = `/home/alex/EriDiffusion/inference-flame/src/models/klein.rs` plus Modular `architectures/flux2`. Real-weight 9B transformer path with a fast truncated smoke, a complete all-resident 8+24-block tiny-token smoke, and a block-streamed 1024 forward used by the image smoke. That image smoke is not SerenityTrainer `CPU_OFFLOADED` activation/layer parity, training backward parity, or speed/VRAM parity.
...
Ideogram-4 single-stream DiT (inference). Reference = diffusers `/home/alex/ideogram4-ref/src/ideogram4/modeling_ideogram4.py` (NOT SerenityTrainer). Weight-only FP8: Linear `.weight` is F8_E4M3 + per-row `.weight_scale` → BF16 via `ops/fp8.load_fp8_dequant`; bias/norm/embed stay BF16. Blocks loaded per-layer to bound VRAM. Config: hidden 4608, 34 layers, 18 heads, head_dim 256, adaln_dim 512, llm_dim 53248.
...
Joint MMDiT block + multi-block LoRA stack for the SD3.5 trainer (8 slots/block: ctx+x × qkv/proj/fc1/fc2; base weights frozen). Math is **measured-authoritative vs the REAL diffusers `JointTransformerBlock`** (the class SerenityTrainer `SD3Transformer2DModel` uses), not just self-consistent.
...
### `models/lingbotvideo/vae_encoder.mojo` — `LingBotWanVaeEncoder[H,W]` ✅ (Wan2.1 3D causal VAE encode; added 2026-07-11)
The encode side of the LingBot Wan2.1 VAE (z_dim=16, F32 throughout). `load(path, ctx)` reads `encoder.*`+`quant_conv.*` (5D OIDHW==FCQRS via `conv3d_fcqrs_cudnn`). Three entry points share one temporal driver (`_encode_video_raw32` = normalize→chunked encoder→quant_conv, returning the raw 32-ch NDHWC latent; the tail does `mode()`=first-16 + `(mu-mean)*inv_std`):
- `encode(pixel, ctx) raises -> Tensor` — image mode T=1, `[1,3,1,H,W]`→`[1,16,1,H/8,W/8]`. cos 0.99999999 vs diffusers.
- `encode_video(clip, ctx) raises -> Tensor` — temporal video encode: causal `WanFeatCache` threaded through every conv incl. temporal downsample (`time_conv` stride-2 at down_blocks 5/8); chunked (first frame + 4-frame chunks, cache persists). `[1,3,T,H,W]`→`[1,16,Tlat,H/8,W/8]`. **cos 0.99999998** (gate `parity/vae_encode_video_probe.mojo`).
- `encode_video_tiled(clip, ctx) raises -> Tensor` — spatial tiling (ports diffusers `tiled_encode`): 256px tiles / 192 stride, per-tile temporal encode, host `blend_v`/`blend_h` 8-latent crossfade, assemble+crop. Keeps each encoder pass ≤256² so high-res fits 16GB (full-frame 1152×640 OOMs). Mid-attn uses a runtime→comptime `_mid_attn`/`_attn_seq[SEQ]` dispatch. **cos 0.9999999857** (gate `parity/vae_encode_tiled_probe.mojo`, `enable_tiling` oracle).

### `models/lingbotvideo/lingbot_image_preprocess.mojo` — condition-image + video resize ✅
- `load_condition_image(path, H, W, ctx, drop_alpha=False) raises -> Tensor` — decode→cover-resize+crop→`[1,3,1,H,W]` in [0,1]. `drop_alpha=True` = PIL `convert("RGB")` (drops alpha) vs the default composite-over-white.
- `resize_video_bicubic(clip, oh, ow, ctx) raises -> Tensor` — PIL-exact per-frame bicubic of NCTHW `[1,3,T,ih,iw]`→`[1,3,T,oh,ow]`, clamp [0,1] (mirrors creator `resize_video_tensor`; the super-res upscale step).
...
Shared SerenityTrainer FLUX.2 dev/Klein flow-matching scalar schedule and per-step
...
- `build_flux2_sigma_schedule(num_steps, image_seq_len) raises -> List[Float32]` — SerenityTrainer `np.linspace(1.0, 1/num_steps, num_steps)` plus terminal `0.0`, shifted by dynamic empirical `mu`.
...
- `flux2_scheduler_timestep_from_sigma(sigma) -> Float32` — Diffusers scheduler timestep value before SerenityTrainer `/ 1000`.
...
- `flux2_cfg_batch_size(guidance_scale, guidance_embeds) -> Int` and `flux2_guidance_embed_value(cfg_scale) -> Float32` — SerenityTrainer true-CFG versus guidance-embed branch.
