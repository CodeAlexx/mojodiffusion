...
FT_MODE = os.environ.get("KREA2_FT_ORACLE") == "1"


def lin_lora(x, W, A, B):
    # KREA2_FT_ORACLE=1 (full-FT campaign 2026-07-07): plain base linear, NO
    # LoRA delta — the FT backward has no adapter dX contribution, so the FT
    # reference must exclude it too. Same seeds/inputs; refs dumped kref_ft_*.
    if FT_MODE:
        return x @ W.T
...
def main():
    ...
    PREF = "kref_ft_" if FT_MODE else "kref_"
    ...
    # base WEIGHT grads (full-FT campaign 2026-07-07): in FT_MODE these gate
    # the FT backward's dW arms end-to-end (no LoRA in the graph).
    for nm in names_io:
        Wb(PREF + ("%s_dW" % nm), W[nm].grad)
    if FT_MODE:
        # v2 FULL SURFACE (FULL_SURFACE_PLAN_2026-07-08 Phase B row krea2):
        # the 4 rms-norm raw scales + the mod.lin 1D parameter (mmdit.py
        # DoubleSharedModulation: out = vec + self.lin — a [6F] Parameter,
        # NOT a Linear; no weight/bias split exists). These params already
        # require grad above; dump their grads for the Mojo FT gate's arms.
        Wb(PREF + "qnorm_dW", qnorm.grad)
        Wb(PREF + "knorm_dW", knorm.grad)
        Wb(PREF + "prenorm_dW", prenorm.grad)
        Wb(PREF + "postnorm_dW", postnorm.grad)
        Wb(PREF + "mod_lin_dW", mod_lin.grad)
