# qwen_train_ref_artifact_smoke.mojo -- Qwen SerenityTrainer one-step artifact gate.
#
# Header-only, no-CUDA gate. It opens the real local SerenityTrainer Qwen one-step
...
# qwen_lora_adamw_step(...). This no-CUDA smoke consumes the SerenityTrainer artifacts
...
comptime STEP_DUMP = "/home/alex/serenity_trainer-mojo/parity/qwen_train_ref_step000.safetensors"
comptime ADAPTER_DUMP = "/home/alex/serenity_trainer-mojo/parity/qwen_train_ref_step000_adapters.safetensors"
comptime META_JSON = "/home/alex/serenity_trainer-mojo/parity/qwen_train_ref_meta.json"
...
def _read_file(path: String) raises -> List[UInt8]:
    ...
    if fd < 0:
        raise Error(String("failed to open Qwen reference trainer artifact: ") + path)
    var out = List[UInt8]()
    ...
    while True:
        ...
        if n < 0:
            buf.free()
            _ = sys_close(fd)
            raise Error(String("failed to read Qwen reference trainer artifact: ") + path)
        if n == 0:
            break
        for i in range(n):
            out.append(buf[i])
    ...
    buf.free()
    _ = sys_close(fd)
    _require(len(out) > 0, String("empty Qwen reference trainer artifact: ") + path)
    return out^
...
def _require_contains(haystack: List[UInt8], needle: String) raises:
    _require(
        ...
        String("Qwen reference trainer meta missing substring: ") + needle,
    )
...
def _require_tensor(
    ...
    _require(key in st.tensors, String("missing Qwen reference trainer tensor ") + key)
    var info = st.tensor_info(key)
    _require(
        info.dtype == dtype,
...
def _check_meta_json() raises:
    ...
    _require_contains(meta, String('"safetensors": "/home/alex/serenity_trainer-mojo/parity/qwen_train_ref_step000.safetensors"'))
    _require_contains(meta, String('"adapter_safetensors": "/home/alex/serenity_trainer-mojo/parity/qwen_train_ref_step000_adapters.safetensors"'))

    # SerenityTrainer zero-lr state-init evidence, not nonzero AdamW update parity:
