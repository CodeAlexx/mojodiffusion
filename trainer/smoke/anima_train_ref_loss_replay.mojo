# Replay the real Serenity Anima train-step dump through the Mojo loss path.
#
...
# claim Anima transformer forward, backward, optimizer, or sampler parity.
...
comptime SERENITY_LOSS = Float32(0.0667838305234909)
...
def main() raises:
    ...
    print("=== Anima train-ref loss replay ===")
    print("[parity]", PARITY)
    ...
    if err > LOSS_EPS or err_const > LOSS_EPS:
        raise Error("Anima train-ref loss replay mismatch")

    print("ANIMA TRAIN REF LOSS REPLAY PASS")
