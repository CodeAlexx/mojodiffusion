...
struct ShardedSafeTensors(Movable):
    ...
    def has_tensor(self, name: String) -> Bool:
        """True if `name` is present (non-raising existence check; used e.g. to
        detect an mxfp4 `<name>_blocks` sidecar vs a plain/fp8 weight)."""
        return name in self.name_to_shard
