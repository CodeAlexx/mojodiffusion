...
def write_speed_json(
    ...
    *,
    ...
    serenity_trainer_speed_json: Path | None,
    ...
    data: dict[str, Any] = {
        ...
        "note": "Mojo-side supervisor evidence only unless paired SerenityTrainer fields are supplied and strict-speed passes.",
    }
    ...
    if serenity_trainer_speed_json is not None:
        serenity_data = load_json(serenity_trainer_speed_json)
        if isinstance(serenity_data.get("serenity_trainer"), dict):
            data["serenity_trainer"] = serenity_data["serenity_trainer"]
        elif isinstance(serenity_data.get("reference"), dict):
            data["serenity_trainer"] = serenity_data["reference"]
        else:
            data["serenity_trainer"] = serenity_data
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--serenity_trainer-speed-json", type=Path)
...
def main() -> int:
    ...
    if args.write_speed_json is not None:
        write_speed_json(
            ...
            serenity_trainer_speed_json=args.serenity_trainer_speed_json,
