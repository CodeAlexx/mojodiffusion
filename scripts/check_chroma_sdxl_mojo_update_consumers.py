...
The Chroma and SDXL adapter dumps are update-bearing SerenityTrainer oracles. They
...
that Mojo update against SerenityTrainer.
"""
...
SPECS: dict[str, ModelSpec] = {
    "chroma": ModelSpec(
        ...
        train_loop=REPO / "trainer/src/serenity_trainer/trainer/train_chroma_real.mojo",
        update_sources=(
            REPO / "trainer/src/serenity_trainer/trainer/train_chroma_real.mojo",
    ...
    "sdxl": ModelSpec(
        ...
        train_loop=REPO / "trainer/src/serenity_trainer/trainer/train_sdxl_real.mojo",
        update_sources=(
            REPO / "trainer/src/serenity_trainer/trainer/train_sdxl_real.mojo",
...
def update_source_evidence(spec: ModelSpec) -> dict[str, list[str]]:
    ...
    for path in spec.update_sources:
        ...
        if path.exists():
            ...
            if "serenity_lr_for_optimizer_step" in text:
                labels.append("serenity_lr_for_optimizer_step")
...
def assess_model(spec: ModelSpec, files: list[Path]) -> dict[str, Any]:
    ...
    if missing_artifacts:
        blockers.append("missing required SerenityTrainer update oracle artifacts or wrapper")
    ...
    if not missing_artifacts:
        ...
        if not oracle["has_update"]:
            blockers.append("SerenityTrainer adapter dump is not update-bearing")
    ...
    if not passing_full_adamw_consumers and not missing_grad_phase_blocks_full_parity:
        blockers.append(
            ...
            "adapter_after against SerenityTrainer"
        )
    ...
    return {
        "model": spec.name,
        "oracle_scope": (
            "SerenityTrainer update-bearing adapter oracle. Update-delta artifact "
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Check whether Chroma/SDXL update-bearing reference trainer oracles have Mojo update consumers."
    )
    parser.add_argument(
        "model",
        nargs="?",
