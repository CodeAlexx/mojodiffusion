...
# musubi-correct causal_offset: musubi ltx_2/components/patchifiers.py:240 (=1,
# "+1 ensures the timestamp corresponds to the first fully-available sample"),
# official LTX-2 ref ltx2_python_reference.py:111, the frame-proven MVP
# ltx2_t2v_av_mvp.mojo:128, SimpleTuner ltxvideo2 transformer default. Was 0
# (musubi-WRONG) — the rope VALUE differs by 1/frame_rate on every frame f>=1.
# Corrected 2026-07-18 (lead ruling (a)); all AV oracle refs regenerated.
CAUSAL_OFFSET = 1
VAE_SF = (8, 32, 32)
FRAME_RATE = 25.0
...
def run_block(w, inp, lora):
    ...
    # 4. FFNs (:766-790); clamp omitted. FFN LoRA (ff.net.0.proj/net.2 +
    # audio_ff.net.0.proj/net.2) applied via _lora_add — NO-OP when the key is
    # absent (lora.get() is None), so the attention-only block gate is byte-
    # identical and only the FFN-preset paths see the delta.
    mod_ff = fused_modulate(rms_norm(hs, w.get("norm3.weight"), EPS), sc_mlp, sh_mlp)
    h1_v = _lora_add(linear3d(mod_ff, w["ff.net.0.proj.weight"], w["ff.net.0.proj.bias"]),
                     mod_ff, lora, "ff.net.0.proj")
    h1g_v = gelu_approximate(h1_v)
    ...
    mod_aff = fused_modulate(rms_norm(ahs, w.get("audio_norm3.weight"), EPS),
                             a_sc_mlp, a_sh_mlp)
    h1_a = _lora_add(linear3d(mod_aff, w["audio_ff.net.0.proj.weight"], w["audio_ff.net.0.proj.bias"]),
                     mod_aff, lora, "audio_ff.net.0.proj")
    h1g_a = gelu_approximate(h1_a)
...
def build_inputs(device):
    ...
    # video h/w max_pos is a FIXED [20, 2048, 2048] in musubi (rope.py:190,
    # model_configurator.py:55, model.py:174; MVP BASE_HW=2048) — NOT the
    # geometry-dependent VAE_SF*grid this oracle previously used (musubi-WRONG;
    # corrected 2026-07-18 alongside causal_offset — caught by the rope gate).
