# serenitymojo.serve.serenity_worker_sd3 — STANDALONE per-kind SD3.5 Large worker.
#
...
# Like the other per-kind workers, this entrypoint imports the full SD3 GPU+MAX
...
# `pixi run build-worker-sd3-raw` (--optimization-level 2); never build it from an
# interactive session.
#
# Invoked:  serenity_worker_sd3 <fd>
#   <fd> = inherited AF_UNIX socket fd (decimal), exactly as the sdxl/zimage/
#          qwenimage/ideogram4 workers.
#
...
def _sd3_worker_loop(mut backend: Sd3Backend, fd: Int32) raises:
    """Identical control flow to the sdxl/zimage/qwenimage/ideogram4 worker loops,
    specialized to Sd3Backend. One job at a time (single-GPU contract)."""
    set_nonblock(fd)
