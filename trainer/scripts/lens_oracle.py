#!/usr/bin/env python
"""Lens Serenity oracle — dumps parity references from Serenity's OWN deps.

Reference policy: Serenity ONLY. The `lens` package (lens/transformer.py) is
Serenity's actual dependency (modules/model/LensModel.py imports it), so it is
the 1:1 forward-math source. NO Rust / inference-flame.

Run with Serenity's venv:
