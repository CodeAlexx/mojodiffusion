# training/oft_serenity_trainer_device.mojo -- GPU SerenityTrainer-OFT block-size-4 helpers.
#
# This is the device-resident counterpart to training/oft_serenity_trainer.mojo for the
# direct LyCORIS path. It implements the SerenityTrainer input-side OFT rotation:
...
def oft_serenity_rotate_b4(x: Tensor, vec: Tensor, ctx: DeviceContext) raises -> Tensor:
    """Apply SerenityTrainer-OFT input rotation with block_size=4 on device."""
    var dims = _shape_rows_in(x, String("oft_serenity_rotate_b4"))
    var rows = dims[0]
    var in_f = dims[1]
    var r = in_f // 4
    var v2 = _validate_vec_b4(vec, r, String("oft_serenity_rotate_b4"))
    var dt = x.dtype().to_mojo_dtype()
    if dt != DType.float32 and dt != DType.bfloat16 and dt != DType.float16:
        raise Error("oft_serenity_rotate_b4: unsupported x dtype")
...
def oft_serenity_rotate_backward_b4(
    d_x_rot: Tensor, x: Tensor, vec: Tensor, ctx: DeviceContext,
) raises -> OFTOTDeviceGrads:
    """Backward for x_rot = oft_serenity_rotate_b4(x, vec)."""
    var dims = _shape_rows_in(x, String("oft_serenity_rotate_backward_b4"))
    var rows = dims[0]
    var in_f = dims[1]
    if d_x_rot.shape() != x.shape():
        raise Error("oft_serenity_rotate_backward_b4: d_x_rot/x shape mismatch")
    if d_x_rot.dtype() != x.dtype():
        raise Error("oft_serenity_rotate_backward_b4: d_x_rot/x dtype mismatch")
    var r = in_f // 4
    var v2 = _validate_vec_b4(vec, r, String("oft_serenity_rotate_backward_b4"))
    var dt = x.dtype().to_mojo_dtype()
    if dt != DType.float32 and dt != DType.bfloat16 and dt != DType.float16:
        raise Error("oft_serenity_rotate_backward_b4: unsupported x dtype")
