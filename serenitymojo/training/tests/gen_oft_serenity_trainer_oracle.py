#!/usr/bin/env python3
# SerenityTrainer OFT oracle: run SerenityTrainer's OWN OFTModule.forward (Linear → rotates
# the INPUT in blocks via 5-term Neumann Cayley) and dump the triu-vector param,
# block geometry, W, x, and y for the Mojo OFT-Neumann gate (oft_serenity_trainer_parity.mojo).
# MJ-1024.
#
# Run: python3 serenitymojo/training/tests/gen_oft_serenity_trainer_oracle.py
import sys, types
...
with torch.no_grad():
    y = m.forward(x)                                    # SerenityTrainer's OWN OFT forward [M,OUT]
...
save_file(out, "/tmp/oft_serenity_oracle.safetensors")
print("WROTE /tmp/oft_serenity_oracle.safetensors  (5-term Neumann, input-side, eps=0)")
