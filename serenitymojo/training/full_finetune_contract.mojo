# training/full_finetune_contract.mojo -- SerenityTrainer full-finetune readiness map.
#
...
#
# Reference sources:
#   /home/alex/SerenityTrainer/modules/modelSetup/*FineTuneSetup.py
#   /home/alex/SerenityTrainer-anima-ref/modules/modelSetup/AnimaFineTuneSetup.py
#
# Scope:
#   * maps target SerenityTrainer model families that register TrainingMethod.FINE_TUNE
...
comptime SERENITY_FULL_FT_LOOP_UNSUPPORTED = 0
comptime SERENITY_FULL_FT_LOOP_FAIL_LOUD_ONLY = 1
comptime SERENITY_FULL_FT_LOOP_WIRED = 2
...
@fieldwise_init
struct SerenityFullFinetuneTarget(Copyable, Movable):
    var key: String
    var label: String
    var reference_root: String
    var setup_file: String
    var model_types_csv: String
    var serenity_trainer_fine_tune_registered: Bool
    ...
    def has_product_full_finetune_loop(self) -> Bool:
        return self.product_loop_status == SERENITY_FULL_FT_LOOP_WIRED

    def can_run_full_finetune(self) -> Bool:
        return (
            self.serenity_trainer_fine_tune_registered
...
@fieldwise_init
struct SerenityFullFinetuneResumeSidecarSpec(Copyable, Movable):
...
@fieldwise_init
struct SerenityFullFinetuneProductRunPlan(Copyable, Movable):
    var requested_model_type: String
    var optimizer: Int
    var target: SerenityFullFinetuneTarget
...
def full_finetune_loop_status_name(status: Int) -> String:
    if status == SERENITY_FULL_FT_LOOP_UNSUPPORTED:
        return String("unsupported")
    if status == SERENITY_FULL_FT_LOOP_FAIL_LOUD_ONLY:
        return String("fail_loud_only")
    if status == SERENITY_FULL_FT_LOOP_WIRED:
        return String("wired")
...
def _target(
    ...
    runner: String,
    note: String,
) -> SerenityFullFinetuneTarget:
    return SerenityFullFinetuneTarget(
        key,
        label,
        reference_root,
        setup_file,
...
def full_finetune_targets() -> List[SerenityFullFinetuneTarget]:
    var out = List[SerenityFullFinetuneTarget]()
    var root = String("/home/alex/SerenityTrainer")
    var anima_root = String("/home/alex/SerenityTrainer-anima-ref")
    var note = String("SerenityTrainer FINE_TUNE is registered; Mojo product loop is still LoRA-only/full-weight unsupported")
    ...
    out.append(_target(
        ...
        String("Anima FINE_TUNE reference is local SerenityTrainer-anima-ref; Mojo product loop is still LoRA-only/full-weight unsupported"),
    ))
    return out^


def full_finetune_target_for_key(key: String) raises -> SerenityFullFinetuneTarget:
...
def full_finetune_target_for_model_type(model_type: String) raises -> SerenityFullFinetuneTarget:
...
def full_finetune_target_for_product_model_type(model_type: String) raises -> SerenityFullFinetuneTarget:
    if (
...
def full_finetune_model_type_has_serenity_trainer_registration(model_type: String) -> Bool:
    try:
        var target = full_finetune_target_for_model_type(model_type)
        return target.serenity_trainer_fine_tune_registered
    except e:
        return False
...
def default_full_finetune_resume_sidecar_spec() -> SerenityFullFinetuneResumeSidecarSpec:
    return SerenityFullFinetuneResumeSidecarSpec(
        True,
        True,
        True,
        True,
...
) raises -> SerenityFullFinetuneProductRunPlan:
    var target = full_finetune_target_for_product_model_type(requested_model_type)
    return SerenityFullFinetuneProductRunPlan(
...
def validate_full_finetune_resume_sidecar_spec(
    spec: SerenityFullFinetuneResumeSidecarSpec,
) raises:
...
def validate_full_finetune_product_run_plan(
    plan: SerenityFullFinetuneProductRunPlan,
) raises:
    ...
    if plan.product_loop_status != SERENITY_FULL_FT_LOOP_UNSUPPORTED:
...
def full_finetune_product_run_blocker(plan: SerenityFullFinetuneProductRunPlan) -> String:
    return (
        String("model_type=")
...
def validate_full_finetune_target_contract(target: SerenityFullFinetuneTarget) raises:
    if not target.serenity_trainer_fine_tune_registered:
        raise Error(
            String("full-finetune contract: target has no SerenityTrainer FINE_TUNE registration: ")
            + target.key
        )
    ...
    if target.product_loop_status == SERENITY_FULL_FT_LOOP_WIRED:
        raise Error(
