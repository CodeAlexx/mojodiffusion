...
pub(crate) fn visual_health_for_output(output_path: &str, expected: Option<(u32, u32)>) -> Value {
    ...
    // Low-key/chiaroscuro images can have a small global spread while still
    // carrying strong detail and a full tonal/color range.  Treat low spread
    // as a defect only when a second placeholder signal agrees with it.
    if avg_stddev < VISUAL_MIN_AVG_STDDEV
        && (luminance_range < VISUAL_MIN_LUMINANCE_RANGE
            || edge_energy < VISUAL_MIN_EDGE_ENERGY
            || color_bins.len() < VISUAL_MIN_COLOR_BINS)
    {
    ...
    for region in &regions {
        ...
        if region.gray_stddev < VISUAL_MIN_REGION_GRAY_STDDEV
            && (region.edge_energy < VISUAL_MIN_EDGE_ENERGY
                || region.color_bins < VISUAL_MIN_REGION_COLOR_BINS)
        {
            failures.push(format!(
