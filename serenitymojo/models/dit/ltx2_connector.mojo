...
struct LTX2ConnectorWeights(Movable):
    ...
    def load(
        ...
        # Learnable pad registers (Embeddings1DConnector.num_learnable_registers,
        # default 128; key `{connector}.learnable_registers` [128, inner] BF16).
        # The reference REPLACES pad rows with these before the blocks — see
        # ltx2_connector_forward(valid_len=...).
        if _st_has(st, base + "learnable_registers"):
            var rg = st.tensor_view(base + "learnable_registers")
            name_to_idx[String("learnable_registers")] = len(weights)
            weights.append(ArcPointer(Tensor.from_view_as_bf16(rg, ctx)))
...
# ── learnable-register pad replacement (Embeddings1DConnector reference) ─────
# ltx_core embeddings_connector.py:_replace_padded_with_learnable_registers:
# the connector input is RIGHT-padded ([valid, pad] — EmbeddingsProcessor
# reorders left-padded tokenizer output before the connector); pad row i is
# REPLACED with learnable_registers[i % 128] (registers tiled seq_len/128
# times), then the attention mask is ZEROED — so the connector blocks AND the
# downstream DiT cross-attention run unmasked over trained register tokens.
# Running unmasked over raw/zero pad rows instead imprints a foreground
# lattice artifact (measured 2026-07-09, three staged runs, two prompts).
def _replace_pad_rows_with_registers(
    weights: LTX2ConnectorWeights,
    x: Tensor,       # [1, N, inner]
    valid_len: Int,  # rows >= valid_len are pads
    seq_len: Int,
    inner: Int,
    ctx: DeviceContext,
) raises -> Tensor:
    ref regs = weights._w(String("learnable_registers"))  # [R, inner]
    var num_regs = regs.shape()[0]
    if regs.shape()[1] != inner:
        raise Error("ltx2 connector registers: inner dim mismatch")
    if seq_len % num_regs != 0:
        raise Error("ltx2 connector registers: seq_len % num_registers != 0")
    var rh = regs.to_host(ctx)  # List[Float32], R*inner (BF16 exact in F32)

    # x_out = x * mask + registers_tiled * (1 - mask), computed as one
    # broadcast mul plus an additive term that is zero on valid rows.
    var mvals = List[Float32](capacity=seq_len)
    var pvals = List[Float32](capacity=seq_len * inner)
    for i in range(seq_len):
        if i < valid_len:
            mvals.append(Float32(1.0))
            for _ in range(inner):
                pvals.append(Float32(0.0))
        else:
            mvals.append(Float32(0.0))
            var rbase = (i % num_regs) * inner
            for j in range(inner):
                pvals.append(rh[rbase + j])
    var m = Tensor.from_host(mvals, _shape3(1, seq_len, 1), x.dtype(), ctx)
    var pad_regs = Tensor.from_host(
        pvals, _shape3(1, seq_len, inner), x.dtype(), ctx
    )
    return add(mul(x, m, ctx), pad_regs, ctx)


# ── connector forward (the public entry) ─────────────────────────────────────
# Runs the connector on the PRE-connector context `x` [1, N, inner].
# valid_len >= 0: reference semantics — rows >= valid_len (the pads of a
#   RIGHT-padded context) are replaced with the checkpoint's learnable
#   registers before the blocks; attention then runs unmasked (the reference
#   zeroes the mask after replacement, so mask-free sdpa is EXACT here).
# valid_len < 0 (default): legacy mask=None behavior — whole sequence treated
#   as real, no register replacement (kept for the parity smokes, whose Python
#   reference runs the connector the same way).
...
def ltx2_connector_forward[N: Int, H: Int, Dh: Int](
    ...
    var h: Tensor
    if valid_len >= 0 and valid_len < N:
        if not weights._has(String("learnable_registers")):
            raise Error(
                "ltx2_connector_forward: valid_len given but checkpoint has no"
                " learnable_registers (refusing the lattice-artifact path)"
            )
        h = _replace_pad_rows_with_registers(
            weights, x, valid_len, N, cfg.inner_dim, ctx
        )
    else:
        h = weights._clone(x, ctx)
