#!/usr/bin/env python3
"""
Dump real SerenityTrainer ZImage train-step reference data for Mojo parity gates.

This script imports SerenityTrainer by path, uses its real ZImage model loader,
...
Run with the SerenityTrainer venv, for example:
  /home/alex/SerenityTrainer/venv/bin/python scripts/zimage_dump_train_ref.py --dry-run
"""
...
DEFAULT_SERENITY_TRAINER = Path("/home/alex/SerenityTrainer")
DEFAULT_CONFIG = DEFAULT_SERENITY_TRAINER / "configs" / "eri2_zimage_base_2500.json"
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Dump one bounded SerenityTrainer ZImage train-step parity ref."
    )
    parser.add_argument("--serenity_trainer", type=Path, default=DEFAULT_SERENITY_TRAINER)
    ...
    parser.add_argument(
        "--allow-cache-build",
        action="store_true",
        help="Allow SerenityTrainer to build missing latent/text caches during start_next_epoch().",
    )
    parser.add_argument(
        "--torch-seed",
        type=int,
        default=None,
        help="Optional process RNG seed before SerenityTrainer setup. Default preserves SerenityTrainer's process RNG behavior.",
    )
    parser.add_argument(
        "--dry-run",
        action="store_true",
    ...
    parser.add_argument(
        ...
        action="store_true",
        help=(
            "Run the bounded SerenityTrainer step and write timing/loss metadata only. "
...
def add_serenity_trainer_to_path(root: Path) -> None:
    root = root.resolve()
    if not (root / "modules").is_dir():
        raise FileNotFoundError(f"SerenityTrainer root does not contain modules/: {root}")
    sys.path.insert(0, str(root))


def enter_serenity_trainer_root(args: argparse.Namespace) -> None:
    """SerenityTrainer factory.import_dir uses cwd-relative module paths."""
    args.serenity_trainer = args.serenity_trainer.resolve()
    args.config = args.config.resolve()
    if args.secrets is not None:
        args.secrets = args.secrets.resolve()
    args.out_dir = args.out_dir.resolve()
    os.chdir(args.serenity_trainer)
...
def validate_config(config: Any, args: argparse.Namespace, *, check_cache: bool = True) -> None:
    ...
    if not args.dry_run:
        if str(config.train_device).split(":", 1)[0] != "cuda":
            raise RuntimeError(
                ...
                "SerenityTrainer reference dumps must use --train-device cuda."
            )
    ...
    if check_cache and config.latent_caching and not args.allow_cache_build and not cache_present(config):
        raise FileNotFoundError(
            ...
            "--allow-cache-build to let SerenityTrainer build it."
        )


def run_dry_run(args: argparse.Namespace, config: Any) -> None:
    info = {
        "serenity_trainer": str(args.serenity_trainer.resolve()),
...
def train_and_dump(args: argparse.Namespace, config: Any) -> dict[str, Any]:
    ...
    if accumulated_steps == 0:
        raise RuntimeError("No batches were produced by the SerenityTrainer data loader")
...
def main() -> None:
    args = parse_args()
    enter_serenity_trainer_root(args)
    add_serenity_trainer_to_path(args.serenity_trainer)
