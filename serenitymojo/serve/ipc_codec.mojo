...
def encode_start(p: JobParams) raises -> String:
    ...
    # krea2 FlowEdit params (Phase C1; mirror wire JobParams edit_* block).
    o.set("edit_src_image", JSONValue.from_string(p.edit_src_image))
    o.set("edit_src_prompt", JSONValue.from_string(p.edit_src_prompt))
    o.set("edit_src_negative", JSONValue.from_string(p.edit_src_negative))
    o.set("edit_nmax", JSONValue.from_int(p.edit_nmax))
    o.set("edit_nmin", JSONValue.from_int(p.edit_nmin))
    o.set("edit_src_cfg", JSONValue.from_float(p.edit_src_cfg))
    o.set("edit_auto_mask", JSONValue.from_bool(p.edit_auto_mask))
    o.set("edit_mask_q", JSONValue.from_float(p.edit_mask_q))
    o.set("edit_mask_dilate", JSONValue.from_int(p.edit_mask_dilate))
    o.set("edit_mask_warmup", JSONValue.from_int(p.edit_mask_warmup))
...
def decode_start(obj: JSONValue) raises -> JobParams:
    ...
    # krea2 FlowEdit params: TOLERANT reads (has-key guard) so starts encoded by
    # an older parent (pure-Mojo daemon not yet emitting edit_*) still decode.
    if obj.contains("edit_src_image"):
        p.edit_src_image = obj["edit_src_image"].as_string()
        p.edit_src_prompt = obj["edit_src_prompt"].as_string()
        p.edit_src_negative = obj["edit_src_negative"].as_string()
        p.edit_nmax = Int(obj["edit_nmax"].as_float())
        p.edit_nmin = Int(obj["edit_nmin"].as_float())
        p.edit_src_cfg = obj["edit_src_cfg"].as_float()
        p.edit_auto_mask = obj["edit_auto_mask"].as_bool()
        p.edit_mask_q = obj["edit_mask_q"].as_float()
        p.edit_mask_dilate = Int(obj["edit_mask_dilate"].as_float())
        p.edit_mask_warmup = Int(obj["edit_mask_warmup"].as_float())
