...
# ---------------------------------------------------------------------------
# Fast bulk offload/restore (memcpy, NOT a per-byte scalar loop). The scalar
# `offload_to_host`/`restore_to_device` above cost ~130ms per 200MB (see
# tensor.mojo from_view_raw note) — fine for a handful of block-input tensors,
# fatal for the full ~15-20GB activation tape (KLEIN_SAVE_ACTIVATIONS). These
# stage through a pinned host buffer and `sys_memcpy` the bytes verbatim, so the
# restored Tensor is byte-identical to the offloaded one (bf16 stays bf16).
# ---------------------------------------------------------------------------

def offload_to_host_fast(t: Tensor, ctx: DeviceContext) raises -> HostOffload:
    var n = t.nbytes()
    var staging = ctx.enqueue_create_host_buffer[DType.uint8](n)
    ctx.enqueue_copy(dst_buf=staging, src_buf=t.buf)
    ctx.synchronize()
    var host = List[UInt8](unsafe_uninit_length=n)
    var dst = BytePtr(unsafe_from_address=Int(host.unsafe_ptr()))
    var src = BytePtr(unsafe_from_address=Int(staging.unsafe_ptr()))
    _ = sys_memcpy(dst, src, n)
    return HostOffload(host^, t.shape(), t.dtype())


def restore_to_device_fast(off: HostOffload, ctx: DeviceContext) raises -> Tensor:
    var n = len(off.host)
    var staging = ctx.enqueue_create_host_buffer[DType.uint8](n)
    var dst = BytePtr(unsafe_from_address=Int(staging.unsafe_ptr()))
    var src = BytePtr(unsafe_from_address=Int(off.host.unsafe_ptr()))
    _ = sys_memcpy(dst, src, n)
    var dev = ctx.enqueue_create_buffer[DType.uint8](n)
    ctx.enqueue_copy(dst_buf=dev, src_buf=staging)
    ctx.synchronize()
