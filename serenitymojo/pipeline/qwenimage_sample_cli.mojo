...
# The RESIDENT (whole-load) Qwen2.5-VL encode needs ~17.4 GB (measured child
# footprint) — physically impossible on a 16 GB card, even in a fresh child.
# encode_captions_from_strings routes below this bar to the LAYER-STREAMED
# encoder (qwenimage_qwen25vl_streamed, peak ~2.7 GB, parity-gated
# cos>=0.999 vs the torch oracle); 24 GB boxes keep the resident path.
comptime _RESIDENT_ENCODE_MIN_FREE_BYTES = Int(17400) * 1024 * 1024
# Fork preflight: the encode CHILD now needs only the STREAMED path's ~2.7 GB
# peak + its CUDA context in the worst case (the child itself picks resident
# when device-global free clears the resident bar). Was 17400 MiB — which a
# 16 GB card can never satisfy (total VRAM < 17.4 GB).
comptime _ENCODE_CHILD_MIN_FREE_BYTES = Int(3600) * 1024 * 1024
...
# LAYER-STREAMED encode (16 GB fit): identical contract to the resident path —
# same tokenization (_tokenize_for_encoder), same EXTRACT_LAYER-27 + model.norm
# forward (executed layer-by-layer by the parity-gated Qwen25VLEncoder._layer),
# same [DROP_IDX, DROP_IDX+N_TXT_KEPT) trim. Both prompts advance together so
# the ~15 GB text_encoder is read ONCE. Peak TE VRAM ~2.7 GB.
# Parity: models/text_encoder/parity/qwenimage_te_streamed_parity (DiT-consumed
# cos 0.999876 vs the bf16 torch oracle, 2026-07-16).
def _encode_captions_streamed(
    prompt: String, negative: String, ctx: DeviceContext
) raises -> QwenCaps:
    print("[text] Qwen2.5-VL LAYER-STREAMED encoder (16GB fit), N_TXT_KEPT=",
          N_TXT_KEPT, "DROP_IDX=", DROP_IDX)
    var tok = Qwen3Tokenizer(TOK_JSON)
    var ptup = _tokenize_for_encoder(tok, prompt)
    var ntup = _tokenize_for_encoder(tok, negative)
    var ids_list = List[List[Int]]()
    ids_list.append(ptup[0].copy())
    ids_list.append(ntup[0].copy())
    var full = encode_qwen25vl_final_streamed(
        TEXT_ENCODER_DIR, ids_list, EXTRACT_LAYER, ctx
    )
    var pos = slice(full[0][], 1, DROP_IDX, N_TXT_KEPT, ctx)
    var neg = slice(full[1][], 1, DROP_IDX, N_TXT_KEPT, ctx)
    return QwenCaps(pos^, neg^, ptup[1], ntup[1])
...
# ROUTING (16 GB fit): the resident whole-load encoder needs ~17.4 GB — below
# that bar (device-global free), run the LAYER-STREAMED encode instead. A
# 16 GB card can NEVER clear the resident bar (total < 17.4 GB), so it always
# streams; a clean 24 GB box keeps the resident path.
def encode_captions_from_strings(
    prompt: String, negative: String, ctx: DeviceContext
) raises -> QwenCaps:
    var free_bytes = cu_mem_get_info().free_bytes
    if free_bytes < _RESIDENT_ENCODE_MIN_FREE_BYTES:
        print("[text] free VRAM", free_bytes // (1024 * 1024),
              "MiB < resident-encoder need",
              _RESIDENT_ENCODE_MIN_FREE_BYTES // (1024 * 1024),
              "MiB → LAYER-STREAMED Qwen2.5-VL encode")
        return _encode_captions_streamed(prompt, negative, ctx)
