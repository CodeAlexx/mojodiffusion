...
# ── ACE-Step-1.5 DiT block vocabulary (acestep_block_graph.mojo). The two ops
# the acestep block needs that the zimage/krea2 vocabulary lacks. Each arm
# (engine.mojo) calls ONLY the existing parity-proven backward — NO new math.
#  * OPK_ROPE_HALFSPLIT: y = rope_halfsplit(x, cos, sin) — Qwen3 rotate_half
#    (acestep_dit.mojo `_self_attn`), NOT the interleaved OPK_ROPE (FLUX/Klein).
#    Backward = rope_backward(g, cos, sin, interleaved=False), the exact inverse
#    the halfsplit forward's degenerate doubled table requires (rope docstring:
#    "False = Z-Image halfsplit pairing (i, i+D/2)"). saved=[cos, sin] (frozen
#    tables); one input edge (x).
#  * OPK_LINEAR_DX: y = linear(x, W_frozen) — a NON-LoRA projection (acestep's
#    MLP gate/up/down are frozen; only q/k/v/o self+cross carry LoRA). Backward =
#    linear_backward_dx(g, W, M, in_f, out_f) — dx only, W never gets a d_w
#    (frozen). Routes d_x through the frozen MLP so upstream (cross/self LoRA)
#    grads see x1's full grad. saved=[W]; meta=[M, in_f, out_f]; one input edge.
comptime OPK_ROPE_HALFSPLIT = 25
comptime OPK_LINEAR_DX = 26
# ── LTX-2.3 VIDEO block, COARSE (Option A; ltx2_video_block_graph.mojo). ONE
# composite kind/block, the OPK_KREA2_SINGLE_BLOCK precedent (:82): apply_ltx2v
# reboxes the saved block INPUT (recompute checkpoint), RECOMPUTES the forward
# (ltx2_video_block_train_forward, from the saved input only), and calls the
# WHOLE ltx2_video_block_backward oracle (models/ltx2/ltx2_video_backward.mojo) —
# so every internal >=3-way fold (gated-residual add(hidden, gate*attn), the 3
# rms/AdaLN modulate folds, the attn/FFN folds) stays INSIDE the oracle (C15
# trivially satisfied at graph level; the block input hidden is the ONLY tracked
# edge → its single contribution needs no fan-in fold). The n_slots LoRA grad
# pairs are HOST List[Float32] by the oracle's construction (LoraPairGrad.d_a/d_b,
# ltx2_av_backward.mojo:144) and CANNOT flow through the engine's TArc-only
# edge/Dict machinery — so execute_ltx2v_block returns the WHOLE
# LTX2VideoBlockGrads (d_hidden sunk through the engine LEAF; the host LoRA list
# captured out-of-band), NOT a Dict[Int,TArc]. One tracked edge (hidden). Block
# is shape-preserving so the record wrapper re-boxes the INPUT as the output
