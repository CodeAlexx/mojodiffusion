...
def check_resident_fp8_dequant_contract(failures: list[str]) -> None:
    ...
    streamed_end = stream_text.find("\n    def build_fixed_stage", streamed_start + 1)
    streamed_body = stream_text[streamed_start:streamed_end]
    if "fp8_e4m3_dequant_to_bf16_no_sync" in streamed_body:
        # The measured product streamer intentionally batches all per-tensor
        # launches behind one block-end fence (~86 fences -> one). This is safe
        # only when both the explicit contract marker and the final fence remain.
        if "stream-dequant-contract: one-fence-after-complete-block" not in streamed_body:
            failures.append(
                "serenitymojo/offload/ltx2_block_stream.mojo:1: streamed_fp8_no_sync: missing explicit one-fence contract marker"
            )
        if "ctx.synchronize()" not in streamed_body:
            failures.append(
                "serenitymojo/offload/ltx2_block_stream.mojo:1: streamed_fp8_no_sync: missing block-end DeviceContext fence"
            )


def check_final_latent_handoff_contract(failures: list[str]) -> None:
    """The denoise/decode process boundary must preserve the BF16 carrier."""
    rel = "serenitymojo/pipeline/ltx2_t2v_av_hq.mojo"
    text = (REPO / rel).read_text()
    required = (
        "cast_tensor(v1, STDtype.BF16, ctx)",
        "cast_tensor(a1, STDtype.BF16, ctx)",
        "cast_tensor(v_final, STDtype.BF16, ctx)",
        "cast_tensor(a_final, STDtype.BF16, ctx)",
    )
    for needle in required:
        if needle not in text:
            failures.append(
                f"{rel}:1: final_latent_handoff: missing BF16 handoff `{needle}`"
            )
    forbidden = (
        "cast_tensor(v1, STDtype.F32, ctx)",
        "cast_tensor(a1, STDtype.F32, ctx)",
        "cast_tensor(v_final, STDtype.F32, ctx)",
        "cast_tensor(a_final, STDtype.F32, ctx)",
    )
    for needle in forbidden:
        if needle in text:
            failures.append(
                f"{rel}:1: final_latent_handoff: denoise result widened to F32 `{needle}`"
            )
...
def main() -> int:
    ...
    if args.scope == "all":
        ...
        check_final_latent_handoff_contract(failures)
