...
def main() raises:
    ...
    # NO-LORA TWIN (proof A/B): identical to ideogram4_generate_lora.mojo except
    # the LoRA is NOT applied — same giger prompt + seed + steps. The only
    # difference vs the +lora render is the adapter, so a visible Giger shift is
    # attributable to the trained LoRA (mirrors the Rust base-vs-+lora proof).
    print("no-lora base render (LoRA intentionally not applied)")
    ...
    save_png(img, "/home/alex/mojodiffusion/output/ideogram4_giger_nolora_proof.png", ctx)
    print("saved output/ideogram4_giger_nolora_proof.png")
