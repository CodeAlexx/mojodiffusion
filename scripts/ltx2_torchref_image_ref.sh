#!/usr/bin/env bash
# LTX-2.3 torchref ORACLE reference run — IMAGE arm (eri2 identity, 512x512 F=1).
# Derived from ltx2_torchref_band_ref.sh (2026-07-16 torchref-parity audit).
# Community IMAGE recipe: lr 6e-5, rank/alpha 64, uniform_prob 0.30
# (flags verified against torchref argparse: --shifted_logit_uniform_prob
...
set -euo pipefail
OUT=/home/alex/mojodiffusion/output/ltx2_torchref_image_ref
mkdir -p "$OUT"
...
exec .venv/bin/accelerate launch --num_cpu_threads_per_process 1 --mixed_precision bf16 \
  ...
  --seed 42 \
  --output_dir "$OUT" \
  --output_name ltx2_torchref_image_ref \
