...
final decoded pixel blend. It is not an arbitrary ComfyUI graph executor.
...
`image1*(1-mask)+image2*mask`. This slice requires the `init_image`/base image
to already match the decoded output size; Comfy `ImageScale(area)` base-image
resize parity is still fail-loud. Z-Image rejects LanPaint sampler-loop fields
via `reject_unsupported_lanpaint_sampler_params(...)`; backends without this
split still reject unsupported LanPaint metadata through
...
- `LanPaint_KSampler`, `LanPaint_KSamplerAdvanced`,
  ...
  `LanPaint_MaskBlend` is limited to final decoded pixel compositing.
