...
def main() raises:
    ...
    # Product rectangles use a non-128-aligned joint sequence (for example
    # Ideogram-4 1152x896 is S=5056). The flash primitive already pads and
    # applies the real-length mask internally; gate that route against the
    # online key-mask oracle instead of falling back (which only supports
    # Dh<=128 and therefore rejects Ideogram's Dh=256).
    comptime S_RECT = 96
    comptime H_RECT = 2
    comptime Dh_RECT = 128
    comptime N_TXT_RECT = 32
    var real_txt_len_rect = 17
    var n_rect = B * S_RECT * H_RECT * Dh_RECT
    var scale_rect = Float32(1.0) / sqrt(Float32(Dh_RECT))
    var key_rect_ref = sdpa_qwen_keymask[B, S_RECT, H_RECT, Dh_RECT, N_TXT_RECT](
        Tensor.from_host(_fill_q(n_rect), _bshd(B, S_RECT, H_RECT, Dh_RECT), STDtype.BF16, ctx),
        Tensor.from_host(_fill_k(n_rect), _bshd(B, S_RECT, H_RECT, Dh_RECT), STDtype.BF16, ctx),
        Tensor.from_host(_fill_v(n_rect), _bshd(B, S_RECT, H_RECT, Dh_RECT), STDtype.BF16, ctx),
        real_txt_len_rect,
        scale_rect,
        ctx,
    )
    var flash_rect = sdpa_qwen_flash_padmask[B, S_RECT, H_RECT, Dh_RECT, N_TXT_RECT](
        Tensor.from_host(_fill_q(n_rect), _bshd(B, S_RECT, H_RECT, Dh_RECT), STDtype.BF16, ctx),
        Tensor.from_host(_fill_k(n_rect), _bshd(B, S_RECT, H_RECT, Dh_RECT), STDtype.BF16, ctx),
        Tensor.from_host(_fill_v(n_rect), _bshd(B, S_RECT, H_RECT, Dh_RECT), STDtype.BF16, ctx),
        real_txt_len_rect,
    ...
    var r5 = h.compare_host(
        _semantic_qwen_rows(
            flash_rect.to_host(ctx), B, S_RECT, H_RECT, Dh_RECT,
            N_TXT_RECT, real_txt_len_rect,
        ),
        _semantic_qwen_rows(
            key_rect_ref.to_host(ctx), B, S_RECT, H_RECT, Dh_RECT,
            N_TXT_RECT, real_txt_len_rect,
        ),
    )
    print("qwen nonaligned flash semantic rows vs keymask:", r5)
    all_pass = all_pass and r5.passed
