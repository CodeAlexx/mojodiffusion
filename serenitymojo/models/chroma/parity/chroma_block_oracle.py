...
#       serenitymojo/models/chroma/parity/chroma_block_oracle.py
# Full-FT base-only refs (d_ftref_*/s_ftref_*, for chroma_block_ft_parity.mojo):
#   CHROMA_FT_ORACLE=1 /home/alex/serenityflow-v2/.venv/bin/python \
#       serenitymojo/models/chroma/parity/chroma_block_oracle.py
...
# CHROMA_FT_ORACLE=1 (full-FT rollout item 3, 2026-07-07): BASE-ONLY mode — the
# krea2 KREA2_FT_ORACLE pattern. The FT block backward has NO adapter anywhere,
# and a LoRA-inclusive graph shifts BOTH dX (adapter path) and dW (downstream
# activations move), so the FT refs must exclude LoRA entirely: lora_apply
# becomes the identity and refs are dumped with d_ftref_/s_ftref_ prefixes
# (LoRA grad refs skipped — the adapters have no grad in this graph). Same
# seeds/inputs; the d_in_*/s_in_* input dumps are byte-identical in both modes.
FT_MODE = os.environ.get("CHROMA_FT_ORACLE") == "1"
...
def lora_apply(base_y, x, lo):
    if FT_MODE:
        return base_y                # base-only graph (full-FT reference)
    t = x @ lo["a"].T                # [M, rank]
    dy = t @ lo["b"].T               # [M, out]
...
def gen_double():
    ...
    RD = "d_ftref_" if FT_MODE else "d_ref_"
    Wbin(RD + "img_out", img_out)
    Wbin(RD + "txt_out", txt_out)
    Wbin(RD + "d_img", img.grad)
    Wbin(RD + "d_txt", txt.grad)
    # fused weight-grad references (gate compares the Mojo fused d_wqkv to this)
    for nm, w in [("im", iw), ("tm", tw)]:
        Wbin(RD + "%s_d_wqkv" % nm, fuse3(w["to_q"].grad, w["to_k"].grad, w["to_v"].grad))
        Wbin(RD + "%s_d_bqkv" % nm, torch.cat([w["to_q_b"].grad, w["to_k_b"].grad, w["to_v_b"].grad]))
        Wbin(RD + "%s_d_wproj" % nm, w["out"].grad)
        Wbin(RD + "%s_d_bproj" % nm, w["out_b"].grad)
        Wbin(RD + "%s_d_wmlp0" % nm, w["mlp0"].grad)
        Wbin(RD + "%s_d_bmlp0" % nm, w["mlp0_b"].grad)
        Wbin(RD + "%s_d_wmlp2" % nm, w["mlp2"].grad)
        Wbin(RD + "%s_d_bmlp2" % nm, w["mlp2_b"].grad)
        Wbin(RD + "%s_d_q_norm" % nm, w["q_norm"].grad)
        Wbin(RD + "%s_d_k_norm" % nm, w["k_norm"].grad)
    for nm, m in [("im", im), ("tm", tm)]:
        for kk in ["shift1", "scale1", "gate1", "shift2", "scale2", "gate2"]:
            Wbin(RD + "%s_d_%s" % (nm, kk), m[kk].grad)
    # LoRA grad references (base-only FT graph has no adapter grads)
    if not FT_MODE:
...
def gen_single():
    ...
    RS = "s_ftref_" if FT_MODE else "s_ref_"
    Wbin(RS + "out", out)
    Wbin(RS + "d_x", x.grad)
    Wbin(RS + "d_w1", torch.cat([w["to_q"].grad, w["to_k"].grad, w["to_v"].grad, w["proj_mlp"].grad], dim=0))
    Wbin(RS + "d_b1", torch.cat([w["to_q_b"].grad, w["to_k_b"].grad, w["to_v_b"].grad, w["proj_mlp_b"].grad]))
    Wbin(RS + "d_w2", w["w2"].grad)
    Wbin(RS + "d_b2", w["b2"].grad)
    Wbin(RS + "d_q_norm", w["q_norm"].grad)
    Wbin(RS + "d_k_norm", w["k_norm"].grad)
    for kk in ["shift", "scale", "gate"]:
        Wbin(RS + "d_%s" % kk, m[kk].grad)
    if not FT_MODE:
