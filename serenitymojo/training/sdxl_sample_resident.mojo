...
#
# ── CONDITIONING (MJ-1088 — corrected) ────────────────────────────────────────
...
# This sampler reuses the SAME context/y the train step already built as the COND
# conditioning.
#
# The UNCOND branch is the subtle part. The FIRST version fabricated ZEROED
# context/y as the unconditional. For SDXL that is WRONG (not merely approximate):
# the ADM `y` vector's time-ids half encodes the render resolution, so a zeroed
# y_uncond drives the uncond UNet pass out-of-distribution, and cfg amplifies that
# garbage into structured/geometric-noise output. The reference pipeline
# (pipeline/sdxl_sample_cli.mojo:157-164,328-330) never zeros the uncond — it loads
# REAL context_uncond/y_uncond (the empty/negative CLIP encode) from a sidecar.
#
# So this sampler does NOT fabricate a zeros-uncond. Real CFG is available only
# when real uncond embeds are supplied (the optional context_uncond/y_uncond
# params — a drop-in real-CFG replacement). When they are absent (today), the
# sampler runs cfg=1.0 (NO guidance): eps = eps_c, no uncond forward, coherent
# output from the valid cached caption conditioning + live LoRA. This still
# exercises the real denoise + decode + PNG path; guidance quality returns for
# free the day empty-prompt embeds are cached and threaded through.
#
# ── MEMORY NOTE ───────────────────────────────────────────────────────────────
...
def sdxl_sample_resident[H: Int, W: Int](
    ...
    cfg: Float32,
    ctx: DeviceContext,
    context_uncond: Optional[Tensor] = Optional[Tensor](None),  # [1,77,2048] REAL uncond (see below)
    y_uncond: Optional[Tensor] = Optional[Tensor](None),        # [1,2816]  REAL uncond ADM vector
) raises -> Tensor:
    if n_steps < 1:
    ...
    # ── UNCOND / CFG (MJ-1088 fix) ────────────────────────────────────────────
    # v1 fabricated ctx_uncond=zeros, y_uncond=zeros as the CFG unconditional
    # branch. For SDXL that is WRONG, not merely approximate: the ADM `y` vector's
    # time-ids half encodes the render resolution (original/crop/target size), so a
    # ZEROED y_uncond makes the uncond UNet pass out-of-distribution, and at
    # cfg=7.5 the guidance term eps_u + cfg*(eps_c-eps_u) is dominated by that
    # garbage — the latent diverges and decodes to structured/geometric-noise
    # blocks (the observed failure). The reference pipeline
    # (pipeline/sdxl_sample_cli.mojo:157-164,328-330) does NOT zero the uncond: it
    # loads REAL `context_uncond`/`y_uncond` (the empty/negative CLIP encode) from
    # the sidecar. Until such embeds are threaded here (pass them via the optional
    # params — a drop-in real CFG), the only CORRECT choice is NO guidance: run
    # cfg=1.0, where eps = eps_c and the uncond branch is not needed at all. This
    # exercises the real denoise + decode with the live LoRA using ONLY the valid
    # cached caption conditioning — coherent (if unguided) output, never garbage.
    var has_uncond = context_uncond.__bool__() and y_uncond.__bool__()
    var do_cfg = has_uncond and cfg > Float32(1.0) + Float32(1.0e-6)
    if not do_cfg and cfg > Float32(1.0) + Float32(1.0e-6):
        print(
            "[sdxl-sample] cfg", cfg,
            "requested but no real uncond embeds provided -> running cfg=1.0",
            "(no guidance). Fabricated zeros-uncond is disabled (MJ-1088:",
            "it yields structured-noise garbage for SDXL).",
        )
    # Real uncond conditioning (only referenced when do_cfg). Cloned once; the
    # placeholder aliases the cond tensors when CFG is off (never used then).
    var ctx_uncond = context.clone(ctx)
    var y_u = y.clone(ctx)
    if do_cfg:
        ctx_uncond = context_uncond.value().clone(ctx)
        y_u = y_uncond.value().clone(ctx)
    ...
    for i in range(n_steps):
        ...
        # COND forward through the FROZEN base + LIVE LoRA (NHWC eps).
        ...
        # UNCOND forward + CFG combine ONLY when a real uncond was supplied.
        var eps_u = List[Float32]()
        if do_cfg:
            var fwd_u = sdxl_real_forward[H, W](
                x_in_nhwc.clone(ctx), t.clone(ctx), y_u.clone(ctx), ctx_uncond.clone(ctx), w, lora, ctx
            )
            eps_u = fwd_u.out.to_host(ctx)             # NHWC flat [H*W*4]

        # Euler eps-pred step in NCHW host order. eps is NHWC; convert each
        # element's index NHWC(h,w,c) -> NCHW(c,h,w) on the fly. CFG when do_cfg,
        # else the plain cond eps (cfg=1.0 semantics).
        ...
        for hh in range(H):
            for ww in range(W):
                for c in range(4):
                    ...
                    var e = eps_c[nhwc_i]
                    if do_cfg:
                        e = eps_u[nhwc_i] + cfg * (eps_c[nhwc_i] - eps_u[nhwc_i])
