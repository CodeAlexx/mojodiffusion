# serenity_trainer_cache_preflight_smoke.mojo
#
# Run:
#   timeout 180 prlimit --as=12000000000 pixi run mojo run -I . serenitymojo/training/serenity_trainer_cache_preflight_smoke.mojo
...
def _check(cond: Bool, msg: String) raises:
    if not cond:
        raise Error(String("serenity_trainer_cache_preflight_smoke FAILED: ") + msg)
...
def _write_config(path: String, model_type: String, only_cache: Bool) raises:
    ...
    content += '"workspace_dir":"/tmp/reference-cache-preflight-workspace",'
    content += '"cache_dir":"/tmp/reference-cache-preflight-cache",'
...
def _check_model(model_type: String) raises:
    var path = String("/tmp/serenity_cache_preflight_") + model_type + String(".json")
    _write_config(path, model_type, False)
    var cfg = read_model_config(path)
    var plan = create_serenity_trainer_cache_preflight_plan(cfg)
    validate_serenity_trainer_cache_preflight_plan(plan)
...
def _expect_model_raises(model_type: String) raises:
    var path = String("/tmp/serenity_cache_preflight_blocked_") + model_type + String(".json")
    _write_config(path, model_type, False)
    var cfg = read_model_config(path)
    var raised = False
    try:
...
def _expect_only_cache_raw_status(model_type: String, should_pass: Bool) raises:
    var path = String("/tmp/serenity_cache_preflight_only_") + model_type + String(".json")
    ...
    try:
        ...
        if should_pass:
            ...
            print("  only_cache raw VAE ready:", serenity_trainer_cache_preflight_summary(plan))
    except e:
        raised = True
...
def main() raises:
    print("==== SerenityTrainer product cache preflight smoke ====")
    ...
    print("serenity_trainer_cache_preflight_smoke PASS")
