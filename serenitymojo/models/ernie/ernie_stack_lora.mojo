...
# adapter's d_A/d_B, and supports an AdamW step + a SerenityTrainer raw LoRA save
...
#   * training/{lora_save, train_step, optim} : LoraAdapter, _lora_adamw, save_lora_serenity_trainer.
#
...
from serenitymojo.models.ernie.lora_block import (
    ...
    ernie_block_lora_forward_device_tensor_b2, ernie_block_lora_backward_device_tensors_b2,
...
def ernie_stack_lora_forward_resident_device[
    ...
    # SerenityTrainer masks padded text keys [N_IMG+real_len, S); built in x's compute
...
# ══════════════════════════════════════════════════════════════════════════════
# TRUE BATCH-2 (row-stacked) resident-device stack fwd/bwd. Two samples processed
# as ONE [2S, D] = [img0|txt0|img1|txt1] forward/backward through the b2 block.
# Mirrors ernie_stack_lora_forward/backward_resident_device EXACTLY, differing only
# in the row-stack construction, per-sample AdaLN vectors (mv0/mv1, f_scale0/1,
# f_shift0/1), per-sample RoPE tables (pre-tiled by concat along the row axis) and
# per-sample text-pad masks. Backward returns HOST-LIST ErnieLoraGrads (the SAME
# arm as b1 → fused_lora_adamw_plain_step); the 7×L dA/dB are already summed over
# BOTH sample halves (= the batch gradient; = mean of the two b1 grads once the
# trainer's 0.5 joint-loss scale is folded into d_out0/d_out1).
#
# NOTE (MJ-1071): this exercises the BF16-resident device backward whose final-layer
# and per-block modulate_backward calls pass an F32 `_t(...)` scale against BF16
# go/x — a pre-existing dtype crash NOT fixed here (mirrored, so b1 and b2 crash
# identically). Compiles; cannot run past the first backward until MJ-1071 is fixed.
# ══════════════════════════════════════════════════════════════════════════════
struct ErnieStackForwardB2(Movable):
    var out0: List[Float32]      # [N_IMG, out_ch] sample-0 image velocity (host)
    var out1: List[Float32]      # [N_IMG, out_ch] sample-1 image velocity (host)
    var blk_x_in: List[TArc]     # num_layers × [2S, D] checkpoint inputs
    var x_final: TArc            # [2S, D] output of the last block (pre final layer)
    var ln_x: TArc               # [2S, D] layer_norm(x_final) (non-learnable)

    def __init__(
        out self,
        var out0: List[Float32], var out1: List[Float32],
        var blk_x_in: List[TArc], var x_final: TArc, var ln_x: TArc,
    ):
        self.out0 = out0^
        self.out1 = out1^
...
def ernie_stack_lora_forward_resident_device_b2[
    H: Int, Dh: Int, N_IMG: Int, N_TXT: Int, S: Int
](
    img_tokens0: List[Float32], txt_tokens0: List[Float32],
    img_tokens1: List[Float32], txt_tokens1: List[Float32],
    base: ErnieStackBase, blocks: List[ErnieBlockWeights],
    lora: ErnieLoraDeviceSet, mv0: ErnieModVecs, mv1: ErnieModVecs,
    f_scale0: List[Float32], f_shift0: List[Float32],
    f_scale1: List[Float32], f_shift1: List[Float32],
    cos0: Tensor, sin0: Tensor, cos1: Tensor, sin1: Tensor,
    D: Int, F: Int, in_ch: Int, text_in: Int, out_ch: Int, eps: Float32,
    ctx: DeviceContext,
    text_real_len0: Int = -1, text_real_len1: Int = -1,
) raises -> ErnieStackForwardB2:
    comptime S2 = 2 * S
    var num_layers = len(blocks)

    # per-sample patch/text embed → per-sample [S,D] streams, then row-stack [2S,D].
    var img_bias0 = Optional[Tensor](base.patch_b[].clone(ctx))
    var img0 = linear(
        _t_bf16(img_tokens0, [N_IMG, in_ch], ctx), base.patch_w[], img_bias0^, ctx
    )
    var no_txt_bias0 = Optional[Tensor](None)
    var txt0 = linear(_t_bf16(txt_tokens0, [N_TXT, text_in], ctx), base.text_proj[], no_txt_bias0^, ctx)
    var x0 = concat(0, ctx, img0, txt0)                        # [S,D]

    var img_bias1 = Optional[Tensor](base.patch_b[].clone(ctx))
    var img1 = linear(
        _t_bf16(img_tokens1, [N_IMG, in_ch], ctx), base.patch_w[], img_bias1^, ctx
    )
    var no_txt_bias1 = Optional[Tensor](None)
    var txt1 = linear(_t_bf16(txt_tokens1, [N_TXT, text_in], ctx), base.text_proj[], no_txt_bias1^, ctx)
    var x1 = concat(0, ctx, img1, txt1)                        # [S,D]

    var x = concat(0, ctx, x0, x1)                             # [2S,D]
    var x_arc = TArc(x^)

    # tiled RoPE tables: sample0 rows [0:S*H] then sample1 rows [S*H:2S*H].
    var cos_b2 = concat(0, ctx, cos0, cos1)                    # [2S*H, Dh]
    var sin_b2 = concat(0, ctx, sin0, sin1)

    # per-sample text-pad masks ([1,H,S,S] in x's compute dtype), constant/blocks.
    var mask0 = Optional[TArc](None)
    if text_real_len0 >= 0 and text_real_len0 < N_TXT:
        mask0 = Optional[TArc](TArc(
            build_ernie_text_pad_mask_fwd(H, S, N_IMG, text_real_len0, x_arc[].dtype(), ctx)
        ))
    var mask1 = Optional[TArc](None)
    if text_real_len1 >= 0 and text_real_len1 < N_TXT:
        mask1 = Optional[TArc](TArc(
            build_ernie_text_pad_mask_fwd(H, S, N_IMG, text_real_len1, x_arc[].dtype(), ctx)
        ))
    ...
    for bi in range(num_layers):
        blk_x_in.append(x_arc.copy())
        var bl = _block_lora_dev_for(lora, bi)
        var fwd = ernie_block_lora_forward_device_tensor_b2[H, Dh, S](
            x_arc.copy(), blocks[bi], mv0, mv1, bl, cos_b2, sin_b2, D, F, eps, ctx,
            mask0.copy(), mask1.copy(),
        )
        x_arc = fwd.out.copy()
    ...
    # per-sample final-layer AdaLN via [2, D] f_scale/f_shift (rows split evenly).
    var x_out = modulate(
        ln_x,
        _t_bf16(_stack2(f_scale0, f_scale1), [2, D], ctx),
        _t_bf16(_stack2(f_shift0, f_shift1), [2, D], ctx),
        ctx,
    )
    var final_bias = Optional[Tensor](base.final_lin_b[].clone(ctx))
    var patches = linear(x_out, base.final_lin_w[], final_bias^, ctx)   # [2S, out_ch]
    var out_img0 = slice(patches, 0, 0, N_IMG, ctx).to_host(ctx)        # sample-0 img
    var out_img1 = slice(patches, 0, S, N_IMG, ctx).to_host(ctx)        # sample-1 img

    return ErnieStackForwardB2(
        out_img0^, out_img1^, blk_x_in^, x_arc.copy(), TArc(ln_x^),
    )


def ernie_stack_lora_backward_resident_device_b2[
    H: Int, Dh: Int, N_IMG: Int, N_TXT: Int, S: Int
](
    ...
    # tiled RoPE tables (must match the forward call).
    var cos_b2 = concat(0, ctx, cos0, cos1)
    var sin_b2 = concat(0, ctx, sin0, sin1)

    # per-sample masks: bwd [H*S,S] F32 for sdpa_backward_masked; fwd for recompute.
    var has_mask0 = text_real_len0 >= 0 and text_real_len0 < N_TXT
    var has_mask1 = text_real_len1 >= 0 and text_real_len1 < N_TXT
    var bwd_mask0 = Optional[TArc](None)
    var bwd_mask1 = Optional[TArc](None)
    if has_mask0:
        bwd_mask0 = Optional[TArc](TArc(
            build_ernie_text_pad_mask_bwd(H, S, N_IMG, text_real_len0, ctx)
        ))
    if has_mask1:
        bwd_mask1 = Optional[TArc](TArc(
            build_ernie_text_pad_mask_bwd(H, S, N_IMG, text_real_len1, ctx)
        ))

    # d_patches [2S, out_ch] = [d_img0 | 0_txt | d_img1 | 0_txt] (loss on img only).
    var d_img0 = _t_bf16(d_out0, [N_IMG, out_ch], ctx)
    var d_txt0 = _t_bf16(_zeros(N_TXT * out_ch), [N_TXT, out_ch], ctx)
    var d_p0 = concat(0, ctx, d_img0, d_txt0)                  # [S, out_ch]
    var d_img1 = _t_bf16(d_out1, [N_IMG, out_ch], ctx)
    var d_txt1 = _t_bf16(_zeros(N_TXT * out_ch), [N_TXT, out_ch], ctx)
    var d_p1 = concat(0, ctx, d_img1, d_txt1)
    var d_patches = concat(0, ctx, d_p0, d_p1)                 # [2S, out_ch]

    var d_x_out = linear_backward_dx(d_patches, base.final_lin_w[], S2, D, out_ch, ctx)
    # MJ-1071 MIRROR (stack final-layer crash site — ernie_stack_lora.mojo:1039 in
    # b1): _t(...) F32 scale vs BF16 ln_x. [B,D] ⇒ compute_param_grads=False.
    var mbf = modulate_backward(
        cast_tensor(d_x_out, saved.ln_x[].dtype(), ctx, False),
        saved.ln_x[], _t_as(_stack2(f_scale0, f_scale1), [2, D], saved.ln_x[].dtype(), ctx), ctx,
    ...
    # per-sample recompute masks in the recompute input's compute dtype.
    var fwd_mask0 = Optional[TArc](None)
    var fwd_mask1 = Optional[TArc](None)
    if num_layers > 0:
        if has_mask0:
            fwd_mask0 = Optional[TArc](TArc(
                build_ernie_text_pad_mask_fwd(
                    H, S, N_IMG, text_real_len0, saved.blk_x_in[0][].dtype(), ctx
                )
            ))
        if has_mask1:
            fwd_mask1 = Optional[TArc](TArc(
                build_ernie_text_pad_mask_fwd(
                    H, S, N_IMG, text_real_len1, saved.blk_x_in[0][].dtype(), ctx
                )
            ))
    ...
    while bi >= 0:
        var bl = _block_lora_dev_for(lora, bi)
        var refwd = ernie_block_lora_forward_device_tensor_b2[H, Dh, S](
            saved.blk_x_in[bi].copy(), blocks[bi], mv0, mv1, bl,
            cos_b2, sin_b2, D, F, eps, ctx,
            fwd_mask0.copy(), fwd_mask1.copy(),
        )
        var bg = ernie_block_lora_backward_device_tensors_b2[H, Dh, S](
            d_x_arc[], blocks[bi], mv0, mv1, bl, refwd.saved,
            cos_b2, sin_b2, D, F, eps, ctx,
            bwd_mask0.copy(), bwd_mask1.copy(),
        )
        d_x_arc = bg.d_x.copy()
        d_shared_mod = _add_lists(d_shared_mod, bg.d_shared_mod)
        var base_idx = bi * ERNIE_SLOTS
    ...
    return ErnieLoraGrads(
        host_grads.d_a.copy(), host_grads.d_b.copy(),
        List[Float32](), List[Float32](),
        d_shared_mod^, List[Float32](), List[Float32](), List[Float32](),
        nonfinite,
    )
...
# ── RESIDENT fused AdamW — MJ-1070/MJ-1085 crash-class fix ────────────────────
# The retired per-step fused arm (fused_lora_adamw_plain_step, called inline by
# the trainer's ERNIE_GPU_ADAMW branch) allocates FRESH PINNED host staging EVERY
# step; under pinned pressure MAX can hand back an UNMAPPED buffer without raising
# -> segfault. The resident arm holds persistent device P/M/V and stages ONCE at
# init; per step only grads go up and params come back to the host set.ad mirror
# (the trainer's per-step ernie_lora_set_to_device upload then pushes them to the
# device). Identical F32-moment math to the flat fused step over set.ad.
def ernie_lora_adamw_state_init(
    set: ErnieLoraSet, ctx: DeviceContext
) raises -> LoraAdamWPlainDeviceState:
    return lora_adamw_plain_device_state_init(
        set.ad, 0, len(set.ad), ctx
    )


def ernie_lora_adamw_step_resident(
...
# ── per-block SerenityTrainer raw prefix scheme ──────────────────────────────────
# SerenityTrainer ErnieLoRASetup wraps model.transformer with prefix "transformer".
...
# save_lora_serenity_trainer appends .alpha / .lora_down.weight / .lora_up.weight.
