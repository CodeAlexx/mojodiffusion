...
print(f"| SimpleTuner | int8-sdnq+H256 (ConvRot, dequant-first*) | 0.50x | {1/it_cr:.3f} | ~12.5 GB whole-GPU** | stream-mean loss {st_off:+.4f} ({st_rel:+.2f}%) over {n_cr} samples |")
print()
print("**nvidia-smi whole-GPU peak during the ST run (incl. ~0.5 GB desktop);")
print("ours are internal-allocator peaks — methods differ, the 2x gap doesn't.")
