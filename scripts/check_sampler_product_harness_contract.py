#!/usr/bin/env python3
"""Static guard for the SerenityTrainer product sampler harness contract.

This is a scaffold guard, not a speed-parity claim. It verifies that the local
Mojo harness mirrors SerenityTrainer's sampler lifecycle and exposes the required
...
def main() -> int:
    require(
        SERENITY_TRAINER / "modules/util/config/SampleConfig.py",
        "SerenityTrainer SampleConfig fields",
        [
    ...
    require(
        SERENITY_TRAINER / "modules/modelSampler/BaseModelSampler.py",
        "SerenityTrainer BaseModelSampler lifecycle",
        [
    ...
    require(
        SERENITY_TRAINER / "modules/trainer/GenericTrainer.py",
        "SerenityTrainer training sample loop",
        [
            "def __sample_loop(",
    ...
    for name in [
        ...
        require(
            SERENITY_TRAINER / f"modules/modelSampler/{name}",
            f"SerenityTrainer {name} sample surface",
            [
