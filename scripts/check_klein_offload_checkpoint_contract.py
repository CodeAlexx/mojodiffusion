...
TRAIN = REPO / "trainer/src/serenity_trainer/trainer/train_klein_real.mojo"
...
TRAIN_LOOP_POLICY = REPO / "serenitymojo/training/serenity_trainer_train_loop_policy.mojo"
...
KLEIN_CONTROL_SMOKE = REPO / "trainer/smoke/klein_train_control_wiring_smoke.mojo"
...
CORE_STATUS = REPO / "serenitymojo/docs/SERENITY_TRAINER_CORE_PORT_STATUS_2026-06-06.md"
...
SERENITY_GRADIENT_CHECKPOINTING_METHOD = (
    SERENITY_TRAINER / "modules/util/enum/GradientCheckpointingMethod.py"
)
SERENITY_CHECKPOINTING_UTIL = SERENITY_TRAINER / "modules/util/checkpointing_util.py"
SERENITY_LAYER_OFFLOAD_CONDUCTOR = SERENITY_TRAINER / "modules/util/LayerOffloadConductor.py"
SERENITY_QUANTIZATION_UTIL = SERENITY_TRAINER / "modules/util/quantization_util.py"
SERENITY_FLUX2_LORA_8GB_PRESET = SERENITY_TRAINER / "training_presets/#flux2 LoRA 8GB.json"
SERENITY_FLUX2_FINETUNE_16GB_PRESET = SERENITY_TRAINER / "training_presets/#flux2 Finetune 16GB.json"

TRAIN_REF_STEP = "/home/alex/serenity_trainer-mojo/parity/klein_train_ref_step000.safetensors"

KNOWN_BLOCKERS: tuple[str, ...] = (
    "No accepted Mojo replay reruns Klein `predict -> backward_lora` from "
    f"`{TRAIN_REF_STEP}` and compares all adapter gradients against SerenityTrainer; "
    ...
    "SerenityTrainer; `scripts/check_klein_adamw_state_init_replay.py`, "
    ...
    "No later SerenityTrainer Klein dump exists yet with `lr_before > 0` and nonzero "
...
def gather_report() -> ContractReport:
    sources = {
        ...
        "serenity_grad_ckpt_method": read_source(SERENITY_GRADIENT_CHECKPOINTING_METHOD),
        "serenity_checkpointing_util": read_source(SERENITY_CHECKPOINTING_UTIL),
        "serenity_layer_offload_conductor": read_source(SERENITY_LAYER_OFFLOAD_CONDUCTOR),
        "serenity_quantization_util": read_source(SERENITY_QUANTIZATION_UTIL),
        "serenity_flux2_lora_8gb_preset": read_source(SERENITY_FLUX2_LORA_8GB_PRESET),
        "serenity_flux2_finetune_16gb_preset": read_source(SERENITY_FLUX2_FINETUNE_16GB_PRESET),
    }
    ...
    serenity_grad_ckpt_method = sources["serenity_grad_ckpt_method"]
    serenity_checkpointing_util = sources["serenity_checkpointing_util"]
    serenity_layer_offload_conductor = sources["serenity_layer_offload_conductor"]
    serenity_quantization_util = sources["serenity_quantization_util"]
    serenity_flux2_lora_8gb_preset = sources["serenity_flux2_lora_8gb_preset"]
    serenity_flux2_finetune_16gb_preset = sources["serenity_flux2_finetune_16gb_preset"]
    ...
    facts.append(
        fact_for_needles(
            ...
            label="SerenityTrainer CPU_OFFLOADED is offload-enabled checkpointing",
            source=serenity_grad_ckpt_method,
            needles=(
            ...
            detail_ok=(
                "SerenityTrainer defines CPU_OFFLOADED as checkpointing enabled with "
                "the offload flag set, distinct from plain ON."
            ),
            detail_missing="SerenityTrainer GradientCheckpointingMethod markers changed.",
        )
    )
    ...
    facts.append(
        fact_for_needles(
            status_if_ok="PASS",
            status_if_missing="WARN",
            label="SerenityTrainer Flux2/Klein checkpoint wrapper uses conductor hooks",
            source=serenity_checkpointing_util,
            needles=(
            ...
            detail_ok=(
                "SerenityTrainer Flux2/Klein uses OffloadCheckpointLayer conductor "
                "hooks around Flux2 transformer blocks and a Qwen3 encoder "
                "checkpoint path with no Qwen activation-offload args."
            ),
            detail_missing="SerenityTrainer Flux2/Klein checkpointing markers changed.",
        )
    )
    ...
    facts.append(
        fact_for_needles(
            status_if_ok="PASS",
            status_if_missing="WARN",
            label="SerenityTrainer conductor moves activations and layers",
            source=serenity_layer_offload_conductor,
            needles=(
            ...
            detail_ok=(
                "SerenityTrainer CPU_OFFLOADED is a conductor-driven activation and "
                "layer movement runtime keyed by layer_offload_fraction, not "
                "only checkpoint-file block streaming."
            ),
            detail_missing="SerenityTrainer LayerOffloadConductor markers changed.",
        )
    )
    ...
    facts.append(
        fact_for_needles(
            status_if_ok="PASS",
            status_if_missing="WARN",
            label="SerenityTrainer layer offload rewrites tensor storage",
            source=serenity_quantization_util,
            needles=(
            ...
            detail_ok=(
                "SerenityTrainer layer offload moves module tensor storage to the "
                "target device/temp allocator."
            ),
            detail_missing="SerenityTrainer quantized offload tensor-move markers changed.",
        )
    )
    ...
    facts.append(
        fact_for_needles(
            status_if_ok="PASS",
            status_if_missing="WARN",
            label="SerenityTrainer Flux2 LoRA low-VRAM preset selects CPU_OFFLOADED",
            source=serenity_flux2_lora_8gb_preset,
            needles=(
                '"gradient_checkpointing": "CPU_OFFLOADED"',
                '"layer_offload_fraction": 0.7',
            ),
            detail_ok=(
                "The SerenityTrainer Flux2 LoRA 8GB preset uses CPU_OFFLOADED with "
                "layer_offload_fraction=0.7."
            ),
            detail_missing="SerenityTrainer Flux2 LoRA 8GB preset no longer has expected CPU_OFFLOADED markers.",
        )
    )
    ...
    facts.append(
        fact_for_needles(
            status_if_ok="PASS",
            status_if_missing="WARN",
            label="SerenityTrainer Flux2 finetune low-VRAM preset requires fused Adafactor path",
            source=serenity_flux2_finetune_16gb_preset,
            needles=(
                '"gradient_checkpointing": "CPU_OFFLOADED"',
            ...
            detail_ok=(
                "The SerenityTrainer Flux2 finetune 16GB preset combines "
                "CPU_OFFLOADED with fused-back-pass Adafactor, a separate "
                "full-finetune/runtime requirement from LoRA weight streaming."
            ),
            detail_missing="SerenityTrainer Flux2 finetune 16GB preset no longer has expected low-VRAM markers.",
        )
    )
    ...
    facts.append(
        fact_for_needles(
            status_if_ok="PASS",
            status_if_missing="WARN",
            label="SerenityTrainer offload/checkpoint config fields",
            source=train_config,
            needles=(
            ...
            detail_ok=(
                "TrainConfig carries typed SerenityTrainer offload/checkpoint policy "
    ...
    facts.append(
        fact_for_needles(
            ...
            needles=(
                ...
                "does not compare SerenityTrainer optimizer moment tensor payloads",
                "does not prove CUDA/GPU parity",
                "does not use a real SerenityTrainer lr_before>0 adapter_after update",
            ),
            detail_ok=(
                "The positive-lr oracle covers only CPU host-list optimizer "
                "math from captured gradients; it is not CUDA/GPU, backward, "
                "SerenityTrainer moment payload, real later-step, or low-memory "
    ...
    facts.append(
        fact_for_needles(
            ...
            detail_ok=(
                "JSON config parsing is wired for SerenityTrainer checkpoint/offload "
    ...
    facts.append(
        fact_for_needles(
            ...
            needles=(
                "CPU_OFFLOADED maps to active SerenityTrainer offload policy",
    ...
    facts.append(
        fact_for_needles(
            status_if_ok="PASS",
            status_if_missing="WARN",
            label="SerenityTrainer conductor policy math is scalar-only",
            source=conductor_policy,
            needles=(
                "SerenityTrainer CPU_OFFLOADED conductor policy",
                "var layer_offload_fraction: Float64",
                "serenity_trainer_conductor_policy_from_fields",
            ...
            detail_ok=(
                "The local conductor policy keeps the SerenityTrainer layer fraction "
                "as Float64 and mirrors activation/layer/async gates plus "
                "target-loaded-byte math without tensor movement."
            ),
            detail_missing="SerenityTrainer conductor policy scalar markers are incomplete.",
        )
    )
    ...
    facts.append(
        fact_for_needles(
            ...
            needles=(
                "Klein activation tape byte plan for SerenityTrainer CPU_OFFLOADED work",
    ...
    facts.append(
        fact_for_needles(
            status_if_ok="PASS",
            status_if_missing="WARN",
            label="conductor policy smoke covers reference trainer low-VRAM fractions",
            source=conductor_policy_smoke,
            needles=(
                "no-CUDA gate for SerenityTrainer CPU_OFFLOADED policy",
                "layer_offload_fraction=0.7",
                "layer_offload_fraction=0.6",
    ...
    facts.append(
        fact_for_needles(
            ...
            detail_ok=(
                "Klein now accepts SerenityTrainer CPU_OFFLOADED policy and routes "
    ...
    facts.append(
        fact_for_needles(
            ...
            detail_ok=(
                ...
                "that the path was replay-validated against SerenityTrainer."
            ),
    ...
    facts.append(
        fact_for_needles(
            ...
            source=use_status,
            needles=(
                "Klein weight streaming is distinct from SerenityTrainer CPU_OFFLOADED",
                "TurboPlannedLoader",
            ...
            detail_ok=(
                ...
                "streaming from SerenityTrainer CPU_OFFLOADED activation/layer "
                "offload and from bounded CUDA replay parity."
            ),
            detail_missing=(
                "The docs no longer explicitly separate turbo weight streaming "
                "from SerenityTrainer CPU_OFFLOADED activation/layer parity."
            ),
        )
    )
    ...
    facts.append(
        fact_for_needles(
            ...
            needles=(
                ...
                "not SerenityTrainer `CPU_OFFLOADED` activation/layer parity",
                "backward replay",
            ...
            detail_ok=(
                ...
                "stand in for SerenityTrainer CPU_OFFLOADED or train parity."
            ),
    ...
    facts.append(
        fact_for_needles(
            ...
            needles=(
                ...
                "wiring/memory proof",
                "not SerenityTrainer sampler/trajectory/VAE-PNG parity",
                "speed/VRAM parity",
    ...
    facts.append(
        fact_for_needles(
            status_if_ok="PASS",
            status_if_missing="WARN",
            label="module inventories distinguish block streaming from reference trainer offload",
            ...
            detail_ok=(
                ...
                "block-streaming as SerenityTrainer training offload parity."
            ),
    ...
    facts.append(
        fact_for_needles(
            ...
            needles=(
                ...
                "not SerenityTrainer `CPU_OFFLOADED` activation/layer parity",
                "PARTIAL REAL RUN",
                "not full SerenityTrainer predict/backward/AdamW replay parity",
            ),
            detail_ok=(
                "The trainer module inventory now separates the partial "
                "block-streamed run from full SerenityTrainer replay/offload parity."
            ),
    ...
    facts.append(
        fact_for_needles(
            ...
            needles=(
                ...
                "not SerenityTrainer activation/layer `CPU_OFFLOADED` parity",
    ...
    facts.append(
        fact_for_needles(
            ...
            needles=(
                ...
                "does not consume SerenityTrainer train-ref tensors",
                "does not execute Klein predict/backward_lora",
    ...
    facts.append(
        fact_for_needles(
            ...
            needles=(
                "real SerenityTrainer adapter dump",
                "all 288 train-ref adapters",
                "adapter_post_clip_grad",
                "klein_lora_adamw_step",
                "does not execute Klein predict/backward_lora",
                "does not compare optimizer moment tensors against SerenityTrainer payloads",
                "does not prove full Mojo predict/backward/AdamW parity",
                "does not prove nonzero SerenityTrainer update parity",
                "does not prove low-memory offload/checkpoint backward parity",
            ),
            detail_ok=(
                ...
                "SerenityTrainer moment-payload, nonzero-update, or low-memory "
                "checkpoint/offload parity."
            ),
    ...
    facts.append(
        fact_for_needles(
            ...
            needles=(
                ...
                "No later SerenityTrainer Klein dump exists yet with `lr_before > 0`",
                "No accepted train-ref bounded-CUDA/offload/checkpoint backward replay proves",
    ...
    facts.append(
        fact_for_needles(
            ...
            needles=(
                ...
                "must still fail with exit `2` until a later Klein SerenityTrainer dump",
                "`lr_before > 0`",
