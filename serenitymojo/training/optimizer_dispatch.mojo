# training/optimizer_dispatch.mojo — SerenityTrainer optimizer name -> Mojo backend.
#
# Groundwork only: this does not wire trainers yet. The config reader sidecar can
# feed the raw SerenityTrainer optimizer string here once its owned files are ready.
# Local target preset audit (2026-06-05): explicit optimizer names in
# /home/alex/SerenityTrainer/{training_presets,configs} and /home/alex/training_presets
# are ADAMW and ADAFACTOR; missing/null optimizer objects inherit SerenityTrainer's
...
def optimizer_dispatch_for_identifier(identifier: String) -> OptimizerDispatch:
    """Resolve a SerenityTrainer optimizer string to the current Mojo backend.

    Missing/null maps to ADAMW because SerenityTrainer's TrainOptimizerConfig default
    ...
    if optimizer_identifier_is_default(identifier):
        return OptimizerDispatch(
            ...
            note="SerenityTrainer default optimizer; product path uses fused AdamW",
        )
        ...
        return OptimizerDispatch(
            ...
            state_kind=OPT_STATE_ADAM_M_V_F32,
            status=OPT_STATUS_SUPPORTED,
            note="available local backend; not a current SerenityTrainer enum member",
        )
        ...
        return OptimizerDispatch(
            ...
            status=OPT_STATUS_ALIASED,
            note="matches SerenityTrainer SGD only for weight_decay=0; Mojo helper uses decoupled WD otherwise",
        )
        ...
        return OptimizerDispatch(
            ...
            status=OPT_STATUS_SUPPORTED,
            note="local EDv2 RAdamScheduleFree backend; not a SerenityTrainer preset identifier",
        )
        ...
        return OptimizerDispatch(
            ...
            state_kind=OPT_STATE_UNSUPPORTED,
            status=OPT_STATUS_UNSUPPORTED,
            note="advanced SerenityTrainer modifier stack is not implemented",
        )
    else:
        return OptimizerDispatch(
            requested=identifier,
            ...
            state_kind=OPT_STATE_UNSUPPORTED,
            status=OPT_STATUS_UNSUPPORTED,
            note="no Mojo product backend mapped for this SerenityTrainer optimizer",
        )
