...
def main() raises:
    ...
    # Eviction proof: first/last FP8-block fwd peak (no monotonic accumulation).
    ...
    for bi in range(n_blocks):
        ...
        if ok_fwd:
            ...
            # Finiteness + shape check on EVERY block (must be [1,128,4096]).
            var osh = out.shape()
            var shape_ok = (
    ...
    # GENUINE eviction proof: per-block FP8 fwd peak must NOT climb across the
    # 43 FP8 blocks (a leak would add ≥256 MB/block → multi-GB growth).
