# nava_hires_pipeline.mojo — NAVA 832×480 end-to-end pipeline.
#
# Faithful 3-forward CFG (cond/neg/mmask) × 25 steps at hi-res:
#   - Video latent: 5×30×52 = 7800 tokens [7800,48]  (832×480 spatial, F=5 temporal)
#   - Audio latent: [34,128] unchanged
#   - VID=1950, SEQ=1984 (vs 320/354 at 256²)
#
...
#
# Inputs:
#   - Text: serenitymojo/pipeline/parity/nava_first_inputs.safetensors
#       pos_text [122,4096] F32, neg_text [126,4096] F32 (resolution-independent)
#   - Seeded latents from nava_fx_hires.safetensors:
#       in_lat_vid [7800,48] F32, in_lat_aud [34,128] F32
#
# Outputs: /home/alex/mojodiffusion/output/nava_hires/
#   frame_00.png .. frame_16.png  (17 frames, 480×832 RGB)
#   audio.wav                      (16 kHz 16-bit stereo PCM)
#
# NOTE: 1984-token SDPA × 30 blocks × 3 forwards × 25 steps is SLOW (~45-90 min).
# Run in BACKGROUND with large timeout. Per-step std printed for sanity.
#
# Run:
#   mkdir -p output/nava_hires
#   pixi run mojo run -I . serenitymojo/pipeline/nava_hires_pipeline.mojo
#
...
comptime HIRES_FX    = "/home/alex/mojodiffusion/serenitymojo/models/dit/parity/nava_fx_hires.safetensors"
comptime OUT_DIR     = "/home/alex/mojodiffusion/output/nava_hires"

comptime NSTEP = 25
comptime VID_G = Float32(3.0)
comptime AUD_G = Float32(2.0)

# Hi-res latent grid: F=5, Hlat=30, Wlat=52 → 7800 tokens
comptime _LAT_TOK = 7800   # F * Hlat * Wlat = 5 * 30 * 52
comptime _LAT_C   = 48
# Wan22 VAE decoder: LH=30, LW=52 → output [1,3,17,480,832]
comptime _LH = 30
comptime _LW = 52
comptime _NFRAMES = 17   # (F-1)*4+1 = 17 for F=5


# ── 2-field Movable struct for latent pair ────────────────────────────────────
struct LatentPairHires(Movable):
    var vid: Tensor  # [7800, 48] F32
...
# ── Phase A: denoise — NavaDiTHires lives only inside this def ────────────────
def _denoise_hires(
    pos_text: Tensor,      # [122,4096] F32 (on device)
    neg_text: Tensor,      # [126,4096] F32 — video negative prompt (uncond)
    init_lat_vid: Tensor,  # [7800,48] F32
    init_lat_aud: Tensor,  # [34,128]  F32
    ctx: DeviceContext,
) raises -> LatentPairHires:
    """Run NSTEP-step faithful 3-forward CFG denoise with NavaDiTHires at 832×480.
    ...
    return LatentPairHires(lv^, la^)


# ── main ──────────────────────────────────────────────────────────────────────
def main() raises:
    var ctx = DeviceContext()
    print("=== NAVA hi-res pipeline 832x480 (", NSTEP, "steps, 3-fwd CFG vid=3 aud=2) ===")

    # Load text inputs from first-run fixture (pos/neg text resolution-independent).
    print("Loading text inputs fixture...")
    var fx_in = ShardedSafeTensors.open(INPUTS_FX)
    var pos_text = Tensor.from_view_as_f32(fx_in.tensor_view("pos_text"), ctx)  # [122,4096]
    var neg_text = Tensor.from_view_as_f32(fx_in.tensor_view("neg_text"), ctx)  # [126,4096]

    # Load seeded hi-res latents from the hi-res fixture (deterministic seed).
    print("Loading hi-res seeded latents...")
    var fx_hr = ShardedSafeTensors.open(HIRES_FX)
    var init_lat_vid = Tensor.from_view_as_f32(fx_hr.tensor_view("in_lat_vid"), ctx)  # [7800,48]
    var init_lat_aud = Tensor.from_view_as_f32(fx_hr.tensor_view("in_lat_aud"), ctx)  # [34,128]

    print("  pos_text:", pos_text.shape(),
          "  init_lat_vid:", init_lat_vid.shape(),
          "  init_lat_aud:", init_lat_aud.shape())

    # Phase A: denoise (NavaDiTHires scoped inside _denoise_hires, freed on return).
    print("Phase A: denoise (832x480, ~45-90 min at full SDPA)...")
    var latents = _denoise_hires(pos_text, neg_text, init_lat_vid, init_lat_aud, ctx)
    print("Phase A done. lv:", latents.vid.shape(), " la:", latents.aud.shape())

    # Phase B: video decode + 17 frames (Wan22 VAE, LH=30, LW=52 → 480×832).
    print("Phase B: video decode (480x832, 17 frames)...")
    var vdec = Wan22VaeImageDecoder[_LH, _LW].load(VID_VAE_CKPT, ctx, f32=True)
    var frames = vdec.decode_video_tokens(latents.vid, 5, ctx)  # [1,3,17,480,832]
    var fs = frames.shape()
    print("  frames out: [", fs[0], ",", fs[1], ",", fs[2], ",", fs[3], ",", fs[4], "]")

    for f in range(_NFRAMES):
        # Slice frame f along temporal dim (dim 2) → [1,3,1,480,832].
        var frm_5d = slice(frames, 2, f, 1, ctx)
        # Reshape to [1,3,480,832].
    ...
    # Phase C: audio decode + WAV (unchanged from 256² pipeline).
    ...
    print("WROTE output/nava_hires/ : 17 frames (480x832) + audio.wav")
