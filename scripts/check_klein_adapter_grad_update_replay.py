...
validates the SerenityTrainer adapter phase dump, proves whether the captured step is
...
NEXT_MOJO_GATE = (
    ...
    "deltas from a real update-bearing SerenityTrainer step. "
    ...
    "None of those gates run Klein predict/backward_lora, compare SerenityTrainer "
    "optimizer moment tensor payloads, or prove a real nonzero SerenityTrainer update. "
    "The bounded synthetic-positive-lr gate can prove AdamW math from this "
    "gradient dump, but a later SerenityTrainer dump with lr_before>0 is still needed "
...
MOJO_PARITY_BLOCKERS = (
    ...
    "moment payloads plus adapter deltas against SerenityTrainer; "
    ...
    "the captured SerenityTrainer step has lr_before=0.0, so real nonzero adapter "
    "update parity still needs a later update-bearing SerenityTrainer dump",
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Inspect Klein/Flux2 SerenityTrainer adapter grad/update evidence."
    )
    ...
    parser.add_argument(
        ...
        action="store_true",
        help=(
            "exit 2 unless the existing SerenityTrainer gradients produce a nonzero "
    ...
    parser.add_argument(
        ...
        help=(
            ...
            "parity evidence against the SerenityTrainer train-ref artifacts"
        ),
    )
...
def adamw_positive_lr_delta_stats(
    ...
    *,
    ...
    # optimizer-math oracle, not a substitute for a real later SerenityTrainer step.
...
def inspect(args: argparse.Namespace) -> dict[str, Any]:
    ...
    return {
        ...
        "scope": "SerenityTrainer Klein adapter gradient/update oracle only; not Mojo backward or AdamW parity.",
        "meta": str(args.meta),
        ...
        "synthetic_positive_lr_adamw": {
            "scope": (
                "bounded optimizer-math replay from existing SerenityTrainer "
                "adapter_post_clip and adapter_post_clip_grad tensors; not a "
                "real later SerenityTrainer model step and not Mojo backward parity"
            ),
            "hparams": opt_hparams,
            "state_step_before": 1,
            "state_step_after": 2,
...
def print_text(summary: dict[str, Any]) -> None:
    print("[klein-adapter-grad-update] PASS SerenityTrainer oracle")
    ...
    try:
        ...
        if args.require_update_bearing and not bool(summary["has_update_oracle"]):
            print(
                ...
                "later SerenityTrainer step with lr_before>0 for nonzero update parity.",
                file=sys.stderr,
            )
            return 2
