...
from serenitymojo.training.full_finetune_contract import (
    ...
    full_finetune_model_type_has_serenity_trainer_registration,
...
def _check_unsupported_target(key: String, expected_runner: String) raises:
    ...
    _check(target.serenity_trainer_fine_tune_registered, key + String(" registered"))
    _check(
        target.real_train_loop_name == expected_runner,
        key + String(" runner mismatch"),
    )
    _check(
        target.product_loop_status == SERENITY_FULL_FT_LOOP_UNSUPPORTED,
...
def _gate_targets() raises:
    ...
    _check(
        anima.reference_root == String("/home/alex/SerenityTrainer-anima-ref"),
        "Anima must use SerenityTrainer-anima-ref",
    )
    ...
    _check(
        not full_finetune_model_type_has_serenity_trainer_registration(String("STABLE_DIFFUSION_3")),
    ...
    _check(
        full_finetune_model_type_has_serenity_trainer_registration(String("FLUX_FILL_DEV_1")),
        "FLUX_FILL_DEV_1 registration",
    )
    _check(
        not full_finetune_model_type_has_serenity_trainer_registration(String("LTX_2")),
        "LTX_2 is outside this full-finetune target contract",
    )
    _check(
        full_finetune_loop_status_name(SERENITY_FULL_FT_LOOP_UNSUPPORTED)
        == String("unsupported"),
        "status name",
    )
