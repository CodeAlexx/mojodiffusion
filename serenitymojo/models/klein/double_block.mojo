...
# int8 W8A8 base ops (mirror single_block.mojo slice 1): quantize the frozen base
# weight TENSORWISE once at load; per step run int8×int8→int32 GEMM with per-token
# activation quant (int8_linear_fwd) — NO per-step dequant.
...
from serenitymojo.ops.linalg_backward import (
    ...
    linear_backward_dw, LinearGrads,
)
...
struct StreamWeights(Copyable, Movable):
    ...
    # int8-resident (pin_residents_int8), the 4 base matmul weights above are the
    # resident int8 [N,K] tensors (dummies for the bf16 GEMM, which is skipped) and
    ...
    def __init__(
        ...
        var int8: Optional[StreamInt8] = None,
    ):
        self.wqkv = wqkv^
        self.wproj = wproj^
        self.wgu = wgu^
...
struct DoubleBlockWeights(Copyable, Movable):
    ...
    # int8-W8A8 loader sidecar → the whole-block payload the int8 forward wants.
    # Some(DoubleBlockInt8) iff both streams were loaded int8-resident; None on the
    # bf16 path (default). The int8 forward takes int8 as a separate argument, so
    # the bf16 forward signature/behaviour is unchanged (ADDITIVE).
    def int8_payload(self) -> Optional[DoubleBlockInt8]:
        if self.img.int8:
            if self.txt.int8:
                return Optional[DoubleBlockInt8](DoubleBlockInt8(
                    self.img.int8.value().copy(), self.txt.int8.value().copy()
                ))
        return Optional[DoubleBlockInt8](None)
...
#
# Targets (per stream, matching SerenityTrainer Flux2LoRASetup):
...
# SerenityTrainer modules with separate A/B tensors and gradients.
#
...
# Per-stream LoRA dropout salts for the 6 SEPARATE SerenityTrainer adapters
# (q,k,v,out,ff_in,ff_out). SerenityTrainer gives every LoRAModule its own dropout
...
# ── 1:1 SerenityTrainer SEPARATE-Linear LoRA adapters per stream ──────────────────
# SerenityTrainer/Flux2LoRASetup.py:57 wraps SEPARATE diffusers nn.Linear modules
...
#
#   diffusers (transformer_flux2.py)         SerenityTrainer wraps  → our slot
...
# = 48, the total is 96 + 48 = 144 — matching SerenityTrainer exactly (was 80).
#
...
#   their OWN lora_down/lora_up. SerenityTrainer has three separate down-projections,
...
# Per-stream LoRA grads: d_A/d_B for the 6 SEPARATE SerenityTrainer adapters
...
# ══════════════════════════════════════════════════════════════════════════════
# int8 W8A8 QUANTIZED-RESIDENT BASE PATH (slice 2: double block forward)
#
# MIRRORS single_block.mojo's slice-1 SingleBlockInt8 pattern + krea2's in-function
# dispatch. Each Klein double block has 8 frozen base matmuls: the img and txt
# streams each own their StreamWeights, so 4 int8 weights per stream:
#   idx 0 = wqkv [3D, D]  (attn qkv proj) — contracts K=D
#   idx 1 = wproj[D,  D]  (attn out proj) — contracts K=D
#   idx 2 = wgu  [2F, D]  (ff gate+up)    — contracts K=D
#   idx 3 = wd   [D,  F]  (ff down)       — contracts K=F
...
# quant (int8_linear_fwd) — NO per-step dequant. LoRA adapters stay bf16 and
# UNCHANGED. F32-accum, bf16 at the GEMM boundary, cast back to the activation
# dtype so the surrounding graph is dtype-identical to the bf16 base path
# (== single_block's _base_fwd_i8; see that header).
# ══════════════════════════════════════════════════════════════════════════════

# int8 payload for ONE stream's 4 frozen base matmul weights. List order = the
# base-matmul index above: [0]=wqkv [1]=wproj [2]=wgu [3]=wd.
# w8t = PRE-TRANSPOSED [K,N] copies for the backward's dX NT GEMM (same int8
# values, transposed ONCE at quantize time — bit-identical dX on the ~2× faster
# i16832 kernels; see single_block.SingleBlockInt8). May be EMPTY (len 0) →
# the bwd falls back to the NN stored-orientation path unchanged.
struct StreamInt8(Copyable, Movable):
    var w8: List[TArc]     # len 4: int8 [N,K]
    var scale: List[TArc]  # len 4: F32 scalar [1]
    var w8t: List[TArc]    # len 4 (or 0 = NN fallback): int8 [K,N]
...
# ── bwd dX dispatch: NT via the pre-transposed W8T when present, else
# TRANSPOSE-ON-VISIT + NT (vec int8 transpose of the already-staged W8 — no
# extra PCIe; the staged-host-W8T design measured copy-bound). Bit-identical
# in every arm (exact int32 accumulate; gate: ops/tests/int8_linear_parity.mojo).
# Mirror of single_block._i8_bwd_dx* — see its header for the measurements. ───
def _i8_bwd_dx_dbl(
    g_bf: Tensor, p8: StreamInt8, idx: Int, ctx: DeviceContext
) raises -> Tensor:
...
def _i8_bwd_dx_f32_dbl(
    g: Tensor, p8: StreamInt8, idx: Int, ctx: DeviceContext
) raises -> Tensor:
    if len(p8.w8t) == len(p8.w8):
...
# int8 payload for a whole double block: one StreamInt8 per stream (img + txt).
struct DoubleBlockInt8(Copyable, Movable):
    var img: StreamInt8
    var txt: StreamInt8

    def __init__(out self, var img: StreamInt8, var txt: StreamInt8):
        self.img = img^
        self.txt = txt^


# Builder: quantize ONE stream's 4 frozen bf16 base weights TENSORWISE → int8.
# w.wqkv/wproj/wgu/wd must be BF16 (== int8_encode_tensorwise's requirement).
def quantize_stream_int8(w: StreamWeights, ctx: DeviceContext) raises -> StreamInt8:
    var w8 = List[TArc]()
    var scale = List[TArc]()
    var w8t = List[TArc]()
    var s0 = int8_tensorwise_scale(w.wqkv[], ctx)
    var q0 = int8_encode_tensorwise(w.wqkv[], s0, ctx)
    w8t.append(TArc(int8_transpose(q0, ctx)))
    w8.append(TArc(q0^))
    scale.append(TArc(s0^))
    var s1 = int8_tensorwise_scale(w.wproj[], ctx)
    var q1 = int8_encode_tensorwise(w.wproj[], s1, ctx)
    w8t.append(TArc(int8_transpose(q1, ctx)))
    w8.append(TArc(q1^))
    scale.append(TArc(s1^))
    var s2 = int8_tensorwise_scale(w.wgu[], ctx)
    var q2 = int8_encode_tensorwise(w.wgu[], s2, ctx)
    w8t.append(TArc(int8_transpose(q2, ctx)))
    w8.append(TArc(q2^))
    scale.append(TArc(s2^))
    var s3 = int8_tensorwise_scale(w.wd[], ctx)
    var q3 = int8_encode_tensorwise(w.wd[], s3, ctx)
    w8t.append(TArc(int8_transpose(q3, ctx)))
    w8.append(TArc(q3^))
    scale.append(TArc(s3^))
    return StreamInt8(w8^, scale^, w8t^)


# Builder: quantize a whole double block (both streams' 8 base weights), ONCE.
def quantize_double_block_int8(
    img_w: StreamWeights, txt_w: StreamWeights, ctx: DeviceContext
) raises -> DoubleBlockInt8:
    return DoubleBlockInt8(
        quantize_stream_int8(img_w, ctx), quantize_stream_int8(txt_w, ctx)
    )


# Frozen base forward y = a @ W[idx]ᵀ (no bias). int8 W8A8 when the payload is
# present (cast F32 act → bf16 at the GEMM, bf16 result → a's dtype), else the
# unchanged bf16 `linear`. Mirror of single_block's `_base_fwd_i8`.
def _base_fwd_i8_dbl(
    a: Tensor, w_bf: Tensor, s8: Optional[StreamInt8], idx: Int, ctx: DeviceContext,
) raises -> Tensor:
    if s8:
        ref p = s8.value()
        var o: Tensor
        if a.dtype() == STDtype.BF16:
            o = int8_linear_fwd(a, p.w8[idx][], p.scale[idx][], ctx)
        elif a.dtype() == STDtype.F32:
            # FUSED F32 boundary (2026-07-11): bit-identical to
            # cast(a,BF16) → int8_linear_fwd → cast(out,F32), minus 2 kernels.
            return int8_linear_fwd_f32(a, p.w8[idx][], p.scale[idx][], ctx)
        else:
            var ab = cast_tensor(a, STDtype.BF16, ctx)
            o = int8_linear_fwd(ab, p.w8[idx][], p.scale[idx][], ctx)
        if o.dtype() != a.dtype():
...
def _stream_pre_lora_resident[
    ...
    var ln1 = lnmod_placeholder(ctx)
    var norm = layer_norm_modulate(x[], mv.scale1[], mv.shift1[], eps, ctx, ln1)
    # SEPARATE q/k/v LoRA (SerenityTrainer attn.to_q/to_k/to_v, transformer_flux2.py
    ...
    if w.nvfp4:
        ...
        ref p8f = s8.value()
        var bands = int8_linear_fwd_f32_bands(
            norm, p8f.w8[0][], p8f.scale[0][], D, D, D, 0, ctx,
        )
        q_pre_flat = int8_band_tensor(bands[0])
        k_pre_flat = int8_band_tensor(bands[1])
        v_flat = int8_band_tensor(bands[2])
    else:
        # FROZEN base wqkv (idx 0): int8 W8A8 when s8 present, else unchanged bf16.
        var qkv = _base_fwd_i8_dbl(norm, w.wqkv[], s8, 0, ctx)   # [N,3D]
        q_pre_flat = slice(qkv, 1, 0, D, ctx)
        k_pre_flat = slice(qkv, 1, D, D, ctx)
        v_flat = slice(qkv, 1, 2 * D, D, ctx)
    if lo.q:
...
def _stream_post_lora_resident(
    ...
    if recompute_only:
        ...
        # recompute consumes ONLY the saved set (attn_res/ln2/mlp_in/gu/gate/
        # up/act) — the aux-off devgrads backward never reads the wd output or
        # the block output. Skip wd (int8 quant+GEMM+dequant on [N,F]), the
        # ff_out LoRA fwd chain and the final residual_gate. All saved fields
        # were produced by the IDENTICAL kernels/order → tape bit-identical.
        var attn_res_arc = TArc(attn_res^)
        return _StreamPost(
...
def double_block_lora_forward_device_resident_scratch[
    ...
    drop: DoubleBlockLoraDropout = DoubleBlockLoraDropout(),
    int8: Optional[DoubleBlockInt8] = None,
    recompute_only: Bool = False,
) raises -> DoubleBlockDeviceForward:
    var scale = Float32(1.0) / sqrt(Float32(Dh))

    # Split the block int8 payload into per-stream StreamInt8 (img+txt each own
    # their StreamWeights → their own 4 int8 weights). None → bf16 base path.
    var img_i8 = Optional[StreamInt8](None)
    var txt_i8 = Optional[StreamInt8](None)
    if int8:
        img_i8 = Optional[StreamInt8](int8.value().img.copy())
        txt_i8 = Optional[StreamInt8](int8.value().txt.copy())

    var ip = _stream_pre_lora_resident[H, Dh](
        img_x, w.img, img_mod, lora.img, N_IMG, D, eps, norm_ones, norm_zeros, ctx, drop.img, img_i8)
    var tp = _stream_pre_lora_resident[H, Dh](
        txt_x, w.txt, txt_mod, lora.txt, N_TXT, D, eps, norm_ones, norm_zeros, ctx, drop.txt, txt_i8)
    ...
    var ipost = _stream_post_lora_resident(
        img_x, img_att, w.img, img_mod, lora.img, N_IMG, D, F, eps, norm_ones, norm_zeros, ctx, drop.img, img_i8,
        recompute_only)
    var tpost = _stream_post_lora_resident(
        txt_x, txt_att, w.txt, txt_mod, lora.txt, N_TXT, D, F, eps, norm_ones, norm_zeros, ctx, drop.txt, txt_i8,
        recompute_only)
...
def _stream_post_backward_lora_resident_scratch(
    ...
    var d_gate2 = List[Float32]()
    if compute_aux_grads:
        # Aux recompute of mlp_y (== forward wd out). Only d_gate2 depends on its
        # VALUE, so int8 here matches the int8 forward w/o touching the dX path.
        var mlp_y: Tensor
        if s8:
            mlp_y = _base_fwd_i8_dbl(sv.act[], w.wd[], s8, 3, ctx)   # idx 3 = wd
        else:
            var no_bias_mlp = Optional[Tensor](None)
            mlp_y = linear(sv.act[], w.wd[], no_bias_mlp^, ctx)
        grg2 = gate_residual_backward(
            d_out[], sv.attn_res[], mv.gate2[], mlp_y, ctx
        )
        d_gate2 = grg2.d_g.to_host(ctx)
    ...
    var post_mark = scratch.mark()
    # frozen wd backward: d_x ONLY. int8 -> dX GEMM on wd[D,F] (contract N=D).
    var d_d_dx: Tensor
    if s8:
        ref p8 = s8.value()
        var g_bf = cast_tensor(grg2.d_y, STDtype.BF16, ctx)   # bf16 at GEMM boundary
        d_d_dx = _i8_bwd_dx_dbl(g_bf, p8, 3, ctx)   # [N,F] bf16
        if d_d_dx.dtype() != grg2.d_y.dtype():
            d_d_dx = cast_tensor(d_d_dx, grg2.d_y.dtype(), ctx)
    else:
        d_d_dx = linear_backward_dx_scratch(grg2.d_y, w.wd[], N, F, D, ctx, scratch)
    ...
    # frozen wgu backward: d_x ONLY. int8 -> dX GEMM on wgu[2F,D] (contract N=2F).
    var d_gu_dx: Tensor
    if s8:
        ref p8 = s8.value()
        var g_bf = cast_tensor(d_gu, STDtype.BF16, ctx)   # bf16 at GEMM boundary
        d_gu_dx = _i8_bwd_dx_dbl(g_bf, p8, 2, ctx)   # [N,D] bf16
        if d_gu_dx.dtype() != d_gu.dtype():
            d_gu_dx = cast_tensor(d_gu_dx, d_gu.dtype(), ctx)
    else:
        d_gu_dx = linear_backward_dx_scratch(
    ...
    var d_gate1 = List[Float32]()
    if compute_aux_grads:
        # Aux recompute of proj_out (== forward wproj out + LoRA). Only d_gate1
        # depends on its VALUE, so int8 here matches the int8 forward.
        var proj_out: Tensor
        if s8:
            proj_out = _base_fwd_i8_dbl(att[], w.wproj[], s8, 1, ctx)   # idx 1 = wproj
        else:
            var no_bias = Optional[Tensor](None)
            proj_out = linear(att[], w.wproj[], no_bias^, ctx)
        if lo.out:
            var dlt = _klein_lora_fwd_dropout(att[], lo.out.value(), N, drop.out, ctx)
            proj_out = add(proj_out, dlt, ctx)
        grg1 = gate_residual_backward(
    ...
    # frozen wproj backward: d_x ONLY. int8 -> dX GEMM on wproj[D,D] (contract N=D).
    var d_att_t: Tensor
    if s8:
        ref p8 = s8.value()
        var g_bf = cast_tensor(grg1.d_y, STDtype.BF16, ctx)   # bf16 at GEMM boundary
        d_att_t = _i8_bwd_dx_dbl(g_bf, p8, 1, ctx)   # [N,D] bf16
        if d_att_t.dtype() != grg1.d_y.dtype():
            d_att_t = cast_tensor(d_att_t, grg1.d_y.dtype(), ctx)
    else:
        d_att_t = linear_backward_dx(grg1.d_y, w.wproj[], N, D, D, ctx)
...
def _stream_pre_backward_lora_resident[
    ...
    # SEPARATE q/k/v LoRA backward (SerenityTrainer to_q/to_k/to_v, add_q/k/v_proj).
...
def _stream_pre_backward_lora_resident_scratch[
    ...
    # frozen wqkv backward: d_x ONLY. int8 -> dX GEMM on wqkv[3D,D] (contract N=3D).
    var d_norm_t: Tensor
    if s8:
        ref p8 = s8.value()
        var g_bf = cast_tensor(d_qkv, STDtype.BF16, ctx)   # bf16 at GEMM boundary
        d_norm_t = _i8_bwd_dx_dbl(g_bf, p8, 0, ctx)   # [N,D] bf16
        if d_norm_t.dtype() != d_qkv.dtype():
            d_norm_t = cast_tensor(d_norm_t, d_qkv.dtype(), ctx)
    else:
        d_norm_t = linear_backward_dx_scratch(
            d_qkv, w.wqkv[], N, D, 3 * D, ctx, scratch,
        )

    # SEPARATE q/k/v LoRA backward (see resident variant for the SerenityTrainer mapping).
    # Dropout-aware (LoRAModule.py:328); p==0 -> base klein_lora_bwd_device_resident.
    var q_d_a = List[Float32]()
    var q_d_b = List[Float32]()
    var k_d_a = List[Float32]()
...
def double_block_lora_backward_device_resident_scratch[
    ...
    # Split the block payload into per-stream int8 payloads (== the forward).
    var img_s8 = Optional[StreamInt8](None)
    var txt_s8 = Optional[StreamInt8](None)
    if int8:
        img_s8 = Optional[StreamInt8](int8.value().img.copy())
        txt_s8 = Optional[StreamInt8](int8.value().txt.copy())
...
# EXACT math mirrors of the `_scratch` (host-grads) stream backwards the
# product tape path runs — including the int8 dX GEMMs and (at the block level)
# the KLEIN_SDPA_FLASH backward arm, which the `_tensors` graph variants above
# do NOT carry. ONE data-movement change: LoRA d_A/d_B leave as DEVICE F32
# tensors (`_klein_lora_bwd_dropout_tensors`) instead of host lists.
def _stream_post_backward_lora_resident_scratch_devgrads(
    d_out: TArc, x: TArc, att: TArc,
    w: StreamWeights, mv: ModVecsDevice, lo: StreamLoraDevice, sv: StreamSaved,
    N: Int, D: Int, F: Int, eps: Float32, ones: Tensor, ctx: DeviceContext,
    mut scratch: ScratchRingAllocator,
    ...
    # frozen wd backward: d_x ONLY. int8 -> dX GEMM on wd[D,F] (contract N=D).
    var d_d_dx: Tensor
    if s8 and grg2.d_y.dtype() == STDtype.F32:
        # FUSED F32 boundary (2026-07-11): bit-identical to cast→bwd_nn→cast,
        # minus 2 full-tensor kernels. Gate: klein_int8_f32_fusion_parity.mojo.
        ref p8f = s8.value()
        d_d_dx = _i8_bwd_dx_f32_dbl(grg2.d_y, p8f, 3, ctx)
    elif s8:
        ref p8 = s8.value()
        var g_bf = cast_tensor(grg2.d_y, STDtype.BF16, ctx)   # bf16 at GEMM boundary
        d_d_dx = _i8_bwd_dx_dbl(g_bf, p8, 3, ctx)   # [N,F] bf16
    ...
    # frozen wgu backward: d_x ONLY. int8 -> dX GEMM on wgu[2F,D] (contract N=2F).
    var d_gu_dx: Tensor
    if s8 and d_gu.dtype() == STDtype.F32:
        # FUSED F32 boundary (2026-07-11): bit-identical to cast→bwd_nn→cast.
        ref p8f = s8.value()
        d_gu_dx = _i8_bwd_dx_f32_dbl(d_gu, p8f, 2, ctx)
    elif s8:
        ref p8 = s8.value()
        var g_bf = cast_tensor(d_gu, STDtype.BF16, ctx)   # bf16 at GEMM boundary
        d_gu_dx = _i8_bwd_dx_dbl(g_bf, p8, 2, ctx)   # [N,D] bf16
    ...
    # frozen wproj backward: d_x ONLY. int8 -> dX GEMM on wproj[D,D] (contract N=D).
    var d_att_t: Tensor
    if s8 and grg1.d_y.dtype() == STDtype.F32:
        # FUSED F32 boundary (2026-07-11): bit-identical to cast→bwd_nn→cast.
        ref p8f = s8.value()
        d_att_t = _i8_bwd_dx_f32_dbl(grg1.d_y, p8f, 1, ctx)
    elif s8:
        ref p8 = s8.value()
        var g_bf = cast_tensor(grg1.d_y, STDtype.BF16, ctx)   # bf16 at GEMM boundary
        d_att_t = _i8_bwd_dx_dbl(g_bf, p8, 1, ctx)   # [N,D] bf16
    ...
    var out_d_b = Optional[TArc](None)
    if lo.out:
        var lg = _klein_lora_bwd_dropout_tensors(grg1.d_y, att[], lo.out.value(), N, drop.out, ctx)
        d_att_t = add(d_att_t, lg.d_x[], ctx)
        out_d_a = Optional[TArc](lg.d_a.copy())
        out_d_b = Optional[TArc](lg.d_b.copy())
...
def _stream_pre_backward_lora_resident_scratch_devgrads[
    H: Int, Dh: Int
](
    d_q_rms: Tensor, d_k_rms: Tensor, d_v_flat: Tensor,
    w: StreamWeights, mv: ModVecsDevice, lo: StreamLoraDevice, sv: StreamSaved,
    ...
    # frozen wqkv backward: d_x ONLY. int8 -> dX GEMM on wqkv[3D,D] (contract N=3D).
    var d_norm_t: Tensor
    if s8 and d_qkv.dtype() == STDtype.F32:
        # FUSED F32 boundary (2026-07-11): bit-identical to cast→bwd_nn→cast.
        ref p8f = s8.value()
        d_norm_t = _i8_bwd_dx_f32_dbl(d_qkv, p8f, 0, ctx)
    elif s8:
        ref p8 = s8.value()
        var g_bf = cast_tensor(d_qkv, STDtype.BF16, ctx)   # bf16 at GEMM boundary
        d_norm_t = _i8_bwd_dx_dbl(g_bf, p8, 0, ctx)   # [N,D] bf16
...
def double_block_lora_backward_device_resident_scratch_devgrads[
    H: Int, Dh: Int, N_IMG: Int, N_TXT: Int, S: Int
](
    d_io_t: TArc, d_to_t: TArc,
    w: DoubleBlockWeights, img_mod: ModVecsDevice, txt_mod: ModVecsDevice, lora: DoubleBlockLoraDevice,
    ...
    var ipb = _stream_post_backward_lora_resident_scratch_devgrads(
        d_io_t, saved.img.x, saved.img.att, w.img, img_mod, lora.img, saved.img,
        N_IMG, D, F, eps, norm_ones, ctx, scratch, compute_aux_grads, drop.img,
        img_s8,
    )
    var tpb = _stream_post_backward_lora_resident_scratch_devgrads(
        d_to_t, saved.txt.x, saved.txt.att, w.txt, txt_mod, lora.txt, saved.txt,
        N_TXT, D, F, eps, norm_ones, ctx, scratch, compute_aux_grads, drop.txt,
        txt_s8,
    )
    ...
    comptime if KLEIN_SDPA_FLASH:
        if not saved.flash_stats:
            raise Error(
                "double_block devgrads bwd: KLEIN_SDPA_FLASH on but saved tape"
                " has no flash stats (fwd/bwd flag mismatch)"
            )
        var fb = sdpa_flash_backward_f32[1, S, H, Dh](
            saved.flash_q.value(), saved.flash_k.value(),
            saved.flash_v.value(), saved.flash_o.value(),
    ...
    var iprb = _stream_pre_backward_lora_resident_scratch_devgrads[H, Dh](
        d_img_q, d_img_k, d_img_v, w.img, img_mod, lora.img, saved.img,
        N_IMG, D, eps, norm_ones, ctx, scratch, compute_aux_grads, drop.img,
        img_s8,
    )
    var tprb = _stream_pre_backward_lora_resident_scratch_devgrads[H, Dh](
        d_txt_q, d_txt_k, d_txt_v, w.txt, txt_mod, lora.txt, saved.txt,
        N_TXT, D, eps, norm_ones, ctx, scratch, compute_aux_grads, drop.txt,
        txt_s8,
    )
...
#
# Same hand-chain as double_block_lora_backward_device_resident above, but the
# 8 base matmuls (per stream: wqkv, wproj, wgu, wd) are TRAINABLE: each
# projection site emits d_W = d_yᵀ @ x_saved via linear_backward_dw with
# EXPLICIT STDtype.F32 output (the BOOL default = "match input dtype" -> bf16
# grads the F32 optimizer rejects — krea2 trap), alongside the plain d_x carry
# (linear_backward_dx). NO LoRA anywhere, NO int8.
#
# v1 trainable surface = the 8 matmuls only. FROZEN (documented delta vs reference trainer,
# which trains all transformer params): q_norm/k_norm rms scales and the 2x6
# modulation vectors — their grads are SKIPPED (gate_residual_backward_dxdy /
# modulate_backward(compute_param_grads=False)).
#
# dX fold order (C15) matches the LoRA device-resident backward EXACTLY:
# post-stream (wd-dx -> swiglu -> wgu-dx -> modulate -> ln2 ->
# d_attn_res_total = add(grg2.d_x, norm_branch) -> wproj-dx) -> joint d_att
# concat (txt FIRST) -> sdpa -> rope -> cat_backward splits -> pre-stream
# (rms-dx -> d_qkv concat(q,k,v) -> wqkv-dx -> modulate -> ln1) ->
# d_x = add(post_residual_branch, pre_norm_branch).
#
# SDPA arm: v1 consumes the always-saved math tape (saved.q_rope/k_rope/
# v_joint via sdpa_backward) — every fwd variant fills those, including the
# flash forward. A flash-tape arm (saved.flash_*) is P3 trainer wiring.
#
# Oracle: double_block_oracle.py is base-only (no LoRA) and dumps
# ref_{img,txt}_d_{wqkv,wproj,wgu,wd} (torch W.grad) + ref_d_img/ref_d_txt —
# the FT gate (parity/double_block_ft_parity.mojo) compares all at cos>=0.999.
# ═══════════════════════════════════════════════════════════════════════════
struct _StreamPostBackFT(Movable):
    var d_x: TArc        # [N,D] residual branch into the stream input
    var d_att: TArc      # [N,D] grad into the joint-attention slice
    var dw_proj: TArc    # F32 [D,D]
    var dw_gu: TArc      # F32 [2F,D]
    var dw_d: TArc       # F32 [D,F]
    # SURF mod-grad chunks (F32), COMPUTE order:
    #   [0]=d_gate2 [D], [1]=d_shift2 [D], [2]=d_scale2 [D], [3]=d_gate1 [D].
    # EMPTY unless SURF (Phase B).
    var modp: List[TArc]

    def __init__(
        out self, var d_x: TArc, var d_att: TArc,
        var dw_proj: TArc, var dw_gu: TArc, var dw_d: TArc,
        var modp: List[TArc],
    ):
        self.d_x = d_x^
        self.d_att = d_att^
...
def _stream_post_backward_ft_dev[SURF: Bool = False](
    d_out: TArc, x: TArc, att: TArc,
    w: StreamWeights, mv: ModVecsDevice, sv: StreamSaved,
    N: Int, D: Int, F: Int, eps: Float32, ones: Tensor, ctx: DeviceContext,
) raises -> _StreamPostBackFT:
    var modp = List[TArc]()
    # final = residual_gate(attn_res, gate2, mlp); mod grads frozen (v1) OR
    # trainable (SURF: gate2 grad = full gate_residual d_g, d_x/d_y unchanged).
    var grg2 = gate_residual_backward_dxdy(d_out[], mv.gate2[], ctx)

    comptime if SURF:
        # d_gate2 = sum_rows d_out * mlp_out; mlp_out = linear(act, Wd) not
        # saved -> recompute (no bias). FULL gate_residual_backward d_g (d_x/d_y
        # match the dxdy chain above so the dX path is byte-unchanged).
        var nb_mlp = Optional[Tensor](None)
        var mlp_out = linear(sv.act[], w.wd[], nb_mlp^, ctx)
        var grg2f = gate_residual_backward(d_out[], sv.attn_res[], mv.gate2[], mlp_out, ctx, True)
        modp.append(TArc(cast_tensor(grg2f.d_g, STDtype.F32, ctx)))   # [0] d_gate2

    # mlp = linear(act, Wd): TRAINABLE -> d_W (F32) + d_x carry.
    var dw_d = linear_backward_dw(grg2.d_y, sv.act[], N, F, D, ctx, STDtype.F32)
    var d_d_dx = linear_backward_dx(grg2.d_y, w.wd[], N, F, D, ctx)

    var sgb = swiglu_backward(d_d_dx, sv.gate[], sv.up[], ctx)
    var d_gu = concat(1, ctx, sgb.d_gate, sgb.d_up)

    # gu = linear(mlp_in, Wgu): TRAINABLE -> d_W (F32) + d_x carry.
    var dw_gu = linear_backward_dw(d_gu, sv.mlp_in[], N, D, 2 * F, ctx, STDtype.F32)
    var d_gu_dx = linear_backward_dx(d_gu, w.wgu[], N, D, 2 * F, ctx)

    var mb2 = modulate_backward(d_gu_dx, sv.ln2[], mv.scale2[], ctx, SURF)
    comptime if SURF:
    ...
    # residual branch d_x IS d_attn_res_total (passthrough).
    var grg1 = gate_residual_backward_dxdy(d_attn_res_total[], mv.gate1[], ctx)
    var d_x_res_t = d_attn_res_total.copy()

    comptime if SURF:
        # d_gate1 = sum_rows d_attn_res * proj_out; proj_out = linear(att, Wproj)
        # not saved -> recompute (no bias). FULL d_g.
        var nb_proj = Optional[Tensor](None)
        var proj_out = linear(att[], w.wproj[], nb_proj^, ctx)
        var grg1f = gate_residual_backward(d_attn_res_total[], x[], mv.gate1[], proj_out, ctx, True)
        modp.append(TArc(cast_tensor(grg1f.d_g, STDtype.F32, ctx)))   # [3] d_gate1

    # proj_out = linear(att, Wproj): TRAINABLE -> d_W (F32) + d_x carry.
    var dw_proj = linear_backward_dw(grg1.d_y, att[], N, D, D, ctx, STDtype.F32)
    var d_att_t = linear_backward_dx(grg1.d_y, w.wproj[], N, D, D, ctx)

    return _StreamPostBackFT(
        d_x_res_t^, TArc(d_att_t^), TArc(dw_proj^), TArc(dw_gu^), TArc(dw_d^),
        modp^,
    )


struct _StreamPreBackFT(Movable):
    var d_x: TArc        # [N,D] layer_norm branch into the stream input
    var dw_qkv: TArc     # F32 [3D,D]
    # SURF q/k rms-scale grads (F32 [Dh]): [0]=d_qnorm, [1]=d_knorm. EMPTY off.
    var qk: List[TArc]
    # SURF mod-grad chunks (F32): [0]=d_shift1 [D], [1]=d_scale1 [D]. EMPTY off.
    var modp: List[TArc]

    def __init__(
        out self, var d_x: TArc, var dw_qkv: TArc,
        var qk: List[TArc], var modp: List[TArc],
    ):
        self.d_x = d_x^
        self.dw_qkv = dw_qkv^
        self.qk = qk^
...
def _stream_pre_backward_ft_dev[
    H: Int, Dh: Int, SURF: Bool = False
](
    d_q_rms: Tensor, d_k_rms: Tensor, d_v: Tensor,
    w: StreamWeights, mv: ModVecsDevice, sv: StreamSaved,
    N: Int, D: Int, eps: Float32, ones: Tensor, ctx: DeviceContext,
) raises -> _StreamPreBackFT:
    var qk = List[TArc]()
    var modp = List[TArc]()
    # d_x arm: keep the v1 dx-only rms backward BYTE-UNCHANGED. Under SURF add
    # the weight-free d_g arm (rms_norm_backward_dg, F32 out) so the trained-path
    # d_x stays identical to v1 (regression) while d_g is produced independently.
    var d_q_pre = rms_norm_backward_dx(d_q_rms, sv.q_pre[], w.q_norm[], eps, ctx)
    var d_k_pre = rms_norm_backward_dx(d_k_rms, sv.k_pre[], w.k_norm[], eps, ctx)
    comptime if SURF:
        qk.append(TArc(rms_norm_backward_dg(d_q_rms, sv.q_pre[], eps, ctx)))   # [0] d_qnorm
        qk.append(TArc(rms_norm_backward_dg(d_k_rms, sv.k_pre[], eps, ctx)))   # [1] d_knorm
    ...
    # qkv = linear(norm, Wqkv): TRAINABLE -> d_W (F32) + d_x carry.
    ...
    var mb1 = modulate_backward(d_norm_t, sv.ln1[], mv.scale1[], ctx, SURF)
    comptime if SURF:
    ...
    var d_x_norm_t = layer_norm_backward_dx(mb1.d_x, sv.x[], ones, eps, ctx)
    return _StreamPreBackFT(TArc(d_x_norm_t^), TArc(dw_qkv^), qk^, modp^)


struct DoubleBlockFTGrads(Movable):
    var d_img_x: TArc      # [N_IMG,D] img input grad (inter-block carry)
    var d_txt_x: TArc      # [N_TXT,D] txt input grad (inter-block carry)
    # len 8, F32 device, FIXED order:
    #   [0] img wqkv [3D,D]   [1] img wproj [D,D]
    #   [2] img wgu  [2F,D]   [3] img wd    [D,F]
    #   [4] txt wqkv [3D,D]   [5] txt wproj [D,D]
    #   [6] txt wgu  [2F,D]   [7] txt wd    [D,F]
    var dw: List[TArc]
    # SURF q/k rms-scale grads (F32 [Dh]), FIXED order:
    #   [0]=img q_norm [1]=img k_norm [2]=txt q_norm [3]=txt k_norm.
    # EMPTY (len 0) unless SURF (Phase B).
    var d_qk: List[TArc]
    # SURF SHARED modulation-linear FLAT grads (F32 [1,6D]), FIXED order:
    #   [0]=img_mod [1]=txt_mod, packed [shift1|scale1|gate1|shift2|scale2|gate2]
    #   (the double_stream_modulation_{img,txt}.lin.weight output layout). The
    #   STACK sums these across blocks then matmuls silu(vec). EMPTY off.
...
def double_block_ft_backward_dev[
    ...
    D: Int, F: Int, eps: Float32,
    ctx: DeviceContext,
) raises -> DoubleBlockFTGrads:
    var scale = Float32(1.0) / sqrt(Float32(Dh))
    var ones_t = _t_dtype(_ones(D), [D], saved.img.x[].dtype(), ctx)

    var ipb = _stream_post_backward_ft_dev[SURF](
        d_io_t, saved.img.x, saved.img.att, w.img, img_mod, saved.img,
        N_IMG, D, F, eps, ones_t, ctx,
    )
    var tpb = _stream_post_backward_ft_dev[SURF](
        d_to_t, saved.txt.x, saved.txt.att, w.txt, txt_mod, saved.txt,
        N_TXT, D, F, eps, ones_t, ctx,
    )

    # joint d_att (txt FIRST, then img); reshape [N,D]->[1,N,H,Dh] byte no-op.
    var d_tatt_4d = reshape(tpb.d_att[], [1, N_TXT, H, Dh], ctx)
    var d_iatt_4d = reshape(ipb.d_att[], [1, N_IMG, H, Dh], ctx)
    var d_att_joint = concat(1, ctx, d_tatt_4d, d_iatt_4d)   # [1,S,H,Dh]
    ...
    var iprb = _stream_pre_backward_ft_dev[H, Dh, SURF](
        cq.d_1, ck.d_1, cv.d_1, w.img, img_mod, saved.img, N_IMG, D, eps, ones_t, ctx,
    )
    var tprb = _stream_pre_backward_ft_dev[H, Dh, SURF](
        cq.d_0, ck.d_0, cv.d_0, w.txt, txt_mod, saved.txt, N_TXT, D, eps, ones_t, ctx,
    )

    # C15: post residual branch FIRST, pre norm branch second (LoRA order).
    var d_img_x_t = TArc(add(ipb.d_x[], iprb.d_x[], ctx))
    var d_txt_x_t = TArc(add(tpb.d_x[], tprb.d_x[], ctx))
    ...
    dw.append(ipb.dw_gu.copy())
    dw.append(ipb.dw_d.copy())
    dw.append(tprb.dw_qkv.copy())
    dw.append(tpb.dw_proj.copy())
    dw.append(tpb.dw_gu.copy())
    dw.append(tpb.dw_d.copy())

    var d_qk = List[TArc]()
    var mod_flat = List[TArc]()
    comptime if SURF:
        # q/k scale grads: [img_q, img_k, txt_q, txt_k].
        d_qk.append(iprb.qk[0].copy())
        d_qk.append(iprb.qk[1].copy())
        d_qk.append(tprb.qk[0].copy())
        d_qk.append(tprb.qk[1].copy())
        # per-stream mod FLAT [1,6D] in the mod.lin output chunk order
        # [shift1|scale1|gate1|shift2|scale2|gate2]; pre.modp=[d_shift1,d_scale1],
        # post.modp=[d_gate2,d_shift2,d_scale2,d_gate1].
        var im_flat = concat(
            0, ctx,
    ...
    return DoubleBlockFTGrads(d_img_x_t^, d_txt_x_t^, dw^, d_qk^, mod_flat^)
