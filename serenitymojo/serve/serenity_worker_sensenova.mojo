# serenitymojo.serve.serenity_worker_sensenova — STANDALONE per-kind SenseNova worker.
#
...
# newline-JSON IPC loop, but driving the REAL SensenovaBackend (GPU/MAX
# SenseNova-U1-8B-MoT pixel-space T2I pipeline — runtime Qwen3 tokenize +
# forward_und prefix encode + per-step CFG dual forward_gen + fm_head velocity +
# Euler denoise, then pixel-space unpatchify + PNG; NO VAE). The Rust control
# plane (serenity-server) spawns one of these per resident model, identically to
# the zimage/qwenimage/sdxl/ideogram4 workers: `serenity_worker_sensenova <fd>`.
#
# !!! DO NOT BUILD THIS FILE INTERACTIVELY !!!
# Like the other per-kind workers, this entrypoint imports the full SenseNova-U1
...
# via a `pixi run build-worker-sensenova-raw` (--optimization-level 2) target;
# never build it from an interactive session.
#
# Invoked:  serenity_worker_sensenova <fd>
...
from serenitymojo.serve.sensenova_backend import SensenovaBackend
...
def _sensenova_worker_loop(mut backend: SensenovaBackend, fd: Int32) raises:
    """Identical control flow to the zimage/qwenimage/sdxl/ideogram4 worker loops,
    specialized to SensenovaBackend. One job at a time (single-GPU contract)."""
    set_nonblock(fd)
...
def main() raises:
    var args = argv()
    if len(args) < 2:
        print("usage: serenity_worker_sensenova <fd>")
        return
    var fd = Int32(Int(String(args[1])))
    var b = SensenovaBackend()
    _sensenova_worker_loop(b, fd)
