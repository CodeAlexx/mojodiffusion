...
- Save format: keep LoRA output plain PEFT-style safetensors via `save_lora_peft`, matching torchref/diffusers/ComfyUI expectations. Do not save a private-only adapter format.
...
- `FMT_TORCHREF_SDXL` — `lora_unet_….lora_down/.lora_up` + `.alpha` (TE `lora_te*` skipped)
...
- `FMT_DIFFUSION_MODEL` — **`diffusion_model.<module>.lora_A/lora_B.weight`** (peft /
  torchref / ComfyUI); `_map_diffusion_model` strips `diffusion_model.`+`.default`,
...
torchref/Diffusers SDXL names; `models/dit/ideogram4_resident.mojo` accepts
...
| `training/train_minimax_h3.mojo` + `training/adamw8bit.mojo` (2026-08-15/18) | Config-driven H3 LoRA trainer: BF16 compute, device-resident blockwise AdamW8bit, canonical PEFT plus exact uint8/F32 optimizer-state resume, cuDNN flash train attention, and a configurable direct-W8A8 frozen base. The current oracle-matched recipe selects only MLP `fc1`/`fc2` adapters and reads rank/alpha, LR/warmup, gradient clip, timestep buckets, spatial-density jitter, guidance distillation, base-preservation weight/probability, resident block count, save cadence, and sample cadence from `TrainConfig`; sparse preservation uses Musubi's inverse-probability loss scaling. Samples run in a fresh process from `serenity.sample_prompts.v1`. Rootless launch is hard-capped and O2/-j1 builds avoid compiler SIGKILL. | **LIVE** (200-step mechanics: 10.00s median and zero nonfinite; config/resume smoke: 100 adapter pairs and step 1→2 optimizer-state resume; current full recipe at 48/50 resident measures 9.80s clean median and 11.82s preservation-active median; valid 512x320x22 decoded samples; this is not a convergence claim) |
