#!/usr/bin/env python3
"""Static Flux.1-dev one-step dump contract from local SerenityTrainer source.

This script intentionally does not import SerenityTrainer, torch, diffusers, or run
training. It reads the local SerenityTrainer source and baseline config, proves the
...
SERENITY_CONFIG = SERENITY_TRAINER / "configs/flux1_100step_baseline.json"
SERENITY_BASELINE_LORA = SERENITY_TRAINER / "output/flux1_100step_baseline/lora_last.safetensors"

SERENITY_MODEL = SERENITY_TRAINER / "modules/model/FluxModel.py"
SERENITY_SETUP = SERENITY_TRAINER / "modules/modelSetup/BaseFluxSetup.py"
SERENITY_LORA_SETUP = SERENITY_TRAINER / "modules/modelSetup/FluxLoRASetup.py"
SERENITY_DATALOADER = SERENITY_TRAINER / "modules/dataLoader/FluxBaseDataLoader.py"
SERENITY_TRAINER = SERENITY_TRAINER / "modules/trainer/GenericTrainer.py"
SERENITY_NOISE = SERENITY_TRAINER / "modules/modelSetup/mixin/ModelSetupNoiseMixin.py"
SERENITY_FLOW = SERENITY_TRAINER / "modules/modelSetup/mixin/ModelSetupFlowMatchingMixin.py"
SERENITY_ZIMAGE_SETUP = SERENITY_TRAINER / "modules/modelSetup/BaseZImageSetup.py"

DEFAULT_DUMP_PATH = Path("/tmp/serenity_flux1_step1_inputs.safetensors")
DEFAULT_MANIFEST_PATH = Path("/tmp/serenity_flux1_step1_inputs_manifest.json")
...
REQUIRED_READINESS_ARTIFACTS = (
    (
        ...
        "SerenityTrainer tensors through scaled/noisy/packed latent, transformer predicted_flow, flow_target, and loss.",
    ),
    (
        "raw dump manifest",
        DEFAULT_MANIFEST_PATH,
        "BaseFluxSetup.py::predict",
        "Manifest with producer, SerenityTrainer config, base model snapshot, seed, train dtype, and per-tensor metadata.",
    ),
    (
...
def resolve_port_path(value: Any) -> Path | None:
    ...
    if path.is_absolute():
        return path
    return Path("/home/alex/serenity_trainer-mojo") / path
...
def require_config(config: dict[str, Any]) -> Path:
    ...
    print(f"  config_json: {SERENITY_CONFIG}")
    expected = {
        "model_type": "FLUX_DEV_1",
        "training_method": "LORA",
        "batch_size": 1,
    ...
    print(
        "[flux-onestep-dump] PASS SerenityTrainer Flux.1 baseline config "
...
def check_serenity_trainer_sources() -> None:
    require(
        SERENITY_MODEL,
        "SerenityTrainer Flux.1 model helpers",
        [
    ...
    require(
        SERENITY_DATALOADER,
        "SerenityTrainer Flux.1 data/cache outputs",
        [
    ...
    require(
        SERENITY_LORA_SETUP,
        "SerenityTrainer Flux.1 LoRA setup",
        [
            "LoRAModuleWrapper(",
            '"lora_te1"',
            '"lora_te2"',
    ...
    require(
        SERENITY_SETUP,
        "SerenityTrainer Flux.1 predict one-step tensors",
        [
    ...
    require(
        SERENITY_NOISE,
        "SerenityTrainer discrete timestep/noise contract",
        [
            "torch.randn(",
    ...
    require(
        SERENITY_FLOW,
        "SerenityTrainer flow-noise contract",
        [
    ...
    require_block(
        SERENITY_TRAINER,
        "SerenityTrainer GenericTrainer first train-step order",
    ...
    require(
        SERENITY_ZIMAGE_SETUP,
        "Existing SerenityTrainer one-step dump pattern",
        [
            "SERENITY_DUMP_STEP1_INPUTS",
            "SERENITY_DUMP_STEP1_PATH",
            "/tmp/serenity_zimage_step1_inputs.safetensors",
            "safetensors.torch",
            "scaled_latent_image",
            "latent_noise",
            "scaled_noisy_latent_image",
...
def validate_manifest_artifact(path: Path) -> None:
    ...
    if missing:
        ...
        raise SystemExit(1)
    if str(manifest.get("serenity_trainer_config")) != str(SERENITY_CONFIG):
        raise SystemExit(
            "[flux-onestep-dump] manifest serenity_trainer_config mismatch: "
            f"{manifest.get('serenity_trainer_config')!r} != {str(SERENITY_CONFIG)!r}"
        )
...
def validate_contract_artifact(path: Path) -> None:
    ...
    if missing:
        ...
        for field in missing:
            print(f"  missing: {field}")
        raise SystemExit(1)
    if contract.get("reference_repo") != str(SERENITY_TRAINER):
        raise SystemExit(
            "[flux-onestep-dump] contract reference_repo mismatch: "
            f"{contract.get('reference_repo')!r} != {str(SERENITY_TRAINER)!r}"
        )
    if contract.get("default_config") != str(SERENITY_CONFIG):
        raise SystemExit(
            "[flux-onestep-dump] contract default_config mismatch: "
            f"{contract.get('default_config')!r} != {str(SERENITY_CONFIG)!r}"
        )
...
def report_dump_plan(
    ...
    print("  source_anchor: /home/alex/SerenityTrainer/modules/modelSetup/BaseFluxSetup.py::predict")
    print("  trainer_anchor: /home/alex/SerenityTrainer/modules/trainer/GenericTrainer.py::train")
    print("  trigger_env: SERENITY_DUMP_STEP1_INPUTS=1")
    print(f"  trigger_env: SERENITY_DUMP_STEP1_PATH={dump_path}")
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Check the local SerenityTrainer Flux.1 one-step dump contract."
    )
    parser.add_argument(
        "--dump-path",
        type=Path,
...
def main() -> int:
    ...
    if not SERENITY_BASELINE_LORA.exists():
        raise SystemExit(f"[flux-onestep-dump] missing baseline LoRA: {SERENITY_BASELINE_LORA}")
    print(f"[flux-onestep-dump] PASS baseline LoRA artifact: {SERENITY_BASELINE_LORA}")
