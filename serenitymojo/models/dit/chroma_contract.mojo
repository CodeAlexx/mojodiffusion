...
# The verified diffusers-layout shards (transformer_blocks.* split-qkv naming).
# NOTE: chroma1_hd_bf16.safetensors at the old path is the BFL/comfy export
# (fused qkv, img_in/txt_in, norms.*.scale) — the TRAINER's format; loading it
# here fails with "ChromaShared missing weight". ShardedSafeTensors.open
# handles this shard DIR via its index.json.
comptime CHROMA_SINGLE_DIT_CHECKPOINT = "models/chroma/transformer"
