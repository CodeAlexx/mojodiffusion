#!/usr/bin/env python3
"""Report-first guard for Chroma SerenityTrainer LoRA key parity."""
...
SERENITY_SETUP = SERENITY_TRAINER / "modules/modelSetup/ChromaLoRASetup.py"
SERENITY_SAVER = SERENITY_TRAINER / "modules/modelSaver/chroma/ChromaLoRASaver.py"
SERENITY_SAVER_MIXIN = SERENITY_TRAINER / "modules/modelSaver/mixin/LoRASaverMixin.py"
SERENITY_CONVERTER = SERENITY_TRAINER / "modules/util/convert/lora/convert_chroma_lora.py"
SERENITY_CONVERT_UTIL = SERENITY_TRAINER / "modules/util/convert/lora/convert_lora_util.py"
SERENITY_CONFIG = SERENITY_TRAINER / "configs/chroma_100step_baseline.json"
SERENITY_BASELINE = SERENITY_TRAINER / "output/chroma_100step_baseline/lora.safetensors"
...
def check_serenity_source() -> None:
    ...
    require(setup, 'model.text_encoder, "lora_te"', "reference trainer text encoder prefix")
    ...
    require(setup, 'state_dict_has_prefix(model.lora_state_dict, "lora_te")', "reference trainer text encoder resume trigger")
    ...
    print("[chroma-lora-keys] SerenityTrainer source contract: PASS")
    print(
        "[chroma-lora-keys] SerenityTrainer setup inventory: "
        "raw prefixes transformer=lora_transformer text_encoder=lora_te "
        "layer_filter=config.layer_filter.split(',')"
    )
    print(
        "[chroma-lora-keys] SerenityTrainer conversion inventory: "
    ...
    print(
        "[chroma-lora-keys] SerenityTrainer Chroma targets: "
...
def check_saved_header(
    ...
    print(
        "[chroma-lora-keys] SerenityTrainer saved inventory: PASS "
...
def check_mojo_sources(
    ...
    if expected_specs is not None:
        ...
        if unsupported:
            blockers.append(
                "selected Chroma SerenityTrainer baseline includes unsupported Mojo LoRA groups: "
    ...
    if "lora_te" not in stack or "save_chroma_text_encoder" not in stack:
        warnings.append("Chroma stack has no text-encoder lora_te save/resume/training surface for TE-enabled SerenityTrainer configs.")
    if not all(token in stack for token in ("lora_transformer_x_embedder", "lora_transformer_context_embedder", "lora_transformer_proj_out")):
        warnings.append(
            "Chroma stack freezes stack-level SerenityTrainer transformer targets "
...
def check_train_ref_artifact_consumer() -> None:
    ...
    if missing:
        ...
        raise SystemExit(
            "[chroma-lora-keys] missing local Chroma SerenityTrainer train-ref artifact(s): "
...
def main() -> int:
    ...
    if args.safetensors is not None:
        ...
        print("[chroma-lora-keys] SerenityTrainer saved inventory: SKIP (baseline safetensors not present)")
