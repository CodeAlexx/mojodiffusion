...
defines the concrete SerenityTrainer/Mojo artifact bundle required before a Flux.1
...
DEFAULT_MANIFEST = Path("/home/alex/serenity_trainer-mojo/parity/flux1_sampler_parity_manifest.json")
...
REQUIRED_ARTIFACT_KEYS = (
    ...
    "serenity_trainer_pooled_prompt_embedding",
...
def _template_artifact_groups() -> dict[str, Any]:
    return {
        ...
        "paired_text_conditioning": [
            ...
            {
                "serenity_trainer": "serenity_trainer_pooled_prompt_embedding",
...
def required_paired_sampler_fields(target_manifest: Path = DEFAULT_MANIFEST) -> dict[str, Any]:
    return {
        "serenity_trainer_reference": "/home/alex/SerenityTrainer FluxSampler.py, same prompt/seed/resolution/steps/cfg/dtype",
    ...
    if not path.exists():
        return [
            Check(
                False,
                "manifest",
                f"missing {path}; expected paired reference trainer/Mojo Flux.1 sampler evidence manifest",
            )
        ]
