# training/lora_adamw_serenity_fused_parity.mojo — BIT-EQUALITY gate [GPU]:
# fused_lora_adamw_serenity_step (GPU kernel) vs _lora_adamw_precomputed (the host
...
#       serenitymojo/training/lora_adamw_serenity_fused_parity.mojo -o /tmp/adamw_par \
#     && /tmp/adamw_par
...
from serenitymojo.training.lora_adamw_serenity_fused import (
    _rne_bf16_exact, _sr_bf16_exact, fused_lora_adamw_serenity_step,
)
...
def main() raises:
    print("=== lora_adamw_serenity_fused parity: host loop vs GPU kernel ===")
    ...
    if fails != 0:
        raise Error("lora_adamw_serenity_fused_parity: " + String(fails) + " failures")
