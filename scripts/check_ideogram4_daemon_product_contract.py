...
RESIDENT_DIT = REPO / "serenitymojo/models/dit/ideogram4_resident.mojo"
TILED_DECODE = REPO / "serenitymojo/models/vae/ideogram4_tiled_decode.mojo"
...
def check_static(blockers: list[str]) -> dict[str, Any]:
    ...
    resident_dit = read(RESIDENT_DIT, blockers)
    tiled_decode = read(TILED_DECODE, blockers)
    ...
    require(
        "Ideogram4Weights.load" in backend
        and "ideogram4_forward_r_masked[TOTAL_, TEXT_TOKENS]" in backend
        and "self.prompt_tokens" in backend,
        "backend must pass the real prompt length into native padded-text attention",
        blockers,
    )
    require(
        "sdpa_qwen_flash_padmask" in resident_dit
        and "ideogram4_forward_r_masked" in resident_dit,
        "resident Ideogram DiT must exclude padded text from conditional attention",
        blockers,
    )
    require(
        "half_h = TILE_H // 2" in tiled_decode
        and "half_w = TILE_W // 2" in tiled_decode
        and "slice(r, 3, half_w, TILE_W" in tiled_decode,
        "rectangular tiled decode must use independent height and width offsets",
        blockers,
    )
    require(
    ...
    require(
        ...
        and "1152x896" in backend
        and "896x1152" in backend
        and "1344x768" in backend
        and "768x1344" in backend
        and "1280x832" in backend
        and "832x1280" in backend
        and "BUCKET_COUNT = 7" in backend
        ...
        "compiled aspect admission or bounded fail-loud text missing",
        blockers,
    )
    require(
...
def check_artifact(png_path: Path, blockers: list[str]) -> dict[str, Any]:
    ...
    supported_sizes = {(1024, 1024), (1280, 768), (768, 1280)}
    artifact_size = (png_info.get("width"), png_info.get("height"))
    require(
        artifact_size in supported_sizes,
        f"PNG must use a compiled Ideogram4 size, got {artifact_size}",
        blockers,
    )
