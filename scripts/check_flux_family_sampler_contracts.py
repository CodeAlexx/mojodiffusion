#!/usr/bin/env python3
"""Static guard for Flux-family SerenityTrainer sampler contracts.

This is a report-first source guard. It verifies that the local SerenityTrainer
...
SERENITY_SAMPLERS = SERENITY_TRAINER / "modules/modelSampler"
SERENITY_SETUPS = SERENITY_TRAINER / "modules/modelSetup"
SERENITY_MODELS = SERENITY_TRAINER / "modules/model"
SERENITY_CONFIGS = SERENITY_TRAINER / "configs"
...
PORT_DOC = REPO / "SERENITY_MOJO_PORT_REMAINING.md"
...
FLUX1_SPEED_EVIDENCE_GROUPS = (
    (
        "SerenityTrainer seconds/step",
        ("serenity_trainer seconds/step", "serenity_baseline_seconds_per_step", "serenity_seconds_per_step"),
    ),
    ("Mojo seconds/step", ("mojo seconds/step", "mojo_seconds_per_step")),
    ("SerenityTrainer peak VRAM", ("serenity_trainer peak vram", "serenity_peak_vram_mib")),
...
CLAIM_SCAN_FILES = [
    ...
    MOJO_SAMPLING / "serenity_trainer_sampler_contract.mojo",
    MOJO_SAMPLING / "serenity_trainer_sampler_contract_smoke.mojo",
...
def require_flux1_speed_claim_evidence() -> None:
    ...
    run identity plus SerenityTrainer/Mojo speed, VRAM, dtype, and denoise-trajectory
...
def _missing_metric_keys(data: dict[str, Any], keys: tuple[str, ...]) -> list[str]:
    ...
    if not isinstance(metrics, dict):
        return [f"metrics.serenity_trainer.{key}" for key in keys] + [f"metrics.mojo.{key}" for key in keys]
    ...
    for family, spec in SAMPLER_ARTIFACT_REQUIREMENTS.items():
        ...
        if not manifest.exists():
            ...
            _print_inventory_keys(
                ...
                tuple(f"metrics.serenity_trainer.{key}" for key in metric_keys)
...
def require_flux2_dev_current_state() -> None:
    ...
    Flux2-dev is SerenityTrainer ModelType.FLUX_2 only after the loaded transformer
    ...
    expect(
        _json_at(contract, "reference_policy.only_reference") == "/home/alex/SerenityTrainer",
        "contract does not pin /home/alex/SerenityTrainer as the only reference",
    )
    expect(
    ...
    expect(
        ...
        "contract does not record SerenityTrainer ModelType.FLUX_2",
    )
    expect(
    ...
    require(
        MOJO_TRAINING / "serenity_trainer_lifecycle.mojo",
    ...
    require(
        MOJO_TRAINING / "serenity_trainer_lifecycle_smoke.mojo",
    ...
    require(
        MOJO_TRAINING / "serenity_trainer_product_run_smoke.mojo",
    ...
    catalog = read(MOJO_TRAINING / "serenity_trainer_preset_catalog.mojo")
...
def main() -> int:
    require(
        SERENITY_SAMPLERS / "FluxSampler.py",
        "SerenityTrainer Flux.1 sampler",
        [
    ...
    require(
        SERENITY_MODELS / "FluxModel.py",
        "SerenityTrainer Flux.1 model shift",
        [
    ...
    require(
        SERENITY_SETUPS / "BaseFluxSetup.py",
        "SerenityTrainer Flux.1 setup",
        [
    ...
    require(
        SERENITY_SAMPLERS / "Flux2Sampler.py",
        "SerenityTrainer shared Flux2 sampler",
        [
    ...
    require(
        SERENITY_MODELS / "Flux2Model.py",
        "SerenityTrainer Flux2 dev/Klein discriminator",
        [
    ...
    require(
        SERENITY_SETUPS / "BaseFlux2Setup.py",
        "SerenityTrainer shared Flux2 setup",
        [
            "shift = model.calculate_timestep_shift",
            "shift = shift if config.dynamic_timestep_shifting else config.timestep_shift",
            "scaled_noisy_latent_image, sigma = self._add_noise_discrete",
    ...
    require(
        SERENITY_SAMPLERS / "ChromaSampler.py",
        "SerenityTrainer Chroma sampler",
        [
    ...
    require(
        SERENITY_SETUPS / "BaseChromaSetup.py",
        "SerenityTrainer Chroma setup",
        [
    ...
    require_scheduler_config(
        "Flux.1-dev",
        SERENITY_CONFIGS / "flux1_100step_baseline.json",
        {
            "shift": 3.0,
            "base_shift": 0.5,
            "max_shift": 1.15,
    ...
    require_scheduler_config(
        "Chroma1-HD",
        SERENITY_CONFIGS / "chroma_100step_baseline.json",
        {
            "shift": 3.0,
            "base_shift": 0.5,
            "max_shift": 1.15,
    ...
    require(
        ...
        [
            ...
            "serenity_baseline_seconds_per_step",
    ...
    require(
        ...
        [
            ...
            "SERENITY_SCHEDULER_FLUX2_MU_CUSTOM_SIGMAS",
            "SERENITY_CFG_GUIDANCE_EMBED_OR_TEXTBOOK",
        ],
    )
    require(
        MOJO_SAMPLING / "serenity_trainer_sampler_contract_smoke.mojo",
