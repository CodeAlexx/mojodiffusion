#!/usr/bin/env python3
"""Static guard for the SerenityTrainer checkpoint/resume contract.

This checks the Mojo metadata contract against source markers in SerenityTrainer. It
...
CONTRACT = ROOT / "serenitymojo/training/serenity_trainer_resume_contract.mojo"
PRODUCT = ROOT / "serenitymojo/training/serenity_trainer_product_run.mojo"
SMOKE = ROOT / "serenitymojo/training/serenity_trainer_resume_contract_smoke.mojo"
PRODUCT_SMOKE = ROOT / "serenitymojo/training/serenity_trainer_product_run_smoke.mojo"
...
def main() -> int:
    ...
    generic = read(reference trainer / "modules/trainer/GenericTrainer.py")
    progress = read(reference trainer / "modules/util/TrainProgress.py")
    internal_saver = read(reference trainer / "modules/modelSaver/mixin/InternalModelSaverMixin.py")
    internal_loader = read(reference trainer / "modules/modelLoader/mixin/InternalModelLoaderMixin.py")
    generic_lora_saver = read(reference trainer / "modules/modelSaver/GenericLoRAModelSaver.py")
    generic_lora_loader = read(reference trainer / "modules/modelLoader/GenericLoRAModelLoader.py")
    lora_loader = read(reference trainer / "modules/modelLoader/mixin/LoRALoaderMixin.py")
    ...
    require_regex("save-before-sample contract", contract, r"def serenity_save_runs_before_sample\(\).*?return False")
    require("smoke validates missing optimizer", smoke, "missing_optimizer.has_optimizer_state = False")
    require("smoke validates raw LoRA rejection", smoke, "SERENITY_RESUME_SURFACE_RAW_LORA")
    ...
    for marker in [
        ...
        "_latest_backup_path",
        "SerenityProductResumePreflight",
        "validate_serenity_trainer_product_resume_preflight",
    ...
    print("SerenityTrainer resume contract static guard PASS")
    print("  checked source markers: SerenityTrainer trainer/load/save internals")
...
if __name__ == "__main__":
    try:
        ...
        print(f"SerenityTrainer resume contract static guard FAIL: {exc}")
        raise SystemExit(1)
