...
| `models/krea2/` (LoRA TRAINING) | krea2 LoRA-training port: `config.mojo`, `krea2_block.mojo` (SingleStreamDiT block fwd/save-acts + bwd + 8 LoRA Linears), `krea2_stack.mojo` (stack fwd + LoRA backward final→×28, + `*_streamed` per-block-streaming variants), `krea2_cache_reader.mojo`, `train_krea2.mojo` (streaming trainer, `KREA2_V2_GRAPH` seam). VAE=Qwen-Image, TE=Qwen3-VL-4B. Oracle=ai-toolkit. | 🟡 Phases 1-3 cos ~1.0; 4a trainer RUNS — see `parity/KREA2_TRAINING_PARITY_LEDGER.md` |
