# Replay the real Serenity Qwen train-step dump through the Mojo loss path.
#
# This is intentionally a narrow gate: it verifies Mojo computes the same MSE
# loss from Serenity's dumped `output.predicted` and `output.target`. It does
# not claim Qwen transformer forward, backward, optimizer, or sampler parity.
...
comptime PARITY = "trainer/parity/qwen_train_ref_step000.safetensors"
comptime SERENITY_LOSS = Float32(0.08948630839586258)
...
def main() raises:
    ...
    print("=== Qwen train-ref loss replay ===")
    print("[parity]", PARITY)
    ...
    # PyTorch promotes BF16 predicted - F32 target to F32 for this loss.
    ...
    if err > LOSS_EPS or err_const > LOSS_EPS:
        raise Error("Qwen train-ref loss replay mismatch")

    print("QWEN TRAIN REF LOSS REPLAY PASS")
