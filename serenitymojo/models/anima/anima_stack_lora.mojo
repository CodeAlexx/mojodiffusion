...
#   * training/{lora_save, train_step} : LoraAdapter, _lora_adamw, save_lora_serenity_trainer.
#
...
from serenitymojo.models.anima.lora_block import modulate_backward_b2_perhalf
...
from serenitymojo.models.anima.lora_block import (
    ...
    anima_block_lora_forward_b2, anima_block_lora_backward_b2,
    ...
    anima_block_lora_forward_device_tensor_b2, anima_block_lora_backward_device_tensors_b2,
)
...
# ═════════════════════════════════════════════════════════════════════════════
# TRUE BATCH-2 STREAMED LoRA STACK (row-stacked via the BATCH dim, B=2). Two
# samples processed as one B=2 forward/backward: the token-parallel GEMMs/norms/
# modulation batch over 2*S_IMG rows (the row-stacked GEMM whose M-shape change
# triggers MJ-1073); ONLY the per-block self/cross attention runs per-sample
# (anima_block_lora_forward_b2 / _backward_b2 slice the batch dim). The final
# layer + patch-embed are B-parameterized and handle B=2 unchanged. The 10×28 LoRA
# dA/dB accumulate over BOTH samples' rows = the batch gradient. Reuses the b1
# AnimaStackForward struct (all fields are [B,…]-shaped: out=[2*S_img,64], each
# blk_x_in=[2,S_img,D]). GATE: parity/anima_batch2_parity.mojo — loss_B2 vs
# mean(loss_B1(s0),loss_B1(s1)) (binding) + per-sample fwd velocity cos>=0.999
# (binding) + grad cosine INFORMATIONAL (MJ-1073). Caller MUST pass B==2 and the
# SINGLE-sample rope table [S_img*H, Dh/2] (doubled internally via _rope_tile_b2).
# ═════════════════════════════════════════════════════════════════════════════
def _rope_tile_b2(tbl: Tensor, ctx: DeviceContext) raises -> Tensor:
    # Both samples share the SAME image grid, so the single-sample [S_img*H, Dh/2]
    # rope table concatenated with itself along dim 0 gives the [2*S_img*H, Dh/2]
    # b2 table (mirrors krea2 _tile_rope_b2; anima's table already broadcasts H,
    # and both samples' spatial positions are identical → same table).
    return concat(0, ctx, tbl, tbl)


def anima_stack_lora_forward_streamed_b2[
    H: Int, Dh: Int, S_IMG: Int, S_TXT: Int
](
    patches: List[Float32],       # [2*S_img, IN_PATCH]  row-stacked
    t_cond: List[Float32],        # [2, 2048]
    base_adaln: List[Float32],    # [2, 6144]
    context: List[Float32],       # [2, S_txt, 1024]
    base: AnimaStackBase, st: SafeTensors, lora: AnimaLoraSet,
    cos: Tensor, sin: Tensor,     # SINGLE-sample [S_img*H, Dh/2] table (doubled here)
    B: Int, D: Int, JOINT: Int, F: Int, IN_PATCH: Int, OUT_PATCH: Int, eps: Float32,
    ctx: DeviceContext,
) raises -> AnimaStackForward:
    if B != 2:
        raise Error("anima_stack_lora_forward_streamed_b2 requires B==2, got " + String(B))
    var num_blocks = lora.num_blocks
    var cos2 = _rope_tile_b2(cos, ctx)      # [2*S_img*H, Dh/2]
    var sin2 = _rope_tile_b2(sin, ctx)
    var x_emb = _linear_wdev(patches, base.x_embed[], B * S_IMG, IN_PATCH, ctx)
    ...
    for bi in range(num_blocks):
        ...
        var w = load_anima_block_weights_f32(st, bi, ctx)        # swap IN
        var bl = _block_lora_for(lora, bi)
        var fwd = anima_block_lora_forward_b2[H, Dh, S_IMG, S_TXT](
            x.copy(), t_cond.copy(), base_adaln.copy(), context.copy(),
            w, bl, cos2, sin2, B, D, JOINT, F, eps, ctx,
        )
        x = fwd.out.copy()
        # `w` drops here -> swap OUT before next block.
...
def anima_stack_lora_backward_streamed_b2[
    H: Int, Dh: Int, S_IMG: Int, S_TXT: Int
](
    d_out: List[Float32],         # [2*S_img, OUT_PATCH]
    patches: List[Float32],       # [2*S_img, IN_PATCH]
    t_cond: List[Float32], base_adaln: List[Float32], context: List[Float32],
    base: AnimaStackBase, st: SafeTensors, lora: AnimaLoraSet,
    cos: Tensor, sin: Tensor,     # SINGLE-sample [S_img*H, Dh/2] table (doubled here)
    saved: AnimaStackForward,
    ...
    if B != 2:
        raise Error("anima_stack_lora_backward_streamed_b2 requires B==2, got " + String(B))
    var num_blocks = lora.num_blocks
    var ln_ones = _t(_ones(D), [D], ctx)
    var cos2 = _rope_tile_b2(cos, ctx)      # [2*S_img*H, Dh/2]
    var sin2 = _rope_tile_b2(sin, ctx)
    ...
    var d_fl_lin = lbf.d_w.to_host(ctx)
    var d_fl_xmod_3d = reshape(lbf.d_x, _sh3(B, S_IMG, D), ctx)
    var mbf = modulate_backward_b2_perhalf(d_fl_xmod_3d, saved.fl_ln[], saved.fl_scale[], ctx)
    var d_x_final = layer_norm_backward_dx(mbf.d_x, saved.x_final[], ln_ones, eps, ctx)
    var d_x = d_x_final.to_host(ctx)
    ...
    while bi >= 0:
        var w = load_anima_block_weights_f32(st, bi, ctx)        # swap IN
        var bl = _block_lora_for(lora, bi)
        var refwd = anima_block_lora_forward_b2[H, Dh, S_IMG, S_TXT](
            saved.blk_x_in[bi][].to_host(ctx),
            t_cond.copy(), base_adaln.copy(), context.copy(),
            w, bl, cos2, sin2, B, D, JOINT, F, eps, ctx,
        )
        var bg = anima_block_lora_backward_b2[H, Dh, S_IMG, S_TXT](
            d_x.copy(), refwd.saved, w, bl, cos2, sin2, B, D, JOINT, F, eps, ctx,
        )
        d_x = bg.base.d_x.copy()
        d_t_silu_acc = _add_lists(d_t_silu_acc, bg.base.d_t_silu)
        var base_idx = bi * ANIMA_SLOTS
...
def anima_stack_lora_forward_device_resident[
    ...
    # backward READS it instead of recomputing the block forward (the reference trainer-parity
...
# ── AdamW step on EVERY adapter. Hot path: ONE fused GPU launch with the
# IDENTICAL plain-AdamW math (training/lora_adamw_plain_fused.mojo; gates
# training/lora_adamw_plain_fused_parity.mojo +
# models/anima/parity/anima_gpu_adamw_update_parity.mojo — host vs device
# worst-abs-diff class, NOT exact-equality: device FMA contraction can flip
# RNE ties by 1 ulp). Host loop kept below as the gate reference (zimage
# ZIMAGE_FUSED_ADAMW pattern, zimage_stack_lora.mojo:2481).
comptime ANIMA_GPU_ADAMW = True
...
# The pre-fused host scalar loop, retained verbatim as the parity-gate
# reference (reuses the proven per-adapter _lora_adamw).
def anima_lora_adamw_step_unfused(
    mut set: AnimaLoraSet, grads: AnimaLoraGrads, t: Int, lr: Float32,
    ctx: DeviceContext,
    beta1: Float32 = Float32(0.9), beta2: Float32 = Float32(0.999),
    eps: Float32 = Float32(1.0e-8), weight_decay: Float32 = Float32(0.01),
...
# The per-step fused arm (anima_lora_adamw_step above) allocates FRESH PINNED
# host staging EVERY step; under pinned pressure MAX can hand back an UNMAPPED
# buffer without raising -> segfault. The resident arm holds persistent device
# P/M/V and stages ONCE at init; per step only grads go up and params come back.
# Identical F32-moment math to anima_lora_adamw_step over set.ad.
def anima_lora_adamw_state_init(
    set: AnimaLoraSet, ctx: DeviceContext
) raises -> LoraAdamWPlainDeviceState:
    return lora_adamw_plain_device_state_init(
        set.ad, 0, set.num_blocks * ANIMA_SLOTS, ctx
    )


def anima_lora_adamw_step_resident(
...
# private to Mojo resume. Product LoRA saves use the raw SerenityTrainer prefixes below.
...
def _anima_serenity_named(set: AnimaLoraSet) -> List[NamedLora]:
    var named = List[NamedLora]()
    for bi in range(set.num_blocks):
        for s in range(ANIMA_SLOTS):
            named.append(NamedLora(
                _anima_serenity_prefix(bi, s),
                set.ad[bi * ANIMA_SLOTS + s].copy(),
            ))
    return named^


# ── SerenityTrainer diffusers-state-dict key naming (delta 5) ─────────────────────
# Mirrors what AnimaLoRASaver dumps: raw transformer_lora.state_dict() of an reference trainer
...
# with "transformer." (AnimaLoRASetup.py:62). So the full reference trainer key is:
#   transformer.transformer_blocks.{i}.{module}.lora_down.weight  etc.
def _anima_serenity_module(slot: Int) -> String:
    if slot == SLOT_SA_Q:
...
def _anima_serenity_prefix(block_idx: Int, slot: Int) -> String:
    return (String("transformer.transformer_blocks.") + String(block_idx)
            + String(".") + _anima_serenity_module(slot))


def anima_serenity_prefixes(num_blocks: Int) -> List[String]:
    var out = List[String]()
    for bi in range(num_blocks):
        for s in range(ANIMA_SLOTS):
            out.append(_anima_serenity_prefix(bi, s))
    return out^


# ── SAVE every adapter in SerenityTrainer diffusers key naming (reference trainer-loadable) ───────
def save_anima_lora(set: AnimaLoraSet, path: String, ctx: DeviceContext) raises -> Int:
    return save_lora_serenity_trainer(_anima_serenity_named(set), path, ctx)


# Compatibility alias for older Anima reference trainer training entrypoints. The shared saver
# derives alpha from each adapter's scale*rank, preserving BF16 raw reference trainer tensors.
def save_anima_lora_serenity(
...
def load_anima_lora_resume(
    ...
    var prefixes = anima_serenity_prefixes(num_blocks)
    var scale = alpha / Float32(rank)
...
# (save_anima_lora) is raw SerenityTrainer; this `.opt`/state file additionally
...
# ═════════════════════════════════════════════════════════════════════════════
# TRUE BATCH-2 DEVICE-RESIDENT LoRA stack (fast hot path). Row-stacked B==2 clone
# of anima_stack_lora_forward/backward_device_resident: all 28 blocks + saved
# activations + LoRA A/B stay DEVICE tensors, NO to_host inside the block loop,
# ONE bulk D2H of the LoRA grads at the end. Differs from b1 only where the batch
# dim demands it: the rope table is doubled (_rope_tile_b2), the block fwd/bwd are
# the _b2 variants (per-sample attention), and the final-layer modulate backward
# is per-half (modulate_backward_b2_perhalf) — exactly mirroring the proven
# streamed_b2 stack. LoRA d_a/d_b are summed over both samples' 2N rows = the
# batch grad. Base blocks passed resident (BF16 production / F32 parity).
# ═════════════════════════════════════════════════════════════════════════════
def anima_stack_lora_forward_device_resident_b2[
    H: Int, Dh: Int, S_IMG: Int, S_TXT: Int
](
    patches: List[Float32],       # [2*S_img, IN_PATCH] row-stacked
    t_cond: List[Float32], base_adaln: List[Float32], context: List[Float32],
    ...
    if B != 2:
        raise Error("anima_stack_lora_forward_device_resident_b2 requires B==2, got " + String(B))
    var num_blocks = len(blocks)
    var cos2 = _rope_tile_b2(cos, ctx)      # [2*S_img*H, Dh/2]
    var sin2 = _rope_tile_b2(sin, ctx)
    ...
    for bi in range(num_blocks):
        ...
        var bl = _block_lora_dev_for(lora, bi)
        var fwd = anima_block_lora_forward_device_tensor_b2[H, Dh, S_IMG, S_TXT](
            x_arc.copy(), t_silu_arc.copy(), base_t_arc.copy(), ctx_t_arc.copy(),
            blocks[bi], bl, cos2, sin2, B, D, JOINT, F, eps, ctx,
        )
        x_arc = fwd.out.copy()
        blk_saved.append(fwd.saved.copy())

    # ── final layer (identical to b1 device; batch-agnostic) ──
    var fl_h = linear(t_silu_arc[], base.fl_mod1[], Optional[Tensor](None), ctx)
    var fl_modout = linear(fl_h, base.fl_mod2[], Optional[Tensor](None), ctx)
    var base_half = slice(base_t_arc[], 1, 0, 2 * D, ctx)
    var fl_added = add(fl_modout, base_half, ctx)
...
def anima_stack_lora_backward_device_resident_b2[
    H: Int, Dh: Int, S_IMG: Int, S_TXT: Int
](
    d_out: List[Float32],         # [2*S_img, OUT_PATCH]
    patches: List[Float32],
    ...
    if B != 2:
        raise Error("anima_stack_lora_backward_device_resident_b2 requires B==2, got " + String(B))
    var num_blocks = len(blocks)
    var ln_ones = _t(_ones(D), [D], ctx)
    var cos2 = _rope_tile_b2(cos, ctx)      # [2*S_img*H, Dh/2]
    var sin2 = _rope_tile_b2(sin, ctx)
    ...
    # ── final-layer backward (frozen base; dx-only; per-half modulate) ──
    var d_fl_xmod_2d = linear_backward_dx(
        _t(d_out, [B * S_IMG, OUT_PATCH], ctx), base.fl_lin[], B * S_IMG, D, OUT_PATCH, ctx,
    )
    var d_fl_xmod_3d = reshape(d_fl_xmod_2d, _sh3(B, S_IMG, D), ctx)
    ...
    # shared conditioning only needed for the recompute FALLBACK (have_saved=False).
    var t_silu_arc = TArc(_t(_zeros(1), [1], ctx))
    var base_t_arc = TArc(_t(_zeros(1), [1], ctx))
    var ctx_t_arc = TArc(_t(_zeros(1), [1], ctx))
    if not have_saved:
    ...
    while bi >= 0:
        ...
        var do_trace = trace and (bi == num_blocks - 1)
        if have_saved:
            var bg = anima_block_lora_backward_device_tensors_b2[H, Dh, S_IMG, S_TXT](
                d_x[], saved.blk_saved[bi], blocks[bi], bl, cos2, sin2,
                B, D, JOINT, F, eps, ctx, do_trace,
            )
            d_x = bg.d_x.copy()
            d_t_silu_acc = add(d_t_silu_acc, bg.d_t_silu[], ctx)
            ...
            var refwd = anima_block_lora_forward_device_tensor_b2[H, Dh, S_IMG, S_TXT](
                saved.blk_x_in[bi].copy(), t_silu_arc.copy(), base_t_arc.copy(), ctx_t_arc.copy(),
                blocks[bi], bl, cos2, sin2, B, D, JOINT, F, eps, ctx,
            )
            var bg = anima_block_lora_backward_device_tensors_b2[H, Dh, S_IMG, S_TXT](
                d_x[], refwd.saved, blocks[bi], bl, cos2, sin2, B, D, JOINT, F, eps, ctx, do_trace,
            )
            d_x = bg.d_x.copy()
            d_t_silu_acc = add(d_t_silu_acc, bg.d_t_silu[], ctx)
            var base_idx = bi * ANIMA_SLOTS
