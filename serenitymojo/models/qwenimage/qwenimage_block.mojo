...
from serenitymojo.ops.norm_backward import (
    rms_norm_backward, RmsNormBackward, rms_norm_backward_dx,
    layer_norm_backward, LayerNormBackward, layer_norm_backward_dx,
)
...
def _stream_post_backward(
    ...
    var d_ln2 = layer_norm_backward_dx(mb2.d_x, sv.attn_res[], ones, eps, ctx)
    # attn_res feeds BOTH the residual (grg2.d_x) AND ln2 -> SUM on device
    var d_attn_res_total = TArc(add(grg2.d_x, d_ln2, ctx))
...
def _stream_pre_backward[
    ...
    var d_ln1 = layer_norm_backward_dx(mb1.d_x, sv.x[], ones, eps, ctx)
    var d_x_norm = d_ln1.to_host(ctx)
...
def _stream_pre_backward_direct[
    ...
    reshape_in_place(d_q_pre, [N, D])
    reshape_in_place(d_k_pre, [N, D])
    var d_v_flat = reshape(d_v, [N, D], ctx)

    var q_g = _direct_proj_bwd_device(
        direct, q_slot, d_q_pre, sv.normed[], w.wq[], N, D, D, ctx,
    )
    var k_g = _direct_proj_bwd_device(
        direct, k_slot, d_k_pre, sv.normed[], w.wk[], N, D, D, ctx,
    )
