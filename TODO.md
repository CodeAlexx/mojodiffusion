...
| 11 | **Mojo krea2 LoRA training port — ai-toolkit parity (2026-06-25)** | Port ai-toolkit krea2 training to pure Mojo (oracle=ai-toolkit, SingleStreamDiT 28 blocks, Qwen-Image VAE + Qwen3-VL-4B). **Phases 1-3 lead-verified cos ~1.0** (block fwd+bwd+LoRA; stack LoRA backward final→×28 frozen-skip; data cache — latent normalize **cos 1.0 vs ai-toolkit**, 3 dtype bugs fixed). **4a: streaming trainer RUNS end-to-end** on real data (finite loss, LoRA grads nonzero+growing) — mixed-precision bf16 backward resolved (5 rms_norm+3 modulate casts, F32 gates still bit-identical). **Verdict CONFIRMED, NO BUG (lead-run): fixed-objective overfit → loss drops MONOTONICALLY 0.106→0.092/12 steps, grad_norm grows = textbook LoRA.** The flat production-loss was the per-step sigma+noise RESAMPLE (noisy objective) + B=0-init; the "30000× < ideogram4 grad" was a FALSE ALARM (that was a B-weight L1, not a grad norm; real oracle grads ernie/anima 0.0008/0.0015, krea2 in-band). **MEMORY FIXED: per-block `ctx.synchronize()` in streamed fwd/bwd → async, no OOM.** OPEN: LT bucketing (across-step multi-sample); **4b autograd_v2 KREA2_V2_GRAPH** (seam in, default off). LoRA = 8 nn.Linear/block (mod.lin a Parameter, NOT wrapped). | `serenitymojo/models/krea2/parity/KREA2_TRAINING_PARITY_LEDGER.md` |
