# sdxl_train_ref_artifact_smoke.mojo -- SDXL SerenityTrainer one-step artifact gate.
#
# Header/payload-only, no-CUDA gate. It opens the real local SerenityTrainer SDXL
...
# update-delta payloads. This is SerenityTrainer update-delta artifact consumption;
...
comptime STEP_DUMP = "/home/alex/serenity_trainer-mojo/parity/sdxl_train_ref_step000.safetensors"
comptime ADAPTER_DUMP = "/home/alex/serenity_trainer-mojo/parity/sdxl_train_ref_step000_adapters.safetensors"
comptime META_JSON = "/home/alex/serenity_trainer-mojo/parity/sdxl_train_ref_meta.json"
...
def _require_contains(text: String, needle: String, label: String) raises:
    _require(text.find(needle) >= 0, String("missing SDXL reference trainer meta field: ") + label)
...
def _require_tensor(
    ...
    _require(key in st.tensors, String("missing SDXL reference trainer tensor ") + key)
    var info = st.tensor_info(key)
    _require(
        info.dtype == dtype,
...
def _check_meta_json() raises:
    ...
    _require(meta.byte_length() > 0, String("empty SDXL SerenityTrainer metadata JSON"))
    ...
    _require_contains(
        meta,
        String("\"safetensors\": \"/home/alex/serenity_trainer-mojo/parity/sdxl_train_ref_step000.safetensors\""),
        String("step safetensors path"),
    )
    _require_contains(
        meta,
        String("\"adapter_safetensors\": \"/home/alex/serenity_trainer-mojo/parity/sdxl_train_ref_step000_adapters.safetensors\""),
...
def _check_adapter_dump() raises:
    ...
    # Representative SerenityTrainer update delta: adapter_post -> adapter_after.
