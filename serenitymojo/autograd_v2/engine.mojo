...
from serenitymojo.ops.linalg_backward import linear_backward_dx as _linear_backward_dx_eng
...
def apply(node: Node, grads_in: List[TArc], ctx: DeviceContext) raises -> List[TArc]:
    ...
    # out of a live struct, and clone would add a d2d copy + sync per op.
        ...
        out.append(TArc(d_x^))
        return out^
    elif node.kind == OPK_ROPE_HALFSPLIT:
        # saved [cos, sin] (frozen HALF-width [rows,D/2] tables); interleaved=
        # False = the Qwen3/Z-Image halfsplit pairing (acestep _self_attn rope).
        var d_x = rope_backward(g[], node.saved[0][], node.saved[1][], False, ctx)
        var out = List[TArc]()
        out.append(TArc(d_x^))
        return out^
    elif node.kind == OPK_LINEAR_DX:
        # saved [W] (frozen); meta [M, in_f, out_f]. dx-only (W never gets d_w) —
        # the acestep MLP gate/up/down that carry no LoRA, routing d_x only.
        var d_x = _linear_backward_dx_eng(
            g[], node.saved[0][],
            node.saved_meta[0], node.saved_meta[1], node.saved_meta[2], ctx,
        )
        var out = List[TArc]()
...
# ─────────────────────────────────────────────────────────────────────────────
# LTX-2.3 VIDEO block, COARSE (Option A) — the OPK_KREA2_SINGLE_BLOCK pattern.
# apply_ltx2v recomputes the block forward from the saved input and calls the
# WHOLE ltx2_video_block_backward oracle. execute_ltx2v_block is the SAME engine
# algorithm as execute_krea2_block (dep-count BFS, InputBuffer route, ready-queue
# order, fired==reachable): the composite fires once → d_hidden routes through the
# single (hidden) leaf; the host-list LoRA grads are captured out-of-band and the
# WHOLE LTX2VideoBlockGrads is returned (NOT a Dict[Int,TArc]). No StepSlab / no
# capture (coarse math-path phase; slab+capture are the L5 follow-on).
# ─────────────────────────────────────────────────────────────────────────────
from serenitymojo.autograd_v2.node import OPK_LTX2V_VIDEO_BLOCK
from serenitymojo.models.dit.ltx2_dit import LTX2AVBlockWeights as _Ltx2vW
from serenitymojo.models.ltx2.ltx2_video_backward import (
    ltx2_video_block_train_forward as _ltx2v_fwd,
    ltx2_video_block_backward as _ltx2v_bwd,
    ltx2_video_block_train_forward_slab as _ltx2v_fwd_slab,
    ltx2_video_block_backward_slab as _ltx2v_bwd_slab,
    ltx2_video_block_backward_slab_dev as _ltx2v_bwd_slab_dev,
    LTX2VideoBlockGrads as _Ltx2vGrads,
    LTX2VideoBlockGradsDev as _Ltx2vGradsDev,
)
from serenitymojo.models.ltx2.ltx2_av_backward import (
    LoraPairGrad, LoraPairGradDev, Ltx2LoraGradStore,
)


def apply_ltx2v[S_V: Int, N_TXT: Int](
    node: Node, d_video: TArc,
    w: _Ltx2vW, enc: Tensor, v_temb: Tensor, v_prompt_ts: Tensor,
    v_cos: Tensor, v_sin: Tensor, eps: Float32, ctx: DeviceContext,
) raises -> _Ltx2vGrads:
    """Recompute the ltx2 video block forward from the saved input (the conductor's
    recompute-checkpoint discipline), then call the WHOLE ltx2_video_block_backward
    oracle. Returns LTX2VideoBlockGrads (d_hidden + n_slots host-list LoRA pairs) —
    bit-identical to the hand-chain per-block (recompute + block backward) pair
    (ltx2_video_stack.mojo:402-419). `w` already carries the attached LoRA (the
    caller's _attach_block_lora, exactly as the hand-chain loop attaches it)."""
    if node.kind != OPK_LTX2V_VIDEO_BLOCK:
        raise Error("apply_ltx2v: unexpected node kind")
    var fwd = _ltx2v_fwd[S_V, N_TXT](
        w, node.saved[0][], enc, v_temb, v_prompt_ts, v_cos, v_sin, eps, ctx,
    )
    var bg = _ltx2v_bwd[S_V, N_TXT](
        w, fwd.acts, d_video[], v_temb, v_cos, v_sin, eps, ctx,
    )
    return bg^


def execute_ltx2v_block[S_V: Int, N_TXT: Int](
    mut graph: Graph, root: Int, root_grad: TArc,
    w: _Ltx2vW, enc: Tensor, v_temb: Tensor, v_prompt_ts: Tensor,
    v_cos: Tensor, v_sin: Tensor, eps: Float32, ctx: DeviceContext,
) raises -> _Ltx2vGrads:
    """execute_krea2_block's engine algorithm for the ltx2 graph: ONE composite
    node + its hidden leaf. The composite fires once (the seeded root), produces
    LTX2VideoBlockGrads; its d_hidden routes along the hidden edge into the LEAF
    (engine-driven); the host-list LoRA grads are captured and returned whole.
    Single root, single tracked edge → no multi-contribution fan-in (C15 trivial)."""
    var n = len(graph.nodes)
    if root < 0 or root >= n:
        raise Error("execute_ltx2v_block: root out of range")
    ...
    # Step 3: drive the queue (engine.rs:437-570).
    var captured = False
    var captured_lora = List[LoraPairGrad]()
    var d_x_result = TArc(Tensor(root_grad[].buf.copy(), root_grad[].shape(), root_grad[].dtype()))
    var have_dx = False
    var fired = 0
    while len(ready) > 0:
        ...
        for s in range(num_in):
            if not buffers[nid].any_present(s):
                raise Error(
                    String("execute_ltx2v_block: node ") + String(nid)
                    + " missing grad slot " + String(s)
                )
            grads_in.append(buffers[nid].materialize(s, ctx))
        ...
        if graph.nodes[nid].kind != OPK_LTX2V_VIDEO_BLOCK:
            raise Error(
                String("execute_ltx2v_block: unexpected node kind ")
                + String(graph.nodes[nid].kind)
            )
        if captured:
            raise Error("execute_ltx2v_block: more than one composite node fired")
        var bg = apply_ltx2v[S_V, N_TXT](
            graph.nodes[nid], grads_in[0],
            w, enc, v_temb, v_prompt_ts, v_cos, v_sin, eps, ctx,
        )
        # Route the composite's single d_hidden edge into the leaf; stash the
        # host-list LoRA grads out-of-band (they cannot flow as TArc).
        var d_hidden_arc = TArc(
            Tensor(bg.d_hidden.buf.copy(), bg.d_hidden.shape(), bg.d_hidden.dtype()))
        captured_lora = bg.lora.copy()
        captured = True
        var n_edges = len(graph.nodes[nid].edges)
        if n_edges != 1:
            raise Error(
                String("execute_ltx2v_block: composite node must have exactly one edge, got ")
                + String(n_edges)
            )
        var child = graph.nodes[nid].edges[0].node_idx
        if child >= 0:
            var slot = graph.nodes[nid].edges[0].input_nr
            var cslot = graph.nodes[nid].edges[0].contrib_slot
            buffers[child].add(slot, cslot, d_hidden_arc.copy(), ctx)
            _dec_and_maybe_enqueue(dep, ready, in_queue, child)

    if fired != reachable:
        raise Error(
            String("execute_ltx2v_block: fired=") + String(fired)
            + " != reachable=" + String(reachable) + " (dep-count exactness violated)"
        )
    if not captured:
        raise Error("execute_ltx2v_block: composite node never fired")
    if not have_dx:
        raise Error("execute_ltx2v_block: hidden leaf never sank d_hidden")
    # The engine-routed d_hidden (from the leaf) is the authoritative block grad.
    return _Ltx2vGrads(
        Tensor(d_x_result[].buf.copy(), d_x_result[].shape(), d_x_result[].dtype()),
        captured_lora^,
    )


# ─────────────────────────────────────────────────────────────────────────────
# LTX-2.3 VIDEO block — StepSlab variants (autograd_v2 contract C8, L5). Same
# engine algorithm and same coarse composite; the apply arm calls the block
# StepSlab twins and brackets ONE block's fwd+bwd in a slab mark/rewind window.
# COPY-OUT-BEFORE-REWIND: d_hidden is cloned to a non-slab buffer and the LoRA
# grads are already host F32 (_lora_pair_bwd_slab .to_host, C12) BEFORE rewind,
# so the next block's allocations never clobber this block's returned grads.
# Bit-identical to apply_ltx2v/execute_ltx2v_block (the slab twins are byte-
# identical to their non-slab originals); gated by
# autograd_v2/tests/ltx2_block_parity.mojo's slab arm.
# ─────────────────────────────────────────────────────────────────────────────
def apply_ltx2v_slab[S_V: Int, N_TXT: Int](
    node: Node, d_video: TArc,
    w: _Ltx2vW, enc: Tensor, v_temb: Tensor, v_prompt_ts: Tensor,
    v_cos: Tensor, v_sin: Tensor, eps: Float32, ctx: DeviceContext,
    mut slab: StepSlab,
) raises -> _Ltx2vGrads:
    """StepSlab variant of `apply_ltx2v` (this file :1717). mark→recompute
    fwd_slab→bwd_slab→copy d_hidden OUT (non-slab clone) + LoRA grads already
    host→rewind. Returns LTX2VideoBlockGrads with a non-slab d_hidden so it
    survives the next block's slab reuse."""
    if node.kind != OPK_LTX2V_VIDEO_BLOCK:
        raise Error("apply_ltx2v_slab: unexpected node kind")
    var mark = slab.mark()
    var fwd = _ltx2v_fwd_slab[S_V, N_TXT](
        w, node.saved[0][], enc, v_temb, v_prompt_ts, v_cos, v_sin, eps, ctx, slab,
    )
    var bg = _ltx2v_bwd_slab[S_V, N_TXT](
        w, fwd.acts, d_video[], v_temb, v_cos, v_sin, eps, ctx, slab,
    )
    # Copy-out BEFORE rewind: d_hidden -> fresh non-slab buffer; LoRA grads are
    # already host F32 (deep-copy the list).
    var d_hidden_out = bg.d_hidden.clone(ctx)
    var lora_out = bg.lora.copy()
    slab.rewind(mark)
    return _Ltx2vGrads(d_hidden_out^, lora_out^)


def execute_ltx2v_block_slab[S_V: Int, N_TXT: Int](
    mut graph: Graph, root: Int, root_grad: TArc,
    w: _Ltx2vW, enc: Tensor, v_temb: Tensor, v_prompt_ts: Tensor,
    v_cos: Tensor, v_sin: Tensor, eps: Float32, ctx: DeviceContext,
    mut slab: StepSlab,
) raises -> _Ltx2vGrads:
    """StepSlab variant of `execute_ltx2v_block` (this file :1739) — identical
    engine algorithm (dep-count BFS, InputBuffer route, fired==reachable); the
    single composite fires via apply_ltx2v_slab."""
    var n = len(graph.nodes)
    if root < 0 or root >= n:
        raise Error("execute_ltx2v_block_slab: root out of range")
    ...
    while len(ready) > 0:
        ...
        for s in range(num_in):
            if not buffers[nid].any_present(s):
                raise Error(
                    String("execute_ltx2v_block_slab: node ") + String(nid)
                    + " missing grad slot " + String(s)
                )
            grads_in.append(buffers[nid].materialize(s, ctx))
        ...
        if graph.nodes[nid].kind != OPK_LTX2V_VIDEO_BLOCK:
            raise Error(
                String("execute_ltx2v_block_slab: unexpected node kind ")
                + String(graph.nodes[nid].kind)
            )
        if captured:
            raise Error("execute_ltx2v_block_slab: more than one composite node fired")
        var bg = apply_ltx2v_slab[S_V, N_TXT](
            graph.nodes[nid], grads_in[0],
        ...
        if n_edges != 1:
            raise Error(
                String("execute_ltx2v_block_slab: composite node must have exactly one edge, got ")
                + String(n_edges)
            )
        var child = graph.nodes[nid].edges[0].node_idx
        if child >= 0:
    ...
    if fired != reachable:
        raise Error(
            String("execute_ltx2v_block_slab: fired=") + String(fired)
            + " != reachable=" + String(reachable) + " (dep-count exactness violated)"
        )
    if not captured:
        raise Error("execute_ltx2v_block_slab: composite node never fired")
    if not have_dx:
        raise Error("execute_ltx2v_block_slab: hidden leaf never sank d_hidden")
    return _Ltx2vGrads(
        Tensor(d_x_result[].buf.copy(), d_x_result[].shape(), d_x_result[].dtype()),
        captured_lora^,
    )
...
# ─────────────────────────────────────────────────────────────────────────────
# LTX-2.3 VIDEO block — StepSlab + DEVICE-GRAD variants (rung 1/2). Same engine
# algorithm and same coarse composite as apply_ltx2v_slab/execute_ltx2v_block_slab,
# but the block backward is the DEVICE-grad twin (ltx2_video_block_backward_slab_dev):
# LoRA d_A/d_B stay on-device (off-slab .clone, survive the rewind), so the stack
# loop can do ONE boundary ltx2_lora_dev_readback outside the per-block region.
# d_hidden is still cloned OUT (non-slab) before rewind. Bit-identical grads to
# the host-slab path (the slab twins are byte-identical; the readback preserves
# bytes) — gated by ltx2_block_parity's dev engine arm.
# ─────────────────────────────────────────────────────────────────────────────
def apply_ltx2v_slab_dev[S_V: Int, N_TXT: Int](
    node: Node, d_video: TArc,
    w: _Ltx2vW, enc: Tensor, v_temb: Tensor, v_prompt_ts: Tensor,
    v_cos: Tensor, v_sin: Tensor, eps: Float32, ctx: DeviceContext,
    mut slab: StepSlab,
...
) raises -> _Ltx2vGradsDev:
    """Device-grad variant of `apply_ltx2v_slab` (this file). mark→recompute
    fwd_slab→bwd_slab_DEV→copy d_hidden OUT + shallow-copy the device LoRA grad
    list (ArcPointer, buffers off-slab)→rewind. When (store, block_idx>=0) is
    passed, the block backward writes d_A/d_B into the resident store's block
    slots (rung 3, no alloc); else the .clone path (rung 1/2).

    d_hidden_dst (rung 3 ping-pong, LTX2_V2_SLAB): the mandatory copy-d_hidden-
    out-of-the-slab-before-rewind lands in a caller-provided FIXED buffer instead
    of a fresh `.clone` alloc — capture-safe (a memcpy node, no per-step alloc;
    slab.rewind is host-side). d_hidden_dst=None keeps the byte-identical `.clone`
    path (every existing gate stays green, C13). The bytes copied are identical
    either way (same bg.d_hidden), so the returned grad is bit-equal."""
    if node.kind != OPK_LTX2V_VIDEO_BLOCK:
        raise Error("apply_ltx2v_slab_dev: unexpected node kind")
    var mark = slab.mark()
    var fwd = _ltx2v_fwd_slab[S_V, N_TXT](
        w, node.saved[0][], enc, v_temb, v_prompt_ts, v_cos, v_sin, eps, ctx, slab,
    )
    var bg = _ltx2v_bwd_slab_dev[S_V, N_TXT](
        w, fwd.acts, d_video[], v_temb, v_cos, v_sin, eps, ctx, slab, store, block_idx,
    )
    var d_hidden_out: Tensor
    if d_hidden_dst:
        # Copy the slab-resident d_hidden into the caller's FIXED buffer, return a
        # VIEW into it (no fresh alloc). The caller (stack ping-pong) sizes the
        # buffer from d_x.nbytes() == d_hidden.nbytes() (d_hidden shape == hidden
        # shape). Byte-identical to the .clone branch (same bg.d_hidden bytes).
        ref dst = d_hidden_dst.value()
        var nb = bg.d_hidden.nbytes()
        var view = dst.create_sub_buffer[DType.uint8](0, nb)
        ctx.enqueue_copy(dst_buf=view, src_buf=bg.d_hidden.buf)
        d_hidden_out = Tensor(view^, bg.d_hidden.shape(), bg.d_hidden.dtype())
    else:
        d_hidden_out = bg.d_hidden.clone(ctx)
    var lora_out = bg.lora.copy()   # LoraPairGradDev is ArcPointer-backed; buffers are off-slab
    slab.rewind(mark)
    return _Ltx2vGradsDev(d_hidden_out^, lora_out^)


def execute_ltx2v_block_slab_dev[S_V: Int, N_TXT: Int](
    mut graph: Graph, root: Int, root_grad: TArc,
    w: _Ltx2vW, enc: Tensor, v_temb: Tensor, v_prompt_ts: Tensor,
    v_cos: Tensor, v_sin: Tensor, eps: Float32, ctx: DeviceContext,
    mut slab: StepSlab,
    ...
    """Device-grad variant of `execute_ltx2v_block_slab` (this file) — identical
    engine algorithm; the single composite fires via apply_ltx2v_slab_dev and the
    device LoRA grad list is captured out-of-band. (store, block_idx) route the
    grads to the resident store when active. d_hidden_dst (rung 3 ping-pong)
    passes the fixed d_hidden destination through to apply_ltx2v_slab_dev; None
    keeps the byte-identical .clone path."""
    var n = len(graph.nodes)
    if root < 0 or root >= n:
        raise Error("execute_ltx2v_block_slab_dev: root out of range")
    ...
    var captured = False
    var captured_lora = List[LoraPairGradDev]()
    var d_x_result = TArc(Tensor(root_grad[].buf.copy(), root_grad[].shape(), root_grad[].dtype()))
    var have_dx = False
    var fired = 0
    while len(ready) > 0:
        ...
        for s in range(num_in):
            if not buffers[nid].any_present(s):
                raise Error(
                    String("execute_ltx2v_block_slab_dev: node ") + String(nid)
                    + " missing grad slot " + String(s)
                )
            grads_in.append(buffers[nid].materialize(s, ctx))
        ...
        if graph.nodes[nid].kind != OPK_LTX2V_VIDEO_BLOCK:
            raise Error(
                String("execute_ltx2v_block_slab_dev: unexpected node kind ")
                + String(graph.nodes[nid].kind)
            )
        if captured:
            raise Error("execute_ltx2v_block_slab_dev: more than one composite node fired")
        var bg = apply_ltx2v_slab_dev[S_V, N_TXT](
            graph.nodes[nid], grads_in[0],
        ...
        if n_edges != 1:
            raise Error(
                String("execute_ltx2v_block_slab_dev: composite node must have exactly one edge, got ")
                + String(n_edges)
            )
        var child = graph.nodes[nid].edges[0].node_idx
        if child >= 0:
    ...
    if fired != reachable:
        raise Error(
            String("execute_ltx2v_block_slab_dev: fired=") + String(fired)
            + " != reachable=" + String(reachable) + " (dep-count exactness violated)"
        )
    if not captured:
        raise Error("execute_ltx2v_block_slab_dev: composite node never fired")
    if not have_dx:
        raise Error("execute_ltx2v_block_slab_dev: hidden leaf never sank d_hidden")
    return _Ltx2vGradsDev(
        Tensor(d_x_result[].buf.copy(), d_x_result[].shape(), d_x_result[].dtype()),
        captured_lora^,
    )
