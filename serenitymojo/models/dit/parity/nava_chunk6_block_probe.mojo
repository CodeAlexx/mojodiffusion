# NAVA chunk 6: joint AV single-stream block (block 0) parity probe.
# Loads fixtures from nava_fx_chunk6_block0.safetensors (all F32 → cast to BF16),
# loads and dequantises weights from NAVA_fp8.safetensors (prefix backbone.single_blocks.0.),
# runs nava_single_block, then prints ParityHarness comparison vs single0_out.
# Gate: cos >= 0.999.
...
from serenitymojo.models.dit.nava_block import (
    load_nava_single_block, nava_single_block, build_nava_rope_tables,
)

comptime NAVA = "/home/alex/.serenity/models/checkpoints/NAVA/NAVA_fp8.safetensors"
comptime FX   = "/home/alex/mojodiffusion/serenitymojo/models/dit/parity/nava_fx_chunk6_block0.safetensors"


def main() raises:
    var ctx = DeviceContext()
    print("=== NAVA chunk6 single-block-0 parity ===")
    ...
    var ref_host = Tensor.from_view(fx.tensor_view("single0_out"), ctx).to_host(ctx)

    # ── Load weights ──────────────────────────────────────────────────────────
    var st = ShardedSafeTensors.open(NAVA)
    var wb = load_nava_single_block(st, "backbone.single_blocks.0.", ctx)

    # ── Build rope tables (one-time, same math as the old inline build) ───────
    var rope = build_nava_rope_tables(ctx)

    # ── Forward ───────────────────────────────────────────────────────────────
    var out_bf16 = nava_single_block(x_bf16, e_vid_bf16, e_audio_bf16, ctx_bf16, wb, rope, ctx)
    ...
    print("NAVA single-block-0 vs torch:", result)
    if result.passed:
