#!/usr/bin/env python3
"""Static guard for SerenityTrainer text-conditioning contract metadata.

This checks the Mojo contract file against source markers in SerenityTrainer and
SerenityTrainer-anima-ref. It is not an end-to-end tokenizer/encoder parity test.
"""
...
reference trainer = Path("/home/alex/SerenityTrainer")
ANIMA_REF = Path("/home/alex/SerenityTrainer-anima-ref")
CONTRACT = ROOT / "serenitymojo/models/text_encoder/serenity_trainer_conditioning_contract.mojo"
SMOKE = ROOT / "serenitymojo/models/text_encoder/serenity_trainer_conditioning_contract_smoke.mojo"
...
def main() -> int:
    ...
    sources = {
        "sdxl": read(reference trainer / "modules/model/StableDiffusionXLModel.py"),
        "sd3": read(reference trainer / "modules/model/StableDiffusion3Model.py"),
        "qwen": read(reference trainer / "modules/model/QwenModel.py"),
        "ernie": read(reference trainer / "modules/model/ErnieModel.py"),
        "flux1": read(reference trainer / "modules/model/FluxModel.py"),
        "flux2": read(reference trainer / "modules/model/Flux2Model.py"),
        "chroma": read(reference trainer / "modules/model/ChromaModel.py"),
        "zimage": read(reference trainer / "modules/model/ZImageModel.py"),
        "anima": read(ANIMA_REF / "modules/model/AnimaModel.py"),
        "sdxl_setup": read(reference trainer / "modules/modelSetup/BaseStableDiffusionXLSetup.py"),
        "sd3_setup": read(reference trainer / "modules/modelSetup/BaseStableDiffusion3Setup.py"),
        "flux1_setup": read(reference trainer / "modules/modelSetup/BaseFluxSetup.py"),
        "flux2_setup": read(reference trainer / "modules/modelSetup/BaseFlux2Setup.py"),
        "chroma_setup": read(reference trainer / "modules/modelSetup/BaseChromaSetup.py"),
        "qwen_setup": read(reference trainer / "modules/modelSetup/BaseQwenSetup.py"),
        "ernie_setup": read(reference trainer / "modules/modelSetup/BaseErnieSetup.py"),
        "zimage_setup": read(reference trainer / "modules/modelSetup/BaseZImageSetup.py"),
        "anima_setup": read(ANIMA_REF / "modules/modelSetup/BaseAnimaSetup.py"),
    }

    # Reference-only markers from SerenityTrainer.
    ...
    for label, marker in [
        ("SDXL contract", "SERENITY_TEXT_SDXL"),
        ("SD3.5 contract", "SERENITY_OUTPUT_SD3_PAD_AND_APPEND_T5"),
        ("Qwen crop contract", "SERENITY_PROMPT_QWEN_IMAGE_TEMPLATE_CROP34"),
        ("Qwen token input", "token1 = 546"),
        ("Ernie lengths contract", "SERENITY_MASK_LENGTHS"),
        ("Anima dense contract", "SERENITY_PROMPT_ANIMA_QWEN_AND_T5"),
        ("Flux1 pooled contract", "SERENITY_OUTPUT_FLUX_CLIP_POOLED_T5"),
        ("Flux2 dev contract", "SERENITY_PROMPT_MISTRAL_SYSTEM_CHAT"),
        ("Klein contract", "SERENITY_PROMPT_QWEN3_CHAT_NO_THINKING"),
        ("Chroma contract", "SERENITY_TEXT_CHROMA"),
        ("ZImage variable contract", "SERENITY_OUTPUT_VARIABLE_LIST"),
        ("cache readiness struct", "SerenityTrainerTextCacheReadinessContract"),
        ("train cache readiness", "SERENITY_TEXT_CACHE_USE_TRAIN"),
        ("sample cache readiness", "SERENITY_TEXT_CACHE_USE_SAMPLE"),
        ("cache field validator", "validate_serenity_text_conditioning_cache_fields"),
    ...
    require_regex("Flux2 layer count", contract, r"SERENITY_OUTPUT_FLUX2_LAYER_CAT.*?layers = 3")
    require_regex("ZImage variable seq", contract, r"family == SERENITY_TEXT_ZIMAGE:.*?out_seq = 0")
    require_regex("Qwen cached sample mask", contract, r"family == SERENITY_TEXT_QWEN:\s+return True")
    ...
    print("SerenityTrainer conditioning contract static guard PASS")
    print("  checked source markers: SerenityTrainer + SerenityTrainer-anima-ref")
...
if __name__ == "__main__":
    try:
        ...
        print(f"SerenityTrainer conditioning contract static guard FAIL: {exc}")
        raise SystemExit(1)
