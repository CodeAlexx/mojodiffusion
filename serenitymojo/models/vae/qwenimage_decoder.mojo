...
struct QwenImageVaeDecoder[LH: Int, LW: Int]:
    ...
    # cuDNN conv fast path (default ON): decoder weights are stored RAW OIDHW
    # (== cuDNN FCQRS) and dispatched directly to conv3d_fcqrs_cudnn — this
    # also removes the PER-CALL host-transpose (_conv3d_w) the naive path pays.
    # Naive fallback selectable with env QWENVAE_NAIVE_CONV=1 or a load arg.
    var use_cudnn: Bool
    ...
    def load(
        ...
        """Load the VAE decoder weights from a diffusers-format dir. Weights
        are kept RAW (OIDHW/OIHW); the cuDNN path consumes them directly as
        FCQRS, the naive fallback permutes to QRSCF per call (_conv3d_w)."""
        var use_cudnn: Bool
        if use_cudnn_opt:
        ...
        return QwenImageVaeDecoder[Self.LH, Self.LW](
            weights^, name_to_idx^, mean^, inv_std^, use_cudnn
        )
    ...
    # ── CausalConv3d, cuDNN fast path ─────────────────────────────────────────
    # weight is the RAW checkpoint OIDHW [Cout,Cin,kD,kH,kW] == cuDNN FCQRS: no
    # host transpose. Same manual zero left-pad on the temporal axis.
    def _causal_conv3d_fcqrs(
        self,
        x: Tensor,
        weight: Tensor,
        var bias: Tensor,
        ...
        if time_pad > 0:
            ...
            return conv3d_fcqrs_cudnn(
                x_in, weight, Optional[Tensor](bias^),
                1, 1, 1, 0, pad_h, pad_w, ctx,
            )
        return conv3d_fcqrs_cudnn(
            x, weight, Optional[Tensor](bias^),
            1, 1, 1, 0, pad_h, pad_w, ctx,
        )
    ...
    def _conv3d_named(
        ...
        if self.use_cudnn:
            return self._causal_conv3d_fcqrs(
                x, self._w(prefix + ".weight"), b^, pad, pad, pad, ctx
            )
    ...
    def _residual_block(
        ...
        if in_dim != out_dim:
            ...
            if self.use_cudnn:
                h = self._causal_conv3d_fcqrs(
                    x, self._w(prefix + ".conv_shortcut.weight"), bsc^,
                    0, 0, 0, ctx,
                )
            else:
    ...
    def _resample(
        ...
        if self.use_cudnn:
            # raw OIHW [Cout,Cin,kH,kW] -> FCQRS [Cout,Cin,1,kH,kW]:
            # metadata-only depth-1 lift of a device clone (no host transpose).
            ref wr = self._w(prefix + ".resample.1.weight")
            var ws = wr.shape()
            var w5 = reshape_owned(
                self._clone(wr, ctx), _shape5(ws[0], ws[1], 1, ws[2], ws[3])
            )
            return self._causal_conv3d_fcqrs(x_up5d, w5, b^, 0, 1, 1, ctx)
    ...
    def _residual_block_wan21(
        ...
        if in_dim != out_dim:
            ...
            if self.use_cudnn:
                h = self._causal_conv3d_fcqrs(
                    x, self._w(prefix + ".shortcut.weight"), bsc^, 0, 0, 0, ctx
                )
            else:
...
def _clamp_unit(x: Tensor, ctx: DeviceContext) raises -> Tensor:
    ...
    if dt == DType.float32:
        ...
        ctx.enqueue_function[_clamp_kernel_f32](
        ...
        ctx.enqueue_function[_clamp_kernel_bf16](
            X, O, Int64(n), grid_dim=grid, block_dim=_BLOCK
        )
    else:
        ...
        ctx.enqueue_function[_clamp_kernel_f16](
            X, O, Int64(n), grid_dim=grid, block_dim=_BLOCK
        )
    ctx.synchronize()
