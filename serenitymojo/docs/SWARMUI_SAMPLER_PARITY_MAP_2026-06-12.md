...
`image1*(1-mask)+image2*mask`. This slice requires the base `init_image` to
already match the decoded output size; Comfy `ImageScale(area)` resize parity is
still fail-loud. Full `LanPaint_KSampler` sampler-loop runtime parity is still
rejected by `reject_unsupported_lanpaint_sampler_params(...)` on Z-Image, and
unsupported LanPaint metadata remains rejected by
