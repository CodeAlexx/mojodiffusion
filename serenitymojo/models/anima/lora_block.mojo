...
# ══════════════════════════════════════════════════════════════════════════════
# TRUE BATCH-2 LoRA-aware block forward/backward (row-stacked via the BATCH dim).
# BYTE-IDENTICAL to anima_block_lora_forward / _backward except the two attention
# calls run PER SAMPLE. Anima threads a runtime B through every op; the token-
# parallel GEMMs/norms/modulation batch over 2*S_IMG rows automatically (this IS
# the row-stacked GEMM whose M-shape change triggers MJ-1073). The self/cross SDPA
# kernels hardcode template B=1 (they validate qshape[0]==1) — a B=2 call would
# LEAK cross-sample attention. So we slice each [2,S,H,Dh] q/k/v along the batch
# dim into two [1,S,H,Dh], call the SAME [1,...] SDPA per sample, and concat back
# → each per-sample attention shape is IDENTICAL to b1 (forward-velocity binding
# per MJ-1073). The 10 LoRA dA/dB accumulate over BOTH samples' rows = the batch
# gradient. Caller MUST pass B==2.
# ══════════════════════════════════════════════════════════════════════════════
def anima_block_lora_forward_b2[
    H: Int, Dh: Int, S_IMG: Int, S_TXT: Int
](
    x: List[Float32],          # [2,S_img,D] F32 row-stacked residual-stream input
    t_cond: List[Float32],     # [2,2048] F32   per-sample timestep cond
    base_adaln: List[Float32], # [2,6144] F32   per-sample adaLN base
    context: List[Float32],    # [2,S_txt,1024] F32 per-sample frozen text context
    w: AnimaBlockWeights, lora: AnimaBlockLora,
    cos: Tensor, sin: Tensor,  # [2*S_img*H, Dh/2] F32 (doubled table)
    B: Int, D: Int, JOINT: Int, F: Int, eps: Float32,
    ctx: DeviceContext,
) raises -> AnimaBlockForward:
    if B != 2:
        raise Error("anima_block_lora_forward_b2 requires B==2, got " + String(B))
    var ln_ones = _t(_ones(D), [D], ctx)
    var ln_zeros = _t(_zeros(D), [D], ctx)
    var sa_scale = Float32(1.0) / fsqrt(Float32(Dh))
    ...
    var sa_qrope = rope_halfsplit(sa_qrms, cos, sin, ctx)
    var sa_krope = rope_halfsplit(sa_krms, cos, sin, ctx)
    # PER-SAMPLE self-attention (template B=1; slice batch dim, SDPA per sample, concat).
    var sa_att0 = sdpa_nomask[1, S_IMG, H, Dh](
        slice(sa_qrope, 0, 0, 1, ctx), slice(sa_krope, 0, 0, 1, ctx),
        slice(sa_v4, 0, 0, 1, ctx), sa_scale, ctx,
    )
    var sa_att1 = sdpa_nomask[1, S_IMG, H, Dh](
        slice(sa_qrope, 0, 1, 1, ctx), slice(sa_krope, 0, 1, 1, ctx),
        slice(sa_v4, 0, 1, 1, ctx), sa_scale, ctx,
    )
    var sa_att = concat(0, ctx, sa_att0, sa_att1)         # [2,S_img,H,Dh]
    var saf = List[Int](); saf.append(B); saf.append(S_IMG); saf.append(D)
    var sa_attflat = reshape(sa_att, saf.copy(), ctx)
    var sa_attflat_h = sa_attflat.to_host(ctx)            # [B*S_img,D] LoRA input (sa out)
    ...
    var ca_qrms = _rms_per_head(ca_q4, w.ca_qn[], ctx)
    var ca_krms = _rms_per_head(ca_k4, w.ca_kn[], ctx)
    # PER-SAMPLE cross-attention (template B=1; slice batch dim, SDPA per sample, concat).
    var ca_att0 = sdxl_sdpa[1, S_IMG, S_TXT, H, Dh](
        slice(ca_qrms, 0, 0, 1, ctx), slice(ca_krms, 0, 0, 1, ctx),
        slice(ca_v4, 0, 0, 1, ctx), sa_scale, ctx,
    )
    var ca_att1 = sdxl_sdpa[1, S_IMG, S_TXT, H, Dh](
        slice(ca_qrms, 0, 1, 1, ctx), slice(ca_krms, 0, 1, 1, ctx),
        slice(ca_v4, 0, 1, 1, ctx), sa_scale, ctx,
    )
    var ca_att = concat(0, ctx, ca_att0, ca_att1)         # [2,S_img,H,Dh]
    var caf = List[Int](); caf.append(B); caf.append(S_IMG); caf.append(D)
    var ca_attflat = reshape(ca_att, caf.copy(), ctx)
    var ca_attflat_h = ca_attflat.to_host(ctx)            # [B*S_img,D] LoRA input (ca out)
...
def modulate_backward_b2_perhalf(
    go: Tensor,      # [2, S, D] row-batched upstream grad
    x: Tensor,       # [2, S, D] saved pre-modulate (post-LN)
    scale2: Tensor,  # [2, D] per-sample adaLN scale
    ctx: DeviceContext,
) raises -> ModulateBackward:
    """Per-sample modulate backward for the b2 batch dim. The shared op's [B,D]
    path has NO param grads, but anima CHAINS d_scale/d_shift into each block's
    mod-MLP backward — so run the exact [1,D] kernels per half and stack:
    d_x [2,S,D]; d_scale/d_shift [2,D] (per-sample rows)."""
    var m0 = modulate_backward(
        slice(go, 0, 0, 1, ctx), slice(x, 0, 0, 1, ctx),
        slice(scale2, 0, 0, 1, ctx), ctx,
    )
    var m1 = modulate_backward(
        slice(go, 0, 1, 1, ctx), slice(x, 0, 1, 1, ctx),
        slice(scale2, 0, 1, 1, ctx), ctx,
    )
    return ModulateBackward(
        concat(0, ctx, m0.d_x, m1.d_x),
        concat(0, ctx, m0.d_scale, m1.d_scale),
        concat(0, ctx, m0.d_shift, m1.d_shift),
    )


def anima_block_lora_backward_b2[
    H: Int, Dh: Int, S_IMG: Int, S_TXT: Int
](
    d_out: List[Float32],      # [2,S_img,D] row-stacked upstream grad
    saved: AnimaBlockSaved,
    w: AnimaBlockWeights, lora: AnimaBlockLora,
    cos: Tensor, sin: Tensor,  # [2*S_img*H, Dh/2] F32 (doubled table)
    B: Int, D: Int, JOINT: Int, F: Int, eps: Float32,
    ...
    if B != 2:
        raise Error("anima_block_lora_backward_b2 requires B==2, got " + String(B))
    var ln_ones = _t(_ones(D), [D], ctx)
    var sa_scale = Float32(1.0) / fsqrt(Float32(Dh))
    ...
    var d_mlp_xmod = _t(d_mlp_xmod_h, [B, S_IMG, D], ctx)
    var mlp_mb = modulate_backward_b2_perhalf(d_mlp_xmod, saved.mlp.ln[], saved.mlp.scale[], ctx)
    var mlp_dxin = layer_norm_backward_dx(mlp_mb.d_x, saved.mlp.x_in[], ln_ones, eps, ctx)
    d_x_after_ca = add(d_x_after_ca, mlp_dxin, ctx)
    var mlp_dadd = concat(1, ctx, mlp_mb.d_shift, mlp_mb.d_scale, mlp_gg.d_gate)
    var mlp_mlb2 = linear_backward(mlp_dadd, saved.mlp.mod_h[], w.mlp_mod2[], B, 256, 3 * D, ctx)
    ...
    var ca_af4 = List[Int](); ca_af4.append(B); ca_af4.append(S_IMG); ca_af4.append(H); ca_af4.append(Dh)
    var d_ca_att4 = reshape(_t(d_ca_attflat_h, [B, S_IMG, D], ctx), ca_af4.copy(), ctx)
    # PER-SAMPLE rectangular cross-attn backward (template B=1; slice, bwd per sample, concat).
    var ca_sb0 = sdpa_backward_rect[1, S_IMG, S_TXT, H, Dh](
        slice(saved.ca_attn.q_sdpa[], 0, 0, 1, ctx), slice(saved.ca_attn.k_sdpa[], 0, 0, 1, ctx),
        slice(saved.ca_attn.v4[], 0, 0, 1, ctx), slice(d_ca_att4, 0, 0, 1, ctx), sa_scale, ctx,
    )
    var ca_sb1 = sdpa_backward_rect[1, S_IMG, S_TXT, H, Dh](
        slice(saved.ca_attn.q_sdpa[], 0, 1, 1, ctx), slice(saved.ca_attn.k_sdpa[], 0, 1, 1, ctx),
        slice(saved.ca_attn.v4[], 0, 1, 1, ctx), slice(d_ca_att4, 0, 1, 1, ctx), sa_scale, ctx,
    )
    var ca_d_q = concat(0, ctx, ca_sb0.d_q, ca_sb1.d_q)   # [2,S_img,H,Dh]
    var ca_d_k = concat(0, ctx, ca_sb0.d_k, ca_sb1.d_k)   # [2,S_txt,H,Dh]
    var ca_d_v = concat(0, ctx, ca_sb0.d_v, ca_sb1.d_v)   # [2,S_txt,H,Dh]
    var ca_q_flat = List[Int](); ca_q_flat.append(B * S_IMG * H); ca_q_flat.append(Dh)
    var ca_k_flat = List[Int](); ca_k_flat.append(B * S_TXT * H); ca_k_flat.append(Dh)
    var d_ca_q_rms_f = reshape(ca_d_q, ca_q_flat.copy(), ctx)
    var d_ca_k_rms_f = reshape(ca_d_k, ca_k_flat.copy(), ctx)
    var ca_qpre_f = reshape(saved.ca_attn.q_pre[], ca_q_flat.copy(), ctx)
    var ca_kpre_f = reshape(saved.ca_attn.k_pre[], ca_k_flat.copy(), ctx)
    var ca_rbq = rms_norm_backward(d_ca_q_rms_f, ca_qpre_f, w.ca_qn[], Float32(1e-6), ctx)
    var ca_rbk = rms_norm_backward(d_ca_k_rms_f, ca_kpre_f, w.ca_kn[], Float32(1e-6), ctx)
    ...
    var d_ca_q_proj = reshape(ca_rbq.d_x, ca_q3.copy(), ctx)
    var d_ca_k_proj = reshape(ca_rbk.d_x, ca_kv3.copy(), ctx)
    var d_ca_v_proj = reshape(ca_d_v, ca_kv3.copy(), ctx)
    var ca_lbq = linear_backward(d_ca_q_proj, saved.ca_attn.q_ctx_in[], w.ca_q[], B * S_IMG, D, D, ctx)
    var d_ca_q = ca_lbq.d_w.to_host(ctx)
    var ca_lbk = linear_backward(d_ca_k_proj, saved.ca_attn.kv_ctx_in[], w.ca_k[], B * S_TXT, JOINT, D, ctx)
    var ca_lbv = linear_backward(d_ca_v_proj, saved.ca_attn.kv_ctx_in[], w.ca_v[], B * S_TXT, JOINT, D, ctx)
    ...
    var d_ca_xmod = _t(d_ca_xmod_h, [B, S_IMG, D], ctx)
    var ca_mb = modulate_backward_b2_perhalf(d_ca_xmod, saved.ca.ln[], saved.ca.scale[], ctx)
    var ca_dxin = layer_norm_backward_dx(ca_mb.d_x, saved.ca.x_in[], ln_ones, eps, ctx)
    d_x_after_sa = add(d_x_after_sa, ca_dxin, ctx)
    var ca_dadd = concat(1, ctx, ca_mb.d_shift, ca_mb.d_scale, ca_gg.d_gate)
    var ca_mlb2 = linear_backward(ca_dadd, saved.ca.mod_h[], w.ca_mod2[], B, 256, 3 * D, ctx)
    ...
    var sa_af4 = List[Int](); sa_af4.append(B); sa_af4.append(S_IMG); sa_af4.append(H); sa_af4.append(Dh)
    var d_sa_att4 = reshape(_t(d_sa_attflat_h, [B, S_IMG, D], ctx), sa_af4.copy(), ctx)
    # PER-SAMPLE self-attn backward (template B=1; slice, bwd per sample, concat).
    var sa_sb0 = sdpa_backward[1, S_IMG, H, Dh](
        slice(saved.sa_attn.q_sdpa[], 0, 0, 1, ctx), slice(saved.sa_attn.k_sdpa[], 0, 0, 1, ctx),
        slice(saved.sa_attn.v4[], 0, 0, 1, ctx), slice(d_sa_att4, 0, 0, 1, ctx), sa_scale, ctx,
    )
    var sa_sb1 = sdpa_backward[1, S_IMG, H, Dh](
        slice(saved.sa_attn.q_sdpa[], 0, 1, 1, ctx), slice(saved.sa_attn.k_sdpa[], 0, 1, 1, ctx),
        slice(saved.sa_attn.v4[], 0, 1, 1, ctx), slice(d_sa_att4, 0, 1, 1, ctx), sa_scale, ctx,
    )
    var sa_d_q = concat(0, ctx, sa_sb0.d_q, sa_sb1.d_q)   # [2,S_img,H,Dh]
    var sa_d_k = concat(0, ctx, sa_sb0.d_k, sa_sb1.d_k)
    var sa_d_v = concat(0, ctx, sa_sb0.d_v, sa_sb1.d_v)
    var sa_q_flat = List[Int](); sa_q_flat.append(B * S_IMG * H); sa_q_flat.append(Dh)
    var d_sa_q_rope_f = reshape(sa_d_q, sa_q_flat.copy(), ctx)
    var d_sa_k_rope_f = reshape(sa_d_k, sa_q_flat.copy(), ctx)
    var d_sa_q_rms_f = rope_backward(d_sa_q_rope_f, cos, sin, False, ctx)
    var d_sa_k_rms_f = rope_backward(d_sa_k_rope_f, cos, sin, False, ctx)
    var sa_qpre_f = reshape(saved.sa_attn.q_pre[], sa_q_flat.copy(), ctx)
    var sa_kpre_f = reshape(saved.sa_attn.k_pre[], sa_q_flat.copy(), ctx)
    ...
    var d_sa_q_proj = reshape(sa_rbq.d_x, sa3.copy(), ctx)
    var d_sa_k_proj = reshape(sa_rbk.d_x, sa3.copy(), ctx)
    var d_sa_v_proj = reshape(sa_d_v, sa3.copy(), ctx)
    var sa_lbq = linear_backward(d_sa_q_proj, saved.sa_attn.q_ctx_in[], w.sa_q[], B * S_IMG, D, D, ctx)
    var sa_lbk = linear_backward(d_sa_k_proj, saved.sa_attn.q_ctx_in[], w.sa_k[], B * S_IMG, D, D, ctx)
    var sa_lbv = linear_backward(d_sa_v_proj, saved.sa_attn.q_ctx_in[], w.sa_v[], B * S_IMG, D, D, ctx)
    var d_sa_q = sa_lbq.d_w.to_host(ctx)
    ...
    var d_sa_xmod = add(add(_t(d_sa_q_dx, [B, S_IMG, D], ctx), _t(d_sa_k_dx, [B, S_IMG, D], ctx), ctx),
                        _t(d_sa_v_dx, [B, S_IMG, D], ctx), ctx)
    var sa_mb = modulate_backward_b2_perhalf(d_sa_xmod, saved.sa.ln[], saved.sa.scale[], ctx)
    var sa_dxin = layer_norm_backward_dx(sa_mb.d_x, saved.sa.x_in[], ln_ones, eps, ctx)
    d_x_in = add(d_x_in, sa_dxin, ctx)
    var sa_dadd = concat(1, ctx, sa_mb.d_shift, sa_mb.d_scale, sa_gg.d_gate)
    var sa_mlb2 = linear_backward(sa_dadd, saved.sa.mod_h[], w.sa_mod2[], B, 256, 3 * D, ctx)
...
# ══════════════════════════════════════════════════════════════════════════════
# TRUE BATCH-2 DEVICE-RESIDENT block forward/backward. Row-stacked B==2 clones of
# anima_block_lora_forward_device_tensor / _backward_device_tensors, differing
# ONLY where the batch dim must be handled per-sample (the shared SDPA kernels are
# templated on B=1): self- and cross-attention run per sample (slice→sdpa→concat,
# same as anima_block_lora_forward_b2/_backward_b2), and modulate backward uses
# modulate_backward_b2_perhalf (per-half d_scale/d_shift chained into the mod MLP).
# Every other op is row-stacked-M agnostic, so it threads B unchanged. Saved struct
# + LoRA grad contract are IDENTICAL to the b1 device path (AnimaBlockSaved holds
# [2,…] device tensors; d_a/d_b summed over both samples' 2N rows = the batch grad).
# ══════════════════════════════════════════════════════════════════════════════
def anima_block_lora_forward_device_tensor_b2[
    H: Int, Dh: Int, S_IMG: Int, S_TXT: Int
](
    x_arc: TArc,                 # [2,S_img,D] F32 residual-stream input (DEVICE)
    t_silu_arc: TArc,            # [2,2048] silu(t_cond) — uploaded ONCE per step
    base_t_arc: TArc,            # [2,6144] base_adaln — uploaded ONCE per step
    ctx_t_arc: TArc,             # [2,S_txt,1024] frozen text context — ONCE per step
    w: AnimaBlockWeights, lora: AnimaBlockLoraDevice,
    cos: Tensor, sin: Tensor,    # [2*S_img*H, Dh/2] doubled rope table
    B: Int, D: Int, JOINT: Int, F: Int, eps: Float32,
    ctx: DeviceContext,
) raises -> AnimaBlockForwardLoraTensor:
    if B != 2:
        raise Error("anima_block_lora_forward_device_tensor_b2 requires B==2, got " + String(B))
    var ln_ones = _t(_ones(D), [D], ctx)
    var ln_zeros = _t(_zeros(D), [D], ctx)
    var sa_scale = Float32(1.0) / fsqrt(Float32(Dh))
...
def anima_block_lora_backward_device_tensors_b2[
    H: Int, Dh: Int, S_IMG: Int, S_TXT: Int
](
    d_out: Tensor,             # [2,S_img,D] DEVICE
    saved: AnimaBlockSaved,
    w: AnimaBlockWeights, lora: AnimaBlockLoraDevice,
    cos: Tensor, sin: Tensor,  # [2*S_img*H, Dh/2] doubled rope table
    B: Int, D: Int, JOINT: Int, F: Int, eps: Float32,
    ...
    if B != 2:
        raise Error("anima_block_lora_backward_device_tensors_b2 requires B==2, got " + String(B))
    var ln_ones = _t(_ones(D), [D], ctx)
    var sa_scale = Float32(1.0) / fsqrt(Float32(Dh))
    ...
    d_a[SLOT_CA_O] = pg_caout.d_a.copy(); d_b[SLOT_CA_O] = pg_caout.d_b.copy()
    var d_ca_att4 = reshape(pg_caout.d_x, [B, S_IMG, H, Dh], ctx)
    # PER-SAMPLE rectangular cross-attn backward (template B=1; slice, bwd, concat).
    var ca_sb0 = sdpa_backward_rect[1, S_IMG, S_TXT, H, Dh](
        slice(saved.ca_attn.q_sdpa[], 0, 0, 1, ctx), slice(saved.ca_attn.k_sdpa[], 0, 0, 1, ctx),
        slice(saved.ca_attn.v4[], 0, 0, 1, ctx), slice(d_ca_att4, 0, 0, 1, ctx), sa_scale, ctx,
    )
    ...
    var ca_d_k = concat(0, ctx, ca_sb0.d_k, ca_sb1.d_k)   # [2,S_txt,H,Dh]
    var ca_d_v = concat(0, ctx, ca_sb0.d_v, ca_sb1.d_v)   # [2,S_txt,H,Dh]
    var d_ca_q_rms_f = reshape(ca_d_q, [B * S_IMG * H, Dh], ctx)
    var d_ca_k_rms_f = reshape(ca_d_k, [B * S_TXT * H, Dh], ctx)
    var ca_qpre_f = reshape(saved.ca_attn.q_pre[], [B * S_IMG * H, Dh], ctx)
    var ca_kpre_f = reshape(saved.ca_attn.k_pre[], [B * S_TXT * H, Dh], ctx)
    var d_ca_q_pre = rms_norm_backward_dx(d_ca_q_rms_f, ca_qpre_f, w.ca_qn[], Float32(1e-6), ctx)
    var d_ca_k_pre = rms_norm_backward_dx(d_ca_k_rms_f, ca_kpre_f, w.ca_kn[], Float32(1e-6), ctx)
    var d_ca_q_proj = reshape(d_ca_q_pre, [B, S_IMG, D], ctx)
    var d_ca_k_proj = reshape(d_ca_k_pre, [B, S_TXT, D], ctx)
    var d_ca_v_proj = reshape(ca_d_v, [B, S_TXT, D], ctx)
    var pg_caq = _proj_bwd_with_lora_device_tensors(
        d_ca_q_proj, saved.ca_attn.q_ctx_in[], w.ca_q[], lora.ca_q, B * S_IMG, D, D, True, ctx,
    )
    d_a[SLOT_CA_Q] = pg_caq.d_a.copy(); d_b[SLOT_CA_Q] = pg_caq.d_b.copy()
    ...
    d_a[SLOT_SA_O] = pg_saout.d_a.copy(); d_b[SLOT_SA_O] = pg_saout.d_b.copy()
    var d_sa_att4 = reshape(pg_saout.d_x, [B, S_IMG, H, Dh], ctx)
    # PER-SAMPLE self-attn backward (template B=1; slice, bwd, concat).
    var sa_sb0 = sdpa_backward[1, S_IMG, H, Dh](
        slice(saved.sa_attn.q_sdpa[], 0, 0, 1, ctx), slice(saved.sa_attn.k_sdpa[], 0, 0, 1, ctx),
        slice(saved.sa_attn.v4[], 0, 0, 1, ctx), slice(d_sa_att4, 0, 0, 1, ctx), sa_scale, ctx,
    )
    ...
    var sa_d_k = concat(0, ctx, sa_sb0.d_k, sa_sb1.d_k)
    var sa_d_v = concat(0, ctx, sa_sb0.d_v, sa_sb1.d_v)
    var d_sa_q_rope_f = reshape(sa_d_q, [B * S_IMG * H, Dh], ctx)
    var d_sa_k_rope_f = reshape(sa_d_k, [B * S_IMG * H, Dh], ctx)
    var d_sa_q_rms_f = rope_backward(d_sa_q_rope_f, cos, sin, False, ctx)
    var d_sa_k_rms_f = rope_backward(d_sa_k_rope_f, cos, sin, False, ctx)
    var sa_qpre_f = reshape(saved.sa_attn.q_pre[], [B * S_IMG * H, Dh], ctx)
    var sa_kpre_f = reshape(saved.sa_attn.k_pre[], [B * S_IMG * H, Dh], ctx)
    ...
    var d_sa_q_proj = reshape(d_sa_q_pre, [B, S_IMG, D], ctx)
    var d_sa_k_proj = reshape(d_sa_k_pre, [B, S_IMG, D], ctx)
    var d_sa_v_proj = reshape(sa_d_v, [B, S_IMG, D], ctx)
    var pg_saq = _proj_bwd_with_lora_device_tensors(
        d_sa_q_proj, saved.sa_attn.q_ctx_in[], w.sa_q[], lora.sa_q, B * S_IMG, D, D, True, ctx,
    )
    d_a[SLOT_SA_Q] = pg_saq.d_a.copy(); d_b[SLOT_SA_Q] = pg_saq.d_b.copy()
    ...
    d_a[SLOT_SA_V] = pg_sav.d_a.copy(); d_b[SLOT_SA_V] = pg_sav.d_b.copy()
    # 3D reshape REQUIRED before the per-half modulate: pg_*.d_x are [B*S_IMG, D]
    # and modulate_backward_b2_perhalf slices the BATCH dim (dim 0) per sample, so a
    # 2D tensor would be mis-sliced into single rows. (MLP/CA reshape above; SA
    # summed the three q/k/v d_x, so reshape the sum here.)
    var d_sa_xmod = reshape(add(add(pg_saq.d_x, pg_sak.d_x, ctx), pg_sav.d_x, ctx), [B, S_IMG, D], ctx)
    var sa_mb = modulate_backward_b2_perhalf(d_sa_xmod, saved.sa.ln[], saved.sa.scale[], ctx)
    var sa_dxin = layer_norm_backward_dx(sa_mb.d_x, saved.sa.x_in[], ln_ones, eps, ctx)
    d_x_in = add(d_x_in, sa_dxin, ctx)
    var sa_dadd = concat(1, ctx, sa_mb.d_shift, sa_mb.d_scale, sa_gg.d_gate)
