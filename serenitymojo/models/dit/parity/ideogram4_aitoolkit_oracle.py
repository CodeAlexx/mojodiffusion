...
# block backward (models/ideogram4/ideogram4_block_ft.mojo) has NO adapter
...
def main():
    ...
    if FT_MODE:
        # BASE-ONLY full-finetune reference: no LoRA anywhere; every base block
        # param is trainable so torch autograd produces the W.grad refs the FT
        # gate compares (v1 mojo surface = the 6 matmul weights; the extra
        # norm/bias grads are dumped too for future surface extensions).
        adapters = None
        for p in block.parameters():
            p.requires_grad_(True)
        n_train = sum(p.numel() for p in block.parameters() if p.requires_grad)
        print(f"[oracle] FT MODE (base-only, no LoRA): {n_train/1e6:.1f}M trainable params")
    else:
    ...
    out_path = OUT_FT if FT_MODE else OUT
    save_file(fx, out_path)
    print(f"[oracle] saved {len(fx)} tensors -> {out_path}")
    # quick sanity dump
    if FT_MODE:
        # NON-DEGENERACY proof for the 6 matmul dW refs the FT gate consumes.
        for _, path in LORA_SLOTS:
            dW = fx[f"bwd.grad.{path}.weight"]
            print(f"  grad.{path}.weight {tuple(dW.shape)} "
                  f"std={dW.std():.3e} nonzero={int((dW != 0).sum())}/{dW.numel()}")
    else:
        for slot, _ in LORA_SLOTS:
