...
def _qwen_latent_scale_apply[scale_mode: Bool](
    ...
    if dt == DType.float32:
        ...
        comptime if scale_mode:
            ctx.enqueue_function[_qwen_scale_kernel[DType.float32]](X, M, S, O, Int32(inner), Int32(channels), Int64(n), grid_dim=grid, block_dim=_BLOCK)
        else:
            ctx.enqueue_function[_qwen_unscale_kernel[DType.float32]](X, M, S, O, Int32(inner), Int32(channels), Int64(n), grid_dim=grid, block_dim=_BLOCK)
        ...
        comptime if scale_mode:
            ctx.enqueue_function[_qwen_scale_kernel[DType.bfloat16]](X, M, S, O, Int32(inner), Int32(channels), Int64(n), grid_dim=grid, block_dim=_BLOCK)
        else:
            ctx.enqueue_function[_qwen_unscale_kernel[DType.bfloat16]](X, M, S, O, Int32(inner), Int32(channels), Int64(n), grid_dim=grid, block_dim=_BLOCK)
    else:
        ...
        comptime if scale_mode:
            ctx.enqueue_function[_qwen_scale_kernel[DType.float16]](X, M, S, O, Int32(inner), Int32(channels), Int64(n), grid_dim=grid, block_dim=_BLOCK)
        else:
            ctx.enqueue_function[_qwen_unscale_kernel[DType.float16]](X, M, S, O, Int32(inner), Int32(channels), Int64(n), grid_dim=grid, block_dim=_BLOCK)
    ctx.synchronize()
