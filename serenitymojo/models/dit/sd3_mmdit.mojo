...
def _copy_selected_pos_bytes(
    ...
    for row in range(crop_h):
        for col in range(crop_w):
            var src_token = sd3_pos_crop_source_token(
                max_grid, crop_h, crop_w, row, col
            )
...
struct SD3MMDiTPreBlockGate(Movable):
    ...
    def load_large_default(ctx: DeviceContext) raises -> SD3MMDiTPreBlockGate:
        return SD3MMDiTPreBlockGate.load_large_shape[
    ...
    def load_large_shape_from_path[LH: Int, LW: Int](
        ...
        comptime PW = LW // SD3_LARGE_PATCH_SIZE
