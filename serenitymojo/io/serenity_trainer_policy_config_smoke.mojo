# serenity_trainer_policy_config_smoke.mojo - focused SerenityTrainer config-reader slice.
#
...
#
# Run:
#   timeout 180 prlimit --as=16000000000 pixi run mojo run -I . serenitymojo/io/serenity_trainer_policy_config_smoke.mojo
...
def _write_file(path: String, content: String) raises:
    ...
    if fd < 0:
        raise Error(String("SerenityTrainer policy smoke: cannot create ") + path)
    ...
    _ = sys_close(fd)
    if wrote != n:
        raise Error(String("SerenityTrainer policy smoke: short write to ") + path)
...
def _gate_qwen_lora_policy() raises:
    print("--- SerenityTrainer Qwen LoRA-style policy fields ---")
    var body = String("")
    ...
    var c = _read_json(String("/tmp/serenity_trainer_policy_qwen.json"), body)
...
def _gate_adafactor_policy() raises:
    print("--- SerenityTrainer ADAFACTOR defaults and extra distributions ---")
    var body = String("")
    ...
    var c = _read_json(String("/tmp/serenity_trainer_policy_adafactor.json"), body)
...
def _gate_unknown_dtype_fails() raises:
    ...
    try:
        var c = _read_json(
            String("/tmp/serenity_trainer_policy_bad_dtype.json"),
    ...
    if not raised:
        raise Error("SerenityTrainer policy smoke: unknown dtype did not raise")


def main() raises:
    print("=== SerenityTrainer policy config reader smoke ===")
    _gate_qwen_lora_policy()
    _gate_adafactor_policy()
    _gate_unknown_dtype_fails()
    print("serenity_trainer_policy_config_smoke PASS")
