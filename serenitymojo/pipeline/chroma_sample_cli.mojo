...
def _double_block(
    ...
    # [txt512, img] attention (matches inference-flame chroma). Same 1/sqrt(HEAD_DIM)
    # scale as the old sdpa_nomask path, so real_txt_len==512 reproduces it.
    var att = sdpa_qwen_flash_padmask[1, S, HEADS, HEAD_DIM, N_TXT](
...
def _single_block(
    ...
    # MJ-1048: same [txt512, img] joint layout as the double block — mask the
    # T5 pad-row key columns [real_txt_len, N_TXT) here too.
    var att = sdpa_qwen_flash_padmask[1, S, HEADS, HEAD_DIM, N_TXT](
        q, k, v, real_txt_len, Float32(1.0) / sqrt(Float32(HEAD_DIM)), ctx
    )
...
# unpadded T5 token count) so the DiT can mask the ~490 T5 pad rows in the
# [txt512, img] joint attention. Tries primary_key, then fallback_key (pass "" to
# skip — used for the uncond side so a single-key file falls back to cond_real_len).
# Old sidecars lacking these keys default to the full N_TXT (512) — the pre-fix
# behavior (pad rows NOT masked) — printed LOUD.
def _sidecar_real_len(
    ...
    for ref nm in names:
        if nm == primary_key:
            chosen = primary_key.copy()
    if chosen == String("") and fallback_key != String(""):
        for ref nm in names:
            if nm == fallback_key:
                chosen = fallback_key.copy()
    if chosen == String(""):
        print(
            "  [WARN] sidecar", path, "has no key", primary_key,
        ...
        return N_TXT
    var h = Tensor.from_view(st.tensor_view(chosen), ctx).to_host(ctx)
    if len(h) < 1:
        print("  [WARN] sidecar key", chosen, "empty; defaulting real_txt_len to", N_TXT)
        return N_TXT
    var rl = Int(h[0])
    if rl < 1 or rl > N_TXT:
        print(
            "  [WARN] sidecar", chosen, "=", rl, "out of range [1,", N_TXT,
            "]; defaulting real_txt_len to", N_TXT,
        )
        return N_TXT
    print("  [info] sidecar", chosen, "=", rl, "of", N_TXT)
    return rl
...
def _legacy_disk_streaming_main_disabled() raises:
    ...
    # DiT joint attention. cond from caps_pos; uncond from caps_neg (fall back to
    # cond_real_len for a single-key sidecar, mirroring the t5_uncond fallback).
    var cond_real_len = _sidecar_real_len(
        caps_pos, String("cond_real_len"), String(""), ctx
    )
    var uncond_real_len = _sidecar_real_len(
        caps_neg, String("uncond_real_len"), String("cond_real_len"), ctx
    )
...
def main() raises:
    raise Error(
        "disabled: this legacy Chroma CLI reads checkpoint weights during "
