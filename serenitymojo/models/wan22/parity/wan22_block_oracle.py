...
def main():
    ...
    q = rms_norm_lastdim(sa_in @ w["sa_wq"].T + w["sa_bq"], w["sa_qn"]).reshape(1, S, H, Dh)
    k = rms_norm_lastdim(sa_in @ w["sa_wk"].T + w["sa_bk"], w["sa_kn"]).reshape(1, S, H, Dh)
    ...
    caq = rms_norm_lastdim(n3 @ w["ca_wq"].T + w["ca_bq"], w["ca_qn"]).reshape(1, S, H, Dh)
    cak = rms_norm_lastdim(context @ w["ca_wk"].T + w["ca_bk"], w["ca_kn"]).reshape(1, TXT, H, Dh)
