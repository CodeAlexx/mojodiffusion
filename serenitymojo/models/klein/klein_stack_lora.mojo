...
#       (the per-adapter SerenityTrainer-style AdamW).
#
...
from serenitymojo.ops.tensor_algebra import concat, slice, zeros_device, reshape_in_place, add
...
from serenitymojo.models.klein.double_block import (
    ...
    double_block_lora_backward_device_resident_scratch_devgrads,
...
from serenitymojo.models.klein.single_block import (
    ...
    single_block_lora_backward_device_resident_scratch_devgrads,
...
from serenitymojo.training.lora_save import (
    NamedLora, save_lora_serenity_trainer, load_lora_for_resume,
    save_lora_train_state, load_lora_train_state, _read_f32,
)
# PERF (2026-07-11 nsys): the scalar offload_to_host/restore_to_device byte-loop
# cost ~32ms PER BLOCK BOUNDARY (~1.1s/step pure host stall, GPU idle 59%) on the
# 838MB/step block-input tape. The _fast variants are byte-identical (memcpy
# through pinned staging) — alias them in for every tape offload/restore here.
from serenitymojo.training.checkpoint import (
    HostOffload,
    offload_to_host_fast as offload_to_host,
    restore_to_device_fast as restore_to_device,
)
from serenitymojo.training.lora_adamw_serenity_fused import (
    ...
    fused_lora_adamw_serenity_step_resident, lora_adamw_serenity_device_state_sync_moments,
    fused_lora_adamw_serenity_step_resident_devgrads,
)
from serenitymojo.models.klein.activation_tape import (
    KleinStackLoraOffloadedTape,
    SingleBlockSavedOffload,
    DoubleBlockSavedOffload,
    offload_single_block_saved,
    offload_double_block_saved,
    restore_single_block_saved,
    restore_double_block_saved,
)
...
# Matches SerenityTrainer LoRAModule.initialize_weights for lora_down/lora_up shape.
...
def _klein_resident_adapter(
    lo: LoraAdapter, state: LoraAdamWSerenityDeviceState, idx: Int,
...
def klein_lora_set_to_device_resident(
    set: KleinLoraSet,
    dbl_state: LoraAdamWSerenityDeviceState,
    sgl_state: LoraAdamWSerenityDeviceState,
    ctx: DeviceContext,
) raises -> KleinLoraDeviceSet:
    """v2 engine (resident-set): device LoRA set built ONCE, every adapter a
    zero-copy sub-buffer view into the persistent reference trainer-AdamW parameter buffers
...
def klein_lora_adamw_step_resident(
    mut dbl_state: LoraAdamWSerenityDeviceState,
    mut sgl_state: LoraAdamWSerenityDeviceState,
    ...
    var seed = UInt32(t)
    fused_lora_adamw_serenity_step_resident(
...
def _stream_weights_from_block(
    ...
    var wqkv_k = ap + String(".qkv.weight")
    var wproj_k = ap + String(".proj.weight")
    var wgu_k = mp + String(".0.weight")
    var wd_k = mp + String(".2.weight")
    var wqkv = _block_tensor_base(block, wqkv_k)
    var wproj = _block_tensor_base(block, wproj_k)
    var wgu = _block_tensor_base(block, wgu_k)
    var wd = _block_tensor_base(block, wd_k)
    var qn = _block_tensor_base(block, ap + String(".norm.query_norm.scale"))
    var kn = _block_tensor_base(block, ap + String(".norm.key_norm.scale"))
    # int8-resident handle (pin_residents_int8): the 4 base weights are int8 [N,K]
    # and each carries its F32 scalar scale under resident_i8_scale_key(name). Pair
    # them into a StreamInt8 (list order [wqkv, wproj, wgu, wd] — matches
    # quantize_stream_int8 / the int8 forward's base-matmul index) so the loader's
    # quantize-at-load produces the SAME payload as the block-direct quantize.
    var i8 = Optional[StreamInt8](None)
    if wqkv[].dtype() == STDtype.I8:
        var w8 = List[TArc]()
        var sc = List[TArc]()
        w8.append(wqkv.copy())
        sc.append(_block_tensor_base(block, resident_i8_scale_key(wqkv_k)))
        w8.append(wproj.copy())
        sc.append(_block_tensor_base(block, resident_i8_scale_key(wproj_k)))
        w8.append(wgu.copy())
        sc.append(_block_tensor_base(block, resident_i8_scale_key(wgu_k)))
        w8.append(wd.copy())
...
def _single_weights_from_block(
    ...
    var w1_k = prefix + String(".linear1.weight")
    var w2_k = prefix + String(".linear2.weight")
    var w1 = _block_tensor_base(block, w1_k)
    var qn = _block_tensor_base(block, prefix + String(".norm.query_norm.scale"))
    var kn = _block_tensor_base(block, prefix + String(".norm.key_norm.scale"))
    # int8-resident handle (pin_residents_int8): w1/w2 are int8 [N,K] + F32 scalar
    # scales; assemble the SingleBlockInt8 payload (list order [w1, w2] — matches
    # quantize_single_block_int8 / the int8 forward's base-matmul index) and build
    # the weights via the int8 constructor (bf16 fields are [1,1] dummies).
    if w1[].dtype() == STDtype.I8:
        var w2 = _block_tensor_base(block, w2_k)
        var w8 = List[TArc]()
        var sc = List[TArc]()
        w8.append(w1.copy())
        sc.append(_block_tensor_base(block, resident_i8_scale_key(w1_k)))
        w8.append(w2.copy())
...
struct KleinLoraGrads(Copyable, Movable):
    ...
    # OPT-IN img-EDIT reference-branch grad (models/klein/img_in_ref.mojo). The
    # load-bearing grad of the NEW trained img_in_ref [D, in_ch] projection. EMPTY
    # (len 0) on the default text-to-image path (no ref supplied) — see the
    # device-turbo forward/backward `ref_tokens_t`/`img_in_ref_w` hooks. Trailing
    # + defaulted so every existing KleinLoraGrads(...) call site stays valid.
    var d_img_in_ref: List[Float32]   # [D, in_ch]  (empty when no ref)
...
struct KleinLoraTensorGrads(Copyable, Movable):
    ...
    var d_final_scale: List[Float32]
    # OPT-IN img-EDIT reference-branch grad (models/klein/img_in_ref.mojo). The
    # load-bearing grad of the NEW trained img_in_ref [D, in_ch] projection. Kept as
    # a HOST List[Float32] (a small trained-param grad) even though the block LoRA
    # d_A/d_B stay on device — it must survive the resident-grads path. EMPTY (len 0)
    # on the default text-to-image path. Trailing + defaulted so every existing
    # KleinLoraTensorGrads(...) call site stays valid.
    var d_img_in_ref: List[Float32]   # [D, in_ch]  (empty when no ref)
...
def klein_stack_lora_forward_device_inputs_offload_turbo_moddev_rope_scratch[
    ...
    ctx: DeviceContext,
    mut scratch: ScratchRingAllocator,
    # OPT-IN img-EDIT reference-conditioning projection (models/klein/img_in_ref.mojo).
    # When BOTH are supplied, the input image projection becomes
    #   img = linear(noise, img_in) + linear(ref_tokens, img_in_ref)
    # (the SAME summed projection klein_stack_forward / img_in_ref_forward compute:
    # a plain no-bias linear on the ref tokens, added to the base img projection).
    # Absent (default None) => the EXACT existing text-to-image path, byte-for-byte
    # (C13 flags-off). ref_tokens_t [N_IMG, in_ch] and img_in_ref_w [D, in_ch] are
    # CONSTANT across denoise steps (treat as resident base inputs). The block stack
    # in between is untouched.
    ref_tokens_t: Optional[TArc] = None,
    img_in_ref_w: Optional[TArc] = None,
) raises -> KleinStackForward:
    ...
    var no_bias = Optional[Tensor](None)
    var img = TArc(linear(img_tokens_t[], base.img_in[], no_bias^, ctx))
    # OPT-IN reference projection (img-EDIT). Identical ops to img_in_ref_forward's
    # ref branch (no-bias linear + add); skipped entirely when no ref is supplied.
    if Bool(ref_tokens_t) and Bool(img_in_ref_w):
        var no_bias_ref = Optional[Tensor](None)
        var ref_proj = linear(
            ref_tokens_t.value()[], img_in_ref_w.value()[], no_bias_ref^, ctx
        )
        img = TArc(add(img[], ref_proj, ctx))
    var no_bias_txt = Optional[Tensor](None)
    var txt = TArc(linear(txt_tokens_t[], base.txt_in[], no_bias_txt^, ctx))
    var img_in_act = img.copy()
    var txt_in_act = txt.copy()
...
def klein_stack_lora_forward_device_inputs_offload_turbo_moddev_rope_scratch_offloaded_tape[
    ...
    device_tape: Bool = False,
    # OPT-IN img-EDIT reference-conditioning projection (models/klein/img_in_ref.mojo),
    # IDENTICAL to the primary device-turbo forward hook. When BOTH are supplied the
    # input image projection becomes img = linear(noise, img_in) + linear(ref, img_in_ref)
    # BEFORE the projected `img` enters the block tape — so the constant ref inputs are
    # folded into the FIRST block's tape input, never taped/offloaded/recomputed
    # separately (they are resident carriers passed straight to backward, like base.img_in).
    # Absent (default None) => the EXACT existing tape path, byte-for-byte (C13 flags-off).
    ref_tokens_t: Optional[TArc] = None,
    img_in_ref_w: Optional[TArc] = None,
) raises -> KleinStackLoraOffloadedTape:
    """Forward for SerenityTrainer CPU_OFFLOADED-style Klein LoRA training.
    ...
    When `save_activations` (KLEIN_SAVE_ACTIVATIONS) is set, each block's FULL
    forward activation set (SingleBlockSaved / DoubleBlockSaved) is additionally
    offloaded to host, so the backward can consume it directly instead of
    recomputing the block forward. The saved set is exactly what the block
    backward reads — byte-identical to what the recompute path would produce on
    the int8 path (both use `*_forward_device_resident_scratch` for singles and
    `double_block_lora_forward_device_resident_scratch` for doubles).
    """
    var num_double = lora.num_double
    var num_single = lora.num_single
    ...
    var no_bias = Optional[Tensor](None)
    var img = TArc(linear(img_tokens_t[], base.img_in[], no_bias^, ctx))
    # OPT-IN reference projection (img-EDIT) — IDENTICAL ops to the primary hook and
    # to img_in_ref_forward's ref branch (no-bias linear + add), applied to `img`
    # BEFORE it is copied into the block tape below. The ref inputs (ref_tokens_t,
    # img_in_ref_w) are CONSTANT resident carriers and are NEVER pushed onto the
    # per-block activation tape (dbl_img_in / *_dev) — only the summed `img` is.
    # Skipped entirely (byte-identical) when no ref is supplied.
    if Bool(ref_tokens_t) and Bool(img_in_ref_w):
        var no_bias_ref = Optional[Tensor](None)
        var ref_proj = linear(
            ref_tokens_t.value()[], img_in_ref_w.value()[], no_bias_ref^, ctx
    ...
    # KLEIN_DEVICE_TAPE: park block inputs as DEVICE TArc refs (refcount copy,
    # zero bytes moved, no sync) instead of the host round-trip. ~838MB VRAM at
    # 512px/b2; kills the measured ~0.6s/step residual host-tape stall.
    ...
    for bi in range(num_double):
        if device_tape:
            dbl_img_in_dev.append(img.copy())
            dbl_txt_in_dev.append(txt.copy())
        else:
        ...
        if save_activations:
            dbl_saved.append(offload_double_block_saved(fwd.saved, ctx))
        img = fwd.img_out.copy()
        txt = fwd.txt_out.copy()
        loader.mark_active_block_done(ctx)
    var x = TArc(concat(0, ctx, txt[], img[]))
    ...
    for bi in range(num_single):
        ...
        if save_activations:
            if not w.int8:
                # NOTE (int8-flash alignment 2026-07-11): the single forward is
                # now flash under KLEIN_SDPA_FLASH for BOTH arms, so the saved
                # set would byte-match the (flash) bf16 recompute too. The
                # guard is kept as-is — save_activations is only exercised and
                # gated on the int8-resident config — and simply hasn't been
                # validated for the bf16 combo, not because of an sdpa-arm
                # mismatch anymore.
                raise Error(
                    "KLEIN_SAVE_ACTIVATIONS: single block "
                    + String(bi)
                    + " is not int8-resident; the save_activations path is"
                    + " only validated on the int8-resident config (the saved"
                    + " flash activation set now matches the flash recompute"
                    + " on both arms, but the bf16 combo is ungated) —"
                    + " run the int8-resident config or disable the flag."
                )
            sgl_saved.append(offload_single_block_saved(fwd.saved, ctx))
        x = fwd.out.copy()
        loader.mark_active_block_done(ctx)
    ...
    var no_bias_out = Optional[Tensor](None)
    var out = linear(normed, base.final_lin[], no_bias_out^, ctx).to_host(ctx)
    var img_out_dev = List[TArc]()
    var ln_img_out_dev = List[TArc]()
    var off_img_out: HostOffload
    var off_ln_img_out: HostOffload
    if device_tape:
        img_out_dev.append(img_out.copy())
        ln_img_out_dev.append(ln_img_out.copy())
        off_img_out = HostOffload(List[UInt8](), [0], STDtype.F32)
        off_ln_img_out = HostOffload(List[UInt8](), [0], STDtype.F32)
    else:
        off_img_out = offload_to_host(img_out[], ctx)
        off_ln_img_out = offload_to_host(ln_img_out[], ctx)
...
def klein_stack_lora_backward_offload_turbo_moddev_rope_scratch[
    ...
    compute_input_grads: Bool = True,
    compute_aux_grads: Bool = True,
    # OPT-IN img-EDIT reference-conditioning backward (mirror of the forward hook
    # above; models/klein/img_in_ref.mojo). When ref_tokens_t [N_IMG, in_ch] is
    # supplied, d_img_in_ref [D, in_ch] is emitted into KleinLoraGrads.d_img_in_ref
    # — the SAME grad img_in_ref_backward / klein_stack_backward's d_img_in_ref
    # gives: a plain linear_backward_dw on the ref branch with grad_y = d_io (dL wrt
    # the img input-projection output). Absent (default None) => d_img_in_ref stays
    # EMPTY and the pass is byte-identical to the text-to-image backward. This is a
    # TRAINED-param grad (NOT gated by compute_input_grads, which only gates the
    # discarded FROZEN img_in/txt_in grads).
    ref_tokens_t: Optional[TArc] = None,
) raises -> KleinLoraGrads:
    var num_double = lora.num_double
    var num_single = lora.num_single
    ...
    while bi >= 0:
        ...
        if bi >= saved_single_start:
            ...
            # int8-resident: the recompute-only helper reads the bf16 w.w1 (a
            # [1,1] dummy on the int8 path). Recompute via the int8-enabled full
            # forward and take its saved (mirrors the double recompute below,
            # which already reuses the full forward). bf16 path unchanged.
            var rf = single_block_lora_forward_device_resident_scratch[H, Dh, S](
                saved.sgl_x_in[bi], w, single_mod_dev, sl,
                cos_t, sin_t, D, F, eps, norm_ones[], norm_zeros[], ctx, scratch,
                int8=w.int8,
            )
            block_saved = rf.saved.copy()
        else:
            block_saved = single_block_lora_recompute_saved_device_resident_scratch[H, Dh, S](
                saved.sgl_x_in[bi], w, single_mod_dev, sl,
                cos_t, sin_t, D, F, eps, norm_ones[], norm_zeros[], ctx, scratch,
    ...
    # d_io IS dL/d(img input-projection output) [N_IMG, D] — the SAME `d_img_act`
    # img_in_ref_backward consumes. d_img_in_ref = d_ioᵀ @ ref_tokens
    # (linear_backward_dw, the d_w half of img_in_ref_backward's linear_backward).
    # Computed in F32 to match the base klein_stack_backward composition (all-F32
    # _t tensors). Independent of compute_input_grads — this is the TRAINED param's
    # load-bearing grad; empty when no ref supplied.
    var d_img_in_ref = List[Float32]()
    if Bool(ref_tokens_t):
        # Tensor is move-only; cast_tensor materializes an owned F32 tensor (a
        # bitwise D2D clone on the F32->F32 no-op path) so grad_y/x dtypes match
        # linear_backward_dw's requirement and the math is all-F32 (== base).
        var d_io_f32 = cast_tensor(d_io[], STDtype.F32, ctx)
        var ref_f32 = cast_tensor(ref_tokens_t.value()[], STDtype.F32, ctx)
        var d_ref_w = linear_backward_dw(
            d_io_f32, ref_f32, N_IMG, in_ch, D, ctx, STDtype.F32,
        )
        d_img_in_ref = d_ref_w.to_host(ctx)
...
# ══════════════════════════════════════════════════════════════════════════════
# TRUE BATCH-2 stacked-step drivers (2026-07-04, fleet true-batching rung 2).
# Two cache samples processed in a single step. Klein fuses QKV + the joint
# img/txt attention + out-proj + MLP inside each block over a HARDCODED batch
# dim 1 (sdpa_nomask / sdpa_flash [1, S, H, Dh]); there is no B=2 flash kernel,
# so — per the wave directive — attention runs PER SAMPLE: each block is invoked
# TWICE per step (once on each sample's [txt|img] joint sequence), sharing ONE
# base-weight load and ONE LoRA adapter view. The two samples carry INDEPENDENT
# per-sample modulation (their sigmas differ), so each gets its own img/txt/
# single modvecs + final adaLN scale/shift. The shared LoRA adapter grads from
# BOTH halves are SUMMED into one KleinLoraGrads = the batch gradient.
#
# Loss/scaling contract (owned by the CALLER, NOT these drivers): the joint
# 2N-mean MSE loss L = (1/2)(L_s0 + L_s1); its per-sample output grad is HALF
# the single-sample d_loss. The caller passes d_out{0,1} ALREADY scaled by 0.5,
# so summing the two per-sample backward results yields mean(g_s0, g_s1) — the
# exact SerenityTrainer batch-mean gradient. These drivers are a pure linear backprop
# that sums two samples; they do not know the 0.5.
#
# GATE: models/klein/parity/klein_batch2_parity.mojo — loss_B2 == mean(loss_B1s)
# and per-adapter grad cosine >= 0.999 vs mean(g_B1s).
# batch_size==1 never enters here (byte-identical to the single-sample path).
# ══════════════════════════════════════════════════════════════════════════════
struct KleinStackForwardB2(Copyable, Movable):
    var s0: KleinStackForward   # per-sample forward tape (out + checkpoint inputs)
    var s1: KleinStackForward

    def __init__(out self, var s0: KleinStackForward, var s1: KleinStackForward):
        self.s0 = s0^
        self.s1 = s1^


def _klein_b2_final_forward[
    N_IMG: Int, N_TXT: Int
](
    x: TArc,
    base: KleinStackBase,
    final_scale: TArc, final_shift: TArc,
    var img_in_act: TArc, var txt_in_act: TArc,
    var dbl_img_in: List[TArc], var dbl_txt_in: List[TArc],
    var sgl_x_in: List[TArc],
    D: Int, out_ch: Int, eps: Float32,
    ctx: DeviceContext,
) raises -> KleinStackForward:
    """One sample's final layer (per-sample adaLN scale/shift) + tape assembly.
    SAVE_TAIL==0 so no per-block saved activations are retained (backward
    recomputes them from the saved block INPUTS)."""
    var img_out = TArc(slice(x[], 0, N_TXT, N_IMG, ctx))
    var ln_img_out = TArc(layer_norm(
        img_out[], _t(_ones(D), [D], ctx), _t(_zeros(D), [D], ctx), eps, ctx,
    ))
    var normed = modulate(ln_img_out[], final_scale[], final_shift[], ctx)
    var no_bias_out = Optional[Tensor](None)
    var out = linear(normed, base.final_lin[], no_bias_out^, ctx).to_host(ctx)
    return KleinStackForward(
        out^, img_in_act^, txt_in_act^,
        dbl_img_in^, dbl_txt_in^, sgl_x_in^,
        List[DoubleBlockSaved](), List[SingleBlockSaved](),
        img_out^, ln_img_out^,
    )


def klein_stack_lora_forward_b2[
    H: Int, Dh: Int, N_IMG: Int, N_TXT: Int, S: Int
](
    img_tokens_t0: TArc, txt_tokens_t0: TArc,
    img_tokens_t1: TArc, txt_tokens_t1: TArc,
    base: KleinStackBase,
    dbw: List[DoubleBlockWeights], sbw: List[SingleBlockWeights],
    lora: KleinLoraDeviceSet,
    img_mod_dev0: ModVecsDevice, txt_mod_dev0: ModVecsDevice, single_mod_dev0: SingleModVecsDevice,
    img_mod_dev1: ModVecsDevice, txt_mod_dev1: ModVecsDevice, single_mod_dev1: SingleModVecsDevice,
    final_shift0: TArc, final_scale0: TArc,
    final_shift1: TArc, final_scale1: TArc,
    cos_t: Tensor, sin_t: Tensor,
    D: Int, F: Int, in_ch: Int, txt_ch: Int, out_ch: Int, eps: Float32,
    ctx: DeviceContext,
) raises -> KleinStackForwardB2:
    """Resident (no-loader) TRUE batch-2 forward. Mirrors
    klein_stack_lora_forward_device_inputs_resident_moddev_rope run over TWO
    samples: each block loaded once from dbw/sbw, applied per sample with its
    own modvecs. Used by the parity gate."""
    var num_double = len(dbw)
    var num_single = len(sbw)

    var no_bias0 = Optional[Tensor](None)
    var img0 = TArc(linear(img_tokens_t0[], base.img_in[], no_bias0^, ctx))
    var no_bias_txt0 = Optional[Tensor](None)
    var txt0 = TArc(linear(txt_tokens_t0[], base.txt_in[], no_bias_txt0^, ctx))
    var no_bias1 = Optional[Tensor](None)
    var img1 = TArc(linear(img_tokens_t1[], base.img_in[], no_bias1^, ctx))
    var no_bias_txt1 = Optional[Tensor](None)
    var txt1 = TArc(linear(txt_tokens_t1[], base.txt_in[], no_bias_txt1^, ctx))
    var img_in_act0 = img0.copy()
    var txt_in_act0 = txt0.copy()
    var img_in_act1 = img1.copy()
    var txt_in_act1 = txt1.copy()

    var dbl_img_in0 = List[TArc](); var dbl_txt_in0 = List[TArc]()
    var dbl_img_in1 = List[TArc](); var dbl_txt_in1 = List[TArc]()
    for bi in range(num_double):
        dbl_img_in0.append(img0.copy()); dbl_txt_in0.append(txt0.copy())
        dbl_img_in1.append(img1.copy()); dbl_txt_in1.append(txt1.copy())
        var bl = _dbl_lora_dev_for(lora, bi)
        var fwd0 = double_block_lora_forward_device_resident[H, Dh, N_IMG, N_TXT, S](
            img0, txt0, dbw[bi], img_mod_dev0, txt_mod_dev0, bl,
            cos_t, sin_t, D, F, eps, ctx,
        )
        img0 = fwd0.img_out.copy(); txt0 = fwd0.txt_out.copy()
        var fwd1 = double_block_lora_forward_device_resident[H, Dh, N_IMG, N_TXT, S](
            img1, txt1, dbw[bi], img_mod_dev1, txt_mod_dev1, bl,
            cos_t, sin_t, D, F, eps, ctx,
        )
        img1 = fwd1.img_out.copy(); txt1 = fwd1.txt_out.copy()

    var x0 = TArc(concat(0, ctx, txt0[], img0[]))
    var x1 = TArc(concat(0, ctx, txt1[], img1[]))

    var sgl_x_in0 = List[TArc](); var sgl_x_in1 = List[TArc]()
    for bi in range(num_single):
        sgl_x_in0.append(x0.copy()); sgl_x_in1.append(x1.copy())
        var sl = _sgl_lora_dev_for(lora, bi)
        var f0 = single_block_lora_forward_device_resident[H, Dh, S](
            x0, sbw[bi], single_mod_dev0, sl, cos_t, sin_t, D, F, eps, ctx,
        )
        x0 = f0.out.copy()
        var f1 = single_block_lora_forward_device_resident[H, Dh, S](
            x1, sbw[bi], single_mod_dev1, sl, cos_t, sin_t, D, F, eps, ctx,
        )
        x1 = f1.out.copy()

    var s0 = _klein_b2_final_forward[N_IMG, N_TXT](
        x0, base, final_scale0, final_shift0,
        img_in_act0^, txt_in_act0^, dbl_img_in0^, dbl_txt_in0^, sgl_x_in0^,
        D, out_ch, eps, ctx,
    )
    var s1 = _klein_b2_final_forward[N_IMG, N_TXT](
        x1, base, final_scale1, final_shift1,
        img_in_act1^, txt_in_act1^, dbl_img_in1^, dbl_txt_in1^, sgl_x_in1^,
        D, out_ch, eps, ctx,
    )
    return KleinStackForwardB2(s0^, s1^)


def _klein_b2_final_backward[
    N_IMG: Int, N_TXT: Int
](
    d_out: List[Float32],
    base: KleinStackBase,
    final_scale: TArc,
    fwd: KleinStackForward,
    D: Int, out_ch: Int, eps: Float32,
    ctx: DeviceContext,
) raises -> TArc:
    """Per-sample final-layer backward → d_x [S, D] entering the single stack.
    Frozen base (compute_aux_grads=False; no d_final_scale/shift returned)."""
    var d_normed_t = linear_backward_dx(
        _t(d_out, [N_IMG, out_ch], ctx), base.final_lin[], N_IMG, D, out_ch, ctx,
    )
    var mbf = modulate_backward(
        d_normed_t, fwd.ln_img_out[], final_scale[], ctx, False,
    )
    var d_img_out_t = layer_norm_backward_dx(
        mbf.d_x, fwd.img_out[], _t(_ones(D), [D], ctx), eps, ctx,
    )
    var d_txt_zero = zeros_device([N_TXT, D], STDtype.F32, ctx)
    return TArc(concat(0, ctx, d_txt_zero, d_img_out_t))


def klein_stack_lora_backward_b2[
    H: Int, Dh: Int, N_IMG: Int, N_TXT: Int, S: Int
](
    ...
    img_mod_dev0: ModVecsDevice, txt_mod_dev0: ModVecsDevice, single_mod_dev0: SingleModVecsDevice,
    img_mod_dev1: ModVecsDevice, txt_mod_dev1: ModVecsDevice, single_mod_dev1: SingleModVecsDevice,
    final_scale0: TArc, final_scale1: TArc,
    cos_t: Tensor, sin_t: Tensor,
    saved: KleinStackForwardB2,
    D: Int, F: Int, in_ch: Int, txt_ch: Int, out_ch: Int, eps: Float32,
    ctx: DeviceContext,
) raises -> KleinLoraGrads:
    """Resident (no-loader) TRUE batch-2 backward. Interleaved per block:
    recompute+backward BOTH samples (each with its own modvecs), thread each
    sample's d_x independently, and SUM the shared LoRA adapter grads. Callers
    pass d_out{0,1} pre-scaled by 0.5 so the summed grads == mean(g0, g1)."""
    var num_double = len(dbw)
    var num_single = len(sbw)

    var d_x0 = _klein_b2_final_backward[N_IMG, N_TXT](
        d_out0, base, final_scale0, saved.s0, D, out_ch, eps, ctx,
    )
    var d_x1 = _klein_b2_final_backward[N_IMG, N_TXT](
        d_out1, base, final_scale1, saved.s1, D, out_ch, eps, ctx,
    )
    ...
    while bi >= 0:
        var sl = _sgl_lora_dev_for(lora, bi)
        var saved0 = single_block_lora_recompute_saved_device_resident[H, Dh, S](
            saved.s0.sgl_x_in[bi], sbw[bi], single_mod_dev0, sl,
            cos_t, sin_t, D, F, eps, ctx,
        )
        var bg0 = single_block_lora_backward_device_resident[H, Dh, S](
            d_x0, sbw[bi], single_mod_dev0, sl, saved0, cos_t, sin_t,
            D, F, eps, ctx, False,
        )
        d_x0 = bg0.d_x.copy()
        var saved1 = single_block_lora_recompute_saved_device_resident[H, Dh, S](
            saved.s1.sgl_x_in[bi], sbw[bi], single_mod_dev1, sl,
            cos_t, sin_t, D, F, eps, ctx,
        )
        var bg1 = single_block_lora_backward_device_resident[H, Dh, S](
            d_x1, sbw[bi], single_mod_dev1, sl, saved1, cos_t, sin_t,
            D, F, eps, ctx, False,
        )
        d_x1 = bg1.d_x.copy()
        var sbase = bi * SGL_SLOTS
        sgl_d_a[sbase + 0] = _add_lists(bg0.qkv_d_a, bg1.qkv_d_a)
        sgl_d_b[sbase + 0] = _add_lists(bg0.qkv_d_b, bg1.qkv_d_b)
        sgl_d_a[sbase + 1] = _add_lists(bg0.out_d_a, bg1.out_d_a)
        sgl_d_b[sbase + 1] = _add_lists(bg0.out_d_b, bg1.out_d_b)
        bi -= 1

    var d_to0 = TArc(slice(d_x0[], 0, 0, N_TXT, ctx))
    var d_io0 = TArc(slice(d_x0[], 0, N_TXT, N_IMG, ctx))
    var d_to1 = TArc(slice(d_x1[], 0, 0, N_TXT, ctx))
    var d_io1 = TArc(slice(d_x1[], 0, N_TXT, N_IMG, ctx))
    ...
    while di >= 0:
        var bl = _dbl_lora_dev_for(lora, di)
        var fwd0 = double_block_lora_forward_device_resident[H, Dh, N_IMG, N_TXT, S](
            saved.s0.dbl_img_in[di], saved.s0.dbl_txt_in[di],
            dbw[di], img_mod_dev0, txt_mod_dev0, bl, cos_t, sin_t, D, F, eps, ctx,
        )
        var bg0 = double_block_lora_backward_device_resident[H, Dh, N_IMG, N_TXT, S](
            d_io0, d_to0, dbw[di], img_mod_dev0, txt_mod_dev0, bl, fwd0.saved,
            cos_t, sin_t, D, F, eps, ctx, False,
        )
        d_io0 = bg0.img.d_x.copy(); d_to0 = bg0.txt.d_x.copy()
        var fwd1 = double_block_lora_forward_device_resident[H, Dh, N_IMG, N_TXT, S](
            saved.s1.dbl_img_in[di], saved.s1.dbl_txt_in[di],
            dbw[di], img_mod_dev1, txt_mod_dev1, bl, cos_t, sin_t, D, F, eps, ctx,
        )
        var bg1 = double_block_lora_backward_device_resident[H, Dh, N_IMG, N_TXT, S](
            d_io1, d_to1, dbw[di], img_mod_dev1, txt_mod_dev1, bl, fwd1.saved,
            cos_t, sin_t, D, F, eps, ctx, False,
        )
        d_io1 = bg1.img.d_x.copy(); d_to1 = bg1.txt.d_x.copy()
        var dbase = di * DBL_SLOTS
        dbl_d_a[dbase + 0] = _add_lists(bg0.img.q_d_a, bg1.img.q_d_a)
        dbl_d_b[dbase + 0] = _add_lists(bg0.img.q_d_b, bg1.img.q_d_b)
        dbl_d_a[dbase + 1] = _add_lists(bg0.img.k_d_a, bg1.img.k_d_a)
        dbl_d_b[dbase + 1] = _add_lists(bg0.img.k_d_b, bg1.img.k_d_b)
        dbl_d_a[dbase + 2] = _add_lists(bg0.img.v_d_a, bg1.img.v_d_a)
        dbl_d_b[dbase + 2] = _add_lists(bg0.img.v_d_b, bg1.img.v_d_b)
        dbl_d_a[dbase + 3] = _add_lists(bg0.img.out_d_a, bg1.img.out_d_a)
        dbl_d_b[dbase + 3] = _add_lists(bg0.img.out_d_b, bg1.img.out_d_b)
        dbl_d_a[dbase + 4] = _add_lists(bg0.img.ff_in_d_a, bg1.img.ff_in_d_a)
        dbl_d_b[dbase + 4] = _add_lists(bg0.img.ff_in_d_b, bg1.img.ff_in_d_b)
        dbl_d_a[dbase + 5] = _add_lists(bg0.img.ff_out_d_a, bg1.img.ff_out_d_a)
        dbl_d_b[dbase + 5] = _add_lists(bg0.img.ff_out_d_b, bg1.img.ff_out_d_b)
        dbl_d_a[dbase + 6] = _add_lists(bg0.txt.q_d_a, bg1.txt.q_d_a)
        dbl_d_b[dbase + 6] = _add_lists(bg0.txt.q_d_b, bg1.txt.q_d_b)
        dbl_d_a[dbase + 7] = _add_lists(bg0.txt.k_d_a, bg1.txt.k_d_a)
        dbl_d_b[dbase + 7] = _add_lists(bg0.txt.k_d_b, bg1.txt.k_d_b)
        dbl_d_a[dbase + 8] = _add_lists(bg0.txt.v_d_a, bg1.txt.v_d_a)
        dbl_d_b[dbase + 8] = _add_lists(bg0.txt.v_d_b, bg1.txt.v_d_b)
        dbl_d_a[dbase + 9] = _add_lists(bg0.txt.out_d_a, bg1.txt.out_d_a)
        dbl_d_b[dbase + 9] = _add_lists(bg0.txt.out_d_b, bg1.txt.out_d_b)
        dbl_d_a[dbase + 10] = _add_lists(bg0.txt.ff_in_d_a, bg1.txt.ff_in_d_a)
        dbl_d_b[dbase + 10] = _add_lists(bg0.txt.ff_in_d_b, bg1.txt.ff_in_d_b)
        dbl_d_a[dbase + 11] = _add_lists(bg0.txt.ff_out_d_a, bg1.txt.ff_out_d_a)
        dbl_d_b[dbase + 11] = _add_lists(bg0.txt.ff_out_d_b, bg1.txt.ff_out_d_b)
        di -= 1

    return KleinLoraGrads(
        dbl_d_a^, dbl_d_b^, sgl_d_a^, sgl_d_b^,
        List[Float32](), List[Float32](),
        _zeros(6 * D), _zeros(6 * D), _zeros(3 * D),
        List[Float32](), List[Float32](), List[Float32](),
        List[Float32](), List[Float32](),
    )


def klein_stack_lora_forward_device_inputs_offload_turbo_moddev_rope_scratch_b2[
    H: Int, Dh: Int, N_IMG: Int, N_TXT: Int, S: Int
](
    img_tokens_t0: TArc, txt_tokens_t0: TArc,
    img_tokens_t1: TArc, txt_tokens_t1: TArc,
    ...
    """Streaming/fp8-resident TRUE batch-2 forward (trainer path). Mirrors
    klein_stack_lora_forward_device_inputs_offload_turbo_moddev_rope_scratch,
    but streams each block ONCE and applies it to BOTH samples with their own
    modvecs. The loader await/prefetch/mark cadence is unchanged (one block in
    flight); only the per-block compute is doubled."""
    var num_double = lora.num_double
    var num_single = lora.num_single
    ...
    for bi in range(num_double):
        ...
        var w = _double_weights_from_block(handle.block, handle.prefix, ctx)
        var bl = _dbl_lora_dev_for(lora, bi)
        var fwd0 = double_block_lora_forward_device_resident_scratch[H, Dh, N_IMG, N_TXT, S](
            img0, txt0, w, img_mod_dev0, txt_mod_dev0, bl,
            cos_t, sin_t, D, F, eps, norm_ones[], norm_zeros[], ctx, scratch,
        )
        img0 = fwd0.img_out.copy(); txt0 = fwd0.txt_out.copy()
        var fwd1 = double_block_lora_forward_device_resident_scratch[H, Dh, N_IMG, N_TXT, S](
            img1, txt1, w, img_mod_dev1, txt_mod_dev1, bl,
            cos_t, sin_t, D, F, eps, norm_ones[], norm_zeros[], ctx, scratch,
        )
        img1 = fwd1.img_out.copy(); txt1 = fwd1.txt_out.copy()
        loader.mark_active_block_done(ctx)
    ...
    for bi in range(num_single):
        ...
        var f0 = single_block_lora_forward_device_resident_scratch[H, Dh, S](
            x0, w, single_mod_dev0, sl, cos_t, sin_t, D, F, eps,
            norm_ones[], norm_zeros[], ctx, scratch,
        )
        x0 = f0.out.copy()
        var f1 = single_block_lora_forward_device_resident_scratch[H, Dh, S](
            x1, w, single_mod_dev1, sl, cos_t, sin_t, D, F, eps,
            norm_ones[], norm_zeros[], ctx, scratch,
        )
        x1 = f1.out.copy()
        loader.mark_active_block_done(ctx)
...
def klein_stack_lora_backward_offload_turbo_moddev_rope_scratch_b2[
    H: Int, Dh: Int, N_IMG: Int, N_TXT: Int, S: Int
](
    d_out0: List[Float32], d_out1: List[Float32],
    base: KleinStackBase,
    ...
    """Streaming/fp8-resident TRUE batch-2 backward (trainer path). Mirrors
    klein_stack_lora_backward_offload_turbo_moddev_rope_scratch, interleaved:
    each block streamed ONCE, recompute+backward BOTH samples, SUM shared LoRA
    grads. Callers pass d_out{0,1} pre-scaled by 0.5 (== batch-mean gradient).
    Aux/input grads are not computed (frozen base; discarded by AdamW)."""
    var num_double = lora.num_double
    var num_single = lora.num_single
    var norm_ones = TArc(_t(_ones(D), [D], ctx))
    var norm_zeros = TArc(_t(_zeros(D), [D], ctx))
    ...
    while bi >= 0:
        ...
        var w = _single_weights_from_block(handle.block, handle.prefix, D, F, ctx)
        var sl = _sgl_lora_dev_for(lora, bi)
        var saved0 = single_block_lora_recompute_saved_device_resident_scratch[H, Dh, S](
            saved.s0.sgl_x_in[bi], w, single_mod_dev0, sl,
            cos_t, sin_t, D, F, eps, norm_ones[], norm_zeros[], ctx, scratch,
        )
        var bg0 = single_block_lora_backward_device_resident_scratch[H, Dh, S](
            d_x0, w, single_mod_dev0, sl, saved0, cos_t, sin_t,
            D, F, eps, norm_ones[], ctx, scratch, False,
        )
        d_x0 = bg0.d_x.copy()
        var saved1 = single_block_lora_recompute_saved_device_resident_scratch[H, Dh, S](
            saved.s1.sgl_x_in[bi], w, single_mod_dev1, sl,
            cos_t, sin_t, D, F, eps, norm_ones[], norm_zeros[], ctx, scratch,
        )
        var bg1 = single_block_lora_backward_device_resident_scratch[H, Dh, S](
            d_x1, w, single_mod_dev1, sl, saved1, cos_t, sin_t,
            D, F, eps, norm_ones[], ctx, scratch, False,
        )
        d_x1 = bg1.d_x.copy()
        var sbase = bi * SGL_SLOTS
    ...
    while di >= 0:
        ...
        var fwd0 = double_block_lora_forward_device_resident_scratch[H, Dh, N_IMG, N_TXT, S](
            saved.s0.dbl_img_in[di], saved.s0.dbl_txt_in[di],
            w, img_mod_dev0, txt_mod_dev0, bl, cos_t, sin_t, D, F, eps,
            norm_ones[], norm_zeros[], ctx, scratch,
        )
        var bg0 = double_block_lora_backward_device_resident_scratch[H, Dh, N_IMG, N_TXT, S](
            d_io0, d_to0, w, img_mod_dev0, txt_mod_dev0, bl, fwd0.saved,
            cos_t, sin_t, D, F, eps, norm_ones[], ctx, scratch, False,
        )
        d_io0 = bg0.img.d_x.copy(); d_to0 = bg0.txt.d_x.copy()
        var fwd1 = double_block_lora_forward_device_resident_scratch[H, Dh, N_IMG, N_TXT, S](
            saved.s1.dbl_img_in[di], saved.s1.dbl_txt_in[di],
            w, img_mod_dev1, txt_mod_dev1, bl, cos_t, sin_t, D, F, eps,
            norm_ones[], norm_zeros[], ctx, scratch,
        )
        var bg1 = double_block_lora_backward_device_resident_scratch[H, Dh, N_IMG, N_TXT, S](
            d_io1, d_to1, w, img_mod_dev1, txt_mod_dev1, bl, fwd1.saved,
            cos_t, sin_t, D, F, eps, norm_ones[], ctx, scratch, False,
        )
        d_io1 = bg1.img.d_x.copy(); d_to1 = bg1.txt.d_x.copy()
        var dbase = di * DBL_SLOTS
...
def klein_stack_lora_backward_offloaded_tape_turbo_moddev_rope_scratch[
    ...
    compute_input_grads: Bool = True,
    compute_aux_grads: Bool = True,
    # OPT-IN img-EDIT reference-conditioning backward — IDENTICAL to the primary
    # device-turbo backward hook. `ref_tokens_t` [N_IMG, in_ch] is a CONSTANT
    # resident carrier (NOT read from the tape); when supplied, d_img_in_ref
    # [D, in_ch] = linear_backward_dw(d_io, ref_tokens) is emitted into
    # KleinLoraGrads.d_img_in_ref. Absent (default None) => d_img_in_ref stays EMPTY
    # and the pass is byte-identical to the text-to-image tape backward. TRAINED-param
    # grad, independent of compute_input_grads (which gates FROZEN img_in/txt_in grads).
    ref_tokens_t: Optional[TArc] = None,
) raises -> KleinLoraGrads:
    ...
    # KLEIN_DEVICE_TAPE: read the parked device refs directly (no H2D restore).
    var ln_img_out_arc: TArc
    if tape.device_tape():
        ln_img_out_arc = tape.ln_img_out_dev[0].copy()
    else:
        ln_img_out_arc = TArc(restore_to_device(tape.ln_img_out, ctx))
    var mbf = modulate_backward(
        d_normed_t, ln_img_out_arc[],
        base.final_scale[], ctx, compute_aux_grads,
    )
    var d_final_scale = List[Float32]()
    var d_final_shift = List[Float32]()
    ...
    var img_out_arc: TArc
    if tape.device_tape():
        img_out_arc = tape.img_out_dev[0].copy()
    else:
        img_out_arc = TArc(restore_to_device(tape.img_out, ctx))
    var d_img_out_t = layer_norm_backward_dx(
        mbf.d_x, img_out_arc[],
        norm_ones[], eps, ctx,
    )
    ...
    while bi >= 0:
        ...
        var sl = _sgl_lora_dev_for(lora, bi)
        var block_saved: SingleBlockSaved
        if tape.has_saved_activations():
            # KLEIN_SAVE_ACTIVATIONS: reload the forward-saved activation set for
            # THIS block from host and consume it directly — no recompute. The
            # forward saved exactly what the int8 recompute branch below would
            # have produced (both `_forward_device_resident_scratch`), so this is
            # byte-identical to the recompute path.
            block_saved = restore_single_block_saved(tape.sgl_saved[bi], ctx)
        elif w.int8:
            # int8-resident: recompute via the int8-enabled full forward (the
            # recompute-only helper reads the bf16 w.w1, a [1,1] dummy on the
            # int8 path). bf16 path keeps the recompute-only helper unchanged.
            # KLEIN_DEVICE_TAPE: the block input is a parked device ref.
            var x_arc: TArc
            if tape.device_tape():
                x_arc = tape.sgl_x_in_dev[bi].copy()
            else:
                x_arc = TArc(restore_to_device(tape.sgl_x_in[bi], ctx))
            var rf = single_block_lora_forward_device_resident_scratch[H, Dh, S](
    ...
    while di >= 0:
        ...
        var dbl_block_saved: DoubleBlockSaved
        if tape.has_saved_activations():
            # KLEIN_SAVE_ACTIVATIONS: reload the forward-saved DoubleBlockSaved
            # (incl. flash q/k/v/o/stats) and consume it — no recompute. Both
            # forward and the recompute path call the SAME function
            # (`double_block_lora_forward_device_resident_scratch`), so the saved
            # set is byte-identical to what recompute would produce.
            dbl_block_saved = restore_double_block_saved(tape.dbl_saved[di], ctx)
        else:
            # KLEIN_DEVICE_TAPE: block inputs are parked device refs.
            var img_arc: TArc
            var txt_arc: TArc
            if tape.device_tape():
                img_arc = tape.dbl_img_in_dev[di].copy()
                txt_arc = tape.dbl_txt_in_dev[di].copy()
            else:
                img_arc = TArc(restore_to_device(tape.dbl_img_in[di], ctx))
                txt_arc = TArc(restore_to_device(tape.dbl_txt_in[di], ctx))
            var fwd = double_block_lora_forward_device_resident_scratch[H, Dh, N_IMG, N_TXT, S](
                img_arc, txt_arc,
                w, img_mod_dev, txt_mod_dev, bl, cos_t, sin_t, D, F, eps,
                norm_ones[], norm_zeros[], ctx, scratch,
    ...
    # ── OPT-IN reference-branch backward (img-EDIT) — IDENTICAL to the primary hook ──
    # d_io IS dL/d(img input-projection output) [N_IMG, D] (the summed projection's
    # grad, after all double-block backward). d_img_in_ref = d_ioᵀ @ ref_tokens
    # (linear_backward_dw, F32). ref_tokens is the CONSTANT carrier — not from the
    # tape. Empty when no ref supplied.
    var d_img_in_ref = List[Float32]()
    if Bool(ref_tokens_t):
        var d_io_f32 = cast_tensor(d_io[], STDtype.F32, ctx)
        var ref_f32 = cast_tensor(ref_tokens_t.value()[], STDtype.F32, ctx)
...
# ── KLEIN_RESIDENT_GRADS tape backward (2026-07-11) ──────────────────────────
# Same conductor loop, same tape restores, same block math as
# klein_stack_lora_backward_offloaded_tape_turbo_moddev_rope_scratch above —
# but every block backward is the `_devgrads` mirror, so the LoRA d_A/d_B stay
# on DEVICE (KleinLoraTensorGrads) and the per-block D2H syncs + the trainer's
# host grad-norm + the optimizer's ~80MB H2D grad re-upload all disappear.
# Aux/input grads are NOT wired (the product trainer call is aux-off) — fail
# loud rather than silently diverge.
def klein_stack_lora_backward_offloaded_tape_turbo_moddev_rope_scratch_devgrads[
    H: Int, Dh: Int, N_IMG: Int, N_TXT: Int, S: Int
](
    d_out: List[Float32],
    img_tokens: List[Float32], txt_tokens: List[Float32],
    ...
    # OPT-IN img-EDIT reference-conditioning backward (mirror of the host-grads tape
    # backward). `ref_tokens_t` is the CONSTANT resident carrier; when supplied,
    # d_img_in_ref [D, in_ch] is emitted into KleinLoraTensorGrads.d_img_in_ref as a
    # HOST list (a trained-param grad that must survive the resident-grads path).
    # Independent of the aux/input fail-loud guard below — this is neither an aux nor
    # a frozen-input grad. Absent (default None) => d_img_in_ref stays EMPTY.
    ref_tokens_t: Optional[TArc] = None,
) raises -> KleinLoraTensorGrads:
    if compute_input_grads or compute_aux_grads:
        raise Error(
            "klein_stack_lora_backward_offloaded_tape_..._devgrads: aux/input"
            " grads are not wired on the KLEIN_RESIDENT_GRADS path (the"
            " product trainer call is aux-off); set KLEIN_RESIDENT_GRADS=0"
            " for the host-grads tape backward"
        )
    var num_double = lora.num_double
    var num_single = lora.num_single
    var norm_ones = TArc(_t(_ones(D), [D], ctx))
    ...
    var sgl_a_opt = List[Optional[TArc]]()
    var sgl_b_opt = List[Optional[TArc]]()
    for _ in range(num_single * SGL_SLOTS):
        sgl_a_opt.append(Optional[TArc](None))
        sgl_b_opt.append(Optional[TArc](None))
    ...
    while bi >= 0:
        ...
        if tape.has_saved_activations():
            ...
            var rf = single_block_lora_forward_device_resident_scratch[H, Dh, S](
                ...
                cos_t, sin_t, D, F, eps, norm_ones[], norm_zeros[], ctx, scratch,
                int8=w.int8,
                recompute_only=True,   # tape-only: skip dead w2/LoRA-out/gate tail
            )
            block_saved = rf.saved.copy()
        else:
            var x_arc: TArc
        ...
        var bg = single_block_lora_backward_device_resident_scratch_devgrads[H, Dh, S](
            d_x, w, single_mod_dev, sl, block_saved, cos_t, sin_t,
            D, F, eps, norm_ones[], ctx, scratch, compute_aux_grads,
            int8=w.int8,
        )
        d_x = bg.d_x.copy()
        var sbase = bi * SGL_SLOTS
        sgl_a_opt[sbase + 0] = bg.qkv_d_a.copy()
        sgl_b_opt[sbase + 0] = bg.qkv_d_b.copy()
        sgl_a_opt[sbase + 1] = bg.out_d_a.copy()
        sgl_b_opt[sbase + 1] = bg.out_d_b.copy()
        loader.mark_active_block_done(ctx)
        bi -= 1
    ...
    var dbl_a_opt = List[Optional[TArc]]()
    var dbl_b_opt = List[Optional[TArc]]()
    for _ in range(num_double * DBL_SLOTS):
        dbl_a_opt.append(Optional[TArc](None))
        dbl_b_opt.append(Optional[TArc](None))
    ...
    while di >= 0:
        ...
        if tape.has_saved_activations():
            ...
            var fwd = double_block_lora_forward_device_resident_scratch[H, Dh, N_IMG, N_TXT, S](
                ...
                norm_ones[], norm_zeros[], ctx, scratch,
                int8=w.int8_payload(),
                recompute_only=True,   # tape-only: skip dead wd/ff_out/gate tail
            )
            dbl_block_saved = fwd.saved.copy()
        var bg = double_block_lora_backward_device_resident_scratch_devgrads[H, Dh, N_IMG, N_TXT, S](
            d_io, d_to, w, img_mod_dev, txt_mod_dev, bl, dbl_block_saved,
            cos_t, sin_t, D, F, eps, norm_ones[], ctx, scratch, compute_aux_grads,
            int8=w.int8_payload(),
        )
        ...
        d_to = bg.txt.d_x.copy()
        var dbase = di * DBL_SLOTS
        dbl_a_opt[dbase + 0] = bg.img.q_d_a.copy()
        dbl_b_opt[dbase + 0] = bg.img.q_d_b.copy()
        dbl_a_opt[dbase + 1] = bg.img.k_d_a.copy()
        dbl_b_opt[dbase + 1] = bg.img.k_d_b.copy()
        dbl_a_opt[dbase + 2] = bg.img.v_d_a.copy()
        dbl_b_opt[dbase + 2] = bg.img.v_d_b.copy()
        dbl_a_opt[dbase + 3] = bg.img.out_d_a.copy()
        dbl_b_opt[dbase + 3] = bg.img.out_d_b.copy()
        dbl_a_opt[dbase + 4] = bg.img.ff_in_d_a.copy()
        dbl_b_opt[dbase + 4] = bg.img.ff_in_d_b.copy()
        dbl_a_opt[dbase + 5] = bg.img.ff_out_d_a.copy()
        dbl_b_opt[dbase + 5] = bg.img.ff_out_d_b.copy()
        dbl_a_opt[dbase + 6] = bg.txt.q_d_a.copy()
        dbl_b_opt[dbase + 6] = bg.txt.q_d_b.copy()
        dbl_a_opt[dbase + 7] = bg.txt.k_d_a.copy()
        dbl_b_opt[dbase + 7] = bg.txt.k_d_b.copy()
        dbl_a_opt[dbase + 8] = bg.txt.v_d_a.copy()
        dbl_b_opt[dbase + 8] = bg.txt.v_d_b.copy()
        dbl_a_opt[dbase + 9] = bg.txt.out_d_a.copy()
        dbl_b_opt[dbase + 9] = bg.txt.out_d_b.copy()
        dbl_a_opt[dbase + 10] = bg.txt.ff_in_d_a.copy()
        dbl_b_opt[dbase + 10] = bg.txt.ff_in_d_b.copy()
        dbl_a_opt[dbase + 11] = bg.txt.ff_out_d_a.copy()
        dbl_b_opt[dbase + 11] = bg.txt.ff_out_d_b.copy()
        loader.mark_active_block_done(ctx)
        di -= 1

    # Fail-loud unwrap: every trained slot must have produced a device grad.
    var dbl_d_a = List[TArc]()
    var dbl_d_b = List[TArc]()
    for i in range(num_double * DBL_SLOTS):
        dbl_d_a.append(_required_tarc(dbl_a_opt[i], String("dbl d_a slot ") + String(i)))
        dbl_d_b.append(_required_tarc(dbl_b_opt[i], String("dbl d_b slot ") + String(i)))
    var sgl_d_a = List[TArc]()
    var sgl_d_b = List[TArc]()
    for i in range(num_single * SGL_SLOTS):
        sgl_d_a.append(_required_tarc(sgl_a_opt[i], String("sgl d_a slot ") + String(i)))
        sgl_d_b.append(_required_tarc(sgl_b_opt[i], String("sgl d_b slot ") + String(i)))

    # ── OPT-IN reference-branch backward (img-EDIT) — IDENTICAL to the host-grads
    # tape backward. d_io IS dL/d(img input-projection output) [N_IMG, D] after all
    # double-block backward. d_img_in_ref = d_ioᵀ @ ref_tokens (linear_backward_dw,
    # F32). Emitted to host so it survives even though block LoRA grads stay on
    # device. Empty when no ref supplied.
    var d_img_in_ref = List[Float32]()
    if Bool(ref_tokens_t):
        var d_io_f32 = cast_tensor(d_io[], STDtype.F32, ctx)
        var ref_f32 = cast_tensor(ref_tokens_t.value()[], STDtype.F32, ctx)
...
# Flat (pair-ordered) view of the device grads: for pair i the A tensor is at
# 2i and the B tensor at 2i+1, doubles first then singles — the SAME pair
# order as the trainer's host grad-norm/dead-adapter loops, so per-tensor
# sum-of-squares indexes map 1:1.
def klein_lora_tensor_grads_flat_pairs(
    tg: KleinLoraTensorGrads
) raises -> List[TArc]:
    if len(tg.dbl_d_a) != len(tg.dbl_d_b) or len(tg.sgl_d_a) != len(tg.sgl_d_b):
        raise Error("klein_lora_tensor_grads_flat_pairs: a/b length mismatch")
    var out = List[TArc]()
    for i in range(len(tg.dbl_d_a)):
        out.append(tg.dbl_d_a[i].copy())
        out.append(tg.dbl_d_b[i].copy())
    for i in range(len(tg.sgl_d_a)):
        out.append(tg.sgl_d_a[i].copy())
        out.append(tg.sgl_d_b[i].copy())
    return out^


def klein_lora_adamw_step_resident_devgrads(
    mut dbl_state: LoraAdamWSerenityDeviceState,
    mut sgl_state: LoraAdamWSerenityDeviceState,
    mut set: KleinLoraSet, grads: KleinLoraTensorGrads, t: Int, lr: Float32,
    ctx: DeviceContext,
    beta1: Float32 = Float32(0.9), beta2: Float32 = Float32(0.999),
    eps: Float32 = Float32(1.0e-8), weight_decay: Float32 = Float32(0.01),
) raises:
    """klein_lora_adamw_step_resident from DEVICE grad tensors — same scalars,
    same kernel, same SR stream (seed, intra-segment index); the grads are
    gathered into the resident dev_g on device instead of packed+uploaded."""
    if t < 1:
        raise Error("klein_lora_adamw_step_resident_devgrads: t must be >= 1")
    var b1p = Float32(1.0)
    var b2p = Float32(1.0)
    for _ in range(t):
        b1p *= beta1
...
def klein_lora_adamw_step(
    ...
    # training/lora_adamw_serenity_fused_parity.mojo — params bit-equal to the host
...
# ── per-block prefix scheme in SerenityTrainer target order ───────────────────────
# The generic PEFT writer appends .lora_A.weight / .lora_B.weight, but the prefix
# is the SerenityTrainer Flux2 LoRAModule target path. Keep this block-major order in
...
# ── SAVE every adapter as a SerenityTrainer raw LoRA safetensors ──────────────────
...
# SerenityTrainer parity and validation/sampler loading.
...
# ── TRUE-batch ROW-STACKED (b2rs) stack drivers ──────────────────────────────
# Rung B of the klein row-stacked batching (rung A = the gated
# single_block_*_batch pair, parity/klein_single_block_b2rs_parity.mojo).
# Doubles stay interleaved (8 of 32 blocks); the 24 singles run ONCE per pair
# over [2S, D] rows with [2, D] adaLN packs and REAL B=2 cuDNN flash. Drop-in
# for the interleaved _b2 pair (same signatures, same KleinStackForwardB2 /
# KleinLoraGrads types). TAPE LAYOUT NOTE: the STACKED per-block single inputs
# live in s0.sgl_x_in ([2S, D] each); s1.sgl_x_in is EMPTY — only the b2rs
# backward may consume this tape.
...
def _stack_vec2(a: Tensor, b: Tensor, D: Int, ctx: DeviceContext) raises -> TArc:
    """[D] + [D] -> [2, D] stacked per-sample vec (modulate/residual_gate
    row-block broadcast contract)."""
    var st = concat(0, ctx, a, b)
    reshape_in_place(st, [2, D])
    return TArc(st^)


def _stack_single_modvecs(
    mv0: SingleModVecsDevice, mv1: SingleModVecsDevice, D: Int, ctx: DeviceContext
) raises -> SingleModVecsDevice:
    return SingleModVecsDevice(
        _stack_vec2(mv0.shift[], mv1.shift[], D, ctx),
        _stack_vec2(mv0.scale[], mv1.scale[], D, ctx),
        _stack_vec2(mv0.gate[], mv1.gate[], D, ctx),
    )


def klein_stack_lora_forward_device_inputs_offload_turbo_moddev_rope_scratch_b2rs[
    H: Int, Dh: Int, N_IMG: Int, N_TXT: Int, S: Int
](
    img_tokens_t0: TArc, txt_tokens_t0: TArc,
    img_tokens_t1: TArc, txt_tokens_t1: TArc,
    ...
    # row-stack the pair; [2,D] adaLN packs; duplicated rope table (both
    # samples share positions at fixed resolution)
    var x_st = TArc(concat(0, ctx, x0[], x1[]))
    var mv_st = _stack_single_modvecs(single_mod_dev0, single_mod_dev1, D, ctx)
    var cos2 = concat(0, ctx, cos_t, cos_t)
    var sin2 = concat(0, ctx, sin_t, sin_t)

    var sgl_x_in_st = List[TArc]()
    for bi in range(num_single):
        sgl_x_in_st.append(x_st.copy())
        var block_idx = num_double + bi
        var handle = loader.await_block(block_idx, ctx)
        loader.prefetch_next_with_ctx(block_idx, ctx)
        var w = _single_weights_from_block(handle.block, handle.prefix, D, F, ctx)
        var sl = _sgl_lora_dev_for(lora, bi)
        var f = single_block_lora_forward_device_resident_scratch_batch[2, H, Dh, S](
            x_st, w, mv_st, sl, cos2, sin2, D, F, eps,
            norm_ones[], norm_zeros[], ctx, scratch,
        )
        x_st = f.out.copy()
        loader.mark_active_block_done(ctx)

    var x0f = TArc(slice(x_st[], 0, 0, S, ctx))
    var x1f = TArc(slice(x_st[], 0, S, S, ctx))

    var s0 = _klein_b2_final_forward[N_IMG, N_TXT](
        x0f, base, final_scale0, final_shift0,
        img_in_act0^, txt_in_act0^, dbl_img_in0^, dbl_txt_in0^, sgl_x_in_st^,
        D, out_ch, eps, ctx,
    )
    var s1 = _klein_b2_final_forward[N_IMG, N_TXT](
        x1f, base, final_scale1, final_shift1,
        img_in_act1^, txt_in_act1^, dbl_img_in1^, dbl_txt_in1^, List[TArc](),
        D, out_ch, eps, ctx,
    )
    return KleinStackForwardB2(s0^, s1^)


def klein_stack_lora_backward_offload_turbo_moddev_rope_scratch_b2rs[
    H: Int, Dh: Int, N_IMG: Int, N_TXT: Int, S: Int
](
    d_out0: List[Float32], d_out1: List[Float32],
    base: KleinStackBase,
    ...
    """Row-stacked TRUE batch-2 backward: singles recompute+backward ONCE per
    block over the stacked tape (LoRA grads pair-summed inside the GEMM);
    doubles interleaved as in the _b2 pair. Callers pass d_out{0,1} pre-scaled
    by 0.5 (== batch-mean gradient). Consumes ONLY a b2rs forward tape
    (stacked sgl_x_in in s0; s1.sgl_x_in empty)."""
    var num_double = lora.num_double
    var num_single = lora.num_single
    var norm_ones = TArc(_t(_ones(D), [D], ctx))
    var norm_zeros = TArc(_t(_zeros(D), [D], ctx))

    if len(saved.s1.sgl_x_in) != 0 or len(saved.s0.sgl_x_in) != num_single:
        raise Error(
            "klein b2rs backward: tape is not row-stacked (pair with the"
            " _b2rs forward — the interleaved _b2 tape is per-sample)"
        )
    ...
    var d_st = TArc(concat(0, ctx, d_x0[], d_x1[]))
    ...
    while bi >= 0:
        ...
        var w = _single_weights_from_block(handle.block, handle.prefix, D, F, ctx)
        var sl = _sgl_lora_dev_for(lora, bi)
        var rec_saved = single_block_lora_recompute_saved_device_resident_scratch_batch[2, H, Dh, S](
            saved.s0.sgl_x_in[bi], w, mv_st, sl,
            cos2, sin2, D, F, eps, norm_ones[], norm_zeros[], ctx, scratch,
        )
        var bg = single_block_lora_backward_device_resident_scratch_tensors_batch[2, H, Dh, S](
            d_st, w, mv_st, sl, rec_saved, cos2, sin2,
            D, F, eps, norm_ones[], ctx, scratch,
        )
        d_st = bg.d_x.copy()
        var sbase = bi * SGL_SLOTS
        sgl_d_a[sbase + 0] = _graph_grad_to_host(bg.qkv_d_a, "b2rs sgl qkv d_a", ctx)
        sgl_d_b[sbase + 0] = _graph_grad_to_host(bg.qkv_d_b, "b2rs sgl qkv d_b", ctx)
        sgl_d_a[sbase + 1] = _graph_grad_to_host(bg.out_d_a, "b2rs sgl out d_a", ctx)
        sgl_d_b[sbase + 1] = _graph_grad_to_host(bg.out_d_b, "b2rs sgl out d_b", ctx)
        loader.mark_active_block_done(ctx)
        bi -= 1

    var d_x0b = TArc(slice(d_st[], 0, 0, S, ctx))
    var d_x1b = TArc(slice(d_st[], 0, S, S, ctx))
    var d_to0 = TArc(slice(d_x0b[], 0, 0, N_TXT, ctx))
    var d_io0 = TArc(slice(d_x0b[], 0, N_TXT, N_IMG, ctx))
    var d_to1 = TArc(slice(d_x1b[], 0, 0, N_TXT, ctx))
    var d_io1 = TArc(slice(d_x1b[], 0, N_TXT, N_IMG, ctx))
