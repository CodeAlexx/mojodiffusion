# lingbot_i2v_mojo.mojo — FULLY PURE-MOJO image-to-video (no Python in the pipeline).
#
# End-to-end in one Mojo binary:
#   image file (png/jpeg/webp) --load_condition_image--> [1,3,1,H,W]
#   prompt/neg text files -------tokenize_lingbot_prompt (Qwen3Tokenizer)--> ids
#   ids ----------------encode_lingbot_text (Qwen3-VL)--> [1,L,2560]
#   image ---------------LingBotWanVaeEncoder.encode----> cond latent
#   -> lingbot_i2v_generate (quantized MoE, resident+GPU-AdaLN+flash)
#   -> LingBotWanVaeDecoder.decode_video -> [0,1] pixels -> save.
# The ONLY non-Mojo step left is the final mp4/gif CONTAINER encode (a utility,
# not the model pipeline).
#
# Two-phase (comptime attention size needs the prompt length): run once, it
# tokenizes and PRINTS the post-crop L_COND; set comptime L_COND to that and the
# run proceeds. Here L_COND=607 (the Bean dance prompt); it asserts at runtime.
#
# Run:
...
comptime PROMPT_TXT = "/home/alex/mojodiffusion/serenitymojo/models/lingbotvideo/parity/i2v_prompt.txt"
comptime NEG_TXT = "/home/alex/mojodiffusion/serenitymojo/models/lingbotvideo/parity/i2v_neg_prompt.txt"

comptime H = 576
comptime W = 320
comptime GT = 13          # 2s clip (change GT=31 for 5s once verified)
comptime GH = 36
comptime GW = 20
comptime LH = 72
comptime LW = 40
...
def main() raises:
    ...
    # ── (1) PURE-MOJO tokenize prompt + negative ─────────────────────────────
    print("[PURE-MOJO] tokenize (Qwen3Tokenizer)")
    var tok = Qwen3Tokenizer(String(TOK_JSON))
    var prompt_str = Path(String(PROMPT_TXT)).read_text()
    var neg_str = Path(String(NEG_TXT)).read_text()
    var pr = tokenize_lingbot_prompt(tok, prompt_str)
    var ng = tokenize_lingbot_prompt(tok, neg_str)
    var p_ids = pr[0].copy()
    var p_crop = pr[1]
    var n_ids = ng[0].copy()
    var n_crop = ng[1]
    var p_post = len(p_ids) - p_crop
    var n_post = len(n_ids) - n_crop
    print("[PURE-MOJO] L_COND(runtime) =", p_post, " L_UNCOND(runtime) =", n_post)
    if p_post != L_COND or n_post != L_UNCOND:
        raise Error(
            String("recompile with L_COND=") + String(p_post)
            + " L_UNCOND=" + String(n_post)
        )

    # ── (2) PURE-MOJO image decode + preprocess (png/jpeg/webp) ──────────────
    print("[PURE-MOJO] decode + preprocess image (", H, "x", W, ")")
    var image = load_condition_image(String(IMG_PATH), H, W, ctx)   # [1,3,1,H,W]

    # ── (3) Mojo Qwen3-VL text encode -> embeds (free 8GB before denoise) ────
    print("[PURE-MOJO] Qwen3-VL text encode")
    ...
    print("[PURE-MOJO] i2v denoise grid_t =", GT, " S_cond =", S_COND, " steps =", NUM_STEPS)
    ...
    print("[PURE-MOJO] temporal VAE decode")
    ...
    save_safetensors(names, tens, String(PARITY_DIR) + "/lingbot_i2v_mojo_pixels.safetensors", ctx)
    print("[PURE-MOJO] SAVED lingbot_i2v_mojo_pixels.safetensors — NO PYTHON in the pipeline")
