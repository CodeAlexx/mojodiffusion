# sdxl_lora_serenity_save_key_smoke.mojo -- SDXL SerenityTrainer save-key contract.
#
...
from serenitymojo.models.sdxl.sdxl_unet_stack_lora import (
    ...
    sdxl_lora_unsupported_serenity_trainer_targets,
)
from serenitymojo.training.lora_save import _read_f32


comptime OUT = "/tmp/sdxl_lora_serenity_save_key_smoke.safetensors"
...
def main() raises:
    ...
    _require(len(supported) == 10, String("expected 10 implemented ST linear reference trainer prefixes"))
    _require(
        supported[0]
        == String("lora_unet_down_blocks_1_attentions_0_transformer_blocks_0_attn1_to_q"),
        String("supported reference trainer prefix mapping mismatch"),
    )
    var unsupported = sdxl_lora_unsupported_serenity_trainer_targets()
    _require(len(unsupported) >= 15, String("expected explicit unsupported reference trainer targets"))
    ...
    print("[sdxl-lora-reference-save-key] PASS:", OUT)
