...
TARGETS: tuple[LoopTarget, ...] = (
    LoopTarget(
        key="klein",
        label="Klein",
        path=REPO / "trainer/src/serenity_trainer/trainer" / "train_klein_real.mojo",
        cache_patterns=(
            PatternSpec(
    ...
    LoopTarget(
        key="chroma",
        label="Chroma",
        path=REPO / "trainer/src/serenity_trainer/trainer" / "train_chroma_real.mojo",
        cache_patterns=(
            PatternSpec(
    ...
    LoopTarget(
        key="flux1",
        label="Flux.1",
        path=REPO / "trainer/src/serenity_trainer/trainer" / "train_flux_real.mojo",
        cache_patterns=(
            PatternSpec(
                "cache tensor loader returns host F32 compute list",
                re.compile(r"\b_load_cache_compute_f32\b"),
    ...
    LoopTarget(
        key="sd35",
        label="SD3.5",
        path=REPO / "trainer/src/serenity_trainer/trainer" / "train_sd35_real.mojo",
        cache_patterns=(
            PatternSpec(
    ...
    LoopTarget(
        key="zimage",
        label="Z-Image",
        path=REPO / "trainer/src/serenity_trainer/trainer" / "train_zimage_real.mojo",
        cache_patterns=(
            PatternSpec(
...
def sampler_checks(target: LoopTarget) -> list[Check]:
    ...
    if target.sampler_blocker_text in text:
        ...
        if not missing_harness:
            detail = (
                "fail-loud SerenityTrainer product sampler contract is present, but "
