...
This is intentionally no-CUDA. It inspects SerenityTrainer reference files and local
...
SERENITY_FLUX2_SAMPLER = SERENITY_TRAINER / "modules/modelSampler/Flux2Sampler.py"
SERENITY_FLUX2_DATALOADER = SERENITY_TRAINER / "modules/dataLoader/Flux2BaseDataLoader.py"
...
KLEIN_TRAIN = REPO / "trainer/src/serenity_trainer/trainer/train_klein_real.mojo"
...
PORT_STATUS = REPO / "SERENITY_MOJO_PORT_REMAINING.md"

KNOWN_BLOCKERS: tuple[str, ...] = (
    "No accepted token/template parity gate compares SerenityTrainer Qwen3 Klein chat text and token ids against the local Mojo template.",
    "Configured Klein sample cap-cache files are not proven present as BF16 [1,512,12288] or [512,12288] headers.",
    "No accepted seeded trajectory parity artifact exists yet; the sampler has an explicit initial-noise sidecar entry, but still needs matched SerenityTrainer post-patch/post-pack noise and latent trajectory evidence.",
    "No accepted numeric VAE/final-PNG parity gate compares inverse-BN, unpatchify, VAE decode, and image output against SerenityTrainer.",
    "No accepted Klein sampler speed/VRAM parity artifact records matched SerenityTrainer and Mojo prompt, seed, resolution, steps, CFG, dtype, trajectory, denoise seconds/step, VAE time, and peak VRAM.",
)
...
def gather_report() -> ContractReport:
    sources = {
        "serenity_model": read_source(SERENITY_FLUX2_MODEL),
        "serenity_sampler": read_source(SERENITY_FLUX2_SAMPLER),
        "serenity_dataloader": read_source(SERENITY_FLUX2_DATALOADER),
    ...
    facts.append(
        fact_for_needles(
            ...
            label="SerenityTrainer Klein conditioning contract",
            source=sources["serenity_model"],
            needles=(
            ...
            detail_ok="SerenityTrainer Klein uses Qwen3 chat formatting and hidden-state layers [9,18,27].",
            detail_missing="SerenityTrainer Flux2Model conditioning markers changed; inspect reference manually.",
        )
    )
    facts.append(
        fact_for_needles(
    ...
    facts.append(
        fact_for_needles(
            ...
            detail_ok="Default production sampling still uses Mojo BF16 randn, and the explicit parity entry can consume a supplied reference trainer-equivalent post-patch/post-pack initial-noise tensor without dtype casting.",
    ...
    facts.append(
        fact_for_needles(
            status_if_ok="PASS",
            status_if_missing="WARN",
            label="SerenityTrainer sampler RNG and decode path",
            source=sources["serenity_sampler"],
            needles=("torch.randn", "dtype=torch.float32", "noise_scheduler.set_timesteps", "self.model.unscale_latents", "vae.decode"),
            detail_ok="SerenityTrainer Flux2Sampler uses PyTorch F32 noise, scheduler timesteps, unscale/unpatchify, and VAE decode.",
            detail_missing="SerenityTrainer Flux2Sampler markers changed; inspect reference manually.",
        )
    )
    facts.append(
        fact_for_needles(
