...
def main() -> int:
    ...
    require_markers(
        SERENITY_FLUX2_MODEL,
        "SerenityTrainer Flux2Model Qwen3 chat contract",
        (
    ...
    require_markers(
        SERENITY_FLUX2_DATALOADER,
        "SerenityTrainer Flux2BaseDataLoader Qwen3 tokenization contract",
        (
    ...
    for label, prompt in load_sample_prompts(args.samples):
        ...
        serenity_text = hf_tok.apply_chat_template(
            messages,
            tokenize=False,
            add_generation_prompt=True,
            enable_thinking=False,
        )
        if serenity_text != expected_text:
            fail(f"{label}: local template differs from SerenityTrainer/Qwen3 chat template")
