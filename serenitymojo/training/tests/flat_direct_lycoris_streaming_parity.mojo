...
def _test_oft() raises:
    ...
    var y_ref = oft_serenity_forward(x.copy(), set.ad[0].vec.copy(), W.copy(), M, IN, OUT, B, R)
    _check(String("oft forward"), y, y_ref)

    var g = flat_direct_oft_backward_slot(set, 0, d_y.copy(), x.copy(), W.copy(), M)
    var g_ref = oft_serenity_backward(d_y.copy(), x.copy(), set.ad[0].vec.copy(), W.copy(), M, IN, OUT, B, R)
