...
# (AudioDecoder) as it appears in the official LTX-2.3 checkpoints. RefHQ uses
...
#
# Ground truth (read line-by-line and parity tested):
#   /home/alex/LTX-2 at pinned Creator revision 780984275fd47128b02bef9b5c085404276866ee
#   packages/ltx-core/src/ltx_core/model/audio_vae/{audio_vae,
#       causal_conv_2d,upsample,resnet,ops}.py
#
...
#
# === Creator-exact CausalConv2d ===
# The audio conv is 2D [B,Cin,H=T,W=F] with causality on HEIGHT (time) and a
# SYMMETRIC zero pad on WIDTH (freq). The BF16 product path explicitly pads both
# axes, converts NHWC to NCHW, and calls native cuDNN Conv2d with OIHW weights,
# matching Creator's PyTorch operation and pinned cuDNN 9.10.2 runtime. The F32
# diagnostic path retains the verified singleton-axis NDHWC Conv3d fallback.
#
# === Upsample ===
...
from serenitymojo.ops.unary import sqrt_op
...
from serenitymojo.ops.cudnn_conv2d import cudnn_conv2d_bf16_nchw
...
struct LTX2AudioVaeDecoderWeights(Movable):
    ...
    def _conv2d_w_oihw(self, name: String) raises -> Tensor:
        ref w = self._w(name)
        if len(w.shape()) != 4:
            raise Error(
                String("LTX2 AudioVAE: conv weight not rank-4 OIHW: ") + name
            )
        return Tensor(w.buf.copy(), w.shape(), w.dtype())
    ...
    # ── Creator-exact BF16 Conv2d; F32 singleton-Conv3d diagnostic fallback. ──
    def _causal_conv2d_named(
        ...
        if x.dtype() == STDtype.BF16:
            # Creator runs a native 4D NCHW Conv2d. Preserve that operation;
            # the singleton-axis Conv3d fallback is only for the F32 diagnostic
            # arm, where the native BF16 shim is intentionally inapplicable.
            # Creator's F.pad materializes BOTH the causal time padding and the
            # symmetric frequency padding before Conv2d(padding=0). cuDNN's
            # implicit frequency padding changes a handful of BF16 results.
            var x_native5 = Tensor(x5.buf.copy(), x5.shape(), x5.dtype())
            if pad_f > 0:
                var zf_shape = List[Int]()
                zf_shape.append(bsz)
                zf_shape.append(tt + pad_t)
                zf_shape.append(pad_f)
                zf_shape.append(1)
                zf_shape.append(cc)
                var zf_left = self._zeros(zf_shape.copy(), x.dtype(), ctx)
                var zf_right = self._zeros(zf_shape^, x.dtype(), ctx)
                x_native5 = concat(2, ctx, zf_left, x_native5, zf_right)
            var padded_nhwc_shape = List[Int]()
            padded_nhwc_shape.append(bsz)
            padded_nhwc_shape.append(tt + pad_t)
            padded_nhwc_shape.append(ff + 2 * pad_f)
            padded_nhwc_shape.append(cc)
            var padded_nhwc = reshape(x_native5, padded_nhwc_shape^, ctx)
            var to_nchw = List[Int]()
            to_nchw.append(0); to_nchw.append(3)
            to_nchw.append(1); to_nchw.append(2)
            var nchw = permute(padded_nhwc, to_nchw^, ctx)
            var w_oihw = self._conv2d_w_oihw(prefix + ".weight")
            var y_nchw = cudnn_conv2d_bf16_nchw(
                nchw, w_oihw^, b, 1, 1, 0, 0, ctx
            )
    ...
    def _pixel_norm(self, x: Tensor, ch: Int, ctx: DeviceContext) raises -> Tensor:
        # Creator's PixelNorm is an eager BF16 expression, not fused RMSNorm:
        #   mean_sq = mean(x**2, dim=channel, keepdim=True)
        #   rms = sqrt(mean_sq + eps)
        #   return x / rms
        # Preserve each BF16 storage boundary; keeping the chain in one F32
        # fused kernel changes the mel by a BF16 quantum that the deep vocoder
        # measurably amplifies. F32/NAVA callers keep the existing fused path.
        if x.dtype() == STDtype.BF16:
            var dims = List[Int]()
            dims.append(len(x.shape()) - 1)
            var x_sq = mul(x, x, ctx)
            var mean_sq = reduce_mean(x_sq, dims^, True, ctx)
            var rms = sqrt_op(add_scalar(mean_sq, PIXEL_NORM_EPS, ctx), ctx)
            return div(x, rms, ctx)
