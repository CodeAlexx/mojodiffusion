# tests/oft_serenity_trainer_device_smoke.mojo -- GPU SerenityTrainer-OFT block-size-4 gate.
#
# Build/run:
#   cd /home/alex/mojodiffusion && \
#   pixi run mojo build --optimization-level 2 -I . -Xlinker -lm -Xlinker -lcuda \
#     -Xlinker -L.pixi/envs/default/lib -Xlinker -lsqlite3 \
#     serenitymojo/training/tests/oft_serenity_trainer_device_smoke.mojo -o /tmp/oft_device_smoke \
...
from serenitymojo.training.oft_serenity_trainer import oft_serenity_forward, oft_serenity_backward
...
def main() raises:
    ...
    var rot = oft_serenity_rotate_b4(x_dev, vec_dev, ctx)
    var rot_h = rot.to_host(ctx)
    var rot_ref = oft_serenity_forward(x.copy(), vec.copy(), W.copy(), M, IN, IN, b, r)
    ...
    var grads = oft_serenity_rotate_backward_b4(dxr_dev, Tensor.from_host(x.copy(), [M, IN], STDtype.F32, ctx), vec_dev, ctx)
    var host_grads = oft_serenity_backward(d_x_rot.copy(), x.copy(), vec.copy(), W.copy(), M, IN, IN, b, r)
    ...
    var rot_bf16 = oft_serenity_rotate_b4(x_bf16, vec_bf16, ctx)
    ...
    var grads_bf16_vec = oft_serenity_rotate_backward_b4(
    ...
    print("PASS -- device SerenityTrainer-OFT block-size-4 forward/backward matches host oracle with F32 or BF16 vec storage")
