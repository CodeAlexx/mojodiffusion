# flux1_pipeline_smoke.mojo - FLUX.1 Dev end-to-end wiring smoke. Glues:
...
#
# This is a runtime-capable wiring/shape proof. Tokenization (CLIP BPE + T5
# SentencePiece) and parity are NOT yet wired; token ids are placeholders.
#
...
from serenitymojo.models.dit.flux1_contract import validate_flux1_pipeline_contract
...
def main() raises:
    ...
    if plan.latent_h != LATENT_H or plan.latent_w != LATENT_W:
        raise Error("FLUX.1 pipeline constants drifted from pack plan")
    if plan.image_tokens != N_IMG or plan.total_sequence != S:
        raise Error("FLUX.1 pipeline sequence constants drifted from pack plan")
    var ctx = DeviceContext()
    print("=== FLUX.1 Dev pipeline smoke ===", HEIGHT, "x", WIDTH, STEPS, "steps")
    print("[contract] FLUX.1 manifest/checkpoints OK")

    # 1. CLIP-L pooled [1,768] (encode_sdxl returns (last_hidden, pooled)).
    print("[clip] CLIP-L pooled")
    var clip_path = manifest.text_encoder_root + String("/clip_l.safetensors")
    ...
    var t5_path = manifest.text_encoder_root + String("/t5xxl_fp16.safetensors")
