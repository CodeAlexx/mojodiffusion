...
#   SerenityTrainer modules/util/loss/masked_loss.py:11-18 —
...
# they differ only on soft mask interiors — we port the clamp (SerenityTrainer/
...
def masked_case(elem_fn, mask, uw, norm):
    """SerenityTrainer masked_loss.py:11-18 (single flattened sample) followed by
...
def mae_elem(p):
    return torch.nn.functional.l1_loss(p, target, reduction="none")


CASES = {
    # pure SimpleTuner semantics: weights == mask (uw=0), no normalize
    "st_mse_soft": (mse_elem, mask_soft, 0.0, False),
    # SerenityTrainer lever cases (the UI keys)
    "serenity_mse_bin": (mse_elem, mask_bin, 0.1, False),
    "serenity_mse_bin_norm": (mse_elem, mask_bin, 0.1, True),
...
# levers-level composed case: serenity_huber_d01_bin_norm additionally scaled by the
...
out["levers_huber_snr_loss"] = out["serenity_huber_d01_bin_norm_loss"] * w_snr
out["levers_huber_snr_grad"] = out["serenity_huber_d01_bin_norm_grad"] * w_snr
