# tests/dora_serenity_trainer_parity.mojo — Mojo DoRA (wd_on_out=False, per-INPUT axis)
# vs SerenityTrainer's OWN DoRAModule.forward (MJ-1023 re-target).
#
# Oracle: /tmp/dora_serenity_oracle.safetensors from
#   python3 serenitymojo/training/tests/gen_dora_serenity_trainer_oracle.py
# (SerenityTrainer DoRAModule default decompose_output_axis=False, eps=0, alpha=2 rank=4).
#
# Build/run:
#   cd /home/alex/mojodiffusion && rm -f serenitymojo.mojopkg && \
#   pixi run mojo build --optimization-level 2 -I . -Xlinker -lm -Xlinker -lcuda \
#     -Xlinker -L.pixi/envs/default/lib -Xlinker -lsqlite3 \
#     serenitymojo/training/tests/dora_serenity_trainer_parity.mojo -o /tmp/dora_serenity_parity \
#   && /tmp/dora_serenity_parity
...
comptime ORACLE = "/tmp/dora_serenity_oracle.safetensors"
...
def main() raises:
    ...
    print("[dora SerenityTrainer parity] IN=", IN, " OUT=", OUT, " R=", R, " M=", M, " (per-INPUT axis)")
    ...
    var d = DoRAAdapter(
        ...
        _zeros(IN), _zeros(IN),
        False,                              # wd_on_out=False → per-INPUT (SerenityTrainer)
    )
    ...
    # ── BACKWARD vs SerenityTrainer autograd (detached norm) ──
    ...
    if fwd_ok and bwd_ok:
        print("PASS — Mojo DoRA (per-INPUT, wd_on_out=False) reproduces SerenityTrainer DoRAModule fwd + autograd bwd")
    else:
        raise Error("FAIL: Mojo per-input DoRA does not match SerenityTrainer (see cos/nrel above)")
