...
struct OffloadTelemetry(Movable):
    ...
    def __init__(
        ...
        self.name = name
        # Telemetry prints are OPT-IN via TURBO_TELEMETRY=1 (they spammed the
        # run console every summary_every blocks — 2026-07-11). Counters still
        # accumulate only when enabled; the env is the single switch.
        self.enabled = enabled and (env_int("TURBO_TELEMETRY", 0) != 0)
        self.trace = trace
