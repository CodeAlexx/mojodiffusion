...
#
# Design note (SerenityTrainer parity): SerenityTrainer's BaseConfig declares every field
...
# SerenityTrainer TrainingMethod string enum tags. Preserve baseline LoRA behavior
...
# SerenityTrainer GradientCheckpointingMethod string enum:
...
# SerenityTrainer DataType string enum. These are storage/request tags only; model
...
# SerenityTrainer Optimizer tags. Current real LoRA loops still execute AdamW; these
...
struct TrainConfig(Copyable, Movable):
    ...
    # -- SerenityTrainer training method selector --
    ...
    # ── SerenityTrainer runtime/control fields ──
    var batch_size: Int
    var epochs: Int
    ...
    # ── SerenityTrainer dtype policy tags ──
    var train_dtype: Int
    ...
    # ── SerenityTrainer model-part flags/names used by presets ──
    ...
    # ── SerenityTrainer layer/quantization selectors ──
    ...
    # ── SDXL per-text-encoder caption dropout (SerenityTrainer parity) ──
    # SerenityTrainer BaseStableDiffusionXLSetup.predict passes
    ...
    # objects' "dropout_probability" key. SerenityTrainer default = 0.0 (inert), so the
    ...
    # ── SerenityTrainer offload/checkpoint policy ──
    # Defaults mirror SerenityTrainer TrainConfig:
    ...
    # ── SerenityTrainer validation / sample / save / backup cadence ──
    var validation: Bool
    ...
    # ── SerenityTrainer masked/prior-preservation flags ──
    ...
    # fp8 DISK SIDECAR (default True). When quantized_resident=="fp8_e4m3", cache
    # the once-quantized E4M3-resident base to <ckpt>.fp8cache.safetensors and
    # reload it on later launches (~30s) instead of re-quantizing (~3-4 min). The
    # sidecar stores the EXACT quantizer bytes (bit-identical) and is weights-only
    ...
    var prefix_conditioning_p: Float32       # VideoExtend prefix; official tb=8 p=1.0
    var suffix_conditioning_p: Float32       # VideoExtend suffix
    var spatial_crop_conditioning_p: Float32 # Outpaint; video-only
    var temporal_boundary: Int               # prefix/suffix latent-frame count (default 8)
    var spatial_crop_y1: Int                 # Outpaint pixel rect (y1,x1,y2,x2)
    ...
    var audio_loss_balance_beta: Float32         # 0.01, (0,1]
    var audio_loss_balance_eps: Float32          # 0.05, >0
    var audio_loss_balance_min: Float32          # 0.05, >=0
    var audio_loss_balance_max: Float32          # 4.0, >0 & >=min
    var audio_loss_balance_ema_init: Float32     # 1.0 (inv_freq:(0,1]; else >0)
    var audio_loss_balance_target_ratio: Float32 # 0.33, >=0
    var audio_loss_balance_ema_decay: Float32    # 0.99, (0,1)
    ...
    var audio_batch_probability: Float32         # -1.0 sentinel = unset; [0,1] when set
    var use_audio_length_mask: Bool              # False
    var audio_ref_use_negative_positions: Bool   # False (audio_ref_only_ic)
    ...
    def default() -> TrainConfig:
        ...
        Mirrors SerenityTrainer BaseConfig.default_values."""
        return TrainConfig(
            name=String("unknown"),
            checkpoint=String(""),
            vae=String(""),
            ...
            save_every=500,
            sample_every=250,
            save_max_keep=0,   # keep all by default; trainer may apply its own default
        )
    ...
    def __init__(
        ...
        text_encoder_dropout_prob: Float32 = Float32(0.0),    # reference trainer SDXL TE1 dropout
        ...
        var reference_cache_dir: String = String(""),
        var val_reference_cache_dir: String = String(""),
        prefix_conditioning_p: Float32 = Float32(0.0),        # P5.5 intrinsic set — all default-off
        suffix_conditioning_p: Float32 = Float32(0.0),
        spatial_crop_conditioning_p: Float32 = Float32(0.0),
        temporal_boundary: Int = 8,
        spatial_crop_y1: Int = 0,
        spatial_crop_x1: Int = 0,
        spatial_crop_y2: Int = 0,
        ...
        audio_loss_balance_max: Float32 = Float32(4.0),
        audio_loss_balance_ema_init: Float32 = Float32(1.0),
        audio_loss_balance_target_ratio: Float32 = Float32(0.33),
        audio_loss_balance_ema_decay: Float32 = Float32(0.99),
        uncertainty_lr: Float32 = Float32(-1.0),
        video_caption_dropout_rate: Float32 = Float32(0.0),
        audio_caption_dropout_rate: Float32 = Float32(0.0),
        separate_audio_buckets: Bool = False,
        var audio_bucket_strategy: String = String(""),
        audio_bucket_interval: Float32 = Float32(0.0),
        min_audio_batches_per_accum: Int = 0,
        audio_batch_probability: Float32 = Float32(-1.0),
        use_audio_length_mask: Bool = False,
        ...
        self.val_reference_cache_dir = val_reference_cache_dir^
        self.prefix_conditioning_p = prefix_conditioning_p
        self.suffix_conditioning_p = suffix_conditioning_p
        self.spatial_crop_conditioning_p = spatial_crop_conditioning_p
        self.temporal_boundary = temporal_boundary
        self.spatial_crop_y1 = spatial_crop_y1
        self.spatial_crop_x1 = spatial_crop_x1
        self.spatial_crop_y2 = spatial_crop_y2
        self.spatial_crop_x2 = spatial_crop_x2
        self.mask_conditioning_p = mask_conditioning_p
        self.mask_cache_dir = mask_cache_dir^
        self.audio_loss_balance_mode = audio_loss_balance_mode^
        self.audio_loss_balance_beta = audio_loss_balance_beta
        self.audio_loss_balance_eps = audio_loss_balance_eps
        self.audio_loss_balance_min = audio_loss_balance_min
        self.audio_loss_balance_max = audio_loss_balance_max
        self.audio_loss_balance_ema_init = audio_loss_balance_ema_init
        self.audio_loss_balance_target_ratio = audio_loss_balance_target_ratio
        self.audio_loss_balance_ema_decay = audio_loss_balance_ema_decay
        self.uncertainty_lr = uncertainty_lr
        self.video_caption_dropout_rate = video_caption_dropout_rate
        self.audio_caption_dropout_rate = audio_caption_dropout_rate
        ...
        self.min_audio_batches_per_accum = min_audio_batches_per_accum
        self.audio_batch_probability = audio_batch_probability
        self.use_audio_length_mask = use_audio_length_mask
    ...
    def validate_serenity_trainer_policy_config(self) raises:
