...
preserve-region runtime slice and can apply the bounded `LanPaint_MaskBlend`
final decoded pixel blend, including Comfy/PyTorch `area` resize of the
base/original image for the `ImageScale(area)` -> `MaskBlend.image1` role. It is
not an arbitrary ComfyUI graph executor. As of 2026-06-16, the daemon delegates
workflow lowering to `serve/workflow_graph.mojo` instead of carrying a second
local executor implementation, and the static surface gate rejects reintroducing
that drift.
...
genparams passthrough forms. Ideogram-4 also accepts top-level `prompt_json`
from those workflow envelopes, serializing JSON object/array values into the
authored prompt string so structured captions and `bbox` arrays survive the
daemon boundary. For linked `workflow.nodes`/`workflow.edges` bodies, accepted
fields are resolved through the typed graph value store and then mapped into the
existing flat `JobParams`/`serenity.genparams.v1` product path. Arbitrary node
families, custom outputs, and advanced graph behaviors are not implemented.
...
Current LanPaint sampler support remains fenced: visual Comfy UI canvas graphs
lower selected LanPaint sampler/blend/mask-conversion nodes into path-backed
image/mask handles and canonical `lanpaint_*` fields, but full
`LanPaint_KSampler`/`LanPaint_SamplerCustomAdvanced` inner-loop runtime parity
is not accepted. The bounded implemented LanPaint slice is Z-Image final decoded
image blending for `LanPaint_MaskBlend`: load the image-space mask, max-pool it,
Gaussian smooth it with `sigma=(blend_overlap-1)/4`, then composite
`image1*(1-mask)+image2*mask`. For the LanPaint `ImageScale(area)` ->
`MaskBlend.image1` role, the base/original `init_image` is resized to the
decoded output size with Comfy/PyTorch `area` semantics instead of requiring the
file to be pre-scaled. Z-Image rejects LanPaint sampler-loop fields via
`reject_unsupported_lanpaint_sampler_params(...)`; backends without this split
still reject unsupported LanPaint metadata through
...
| Text encode | `CLIPTextEncode` consumes a CLIP handle and emits conditioning. Positive/negative prompts are link-driven, and prompt weights affect conditioning. Ideogram structured captions use JSON text with optional bounding boxes. | `CLIPTextEncode` now requires a typed CLIP link and emits CONDITIONING text. `KSampler` consumes positive and negative CONDITIONING by link, so prompt polarity is not inferred from node title. Prompt syntax metadata is recorded, but `conditioning_weights_applied=false`. Ideogram-4 top-level `prompt_json` string/object/array is serialized into `prompt` and `prompt_raw`, and structured caption-looking prompt strings bypass generic prompt syntax rewriting so JSON keys and `bbox` arrays remain intact. | Weighted spans are still not applied to model conditioning math. Advanced conditioning combine/region nodes are unsupported. Ideogram prompt-builder subgraphs are not executed; they require a top-level `prompt`, `prompt_raw`, or `prompt_json` override. | Positive and negative text encode nodes emit typed conditioning handles; prompt weighting either changes conditioning or is rejected/labelled per request. Ideogram JSON/bbox preservation is statically guarded and still needs fresh runtime PNG sidecar evidence for production acceptance. |
| Empty latent / batch | `EmptyLatentImage` emits a LATENT tensor with width, height, and batch size. Batch size produces multiple images or a batched denoise. | `EmptyLatentImage` emits a typed LATENT placeholder with width/height and requires `batch_size=1`. Product-level flat `images=N` is still accepted outside the latent tensor path and is serialized into indexed output jobs with seed offsets. `EmptyLatentImage.batch_size>1`, `EmptyFlux2LatentImage.batch_size>1`, and `RepeatLatentBatch` now fail before enqueue with HTTP 501. | True Comfy latent-batch execution inside one backend call is not implemented. | `images > 1` emits indexed artifacts/gallery rows through the product queue. Separate latent batch semantics must either use a real backend batch path or continue failing loud before enqueue. |
...
| Image load / resize / img2img | `LoadImage`, resize/crop nodes, `VAEEncode`, img2img, mask and inpaint nodes work as graph nodes. | Flat `init_image` and `creativity` are accepted. `LoadImage` now emits typed IMAGE and MASK handles; `ImageToMask`, `MaskToImage`, `GetImageSize`, `ImageScale`, and `ImageScaleToTotalPixels` lower as path-backed metadata operations with mask source metadata; `VAEEncode` converts IMAGE+VAE into a LATENT handle carrying `init_image`; `SetLatentNoiseMask` can attach a mask path/source to that LATENT; and `KSampler.denoise` maps to `creativity`. Z-Image validates the init path, decodes PNG/JPEG/WebP through MOJO-libs, resizes bilinear to the job size, VAE-encodes, starts denoise at the creativity sigma, and for `SetLatentNoiseMask` img2img applies preserve-region blending after each sampler update using `sigma_next`. The bounded Z-Image `LanPaint_MaskBlend` image1/base-image role can resize the original image to decoded output size with Comfy/PyTorch `area` semantics. Runtime smokes prove graph `LoadImage -> VAEEncode -> KSampler`, `LoadImage -> SetLatentNoiseMask -> KSampler`, and the LanPaint SDXL visual canvas metadata path reach PNG genparams. Qwen and Ideogram4 reject img2img. | Resize/crop and most mask conversion remain path/metadata passthrough; true graph-side image/mask tensor transforms are still absent. Graph img2img currently uses metadata handles to route into the existing backend `init_image` path, with only the bounded Z-Image preserve-mask runtime slice consuming `mask_image` and the bounded MaskBlend image1 role consuming area-resized base pixels. | Image nodes decode/resize into typed graph values, `VAEEncode` feeds sampler latents, bounded Z-Image preserve-mask behavior remains covered by helper/static gates, and unsupported backends reject before CUDA. |
| Mask / inpaint / LanPaint | `SetLatentNoiseMask`, `ImageToMask`, `MaskToImage`, LanPaint sampler nodes, mask blend, and outpaint padding alter denoise and final compositing. | `JobParams.mask_image`, explicit `lanpaint_*` fields, `/v1/generate` genparams, and worker IPC preserve mask and LanPaint metadata. `SetLatentNoiseMask(samples: LATENT, mask: MASK) -> LATENT` lowers mask path/source metadata for downstream samplers. Z-Image consumes that path for the bounded img2img preserve-mask slice using `decode_comfy_mask`, `resize_mask_bilinear`, and `load_comfy_latent_preserve_mask`; manifests record `mask_image`, `mask_channel`, `inpaint_mask_applied`, `inpaint_preserve_active_pixels`, and `inpaint_preserve_mean`. `resize_mask_nearest_exact`, `binarize_lanpaint_denoise_mask`, and `load_lanpaint_latent_preserve_mask` pin the LanPaint hard-mask preparation substrate for the later full LanPaint path. `smooth_lanpaint_blend_mask`, `load_lanpaint_pixel_blend_mask`, and `apply_lanpaint_mask_blend_signed_chw` pin the bounded final decoded image blend: max-pool, Gaussian smooth with `sigma=(blend_overlap-1)/4`, then `image1*(1-mask)+image2*mask`; for `ImageScale(area)` -> `MaskBlend.image1`, the base/original image is area-resized before that formula. `LanPaint_KSampler`, `LanPaint_KSamplerAdvanced`, `LanPaint_SamplerCustomAdvanced`, and `LanPaint_MaskBlend` are accepted graph-lowering nodes that copy sampler/blend widgets into canonical metadata; only Z-Image currently consumes `LanPaint_MaskBlend`, and only as the final decoded pixel blend after VAE decode. `serenitymojo/sampling/inpaint.mojo` has a mask-blend helper and one supplied-score LanPaint overdamped step with synthetic parity coverage. | Full LanPaint sampler inner-loop and outpaint semantics are not backend-executed. Comfy `ImageScale(area)` base-image resize is only accepted for the bounded Z-Image `LanPaint_MaskBlend.image1` role, not arbitrary graph-side IMAGE tensors. Qwen, Ideogram4, and Klein still reject `mask_image`; Z-Image still rejects unsupported LanPaint sampler-loop metadata. `ImagePadForOutpaint`, `ThresholdMask`, `LanPaint_SamplerCustom`, and model-specific edit text encoders remain unsupported/fail-loud. | Backend-specific LanPaint inner-loop/outpaint behavior and parity checks against the LanPaint/SerenityFlow Python oracle must pass before any real backend accepts LanPaint sampler runtime metadata. Until then, Z-Image fails loud through `reject_unsupported_lanpaint_sampler_params(...)`, other real backends fail loud through `reject_unsupported_lanpaint_params(...)`, and non-Z-Image mask runtime controls fail loud through `reject_unsupported_mask_image_params(...)`. |
...
| Failure semantics | Unsupported graph features fail before execution, name the unsupported feature, and never silently no-op. Bad requests are distinct from unsupported features. | Workflow parser raises `[501]` for unsupported graph shape, missing `type_id`, unsupported node type, and missing prompt node. `/v1/generate` preserves that HTTP status and now returns `serenity.generate.error.v1` with `rejection_stage:"workflow_lowering"`, `enqueue_blocked:true`, and the selected model's `capability_profile` when the request carries a top-level model. `/v1/preflight` preserves the graph HTTP status while returning `serenity.generate.preflight.v1` with the same lowering stage/profile data. Bad schema/type errors return 422. Raw disabled surfaces and post-parse prequeue failures now also return structured generate-error bodies before queue fanout. | Graph unsupported behavior is still HTTP-level validation, not full runtime graph validation. Model-specific runtime quality/parity can still fail after enqueue for admitted paths; unsupported settings should continue moving into the shared no-CUDA prequeue gate. | Static checker continues to gate supported markers and 501 sentinel handling; runtime smoke posts unsupported nodes and backend-incompatible options and verifies fail-loud structured responses/states. |

## Exact Current Blockers

- No arbitrary ComfyUI graph executor beyond the bounded typed linked image
  chain. The supported executor must stay owned by `serve/workflow_graph.mojo`;
  daemon-local duplicate executor helpers are a guarded regression.
- No tensor-backed graph MASK value: current MASK support is path/source metadata.
  ...
  preserve-mask slice. LanPaint sampler graph nodes lower metadata only; Z-Image
  additionally consumes `LanPaint_MaskBlend` only as a bounded final decoded
  pixel blend. No advanced typed graph values for CONTROL, UPSCALE_MODEL, IMAGE
  batches, refiner handles, or video frames.
- No separate CLIP/VAE loader override nodes.
- `Empty*LatentImage.batch_size` is accepted only as `1`; `batch_size>1` and
  `RepeatLatentBatch` fail loud before enqueue. Flat top-level `images=N` is
  the serial product fanout path, not true Comfy latent-batch execution.
...
- `LanPaint_KSampler`, `LanPaint_KSamplerAdvanced`,
  ...
  `MaskToImage` are accepted for graph lowering. The sampler nodes do not
  execute real LanPaint inpaint semantics in a backend; Z-Image
  `LanPaint_MaskBlend` is limited to final decoded pixel compositing, with
  area resize only for the base/original `MaskBlend.image1` role.
- Z-Image rejects unsupported LanPaint sampler-loop metadata through
  `reject_unsupported_lanpaint_sampler_params(...)`. Qwen, Ideogram4, and Klein
  reject unsupported `lanpaint_*` metadata through
  `reject_unsupported_lanpaint_params(...)`.
...
- Ideogram-4 structured JSON/bbox prompts are preserved at the Mojo
  daemon/workflow boundary and mirrored in the Rust server/`serenity-graph`
  boundary, but the 2026-06-16 update has only static/admission evidence; a new
  `/v1/generate` artifact must prove `prompt_json` round-trips into PNG metadata
  and the Ideogram result sidecar before it becomes runtime evidence.
...
img2img slice plus bounded Z-Image `LanPaint_MaskBlend` final decoded pixel
blend.
