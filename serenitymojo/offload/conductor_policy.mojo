...
#
# This module is scalar policy only. It mirrors SerenityTrainer's activation/layer
...
@fieldwise_init
struct SerenityTrainerConductorPolicy(Copyable, Movable, ImplicitlyCopyable):
...
def serenity_trainer_conductor_policy_from_fields(
...
) raises -> SerenityTrainerConductorPolicy:
    if total_layer_bytes < 0:
        raise Error("SerenityTrainerConductorPolicy: total_layer_bytes must be non-negative")
    if layer_offload_fraction < Float64(0.0) or layer_offload_fraction > Float64(1.0):
        raise Error("SerenityTrainerConductorPolicy: layer_offload_fraction must be within 0..1")
    ...
    # Mirrors SerenityTrainer LayerOffloadConductor:
    ...
    return SerenityTrainerConductorPolicy(
