...
def _check_streaming_block_init() raises:
    ...
    var yo_ref = oft_serenity_forward(x.copy(), oft.ad[slot_idx].vec.copy(), keep_w.copy(), M, dim, dim, oft_block, dim // oft_block)
    ...
    var og_ref = oft_serenity_backward(d_y.copy(), x.copy(), oft.ad[slot_idx].vec.copy(), keep_w.copy(), M, dim, dim, oft_block, dim // oft_block)
...
def _check_dora_projection(ctx: DeviceContext) raises:
    ...
    if rb.wd_on_out:
        raise Error("wan22 direct smoke: DoRA readback lost SerenityTrainer input-axis magnitude")
...
def _check_oft_projection(ctx: DeviceContext) raises:
    ...
    var y_ref = oft_serenity_forward(x.copy(), oft.ad[slot].vec.copy(), W.copy(), M, dim, dim, block_size, r)
    y_ref = _add_bias(y_ref^, bias.copy(), M, dim)
    _check(String("oft projection+bias"), y, y_ref)
    var g = wan22_direct_oft_projection_backward(oft, slot, d_y.copy(), x.copy(), W.copy(), M)
    var g_ref = oft_serenity_backward(d_y.copy(), x.copy(), oft.ad[slot].vec.copy(), W.copy(), M, dim, dim, block_size, r)
