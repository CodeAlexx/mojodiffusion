...
# SerenityTrainer DoRAModule (LoRAModule.py:473-571) and the DoRA paper §4.3 (NVIDIA
...
# standard LoRA zero leaf) — exactly SerenityTrainer/PEFT init behavior.
#
...
# the SerenityTrainer DoRAModule default decompose_output_axis=False). Returns [in] =
# sqrt(Σ_o W[o,i]²). MJ-1023: SerenityTrainer's default DoRA magnitude is per-input.
...
struct DoRAAdapter(Copyable, Movable):
    ...
    var wd_on_out: Bool           # True = per-OUTPUT m[out] (lycoris wd_on_out);
                                  # False = per-INPUT m[in] (SerenityTrainer default)
...
# (identity at init). Mirrors SerenityTrainer DoRAModule init + dora.rs.
...
def new_dora_adapter(
    ...
    # per-input [in] (SerenityTrainer default decompose_output_axis=False).
...
def dora_effective_weight(w_orig: List[Float32], d: DoRAAdapter) raises -> DoRAEff:
    ...
    # else (SerenityTrainer default): den[in]=‖WP[:,i]‖, m[in].
...
def dora_backward(d_y_h: List[Float32], x_h: List[Float32], w_orig: List[Float32], d: DoRAAdapter, M: Int) raises -> DoRAGrads:
    ...
    # Axis-aware: k = o (per-output m[out]) or i (per-input m[in], SerenityTrainer).
...
def dora_adamw(
    ...
    # Magnitude m: no weight decay (a norm scalar; reference trainer/PEFT do not decay it).
