...
# reference trainer trainer. Produces one cache file per image, each holding BOTH the trainer's
# keys:
#
#   latent       : F32 [1,16,64,64]    RAW Qwen-Image VAE mean latent (NO
#                  scale_latents — train_anima_serenity applies it; we cache the raw
#                  encoder mean, std ~1.0, the same thing anima_prepare_serenity512 caches)
...
# Full reference trainer text path -> FROZEN context [1,512,1024] (F32). Forward only.
...
def main() raises:
    ...
    print("==== giger3_prepare — PURE-MOJO cache for the Anima reference trainer trainer ====")
