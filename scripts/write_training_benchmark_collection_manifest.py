...
CASES: tuple[BenchmarkCollectionCase, ...] = (
    ...
    BenchmarkCollectionCase(
        model="zimage",
        correctness_lane="serenity_trainer",
        mojo_runner="trainer/src/serenity_trainer/trainer/train_zimage_real.mojo",
        target_batch=2,
        ...
        blocker=(
            ...
            "product-loop phase timings; needs SerenityTrainer batch-2/1024 parity "
            "and complete profiler counters; a strict Eri2 batch-2 SerenityTrainer "
            ...
            "with SerenityTrainer BF16 runtime/step boundary and live LoRA dump dtype "
    ...
    BenchmarkCollectionCase(
        model="klein",
        correctness_lane="serenity_trainer",
        mojo_runner="trainer/src/serenity_trainer/trainer/train_klein_real.mojo",
        target_batch=1,
        target_resolution="1024",
        dtype="BF16",
        ...
        blocker=(
            ...
            "SerenityTrainer 1024 parity, complete counters, and a "
...
def build_manifest() -> str:
    lines: list[str] = [
        ...
        "## Next-Model Blocker",
        "",
        "Klein now has a Mojo-current one-step 512px host-grad-compat scorecard artifact, but it is still not 1024px SerenityTrainer parity or a device-fast path. SDXL still lacks a shared Mojo scorecard artifact and must not be accepted as numeric grad/update or device-ABI evidence until BF16 backward grad-dtype unification and the ST-only versus full-UNet adapter-surface gap are fixed.",
        "",
        ...
        "",
        "Local no-GPU replay gates require `/home/alex/serenity_trainer-mojo/parity/*_train_ref_meta.json`; if those meta files are absent, the gate is blocked before model-level parity is evaluated.",
        "",
        "## Matrix Cases",
        "",
    ]
    for case in CASES:
        ...
        if case.model == "zimage":
            var_correctness_lane = (
                ...
                "SerenityTrainer update-bearing adapter oracle collected at "
                ...
                "layer-0 adapter metadata smoke collected with SerenityTrainer BF16 runtime/step boundary and live LoRA dump dtype recorded, while BF16 Mojo storage boundary remains required; "
