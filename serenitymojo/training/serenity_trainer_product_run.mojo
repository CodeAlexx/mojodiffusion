# training/serenity_trainer_product_run.mojo - SerenityTrainer train.py entrypoint contract.
#
...
# execution. This layer proves the SerenityTrainer entrypoint decisions and fails
...
from serenitymojo.training.serenity_trainer_lifecycle import (
    ...
    SerenityGenericTrainerLifecyclePlan,
    SerenityProductRunPlan,
    create_generic_trainer_lifecycle_plan,
    create_serenity_trainer_product_run_plan,
    generic_trainer_lifecycle_summary,
    serenity_trainer_product_run_summary,
    validate_generic_trainer_lifecycle_plan,
    validate_serenity_trainer_product_run_plan,
)
from serenitymojo.training.serenity_trainer_cache_preflight import (
    SerenityCachePreflightPlan,
    create_serenity_trainer_cache_preflight_plan,
    serenity_trainer_cache_preflight_summary,
...
from serenitymojo.training.serenity_trainer_preset_catalog import (
    SerenityPresetCatalogEntry,
    default_serenity_trainer_preset_entry,
    validate_serenity_trainer_preset_entry,
)
...
@fieldwise_init
struct SerenityProductResumePreflight(Copyable, Movable):
...
@fieldwise_init
struct SerenityTrainEntrypointPlan(Copyable, Movable):
    ...
    var product_plan: SerenityProductRunPlan
    var lifecycle_plan: SerenityGenericTrainerLifecyclePlan
    var cache_preflight: SerenityCachePreflightPlan
    var resume_preflight: SerenityProductResumePreflight
...
def _resume_surface_for_training_method(training_method: Int) raises -> Int:
    if training_method == TRAINING_METHOD_LORA:
        return SERENITY_RESUME_SURFACE_INTERNAL_BACKUP
    if training_method == TRAINING_METHOD_FINE_TUNE:
        return SERENITY_RESUME_SURFACE_FULL_FINETUNE_INTERNAL
    raise Error("SerenityTrainer product resume preflight: unsupported training method")
...
def create_serenity_trainer_product_resume_preflight(
    cfg: TrainConfig,
) raises -> SerenityProductResumePreflight:
    ...
    if not cfg.continue_last_backup:
        return SerenityProductResumePreflight(
            cfg.name.copy(),
            cfg.training_method,
            cfg.optimizer,
            resume_surface,
    ...
    var meta = serenity_internal_meta_path(last)
    var lora_state = serenity_internal_lora_state_path(last)
    var opt_state = serenity_internal_optimizer_state_path(last)
...
def validate_serenity_trainer_product_resume_preflight(
    plan: SerenityProductResumePreflight,
) raises:
    if plan.fail_loud_policy != String("validate_internal_backup_sidecars_before_product_resume"):
        raise Error("SerenityTrainer product resume preflight: fail-loud policy drift")
    ...
    if (
        ...
        raise Error("SerenityTrainer product resume preflight: unsupported training method")
    var manifest = SerenityResumeManifest(
...
def serenity_trainer_product_resume_preflight_summary(plan: SerenityProductResumePreflight) -> String:
    if not plan.continue_last_backup:
...
def create_serenity_trainer_train_entrypoint_plan(
    config_path: String, cfg: TrainConfig,
) raises -> SerenityTrainEntrypointPlan:
    if config_path == String("") or not path_exists(config_path):
        raise Error(String("SerenityTrainer product run: config path is missing: ") + config_path)

    var product_plan = create_serenity_trainer_product_run_plan(cfg)
    validate_serenity_trainer_product_run_plan(product_plan)
    ...
    var resume_preflight = create_serenity_trainer_product_resume_preflight(cfg)
    validate_serenity_trainer_product_resume_preflight(resume_preflight)

    var cadence = serenity_sample_cadence_from_train_config(config_path, cfg)
    ...
    return SerenityTrainEntrypointPlan(
        config_path.copy(),
        config_path.copy(),
        String(""),
        String(""),
...
def validate_serenity_trainer_train_entrypoint_plan(plan: SerenityTrainEntrypointPlan) raises:
    validate_serenity_trainer_product_run_plan(plan.product_plan)
    validate_generic_trainer_lifecycle_plan(plan.lifecycle_plan)
    validate_serenity_trainer_cache_preflight_plan(plan.cache_preflight)
    validate_serenity_trainer_product_resume_preflight(plan.resume_preflight)
    if plan.concept_file_required and not plan.concept_file_present:
        raise Error(
            String("SerenityTrainer product run: concept_file_name does not exist: ")
            + plan.product_plan.concept_file_name
        )
    if plan.sample_file_required and not plan.sample_file_present:
        raise Error(
            String("SerenityTrainer product run: sampling is enabled but sample file does not exist: ")
...
def _json_prefix_len_without_closing_object(data: List[UInt8]) raises -> Int:
    if len(data) == 0:
        raise Error("SerenityTrainer preset materializer: empty base config")
    var i = len(data) - 1
    while i >= 0:
        var ch = data[i]
        if _is_json_ws(ch):
            ...
            continue
        if ch != 0x7D:
            raise Error("SerenityTrainer preset materializer: base config is not a JSON object")
        return i
    raise Error("SerenityTrainer preset materializer: base config has no JSON object")
...
def _assert_json_safe_string(value: String, field: String) raises:
    ...
    for i in range(value.byte_length()):
        ...
        if ch < 0x20 or ch == 0x22 or ch == 0x5C:
            raise Error(
                String("SerenityTrainer preset materializer: ")
                + field
...
def _training_method_name(method: Int) raises -> String:
    ...
    raise Error("SerenityTrainer preset materializer: unsupported training method")
...
def _optimizer_name(optimizer: Int) raises -> String:
    ...
    raise Error("SerenityTrainer preset materializer: unsupported optimizer")
...
def _write_list_prefix_at(
    ...
    buf.free()
    if wrote != count:
        raise Error("SerenityTrainer preset materializer: short write while copying base config")
    return offset + wrote
...
def _write_string_at(fd: Int, content: String, offset: Int) raises -> Int:
    ...
    buf.free()
    if wrote != n:
        raise Error("SerenityTrainer preset materializer: short write while appending overrides")
    return offset + wrote


def _resolved_preset_config_path(entry: SerenityPresetCatalogEntry) -> String:
    return String("/tmp/mojo-reference-presets/") + entry.preset_id + String(".json")


def _materialized_preset_override_suffix(
    entry: SerenityPresetCatalogEntry,
    ...
    suffix += (
        String('"mojo_serenity_trainer_preset_id":')
        + _json_string(entry.preset_id, String("mojo_serenity_trainer_preset_id"))
        + String(",")
    )
    suffix += (
        String('"mojo_serenity_trainer_reference_path":')
        + _json_string(entry.serenity_reference_path, String("mojo_serenity_trainer_reference_path"))
        + String(",")
    )
    suffix += (
        String('"mojo_serenity_trainer_variant_kind":')
        + _json_string(entry.variant_kind, String("mojo_serenity_trainer_variant_kind"))
        + String(",")
    )
    suffix += (
        String('"mojo_serenity_trainer_vram_tier_gb":')
...
def materialize_serenity_trainer_preset_config(
    entry: SerenityPresetCatalogEntry,
    concept_file_name: String,
    sample_definition_file_name: String,
) raises -> String:
    var status = sys_system(String("mkdir -p /tmp/mojo-reference-presets"))
    if status != 0:
        raise Error("SerenityTrainer preset materializer: cannot create /tmp/mojo-reference-presets")
    ...
    if fd < 0:
        raise Error(String("SerenityTrainer preset materializer: cannot create ") + out_path)
...
def _apply_preset_entrypoint_overrides(
    ...
    if concept_file_name != String(""):
        ...
        raise Error(
            String("SerenityTrainer preset catalog: ")
            + entry.preset_id
...
def create_serenity_trainer_train_entrypoint_plan_from_preset(
    preset: String,
    concept_file_name: String,
    sample_definition_file_name: String,
) raises -> SerenityTrainEntrypointPlan:
    ...
    if not entry.product_run_wired:
        if entry.training_method == TRAINING_METHOD_FINE_TUNE:
            ...
            raise Error(
                String("SerenityTrainer preset catalog: full-finetune preset is cataloged but not product-wired: ")
                + entry.preset_id
                + String("; ")
                + full_finetune_product_run_blocker(full_ft_plan)
            )
        raise Error(
            String("SerenityTrainer preset catalog: preset is cataloged but not product-wired: ")
            + entry.preset_id
        )
    ...
    var resolved_config_path = materialize_serenity_trainer_preset_config(
        entry,
        cfg.concept_file_name,
        cfg.sample_definition_file_name,
    )
    var resolved_cfg = read_model_config(resolved_config_path)
    var plan = create_serenity_trainer_train_entrypoint_plan(resolved_config_path, resolved_cfg)
    plan.source_config_path = entry.config_path.copy()
    plan.named_preset_id = entry.preset_id.copy()
    plan.preset_reference_path = entry.serenity_reference_path.copy()
    plan.preset_reference_kind = entry.serenity_reference_kind.copy()
...
def serenity_trainer_train_entrypoint_summary(plan: SerenityTrainEntrypointPlan) -> String:
    var preset = String("")
    ...
    return (
        ...
        + preset
        + String(" -> ")
        + serenity_trainer_product_run_summary(plan.product_plan)
        + String(" ")
        ...
        + plan.sample_cadence.sample_definition_file_name
        + String(" ")
        + serenity_trainer_cache_preflight_summary(plan.cache_preflight)
        + String(" resume=")
        + serenity_trainer_product_resume_preflight_summary(plan.resume_preflight)
        + String(" workspace_config=")
        + plan.workspace_config_dir
    )


def serenity_trainer_train_runner_command(plan: SerenityTrainEntrypointPlan) -> String:
    """Dry-run command for the resolved Mojo runner.

    The command is intentionally informational. This product layer validates the
    SerenityTrainer entrypoint contract; it does not spawn model loops or touch CUDA.
    """
    return (
...
def validate_serenity_trainer_model_loop_direct_invocation_ready(
    plan: SerenityTrainEntrypointPlan,
) raises:
    validate_serenity_trainer_train_entrypoint_plan(plan)
    if plan.lifecycle_plan.invocation_kind == SERENITY_MODEL_LOOP_INVOCATION_ABSENT:
        raise Error(
            String("SerenityTrainer product run: direct model-loop invocation is absent; ")
            + plan.lifecycle_plan.invocation_blocker
            + String("; dry-run command only: ")
            + serenity_trainer_train_runner_command(plan)
        )
    validate_generic_trainer_lifecycle_plan(plan.lifecycle_plan)
