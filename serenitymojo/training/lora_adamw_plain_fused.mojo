...
# quantization). Sibling of lora_adamw_serenity_fused.mojo, which implements the
# SerenityTrainer bf16-moment+SR semantics Klein uses — do NOT mix them up: a
...
#   mi = beta1*m[i] + (1-beta1)*g            # classic form (reference trainer file uses the
...
#
# Data movement mirrors lora_adamw_serenity_fused (pack via sys_memcpy into pinned
...
# ── hi-res inline-sample offload (zimage free-then-render pattern; mirrors
# automagic3_device_state_offload_to_host) ────────────────────────────────────
# The resident AdamW state (dev_p bf16 + dev_g + dev_m/dev_v) does not co-fit
# with a 1024 inline render on a 24GB card alongside the fp8 base. Offload
# snapshots p/m/v to host, swaps the device buffers for 1-elt placeholders
# (frees the VRAM for the render), and restore rebuilds + re-uploads. dev_g is
# a per-step transient (fully overwritten by the streamed backward) — restore
# recreates it zero-filled instead of snapshotting.
struct LoraAdamWHostSnapshot(Movable):
    var p: HostBuffer[DType.uint8]
    var m: HostBuffer[DType.uint8]
    var v: HostBuffer[DType.uint8]
    ...
    def __init__(
        ...
        self.p = p^; self.m = m^; self.v = v^


def lora_adamw_plain_device_state_offload_to_host(
    mut state: LoraAdamWPlainDeviceState, ctx: DeviceContext
) raises -> LoraAdamWHostSnapshot:
    """D2H-snapshot p/m/v and free the device buffers. Only legal follow-ups:
    lora_adamw_plain_device_state_restore_from_host or drop."""
    var moment_bsz = _state_dtype_bytes(state.moment_dtype)
    var hp = ctx.enqueue_create_host_buffer[DType.uint8](state.total * 2)
    var hm = ctx.enqueue_create_host_buffer[DType.uint8](state.total * moment_bsz)
    var hv = ctx.enqueue_create_host_buffer[DType.uint8](state.total * moment_bsz)
    ctx.enqueue_copy(dst_buf=hp, src_buf=state.dev_p)
    ctx.enqueue_copy(dst_buf=hm, src_buf=state.dev_m)
    ctx.enqueue_copy(dst_buf=hv, src_buf=state.dev_v)
    ctx.synchronize()
    state.dev_p = ctx.enqueue_create_buffer[DType.uint8](1)
    state.dev_g = ctx.enqueue_create_buffer[DType.uint8](1)
    state.dev_m = ctx.enqueue_create_buffer[DType.uint8](1)
    state.dev_v = ctx.enqueue_create_buffer[DType.uint8](1)
    ctx.synchronize()
    return LoraAdamWHostSnapshot(hp^, hm^, hv^)


def lora_adamw_plain_device_state_restore_from_host(
    mut state: LoraAdamWPlainDeviceState,
    var snap: LoraAdamWHostSnapshot,
    ctx: DeviceContext,
) raises:
    """Rebuild the device buffers from an offload snapshot (inverse of offload).
    Rebuild the model's resident LoRA views afterwards — they alias dev_p."""
    var grad_bsz = _state_dtype_bytes(state.grad_dtype)
    var moment_bsz = _state_dtype_bytes(state.moment_dtype)
    state.dev_p = ctx.enqueue_create_buffer[DType.uint8](state.total * 2)
    state.dev_g = ctx.enqueue_create_buffer[DType.uint8](state.total * grad_bsz)
    state.dev_m = ctx.enqueue_create_buffer[DType.uint8](state.total * moment_bsz)
    state.dev_v = ctx.enqueue_create_buffer[DType.uint8](state.total * moment_bsz)
    ctx.enqueue_copy(dst_buf=state.dev_p, src_buf=snap.p)
    ctx.enqueue_copy(dst_buf=state.dev_m, src_buf=snap.m)
    ctx.enqueue_copy(dst_buf=state.dev_v, src_buf=snap.v)
    state.dev_g.enqueue_fill(UInt8(0))
    ctx.synchronize()
