# klein9b_prepare_alina.mojo — Klein-9B prepare driver for the Alina set.
#
# 9B twin of klein_prepare_alina.mojo (which, despite one stale print, is the
# 4B pipeline: Qwen3Config.klein_9b(), 7680-dim). This variant reads the
# sample_N.{safetensors,txt} names training/klein_stage_images.py actually
# writes, encodes with Qwen3-8B at the klein_9b config (12288-dim), and
# writes the cache where train_klein_real.mojo's comptime CACHE_DIR points
# (output/klein_train/cache/data). Built 2026-08-13 to rebuild the 3-step
# anchor cache after the original was deleted for H3 disk space.
#
...
comptime STAGE_DIR = "/home/alex/mojodiffusion/output/alina9b_stage"
comptime CACHE_DIR = "/home/alex/mojodiffusion/output/klein_train/cache/data"
comptime IH = 512
comptime IW = 512
comptime SEQ = 512
...
def main() raises:
    ...
    var qenc = Qwen3Encoder.load(QWEN8_DIR, Qwen3Config.klein_9b(), ctx)
