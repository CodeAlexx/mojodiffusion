...
#
# This consumes the real SerenityTrainer step0 and adapter dumps, verifies the selected
...
def _require_adapter_matrix(
    ...
    _require(key in st.tensors, String("missing reference trainer adapter tensor ") + key)
    var info = st.tensor_info(key)
...
def _adapter_from_dump_layer(
    ...
    # The SerenityTrainer dump stores live LoRA tensors as F32 host comparison data.
...
def _check_meta_json() raises:
    ...
    _require(meta.byte_length() > 0, String("empty ZImage SerenityTrainer metadata JSON"))
...
def main() raises:
    ...
    print(
        "[zimage-selected-grad-replay] exact_serenity_geometry img_rows=",
        IMG_ROWS,
        " cap_valid=(",
        VALID_CAP0,
        ",",
