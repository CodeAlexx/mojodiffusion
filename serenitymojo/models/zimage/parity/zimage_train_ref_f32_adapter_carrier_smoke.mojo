...
# SerenityTrainer's live LoRA params, not authority for Mojo device storage.
...
def main() raises:
    ...
    print(
        "[zimage-adapter-oracle-metadata] scope=SerenityTrainer BF16 runtime/step boundary plus live LoRA dump metadata; no device upload; adapter dump dtype is not Mojo storage authority"
    )
