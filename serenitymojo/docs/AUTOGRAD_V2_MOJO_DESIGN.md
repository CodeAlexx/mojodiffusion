...
| two-cursor ring | serenitymojo/scratch_ring.mojo | reference trainer StaticLayerAllocator shape (fwd cursor + reverse cursor, sub-buffer views) — the step-slab primitive ALREADY EXISTS; v2 makes it the engine's only steady-state allocator. |
...
| resident-set states | training/lora_adamw_{plain,reference}_fused.mojo | persistent device P/M/V (06-11). dev_p is the live param buffer the engine's AccumulateGrad/optimizer integration targets. |
...
reference trainer-resident optimizer already in. Variance-class gates + step time.
