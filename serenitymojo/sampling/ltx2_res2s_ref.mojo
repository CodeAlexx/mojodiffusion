...
def res2s_ref_loop[
    ...
    var n_steps = sched.n_full_steps
    ...
    for step in range(n_steps):
        ...
        if dump_evals:
            dump_names.append(dump_prefix + "_e" + _pad2(step) + "_d1_video")
            dump_tensors.append(ArcPointer[Tensor](den_v1.clone(ctx)))
            dump_names.append(dump_prefix + "_e" + _pad2(step) + "_d1_audio")
            dump_tensors.append(ArcPointer[Tensor](den_a1.clone(ctx)))
        ...
        if dump_evals:
            dump_names.append(dump_prefix + "_e" + _pad2(step) + "_d2_video")
            dump_tensors.append(ArcPointer[Tensor](den_v2.clone(ctx)))
            dump_names.append(dump_prefix + "_e" + _pad2(step) + "_d2_audio")
            dump_tensors.append(ArcPointer[Tensor](den_a2.clone(ctx)))
    ...
    if sched.final_denoise and n_steps == sched.n_full_steps:
