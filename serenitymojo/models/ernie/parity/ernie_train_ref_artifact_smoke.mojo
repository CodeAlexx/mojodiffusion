# ernie_train_ref_artifact_smoke.mojo -- Ernie SerenityTrainer one-step artifact gate.
#
# Header and scalar-payload, no-CUDA gate. It opens the real local SerenityTrainer
...
comptime STEP_DUMP = "/home/alex/serenity_trainer-mojo/parity/ernie_train_ref_step000.safetensors"
comptime ADAPTER_DUMP = "/home/alex/serenity_trainer-mojo/parity/ernie_train_ref_step000_adapters.safetensors"
comptime META_JSON = "/home/alex/serenity_trainer-mojo/parity/ernie_train_ref_meta.json"
...
def _require_contains(text: String, needle: String, label: String) raises:
    _require(text.find(needle) >= 0, String("missing Ernie reference trainer meta field: ") + label)
...
def _require_tensor(
    ...
    _require(key in st.tensors, String("missing Ernie reference trainer tensor ") + key)
    var info = st.tensor_info(key)
    _require(
        info.dtype == dtype,
...
def _check_meta_json() raises:
    ...
    _require(meta.byte_length() > 0, String("empty Ernie SerenityTrainer metadata JSON"))
...
def main() raises:
    ...
    # no-CUDA artifact smoke consumes the zero-lr SerenityTrainer dumps and metadata;
