# LTX-2 / LTX-2.3 Prompting Guide (distilled)

- **Current LTX-2.3 source**: https://ltx.io/blog/ltx-2-3-prompt-guide
- **Legacy LTX-2 source**: https://ltx.io/blog/prompting-guide-for-ltx-2
...
- Content below is a faithful distillation of the official LTX guides; nothing here is
  invented. Where the guides are silent (negative prompts, parameter values), that is
  stated explicitly. Re-check the current LTX-2.3 source when prompt behavior changes.

## LTX-2.3-specific guidance

- The redesigned text connector makes prompt detail more influential, including facial
  action, timing, pauses, and other physical performance cues.
- Longer, descriptive prompts are preferred for 8–10 second generations. Match prompt
  detail to duration so the model has enough directed action to fill the shot.
- Break dialogue into short quoted phrases and place visible acting or timing directions
  between them instead of supplying one long uninterrupted speech.
- Give synchronized audio its own concrete direction: room tone, ambient effects, music,
  voice character, and changes that occur during the shot.
- Keep the final prompt as one flowing, present-tense paragraph even when it describes
  several sequential beats.
...
- Usually use **4–8 descriptive sentences** covering all key aspects; for an 8–10 second
  LTX-2.3 shot, favor the detailed end of that range and describe a clear temporal arc.
