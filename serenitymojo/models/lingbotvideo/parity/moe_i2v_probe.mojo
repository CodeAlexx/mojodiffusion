# models/lingbotvideo/parity/moe_i2v_probe.mojo — MoE-30B IMAGE-TO-VIDEO.
#
# Latent-seed i2v: VAE-ENCODE a condition image -> seed video-latent frame 0 ->
# denoise (frame 0 re-injected every step) -> temporal VAE decode -> clip.
# Uses the new LingBotWanVaeEncoder + lingbot_i2v_generate. No DiT changes (i2v is
# pure latent inpainting; in==out==16 channels). Condition = frame 0 of the t2v
# robot clip (320x576, matches geometry + the reused robot embeds), so the i2v
# output's frame 0 should reproduce the condition — a built-in correctness check.
#
# Run WITH cshim flags (S~9951 >= flash threshold):
...
#     serenitymojo/models/lingbotvideo/parity/moe_i2v_probe.mojo
...
def main() raises:
    ...
    print("[MoE-I2V] loading condition image + reused robot embeds")
    var cimg = ShardedSafeTensors.open(String(PARITY_DIR) + "/i2v_condition_image.safetensors")
    var image = _load_f32(cimg, "image", ctx)   # [1,3,1,H,W] in [0,1]
    var demb = ShardedSafeTensors.open(String(PARITY_DIR) + "/oracle_dense_t2v.safetensors")
    var prompt_embeds = _load_f32(demb, "prompt_embeds", ctx)
    var neg_embeds = _load_f32(demb, "neg_embeds", ctx)

    print("[MoE-I2V] VAE-encode condition image -> cond latent")
    var enc = LingBotWanVaeEncoder[H, W].load(VAE_FILE, ctx)
    var cond_latent = enc.encode(image, ctx)    # [1,16,1,LH,LW] (normalized)
    ...
    print("[MoE-I2V] i2v denoise grid_t =", GT, " S_cond =", S_COND, " steps =", NUM_STEPS,
          " resident =", n_res)
    ...
    save_safetensors(names, tens, String(PARITY_DIR) + "/moe_i2v_pixels.safetensors", ctx)
    print("[MoE-I2V] SAVED moe_i2v_pixels.safetensors [0,1]")
