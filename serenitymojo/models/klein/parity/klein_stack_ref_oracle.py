...
#
# Torch oracle for the Klein FULL DiT STACK WITH the OPT-IN img-EDIT REFERENCE
# branch (models/klein/img_in_ref.mojo integrated into klein_stack.mojo). It is
# klein_stack_oracle.py + the additive reference input projection:
#     img = linear(img_tokens, img_in) + linear(ref_tokens, img_in_ref)
# with a NON-ZERO img_in_ref and a NON-DEGENERATE ref_tokens input. Emits the
# load-bearing stack input-token grads (which now carry the ref term through the
# WHOLE backward) AND d_img_in_ref (the ONLY new trained param's grad), via
# torch.autograd. Same block math as the proven small-dim stack oracle so the
# per-block numerics stay byte-matched to Mojo; the NEW surface is the ref proj.
#
# Oracle interpreter (torch is available):
...
# Namespace every artifact so this ref oracle never clobbers the base
# klein_stack_oracle.py .bin files sharing REF_DIR.
PFX = "refstk_"


def W(name, tensor):
    flat = tensor.detach().reshape(-1).to(torch.float32).numpy()
    path = os.path.join(REF_DIR, PFX + name + ".bin")
...
def main():
    ...
    # ── img-EDIT reference branch: NON-DEGENERATE ref tokens + NON-ZERO img_in_ref
    # ref_tokens shares noise_tokens' [N_IMG, IN_CH] shape (img_in_ref.mojo). The
    # weight is NON-ZERO here (the zero-init flags-off identity is a SEPARATE Mojo
    # in-composition check; this oracle exercises the trained, non-zero regime).
    ref_tokens = t2(N_IMG, IN_CH, 0.037, 0.11, 0.5).requires_grad_(True)
    img_in_ref = t2(D, IN_CH, 0.029, 0.19, 0.3).requires_grad_(True)   # [D, in_ch]
    ...
    # ── forward (WITH the additive reference projection) ──
    img = img_tokens @ img_in.T + ref_tokens @ img_in_ref.T   # [N_IMG, D]
    ...
    W("ref_out", out)
    # load-bearing input-token grads (NOW carry the ref term through the stack)
    W("ref_d_img_tokens", img_tokens.grad)
    W("ref_d_txt_tokens", txt_tokens.grad)
    # THE NEW trained param grad (the load-bearing img-EDIT deliverable)
    W("ref_d_img_in_ref", img_in_ref.grad)
    # self-contained extra: grad wrt the reference tokens
    W("ref_d_ref_tokens", ref_tokens.grad)
    ...
    W("in_ref_tokens", ref_tokens); W("in_img_in_ref", img_in_ref)
