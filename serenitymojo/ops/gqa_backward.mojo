# ops/gqa_backward.mojo — BACKWARD for GQA repeat_kv (grouped-sum). F32.
#
...
# grad-routing inverse of the head-broadcast; F32 interior, loud-fail on shape.
#
...
def _repeat_kv_fwd_kernel_f32(
    src: LayoutTensor[DType.float32, _DYN1, MutAnyOrigin],   # [S*Hkv*Dh]
    dst: LayoutTensor[DType.float32, _DYN1, MutAnyOrigin],   # [S*H*Dh]
...
def repeat_kv_f32(
    ...
    """GQA repeat_kv forward, F32. x [1,S,h_kv,Dh] -> [1,S,h_kv*n_rep,Dh]."""
    if x.dtype() != STDtype.F32:
        raise Error("repeat_kv_f32: F32 only")
    if n_rep == 1:
    ...
    ctx.enqueue_function[_repeat_kv_fwd_kernel_f32, _repeat_kv_fwd_kernel_f32](
...
def _repeat_kv_bwd_kernel_f32(
    d_dst: LayoutTensor[DType.float32, _DYN1, MutAnyOrigin],   # [S*H*Dh]
    d_src: LayoutTensor[DType.float32, _DYN1, MutAnyOrigin],   # [S*Hkv*Dh]
    ...
    if idx < total:
        ...
        for r in range(n_rep):
            ...
            acc += rebind[Scalar[DType.float32]](d_dst[src_idx])
        d_src[idx] = rebind[d_src.element_type](acc)


def repeat_kv_backward(
    d_dst: Tensor, s: Int, h_kv: Int, n_rep: Int, dh: Int, ctx: DeviceContext
) raises -> Tensor:
    """Backward of GQA repeat_kv. d_dst [1,S,H,Dh] (H = h_kv*n_rep, F32).
    Returns d_src [1,S,h_kv,Dh] = grouped sum over the n_rep repeated heads."""
    if d_dst.dtype() != STDtype.F32:
        raise Error("repeat_kv_backward: F32 only")
    if n_rep == 1:
    ...
    var out_buf = ctx.enqueue_create_buffer[DType.uint8](src_n * 4)
    var src_rl = RuntimeLayout[_DYN1].row_major(IndexList[1](src_n))
    ...
    ctx.enqueue_function[_repeat_kv_bwd_kernel_f32, _repeat_kv_bwd_kernel_f32](
