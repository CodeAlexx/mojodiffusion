...
# Same F16/F32/BF16 -> F32 -> out_dtype handling as _load_conv3d_qrscf, but keeps
# the on-disk FCQRS [Cout,Cin,Q,R,S] layout (identity, NO transpose) — the layout
# conv3d_fcqrs_cudnn (cuDNN fast path) expects. Using this instead of a raw
# from_view cast keeps the exact F16->F32->cdtype value path the naive loader uses.
def _load_conv3d_fcqrs[
    mut: Bool, //, origin: Origin[mut=mut]
](tv: TensorView[origin], out_dtype: STDtype, ctx: DeviceContext) raises -> Tensor:
    var sh = tv.shape.copy()
    if len(sh) != 5:
        raise Error("Wan22 VAE conv3d fcqrs loader expected rank-5 OIDHW")
    var n = sh[0] * sh[1] * sh[2] * sh[3] * sh[4]
    var host_in = _copy_view_bytes(tv, ctx)
    var f32_buf = List[Float32]()
    f32_buf.resize(n, Float32(0.0))
    if tv.dtype == STDtype.F32:
        ...
        var bp = host_in.unsafe_ptr().bitcast[BFloat16]()
        for i in range(n):
            f32_buf[i] = bp[i].cast[DType.float32]()
    elif tv.dtype == STDtype.F16:
        var hp16 = host_in.unsafe_ptr().bitcast[Float16]()
        for i in range(n):
            f32_buf[i] = hp16[i].cast[DType.float32]()
    else:
        raise Error("Wan22 VAE conv3d fcqrs loader supports F32/BF16/F16 only")
    return Tensor.from_host(f32_buf, sh^, out_dtype, ctx)
...
# The resample conv is a spatial 2D conv (on-disk OIHW [Cout,Cin,Kh,Kw]). This
# loads it as FCQRS [Cout,Cin,1,Kh,Kw] (Q=1 D-kernel) — the layout
# conv3d_fcqrs_cudnn expects (identity repack, NO transpose; same F16/BF16->F32
# ->out_dtype value path as _load_conv2d_qrscf). Lets _resample2d use the cuDNN
# conv3d fast path instead of the naive kernel (measured 87% of decode GPU time).
def _load_conv2d_fcqrs[
    mut: Bool, //, origin: Origin[mut=mut]
](tv: TensorView[origin], out_dtype: STDtype, ctx: DeviceContext) raises -> Tensor:
    var sh = tv.shape.copy()
    if len(sh) != 4:
        raise Error("Wan22 VAE conv2d fcqrs loader expected rank-4 OIHW")
    var cout = sh[0]
    var cin = sh[1]
    var kh = sh[2]
    var kw = sh[3]
    ...
    if tv.dtype == STDtype.F32:
        ...
        raise Error("Wan22 VAE conv2d fcqrs loader supports F32/BF16/F16 only")
    # OIHW [Cout,Cin,Kh,Kw] -> FCQRS [Cout,Cin,1,Kh,Kw] (contiguous flat order
    # is identical; only a Q=1 axis is inserted).
    var osh = List[Int]()
    osh.append(cout); osh.append(cin); osh.append(1); osh.append(kh); osh.append(kw)
    return Tensor.from_host(f32_buf, osh^, out_dtype, ctx)
...
def _dup_up3d_kernel_f32(
    ...
    bsz_w: Int32, d_w: Int32, h_w: Int32, w_w: Int32, cin_w: Int32,
    od_w: Int32, oh_w: Int32, ow_w: Int32, cout_w: Int32,
    factor_t_w: Int32, factor_s_w: Int32, repeats_w: Int32, drop_w: Int32,
):
    var bsz = Int(bsz_w)
    var d = Int(d_w)
    var h = Int(h_w)
...
def _dup_up3d(
    ...
    if dt == DType.float32:
        ...
        ctx.enqueue_function[_dup_up3d_kernel_f32](
            X, O, Int32(b), Int32(d), Int32(h), Int32(w), Int32(cin), Int32(od), Int32(oh), Int32(ow), Int32(out_channels),
            Int32(factor_t), Int32(factor_s), Int32(repeats), Int32(drop), grid_dim=grid, block_dim=_BLOCK,
        )
        ...
        ctx.enqueue_function[_dup_up3d_kernel_bf16](
            X, O, Int32(b), Int32(d), Int32(h), Int32(w), Int32(cin), Int32(od), Int32(oh), Int32(ow), Int32(out_channels),
            Int32(factor_t), Int32(factor_s), Int32(repeats), Int32(drop), grid_dim=grid, block_dim=_BLOCK,
        )
    else:
        ...
        ctx.enqueue_function[_dup_up3d_kernel_f16](
            X, O, Int32(b), Int32(d), Int32(h), Int32(w), Int32(cin), Int32(od), Int32(oh), Int32(ow), Int32(out_channels),
            Int32(factor_t), Int32(factor_s), Int32(repeats), Int32(drop), grid_dim=grid, block_dim=_BLOCK,
        )
    ctx.synchronize()
...
def _time_interleave2_kernel_f32(
    x: LayoutTensor[DType.float32, _DYN1, MutAnyOrigin],
    o: LayoutTensor[DType.float32, _DYN1, MutAnyOrigin],
    bsz_w: Int32, d_w: Int32, h_w: Int32, w_w: Int32, c_w: Int32,
):
    var bsz = Int(bsz_w)
    var d = Int(d_w)
    var h = Int(h_w)
...
def _time_interleave2_ndhwc(x: Tensor, ctx: DeviceContext) raises -> Tensor:
    ...
    if dt == DType.float32:
        ...
        ctx.enqueue_function[_time_interleave2_kernel_f32](
            X, O, Int32(b), Int32(d), Int32(h), Int32(w), Int32(c), grid_dim=grid, block_dim=_BLOCK
        )
    elif dt == DType.bfloat16:
        var X = LayoutTensor[DType.bfloat16, _DYN1, MutAnyOrigin](
        unsafe_ptr=Pointer[Scalar[DType.bfloat16], MutAnyOrigin](
        ...
        ctx.enqueue_function[_time_interleave2_kernel_bf16](
            X, O, Int32(b), Int32(d), Int32(h), Int32(w), Int32(c), grid_dim=grid, block_dim=_BLOCK
        )
    else:
        var X = LayoutTensor[DType.float16, _DYN1, MutAnyOrigin](
        ...
        ctx.enqueue_function[_time_interleave2_kernel_f16](
            X, O, Int32(b), Int32(d), Int32(h), Int32(w), Int32(c), grid_dim=grid, block_dim=_BLOCK
        )
    ctx.synchronize()
...
def _unpatchify2_kernel_f32(
    x: LayoutTensor[DType.float32, _DYN1, MutAnyOrigin],
    o: LayoutTensor[DType.float32, _DYN1, MutAnyOrigin],
    bsz_w: Int32, d_w: Int32, h_w: Int32, w_w: Int32,
):
    var bsz = Int(bsz_w)
    var d = Int(d_w)
    var h = Int(h_w)
...
def _unpatchify2_ndhwc(x: Tensor, ctx: DeviceContext) raises -> Tensor:
    ...
    if dt == DType.float32:
        ...
        ctx.enqueue_function[_unpatchify2_kernel_f32](
            X, O, Int32(b), Int32(d), Int32(h), Int32(w), grid_dim=grid, block_dim=_BLOCK
        )
    elif dt == DType.bfloat16:
        var X = LayoutTensor[DType.bfloat16, _DYN1, MutAnyOrigin](
        unsafe_ptr=Pointer[Scalar[DType.bfloat16], MutAnyOrigin](
        ...
        ctx.enqueue_function[_unpatchify2_kernel_bf16](
            X, O, Int32(b), Int32(d), Int32(h), Int32(w), grid_dim=grid, block_dim=_BLOCK
        )
    else:
        var X = LayoutTensor[DType.float16, _DYN1, MutAnyOrigin](
        ...
        ctx.enqueue_function[_unpatchify2_kernel_f16](
            X, O, Int32(b), Int32(d), Int32(h), Int32(w), grid_dim=grid, block_dim=_BLOCK
        )
    ctx.synchronize()
...
struct Wan22VaeImageDecoder[LH: Int, LW: Int]:
    ...
    def load(
        ...
        for ref nm in st.names():
            ...
            if nm.endswith(".weight") and len(tv.shape) == 5:
                # FCQRS layout for the cuDNN conv3d fast path (conv3d_fcqrs_cudnn).
                t = _load_conv3d_fcqrs(tv, cdtype, ctx)
            elif _is_resample_conv2d_weight(nm):
                # FCQRS [Cout,Cin,1,Kh,Kw] for the cuDNN conv3d fast path used by
                # _resample2d (was _load_conv2d_qrscf for the naive kernel).
                t = _load_conv2d_fcqrs(tv, cdtype, ctx)
            else:
                if f32:
    ...
    def _resample2d(
        ...
        # cuDNN conv3d fast path (weight now FCQRS [Cout,Cin,1,Kh,Kw]). This site
        # was the measured decode bottleneck (naive conv3d = 87% of decode GPU
        # time at 832x576) — the causal-conv cuDNN swap left it naive. Same conv:
        # 1x3x3 spatial, stride 1, pad (0,1,1) symmetric.
        return conv3d_fcqrs_cudnn(
