...
fn flux_guidance_scalar_wins_over_inert_ksampler_cfg() {
    let mut req = json!({"workflow": {
        ...
        "4": {"class_type": "CLIPTextEncode", "inputs": {"clip": ["2", 0], "text": "a painted lighthouse"}},
        ...
        "8": {"class_type": "KSampler", "inputs": {"seed": 7, "steps": 20, "cfg": 1.0, "sampler_name": "euler", "scheduler": "simple", "denoise": 1.0, "model": ["1", 0], "positive": ["5", 0], "negative": ["6", 0], "latent_image": ["7", 0]}},
        "9": {"class_type": "VAEDecode", "inputs": {"samples": ["8", 0], "vae": ["3", 0]}},
        "10": {"class_type": "SaveImage", "inputs": {"images": ["9", 0], "filename_prefix": "flux"}}
    }});
    lower_request(&mut req).expect("FLUX workflow must lower cleanly");
    assert_eq!(req["cfg"].as_f64(), Some(3.5));
    assert_eq!(req["prompt"].as_str(), Some("a painted lighthouse"));
    assert_eq!(req["model"].as_str(), Some("flux1-dev"));
}
