...
OneTrainer-port core/runtime changes from the current trainer port are summarized
in `serenitymojo/docs/ONETRAINER_CORE_PORT_STATUS_2026-06-06.md`. Use that short
...
| Module | Purpose / key defs | Status |
|---|---|---|
| `training/optim.mojo` | `adamw_step` (decoupled WD, ported to match torch.optim.AdamW / OneTrainer exactly: WD on `p` before the adaptive Adam subtraction, NOT folded into `g`), SGD+momentum, global-norm grad clip | **PROVEN** (`optim_converge_parity`: AdamW bowl ratio 9.8e-15) |
...
| `training/full_finetune_contract.mojo` | no-CUDA OneTrainer full-finetune readiness map. Pins the local target families with `TrainingMethod.FINE_TUNE` registrations, keeps Anima pointed at `/home/alex/OneTrainer-anima-ref`, separates shared full-weight save/load plus F32 optimizer sidecar scaffolding from unsupported real product loops, and names required resume sidecar keys: `param.N`, `adam_m.N`, `adam_v.N`, `__meta__=[t_step, accum_count]`, plus the tensor-name manifest | **CONTRACT** (`full_finetune_contract_smoke`: target map, unsupported-loop boundary, sidecar schema failures) |
| `training/onetrainer_train_loop_policy.mojo` | shared OneTrainer real-loop policy used by Qwen, Ernie, Anima, SD3.5, SDXL, Flux.1, Klein, Chroma, and Z-Image. Centralizes LoRA/AdamW validation, checkpoint/offload policy, sample cadence, save cadence, output paths, step paths, final-vs-step paths, and `.state.safetensors` sidecar naming without creating CUDA context. | **CONTRACT** (`onetrainer_train_loop_policy_smoke` plus all nine train-control wiring smokes) |
| `training/onetrainer_train_dry_run.mojo` | no-CUDA product entrypoint dry run. Reads a OneTrainer-style config, validates concept/sample/product-run requirements through `onetrainer_product_run.mojo`, preflights `continue_last_backup` internal backup sidecars, prints workspace dirs and the resolved `serenitymojo/training/<runner>.mojo <config> <max_steps>` command without spawning the model loop or creating `DeviceContext()` | **CONTRACT** (`onetrainer_product_run_smoke` plus direct dry-run command) |
| `training/onetrainer_cache_preflight.mojo` | no-CUDA product cache preflight. Binds each parsed `model_type` to the OneTrainer text-conditioning cache contract and VAE encode/cache contract before product run, exposes required train/sample cache fields, and fails loudly when `only_cache` is requested for a model whose current Mojo encoder cannot emit OneTrainer raw VAE cache. | **CONTRACT** (`onetrainer_cache_preflight_smoke`: all current target model contracts, Qwen/SD3.5 raw-cache pass, plain SD3 block, Klein/Flux/Chroma raw-cache block) |
...
`qwen3_encoder`, `qwen25vl_encoder`. `onetrainer_conditioning_contract.mojo`
is the no-CUDA OneTrainer conditioning/cache-readiness map for the target image
