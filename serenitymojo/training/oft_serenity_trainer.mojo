# training/oft_serenity_trainer.mojo — SerenityTrainer-parity OFT (Diag-OFT, Linear).
#
# Distinct from oft_adapter.mojo (which is the lycoris exact-Cayley / output-side
# variant). This mirrors SerenityTrainer's OFTModule + OFTRotationModule EXACTLY (MJ-1024):
...
def _matmul(a: List[Float32], ra: Int, ca: Int, b: List[Float32], rb: Int, cb: Int) raises -> List[Float32]:
    if ca != rb:
        raise Error("oft_serenity _matmul: dim mismatch")
...
def oft_serenity_skew(vec_block: List[Float32], b: Int) -> List[Float32]:
    var q = _zeros(b * b)
    var k = 0
    for i in range(b):
...
# 5-term Neumann: R = I + 2Q + 2Q² + 2Q³ + Q⁴  (SerenityTrainer use_cayley_neumann default).
def oft_serenity_neumann_r(q: List[Float32], b: Int) raises -> List[Float32]:
    var r = _zeros(b * b)
    for i in range(b):
...
# Backward through y = oft_serenity_forward(x, vec, W). W frozen → only d_vec (+ d_x).
def oft_serenity_backward(
