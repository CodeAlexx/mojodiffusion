# tests/oft_carrier_parity.mojo — SerenityTrainer-OFT full-delta (a,b)-carrier gate.
# The carrier (a=I, b=W_eff-W) reproduces oft_serenity_forward (base x@Wᵀ + carrier),
# and the chained carrier grad reproduces oft_serenity_backward's d_vec. Small dims
...
from serenitymojo.training.oft_stack import oft_serenity_carrier_adapter, oft_serenity_chain_carrier_grads
...
def main() raises:
    ...
    # ── 1. base x@Wᵀ + carrier reproduces oft_serenity_forward ──
    var car = oft_serenity_carrier_adapter(vec.copy(), W.copy(), IN, OUT, b, r)
    ...
    var y_ref = oft_serenity_forward(x.copy(), vec.copy(), W.copy(), M, IN, OUT, b, r)
    ...
    # ── 2. chained carrier grad matches oft_serenity_backward.d_vec ──
    var d_vec_c = oft_serenity_chain_carrier_grads(vec.copy(), W.copy(), sim.d_a, sim.d_b, IN, OUT, b, r)
    var g_ref = oft_serenity_backward(d_y.copy(), x.copy(), vec.copy(), W.copy(), M, IN, OUT, b, r)
