...
from serenitymojo.ops.tensor_algebra import reshape, slice, transpose, concat, mul_scalar
...
struct AceStepDiTConfig(Copyable, Movable, ImplicitlyCopyable):
    ...
    @staticmethod
    def xl_base() -> AceStepDiTConfig:
        # ACE-Step-1.5 xl-base (4B DiT): 32L/2560/32h/8kv/9728. Same in_ch(192),
        # acoustic(64), patch(2), sliding(128) as turbo; encoder_hidden_size=2048
        # (asymmetric) is absorbed by decoder.condition_embedder weight shape.
        return AceStepDiTConfig(
            2560, 32, 8, 128, 9728, 32, 192, 64, 2, 1_000_000.0, 1e-6, 128
        )
...
# LoRA overlay for one projection: out = base + scale * B(A x), where the base
# linear output is `base` and x is that linear's input. A (bf16 [r,in]) and B
# (bf16 [out,r]) live in `bw` under "<key>.lora_A"/"<key>.lora_B" (per-layer
# relative). Absent → base returned unchanged, so the no-LoRA path is byte-for-
# byte the inference spine (existing gates unaffected). LoRA is ADDED, never fused.
def _lora_add(
    x: Tensor, var base: Tensor, key: String,
    bw: Dict[String, ArcPointer[Tensor]], scale: Float32, ctx: DeviceContext,
) raises -> Tensor:
    var ka = key + ".lora_A"
    var kb = key + ".lora_B"
    if ka in bw and kb in bw:
        var xa = linear(x, _w(bw, ka, ctx), None, ctx)    # [*, r]
        var xb = linear(xa, _w(bw, kb, ctx), None, ctx)   # [*, out]
        return _add(base, mul_scalar(xb, scale, ctx), ctx)
    return base^
...
def _tile_rows(
    ...
    ctx.enqueue_function[_tile_rows_kernel](
        SRC, DST, Int32(s), Int32(heads), Int32(half), grid_dim=grid, block_dim=_BLOCK
    )
    ctx.synchronize()
    var sh = [s * heads, half]
    return Tensor(out_buf^, sh^, x.dtype())
...
def _tile_rows_kernel(
    src: LayoutTensor[DType.bfloat16, _DYN1, MutAnyOrigin],
    dst: LayoutTensor[DType.bfloat16, _DYN1, MutAnyOrigin],
    s_w: Int32, heads_w: Int32, half_w: Int32,
):
    var s = Int(s_w)
...
def _sliding_mask_kernel[dtype: DType](
    ...
    h_w: Int32, s_w: Int32, window_w: Int32,
):
    var h = Int(h_w)
    var s = Int(s_w)
    var window = Int(window_w)
    var idx = Int(global_idx.x)
    var total = h * s * s
    if idx < total:
        var j = idx % s
...
def _build_sliding_mask(
    ...
    if dt == DType.float32:
        ...
        ctx.enqueue_function[_sliding_mask_kernel[DType.float32]](DST, Int32(h), Int32(s), Int32(window), grid_dim=grid, block_dim=_BLOCK)
        ...
        ctx.enqueue_function[_sliding_mask_kernel[DType.bfloat16]](DST, Int32(h), Int32(s), Int32(window), grid_dim=grid, block_dim=_BLOCK)
        ...
        ctx.enqueue_function[_sliding_mask_kernel[DType.float16]](DST, Int32(h), Int32(s), Int32(window), grid_dim=grid, block_dim=_BLOCK)
    else:
...
def _self_attn[S: Int, NH: Int](
    ...
    ctx: DeviceContext, lora_scale: Float32 = 0.0,
) raises -> Tensor:
    ...
    var q = _lora_add(hidden, linear(hidden, _w(bw, prefix + ".q_proj.weight", ctx), None, ctx),
                      prefix + ".q_proj", bw, lora_scale, ctx)
    var k = _lora_add(hidden, linear(hidden, _w(bw, prefix + ".k_proj.weight", ctx), None, ctx),
                      prefix + ".k_proj", bw, lora_scale, ctx)
    var v = _lora_add(hidden, linear(hidden, _w(bw, prefix + ".v_proj.weight", ctx), None, ctx),
                      prefix + ".v_proj", bw, lora_scale, ctx)
    ...
    if layer_type == 1 and S > window:
        ...
        attn = sdpa_tiled[1, S, NH, 128](qh, kfull, vfull, smask, scale, ctx)
    else:
        attn = sdpa_nomask[1, S, NH, 128](qh, kfull, vfull, scale, ctx)
    var flat = reshape(attn, [1, S, nh * dh], ctx)
    return _lora_add(flat, linear(flat, _w(bw, prefix + ".o_proj.weight", ctx), None, ctx),
                     prefix + ".o_proj", bw, lora_scale, ctx)


# Cross-attention (q from hidden [1,S,H], kv from enc [1,L,H], NO RoPE).
def _cross_attn[S: Int, L: Int, NH: Int](
    ...
    var q = _lora_add(hidden, linear(hidden, _w(bw, prefix + ".q_proj.weight", ctx), None, ctx),
                      prefix + ".q_proj", bw, lora_scale, ctx)
    var k = _lora_add(enc, linear(enc, _w(bw, prefix + ".k_proj.weight", ctx), None, ctx),
                      prefix + ".k_proj", bw, lora_scale, ctx)
    var v = _lora_add(enc, linear(enc, _w(bw, prefix + ".v_proj.weight", ctx), None, ctx),
                      prefix + ".v_proj", bw, lora_scale, ctx)
    var qh = _qk_norm(reshape(q, [1, S, nh, dh], ctx),
                      _w(bw, prefix + ".q_norm.weight", ctx), S, nh, dh, eps, ctx)
    ...
    var attn = _sdpa_cross[S, L, NH](qh, kfull, vfull, nh, dh, scale, ctx)
    var flat = reshape(attn, [1, S, nh * dh], ctx)
    return _lora_add(flat, linear(flat, _w(bw, prefix + ".o_proj.weight", ctx), None, ctx),
                     prefix + ".o_proj", bw, lora_scale, ctx)
...
def _sdpa_cross[S: Int, L: Int, NH: Int](
    ...
    return sdpa[1, S, NH, 128](q, kpad, vpad, mask, scale, ctx)
...
def _pad_seq(
    ...
    ctx.enqueue_function[_pad_seq_kernel](
        SRC, DST, Int32(s), Int32(l), Int32(nh), Int32(dh), grid_dim=grid, block_dim=_BLOCK
    )
    ctx.synchronize()
    var sh = [1, s, nh, dh]
    return Tensor(out_buf^, sh^, x.dtype())
...
def _pad_seq_kernel(
    src: LayoutTensor[DType.bfloat16, _DYN1, MutAnyOrigin],
    dst: LayoutTensor[DType.bfloat16, _DYN1, MutAnyOrigin],
    s_w: Int32, l_w: Int32, nh_w: Int32, dh_w: Int32,
):
    var s = Int(s_w)
    var l = Int(l_w)
    var nh = Int(nh_w)
...
def _cross_mask(s: Int, l: Int, nh: Int, ctx: DeviceContext) raises -> Tensor:
    ...
    ctx.enqueue_function[_cross_mask_kernel](
        DST, Int32(s), Int32(l), Int32(nh), grid_dim=grid, block_dim=_BLOCK
    )
    ctx.synchronize()
    var sh = [1, nh, s, s]
...
def _cross_mask_kernel(
    dst: LayoutTensor[DType.bfloat16, _DYN1, MutAnyOrigin],
    s_w: Int32, l_w: Int32, nh_w: Int32,
):
    var s = Int(s_w)
    var l = Int(l_w)
    var nh = Int(nh_w)
...
def acestep_block0_forward[S: Int, L: Int, NH: Int](
    hidden: Tensor,        # [1,S,H]
    ...
    ctx: DeviceContext,
    lora_scale: Float32 = 0.0,   # 0 = no LoRA (base path unchanged)
) raises -> Tensor:
    ...
    var cross = _cross_attn[S, L, NH](cross_norm, enc, "cross_attn", bw, cfg, ctx, lora_scale)
    x = _add(x, cross, ctx)
...
def _ew[OP: Int](a: Tensor, b: Tensor, ctx: DeviceContext) raises -> Tensor:
    ...
    ctx.enqueue_function[_ew_kernel[OP]](
...
def _add_bias(x: Tensor, bias: Tensor, ctx: DeviceContext) raises -> Tensor:
    ...
    ctx.enqueue_function[_add_bias_kernel](
        X, Bv, O, Int64(rows * d), Int32(d), grid_dim=grid, block_dim=_BLOCK
    )
    ctx.synchronize()
...
def _layer_bw(
    ...
    # LoRA adapters (optional): copy per-proj A/B if present in `full`.
    var lora_projs = [
        "self_attn.q_proj", "self_attn.k_proj", "self_attn.v_proj", "self_attn.o_proj",
        "cross_attn.q_proj", "cross_attn.k_proj", "cross_attn.v_proj", "cross_attn.o_proj",
    ]
    for lp in lora_projs:
        var ka = String(lp) + ".lora_A"
        var kb = String(lp) + ".lora_B"
        if (pfx + ka) in full:
            bw[ka] = ArcPointer(_clone(full[pfx + ka][], ctx))
        if (pfx + kb) in full:
            bw[kb] = ArcPointer(_clone(full[pfx + kb][], ctx))
    return bw^
...
def acestep_forward[SP: Int, L: Int, NH: Int](
    ...
    ctx: DeviceContext,
    lora_scale: Float32 = 0.0,   # 0 = base (no LoRA); LoRA A/B live in `full` per-layer
) raises -> Tensor:
    var h = cfg.hidden_size
    var eps = cfg.rms_norm_eps
