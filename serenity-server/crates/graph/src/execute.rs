...
fn exec_node(
    ...
    match t {
        ...
        "FluxGuidance" => {
            ...
            // Comfy API prompts name this scalar `guidance`; Comfy UI canvas
            // imports normalize the positional widget to `cfg`.  In either
            // form it is the distilled guidance value the FLUX Mojo backend
            // consumes through JobParams.cfg, and must win over KSampler's
            // required-but-inert cfg=1 field.
            copy_field_if_missing(out, fields, "guidance", "cfg");
        ...
        "Krea2FlowEdit" | "Ideogram4FlowEdit" => {
            // Phase C1: krea2 training-free instruction edit (FlowEdit ODE).
            // Inputs: model(MODEL), src_image(IMAGE), src/tgt positive+negative
            // (CONDITIONING). Widgets: steps, nmax, nmin, src_cfg, tgt_cfg, seed,
            // auto_mask, mask_q, mask_dilate, mask_warmup. Lowers to the flat
            // edit_* JobParams keys (edit_src_image marks the job as an edit);
            // the TGT prompt/negative/cfg ride the standard top-level fields.
            // Emits a symbolic IMAGE (like VAEDecode) so SaveImage terminates
            // the graph on the image route.
            let model_link = links.input(id, "model");
            let src_image_link = links.input(id, "src_image");
            let src_pos_link = links.input(id, "src_positive");
            let src_neg_link = links.input(id, "src_negative");
            let tgt_pos_link = links.input(id, "tgt_positive");
            let tgt_neg_link = links.input(id, "tgt_negative");
            if !model_link.found
                || !src_image_link.found
                || !src_pos_link.found
                || !tgt_pos_link.found
            {
                return Err(GraphError::unsupported(
                    "workflow graph FlowEdit missing required typed input \
                     (model, src_image, src_positive, tgt_positive)",
                )
                .with_node(id));
            }
            if !(ready(store, &model_link)
                && ready(store, &src_image_link)
                && ready(store, &src_pos_link)
                && optional_ready(store, &src_neg_link)
                && ready(store, &tgt_pos_link)
                && optional_ready(store, &tgt_neg_link))
            {
                return Ok(Fire::NotReady);
            }
            require_value_type(store, &model_link, "MODEL", "model")?;
            require_value_type(store, &src_image_link, "IMAGE", "src_image")?;
            require_value_type(store, &src_pos_link, "CONDITIONING", "src_positive")?;
            require_value_type(store, &tgt_pos_link, "CONDITIONING", "tgt_positive")?;
            let model_name = model_name_of(store, &model_link)?;
            if !model_name.is_empty() {
                set_if_missing(out, "model", json!(model_name));
            }
            set_if_missing(
                out,
                "edit_src_image",
                json!(image_path_of(store, &src_image_link)?),
            );
            set_if_missing(
                out,
                "edit_src_prompt",
                json!(cond_text_of(store, &src_pos_link)?),
            );
            if src_neg_link.found {
                require_value_type(store, &src_neg_link, "CONDITIONING", "src_negative")?;
                set_if_missing(
                    out,
                    "edit_src_negative",
                    json!(cond_text_of(store, &src_neg_link)?),
                );
            }
            // TGT prompt/negative ride the standard fields the worker already reads.
            set_if_missing(out, "prompt", json!(cond_text_of(store, &tgt_pos_link)?));
            *saw_prompt = true;
            if tgt_neg_link.found {
                require_value_type(store, &tgt_neg_link, "CONDITIONING", "tgt_negative")?;
                set_if_missing(out, "negative", json!(cond_text_of(store, &tgt_neg_link)?));
            }
            ...
            copy_field_if_missing(out, fields, "src_cfg", "edit_src_cfg");
            copy_field_if_missing(out, fields, "auto_mask", "edit_auto_mask");
            copy_field_if_missing(out, fields, "mask_q", "edit_mask_q");
            copy_field_if_missing(out, fields, "mask_dilate", "edit_mask_dilate");
            copy_field_if_missing(out, fields, "mask_warmup", "edit_mask_warmup");
            add_value(
                store,
                id,
                "IMAGE",
