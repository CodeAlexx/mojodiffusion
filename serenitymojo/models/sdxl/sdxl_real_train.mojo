...
# ═══════════════════════════════════════════════════════════════════════════════
def sdxl_real_forward[L: Int](
    x_nhwc: Tensor, t: Tensor, y: Tensor, context: Tensor,
    ...
    # conv_in (4->320)
    var conv_in_x = x_nhwc.clone(ctx)
    var h = _conv3x3_fwd[H0, H0, 4, MC](x_nhwc, w.conv_in_w, w.conv_in_b, ctx)
    skips.append(TArc(h.clone(ctx)))                                   # skip[0]

    # in1 Res(320->320)
    var rb1 = resblock_forward[1, H0, H0, MC, MC, TEMB, G](h, emb, w.res[R_IN1][], ctx)
    res_acts.append(ArcPointer(_clone_rb_acts(rb1.acts, ctx))); h = rb1.out.clone(ctx)
    skips.append(TArc(h.clone(ctx)))                                   # skip[1]
    # in2 Res(320->320)
    var rb2 = resblock_forward[1, H0, H0, MC, MC, TEMB, G](h, emb, w.res[R_IN2][], ctx)
    ...
    h = downsample_forward[1, H0, H0, MC](h, w.down_w[0][], w.down_b[0][], ctx)
    skips.append(TArc(h.clone(ctx)))                                   # skip[3]

    # in4 Res(320->640)+ST(640,h10,d2)
    var rb4 = resblock_forward[1, H1, H1, MC, C1, TEMB, G](h, emb, w.res[R_IN4][], ctx)
    res_acts.append(ArcPointer(_clone_rb_acts(rb4.acts, ctx))); h = rb4.out.clone(ctx)
    var s4 = sdxl_st_lora_forward[1, H1, H1, C1, NKV, CCTX, H10, HD, CFF1, G, D2](
    ...
    # in5 Res(640->640)+ST
    var rb5 = resblock_forward[1, H1, H1, C1, C1, TEMB, G](h, emb, w.res[R_IN5][], ctx)
    res_acts.append(ArcPointer(_clone_rb_acts(rb5.acts, ctx))); h = rb5.out.clone(ctx)
    var s5 = sdxl_st_lora_forward[1, H1, H1, C1, NKV, CCTX, H10, HD, CFF1, G, D2](
    ...
    # in6 Down(640) H1->H2
    down_x.append(TArc(h.clone(ctx)))
    h = downsample_forward[1, H1, H1, C1](h, w.down_w[1][], w.down_b[1][], ctx)
    skips.append(TArc(h.clone(ctx)))                                   # skip[6]

    # in7 Res(640->1280)+ST(1280,h20,d10)
    var rb7 = resblock_forward[1, H2, H2, C1, C2, TEMB, G](h, emb, w.res[R_IN7][], ctx)
    res_acts.append(ArcPointer(_clone_rb_acts(rb7.acts, ctx))); h = rb7.out.clone(ctx)
    var s7 = sdxl_st_lora_forward[1, H2, H2, C2, NKV, CCTX, H20, HD, CFF2, G, D10](
    ...
    var rb8 = resblock_forward[1, H2, H2, C2, C2, TEMB, G](h, emb, w.res[R_IN8][], ctx)
    res_acts.append(ArcPointer(_clone_rb_acts(rb8.acts, ctx))); h = rb8.out.clone(ctx)
    var s8 = sdxl_st_lora_forward[1, H2, H2, C2, NKV, CCTX, H20, HD, CFF2, G, D10](
    ...
    # middle: Res + ST + Res
    var rbm0 = resblock_forward[1, H2, H2, C2, C2, TEMB, G](h, emb, w.res[R_MID0][], ctx)
    res_acts.append(ArcPointer(_clone_rb_acts(rbm0.acts, ctx))); h = rbm0.out.clone(ctx)
    var sm = sdxl_st_lora_forward[1, H2, H2, C2, NKV, CCTX, H20, HD, CFF2, G, D10](
        h, context.clone(ctx), w.st[ST_MID], lora[ST_MID], ctx)
    st_acts.append(ArcPointer(sm.acts.copy())); h = sm.out.clone(ctx)
    var rbm2 = resblock_forward[1, H2, H2, C2, C2, TEMB, G](h, emb, w.res[R_MID2][], ctx)
    ...
    var ro0 = resblock_forward[1, H2, H2, 2560, C2, TEMB, G](h, emb, w.res[R_OUT0][], ctx)
    res_acts.append(ArcPointer(_clone_rb_acts(ro0.acts, ctx))); h = ro0.out.clone(ctx)
    var so0 = sdxl_st_lora_forward[1, H2, H2, C2, NKV, CCTX, H20, HD, CFF2, G, D10](
    ...
    var ro1 = resblock_forward[1, H2, H2, 2560, C2, TEMB, G](h, emb, w.res[R_OUT1][], ctx)
    res_acts.append(ArcPointer(_clone_rb_acts(ro1.acts, ctx))); h = ro1.out.clone(ctx)
    var so1 = sdxl_st_lora_forward[1, H2, H2, C2, NKV, CCTX, H20, HD, CFF2, G, D10](
    ...
    var ro4 = resblock_forward[1, H1, H1, 1280, C1, TEMB, G](h, emb, w.res[R_OUT4][], ctx)
    res_acts.append(ArcPointer(_clone_rb_acts(ro4.acts, ctx))); h = ro4.out.clone(ctx)
    var so4 = sdxl_st_lora_forward[1, H1, H1, C1, NKV, CCTX, H10, HD, CFF1, G, D2](
    ...
    var ro7 = resblock_forward[1, H0, H0, 640, MC, TEMB, G](h, emb, w.res[R_OUT7][], ctx)
    ...
    var ro8 = resblock_forward[1, H0, H0, 640, MC, TEMB, G](h, emb, w.res[R_OUT8][], ctx)
    ...
    var out = _conv3x3_fwd[H0, H0, MC, 4](sf, w.conv_out_w, w.conv_out_b, ctx)
...
# ═══════════════════════════════════════════════════════════════════════════════
def sdxl_real_backward[L: Int](
    go: Tensor, acts: SdxlRealActs, w: SdxlRealWeights, lora: List[SdxlLoraSet],
    ctx: DeviceContext,
    ...
    var gco = _conv3x3_bwd[H0, H0, MC, 4](go, acts.conv_out_in, w.conv_out_w, ctx)
    ...
    var d_h = ggn.d_x.clone(ctx)         # grad into out8 output [1,H0,H0,320]
    ...
    var go8 = resblock_backward[1, H0, H0, 640, MC, TEMB, G](d_h, acts.res_acts[R_OUT8][], w.res[R_OUT8][], ctx)
    ...
    var go7 = resblock_backward[1, H0, H0, 640, MC, TEMB, G](d_h, acts.res_acts[R_OUT7][], w.res[R_OUT7][], ctx)
    ...
    var go6 = resblock_backward[1, H0, H0, 960, MC, TEMB, G](d_h, acts.res_acts[R_OUT6][], w.res[R_OUT6][], ctx)
    ...
    var gu5 = upsample_backward[1, H1, H1, C1](d_h, acts.up_up[1][], w.up_w[1][], ctx)
    var gso5 = sdxl_st_lora_backward[1, H1, H1, C1, NKV, CCTX, H10, HD, CFF1, G, D2](
    ...
    var dso5x = _host_to_dev4(gso5.d_x, 1, H1, H1, C1, ctx)
    var go5 = resblock_backward[1, H1, H1, 960, C1, TEMB, G](dso5x, acts.res_acts[R_OUT5][], w.res[R_OUT5][], ctx)
    ...
    var gso4 = sdxl_st_lora_backward[1, H1, H1, C1, NKV, CCTX, H10, HD, CFF1, G, D2](
    ...
    var dso4x = _host_to_dev4(gso4.d_x, 1, H1, H1, C1, ctx)
    var go4 = resblock_backward[1, H1, H1, 1280, C1, TEMB, G](dso4x, acts.res_acts[R_OUT4][], w.res[R_OUT4][], ctx)
    ...
    var gso3 = sdxl_st_lora_backward[1, H1, H1, C1, NKV, CCTX, H10, HD, CFF1, G, D2](
    ...
    var dso3x = _host_to_dev4(gso3.d_x, 1, H1, H1, C1, ctx)
    var go3 = resblock_backward[1, H1, H1, 1920, C1, TEMB, G](dso3x, acts.res_acts[R_OUT3][], w.res[R_OUT3][], ctx)
    ...
    var gu2 = upsample_backward[1, H2, H2, C2](d_h, acts.up_up[0][], w.up_w[0][], ctx)
    var gso2 = sdxl_st_lora_backward[1, H2, H2, C2, NKV, CCTX, H20, HD, CFF2, G, D10](
    ...
    var dso2x = _host_to_dev4(gso2.d_x, 1, H2, H2, C2, ctx)
    var go2 = resblock_backward[1, H2, H2, 1920, C2, TEMB, G](dso2x, acts.res_acts[R_OUT2][], w.res[R_OUT2][], ctx)
    ...
    var gso1 = sdxl_st_lora_backward[1, H2, H2, C2, NKV, CCTX, H20, HD, CFF2, G, D10](
    ...
    var dso1x = _host_to_dev4(gso1.d_x, 1, H2, H2, C2, ctx)
    var go1 = resblock_backward[1, H2, H2, 2560, C2, TEMB, G](dso1x, acts.res_acts[R_OUT1][], w.res[R_OUT1][], ctx)
    ...
    var gso0 = sdxl_st_lora_backward[1, H2, H2, C2, NKV, CCTX, H20, HD, CFF2, G, D10](
    ...
    var dso0x = _host_to_dev4(gso0.d_x, 1, H2, H2, C2, ctx)
    var go0 = resblock_backward[1, H2, H2, 2560, C2, TEMB, G](dso0x, acts.res_acts[R_OUT0][], w.res[R_OUT0][], ctx)
    ...
    var gm2 = resblock_backward[1, H2, H2, C2, C2, TEMB, G](d_mid_out, acts.res_acts[R_MID2][], w.res[R_MID2][], ctx)
    d_emb = add(d_emb, gm2.d_emb_in, ctx)
    var gsm = sdxl_st_lora_backward[1, H2, H2, C2, NKV, CCTX, H20, HD, CFF2, G, D10](
    ...
    var gsmx = _host_to_dev4(gsm.d_x, 1, H2, H2, C2, ctx)
    var gm0 = resblock_backward[1, H2, H2, C2, C2, TEMB, G](gsmx, acts.res_acts[R_MID0][], w.res[R_MID0][], ctx)
    ...
    var gs8 = sdxl_st_lora_backward[1, H2, H2, C2, NKV, CCTX, H20, HD, CFF2, G, D10](
    ...
    var gs8x = _host_to_dev4(gs8.d_x, 1, H2, H2, C2, ctx)
    var gin8 = resblock_backward[1, H2, H2, C2, C2, TEMB, G](gs8x, acts.res_acts[R_IN8][], w.res[R_IN8][], ctx)
    ...
    var gs7 = sdxl_st_lora_backward[1, H2, H2, C2, NKV, CCTX, H20, HD, CFF2, G, D10](
    ...
    var gs7x = _host_to_dev4(gs7.d_x, 1, H2, H2, C2, ctx)
    var gin7 = resblock_backward[1, H2, H2, C1, C2, TEMB, G](gs7x, acts.res_acts[R_IN7][], w.res[R_IN7][], ctx)
    d_emb = add(d_emb, gin7.d_emb_in, ctx)
    var d_in6_out = gin7.d_x.clone(ctx)          # grad into in6 (down) output [1,H2,H2,640]

    # ── in6 output grad = (from in7) + skip slab[6] ; Down bwd H1->H2 ──
    d_in6_out = add(d_in6_out, d_skip[6][], ctx)
    var gin6 = downsample_backward[1, H1, H1, C1](d_in6_out, acts.down_x[1][], w.down_w[1][], ctx)
    var d_in5_out = gin6.d_x.clone(ctx)          # grad into in5 output [1,H1,H1,640]

    # ── in5 output grad = (from in6) + skip slab[5] ; ST2 -> Res(640->640) ──
    d_in5_out = add(d_in5_out, d_skip[5][], ctx)
    var gs5 = sdxl_st_lora_backward[1, H1, H1, C1, NKV, CCTX, H10, HD, CFF1, G, D2](
    ...
    var gs5x = _host_to_dev4(gs5.d_x, 1, H1, H1, C1, ctx)
    var gin5 = resblock_backward[1, H1, H1, C1, C1, TEMB, G](gs5x, acts.res_acts[R_IN5][], w.res[R_IN5][], ctx)
    ...
    var gs4 = sdxl_st_lora_backward[1, H1, H1, C1, NKV, CCTX, H10, HD, CFF1, G, D2](
    ...
    var gs4x = _host_to_dev4(gs4.d_x, 1, H1, H1, C1, ctx)
    var gin4 = resblock_backward[1, H1, H1, MC, C1, TEMB, G](gs4x, acts.res_acts[R_IN4][], w.res[R_IN4][], ctx)
    d_emb = add(d_emb, gin4.d_emb_in, ctx)
    var d_in3_out = gin4.d_x.clone(ctx)          # grad into in3 (down) output [1,H1,H1,320]

    # ── in3 output grad = (from in4) + skip slab[3] ; Down bwd H0->H1 ──
    d_in3_out = add(d_in3_out, d_skip[3][], ctx)
    var gin3 = downsample_backward[1, H0, H0, MC](d_in3_out, acts.down_x[0][], w.down_w[0][], ctx)
    var d_in2_out = gin3.d_x.clone(ctx)

    # ── in2 output grad = (from in3) + skip slab[2] ; Res(320->320) ──
    d_in2_out = add(d_in2_out, d_skip[2][], ctx)
    var gin2 = resblock_backward[1, H0, H0, MC, MC, TEMB, G](d_in2_out, acts.res_acts[R_IN2][], w.res[R_IN2][], ctx)
    ...
    var gin1 = resblock_backward[1, H0, H0, MC, MC, TEMB, G](d_in1_out, acts.res_acts[R_IN1][], w.res[R_IN1][], ctx)
    ...
    var gci = _conv3x3_bwd[H0, H0, 4, MC](d_in0_out, acts.conv_in_x, w.conv_in_w, ctx)
