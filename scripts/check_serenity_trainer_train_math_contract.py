#!/usr/bin/env python3
"""SerenityTrainer shared train-math contract guard.
...
factors, and SerenityTrainer masked-loss element scaling. It deliberately does not
...
TRAIN_LOOPS = tuple(sorted((REPO / "trainer/src/serenity_trainer/trainer").glob("train_*_real.mojo")))
...
class Report:
    ...
    def warn(self, message: str) -> None:
        ...
        print(f"[reference-train-math] WARN {message}")


def fail(message: str) -> None:
    raise SystemExit(f"[reference-train-math] FAIL {message}")
...
def check_adamw_ref() -> None:
    ...
    print(
        "[reference-train-math] PASS AdamW ref "
...
def check_lr_ref() -> None:
    ...
    print("[reference-train-math] PASS LR refs cosine/linear/rex against local formulas")
...
def check_masked_loss_ref() -> None:
    ...
    print("[reference-train-math] PASS masked-loss element scaling ref")
...
def check_adamw_order_contracts() -> None:
    """Guard SerenityTrainer/PyTorch AdamW order: decay param, then Adam subtract."""
    errors: list[str] = []
    for path, func_name in ADAMW_ORDER_SURFACES:
        ...
        if min(pos for pos, _line in decay_lines) > first_adaptive_pos:
            errors.append(
                f"{rel}:{func_name} missing PyTorch/SerenityTrainer order "
                f"(decoupled weight decay before adaptive Adam subtraction: {first_adaptive})"
            )
    if errors:
        fail(
            "AdamW order static contract failed; PyTorch/SerenityTrainer AdamW "
            "requires decoupled weight decay before the adaptive subtraction:\n  - "
            + "\n  - ".join(errors)
        )
    print(
        "[reference-train-math] PASS AdamW order static guard "
...
def check_source_contracts(report: Report) -> None:
    ...
    for needle, label in (
        ...
        ("Float32(0.9)", "SerenityTrainer REX d=0.9 constant"),
    ):
    ...
    for needle, label in (
        ...
        ("def validate_serenity_trainer_policy_config", "shared policy validation"),
    ):
    ...
    for needle, label in (
        ("validate_serenity_train_math_policy", "shared train-math policy validator"),
        ("serenity_lr_for_optimizer_step", "shared optimizer-step LR resolver"),
        ("lr_for_step", "SerenityTrainer LR schedule binding"),
    ...
    for needle, label in (
        ...
        ("adapter_post -> adapter_after", "SerenityTrainer update delta comparison wording"),
    ...
    for needle, label in (
        ...
        (
            "SerenityTrainer zero-lr adapter state-init oracle only; not Mojo backward",
    ...
    print("[reference-train-math] PASS source contract surfaces are present")
...
def check_product_binding(report: Report) -> None:
    ...
    no_policy = [
        ...
        if "validate_serenity_train_math_policy" not in text
        or "validate_serenity_gradient_checkpointing_policy" not in text
    ]
    if no_policy:
        fail("target loops missing shared SerenityTrainer train-math policy calls: " + ", ".join(no_policy))

    lr_bound = [name for name, text in found.items() if "serenity_lr_for_optimizer_step(" in text]
    if len(lr_bound) != len(found):
        report.warn(
            "LR scheduler binding is partial: "
            f"{len(lr_bound)}/{len(found)} target loops call serenity_lr_for_optimizer_step "
    ...
    masked_bound = [
        name
        for name, text in found.items()
        if "validate_serenity_train_math_policy" in text
    ]
    if len(masked_bound) != len(found):
        report.warn(
            "masked/prior-loss config is not fail-loud product-bound in every target loop: "
            f"{len(masked_bound)}/{len(found)} loops call validate_serenity_train_math_policy "
...
def main() -> int:
    ...
    if report.warnings:
        print(f"[reference-train-math] WARNINGS: {len(report.warnings)}")
        if args.strict_product:
            return 1
        print("[reference-train-math] report-only PASS; use --strict-product for production binding")
        return 0

    print("[reference-train-math] PASS")
    return 0
