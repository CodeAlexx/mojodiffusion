...
Qwen, Ernie, and Anima currently have valid SerenityTrainer step-0 adapter dumps
...
SPECS: dict[str, ModelSpec] = {
    ...
    "ernie": ModelSpec(
        ...
        update_sources=(
            REPO / "trainer/src/serenity_trainer/trainer/train_ernie_real.mojo",
    ...
    "anima": ModelSpec(
        ...
        update_sources=(
            REPO / "trainer/src/serenity_trainer/trainer/train_anima_real.mojo",
...
def assess_model(spec: ModelSpec, files: list[Path]) -> dict[str, Any]:
    ...
    if missing_artifacts:
        blockers.append("missing required SerenityTrainer zero-lr state-init artifacts or wrapper")

    oracle: dict[str, Any] | None = None
    if not missing_artifacts:
        oracle = inspect_zero_lr_oracle(spec)
        if not oracle["has_zero_lr_state_init_oracle"]:
            blockers.append("SerenityTrainer adapter dump is not a zero-lr state-init oracle")
    ...
    if not passing_candidates:
        blockers.append(
            ...
            "adapters plus optimizer state initialization against SerenityTrainer"
        )
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Check whether zero-lr reference trainer state-init adapter oracles have Mojo consumers."
    )
