...
struct Tensor(Movable):
    ...
    def from_view_raw[
        ...
        # Bulk memcpy the mmap bytes into the pinned host buffer (same fast path
        # as from_view; a per-byte scalar loop here cost ~130ms per 201MB FP8
        # expert tensor → tens of seconds/step on LingBot-Video's streamed MoE).
        var host = ctx.enqueue_create_host_buffer[DType.uint8](nbytes)
        var dst = BytePtr(unsafe_from_address=Int(host.unsafe_ptr()))
        var src = BytePtr(unsafe_from_address=Int(tv.data.unsafe_ptr()))
        _ = sys_memcpy(dst, src, nbytes)
    ...
    def from_view_as_bf16[
        ...
        if tv.dtype == STDtype.BF16:
            # BF16-on-disk: destination dtype == source dtype, so there is NO
            # conversion to do and this is a pure byte copy. It used to run a
            # byte-at-a-time scalar loop, which for a 737 MiB block is ~774 M
            # iterations. MEASURED on this box: scalar loop 0.745 GB/s vs
            # sys_memcpy 14.99 GB/s = 20x, and byte-identity gated 80/80 keys
            # (models/ltx2/parity/ltx2_av_block_load_decomp_probe.mojo). Every
            # model loading BF16-on-disk weights pays this on every tensor.
            var dst = BytePtr(unsafe_from_address=Int(host_out.unsafe_ptr()))
            var src = BytePtr(unsafe_from_address=Int(tv.data.unsafe_ptr()))
            _ = sys_memcpy(dst, src, nbytes_out)
        else:
            # Stage the source bytes in a typed host buffer so Mojo can read
            # F32/F16 scalars safely before casting to BF16. The STAGING is a
            # pure byte copy (same 20x class as above); only the CAST below is
            # genuinely element-wise.
            var host_in = ctx.enqueue_create_host_buffer[DType.uint8](nbytes_in)
            var hp = host_in.unsafe_ptr()
            var sdst = BytePtr(unsafe_from_address=Int(hp))
            var ssrc = BytePtr(unsafe_from_address=Int(tv.data.unsafe_ptr()))
            _ = sys_memcpy(sdst, ssrc, nbytes_in)
