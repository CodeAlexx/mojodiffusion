#!/usr/bin/env python3
"""Audit Klein/Flux2 LoRA target parity against SerenityTrainer.

SerenityTrainer's Flux2/Klein LoRA setup wraps separate Linear modules for the double
blocks. The old mojodiffusion Klein trainer used fused qkv/proj LoRA slots
instead. That path is useful implementation history, but it is not SerenityTrainer
...
PORT = Path("/home/alex/serenity_trainer-mojo")
TRAIN_REF_PREFIX = PORT / "parity/klein_train_ref_step000"

SERENITY_FLUX2_MODEL = SERENITY_TRAINER / "modules/model/Flux2Model.py"
SERENITY_FLUX2_SETUP = SERENITY_TRAINER / "modules/modelSetup/Flux2LoRASetup.py"
...
CORE_TRAIN = REPO / "trainer/src/serenity_trainer/trainer/train_klein_real.mojo"
...
PORT_STACK = PORT / "src/serenity_trainer_mojo/model/klein/klein_stack_lora.mojo"
PORT_DOUBLE = PORT / "src/serenity_trainer_mojo/model/klein/double_block.mojo"
PORT_SINGLE = PORT / "src/serenity_trainer_mojo/model/klein/single_block.mojo"
...
def check_adapter_grad_update_gate(path: Path) -> None:
    text = read(path)
    for needle in (
        "Klein/Flux2 SerenityTrainer adapter gradient/update oracle gate",
        "adapter_pre_clip_grad",
        "adapter_post_clip_grad",
...
def check_loss_replay_gate(path: Path, require_artifacts: bool) -> None:
    text = read(path)
    for needle in (
        "Klein/Flux2 SerenityTrainer loss and d_loss replay guard",
...
def main() -> int:
    ...
    parser.add_argument(
        "--strict-core",
        action="store_true",
        help="fail unless mojodiffusion core Klein LoRA has the SerenityTrainer split-slot contract",
    )
    parser.add_argument(
        "--safetensors",
        type=Path,
    ...
    serenity_model = read(SERENITY_FLUX2_MODEL)
    serenity_setup = read(SERENITY_FLUX2_SETUP)
    ...
    missing_model = require_all("SerenityTrainer Flux2Model", serenity_model, DOUBLE_TARGETS + SINGLE_TARGETS)
    missing_setup = require_all("SerenityTrainer Flux2LoRASetup", serenity_setup, ["LoRAModuleWrapper"])
    if missing_model or missing_setup:
        print("[klein-lora-contract] FAIL: SerenityTrainer source scan did not find expected targets")
        if missing_model:
    ...
    print("[klein-lora-contract] SerenityTrainer double targets:", expected_dbl_slots)
    print("[klein-lora-contract] SerenityTrainer single targets:", expected_single_slots)
    ...
    if port_slots != expected_dbl_slots:
        print("[klein-lora-contract] FAIL: SerenityTrainer-named port mirror is not split-slot parity")
        return 1
    ...
    if port_missing:
        print("[klein-lora-contract] FAIL: SerenityTrainer-named port mirror is missing split-slot markers")
    ...
    if core_slots != expected_dbl_slots:
        core_blockers.append(
            ...
            f"{core_slots}; SerenityTrainer requires {expected_dbl_slots}"
        )
    if "SGL_SAVE_TAIL = 0" not in core:
        core_blockers.append("core stack still saves the old single-block tail instead of all SerenityTrainer single blocks")
    ...
    if not single_builder_has_full_ff_dim(core):
        core_blockers.append("core single-block adapters still lack the SerenityTrainer F/mlp_hidden dimensions")
    ...
    if missing_single:
        core_blockers.append(
            "core single_block is not full SerenityTrainer fused-qkv-mlp coverage, missing "
    ...
    if "saved.att_flat[], lora.out" in core_single:
        core_blockers.append("core single_block still uses attention-only input for the SerenityTrainer to_out LoRA")
    ...
    if "stochastic" not in core and "stochastic" not in read_optional(CORE_LORA_ADAPTER):
        core_blockers.append("core LoRA AdamW is not the SerenityTrainer stochastic-BF16 rounding helper")
    if "save_lora_serenity_trainer" not in core:
        core_blockers.append("core save_klein_lora is not using the SerenityTrainer raw LoRA saver")
    if ".lora_down.weight" not in core_lora_save or ".lora_up.weight" not in core_lora_save:
        core_blockers.append("shared LoRA saver does not emit raw SerenityTrainer lora_down/lora_up tensors")
    if ".alpha" not in core_lora_save or "_bf16_scalar" not in core_lora_save:
        core_blockers.append("shared LoRA saver does not emit BF16 SerenityTrainer alpha tensors")
    if ".lora_down.weight" not in core_lora_save or "adapter_scale = alpha_h[0] / Float32(rank)" not in core_lora_save:
        core_blockers.append("shared LoRA resume loader cannot read raw SerenityTrainer alpha/down/up files")

    if core_blockers:
        print("[klein-lora-contract] WARN: mojodiffusion core Klein trainer is not SerenityTrainer parity")
