# sdxl_tiled_decode.mojo — SDXL VAE tiled decode (3x3 overlap + feathered blend).
#
# The whole-frame 128-latent LdmVaeDecoder[128,128,4] decode of an SDXL latent peaks
# past 24 GB beside the per-job working set (MEASURED: CUDA_ERROR_OUT_OF_MEMORY at the
# 1024^2 decode in SdxlBackend). Each 64-latent crop (LdmVaeDecoder[64,64,4]) is a small
# alloc that fits. Crops sit at latent stride 32 (positions 0/32/64) so adjacent image
# tiles overlap by half a tile; a separable feathered cross-fade erases the conv-boundary
# seams. Tile vars are reassigned per row so prior tiles free → retained peak stays near
# the single-tile working set.
#
# Blend math (_weight_tensor/_xfade/_blend3) is byte-identical to zimage_tiled_decode /
# flux_tiled_decode — it is VAE-agnostic (operates on decoded image tensors); only the
...
from serenitymojo.models.vae.ldm_decoder import LdmVaeDecoder, load_sdxl_ldm_decoder
...
# ── tiled SDXL VAE decode — 3x3 OVERLAPPING latent crops + feathered blend ────
# latent NCHW [1,4,LATENT_H,LATENT_W] -> image NCHW [1,3,8*LATENT_H,8*LATENT_W].
# Decoder supplied at the TILE shape (LATENT/2), reused for all 9 crops at stride TILE/2.
def sdxl_tiled_decode_with_decoder[
    LATENT_H: Int, LATENT_W: Int, TILE_H: Int, TILE_W: Int
](
    latent: Tensor, dec: LdmVaeDecoder[TILE_H, TILE_W, 4], ctx: DeviceContext
) raises -> Tensor:
...
def sdxl_tiled_decode[
    ...
    var dec = load_sdxl_ldm_decoder[TILE_H, TILE_W](vae_path, ctx)
    return sdxl_tiled_decode_with_decoder[
