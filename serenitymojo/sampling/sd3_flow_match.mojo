...
#
# SerenityTrainer's SD3 sampler deep-copies FlowMatchEulerDiscreteScheduler and calls
...
def build_sd3_serenity_trainer_sigmas(
    num_steps: Int, shift: Float32
) raises -> List[Float32]:
    """SerenityTrainer SD3 set_timesteps(num_steps) scalar schedule.

    This intentionally does not use the repo's generic SD3 shifted schedule.
    SerenityTrainer does not pass custom sigmas for SD3, so diffusers samples between
    the scheduler's shifted training sigma bounds and then applies the static
    FlowMatch shift inside set_timesteps.
    """
    if num_steps <= 0:
        raise Error("build_sd3_serenity_trainer_sigmas: num_steps must be > 0")
...
struct SD3FlowMatchScheduler(Movable):
    """Host scalar schedule plus tensor update helpers for SerenityTrainer SD3."""
    ...
    def __init__(out self, num_steps: Int, shift: Float32) raises:
        self._sigmas = build_sd3_serenity_trainer_sigmas(num_steps, shift)
