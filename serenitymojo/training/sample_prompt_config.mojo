...
def sample_time_unit_from_string(unit: String) raises -> Int:
    ...
    raise Error(String("sample cadence: unknown SerenityTrainer time unit ") + unit)
...
def _read_top_level_prompt_list(var bytes: List[UInt8]) raises -> SamplePromptConfig:
    var cfg = SamplePromptConfig()
    cfg.schema = String("serenity_trainer.samples.v1")
...
# ── STANDARD sample-prompts contract helpers (serenity.sample_prompts.v1) ─────
# Shared by every product trainer. The contract: a trainer reads its recipe from
# a config JSON; the sample prompts live in a SEPARATE serenity.sample_prompts.v1
# JSON referenced by the config's validation_prompts_file key. When that key is
# NON-EMPTY the trainer must condition each validation render on the PROMPT'S
# pre-encoded caps (not the current step's cached caption). When it is EMPTY the
# trainer keeps the legacy cached-caption behavior but must warn LOUDLY that the
# render is not a prompt-faithful validation (the MJ-1045 trap).


def caps_sampling_active(sample_file: String) -> Bool:
    """True when a validation_prompts_file / sample_definition_file_name is set.

    This is the switch between the prompt-faithful caps path (non-empty) and the
    legacy cached-caption fallback (empty)."""
    return sample_file != String("")


def assert_enabled_sample_prompts(cfg: SamplePromptConfig, model_label: String) raises:
    """Fail loud unless the config has >=1 ENABLED prompt. read_sample_prompt_config
    already enforces that every enabled prompt carries caps.positive/negative when
    precache_required (the v1 default), so this only guards the >=1 count."""
    var enabled = 0
    for i in range(len(cfg.prompts)):
        if cfg.prompts[i].enabled:
            enabled += 1
    if enabled == 0:
        raise Error(
            model_label
            + String(": validation_prompts_file has no ENABLED prompt; ")
            + String("sample-during-training needs at least one")
        )


def warn_legacy_cached_caption_sampling(model_label: String):
    """The LOUD one-line warning printed at the render seam when NO
    validation_prompts_file is set (the render reuses the current step's cached
    caption embeds — NOT a prompt-faithful validation; see MJ-1045)."""
    print(
        String("[") + model_label + String("] WARNING legacy cached-caption ")
        + String("sampling (no validation_prompts_file) - not a prompt-faithful ")
        + String("validation (MJ-1045 class)")
    )
