...
def sys_rename(oldpath: String, newpath: String) -> Int:
    """rename(2) oldpath -> newpath. Returns 0 on success, -1 on error. On the
    SAME filesystem this is ATOMIC: newpath always refers to either the old or the
    new file, never a partial one — the primitive behind temp+rename crash-safe
    writes (safetensors_writer.mojo). Two NUL-terminated cstr copies, same idiom as
    sys_remove (String.unsafe_ptr() is not reliably NUL-terminated)."""
    var no = oldpath.byte_length()
    var obuf = alloc[UInt8](no + 1)
    var osrc = oldpath.as_bytes()
    for i in range(no):
        obuf[i] = osrc[i]
    obuf[no] = 0
    var nn = newpath.byte_length()
    var nbuf = alloc[UInt8](nn + 1)
    var nsrc = newpath.as_bytes()
    for i in range(nn):
        nbuf[i] = nsrc[i]
    nbuf[nn] = 0
    var rc = Int(external_call["rename", Int32](
        BytePtr(unsafe_from_address=Int(obuf)),
        BytePtr(unsafe_from_address=Int(nbuf)),
    ))
    obuf.free()
    nbuf.free()
    return rc
