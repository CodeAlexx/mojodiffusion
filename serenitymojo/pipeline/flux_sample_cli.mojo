...
def _pack_latent_shape[LATENT_H_: Int, LATENT_W_: Int](
    z_nchw: Tensor, ctx: DeviceContext
) raises -> Tensor:
    comptime IMG_H2_ = LATENT_H_ // 2
    comptime IMG_W2_ = LATENT_W_ // 2
    comptime N_IMG_ = IMG_H2_ * IMG_W2_
    var s6 = List[Int]()
    s6.append(1)
...
def _pack_latent(z_nchw: Tensor, ctx: DeviceContext) raises -> Tensor:
    """Compatibility wrapper for the proven fixed 1024-square CLI path."""
    return _pack_latent_shape[LATENT_H, LATENT_W](z_nchw, ctx)


# ── unpack [1, h2*w2, 64] -> [1,16,2*h2,2*w2] ────────────────────────────────
def _unpack_latent_shape[LATENT_H_: Int, LATENT_W_: Int](
...
def _unpack_latent(packed: Tensor, ctx: DeviceContext) raises -> Tensor:
    """Compatibility wrapper for the proven fixed 1024-square CLI path."""
    return _unpack_latent_shape[LATENT_H, LATENT_W](packed, ctx)
...
def main() raises:
    raise Error(
        "disabled: this legacy Flux CLI reads checkpoint weights during "
