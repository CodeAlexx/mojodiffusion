...
comptime DEFAULT_ASPECT_LADDER_LEN = 7
comptime DEFAULT_ASPECT_LADDER_X100 = [100, 133, 75, 178, 56, 150, 67]
# Backward-compatible Z-Image names. The values are the shared default ladder,
# not a model-specific derivation; newer image inference dispatchers import the
# DEFAULT_* names directly so their production size sets cannot drift.
comptime ZIMAGE_T2D_LADDER_LEN = DEFAULT_ASPECT_LADDER_LEN
comptime ZIMAGE_T2D_LADDER_X100 = DEFAULT_ASPECT_LADDER_X100
...
# ── EDGE-GENERALIZED comptime latent ladder (T2.D krea2 reuse, 2026-07-06) ────
#
# The zimage helpers above are hard-pinned to the 512px/align-64 budget
# (edge E=512, e = E/align = 8; the `64` and `6400` constants ARE e² and e²·100).
# Krea-2 buckets at OTHER area budgets (512px → e=8, 1024px → e=16), so its
# comptime dispatch needs the SAME SimpleTuner-parity integer derivation
# parameterized on the edge units e = E/align. These are ADDITIVE (the zimage
# functions are byte-untouched, so zimage_comptime_ladder_gate stays valid); a
# krea2 gate proves aspect_lat_{h,w}_units(x100, e) == generate_aspect_buckets
# for the krea2 area, exactly as the zimage gate does for e=8.
#
# Derivation (align=64, VAE stride=8, so latent = target/8 = 64*units/8 = 8*units;
# see the zimage block above for the full SimpleTuner map):
#   E      = pixel_resolution = align * e            (align-multiple by construction)
#   w_units = round(e * sqrt(a)) = nearest_sqrt(e²·aspect_x100 / 100)
#   h_units = round(e / sqrt(a)) = nearest_sqrt(e²·100 / aspect_x100)
#   square snap (round(W/H,2)==1.0 ⇔ w_units==h_units): latent = 8*e (== E/8).
# For e=8 this reduces EXACTLY to zimage_t2d_lat_{w,h} (e²=64, e²·100=6400).


def aspect_lat_w_units(aspect_x100: Int, e_units: Int) -> Int:
    """Latent W (= target_w/8, align 64, VAE stride 8) of the bucket for the
    2-decimal ladder aspect aspect_x100/100 under edge units e_units (= E/align).
    Integer-only; comptime-evaluable. Generalizes zimage_t2d_lat_w (e_units=8)."""
    var e2 = e_units * e_units
    var wu = zimage_t2d_nearest_sqrt_units(e2 * aspect_x100, 100)
    if wu < 1:
        wu = 1
    var hu = zimage_t2d_nearest_sqrt_units(e2 * 100, aspect_x100)
    if hu < 1:
        hu = 1
    if wu == hu:  # 2dp aspect key == 1.0 -> square snap to (E x E)
        return 8 * e_units
    return 8 * wu


def aspect_lat_h_units(aspect_x100: Int, e_units: Int) -> Int:
    """Latent H (= target_h/8, align 64, VAE stride 8) of the bucket for the
    2-decimal ladder aspect aspect_x100/100 under edge units e_units (= E/align).
    Integer-only; comptime-evaluable. Generalizes zimage_t2d_lat_h (e_units=8)."""
    var e2 = e_units * e_units
    var wu = zimage_t2d_nearest_sqrt_units(e2 * aspect_x100, 100)
    if wu < 1:
        wu = 1
