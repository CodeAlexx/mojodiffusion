...
from serenitymojo.ops.norm_backward import group_norm_backward_dx
...
def sdxl_real_backward[H: Int, W: Int](
    ...
    # conv2d_backward/linear_backward hits its mixed path (the frozen out_gn uses
    # group_norm_backward_dx with the weight widened at the call site below).
    ...
    # frozen out_gn (LoRA training; d_g/d_b were discarded) -> d_x only. Widen the
    # BF16 checkpoint weight to the F32 activation dtype exactly like the full
    # group_norm_backward's mixed path did internally.
    var d_h = group_norm_backward_dx(
        d_silu, acts.final_gn_in,
        cast_tensor(w.out_gn_w, acts.final_gn_in.dtype(), ctx),
        G, GN_EPS_RES, ctx)              # grad into out8 output [1,H0,W0,320]
