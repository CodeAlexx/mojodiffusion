...
#
# TRUE BATCH-2 DEVICE-RESIDENT vs STREAMED parity gate for the Anima LoRA fast path.
#
# The b2 device-resident path (anima_stack_lora_forward/backward_device_resident_b2,
# all L-block activations + LoRA A/B kept as DEVICE tensors, per-sample attention)
# must compute the SAME thing as the proven host streamed_b2 path
# (anima_stack_lora_forward/backward_streamed_b2). Speed must not change the math.
# Runs BOTH on identical B=2 inputs with a NON-ZERO LoRA B and compares:
...
#
# F32 resident blocks + F32 device LoRA so it matches the F32 streamed_b2 path
# bit-closely (BF16 base is a production-speed concern, gated separately).
#
# Build (build-only compile check; run needs GPU):
...
def _seed_b(mut set: AnimaLoraSet, seed: UInt64):
    ...
    for i in range(len(set.ad)):
        ...
        for j in range(nb):
            set.ad[i].b[j] = r[j].cast[DType.bfloat16]()
        s += 17
...
# SINGLE-sample [S_img*H, Dh/2] rope table (the b2 stack doubles it internally).
def _rope_tables(ctx: DeviceContext) raises -> _Rope:
    ...
    var cos = Tensor.from_host(cosl, [S_IMG * H, half], STDtype.F32, ctx)
    var sin = Tensor.from_host(sinl, [S_IMG * H, half], STDtype.F32, ctx)
...
def main() raises:
    ...
    print("==== anima_b2_device_vs_streamed_parity (TRUE batch-2 device vs streamed) ====")
    ...
    # ── B=2 row-stacked pseudo-random inputs ──
    ...
    # ── STREAMED_b2 reference ──
    var fwd_s = anima_stack_lora_forward_streamed_b2[H, Dh, S_IMG, S_TXT](
    ...
    var grads_s = anima_stack_lora_backward_streamed_b2[H, Dh, S_IMG, S_TXT](
        d_out.copy(), patches.copy(), t_cond.copy(), base_adaln.copy(), context.copy(),
        base, st, lora, ropes.cos, ropes.sin, fwd_s,
        B, D, JOINT, F, IN_PATCH, OUT_PATCH, EPS, ctx,
    )
    print("streamed_b2 forward+backward done")

    # ── DEVICE-RESIDENT_b2 path under test ──
    var fwd_r = anima_stack_lora_forward_device_resident_b2[H, Dh, S_IMG, S_TXT](
        patches.copy(), t_cond.copy(), base_adaln.copy(), context.copy(),
        base, blocks, lora_dev, ropes.cos, ropes.sin,
        B, D, JOINT, F, IN_PATCH, OUT_PATCH, EPS, ctx,
    )
    var grads_r = anima_stack_lora_backward_device_resident_b2[H, Dh, S_IMG, S_TXT](
        d_out.copy(), patches.copy(), t_cond.copy(), base_adaln.copy(), context.copy(),
        base, blocks, lora_dev, ropes.cos, ropes.sin, fwd_r,
        B, D, JOINT, F, IN_PATCH, OUT_PATCH, EPS, ctx, False,
    )
    print("device-resident_b2 forward+backward done")
    ...
    print("  device nonfinite =", grads_r.nonfinite_lora_grads,
          "  streamed nonfinite =", grads_s.nonfinite_lora_grads)

    if all_ok and grads_r.nonfinite_lora_grads == 0:
        print("VERDICT: PASS — device-resident_b2 == streamed_b2 (out + all LoRA grads + d_patches + d_t_silu, cos>=0.999)")
    else:
        print("VERDICT: FAIL")
