...
Mojo. Python/PyTorch and SerenityTrainer may generate parity references and baseline
...
- Do not modify SerenityTrainer. Treat `/home/alex/SerenityTrainer` as a read-only source
  of truth for Klein, SDXL, Z-Image, and Ernie presets, losses, timestep policy,
  and sampling references. Use `/home/alex/SerenityTrainer-anima-ref` for Anima.
...
SerenityTrainer 512 baseline on the Klein/Alina dataset:

- Preset: `/home/alex/SerenityTrainer/configs/alina_zimage_serenity_preset_100_baseline.json`
...
run is broken. The target loss scale is the SerenityTrainer baseline above.
...
SerenityTrainer ERNIE reference files:

- `/home/alex/SerenityTrainer/training_presets/#ernie LoRA 8GB.json`
- `/home/alex/SerenityTrainer/training_presets/#ernie LoRA 16GB.json`
- `/home/alex/SerenityTrainer/modules/modelSetup/BaseErnieSetup.py`
- `/home/alex/SerenityTrainer/modules/model/ErnieModel.py`
- `/home/alex/SerenityTrainer/modules/dataLoader/ErnieBaseDataLoader.py`
- `/home/alex/SerenityTrainer/modules/modelSampler/ErnieSampler.py`
...
- The SerenityTrainer LoRA train preset uses LOGIT_NORMAL with default
...
- Do not use SerenityTrainer MGDS, Rust caches, Python caches, or fixed text
  ...
  LoRA save/load path.
- Do not attempt full-F32 ERNIE residency. SerenityTrainer uses BF16 train/output,
...
- Batch-2 parity needs attention masking or exact same-length text buckets.
  SerenityTrainer trims text embeddings to batch max length but still passes
...
- Compare recipe against SerenityTrainer or the model-specific reference tree.
