...
#   written.
# - `save_dora_serenity_trainer` writes [out,1] for wd_on_out=true and [1,in] for the
#   SerenityTrainer default wd_on_out=false.
...
# SerenityTrainer uses the same key names as the LyCORIS save but preserves the DoRA
# decomposition axis in `dora_scale` shape. Linear per-input default is [1,in].
def save_dora_serenity_trainer(
    adapters: List[NamedDoRA], path: String, ctx: DeviceContext
) raises -> Int:
    if len(adapters) == 0:
        raise Error("save_dora_serenity_trainer: refusing to write an empty DoRA file")
    ...
    for ref nd in adapters:
        ...
        if len(a.a) != R * IN:
            raise Error(String("save_dora_serenity_trainer: lora_down numel ") + String(len(a.a)) + " != rank*in for '" + nd.prefix + "'")
        if len(a.b) != OUT * R:
            raise Error(String("save_dora_serenity_trainer: lora_up numel ") + String(len(a.b)) + " != out*rank for '" + nd.prefix + "'")
        if len(a.m) != mlen:
            raise Error(String("save_dora_serenity_trainer: magnitude numel ") + String(len(a.m)) + " != decomposition axis for '" + nd.prefix + "'")
