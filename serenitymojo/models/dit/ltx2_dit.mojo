...
def _av_view_to_bf16[
    mut: Bool, //, origin: Origin[mut=mut]
](tv: TensorView[origin], ctx: DeviceContext) raises -> Tensor:
    """BF16 materialisation of one checkpoint view for the AV block loader.

    ADDITIVE fast path — `Tensor.from_view_as_bf16` is untouched (156 call sites
    repo-wide; changing it would put every other trainer's anchors at risk).

    For a BF16-on-disk view there is NO conversion to do: the destination dtype
    already equals the source dtype, so the operation is a raw byte copy — which
    is exactly what `Tensor.from_view` (tensor.mojo:174) does, via `sys_memcpy`.
    `from_view_as_bf16` instead reaches its `tv.dtype == BF16` branch
    (tensor.mojo:238-241) and copies the bytes ONE AT A TIME in a scalar loop.
    Both produce a byte-identical Tensor with the same shape and dtype; only the
    copy instruction differs. MEASURED on block 0 (86 keys, 737.75 MiB):
        scalar byte loop  964.90 ms/block   (0.75 GB/s)  = 89.2% of the load
        sys_memcpy         47.29 ms/block  (15.23 GB/s)
    i.e. 917.6 ms/block, x96 materialisations/step.

    The 6 F32 scale_shift tables genuinely need the F32->BF16 cast and keep the
...
struct LTX2AVBlockWeights(Movable):
    ...
    def _linear_lora_delta_slab(
        ...
        """StepSlab variant of `_linear_lora_delta` (this file :1114) —
        byte-identical (same A/B GEMMs, same scale+add); only alloc source
        changes."""
        var out = base_out^
        for i in range(len(self.lora_names)):
            if self.lora_names[i] == w_key:
                var down = linear_slab(x, self.lora_a[i][], None, ctx, slab)
                var up = linear_slab(down, self.lora_b[i][], None, ctx, slab)
                var scaled = mul_scalar_slab(up, self.lora_scales[i], ctx, slab)
                out = add_slab(out, scaled, ctx, slab)
        return out^
    ...
    def _linear_b_slab(
        ...
        """StepSlab variant of `_linear_b` (this file :1133) — BF16/F32 dequant
        path only. The LTX-2 training stack loads via from_fp8_block (weights
        BF16, no resident-fp8; MEASURED ltx2_weight_dtype_probe), so the
        F8_E4M3 branch is unreachable under the slab/capture path and there is
        no linear_fp8_slab: it fails loud. The bias clone routes through
        reshape_slab (byte-identical D2D copy, capture-safe)."""
        ref w = self._w(w_key)
        ref b = self._w(b_key)
        if w.dtype() == STDtype.F8_E4M3:
            raise Error(
                String("LTX2AV._linear_b_slab: '") + w_key
                + "' is F8_E4M3 resident-fp8; the slab/capture path is BF16-only"
                + " (no linear_fp8_slab). Run without resident-fp8 mode"
                + " (from_fp8_block), or build+gate linear_fp8_slab."
            )
        var bias_clone = reshape_slab(b, b.shape(), ctx, slab)
        var out = linear_slab(x, w, Optional[Tensor](bias_clone^), ctx, slab)
        return self._linear_lora_delta_slab(x, w_key, out^, ctx, slab)
...
# ═════════════════════════════════════════════════════════════════════════════
# StepSlab twins of the LTX-2 video-block helpers (autograd_v2 contract C8).
# Byte-identical math (same ops, same order); ONLY the allocation source changes
# (each base op -> its *_slab twin). Threaded by _av_attention_train_slab /
# ltx2_video_block_{train_forward,backward}_slab.
# ═════════════════════════════════════════════════════════════════════════════
def _rms_norm_no_affine_slab(
    x: Tensor, eps: Float32, ctx: DeviceContext, mut slab: StepSlab
) raises -> Tensor:
    """StepSlab variant of `_rms_norm_no_affine` (this file :440). The all-ones
    affine weight is filled ON-DEVICE from the slab (full_device_slab — no host
    upload, no off-slab alloc) so the captured region stays alloc-clean; 1.0 is
    exact in bf16/f32 so it is byte-identical to the from_host ones."""
    var d = x.shape()[len(x.shape()) - 1]
    var w = full_device_slab(_shape1(d), Float32(1.0), x.dtype(), ctx, slab)
    return rms_norm_slab(x, w, eps, ctx, slab)


def _modulate_bc_slab(
    x: Tensor, scale: Tensor, shift: Tensor, ctx: DeviceContext, mut slab: StepSlab
) raises -> Tensor:
    """StepSlab variant of `_modulate_bc` (this file :1174)."""
    var one_plus = add_scalar_slab(scale, Float32(1.0), ctx, slab)
    return add_slab(mul_slab(x, one_plus, ctx, slab), shift, ctx, slab)


def _ada_row_pertok_slab(
    table: Tensor, temb: Tensor, idx: Int, dim: Int, n: Int, ctx: DeviceContext,
    mut slab: StepSlab,
) raises -> Tensor:
    """StepSlab variant of `_ada_row_pertok` (this file :1183)."""
    var trow = slice_slab(table, 0, idx, 1, ctx, slab)
    var trow3 = reshape_slab(trow, _shape3(1, 1, dim), ctx, slab)
    var tchunk = slice_slab(temb, 2, idx * dim, dim, ctx, slab)
    return add_slab(trow3, tchunk, ctx, slab)


def _rms_norm_opt_slab(
    x: Tensor,
    weights: LTX2AVBlockWeights,
    w_key: String,
    eps: Float32,
    ...
    """StepSlab variant of `_rms_norm_opt` (this file :1159)."""
    if weights._has(w_key):
        return rms_norm_slab(x, weights._w(w_key), eps, ctx, slab)
    return _rms_norm_no_affine_slab(x, eps, ctx, slab)


def _kv_modulate_slab(
    context: Tensor,
    psst: Tensor,
    prompt_ts: Tensor,
    seq: Int, dim: Int, ctx: DeviceContext,
    ...
    """StepSlab variant of `_kv_modulate` (this file :1270)."""
    var shift_row = reshape_slab(
        slice_slab(psst, 0, 0, 1, ctx, slab), _shape3(1, 1, dim), ctx, slab)
    var scale_row = reshape_slab(
        slice_slab(psst, 0, 1, 1, ctx, slab), _shape3(1, 1, dim), ctx, slab)
    var pt_shift = slice_slab(prompt_ts, 2, 0, dim, ctx, slab)
    var pt_scale = slice_slab(prompt_ts, 2, dim, dim, ctx, slab)
    var shift_kv = add_slab(shift_row, pt_shift, ctx, slab)
    var scale_kv = add_slab(scale_row, pt_scale, ctx, slab)
    return _modulate_bc_slab(context, scale_kv, shift_kv, ctx, slab)
