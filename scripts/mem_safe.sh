...
# desktop session before a large child scope reaches MemoryMax. Use the
# passwordless rootless scripts/mem_safe_runtime.sh wrapper for image/model
# runtimes or any larger job.
#
...
if (( max_bytes > safe_user_bytes )); then
  ...
  echo "mem_safe: large runtimes must use scripts/mem_safe_runtime.sh" >&2
  exit 78
fi
