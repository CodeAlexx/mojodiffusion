...
def _case_delta_zero_bf16_init(ctx: DeviceContext) raises:
    ...
    print("[dora-device] SerenityTrainer BF16 delta-zero init shortcut")
    var dev = dora_device_from_host(d, ctx)
    if dev.m[].dtype() != STDtype.BF16:
    ...
    var y_ref = dora_substitution_forward(x.copy(), W.copy(), d_ref, M)
    _check(
        String("SerenityTrainer BF16 delta-zero init forward"),
    ...
    _check(
        String("SerenityTrainer BF16/F32 delta-zero init forward"),
    ...
    _check(
        String("SerenityTrainer F32/BF16 delta-zero init forward"),
...
def main() raises:
    ...
    _case(ctx, String("SerenityTrainer per-input magnitude"), False, STDtype.F32)
    _case(ctx, String("lycoris per-output magnitude"), True, STDtype.F32)
    _case(ctx, String("SerenityTrainer BF16 boundary"), False, STDtype.BF16)
