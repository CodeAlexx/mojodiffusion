...
# adapter's d_A/d_B, supports an AdamW step + a SerenityTrainer raw-key save across
...
#   * training/{train_step, lora_save} : LoraAdapter, _lora_adamw, save_lora_serenity_trainer.
#
...
from serenitymojo.ops.norm_backward import group_norm_backward_dx, layer_norm_backward_dx
...
# blocks (diffusers Transformer2DModel proj_in/proj_out -> SerenityTrainer
...
def _basic_block_lora_backward[
    ...
    # frozen LN3 weight -> d_x only (skips the discarded d_g/d_b param reductions);
    # weight widened to the activation dtype exactly like the full call's mixed path.
    var d_ln3 = layer_norm_backward_dx(
        d_x3n, _d(acts.x_after2, ctx),
        cast_tensor_if_needed(_d(w.norm3_w, ctx), acts.x_after2[].dtype(), ctx),
        LN_EPS, ctx)
    var d_x_after2 = add(d_out.clone(ctx), d_ln3, ctx)   # FF-resid + LN3 paths
    ...
    var d_ln2 = layer_norm_backward_dx(
        g_a2.d_xq.clone(ctx), _d(acts.x_after1, ctx),
        cast_tensor_if_needed(_d(w.norm2_w, ctx), acts.x_after1[].dtype(), ctx),
        LN_EPS, ctx)
    var d_x_after1 = add(d_x_after2, d_ln2, ctx)    # resid + LN2 path
    ...
    var d_ln1 = layer_norm_backward_dx(
        d_x1n, _d(acts.x_in, ctx),
        cast_tensor_if_needed(_d(w.norm1_w, ctx), acts.x_in[].dtype(), ctx),
        LN_EPS, ctx)
    var d_x = add(d_x_after1, d_ln1, ctx)           # resid + LN1 path
...
def sdxl_st_lora_backward[
    ...
    # frozen GN weight (not a LoRA target) -> d_x only.
    var d_gn = group_norm_backward_dx(
        d_xn, _d(acts.x, ctx),
        cast_tensor_if_needed(_d(w.gn_w, ctx), acts.x[].dtype(), ctx),
        G, GN_EPS_ST, ctx)
    var d_x = add(go.clone(ctx), d_gn, ctx)         # residual + GroupNorm path
...
# Hot path (SDXL_GPU_ADAMW=True): ONE fused GPU launch over the flat block
# adapters (+ one tiny launch per ST-level proj adapter, when present) with the
# IDENTICAL plain-AdamW math (training/lora_adamw_plain_fused.mojo mirrors
# _adamw_host_list; gate training/lora_adamw_plain_fused_parity.mojo — host F32
# vs device F32 is a ±1-ulp FMA-contraction class, NOT bit-exact; see
# models/sdxl/parity/sdxl_gpu_adamw_parity_smoke.mojo for THIS function's gate).
# Host scalar loop kept below (sdxl_lora_adamw_step_unfused) as the gate
# reference/fallback — the zimage ZIMAGE_FUSED_ADAMW pattern.
comptime SDXL_GPU_ADAMW = True
...
def sdxl_lora_adamw_step(
    ...
    comptime if SDXL_GPU_ADAMW:
        var n = set.num_blocks * SDXL_SLOTS
        fused_lora_adamw_plain_step(
            set.ad, grads.d_a, grads.d_b, 0, n,
            t, lr, beta1, beta2, eps, weight_decay, ctx,
        )
        # ST-level proj_in/proj_out adapters (when present): same fused kernel,
        # one 1-adapter launch each (tiny [C,rank] tensors; copy-out/copy-back
        # mirrors the host path's Optional handling).
        if set.proj_in_ad and len(grads.d_proj_in_a) > 0:
            var pi = List[LoraAdapter]()
            pi.append(set.proj_in_ad.value().copy())
            var pi_da = List[List[Float32]]()
            pi_da.append(grads.d_proj_in_a.copy())
            var pi_db = List[List[Float32]]()
            pi_db.append(grads.d_proj_in_b.copy())
            fused_lora_adamw_plain_step(
                pi, pi_da, pi_db, 0, 1,
                t, lr, beta1, beta2, eps, weight_decay, ctx,
            )
            set.proj_in_ad = Optional[LoraAdapter](pi[0].copy())
        if set.proj_out_ad and len(grads.d_proj_out_a) > 0:
            var po = List[LoraAdapter]()
            po.append(set.proj_out_ad.value().copy())
            var po_da = List[List[Float32]]()
            po_da.append(grads.d_proj_out_a.copy())
            var po_db = List[List[Float32]]()
            po_db.append(grads.d_proj_out_b.copy())
            fused_lora_adamw_plain_step(
                po, po_da, po_db, 0, 1,
                t, lr, beta1, beta2, eps, weight_decay, ctx,
            )
            set.proj_out_ad = Optional[LoraAdapter](po[0].copy())
    else:
...
# The original host scalar loop (reuses the proven per-adapter _lora_adamw).
# RETAINED as the SDXL_GPU_ADAMW=False fallback and the parity-gate reference.
def sdxl_lora_adamw_step_unfused(
    mut set: SdxlLoraSet, grads: SdxlStLoraGrads, t: Int, lr: Float32,
    ctx: DeviceContext,
    beta1: Float32 = Float32(0.9), beta2: Float32 = Float32(0.999),
    eps: Float32 = Float32(1.0e-8), weight_decay: Float32 = Float32(0.01),
...
# ── RESIDENT AdamW arm (MJ-1087) ──────────────────────────────────────────────
# The per-step fused arm above (sdxl_lora_adamw_step -> fused_lora_adamw_plain_step)
# allocates FRESH PINNED host staging (host_p/host_g/host_m/host_v) on EVERY step.
# Under a large resident pinned base (fp8 UNet + LoRA), MAX can hand back an
# UNMAPPED pinned buffer without raising -> the MJ-1070/MJ-1085 segv class. The
# resident arm (krea2/zimage/sd35 production pattern) allocates device P/M/V AND
# the pinned G-staging ONCE at init; per step only grads go up and params come
# back. No per-step pinned allocation = that crash mechanism cannot recur.
#
# SDXL wrinkle: the trainer holds N_ST=11 SEPARATE SdxlLoraSet objects (each its
# own flat `.ad`), so there is ONE resident state PER set. `LoraAdamWPlainDeviceState`
# is move-only (NOT Copyable) and Mojo's List requires Copyable+Movable, so a bare
# List[Optional[LoraAdamWPlainDeviceState]] fails to compile — the state is wrapped
# in an ArcPointer (Copyable+Movable) so List[Optional[StateArc]] is legal. The Arc
# shares one heap cell, so `arc[]` mutates THE state (writeback-free).
#
# The state covers only the block adapters `set.ad[0:n]` (n = num_blocks*SDXL_SLOTS)
# — the ~28M-element bulk that was the large pinned staging. The tiny ST-level
# proj_in/proj_out adapters (Optional, usually absent) go through the host-scalar
# `_lora_adamw` (ZERO pinned staging, bit-identical to the unfused reference) so
# NO per-step pinned allocation survives anywhere in the resident arm.
comptime StateArc = ArcPointer[LoraAdamWPlainDeviceState]

# Resident arm ON by default for the plain-LoRA optimizer (MJ-1087). The trainer
# holds a List[Optional[StateArc]] via sdxl_lora_adamw_states_new and steps it
# with sdxl_lora_adamw_step_all_resident; SDXL_GPU_ADAMW's per-step fused arm is
# retired to the parity-gate reference role.
comptime SDXL_RESIDENT_ADAMW = True


# Can this set's block grads drive a bulk resident step? (all n block adapters
# present with grad lens matching the adapter param lens, and > 0 total elems).
# When False the caller falls back to the host-scalar path, preserving the old
# skip-empty semantics.
def _sdxl_set_grads_resident_ok(set: SdxlLoraSet, grads: SdxlStLoraGrads) -> Bool:
    var n = set.num_blocks * SDXL_SLOTS
    ...
    for i in range(n):
        ...
        if len(grads.d_a[i]) != n_a or len(grads.d_b[i]) != n_b:
            return False
        total += n_a + n_b
    return total > 0


# ONE persistent device AdamW state for a set's block adapters (set.ad[0:n]).
# dev_m/dev_v are seeded from the adapters' current host ma/va, so a lazy build
# at ANY step captures the correct moments.
def sdxl_lora_adamw_state_init(
    set: SdxlLoraSet, ctx: DeviceContext
) raises -> LoraAdamWPlainDeviceState:
    var n = set.num_blocks * SDXL_SLOTS
    return lora_adamw_plain_device_state_init(set.ad, 0, n, ctx)


# Per-set resident step: block adapters through the persistent device P/M/V
# (params synced back to set.ad each step by the resident helper), proj adapters
# through the host-scalar loop. Same PLAIN-AdamW math as sdxl_lora_adamw_step.
def sdxl_lora_adamw_step_resident(
    ...
    # ST-level proj_in/proj_out adapters (when present): host-scalar (no pinned
    # staging, bit-identical to sdxl_lora_adamw_step_unfused).
    if set.proj_in_ad and len(grads.d_proj_in_a) > 0:
        var pi = set.proj_in_ad.value().copy()
        var lg_pi = LoraGrads(grads.d_proj_in_a.copy(), grads.d_proj_in_b.copy())
        _lora_adamw(pi, lg_pi, t, lr, ctx, beta1, beta2, eps, weight_decay)
...
# Build the empty per-set resident-state list (one Optional slot per ST set, all
# None). The trainer holds this across steps and passes it to
# sdxl_lora_adamw_step_all_resident, which lazily fills each slot on first use.
def sdxl_lora_adamw_states_new(n_sets: Int) -> List[Optional[StateArc]]:
    var states = List[Optional[StateArc]]()
    for _ in range(n_sets):
        states.append(Optional[StateArc](None))
    return states^


# Step ONE ST set (index `set_idx` in `states`) with the resident arm. The trainer
# holds `states` (from sdxl_lora_adamw_states_new) across steps and calls this once
# per set inside its per-step set loop — grads are produced and consumed per set
# (SdxlStLoraGrads is move-only, so it can NOT live in a List; it is borrowed here).
# On first use for a set the device state is lazily built; thereafter it persists.
# A set whose block grads do not line up (has-empty-grads) drops any stale state
# and falls back to the host-scalar unfused path, preserving skip-empty semantics.
def sdxl_lora_adamw_step_set_resident(
    mut states: List[Optional[StateArc]],
    set_idx: Int,
    mut set: SdxlLoraSet,
    grads: SdxlStLoraGrads,
    t: Int, lr: Float32, ctx: DeviceContext,
    beta1: Float32 = Float32(0.9), beta2: Float32 = Float32(0.999),
    eps: Float32 = Float32(1.0e-8), weight_decay: Float32 = Float32(0.01),
) raises:
    if set_idx < 0 or set_idx >= len(states):
        raise Error("sdxl_lora_adamw_step_set_resident: set_idx out of range")
    if _sdxl_set_grads_resident_ok(set, grads):
        if not states[set_idx]:
            var st = sdxl_lora_adamw_state_init(set, ctx)
            states[set_idx] = Optional[StateArc](StateArc(st^))
        var arc = states[set_idx].value().copy()   # shared Arc -> same state cell
        sdxl_lora_adamw_step_resident(
            arc[], set, grads, t, lr, ctx, beta1, beta2, eps, weight_decay,
        )
    else:
        # drop any stale resident state, fall back to host-scalar for this set.
        states[set_idx] = Optional[StateArc](None)
        sdxl_lora_adamw_step_unfused(
            set, grads, t, lr, ctx, beta1, beta2, eps, weight_decay,
        )


# -- per-block SerenityTrainer raw legacy prefix scheme ----------------------------
# SerenityTrainer's SDXL LoRAModuleWrapper installs adapters on every Linear/Conv2d
...
# that bounded surface explicit: implemented ST linears save as reference trainer raw keys,
...
# matching SerenityTrainer raw module prefixes, e.g.
#   lora_unet_down_blocks_1_attentions_0_transformer_blocks_0_attn1_to_q
# and save_lora_serenity_trainer appends .alpha/.lora_down.weight/.lora_up.weight.
...
def sdxl_lora_supported_unet_prefixes(st_prefix: String, num_blocks: Int) -> List[String]:
    """Implemented SerenityTrainer SDXL UNet LoRA module prefixes.

    This is intentionally identical to `sdxl_lora_prefixes`, but named for the
    contract checker/smokes so the supported reference trainer save surface is source-visible.
    """
    return sdxl_lora_prefixes(st_prefix, num_blocks)


def sdxl_lora_unsupported_serenity_trainer_targets() -> List[String]:
    """SerenityTrainer SDXL LoRA targets not yet implemented by this Mojo carrier."""
    var out = List[String]()
...
def sdxl_lora_requires_text_encoder_surface(enable_te1: Bool, enable_te2: Bool) raises:
    if enable_te1 or enable_te2:
        raise Error(
            "SDXL SerenityTrainer LoRA save requested text-encoder adapters "
...
# -- SAVE every adapter as a SerenityTrainer raw-keyed safetensors -----------------
def save_sdxl_lora(
    set: SdxlLoraSet, st_prefix: String, path: String, ctx: DeviceContext
) raises -> Int:
    return save_lora_serenity_trainer(_sdxl_named_loras(set, st_prefix), path, ctx)
