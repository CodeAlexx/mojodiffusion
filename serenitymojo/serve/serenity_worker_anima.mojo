# serenitymojo.serve.serenity_worker_anima — STANDALONE per-kind Anima worker.
#
...
# newline-JSON IPC loop, but driving the REAL AnimaBackend (GPU/MAX Anima pipeline
# — RUNTIME Qwen3-0.6B + net.llm_adapter text encode of params.prompt/.negative +
# Anima DiT 30-step direct-velocity Euler CFG denoise + Qwen-Image VAE decode).
# The Rust control plane (serenity-server) spawns one of these per resident model,
# identically to the sdxl/zimage/qwenimage/ideogram4 workers:
#   serenity_worker_anima <fd>
#
# !!! DO NOT BUILD THIS FILE INTERACTIVELY !!!
# Like the other per-kind workers, this entrypoint imports the full Anima GPU+MAX
...
# `pixi run build-worker-anima-raw` (--optimization-level 2); never build it from
# an interactive session.
#
# Invoked:  serenity_worker_anima <fd>
#   <fd> = inherited AF_UNIX socket fd (decimal), exactly as the sibling workers.
#
...
from serenitymojo.serve.anima_backend import AnimaBackend
...
def _anima_worker_loop(mut backend: AnimaBackend, fd: Int32) raises:
    """Identical control flow to the sdxl/zimage/qwenimage/ideogram4 worker loops,
    specialized to AnimaBackend. One job at a time (single-GPU contract)."""
    set_nonblock(fd)
    ...
    while True:
        ...
        while True:
            ...
            if terminal:
                ...
                # (mirrors the sdxl / zimage / qwenimage / ideogram4 worker trim).
                try:
...
def main() raises:
    var args = argv()
    if len(args) < 2:
        print("usage: serenity_worker_anima <fd>")
        return
    var fd = Int32(Int(String(args[1])))
    var b = AnimaBackend()
    _anima_worker_loop(b, fd)
