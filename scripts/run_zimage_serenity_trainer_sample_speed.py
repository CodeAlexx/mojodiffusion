#!/usr/bin/env python3
"""Run a timed SerenityTrainer Z-Image sampler reference.

This is a parity-evidence helper. It imports SerenityTrainer and executes the same
ZImageSampler logic, but writes explicit timing/VRAM metadata for the strict
Mojo gate. It does not modify SerenityTrainer and does not claim speed parity.
"""
...
DEFAULT_SAMPLE_FILE = Path("/home/alex/SerenityTrainer/training_samples/eri2_5prompts_1024.json")
DEFAULT_PROMPT_ID = "p1"
DEFAULT_OUT = Path("/home/alex/serenity_trainer-mojo/parity/zi_SERENITY_timed_1024.png")
DEFAULT_SPEED_JSON = Path("/home/alex/serenity_trainer-mojo/parity/zimage_sampler_speed.json")
...
def _load_model(args: argparse.Namespace) -> Any:
    ...
    if model_loader is None or model_setup is None:
        raise RuntimeError("could not create SerenityTrainer Z-Image loader/setup")
...
def _run_timed_serenity_trainer_sample(
    model: Any,
    *,
    prompt: str,
    negative: str,
    ...
    with torch.no_grad(), model.autocast_context:
        ...
        with _timed_stage(torch) as denoise_timer:
            for i, timestep in enumerate(timesteps):
                ...
                print(f"[reference-zimage-sample] denoise step={i + 1}/{len(timesteps)}")
...
def _write_speed_json(
    ...
    *,
    ...
    data = {
        "schema": "serenity.zimage.serenity_trainer_sampler_speed.v1",
        "accepted_speed_parity": False,
        "speed_parity_claim": "not claimed",
        "serenity_trainer": dict(serenity_trainer),
        "measurement": {
            "source": "SerenityTrainer ZImageSampler logic via scripts/run_zimage_serenity_trainer_sample_speed.py",
    ...
    data["serenity_trainer"]["peak_vram_mib"] = peak_vram_mib if peak_vram_mib is not None else 0
    data["serenity_trainer"]["artifact_paths"] = list(serenity_trainer.get("artifact_paths", [])) + [str(path)]
    path.parent.mkdir(parents=True, exist_ok=True)
    path.write_text(json.dumps(data, indent=2, sort_keys=True) + "\n", encoding="utf-8")
    print(f"[reference-zimage-sample] wrote speed JSON: {path}")


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Timed SerenityTrainer Z-Image sampler evidence helper.")
...
def main() -> int:
    ...
    try:
        ...
        serenity_trainer = _run_timed_serenity_trainer_sample(
            model,
            prompt=prompt,
            negative=negative,
            height=height,
    ...
    _write_speed_json(
        args.speed_json,
        serenity_trainer=serenity_trainer,
    ...
    print(
        "[reference-zimage-sample] PASS "
        f"steps={steps} denoise_s_per_step={serenity_trainer['denoise_seconds_per_step']:.6f} "
        f"vae_s={serenity_trainer['vae_decode_seconds']:.6f} peak_vram_mib={poller.peak_mib}"
    )
    return 0
