# training/oft_stack.mojo — SerenityTrainer-OFT via the shared (a,b) carrier (FULL
...
# y = x@Wᵀ + x@bᵀ = x@W_effᵀ (== oft_serenity_forward). VRAM-bound (r_eff=in) → preflight
...
# Gated vs oft_serenity_backward in tests/oft_carrier_parity.mojo.
...
from serenitymojo.training.oft_serenity_trainer import (
    oft_serenity_skew, oft_serenity_neumann_r, _neumann_backward,
)
...
def oft_serenity_carrier_r_eff(IN: Int) -> Int:
    return IN


# Materialize the FULL-delta carrier (a=I_in, b=W_eff-W). Carrier scale == 1.
def oft_serenity_carrier_adapter(
    ...
    for g in range(r):
        ...
        var q = oft_serenity_skew(vblk, b)
        var rg = oft_serenity_neumann_r(q, b)                 # [b,b]
        var base = g * b
        for o in range(OUT):
            for k in range(b):
...
def oft_serenity_chain_carrier_grads(
    ...
    if len(d_b_carrier) != OUT * IN:
        raise Error("oft_serenity_chain_carrier_grads: d_b numel mismatch")
...
# OFT trainable = the triu vec per slot (F32). NOTE: the SerenityTrainer-OFT triu-vec
...
def klein_oft_carrier_lists(
    ...
    for i in range(len(set.dbl)):
        if set.dbl_active[i]:
            ref sl = set.dbl[i]
            dbl.append(oft_serenity_carrier_adapter(sl.vec, sl.w, sl.in_f, sl.out_f, sl.b, sl.r))
        else:
    ...
    for i in range(len(set.sgl)):
        if set.sgl_active[i]:
            ref sl = set.sgl[i]
            sgl.append(oft_serenity_carrier_adapter(sl.vec, sl.w, sl.in_f, sl.out_f, sl.b, sl.r))
        else:
...
def klein_oft_chain_all(
    ...
    for i in range(len(set.dbl)):
        if set.dbl_active[i]:
            ref sl = set.dbl[i]
            dbl.append(oft_serenity_chain_carrier_grads(sl.vec, sl.w, dbl_d_a[i], dbl_d_b[i], sl.in_f, sl.out_f, sl.b, sl.r))
        else:
    ...
    for i in range(len(set.sgl)):
        if set.sgl_active[i]:
            ref sl = set.sgl[i]
            sgl.append(oft_serenity_chain_carrier_grads(sl.vec, sl.w, sgl_d_a[i], sgl_d_b[i], sl.in_f, sl.out_f, sl.b, sl.r))
        else:
...
def save_klein_oft(set: KleinOFTSet, path: String, ctx: DeviceContext) raises -> Int:
    """Save SerenityTrainer-format OFT modules as <prefix>.oft_R.weight.

    SerenityTrainer's OFTModule owns OFTRotationModule.weight with shape
