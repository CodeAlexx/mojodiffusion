# serenitymojo/pipeline/anima_prepare_serenity512.mojo
#
# ANIMA trainer prepare — produce a REAL cached training sample for train_anima_serenity.
#
# Chunk D wire 1 (TRAINING_PLAN_anima.md §D). This replaces the SYNTHETIC
...
#            here — the trainer applies scale_latents per reference trainer delta-2; we cache the
#            raw encoder mean, the same thing the reference trainer pipeline hands the setup).
#
# PATH:
...
#
# The lift to the reference trainer 5D [1,16,1,LH,LW] (vae_frame_dim) is conceptual: the trainer
...
#       serenitymojo/pipeline/anima_prepare_serenity512.mojo -o /tmp/anima_prepare_serenity512
#   /tmp/anima_prepare_serenity512 <image.png> <out_cache_dir>
...
def _image_to_nchw_m1p1(
    ...
    if w != IW or h != IH:
        raise Error(
            String("anima_prepare_serenity512: image must be ") + String(IW) + "x"
...
def main() raises:
    ...
    print("==== anima_prepare_serenity512 — REAL VAE encode for the trainer ====")
    ...
    print("VERDICT: real cache ready — point train_anima_serenity CACHE_DIR at", out_dir)
