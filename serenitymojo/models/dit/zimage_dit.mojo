...
def _padtok_kernel_bf16(
    ...
    total_len_w: Int32, dim_w: Int32, real_len_w: Int32,
):
    var total_len = Int(total_len_w)
    var dim = Int(dim_w)
...
struct NextDiT[HL: Int, WL: Int, CAPLEN: Int]:
    ...
    def _apply_pad_token(
        ...
        unsafe_ptr=Pointer[Scalar[DType.bfloat16], MutAnyOrigin](
            unsafe_from_address=Int(feat.buf.unsafe_ptr().bitcast[BFloat16]())
        ),
        runtime_layout=rl,
    )
            var P = LayoutTensor[DType.bfloat16, _DYN1, MutAnyOrigin](
        unsafe_ptr=Pointer[Scalar[DType.bfloat16], MutAnyOrigin](
            unsafe_from_address=Int(pad.buf.unsafe_ptr().bitcast[BFloat16]())
        ),
        runtime_layout=prl,
    )
            var O = LayoutTensor[DType.bfloat16, _DYN1, MutAnyOrigin](
        ...
        runtime_layout=rl,
            ...
            ctx.enqueue_function[_padtok_kernel_bf16](
                F, P, O, Int32(total_len), Int32(dim), Int32(real_len), grid_dim=grid, block_dim=_BLOCK
            )
        else:
            var F = LayoutTensor[DType.float32, _DYN1, MutAnyOrigin](
        unsafe_ptr=Pointer[Scalar[DType.float32], MutAnyOrigin](
            unsafe_from_address=Int(feat.buf.unsafe_ptr().bitcast[Float32]())
        ),
        runtime_layout=rl,
    )
            var P = LayoutTensor[DType.float32, _DYN1, MutAnyOrigin](
        unsafe_ptr=Pointer[Scalar[DType.float32], MutAnyOrigin](
            unsafe_from_address=Int(pad.buf.unsafe_ptr().bitcast[Float32]())
        ),
        runtime_layout=prl,
    )
            var O = LayoutTensor[DType.float32, _DYN1, MutAnyOrigin](
        ...
        runtime_layout=rl,
            ...
            ctx.enqueue_function[_padtok_kernel_f32](
                F, P, O, Int32(total_len), Int32(dim), Int32(real_len), grid_dim=grid, block_dim=_BLOCK
            )
        ctx.synchronize()
