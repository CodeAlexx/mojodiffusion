# LTX-2.5 prompt conditioner — pure Mojo, no Rust, no fixtures.
#
# Identical to ltx2_encode_prompt.mojo except the text tower: LTX-2.5 pairs the
# DiT with Gemma-4-12B, so this calls encode_gemma4_hidden_states_streamed
# (gated on Lightricks' OWN fine-tuned weights: all 49 states cos >= 0.999,
# worst 0.99991). Everything downstream is unchanged and already correct for
# 2.5: the 49-state concat is 49*3840 = 188160, and the projection tensors are
# looked up under the SAME names LTX-2.5 ships --
# `text_embedding_projection.{video,audio}_aggregate_embed` [4096,188160] /
# [2048,188160], both WITH biases.
#
# For 2.5 the projection lives INSIDE the text-encoder file, so pass that same
# path for BOTH the encoder arg and the checkpoint arg:
#   ltx25_encode_prompt <TE.safetensors> <tokenizer.json> <TE.safetensors> \
#       <out.safetensors> "<prompt>" "<negative>"
#
...
from serenitymojo.models.text_encoder.gemma4_ltx_streamed import (
    GEMMA4_HIDDEN as GEMMA_HIDDEN,
    GEMMA4_LAYERS as GEMMA_LAYERS,
    GEMMA4_MAX_TOKENS as GEMMA_MAX_TOKENS,
    Gemma4HiddenBatch as GemmaHiddenBatch,
...
comptime _LTX25_TOKENIZER_JSON_BYTES = 32169626
...
def _tokenize_ltx25_prompts(
    gemma_checkpoint: String,
    tokenizer_json: String,
    prompt: String,
    negative_prompt: String,
) raises -> List[List[Int]]:
    if tokenizer_json == "-" or tokenizer_json == "embedded":
        # Keep the mmap owner alive while its origin-bound bytes are parsed.
        # Qwen3Tokenizer owns all parsed tables after the factory returns.
        var checkpoint = SafeTensors.open(gemma_checkpoint)
        if not checkpoint.has_tensor("tokenizer_json"):
            raise Error(
                "LTX-2.5 embedded tokenizer: missing tokenizer_json tensor"
            )
        var info = checkpoint.tensor_info("tokenizer_json")
        if info.dtype != STDtype.U8:
            raise Error(
                String("LTX-2.5 embedded tokenizer: expected U8, got ")
                + info.dtype.name()
            )
        if len(info.shape) != 1 or info.shape[0] <= 0:
            raise Error(
                "LTX-2.5 embedded tokenizer: expected nonempty rank-1 tensor"
            )
        if info.shape[0] != info.size:
            raise Error(
                "LTX-2.5 embedded tokenizer: U8 shape/byte-count mismatch"
            )
        if info.size != _LTX25_TOKENIZER_JSON_BYTES:
            raise Error(
                String("LTX-2.5 embedded tokenizer: expected ")
                + String(_LTX25_TOKENIZER_JSON_BYTES) + String(" bytes, got ")
                + String(info.size)
            )
        var json_bytes = checkpoint.tensor_bytes("tokenizer_json")
        if len(json_bytes) != info.size:
            raise Error(
                "LTX-2.5 embedded tokenizer: mapped byte-count mismatch"
            )
        var tokenizer = Qwen3Tokenizer.from_json_bytes(json_bytes)
...
def encode_ltx2_prompts(
    ...
    var ids = _tokenize_ltx25_prompts(
        gemma_checkpoint, tokenizer_json, prompt, negative_prompt
    )

    var ctx = DeviceContext()
    print("LTX2_ACTIVITY loading Gemma text encoder")
    var hidden = encode_gemma4_hidden_states_streamed(
...
def main() raises:
    ...
    if len(args) != 7:
        raise Error(
            "usage: ltx25_encode_prompt GEMMA_TE TOKENIZER_JSON "
            "LTX_CHECKPOINT OUTPUT PROMPT NEGATIVE_PROMPT; "
            "TOKENIZER_JSON may be '-' or 'embedded'"
        )
    encode_ltx2_prompts(
