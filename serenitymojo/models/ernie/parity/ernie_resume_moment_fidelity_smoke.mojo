# ernie_resume_moment_fidelity_smoke.mojo — save/resume FULL-moment fidelity gate.
#
# The MJ-1077 / ERI-0217 class: a resume that reloads LoRA A/B but silently ZEROES
# the AdamW moments (ma/va/mb/vb) restarts momentum and corrupts the run. This gate
# proves — WITHOUT the base model, a cache, or a denoise step — that ernie's
# save/load LoRA STATE path carries every moment and param element-for-element.
#
# What it proves:
#   1. build_ernie_lora_set builds the flat 7×num_layers adapter carrier.
#   2. Seed KNOWN NONZERO A/B (BF16 storage) + ALL FOUR moments (F32) on EVERY
#      adapter with a distinct per-(adapter,element) pattern — a zeroed-moment
#      restart or a cross-wired moment (mb->ma etc.) would show up immediately.
#   3. save_ernie_lora_state -> load_ernie_lora_state restores A/B + m/v with
#      ZERO element mismatches across the whole set.
#   4. lora_train_state_has_moments detects the .state as moment-carrying.
#   5. The RAW SerenityTrainer resume (save_ernie_lora -> load_ernie_lora_resume)
#      correctly ZEROES moments (that path is weights-only by contract; the .state
#      path is the one that carries momentum across a resume).
...
#
# Bar: exact — every restored A/B (compared as BF16) and every restored moment must
...
#     serenitymojo/models/ernie/parity/ernie_resume_moment_fidelity_smoke.mojo \
#     -o /tmp/gate_ernie
...
comptime STATE_OUT = "/tmp/ernie_resume_fidelity.state"
comptime RAW_OUT = "/tmp/ernie_resume_fidelity.safetensors"
comptime BAD_PATH = "/tmp/ernie_resume_fidelity_missing.state"
...
# bf16- AND f32-exact, always NONZERO (multiple of 1/16 in [1/16, 17/16]); `salt`
# separates A / B / ma / va / mb / vb so a cross-wired restore is caught.
def _val(i: Int, j: Int, salt: Int) -> Float32:
    var k = ((i * 31 + j * 7 + salt) % 17) + 1
    return Float32(k) * Float32(0.0625)


def _seed(mut set: ErnieLoraSet):
...
# Compare a restored set element-for-element against the seed pattern. `expect_mv`
# False = the raw resume path (moments must be exactly ZERO). Returns mismatch #s.
def _check(set: ErnieLoraSet, expect_mv: Bool) -> Mism:
    var w_mism = 0   # A/B weight mismatches
    var m_mism = 0   # moment mismatches
    for i in range(len(set.ad)):
        for j in range(len(set.ad[i].a)):
...
def main() raises:
    print("=== ernie resume moment-fidelity gate ===")
    var ctx = DeviceContext()

    # Small synthetic stack: 2 layers, D=8, F=16, rank=2 -> 14 adapters.
    var num_layers = 2
    var lora = build_ernie_lora_set(num_layers, 8, 16, 2, Float32(4.0))
    _seed(lora)
    print("seeded", len(lora.ad), "adapters with nonzero A/B + m/v")

    # (1) FULL state round-trip: moments MUST survive.
    _ = save_ernie_lora_state(lora, String(STATE_OUT), ctx)
    var st = load_ernie_lora_state(num_layers, 2, Float32(4.0), String(STATE_OUT), ctx)
    ...
    var probes = ernie_lora_prefixes(num_layers)
    ...
    _ = save_ernie_lora(lora, String(RAW_OUT), ctx)
    var raw = load_ernie_lora_resume(num_layers, 2, Float32(4.0), String(RAW_OUT), ctx)
    ...
    var raised = False
    try:
        _ = load_ernie_lora_state(num_layers, 2, Float32(4.0), String(BAD_PATH), ctx)
    except:
        raised = True
    if not raised:
    ...
    print("=== PASS: ernie FULL-moment resume is element-exact ===")
