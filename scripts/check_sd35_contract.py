#!/usr/bin/env python3
"""Static guard for SD3.5/SD35 SerenityTrainer parity blockers.
...
SERENITY_TRAINER_MOJO = Path("/home/alex/serenity_trainer-mojo")

TARGET_MODEL_TYPE = "STABLE_DIFFUSION_35"
PLAIN_SD3_MODEL_TYPE = "STABLE_DIFFUSION_3"

CONFIGS = [
    SERENITY_TRAINER / "configs/sd35m_100step_baseline.json",
    SERENITY_TRAINER / "configs/sd35m_benchmark.json",
]
...
SD35_TRAIN = REPO / "trainer/src/serenity_trainer/trainer/train_sd35_real.mojo"
SD35_CACHE = REPO / "serenitymojo/training/prepare_sd35_cache.py"
SD35_METRICS = SERENITY_TRAINER / "output/sd35m_100step_baseline/metrics.json"
SD35_BASELINE_LORA = SERENITY_TRAINER / "output/sd35m_100step_baseline/lora_last.safetensors"
SD3_TRAIN_REF_CONTRACT = SERENITY_TRAINER_MOJO / "parity/sd3_train_ref_contract.json"
SD3_TRAIN_REF_BLOCKERS = SERENITY_TRAINER_MOJO / "parity/sd3_train_ref_blockers.json"
SD3_SAMPLER_HELPER_REF = SERENITY_TRAINER_MOJO / "parity/sd3_sampler_helper_ref.json"
SERENITY_MODEL_TYPE = SERENITY_TRAINER / "modules/util/enum/ModelType.py"
SERENITY_SD3_DATALOADER = SERENITY_TRAINER / "modules/dataLoader/StableDiffusion3BaseDataLoader.py"
...
def resolve_serenity_trainer_mojo_path(value: Any) -> Path | None:
    if not isinstance(value, str) or not value:
        return None
    path = Path(value)
    if path.is_absolute():
        return path
    return SERENITY_TRAINER_MOJO / path
...
def check_serenity_trainer_sd35_target(blockers: list[str]) -> None:
    enum_source = read(SERENITY_MODEL_TYPE)
    dataloader_source = read(SERENITY_SD3_DATALOADER)

    if f"{TARGET_MODEL_TYPE} = '{TARGET_MODEL_TYPE}'" not in enum_source:
        blockers.append(f"{SERENITY_MODEL_TYPE}: missing SD3.5 enum {TARGET_MODEL_TYPE}")
    if "def is_stable_diffusion_3_5" not in enum_source:
        blockers.append(f"{SERENITY_MODEL_TYPE}: missing explicit SD3.5 model-type helper")
    if f"return self == ModelType.{TARGET_MODEL_TYPE}" not in enum_source:
        blockers.append(f"{SERENITY_MODEL_TYPE}: SD3.5 helper does not target {TARGET_MODEL_TYPE}")
    ...
    if target_registration not in dataloader_source:
        blockers.append(f"{SERENITY_SD3_DATALOADER}: SD3.5 data loader is not registered")
    if plain_registration in dataloader_source:
        blockers.append(
            f"{SERENITY_SD3_DATALOADER}: plain SD3 data loader is registered; "
...
def check_reference_artifacts(blockers: list[str]) -> None:
    ...
    if isinstance(config_path, str) and config_path:
        ...
        if not resolved_config.is_absolute():
            resolved_config = SERENITY_TRAINER / resolved_config
    ...
    if status != "completed":
        blockers.append(
            f"{SD35_METRICS}: local SerenityTrainer baseline is not usable "
            f"(status={status!r}, steps={steps!r}, error={metrics.get('error')!r})"
        )
    elif steps <= 0:
        blockers.append(f"{SD35_METRICS}: local SerenityTrainer baseline completed with no training steps")
    elif requested_steps > 0 and steps < requested_steps:
        blockers.append(
            f"{SD35_METRICS}: local SerenityTrainer baseline stopped early "
    ...
    if not SD35_BASELINE_LORA.exists():
        blockers.append(f"{SD35_BASELINE_LORA}: missing local SerenityTrainer SD3.5 LoRA baseline artifact")
    elif SD35_BASELINE_LORA.stat().st_size <= 0:
        blockers.append(f"{SD35_BASELINE_LORA}: local SerenityTrainer SD3.5 LoRA baseline artifact is empty")
...
def check_train_ref_contract(blockers: list[str]) -> None:
    ...
    if policy.get("required_device") != "SerenityTrainer CUDA path":
        blockers.append(
            f"{SD3_TRAIN_REF_CONTRACT}: required_device={policy.get('required_device')!r}; "
            "expected SerenityTrainer CUDA path"
        )
    ...
    for field, label in [
        ...
        output_path = resolve_serenity_trainer_mojo_path(outputs.get(field))
        if output_path is None:
            blockers.append(f"{SD3_TRAIN_REF_CONTRACT}: missing default_outputs.{field}")
        elif not output_path.exists():
            blockers.append(f"{output_path}: missing SD3.5 SerenityTrainer CUDA {label} dump")
        elif output_path.stat().st_size <= 0:
            blockers.append(f"{output_path}: SD3.5 SerenityTrainer CUDA {label} dump is empty")
...
def check_train_ref_blockers(blockers: list[str]) -> None:
    ...
    if artifact.get("one_step_dump_produced") is not True:
        blockers.append(f"{SD3_TRAIN_REF_BLOCKERS}: no SerenityTrainer CUDA one-step train dump was produced")
...
def collect_blockers() -> list[str]:
    ...
    check_serenity_trainer_sd35_target(blockers)
    ...
    if '"latent_image"' not in train and "latent" in train:
        blockers.append(f"{SD35_TRAIN}: train path does not consume SerenityTrainer latent_image cache")
    if has_any(cache, ["dist.sample()", "lat = (lat -", "latent_scaled"]):
        blockers.append(f"{SD35_CACHE}: cache writer does not match SerenityTrainer latent mean/scale timing")
...
def build_readiness_report(blockers: list[str]) -> dict[str, Any]:
    ...
    return {
        ...
        "required_serenity_trainer_inputs": {
        ...
        "required_next_evidence": [
            "Regenerate the SerenityTrainer SD3.5 baseline configs with model_type STABLE_DIFFUSION_35.",
            "Run a completed SerenityTrainer 100-step SD3.5 LoRA baseline with loss, grad norm, speed, and saved LoRA artifact.",
            "Dump SerenityTrainer CUDA one-step SD3.5 train tensors plus LoRA adapter tensors for Mojo replay.",
