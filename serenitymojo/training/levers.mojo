...
from serenitymojo.ops.loss_fns import (
    LossGrad, mse_loss_grad, mae_loss_grad, huber_loss_grad, smooth_l1_loss_grad,
    min_snr_gamma_weight, mask_weights, masked_mse_loss_grad,
    masked_mae_loss_grad, masked_huber_loss_grad, masked_smooth_l1_loss_grad,
)
from serenitymojo.training.train_config import (
    TrainConfig, LOSS_FN_MSE, LOSS_FN_HUBER, LOSS_FN_SMOOTH_L1, LOSS_FN_MAE,
)
...
def levers_loss_grad(
    ...
    elif cfg.loss_fn == LOSS_FN_MAE:
        lg = mae_loss_grad(pred, target)
...
def levers_masked_loss_grad(
    ...
    Semantics (ops/loss_fns.mojo, torch-gated by masked_loss_parity.mojo):
      weights = clamp(mask, cfg.unmasked_weight, 1)   (SerenityTrainer
    ...
    if cfg.loss_fn == LOSS_FN_HUBER:
        ...
        lg = masked_mae_loss_grad(pred, target, w, norm)
    else:
        raise Error(
...
#     serenity_lr_for_optimizer_step) — SimpleTuner runs its LR scheduler for
...
# ── F32-MASTER levers optimizer (MJ-1112) ────────────────────────────────────
# The bf16 wrapper above casts each adapter's bf16 a/b -> F32, steps, and writes
# back (RNE, or SR for automagic3). For an F32-master trainer (ltx2 F32Lora) that
# bf16 round-trip is exactly the master-corrupting hop MJ-1108/MJ-1112 fixed. This
# variant runs the SAME dispatch on F32 lists IN PLACE — no _levers_bf16_to_f32,
# no _levers_writeback_bf16, no automagic3_writeback_bf16_sr (SR is MOOT on a true
# F32 master). All state (LeversOptimizerState) is F32/dtype-blind. Resume: the
# k!=1 guard fires until the LeversOptimizerState sidecar lands (owed follow-up).
struct F32AdapterView(Copyable, Movable):
    """A shape-carrying view of one F32 master adapter for the F32 optimizer +
    lazy_init. a=[rank,in_f], b=[out_f,rank] — MUTATED in place by the step."""
    var a: List[Float32]
    var b: List[Float32]
    var rank: Int
    var in_f: Int
...
def _levers_optimizer_lazy_init_f32(
    cfg: TrainConfig,
    views: List[F32AdapterView],
    k: Int,
    start: Int,
    end: Int,
    mut st: LeversOptimizerState,
) raises:
    """F32 twin of _levers_optimizer_lazy_init: reads shapes from the view's
    rank/in_f/out_f (F32Lora has no such fields). Same k!=1 resume fail-loud."""
    if st.initialized:
        if st.kind != cfg.optimizer or st.start != start or st.end != end:
            raise Error("levers_optimizer_step_f32: state kind/range changed mid-run")
        return
    if k != 1:
        raise Error(
            String("levers_optimizer_step_f32: first call at step ") + String(k)
            + String(" — levers optimizer state has no resume sidecar yet;")
            + String(" restart from step 1 or use optimizer=ADAMW to resume")
        )
    if start < 0 or end > len(views) or start >= end:
        raise Error("levers_optimizer_step_f32: bad adapter range")
    if cfg.optimizer == TRAIN_OPTIMIZER_ADAFACTOR:
        for i in range(start, end):
            st.ada.append(AdafactorState(views[i].rank, views[i].in_f))
            st.ada.append(AdafactorState(views[i].out_f, views[i].rank))
    elif cfg.optimizer == TRAIN_OPTIMIZER_SCHEDULE_FREE_ADAMW:
        for i in range(start, end):
            st.sf.append(AdamWScheduleFreeState(len(views[i].a)))
            st.sf.append(AdamWScheduleFreeState(len(views[i].b)))
    elif cfg.optimizer == TRAIN_OPTIMIZER_ADAMW_8BIT:
        st.a8_qmap_signed = adam8bit_create_dynamic_map(True)
        st.a8_qmap_unsigned = adam8bit_create_dynamic_map(False)
        for i in range(start, end):
            st.a8.append(Adam8bitState(len(views[i].a)))
            st.a8.append(Adam8bitState(len(views[i].b)))
    elif cfg.optimizer == TRAIN_OPTIMIZER_AUTOMAGIC3:
        var h = automagic3_clamp_h(AUTOMAGIC3_DEFAULT_H)
        st.auto3_ctl.init_lr(Float64(cfg.lr))
        st.auto3_ctl.seed_rng(cfg.seed ^ UInt64(0xA17031C3))
        for i in range(start, end):
            st.auto3.append(Automagic3State(views[i].rank, views[i].in_f, h))
            st.auto3.append(Automagic3State(views[i].out_f, views[i].rank, h))
    else:
        raise Error(
            String("levers_optimizer_step_f32: optimizer tag ")
            + String(cfg.optimizer) + String(" has no levers dispatch")
        )
    st.kind = cfg.optimizer
    st.start = start
    st.end = end
...
def levers_optimizer_step_host_f32(
    cfg: TrainConfig,
    mut views: List[F32AdapterView],
    d_a: List[List[Float32]],
    d_b: List[List[Float32]],
    k: Int,
    step_lr: Float32,
    ...
    """One F32-master levers optimizer step over views[start:end) A+B at trainer
    step k. Hyperparameter parsing + dispatch are IDENTICAL to
    levers_optimizer_step_host; the ONLY difference is the *_step fns run IN PLACE
    on the F32 view lists (no bf16 cast / writeback). d_a/d_b indexed by ABSOLUTE
    adapter index; state indexed 2*(i-start) (A) / +1 (B)."""
    _levers_optimizer_lazy_init_f32(cfg, views, k, start, end, st)
    if len(d_a) < end or len(d_b) < end:
        raise Error("levers_optimizer_step_f32: grads shorter than adapter range")
    ...
    if cfg.optimizer == TRAIN_OPTIMIZER_ADAFACTOR:
        ...
        for i in range(start, end):
            var idx = 2 * (i - start)
            adafactor_step_2d(
                views[i].a, d_a[i], st.ada[idx], lr, beta2_decay,
                Float64(-1.0), eps2, d, wd)
            adafactor_step_2d(
                views[i].b, d_b[i], st.ada[idx + 1], lr, beta2_decay,
                Float64(-1.0), eps2, d, wd)
    elif cfg.optimizer == TRAIN_OPTIMIZER_SCHEDULE_FREE_ADAMW:
        var k0 = st.sf_ctl.k
        if k0 != k - 1:
            raise Error(
                String("levers_optimizer_step_f32: schedule-free k desync (ctl.k=")
                + String(k0) + String(", trainer step=") + String(k) + String(")"))
        var lr = Float64(cfg.lr)
        var beta1 = Float64(cfg.beta1)
        var beta2 = Float64(cfg.beta2)
        var eps = Float64(cfg.eps)
        ...
        for i in range(start, end):
            var idx = 2 * (i - start)
            adamw_schedulefree_step_param(
                views[i].a, d_a[i], st.sf[idx], k0, lr, beta1, beta2, eps, wd,
                warmup, True)
            adamw_schedulefree_step_param(
                views[i].b, d_b[i], st.sf[idx + 1], k0, lr, beta1, beta2, eps, wd,
                warmup, True)
        st.sf_ctl.end_step(adamw_schedulefree_adjusted_lr(k0, lr, beta2, warmup))
    elif cfg.optimizer == TRAIN_OPTIMIZER_ADAMW_8BIT:
        for i in range(start, end):
            var idx = 2 * (i - start)
            adamw8bit_step(
                views[i].a, d_a[i], st.a8[idx],
                st.a8_qmap_signed, st.a8_qmap_unsigned, k,
                step_lr, cfg.beta1, cfg.beta2, cfg.eps, cfg.weight_decay)
            adamw8bit_step(
                views[i].b, d_b[i], st.a8[idx + 1],
                st.a8_qmap_signed, st.a8_qmap_unsigned, k,
                step_lr, cfg.beta1, cfg.beta2, cfg.eps, cfg.weight_decay)
    elif cfg.optimizer == TRAIN_OPTIMIZER_AUTOMAGIC3:
        var beta2 = AUTOMAGIC3_DEFAULT_BETA2
        ...
        for i in range(start, end):
            var idx = 2 * (i - start)
            # F32 master -> automagic3_step_2d writes the F32 result in place;
            # NO SR writeback (SR is only for the bf16 truncation, moot on F32).
            automagic3_step_2d(
                views[i].a, d_a[i], st.auto3[idx], beta2, eps, clip, wd,
                st.auto3_ctl)
            automagic3_step_2d(
                views[i].b, d_b[i], st.auto3[idx + 1], beta2, eps, clip, wd,
                st.auto3_ctl)
        st.auto3_ctl.apply_vote()
    else:
        raise Error(
...
def levers_optimizer_step(
    ...
    """The single trainer call for the T1.C optimizer lever: host optimizer
...
# the T1.A-T1.F module as landed; ONLY this reference trainer-state resident sync is new.
#
# klein's v2 engine keeps its live bf16 LoRA params in TWO reference trainer-semantics
# LoraAdamWSerenityDeviceState buffers (dbl + sgl; training/lora_adamw_serenity_fused.mojo
...
# inverse of the resident reference trainer step's P readback
# (fused_lora_adamw_serenity_step_resident's host_p<-dev_p + memcpy-out,
# lora_adamw_serenity_fused.mojo). Correctness over speed: one host_p pack + one
...
# ══════════════════════════════════════════════════════════════════════════════
from serenitymojo.training.lora_adamw_serenity_fused import LoraAdamWSerenityDeviceState


def levers_optimizer_sync_resident_serenity(
    mut opt_state: LoraAdamWSerenityDeviceState,
    adapters: List[LoraAdapter],
    ctx: DeviceContext,
) raises:
    """reference trainer-state sibling of levers_optimizer_sync_resident: pack the
    host-stepped bf16 a/b of the WHOLE adapter list into the pinned host_p
    mirror (A then B per adapter, flat — the lora_adamw_serenity_device_state_init
    layout) and upload ONE H2D into the live dev_p that the device LoRA
    views sub-buffer."""
    if 2 * len(adapters) != opt_state.nseg:
        raise Error(
            String("levers_optimizer_sync_resident_serenity: adapter list len ")
    ...
    for i in range(len(adapters)):
        ...
        if len(adapters[i].a) != n_a or len(adapters[i].b) != n_b:
            raise Error(
                String("levers_optimizer_sync_resident_serenity: adapter len")
