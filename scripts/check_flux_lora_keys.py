#!/usr/bin/env python3
"""Static/header guard for Flux.1-dev SerenityTrainer LoRA key parity."""
...
SERENITY_SETUP = SERENITY_TRAINER / "modules/modelSetup/FluxLoRASetup.py"
SERENITY_SAVER = SERENITY_TRAINER / "modules/modelSaver/flux/FluxLoRASaver.py"
SERENITY_SAVER_MIXIN = SERENITY_TRAINER / "modules/modelSaver/mixin/LoRASaverMixin.py"
SERENITY_CONVERTER = SERENITY_TRAINER / "modules/util/convert/lora/convert_flux_lora.py"
SERENITY_CONVERT_UTIL = SERENITY_TRAINER / "modules/util/convert/lora/convert_lora_util.py"
SERENITY_BASELINE = SERENITY_TRAINER / "output/flux1_100step_baseline/lora_last.safetensors"
...
FLUX_DEV_SERENITY_TRANSFORMER_ADAPTERS = 10 + 19 * 14 + 38 * 6
FLUX_DEV_MISSING_TRANSFORMER_ADAPTERS = (
    FLUX_DEV_SERENITY_TRANSFORMER_ADAPTERS - FLUX_DEV_SUPPORTED_TRANSFORMER_ADAPTERS
)
...
def _stack_specs() -> list[PrefixSpec]:
    return [
        PrefixSpec(
            ...
            "stack",
            False,
            f"lora_transformer.{serenity_module}",
        )
        for legacy_name, serenity_module, in_f, out_f in STACK_TARGETS
    ]
...
def _double_supported_specs(
    ...
    serenity_base = f"lora_transformer.transformer_blocks.{bi}."
    for legacy_name, serenity_suffix, in_f, out_f in _double_image_targets(d_model, f_mlp):
        specs.append(PrefixSpec(base + legacy_name, in_f, out_f, "double", True, serenity_base + serenity_suffix))
    for legacy_name, serenity_suffix, in_f, out_f in _double_text_targets(d_model, f_mlp):
        specs.append(PrefixSpec(base + legacy_name, in_f, out_f, "double", True, serenity_base + serenity_suffix))
    return specs
...
def _double_missing_specs(bi: int, d_model: int = DEFAULT_D) -> list[PrefixSpec]:
    ...
    return [
        ...
        PrefixSpec(
            ...
            "double_norm",
            False,
            serenity_base + "norm1_context.linear",
        ),
    ]
...
def _single_supported_specs(
    ...
    serenity_base = f"lora_transformer.single_transformer_blocks.{bi}."
    return [
        PrefixSpec(base + legacy_name, in_f, out_f, "single", True, serenity_base + serenity_suffix)
        for legacy_name, serenity_suffix, in_f, out_f in _single_targets(d_model, f_mlp)
    ]
...
def build_serenity_transformer_specs(
    num_double: int,
    num_single: int,
    d_model: int = DEFAULT_D,
    f_mlp: int = DEFAULT_FMLP,
...
def expected_specs_for_adapter_count(expected_adapters: int | None) -> list[PrefixSpec] | None:
    if expected_adapters == FLUX_DEV_SERENITY_TRANSFORMER_ADAPTERS:
        return build_serenity_transformer_specs(DEFAULT_NUM_DOUBLE, DEFAULT_NUM_SINGLE)
...
def check_expected_partition() -> None:
    ...
    full = build_serenity_transformer_specs(DEFAULT_NUM_DOUBLE, DEFAULT_NUM_SINGLE)
    ...
    if len(full) != FLUX_DEV_SERENITY_TRANSFORMER_ADAPTERS:
        raise SystemExit(
            "[flux-lora-keys] checker reference trainer inventory mismatch: "
            f"{len(full)} != {FLUX_DEV_SERENITY_TRANSFORMER_ADAPTERS}"
        )
    ...
    if supported_set | missing_set != full_set:
        raise SystemExit("[flux-lora-keys] supported+missing does not equal full reference trainer inventory")
...
def _print_grouped_missing_targets(num_double: int, num_single: int) -> None:
    ...
    print(
        "[flux-lora-keys] Missing reference trainer transformer targets by SerenityTrainer module path "
    ...
    for group in ("stack", "double_norm", "single_norm"):
        ...
        for spec in group_specs:
            print(
                "[flux-lora-keys]     "
                f"{spec.serenity_module} -> {spec.prefix} ({_shape_text(spec)})"
            )
...
def check_serenity_source() -> None:
    setup = read_text(SERENITY_SETUP)
    saver = read_text(SERENITY_SAVER)
    saver_mixin = read_text(SERENITY_SAVER_MIXIN)
    converter = read_text(SERENITY_CONVERTER)
    convert_util = read_text(SERENITY_CONVERT_UTIL)

    require(setup, "LoRAModuleWrapper", "reference trainer LoRA wrapper")
    require(setup, 'model.transformer, "lora_transformer"', "reference trainer transformer prefix")
    require(setup, 'model.text_encoder_1, "lora_te1"', "reference trainer text encoder 1 prefix")
    require(setup, 'model.text_encoder_2, "lora_te2"', "reference trainer text encoder 2 prefix")
    require(setup, "config.layer_filter.split", "reference trainer layer filter")
    require(setup, 'state_dict_has_prefix(model.lora_state_dict, "lora_te1")', "reference trainer TE1 resume trigger")
    require(setup, 'state_dict_has_prefix(model.lora_state_dict, "lora_te2")', "reference trainer TE2 resume trigger")
    ...
    print("[flux-lora-keys] SerenityTrainer source contract: PASS")
...
def check_saved_header(path: Path, expected_adapters: int | None, expected_rank: int | None) -> None:
    ...
    sample_keys = SAMPLE_SERENITY_KEYS
    ...
    print(
        "[flux-lora-keys] SerenityTrainer saved inventory: PASS "
...
def check_mojo_sources(strict_port: bool, report_missing: bool) -> None:
    ...
    require(lora_save, ".lora_down.weight", "shared reference trainer down suffix")
    require(lora_save, ".lora_up.weight", "shared reference trainer up suffix")
    require(lora_save, ".alpha", "shared reference trainer alpha suffix")
    require(stack, "flux_lora_serenity_transformer_prefixes", "Flux full reference trainer transformer inventory")
    require(stack, "flux_lora_missing_serenity_transformer_prefixes", "Flux missing reference trainer transformer inventory")
    require(stack, "require_flux_lora_serenity_transformer_complete", "Flux fail-loud transformer guard")
    require(stack, "require_flux_lora_text_encoder_disabled", "Flux fail-loud TE guard")
    require(stack, "lora_transformer_context_embedder", "reference trainer stack context embedder key")
    require(stack, "lora_transformer_x_embedder", "reference trainer stack x embedder key")
    require(stack, "lora_transformer_norm_out_linear", "reference trainer final norm modulation key")
    require(stack, "lora_transformer_proj_out", "reference trainer final projection key")
    require(stack, "time_text_embed_guidance_embedder_linear_1", "reference trainer guidance embedder key")
    require(stack, "time_text_embed_text_embedder_linear_1", "reference trainer pooled text embedder key")
    require(stack, "time_text_embed_timestep_embedder_linear_1", "reference trainer timestep embedder key")
    require(stack, "norm1_context_linear", "reference trainer double context norm modulation key")
    require(stack, "norm1_linear", "reference trainer double image norm modulation key")
    require(stack, "norm_linear", "reference trainer single norm modulation key")
    ...
    if "save_lora_serenity_trainer" not in stack:
        blockers.append("Mojo Flux save hook does not call save_lora_serenity_trainer")
    if "double_blocks." in stack or "single_blocks." in stack:
        blockers.append("Mojo Flux prefixes are BFL/local double_blocks/single_blocks, not SerenityTrainer lora_transformer_* keys")
    if "Flux LoRA transformer surface is not full SerenityTrainer parity" not in stack:
        blockers.append("Mojo Flux missing reference trainer transformer targets are not fail-loud")
    ...
    if blockers:
        ...
        print(
            ...
            f"serenity_transformer={FLUX_DEV_SERENITY_TRANSFORMER_ADAPTERS} "
...
def main() -> int:
    ...
    parser.add_argument(
        "--report-missing",
        action="store_true",
        help="Print the fail-loud missing Flux transformer targets grouped by SerenityTrainer module path.",
    )
    parser.add_argument("--safetensors", type=Path, default=SERENITY_BASELINE if SERENITY_BASELINE.exists() else None)
    parser.add_argument("--expected-adapters", type=int, default=504)
    parser.add_argument("--expected-rank", type=int, default=16)
    args = parser.parse_args()

    check_serenity_source()
