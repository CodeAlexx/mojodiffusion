...
from serenitymojo.training.oft_stack import (
    oft_serenity_carrier_adapter, oft_serenity_chain_carrier_grads,
)
...
# ── SerenityTrainer OFT full-delta carrier ────────────────────────────────────────
...
def anima_oft_carrier_set(set: AnimaOFTSet, D: Int, JOINT: Int, F: Int) raises -> AnimaLoraSet:
    ...
    for i in range(len(set.ad)):
        if set.active[i]:
            ref sl = set.ad[i]
            ad.append(oft_serenity_carrier_adapter(sl.vec, sl.w, sl.in_f, sl.out_f, sl.b, sl.r))
        else:
            var dims = anima_lycoris_slot_dims(i % ANIMA_SLOTS, D, JOINT, F)
            ad.append(_inactive_carrier(dims[0], dims[1]))
...
def anima_oft_chain_all(
    ...
    for i in range(len(set.ad)):
        if set.active[i]:
            ref sl = set.ad[i]
            out.append(oft_serenity_chain_carrier_grads(
