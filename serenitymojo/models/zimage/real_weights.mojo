...
# SerenityTrainer scheme (transformer_z_image.py patchify_and_embed + _pad_with_ids,
...
# scheme below is byte-exact SerenityTrainer. For a SHORTER caption in the same
# bucket (cap_padded < cap_len) the rows [cap_padded, cap_len) are SerenityTrainer's
# BATCH-PADDING rows, which SerenityTrainer attention-MASKS out of the unified
...
def build_positions(
    ...
    if real_cap < 0 or real_cap > cap_len:
        real_cap = cap_len
    # cap_padded = valid_cap rounded up to the next multiple of 32 (== SerenityTrainer
    ...
    for i in range(cap_len):
        var pl = List[Int]()
        if i < cap_padded:
            # SerenityTrainer: sequential (i+1,0,0) for the WHOLE padded caption block,
            # including the last-row-repeat rows in [valid_cap, cap_padded).
            pl.append(i + 1); pl.append(0); pl.append(0)
        else:
            # Rows [cap_padded, cap_len): SerenityTrainer batch-padding (masked out
            # there). Positioned (0,0,0); see header note on the masking gap.
            pl.append(0); pl.append(0); pl.append(0)
        cap_pos.append(pl^)
    # SerenityTrainer image offset = cap_len+1 where cap_len is the PADDED caption
