...
from serenitymojo.training.ltx2.config import (
    MODE_AV, PRESET_T2V, PRESET_AUDIO, PRESET_AUDIO_REF_ONLY_IC, LTX2TrainerConfig,
)
...
def run_acceptance(print_details: Bool = True) raises -> LTX2Readiness:
    ...
    _check(len(modules_per_block_for_preset(PRESET_AUDIO)) == 14, "audio modules per block (14)")
    _check(target_count_for_preset(PRESET_AUDIO) == 672, "audio LoRA target count (672)")
    _check(len(modules_per_block_for_preset(PRESET_AUDIO_REF_ONLY_IC)) == 18, "audio_ref_only_ic modules per block (18)")
    _check(target_count_for_preset(PRESET_AUDIO_REF_ONLY_IC) == 864, "audio_ref_only_ic LoRA target count (864)")
    # P6.0: audio cache-key contract (cache_records is the authority — _ltx2_audio
    # suffix + audio_latents_{T}x{F}x{C}; the latter checked above).
    _check(AUDIO_CACHE_SUFFIX == "_ltx2_audio.safetensors", "audio cache suffix contract")
    _check(
    ...
    if print_details:
        ...
        # Print the blocker ONLY when something is actually blocking. This line
        # used to print unconditionally, so it kept claiming the AV backward and
        # LoRA runtime were unwired for every run after they landed and gated.
        if not report.production_training_ready():
