...
def sd3_pos_crop_origin(max_grid: Int, crop_grid: Int) raises -> Int:
    """Diffusers PatchEmbed.cropped_pos_embed centered crop origin."""
    if max_grid <= 0 or crop_grid <= 0 or crop_grid > max_grid:
        raise Error("sd3_pos_crop_origin: invalid grid")
    return (max_grid - crop_grid) // 2


def sd3_pos_crop_source_token(
    max_grid: Int, crop_h: Int, crop_w: Int, row: Int, col: Int
) raises -> Int:
    """Flat source token selected by the centered rectangular position crop."""
    if row < 0 or row >= crop_h or col < 0 or col >= crop_w:
        raise Error("sd3_pos_crop_source_token: crop coordinate out of range")
    var top = sd3_pos_crop_origin(max_grid, crop_h)
    var left = sd3_pos_crop_origin(max_grid, crop_w)
    return (top + row) * max_grid + left + col


def sd3_lowmem_tile_start(full: Int, tile: Int, index: Int) raises -> Int:
    """Endpoint-balanced five-tile offset used by SD3's low-memory VAE path.

    Integer interpolation keeps starts monotonic and forces tile 4 to end exactly
    on the canvas even when (full - tile) is not divisible by four."""
    if full <= 0 or tile <= 0 or tile > full or index < 0 or index > 4:
        raise Error("sd3_lowmem_tile_start: invalid geometry")
