...
| `models/minimax_h3/h3_token_refiner_train.mojo` (2026-08-18) | Trainable two-block H3 token-refiner twin for the full AiToolkit topology. Runs qkv/out/fc1/fc2 LoRA before the main packed stack, chains d_text back from the main stack, and maps product FC1 `[value|gate]` compute gradients back to canonical PEFT `[gate|value]` before AdamW/save. | **GPU-GATED** (Torch BF16 autograd: output/d_x cos >0.99998; all eight A/B grads >0.99985; exact-recipe state resume step 1->2) |
