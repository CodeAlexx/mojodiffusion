...
from serenitymojo.ops.random import randn, rand_uniform
...
# A ~ REAL kaiming_uniform(a=sqrt(5)) = U(-1/sqrt(in), +1/sqrt(in)) — matching
# PEFT/torch nn.init.kaiming_uniform_(a=sqrt(5)) which reduces to that bound
# (gain=sqrt(2/(1+5)), std=gain/sqrt(in), bound=sqrt(3)*std=1/sqrt(in)). B = 0
# (PEFT identity). Previously this drew a Gaussian of matching std as an
# approximation; rand_uniform (same ChaCha stream as randn) makes it the correct
# DISTRIBUTION (cross-runtime bit-parity with torch's RNG is not achievable, but
# the family + bound now match the reference; the Rust port uses real kaiming too).
def make_lora_adapter(
    ...
    var a = rand_uniform(a_sh^, seed, STDtype.BF16, -bound, bound, ctx)
...
def make_loha_adapter(
    ...
    # reference trainer: nn.init.constant_(hada_w2_a, 0) on the UP [out,rank] factor;
...
def make_dora_adapter(
    ...
    var a_sh = List[Int](); a_sh.append(rank); a_sh.append(in_features)
    var b_sh = List[Int](); b_sh.append(out_features); b_sh.append(rank)
    # real kaiming_uniform(a=sqrt(5)) = U(-1/sqrt(in), +1/sqrt(in)) (see make_lora_adapter).
    var bound = Float32(1.0) / sqrt(Float32(in_features))
    var a = rand_uniform(a_sh^, seed, STDtype.BF16, -bound, bound, ctx)
    var b = zeros_device(b_sh^, STDtype.BF16, ctx)
