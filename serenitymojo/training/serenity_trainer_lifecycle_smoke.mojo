# serenity_trainer_lifecycle_smoke.mojo - SerenityTrainer GenericTrainer control smoke.
#
# Run:
#   timeout 180 prlimit --as=16000000000 pixi run mojo run -I . serenitymojo/training/serenity_trainer_lifecycle_smoke.mojo
...
def _check(cond: Bool, msg: String) raises:
    if not cond:
        raise Error(String("serenity_trainer_lifecycle_smoke FAILED: ") + msg)
...
def _gate_timed_actions() raises:
    ...
    var single_state = SerenityTimedActionState(Float64(0.0), Float64(10.0))
    _check(
...
def _gate_step_decisions() raises:
    ...
    var p = SerenityTrainProgress.zero()
    p.global_step = 49
    var validate_state = SerenityTimedActionState.zero()
    ...
    var stop_state = SerenityTimedActionState.zero()
...
def _expect_product_plan_raises(
    ...
    var raised = False
    try:
        var plan = create_serenity_trainer_product_run_plan(cfg)
...
def _gate_product_run_plan() raises:
    ...
    _check(plan.runner_kind == SERENITY_PRODUCT_RUNNER_LORA_REAL, "Qwen runner kind")
    ...
    _check(lifecycle.invocation_kind == SERENITY_MODEL_LOOP_INVOCATION_ABSENT, "Qwen direct invocation absent")
    _check(not lifecycle.direct_invocation_supported, "Qwen direct invocation blocked")
    _check(plan.sampler_plan.family == SERENITY_SAMPLER_QWEN, "Qwen sampler family")
    ...
    print(serenity_trainer_product_run_summary(plan))
    print(generic_trainer_lifecycle_summary(lifecycle))

    cfg.name = String("klein9b")
    plan = create_serenity_trainer_product_run_plan(cfg)
    validate_serenity_trainer_product_run_plan(plan)
    _check(plan.runner_name == String("train_klein_real"), "Klein runner name")
    _check(plan.sampler_plan.family == SERENITY_SAMPLER_FLUX2_KLEIN, "Klein/Flux2 sampler family")

    cfg.name = String("zimage")
    plan = create_serenity_trainer_product_run_plan(cfg)
    validate_serenity_trainer_product_run_plan(plan)
    _check(plan.runner_name == String("train_zimage_real"), "Z-Image runner name")
    _check(plan.sampler_plan.family == SERENITY_SAMPLER_ZIMAGE, "Z-Image sampler family")

    cfg.name = String("chroma")
    plan = create_serenity_trainer_product_run_plan(cfg)
    validate_serenity_trainer_product_run_plan(plan)
    _check(plan.runner_name == String("train_chroma_real"), "Chroma runner name")
    _check(plan.sampler_plan.family == SERENITY_SAMPLER_CHROMA, "Chroma sampler family")

    cfg.name = String("flux")
    plan = create_serenity_trainer_product_run_plan(cfg)
    validate_serenity_trainer_product_run_plan(plan)
    _check(plan.runner_name == String("train_flux_real"), "Flux runner name")
    _check(plan.sampler_plan.family == SERENITY_SAMPLER_FLUX1_DEV, "Flux sampler family")
    ...
    _check(plan.sampler_plan.family == SERENITY_SAMPLER_FLUX2_KLEIN, "FLUX_2 Klein sampler family")
...
def main() raises:
    print("==== SerenityTrainer lifecycle smoke ====")
    _gate_train_progress()
    _gate_timed_actions()
    _gate_step_decisions()
    _gate_product_run_plan()
    print("serenity_trainer_lifecycle_smoke PASS")
