...
from serenitymojo.ops.rope import rope_halfsplit, rope_halfsplit_slab
...
# _slab twin of apply_ltx2_rope: routes to rope_halfsplit_slab (byte-identical
# kernels, slab-owned output). C8 steady-state routing for the captured region.
def apply_ltx2_rope_slab(
    x: Tensor, cos: Tensor, sin: Tensor, ctx: DeviceContext, mut slab: StepSlab
) raises -> Tensor:
    return rope_halfsplit_slab(x, cos, sin, ctx, slab)
