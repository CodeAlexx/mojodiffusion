...
ZIMAGE_PRODUCT_TRAINER = REPO / "trainer/src/serenity_trainer/trainer/train_zimage_real.mojo"
KLEIN_PRODUCT_TRAINER = REPO / "trainer/src/serenity_trainer/trainer/train_klein_real.mojo"
...
REQUIRED_FILES = (
    ...
    "artifacts/training_perf/zimage_serenity_trainer_train_ref_blocked_2026-06-30.md",
    "artifacts/training_perf/zimage_serenity_trainer_input_dump_2026-06-30.md",
...
REQUIRED_TEXT = {
    "TRAINER_SPEED_ROADMAP_2026-06-30.md": (
        ...
        "reduced-depth torchref stack parity",
        ...
        "torchref resume oracle evidence",
        "--expect-match",
        "passes against the local torchref LoRA output",
        ...
        "LTMAX=896",
        "zimage_serenity_trainer_train_ref_blocked_2026-06-30.md",
        "zimage_serenity_trainer_input_dump_2026-06-30.md",
    ...
    "serenitymojo/models/zimage/parity/zimage_train_ref_loss_replay.mojo": (
        "ZImage SerenityTrainer train-ref loss bridge",
    ...
    "serenitymojo/models/zimage/parity/zimage_train_ref_device_loss_replay.mojo": (
        "ZImage SerenityTrainer train-ref device loss-root replay",
        "zimage_train_ref_step000.safetensors",
        "zimage_step_io_write_flow_mse_d_patches",
        ...
        "sync_count == 1",
        "real SerenityTrainer step0 dump through v5 device flow-MSE d_patches root",
    ...
    "scripts/check_adapter_update_replay.py": (
        "SerenityTrainer adapter phase/update oracle only",
    ...
    "scripts/check_update_bearing_readiness.py": (
        ...
        "update-bearing SerenityTrainer oracle evidence",
    ),
    "scripts/check_zimage_adapter_update_replay.py": (
    ...
    "scripts/check_zimage_adamw_update_replay.py": (
        "Replay ZImage SerenityTrainer step-1 AdamW",
        "adapter_post_clip",
        "adapter_post_clip_grad",
        "full CPU replay of SerenityTrainer step001 AdamW update",
        "not transformer forward/backward",
        "fused device optimizer parity",
        "19046400",
    ),
    "serenitymojo/models/zimage/parity/zimage_train_ref_adamw_update_replay.mojo": (
        "ZImage SerenityTrainer train-ref AdamW update replay",
        "adapter_post_clip",
        "sampled Mojo scalar AdamW replay",
    ...
    "serenitymojo/models/zimage/parity/zimage_train_ref_fused_adamw_update_replay.mojo": (
        "ZImage SerenityTrainer train-ref fused AdamW update replay",
        "DeviceTrainableSet",
        "DeviceGradSet",
        "DeviceAdamWState",
        "device_adamw_train_step_update",
    ...
    "serenitymojo/models/zimage/parity/zimage_train_ref_f32_adapter_carrier_smoke.mojo": (
        "ZImage SerenityTrainer train-ref adapter oracle metadata smoke",
        "boundaries stay BF16 in/out",
        "SerenityTrainer's live LoRA params",
        "STEP_DUMP",
        "STDtype.BF16",
        "adapter_before.",
        "adapter_post_clip_grad.",
    ...
    "serenitymojo/models/zimage/parity/zimage_train_ref_selected_grad_replay.mojo": (
        "ZImage SerenityTrainer train-ref selected-gradient replay preflight",
        "zimage_train_ref_step000.safetensors",
        "zimage_train_ref_step000_adapters.safetensors",
    ...
    "artifacts/training_perf/zimage_serenity_trainer_train_ref_blocked_2026-06-30.md": (
        "ZImage SerenityTrainer Train-Ref State-Init And AdamW Update Evidence",
        "eri2_zimage_base_2500.json",
        ...
        "zimage_train_ref_step001_adapters.safetensors",
        "evidence=state-init",
        "SerenityTrainer update-bearing adapter oracle",
    ...
    "artifacts/training_perf/zimage_serenity_trainer_input_dump_2026-06-30.md": (
        "ZImage SerenityTrainer Input Dump",
    ...
    "artifacts/training_perf/zimage_selected_grad_replay_blocker_2026-06-30.md": (
        ...
        "SerenityTrainer's ZImage base/train/output path is BF16",
        "live LoRA params are `FLOAT_32`",
        "Normal SerenityTrainer final LoRA export uses `output_dtype=BFLOAT_16`",
    ...
    "artifacts/training_perf/krea2_txtfusion_devicegrad_realcache_resume_smoke_2026-06-30.md": (
        ...
        "torchref full-surface loss/gradient/update parity",
        "KREA2_TXTFUSION_LORA",
        "-DKREA2_TXTFUSION_LORA=1",
        "-DKREA2_LTMAX=896",
    ...
    "artifacts/training_perf/krea2_stack_adamw_update_replay_2026-06-30.md": (
        "Krea2 Reduced-Depth torchref AdamW Update Replay",
        "reduced-depth torchref `SingleStreamDiT` block-stack gradient",
    ...
    "scripts/write_training_benchmark_collection_manifest.py": (
        ...
        "SerenityTrainer or torchref correctness lane",
        "Mojo current lane",
        ...
        "real-cache torchref loss/gradient/update/resume parity",
        ...
        "SerenityTrainer update-bearing adapter oracle collected",
        "zimage_train_ref_step001_adapters.safetensors",
    ...
    "artifacts/training_perf/klein_mojo_current_2026-06-30.md": (
        ...
        "visible-counter-lower-bound",
        "SerenityTrainer parity is not proven",
    ),
    "serenitymojo/configs/klein9b_scorecard_smoke.json": (
        '"model_type": "klein"',
...
def _validate_benchmark_matrix() -> bool:
    ...
    expected = (
        (0, 'String("krea2")', "PERF_LANE_TORCHREF", "1,", 'String("512 or 1024")'),
        (1, 'String("zimage")', "PERF_LANE_SERENITY_TRAINER", "2,", 'String("1024")'),
        (2, 'String("klein")', "PERF_LANE_SERENITY_TRAINER", "1,", 'String("1024")'),
        (3, 'String("sdxl")', "PERF_LANE_SERENITY_TRAINER", "1,", 'String("1024")'),
    )
...
def _validate_zimage_adapter_oracle_metadata() -> bool:
    ...
    if "zimage_serenity_trainer_f32_lora_adapter_to_device" in lora_block:
    ...
    for needle in (
        ...
        "replay_scale= 0.0625",
        "no device upload",
        "SerenityTrainer BF16 runtime/step boundary plus live LoRA dump metadata",
    ...
    if krea2_txtfusion_oracle.exists():
        ...
        for forbidden in (
            ...
            if forbidden in oracle_text:
                print("training speed roadmap contract: FAIL Krea2 txtfusion oracle must follow torchref BF16 config, not F32")
                print(f"  {forbidden!r}")
                return 1
