...
# Per-stream LoRA dropout salts for the 6 SEPARATE Serenity adapters
# (q,k,v,out,ff_in,ff_out). Serenity gives every LoRAModule its own dropout
...
# ── 1:1 Serenity SEPARATE-Linear LoRA adapters per stream ──────────────────
# Serenity/Flux2LoRASetup.py:57 wraps SEPARATE diffusers nn.Linear modules
...
#
#   diffusers (transformer_flux2.py)         Serenity wraps  → our slot
...
# = 48, the total is 96 + 48 = 144 — matching Serenity exactly (was 80).
#
...
#   their OWN lora_down/lora_up. Serenity has three separate down-projections,
...
# Per-stream LoRA grads: d_A/d_B for the 6 SEPARATE Serenity adapters
...
def _stream_pre_lora_resident[
    ...
    var no_bias = Optional[Tensor](None)
    var qkv = linear(norm, w.wqkv[], no_bias^, ctx)   # [N,3D]
    # SEPARATE q/k/v LoRA (Serenity attn.to_q/to_k/to_v, transformer_flux2.py
...
def _stream_post_lora_resident(
    ...
    var no_bias = Optional[Tensor](None)
    var out = linear(att[], w.wproj[], no_bias^, ctx)   # [N,D]
    # LoRA on attn out-proj (Serenity attn.to_out.0 / attn.to_add_out,
...
def double_block_lora_forward_device_resident_scratch[
    ...
    # per stream to localize the double-block forward op vs reference trainer sub-module hooks.
    if len(dump_path) > 0:
        var dfn = List[String]()
...
def _stream_pre_backward_lora_resident[
    ...
    # SEPARATE q/k/v LoRA backward (Serenity to_q/to_k/to_v, add_q/k/v_proj).
...
def _stream_pre_backward_lora_resident_scratch[
    ...
    # SEPARATE q/k/v LoRA backward (see resident variant for the Serenity mapping).
    # Dropout-aware (LoRAModule.py:328); p==0 -> base klein_lora_bwd_device_resident.
    var q_d_a = List[Float32]()
    var q_d_b = List[Float32]()
    var k_d_a = List[Float32]()
...
def double_block_lora_backward_device_resident_scratch[
    ...
    # OUTPUT == INPUT to sdpa_backward). Comparing this to reference trainer's to_out/to_add_out
