...
from serenitymojo.ops.tensor_algebra import reshape_owned, reshape_in_place, mul, add, mul_scalar, slice, concat
...
# SerenityTrainer's ErnieImageTransformer2DModel builds a per-key bool mask
...
def ernie_block_lora_forward_device_tensor[
    ...
    # SerenityTrainer masks padded TEXT keys (transformer_ernie_image.py:393-421); the
...
def ernie_block_lora_backward_device_tensors[
    ...
    var grg2 = gate_residual_backward(
        d_out, saved.h[], _t_as(mv.gate_mlp.copy(), [D], d_out.dtype(), ctx), mlp_out_y, ctx
    )
    var d_gate_mlp = grg2.d_g.to_host(ctx)
    ...
    var mb_mlp = modulate_backward(
        ...
        saved.mlp_norm[], _t_as(mv.scale_mlp.copy(), [D], saved.mlp_norm[].dtype(), ctx), ctx
    )
    var d_scale_mlp = mb_mlp.d_scale.to_host(ctx)
    var d_shift_mlp = mb_mlp.d_shift.to_host(ctx)
    ...
    var grg1 = gate_residual_backward(
        d_h, saved.x[], _t_as(mv.gate_msa.copy(), [D], d_h.dtype(), ctx), att_out_y, ctx
    )
    var d_gate_msa = grg1.d_g.to_host(ctx)
    ...
    var mb_sa = modulate_backward(
        ...
        saved.sa_norm[], _t_as(mv.scale_msa.copy(), [D], saved.sa_norm[].dtype(), ctx), ctx
    )
    var d_scale_msa = mb_sa.d_scale.to_host(ctx)
    var d_shift_msa = mb_sa.d_shift.to_host(ctx)
...
# ══════════════════════════════════════════════════════════════════════════════
# TRUE BATCH-2 (row-stacked) block fwd/bwd. Two samples processed as ONE
# row-stacked [2S, D] = [img0|txt0|img1|txt1] forward/backward. Mirrors the B=1
# device-tensor variants above EXACTLY, differing only where batch-2 semantics
# require it (krea2 krea2_block.mojo:1782-2076 is the proven template):
#   * ALL token-parallel GEMMs/norms run at M=2S rows (the compute win + true batch).
#   * AdaLN modulate/residual_gate use per-sample [2,D] vectors (rows split evenly
#     into two contiguous S ranges — ops/elementwise modulate/residual_gate [B,D]).
#   * ATTENTION is per-sample: NEVER a B=2 kernel. The SDPA runs TWICE on the two
#     [1,S,H,Dh] slices (each with its own text-pad mask), then att is concat'd.
#   * RoPE tables are pre-tiled to [2S*H, Dh] (concat of the two per-sample tables).
#   * The 7 LoRA dA/dB accumulate over BOTH sample halves (the M=2S GEMM sums both
#     row ranges) → the batch gradient (= mean of the two B=1 grads once the
#     trainer's 0.5 joint-loss scale is folded in).
# batch_size=1 stays byte-identical: the b1 variants above are untouched.
# ══════════════════════════════════════════════════════════════════════════════
...
# stack two [D] mod vectors into a flat [2*D] host list (row0=sample0, row1=sample1
# for a [2, D] modulate/gate tensor — per-sample adaLN).
def _stack2(a: List[Float32], b: List[Float32]) -> List[Float32]:
    var o = List[Float32]()
    for i in range(len(a)):
        o.append(a[i])
...
def ernie_block_lora_forward_device_tensor_b2[
    H: Int, Dh: Int, S: Int
](
    x_arc: TArc,                 # [2S, D]  row-stacked [s0(img|txt) | s1(img|txt)]
    w: ErnieBlockWeights,
    mv0: ErnieModVecs, mv1: ErnieModVecs,
    lora: ErnieBlockLoraDevice,
    cos: Tensor, sin: Tensor,    # [2S*H, Dh]  pre-tiled (sample0 rows then sample1)
    D: Int, F: Int, eps: Float32,
    ctx: DeviceContext,
    text_pad_mask0: Optional[TArc] = None,  # [1,H,S,S] sample-0; None=full attn
    text_pad_mask1: Optional[TArc] = None,  # [1,H,S,S] sample-1; None=full attn
) raises -> ErnieBlockForwardLoraTensor:
    comptime S2 = 2 * S
    var scale = Float32(1.0) / sqrt(Float32(Dh))
    var M = S2

    # per-sample AdaLN vectors stacked [2, D] (rows split evenly → per sample).
    var sc_msa = _stack2(mv0.scale_msa, mv1.scale_msa)
    var sh_msa = _stack2(mv0.shift_msa, mv1.shift_msa)
    var gt_msa = _stack2(mv0.gate_msa, mv1.gate_msa)
    var sc_mlp = _stack2(mv0.scale_mlp, mv1.scale_mlp)
    var sh_mlp = _stack2(mv0.shift_mlp, mv1.shift_mlp)
    var gt_mlp = _stack2(mv0.gate_mlp, mv1.gate_mlp)

    # --- self-attention sub-block ---
    var sa_norm = rms_norm(x_arc[], w.sa_norm[], eps, ctx)      # [2S,D]
    var sa_in = modulate(
        sa_norm,
        _t_as(sc_msa, [2, D], sa_norm.dtype(), ctx),
        _t_as(sh_msa, [2, D], sa_norm.dtype(), ctx),
        ctx,
    )
    ...
    var q_f = ernie_lora_apply_device(q_base^, sa_in, lora.to_q, M, ctx)
    var k_f = ernie_lora_apply_device(k_base^, sa_in, lora.to_k, M, ctx)
    var v_f = ernie_lora_apply_device(v_base^, sa_in, lora.to_v, M, ctx)

    var q_pre = reshape_owned(q_f^, [1, S2, H, Dh])
    var k_pre = reshape_owned(k_f^, [1, S2, H, Dh])
    var v = reshape_owned(v_f^, [1, S2, H, Dh])

    var q_rms = rms_norm(q_pre, w.q_norm[], eps, ctx)
    var k_rms = rms_norm(k_pre, w.k_norm[], eps, ctx)
    var q_rope = rope_halfsplit_full(q_rms, cos, sin, ctx)     # [1,2S,H,Dh]
    var k_rope = rope_halfsplit_full(k_rms, cos, sin, ctx)

    # SDPA per-sample: slice [1,2S,H,Dh] → two [1,S,H,Dh] (own mask), concat att.
    var q0 = slice(q_rope, 1, 0, S, ctx)
    var q1 = slice(q_rope, 1, S, S, ctx)
    var k0 = slice(k_rope, 1, 0, S, ctx)
    var k1 = slice(k_rope, 1, S, S, ctx)
    var v0 = slice(v, 1, 0, S, ctx)
    var v1 = slice(v, 1, S, S, ctx)
    var att0: Tensor
    if text_pad_mask0:
        att0 = sdpa[1, S, H, Dh](q0, k0, v0, text_pad_mask0.value()[], scale, ctx)
    else:
        att0 = sdpa_nomask[1, S, H, Dh](q0, k0, v0, scale, ctx)
    var att1: Tensor
    if text_pad_mask1:
        att1 = sdpa[1, S, H, Dh](q1, k1, v1, text_pad_mask1.value()[], scale, ctx)
    else:
        att1 = sdpa_nomask[1, S, H, Dh](q1, k1, v1, scale, ctx)
    var att = concat(1, ctx, att0, att1)                       # [1,2S,H,Dh]
    var att_flat = reshape_owned(att^, [S2, D])

    var no_bias_o = Optional[Tensor](None)
    var att_out_base = linear(att_flat, w.wo[], no_bias_o^, ctx)
    var att_out = ernie_lora_apply_device(att_out_base^, att_flat, lora.to_out, M, ctx)

    var h = residual_gate(x_arc[], _t(gt_msa, [2, D], ctx), att_out, ctx)

    # --- MLP sub-block (GELU-gated) ---
    var mlp_norm = rms_norm(h, w.mlp_norm[], eps, ctx)
    var mlp_in = modulate(
        mlp_norm, _t(sc_mlp, [2, D], ctx), _t(sh_mlp, [2, D], ctx), ctx
    )
    ...
    var no_bias_u = Optional[Tensor](None)
    var up_base = linear(mlp_in, w.wup[], no_bias_u^, ctx)
    var gate_pre = ernie_lora_apply_device(gate_base^, mlp_in, lora.gate_proj, M, ctx)
    var up = ernie_lora_apply_device(up_base^, mlp_in, lora.up_proj, M, ctx)
    ...
    var no_bias_d = Optional[Tensor](None)
    var mlp_out_base = linear(activated, w.wdown[], no_bias_d^, ctx)
    var mlp_out = ernie_lora_apply_device(mlp_out_base^, activated, lora.linear_fc2, M, ctx)

    var result = residual_gate(h, _t(gt_mlp, [2, D], ctx), mlp_out, ctx)
...
def ernie_block_lora_backward_device_tensors_b2[
    H: Int, Dh: Int, S: Int
](
    d_out: Tensor,               # [2S, D]  upstream grad of the row-stacked output
    w: ErnieBlockWeights,
    mv0: ErnieModVecs, mv1: ErnieModVecs,
    lora: ErnieBlockLoraDevice,
    saved: ErnieBlockSaved,
    cos: Tensor, sin: Tensor,    # [2S*H, Dh]  pre-tiled (must match the forward)
    D: Int, F: Int, eps: Float32,
    ctx: DeviceContext,
    text_pad_mask0_f32: Optional[TArc] = None,  # [H*S,S] sample-0; None=unmasked
    text_pad_mask1_f32: Optional[TArc] = None,  # [H*S,S] sample-1; None=unmasked
) raises -> ErnieBlockLoraTensorBackward:
    comptime S2 = 2 * S
    var scale = Float32(1.0) / sqrt(Float32(Dh))
    var M = S2
    ...
    var mlp_out_base = linear(saved.activated[], w.wdown[], nb^, ctx)
    var mlp_out_y = ernie_lora_apply_device(
        mlp_out_base^, saved.activated[], lora.linear_fc2, M, ctx
    )
    # MJ-1071 MIRROR: _t(...) makes an F32 gate against the BF16-resident grad_out —
    # the SAME (pre-existing, not-ours-to-fix) dtype bug as the b1 backward. [B,D]
    # gate ⇒ compute_gate_grad=False (shared-mod grads discarded by the LoRA arm).
    var grg2 = gate_residual_backward(
        d_out, saved.h[], _t_as(gt_mlp, [2, D], d_out.dtype(), ctx), mlp_out_y, ctx,
        compute_gate_grad=False,
    )

    var pg_down = _proj_bwd_with_lora_device_tensors(
        grg2.d_y, saved.activated[], w.wdown[], lora.linear_fc2, M, F, D, ctx,
    )
    ...
    var pg_gate = _proj_bwd_with_lora_device_tensors(
        d_gate_pre, saved.mlp_in[], w.wgate[], lora.gate_proj, M, D, F, ctx,
    )
    var pg_up = _proj_bwd_with_lora_device_tensors(
        d_up, saved.mlp_in[], w.wup[], lora.up_proj, M, D, F, ctx,
    )
    var d_mlp_in = add(pg_gate.d_x, pg_up.d_x, ctx)

    # MJ-1071 MIRROR (block mlp modulate crash site — lora_block.mojo:856 in b1):
    # _t(...) F32 scale vs BF16 mlp_norm. [B,D] ⇒ compute_param_grads=False.
    var mb_mlp = modulate_backward(
        cast_tensor(d_mlp_in, saved.mlp_norm[].dtype(), ctx, False),
        saved.mlp_norm[], _t_as(sc_mlp, [2, D], saved.mlp_norm[].dtype(), ctx), ctx,
    ...
    var att_out_base = linear(saved.att_flat[], w.wo[], nb2^, ctx)
    var att_out_y = ernie_lora_apply_device(
        att_out_base^, saved.att_flat[], lora.to_out, M, ctx
    )
    var grg1 = gate_residual_backward(
        d_h, saved.x[], _t_as(gt_msa, [2, D], d_h.dtype(), ctx), att_out_y, ctx,
        compute_gate_grad=False,
    )

    var pg_o = _proj_bwd_with_lora_device_tensors(
        grg1.d_y, saved.att_flat[], w.wo[], lora.to_out, M, D, D, ctx,
    )

    reshape_in_place(pg_o.d_x, [1, S2, H, Dh])
    # SDPA backward per-sample: slice saved q/k/v + d_att into the two S halves,
    # run TWO backwards (own text-pad mask), concat dQ/dK/dV back to [1,2S,H,Dh].
    var q0 = slice(saved.q_rope[], 1, 0, S, ctx)
    var q1 = slice(saved.q_rope[], 1, S, S, ctx)
    var k0 = slice(saved.k_rope[], 1, 0, S, ctx)
    var k1 = slice(saved.k_rope[], 1, S, S, ctx)
    var v0 = slice(saved.v[], 1, 0, S, ctx)
    var v1 = slice(saved.v[], 1, S, S, ctx)
    var dpo0 = slice(pg_o.d_x, 1, 0, S, ctx)
    var dpo1 = slice(pg_o.d_x, 1, S, S, ctx)
    var sb0: SdpaGrads
    if text_pad_mask0_f32:
        sb0 = sdpa_backward_masked[1, S, H, Dh](
            q0, k0, v0, text_pad_mask0_f32.value()[], dpo0, scale, ctx
        )
    else:
        sb0 = sdpa_backward[1, S, H, Dh](q0, k0, v0, dpo0, scale, ctx)
    var sb1: SdpaGrads
    if text_pad_mask1_f32:
        sb1 = sdpa_backward_masked[1, S, H, Dh](
            q1, k1, v1, text_pad_mask1_f32.value()[], dpo1, scale, ctx
        )
    else:
        sb1 = sdpa_backward[1, S, H, Dh](q1, k1, v1, dpo1, scale, ctx)
    var d_q_sb = concat(1, ctx, sb0.d_q, sb1.d_q)   # [1,2S,H,Dh]
    var d_k_sb = concat(1, ctx, sb0.d_k, sb1.d_k)
    var d_v_sb = concat(1, ctx, sb0.d_v, sb1.d_v)

    var d_q_rms = rope_halfsplit_full_backward(d_q_sb, cos, sin, ctx)
    var d_k_rms = rope_halfsplit_full_backward(d_k_sb, cos, sin, ctx)

    var d_q_pre = rms_norm_backward_dx(d_q_rms, saved.q_pre[], w.q_norm[], eps, ctx)
    var d_k_pre = rms_norm_backward_dx(d_k_rms, saved.k_pre[], w.k_norm[], eps, ctx)

    reshape_in_place(d_q_pre, [S2, D])
    reshape_in_place(d_k_pre, [S2, D])
    reshape_in_place(d_v_sb, [S2, D])

    var pg_q = _proj_bwd_with_lora_device_tensors(
        d_q_pre, saved.sa_in[], w.wq[], lora.to_q, M, D, D, ctx,
    )
    var pg_k = _proj_bwd_with_lora_device_tensors(
        d_k_pre, saved.sa_in[], w.wk[], lora.to_k, M, D, D, ctx,
    )
    var pg_v = _proj_bwd_with_lora_device_tensors(
        d_v_sb, saved.sa_in[], w.wv[], lora.to_v, M, D, D, ctx,
    )
    var d_sa_in = add(add(pg_q.d_x, pg_k.d_x, ctx), pg_v.d_x, ctx)

    # MJ-1071 MIRROR (block sa modulate crash site — lora_block.mojo:913 in b1).
    var mb_sa = modulate_backward(
        cast_tensor(d_sa_in, saved.sa_norm[].dtype(), ctx, False),
        saved.sa_norm[], _t_as(sc_msa, [2, D], saved.sa_norm[].dtype(), ctx), ctx,
        compute_param_grads=False,
    )
    var d_sa_norm_dx = rms_norm_backward_dx(mb_sa.d_x, saved.x[], w.sa_norm[], eps, ctx)
    var d_x = add(grg1.d_x, d_sa_norm_dx, ctx)
    ...
    # [B,D] modulate/gate backward cannot reduce per-sample param grads; the shared
    # AdaLN mod grads are discarded by the LoRA optimizer arm (fused_lora_adamw_plain
    # consumes only d_a/d_b), so return zeros — same net effect as b1.
    var shared = _zeros_f32(6 * D)
    return ErnieBlockLoraTensorBackward(TArc(d_x^), d_a^, d_b^, shared^)
