...
# ── per-tensor sum-of-squares (KLEIN_RESIDENT_GRADS, 2026-07-11) ─────────────
# One block per tensor: each thread strides its tensor at _BLOCK, accumulates
# g^2 in an F32 register, then a fixed shared-memory tree reduce writes
# out[ti]. NO atomics — the reduction order is fixed by (thread stride, tree),
# so the per-tensor sums are DETERMINISTIC run-to-run (unlike the atomic
# global kernel above). Callers combine the per-tensor sums on host (F64) for
# the global norm AND get the per-adapter zero-grad (dead-adapter) signal from
# the same single launch + one small D2H.
def _per_tensor_sq_kernel[g_dtype: DType](
    g_addr: LayoutTensor[DType.uint64, _DYN1, MutAnyOrigin],
    ...
    if ti >= ntensors:
        return
    var n = Int(rebind[Scalar[DType.int64]](lens[ti]))
    var ga = rebind[Scalar[DType.uint64]](g_addr[ti])
    var gp = UnsafePointer[Scalar[g_dtype], MutExternalOrigin](unsafe_from_address=Int(ga))
    var tid = Int(thread_idx.x)
    var acc: Float32 = 0.0
    ...
    while active > 0:
        ...
        active //= 2
    if tid == 0:
        out_ss[ti] = rebind[out_ss.element_type](sh[0])


def on_device_per_tensor_sumsq(
    grads: List[TArc], ctx: DeviceContext
) raises -> List[Float32]:
    """Deterministic per-tensor sum(g*g), all tensors in ONE launch, one small
    D2H (4 bytes per tensor). F32/BF16/F16 storage accumulates as F32."""
    var nt = len(grads)
    if nt == 0:
        raise Error("on_device_per_tensor_sumsq: empty grad list")
    var grad_dtype = grads[0][].dtype()
    if not _supported_grad_dtype(grad_dtype):
        raise Error(
            String("on_device_per_tensor_sumsq: unsupported grad dtype ")
            + grad_dtype.name()
        )

    var g_host = ctx.enqueue_create_host_buffer[DType.uint8](nt * 8)
    var len_host = ctx.enqueue_create_host_buffer[DType.uint8](nt * 8)
    var gp = g_host.unsafe_ptr().bitcast[UInt64]()
    var lp = len_host.unsafe_ptr().bitcast[Int64]()
    for i in range(nt):
        if grads[i][].dtype() != grad_dtype:
            raise Error("on_device_per_tensor_sumsq: mixed grad dtypes in one launch")
        gp[i] = UInt64(Int(grads[i][].buf.unsafe_ptr()))
        lp[i] = Int64(grads[i][].numel())

    var g_dev = ctx.enqueue_create_buffer[DType.uint8](nt * 8)
    var len_dev = ctx.enqueue_create_buffer[DType.uint8](nt * 8)
    var out_dev = ctx.enqueue_create_buffer[DType.uint8](nt * 4)
    ctx.enqueue_copy(dst_buf=g_dev, src_buf=g_host)
    ctx.enqueue_copy(dst_buf=len_dev, src_buf=len_host)
    ...
    ctx.synchronize()
    var op = out_host.unsafe_ptr().bitcast[Float32]()
...
# ── multi-tensor in-place F32 scale (global-norm clip, resident grads) ───────
# Same address-table shape as the norm kernels; per-element g[j] *= s is the
# SAME F32 multiply the host `_scale_inplace` loop performs, so the clipped
# grads are bit-identical to the host clip on identical inputs.
...
def on_device_scale_tensors_f32(
    grads: List[TArc], s: Float32, ctx: DeviceContext
) raises:
    """In-place multiply every F32 grad tensor by `s` (grad clip), one launch."""
    var nt = len(grads)
    if nt == 0:
        raise Error("on_device_scale_tensors_f32: empty grad list")
    ...
    for i in range(nt):
        if grads[i][].dtype() != STDtype.F32:
            raise Error(
                String("on_device_scale_tensors_f32: non-F32 grad dtype ")
                + grads[i][].dtype().name()
            )
        gp[i] = UInt64(Int(grads[i][].buf.unsafe_ptr()))
        total += grads[i][].numel()
