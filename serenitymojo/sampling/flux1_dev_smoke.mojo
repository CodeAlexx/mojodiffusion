...
def _check_aspect_plan(
    width: Int,
    height: Int,
    expected_packed_h: Int,
    expected_packed_w: Int,
    expected_image_tokens: Int,
) raises:
    var plan = build_flux1_packed_latent_plan(width, height, 512)
    _check_equal(String(width) + "x" + String(height) + " packed_h", plan.packed_h, expected_packed_h)
    _check_equal(String(width) + "x" + String(height) + " packed_w", plan.packed_w, expected_packed_w)
    _check_equal(String(width) + "x" + String(height) + " latent_h", plan.latent_h, expected_packed_h * 2)
    _check_equal(String(width) + "x" + String(height) + " latent_w", plan.latent_w, expected_packed_w * 2)
    _check_equal(String(width) + "x" + String(height) + " image_tokens", plan.image_tokens, expected_image_tokens)
    _check_equal(String(width) + "x" + String(height) + " total_sequence", plan.total_sequence, expected_image_tokens + 512)
...
def main() raises:
    ...
    # Finite product ladder geometry.  This checks core planning only; the
    # worker remains fail-loud 1024-only until every compiled execution arm and
    # artifact has model-oracle evidence.
    _check_aspect_plan(1024, 1024, 64, 64, 4096)
    _check_aspect_plan(1152, 896, 56, 72, 4032)
    _check_aspect_plan(896, 1152, 72, 56, 4032)
    _check_aspect_plan(1344, 768, 48, 84, 4032)
    _check_aspect_plan(768, 1344, 84, 48, 4032)
    _check_aspect_plan(1280, 832, 52, 80, 4160)
    _check_aspect_plan(832, 1280, 80, 52, 4160)
