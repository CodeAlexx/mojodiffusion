...
#
# Host-side scalar helpers mirroring local SerenityTrainer:
...
def validate_flux1_flow_match_scheduler_config(
    ...
    """Fail-loud guard for the local SerenityTrainer FLUX.1-dev scheduler config."""
...
def flux1_dynamic_shift(image_seq_len: Int) raises -> Float64:
    """SerenityTrainer FluxModel.calculate_timestep_shift return value.
...
def flux1_model_timestep_from_scheduler_timestep(timestep: Float32) -> Float32:
    """SerenityTrainer transformer input convention: `timestep=expanded_timestep / 1000`."""
...
def flux1_cfg_batch_size() -> Int:
    """SerenityTrainer FluxSampler runs a single model batch for FLUX.1-dev."""
    return 1
...
def flux1_euler_step(
    ...
    """Tensor Euler step matching SerenityTrainer's scheduler update convention."""
