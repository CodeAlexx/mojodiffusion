...
# models/klein/lora_block.mojo) specialized to FLUX's SerenityTrainer target set
# (verified line-by-line against /home/alex/SerenityTrainer/modules/util/convert/lora/
...
#
# WHY 3 SEPARATE q/k/v ADAPTERS (not one fused qkv adapter like Klein): reference trainer/
...
from serenitymojo.ops.norm_backward import (
    rms_norm_backward, layer_norm_backward,
...
from serenitymojo.training.oft_serenity_trainer_device import (
    oft_serenity_rotate_b4, oft_serenity_rotate_backward_b4,
)
...
def _direct_proj_fwd_device(
    ...
    if algo == FLUX_DIRECT_ALGO_OFT:
        ...
        var x_rot = oft_serenity_rotate_b4(x, vec, ctx)
...
def _direct_proj_bwd_device(
    ...
    if algo == FLUX_DIRECT_ALGO_OFT:
        ...
        var g = oft_serenity_rotate_backward_b4(d_x_rot^, x, vec, ctx)
        return _FluxDirectProjectionGradDev(
...
def _stream_post_backward_lora(
    ...
    # ln2 weight is frozen ones → d_x-only backward.
    var lnb2_dx = layer_norm_backward_dx(mb2.d_x, attn_res_t, ones, eps, ctx)  # bf16
    # attn_res feeds residual (grg2.d_x F32) AND ln2 (bf16) → SUM bf16, then F32 for gate bwd.
    var d_attn_res_total = add(_to_bf16(grg2.d_x, ctx), lnb2_dx, ctx)
...
def _stream_pre_backward_lora[
    ...
    # ln1 weight is frozen ones → d_x-only backward.
    var lnb1_dx = layer_norm_backward_dx(mb1.d_x, x_t, ones, eps, ctx)
    var d_x_norm = lnb1_dx.to_host(ctx)
...
def _stream_post_backward_direct(
    ...
    var d_shift2 = mb2.d_shift.to_host(ctx)
    # ln2 weight is frozen ones → d_x-only backward.
    var lnb2_dx = layer_norm_backward_dx(mb2.d_x, attn_res_t, ones, eps, ctx)
    var d_attn_res_total = add(_to_bf16(grg2.d_x, ctx), lnb2_dx, ctx)
...
def _stream_pre_backward_direct[
    ...
    var q_pre_t = _tb16(sv.q_pre.copy(), [1, N, H, Dh], ctx)
    var k_pre_t = _tb16(sv.k_pre.copy(), [1, N, H, Dh], ctx)
    # q_norm/k_norm are frozen in this path (no d_q_norm/d_k_norm grads) → d_x-only.
    var rb_q_dx = rms_norm_backward_dx(dq_b, q_pre_t, w.q_norm[], eps, ctx)
    var rb_k_dx = rms_norm_backward_dx(dk_b, k_pre_t, w.k_norm[], eps, ctx)
    ...
    var d_shift1 = mb1.d_shift.to_host(ctx)
    # ln1 weight is frozen ones → d_x-only backward.
    var lnb1_dx = layer_norm_backward_dx(
        mb1.d_x, _tb16(sv.x.copy(), [N, D], ctx), ones, eps, ctx,
    )
    return _StreamPreBackDirect(
        lnb1_dx.to_host(ctx), d_shift1^, d_scale1^, q_g^, k_g^, v_g^,
    )
...
def single_block_direct_lycoris_backward[
    ...
    # q_norm/k_norm are frozen in this path (no d_q_norm/d_k_norm grads) → d_x-only.
    var rb_q_dx = rms_norm_backward_dx(_to_bf16(d_q_rms, ctx), q_pre_t, w.q_norm[], eps, ctx)
    var rb_k_dx = rms_norm_backward_dx(_to_bf16(d_k_rms, ctx), k_pre_t, w.k_norm[], eps, ctx)
    ...
    var q_g = _direct_proj_bwd_device(
        direct.algo, direct.dora, direct.oft, direct.q_slot,
        rb_q_dx, norm_t, q_w, S, D, D, ctx,
    )
    var k_g = _direct_proj_bwd_device(
        direct.algo, direct.dora, direct.oft, direct.k_slot,
        rb_k_dx, norm_t, k_w, S, D, D, ctx,
    )
    var v_g = _direct_proj_bwd_device(
        direct.algo, direct.dora, direct.oft, direct.v_slot,
    ...
    # ln weight is frozen ones → d_x-only backward.
    var lnb_dx = layer_norm_backward_dx(mb.d_x, x_t, ones_t, eps, ctx)
    var d_x = _add_lists(grg.d_x.to_host(ctx), lnb_dx.to_host(ctx))
...
# ═══════════════════════════════════════════════════════════════════════════
# TRUE BATCH-2 (row-stacked) block forward/backward. Mirrors the b1 double/single
# block funcs above, but the two samples are ROW-STACKED on the token axis:
#   double: img [2*N_IMG, D], txt [2*N_TXT, D]  (sample0 rows then sample1 rows)
#   single: x   [2*S, D]                        (sample0 seq  then sample1 seq)
# The FROZEN base GEMMs (qkv/proj/mlp/w1/w2) run batched over the doubled rows;
# the LoRA backward (flux_lora_bwd over M=2N rows) SUMS both samples' contributions
# into ONE d_a/d_b (the batch gradient — NOT per-sample accumulation). Attention is
# PER-SAMPLE: q/k/v are reshaped to the batched [2, S, H, Dh] so sdpa_nomask[2,...]
# does two independent joint attentions (no cross-sample mixing). Modulation is
# PER-SAMPLE via [2, D] adaLN (modulate / residual_gate accept [B, D]); their
# backward runs with compute_param_grads=False / compute_gate_grad=False since the
# modulation projections are FROZEN (stack-level LoRA is fenced off for the b2
# wave), so d_scale/d_shift/d_gate feed only discarded embed grads.
# ═══════════════════════════════════════════════════════════════════════════

# extract chunk #idx (length d) for BOTH samples from a [2, nchunk*d] row-major
# host list → a [2, d] tensor (row0 = sample0's chunk, row1 = sample1's chunk).
def _mod2_bf16(flat2: List[Float32], nchunk: Int, idx: Int, d: Int, ctx: DeviceContext) raises -> Tensor:
    var stride = nchunk * d
    var o = List[Float32]()
    for s in range(2):
...
def _mod2_f32(flat2: List[Float32], nchunk: Int, idx: Int, d: Int, ctx: DeviceContext) raises -> Tensor:
    var stride = nchunk * d
    var o = List[Float32]()
    for s in range(2):
        var base = s * stride + idx * d
        for i in range(d):
            o.append(flat2[base + i])
    return _tf32(o^, [2, d], ctx)


struct _TwoT(Movable):
    var a: Tensor   # txt stream  [1, 2*N_TXT, H, Dh]
    var b: Tensor   # img stream  [1, 2*N_IMG, H, Dh]

    def __init__(out self, var a: Tensor, var b: Tensor):
        self.a = a^
        self.b = b^


# Build the batched [2, S, H, Dh] joint sequence from the two row-stacked streams.
# txt_rms [1, 2*N_TXT, H, Dh] = [txt0 | txt1] ; img_rms [1, 2*N_IMG, H, Dh].
# sample s joint seq = [txt_s (N_TXT) | img_s (N_IMG)]  (txt FIRST, matching b1).
def _interleave_qkv_b2(
    txt_rms: Tensor, img_rms: Tensor, n_txt: Int, n_img: Int, ctx: DeviceContext
) raises -> Tensor:
    var txt0 = slice(txt_rms, 1, 0, n_txt, ctx)
    var txt1 = slice(txt_rms, 1, n_txt, n_txt, ctx)
    var img0 = slice(img_rms, 1, 0, n_img, ctx)
    var img1 = slice(img_rms, 1, n_img, n_img, ctx)
    var s0 = concat(1, ctx, txt0, img0)   # [1, S, H, Dh]
    var s1 = concat(1, ctx, txt1, img1)   # [1, S, H, Dh]
    return concat(0, ctx, s0, s1)          # [2, S, H, Dh]


# Inverse of _interleave: batched [2, S, H, Dh] → per-stream row-stacked tensors.
def _deinterleave_b2(
    t: Tensor, n_txt: Int, n_img: Int, ctx: DeviceContext
) raises -> _TwoT:
    var a0 = slice(t, 0, 0, 1, ctx)   # [1, S, H, Dh]
    var a1 = slice(t, 0, 1, 1, ctx)
    var txt0 = slice(a0, 1, 0, n_txt, ctx)
    var img0 = slice(a0, 1, n_txt, n_img, ctx)
    var txt1 = slice(a1, 1, 0, n_txt, ctx)
    var img1 = slice(a1, 1, n_txt, n_img, ctx)
    var txt = concat(1, ctx, txt0, txt1)   # [1, 2*N_TXT, H, Dh]
    var img = concat(1, ctx, img0, img1)   # [1, 2*N_IMG, H, Dh]
    return _TwoT(txt^, img^)


# ── b2 per-stream pre (forward): modvecs as [2,D] tensors; N2 = 2*N rows ──────
def _stream_pre_lora_b2[
    H: Int, Dh: Int
](
    x: Tensor, w: StreamWeights, scale1: Tensor, shift1: Tensor, lo: StreamLora,
    N2: Int, D: Int, eps: Float32, ones: Tensor, zeros: Tensor, ctx: DeviceContext,
) raises -> _StreamPreH:
    var ln1 = layer_norm(x, ones, zeros, eps, ctx)
    var norm = modulate(ln1, scale1, shift1, ctx)          # per-sample [2,D]
    var norm_h_f32 = norm.to_host(ctx)
    var b = Optional[Tensor](w.bqkv[].clone(ctx))
    var qkv = linear(norm, w.wqkv[], b, ctx)               # [N2, 3D]
    var q_base = slice(qkv, 1, 0, D, ctx).to_host(ctx)
    var k_base = slice(qkv, 1, D, D, ctx).to_host(ctx)
    var v_base = slice(qkv, 1, 2 * D, D, ctx).to_host(ctx)
    var q_h = flux_lora_apply(q_base, norm_h_f32, lo.to_q, N2, ctx)
    var k_h = flux_lora_apply(k_base, norm_h_f32, lo.to_k, N2, ctx)
    var v_h = flux_lora_apply(v_base, norm_h_f32, lo.to_v, N2, ctx)
    var q_pre = reshape_owned(_t(q_h, [N2, D], ctx)^, [1, N2, H, Dh])
    var k_pre = reshape_owned(_t(k_h, [N2, D], ctx)^, [1, N2, H, Dh])
    var v = reshape_owned(_t(v_h, [N2, D], ctx)^, [1, N2, H, Dh])
    var q_rms = rms_norm(q_pre, w.q_norm[], eps, ctx)
    var k_rms = rms_norm(k_pre, w.k_norm[], eps, ctx)
    return _StreamPreH(
        q_rms^, k_rms^, v^,
...
# ── b2 per-stream post (forward): gate1/scale2/shift2/gate2 as [2,D] ──────────
def _stream_post_lora_b2(
    x: Tensor, att: Tensor, att_h_f32: List[Float32],
    w: StreamWeights, gate1: Tensor, scale2: Tensor, shift2: Tensor, gate2: Tensor,
    lo: StreamLora,
    N2: Int, D: Int, Fmlp: Int, eps: Float32, ones: Tensor, zeros: Tensor,
    ctx: DeviceContext,
) raises -> _StreamPostH:
    var bp = Optional[Tensor](w.bproj[].clone(ctx))
    var out_base = linear(att, w.wproj[], bp, ctx).to_host(ctx)
    var out_h = flux_lora_apply(out_base, att_h_f32, lo.proj, N2, ctx)
    var out = _t(out_h, [N2, D], ctx)
    var attn_res = residual_gate(x, gate1, out, ctx)
    var ln2 = layer_norm(attn_res, ones, zeros, eps, ctx)
    var mlp_in = modulate(ln2, scale2, shift2, ctx)
    var mlp_in_h_f32 = mlp_in.to_host(ctx)
    var b0 = Optional[Tensor](w.bmlp0[].clone(ctx))
    var mlp_pre_base = linear(mlp_in, w.wmlp0[], b0, ctx).to_host(ctx)
    var mlp_pre_h = flux_lora_apply(mlp_pre_base, mlp_in_h_f32, lo.mlp0, N2, ctx)
    var mlp_pre = _t(mlp_pre_h, [N2, Fmlp], ctx)
    var mlp_h = gelu(mlp_pre, ctx)
    var mlp_h_h_f32 = mlp_h.to_host(ctx)
    var b2 = Optional[Tensor](w.bmlp2[].clone(ctx))
    var mlp_base = linear(mlp_h, w.wmlp2[], b2, ctx).to_host(ctx)
    var mlp_out_h = flux_lora_apply(mlp_base, mlp_h_h_f32, lo.mlp2, N2, ctx)
    var mlp = _t(mlp_out_h, [N2, D], ctx)
    var final = residual_gate(attn_res, gate2, mlp, ctx)
    return _StreamPostH(
        final^, attn_res.to_host_bf16(ctx), ln2.to_host_bf16(ctx),
        _f32_to_bf16(mlp_in_h_f32), _f32_to_bf16(mlp_pre_h), _f32_to_bf16(mlp_h_h_f32),
    )
...
# ── b2 stream backward result carriers (only d_x / d_att; modvec+frozen-W grads
#    are discarded for the LoRA-only b2 wave). ─────────────────────────────────
struct _StreamPostBackLB2(Movable):
    var d_x: List[Float32]
    var d_att: List[Float32]

    def __init__(out self, var d_x: List[Float32], var d_att: List[Float32]):
        self.d_x = d_x^
        self.d_att = d_att^


struct _StreamPreBackLB2(Movable):
    var d_x: List[Float32]

    def __init__(out self, var d_x: List[Float32]):
        self.d_x = d_x^


def _stream_post_backward_lora_b2(
    d_out: Tensor, x: Tensor, att: Tensor,
    w: StreamWeights, gate1_f32: Tensor, scale2_bf16: Tensor, gate2_f32: Tensor,
    sv: StreamSaved, lo: StreamLora,
    N2: Int, D: Int, Fmlp: Int, eps: Float32, ones: Tensor,
    mut d_a_slots: List[List[Float32]], mut d_b_slots: List[List[Float32]],
    ctx: DeviceContext,
) raises -> _StreamPostBackLB2:
    var attn_res_t = _tb16(sv.attn_res.copy(), [N2, D], ctx)
    var mlp_h_b = _tb16(sv.mlp_h.copy(), [N2, Fmlp], ctx)
    var b2 = Optional[Tensor](w.bmlp2[].clone(ctx))
    var mlp_base = linear(mlp_h_b, w.wmlp2[], b2, ctx).to_host(ctx)
    var mlp_h_vals_f32 = List[Float32]()
    for i in range(len(sv.mlp_h)):
        mlp_h_vals_f32.append(Float32(sv.mlp_h[i]))
    var mlp_y_h = flux_lora_apply(mlp_base, mlp_h_vals_f32, lo.mlp2, N2, ctx)
    var mlp_y = _t(mlp_y_h, [N2, D], ctx)
    var grg2 = gate_residual_backward(
        d_out, attn_res_t, gate2_f32, mlp_y, ctx, compute_gate_grad=False
    )
    var pm2 = _proj_bwd_with_lora(
        _to_bf16(grg2.d_y, ctx), mlp_h_b, w.wmlp2[], mlp_h_vals_f32, lo.mlp2, D_MLP2,
        N2, Fmlp, D, d_a_slots, d_b_slots, ctx,
    )
    var mlp_pre_t = _tb16(sv.mlp_pre.copy(), [N2, Fmlp], ctx)
    var d_mlp_pre = gelu_backward(pm2.d_x, mlp_pre_t, ctx)
    var mlp_in_t = _tb16(sv.mlp_in.copy(), [N2, D], ctx)
    var mlp_in_vals_f32 = List[Float32]()
    for i in range(len(sv.mlp_in)):
        mlp_in_vals_f32.append(Float32(sv.mlp_in[i]))
    var pm0 = _proj_bwd_with_lora(
        d_mlp_pre, mlp_in_t, w.wmlp0[], mlp_in_vals_f32, lo.mlp0, D_MLP0,
        N2, D, Fmlp, d_a_slots, d_b_slots, ctx,
    )
    var ln2_t = _tb16(sv.ln2.copy(), [N2, D], ctx)
    var mb2 = modulate_backward(pm0.d_x, ln2_t, scale2_bf16, ctx, compute_param_grads=False)
    var lnb2_dx = layer_norm_backward_dx(mb2.d_x, attn_res_t, ones, eps, ctx)
    var d_attn_res_total = add(_to_bf16(grg2.d_x, ctx), lnb2_dx, ctx)
    var d_attn_res_f32 = _to_f32(d_attn_res_total, ctx)
    var bp = Optional[Tensor](w.bproj[].clone(ctx))
    var proj_base = linear(att, w.wproj[], bp, ctx).to_host(ctx)
    var att_h = att.to_host(ctx)
    var proj_y_h = flux_lora_apply(proj_base, att_h, lo.proj, N2, ctx)
    var proj_out = _t(proj_y_h, [N2, D], ctx)
    var grg1 = gate_residual_backward(
        d_attn_res_f32, x, gate1_f32, proj_out, ctx, compute_gate_grad=False
    )
    var d_x_res = grg1.d_x.to_host(ctx)
    var pproj = _proj_bwd_with_lora(
        _to_bf16(grg1.d_y, ctx), att, w.wproj[], att_h, lo.proj, D_PROJ,
        N2, D, D, d_a_slots, d_b_slots, ctx,
    )
    var d_att = pproj.d_x.to_host(ctx)
    return _StreamPostBackLB2(d_x_res^, d_att^)


def _stream_pre_backward_lora_b2[
    H: Int, Dh: Int
](
    d_q_rms: Tensor, d_k_rms: Tensor, d_v: Tensor,
    w: StreamWeights, scale1_bf16: Tensor, sv: StreamSaved, lo: StreamLora,
    N2: Int, D: Int, eps: Float32, ones: Tensor,
    mut d_a_slots: List[List[Float32]], mut d_b_slots: List[List[Float32]],
    ctx: DeviceContext,
) raises -> _StreamPreBackLB2:
    var dq_b = _to_bf16(d_q_rms, ctx)
    var dk_b = _to_bf16(d_k_rms, ctx)
    var dv_b = _to_bf16(d_v, ctx)
    var q_pre_t = _tb16(sv.q_pre.copy(), [1, N2, H, Dh], ctx)
    var k_pre_t = _tb16(sv.k_pre.copy(), [1, N2, H, Dh], ctx)
    var rb_q = rms_norm_backward(dq_b, q_pre_t, w.q_norm[], eps, ctx)
    var rb_k = rms_norm_backward(dk_b, k_pre_t, w.k_norm[], eps, ctx)
    reshape_in_place(rb_q.d_x, [N2, D])
    reshape_in_place(rb_k.d_x, [N2, D])
    var d_v_flat = reshape(dv_b, [N2, D], ctx)
    var norm_t = _tb16(sv.norm.copy(), [N2, D], ctx)
    var d_qkv = concat(1, ctx, rb_q.d_x, rb_k.d_x, d_v_flat)
    var lb_qkv = linear_backward(d_qkv, norm_t, w.wqkv[], N2, D, 3 * D, ctx)
    ...
    var d_v_h = d_v_flat.to_host(ctx)
    if lo.to_q:
        var lg = flux_lora_bwd(d_q_h, norm_vals_f32, lo.to_q.value(), N2, ctx)
        d_a_slots[D_SQ] = lg.d_a.copy()
        d_b_slots[D_SQ] = lg.d_b.copy()
        d_norm = _add_lists(d_norm, lg.d_x)
    if lo.to_k:
        var lg = flux_lora_bwd(d_k_h, norm_vals_f32, lo.to_k.value(), N2, ctx)
        d_a_slots[D_SK] = lg.d_a.copy()
        d_b_slots[D_SK] = lg.d_b.copy()
        d_norm = _add_lists(d_norm, lg.d_x)
    if lo.to_v:
        var lg = flux_lora_bwd(d_v_h, norm_vals_f32, lo.to_v.value(), N2, ctx)
        d_a_slots[D_SV] = lg.d_a.copy()
        d_b_slots[D_SV] = lg.d_b.copy()
        d_norm = _add_lists(d_norm, lg.d_x)
    var ln1_t = _tb16(sv.ln1.copy(), [N2, D], ctx)
    var mb1 = modulate_backward(
        _t(d_norm, [N2, D], ctx), ln1_t, scale1_bf16, ctx, compute_param_grads=False
    )
    var x_t = _tb16(sv.x.copy(), [N2, D], ctx)
    var lnb1_dx = layer_norm_backward_dx(mb1.d_x, x_t, ones, eps, ctx)
    var d_x_norm = lnb1_dx.to_host(ctx)
    return _StreamPreBackLB2(d_x_norm^)


# ── b2 block backward result carriers ────────────────────────────────────────
struct DoubleBlockLoraBackwardB2(Movable):
    var d_img_x: List[Float32]   # [2*N_IMG, D]
    var d_txt_x: List[Float32]   # [2*N_TXT, D]
    var lora: DoubleBlockLoraGrads

    def __init__(
        out self, var d_img_x: List[Float32], var d_txt_x: List[Float32],
        var lora: DoubleBlockLoraGrads,
    ):
...
struct SingleBlockLoraBackwardB2(Movable):
    var d_x: List[Float32]       # [2*S, D]
    var lora: SingleBlockLoraGrads

    def __init__(out self, var d_x: List[Float32], var lora: SingleBlockLoraGrads):
        self.d_x = d_x^
        self.lora = lora^


# ── DOUBLE block b2 forward ──────────────────────────────────────────────────
def double_block_lora_forward_b2[
    H: Int, Dh: Int, N_IMG: Int, N_TXT: Int, S: Int
](
    img: List[Float32], txt: List[Float32],
    w: DoubleBlockWeights, img_mod_flat2: List[Float32], txt_mod_flat2: List[Float32],
    lora: DoubleBlockLora,
    cos2: Tensor, sin2: Tensor,
    D: Int, Fmlp: Int, eps: Float32,
    ctx: DeviceContext,
) raises -> DoubleBlockForward:
    var scale = Float32(1.0) / sqrt(Float32(Dh))
    var ones_t = _t(_ones(D), [D], ctx)
    var zeros_t = _t(_zeros(D), [D], ctx)
    var cos_b = _to_bf16(cos2, ctx)
    var sin_b = _to_bf16(sin2, ctx)
    var NI2 = 2 * N_IMG
    var NT2 = 2 * N_TXT

    var img_x = _t(img, [NI2, D], ctx)
    var txt_x = _t(txt, [NT2, D], ctx)

    # modvec chunk order in the flat [6D]: shift1,scale1,gate1,shift2,scale2,gate2.
    var img_shift1 = _mod2_bf16(img_mod_flat2, 6, 0, D, ctx)
    var img_scale1 = _mod2_bf16(img_mod_flat2, 6, 1, D, ctx)
    var img_gate1 = _mod2_bf16(img_mod_flat2, 6, 2, D, ctx)
    var img_shift2 = _mod2_bf16(img_mod_flat2, 6, 3, D, ctx)
    var img_scale2 = _mod2_bf16(img_mod_flat2, 6, 4, D, ctx)
    var img_gate2 = _mod2_bf16(img_mod_flat2, 6, 5, D, ctx)
    var txt_shift1 = _mod2_bf16(txt_mod_flat2, 6, 0, D, ctx)
    var txt_scale1 = _mod2_bf16(txt_mod_flat2, 6, 1, D, ctx)
    var txt_gate1 = _mod2_bf16(txt_mod_flat2, 6, 2, D, ctx)
    var txt_shift2 = _mod2_bf16(txt_mod_flat2, 6, 3, D, ctx)
    var txt_scale2 = _mod2_bf16(txt_mod_flat2, 6, 4, D, ctx)
    var txt_gate2 = _mod2_bf16(txt_mod_flat2, 6, 5, D, ctx)

    var ip = _stream_pre_lora_b2[H, Dh](
        img_x, w.img, img_scale1, img_shift1, lora.img, NI2, D, eps, ones_t, zeros_t, ctx
    )
    var tp = _stream_pre_lora_b2[H, Dh](
        txt_x, w.txt, txt_scale1, txt_shift1, lora.txt, NT2, D, eps, ones_t, zeros_t, ctx
    )

    var q_batched = _interleave_qkv_b2(tp.q_rms, ip.q_rms, N_TXT, N_IMG, ctx)   # [2,S,H,Dh]
    var k_batched = _interleave_qkv_b2(tp.k_rms, ip.k_rms, N_TXT, N_IMG, ctx)
    var v_batched = _interleave_qkv_b2(tp.v, ip.v, N_TXT, N_IMG, ctx)

    var q_rope = rope_interleaved(q_batched, cos_b, sin_b, ctx)
    var k_rope = rope_interleaved(k_batched, cos_b, sin_b, ctx)
    var att = sdpa_nomask[2, S, H, Dh](q_rope, k_rope, v_batched, scale, ctx)    # [2,S,H,Dh]

    var deatt = _deinterleave_b2(att, N_TXT, N_IMG, ctx)   # a=txt [1,NT2,..], b=img [1,NI2,..]
    var txt_att = reshape(deatt.a, [NT2, D], ctx)
    var img_att = reshape(deatt.b, [NI2, D], ctx)
    var img_att_h_f32 = img_att.to_host(ctx)
    var txt_att_h_f32 = txt_att.to_host(ctx)

    var ipost = _stream_post_lora_b2(
        img_x, img_att, img_att_h_f32, w.img,
        img_gate1, img_scale2, img_shift2, img_gate2, lora.img,
        NI2, D, Fmlp, eps, ones_t, zeros_t, ctx,
    )
    var tpost = _stream_post_lora_b2(
        txt_x, txt_att, txt_att_h_f32, w.txt,
        txt_gate1, txt_scale2, txt_shift2, txt_gate2, lora.txt,
        NT2, D, Fmlp, eps, ones_t, zeros_t, ctx,
    )
    ...
    var saved = DoubleBlockSaved(
        img_saved^, txt_saved^,
        q_rope.to_host_bf16(ctx), k_rope.to_host_bf16(ctx), v_batched.to_host_bf16(ctx),
    )
...
# ── DOUBLE block b2 backward ─────────────────────────────────────────────────
def double_block_lora_backward_b2[
    H: Int, Dh: Int, N_IMG: Int, N_TXT: Int, S: Int
](
    d_img_out: List[Float32], d_txt_out: List[Float32],
    w: DoubleBlockWeights, img_mod_flat2: List[Float32], txt_mod_flat2: List[Float32],
    ...
    D: Int, Fmlp: Int, eps: Float32,
    ctx: DeviceContext,
) raises -> DoubleBlockLoraBackwardB2:
    var scale = Float32(1.0) / sqrt(Float32(Dh))
    var ones_t = _t(_ones(D), [D], ctx)
    var NI2 = 2 * N_IMG
    var NT2 = 2 * N_TXT
    ...
    var img_scale1 = _mod2_bf16(img_mod_flat2, 6, 1, D, ctx)
    var img_gate1 = _mod2_f32(img_mod_flat2, 6, 2, D, ctx)
    var img_scale2 = _mod2_bf16(img_mod_flat2, 6, 4, D, ctx)
    var img_gate2 = _mod2_f32(img_mod_flat2, 6, 5, D, ctx)
    var txt_scale1 = _mod2_bf16(txt_mod_flat2, 6, 1, D, ctx)
    var txt_gate1 = _mod2_f32(txt_mod_flat2, 6, 2, D, ctx)
    var txt_scale2 = _mod2_bf16(txt_mod_flat2, 6, 4, D, ctx)
    var txt_gate2 = _mod2_f32(txt_mod_flat2, 6, 5, D, ctx)

    var d_io_t = _tf32(d_img_out, [NI2, D], ctx)
    var d_to_t = _tf32(d_txt_out, [NT2, D], ctx)
    var img_x = _tb16(saved.img.x.copy(), [NI2, D], ctx)
    var txt_x = _tb16(saved.txt.x.copy(), [NT2, D], ctx)
    var img_att = _tb16(saved.img.att.copy(), [NI2, D], ctx)
    var txt_att = _tb16(saved.txt.att.copy(), [NT2, D], ctx)
    ...
    var ipb = _stream_post_backward_lora_b2(
        d_io_t, img_x, img_att, w.img, img_gate1, img_scale2, img_gate2,
        saved.img, lora.img, NI2, D, Fmlp, eps, ones_t, ia, ib, ctx,
    )
    var tpb = _stream_post_backward_lora_b2(
        d_to_t, txt_x, txt_att, w.txt, txt_gate1, txt_scale2, txt_gate2,
        saved.txt, lora.txt, NT2, D, Fmlp, eps, ones_t, ta, tb, ctx,
    )

    # join the per-stream attention-slice grads → batched [2,S,H,Dh] (bf16).
    var d_iatt_4d = _t(ipb.d_att.copy(), [1, NI2, H, Dh], ctx)
    var d_tatt_4d = _t(tpb.d_att.copy(), [1, NT2, H, Dh], ctx)
    var d_att_batched = _interleave_qkv_b2(d_tatt_4d, d_iatt_4d, N_TXT, N_IMG, ctx)

    var q_rope_t = _tb16(saved.q_rope.copy(), [2, S, H, Dh], ctx)
    var k_rope_t = _tb16(saved.k_rope.copy(), [2, S, H, Dh], ctx)
    var v_batched_t = _tb16(saved.v_joint.copy(), [2, S, H, Dh], ctx)
    var sb = sdpa_backward[2, S, H, Dh](q_rope_t, k_rope_t, v_batched_t, d_att_batched, scale, ctx)

    var d_q_batched = rope_backward(_to_f32(sb.d_q, ctx), cos2, sin2, True, ctx)
    var d_k_batched = rope_backward(_to_f32(sb.d_k, ctx), cos2, sin2, True, ctx)
    var d_v_batched = _to_f32(sb.d_v, ctx)

    var dq = _deinterleave_b2(d_q_batched, N_TXT, N_IMG, ctx)
    var dk = _deinterleave_b2(d_k_batched, N_TXT, N_IMG, ctx)
    var dv = _deinterleave_b2(d_v_batched, N_TXT, N_IMG, ctx)

    var iprb = _stream_pre_backward_lora_b2[H, Dh](
        dq.b, dk.b, dv.b, w.img, img_scale1, saved.img, lora.img,
        NI2, D, eps, ones_t, ia, ib, ctx,
    )
    var tprb = _stream_pre_backward_lora_b2[H, Dh](
        dq.a, dk.a, dv.a, w.txt, txt_scale1, saved.txt, lora.txt,
        NT2, D, eps, ones_t, ta, tb, ctx,
    )

    var d_img_x = _add_lists(ipb.d_x, iprb.d_x)
    var d_txt_x = _add_lists(tpb.d_x, tprb.d_x)
    return DoubleBlockLoraBackwardB2(
        d_img_x^, d_txt_x^,
        DoubleBlockLoraGrads(StreamLoraGrads(ia^, ib^), StreamLoraGrads(ta^, tb^)),
    )


# ── SINGLE block b2 forward ──────────────────────────────────────────────────
def single_block_lora_forward_b2[
    H: Int, Dh: Int, S: Int
](
    x: List[Float32],
    w: SingleBlockWeights, sgl_mod_flat2: List[Float32], lora: SingleBlockLora,
    cos2: Tensor, sin2: Tensor,
    D: Int, Fmlp: Int, eps: Float32,
    ctx: DeviceContext,
) raises -> SingleBlockForward:
    ...
    # single mod flat [3D]: shift, scale, gate.
    var shift_bf16 = _mod2_bf16(sgl_mod_flat2, 3, 0, D, ctx)
    var scale_bf16 = _mod2_bf16(sgl_mod_flat2, 3, 1, D, ctx)
    var gate_bf16 = _mod2_bf16(sgl_mod_flat2, 3, 2, D, ctx)

    var x_t = _t(x, [S2, D], ctx)
    var ln_t = layer_norm(x_t, ones_t, zeros_t, eps, ctx)
    var norm_t = modulate(ln_t, scale_bf16, shift_bf16, ctx)   # per-sample [2,D]
    var norm_h = norm_t.to_host(ctx)

    var b1 = Optional[Tensor](w.b1[].clone(ctx))
    var fused = linear(norm_t, w.w1[], b1, ctx)                # [S2, 3D+Fmlp]

    var q_base = slice(fused, 1, 0, D, ctx).to_host(ctx)
    var k_base = slice(fused, 1, D, D, ctx).to_host(ctx)
    var v_base = slice(fused, 1, 2 * D, D, ctx).to_host(ctx)
    var mlp_base = slice(fused, 1, 3 * D, Fmlp, ctx).to_host(ctx)

    var q_h = flux_lora_apply(q_base, norm_h, lora.to_q, S2, ctx)
    var k_h = flux_lora_apply(k_base, norm_h, lora.to_k, S2, ctx)
    var v_h = flux_lora_apply(v_base, norm_h, lora.to_v, S2, ctx)
    var mlp_in_h = flux_lora_apply(mlp_base, norm_h, lora.proj_mlp, S2, ctx)

    var q_pre = reshape_owned(_t(q_h, [S2, D], ctx)^, [2, S, H, Dh])
    var k_pre = reshape_owned(_t(k_h, [S2, D], ctx)^, [2, S, H, Dh])
    var v = reshape_owned(_t(v_h, [S2, D], ctx)^, [2, S, H, Dh])

    var q_rms = rms_norm(q_pre, w.q_norm[], eps, ctx)
    var k_rms = rms_norm(k_pre, w.k_norm[], eps, ctx)
    var q_rope = rope_interleaved(q_rms, cos_b, sin_b, ctx)
    var k_rope = rope_interleaved(k_rms, cos_b, sin_b, ctx)
    var att = sdpa_nomask[2, S, H, Dh](q_rope, k_rope, v, scale, ctx)   # [2,S,H,Dh]
    var att_flat = reshape_owned(att^, [S2, D])

    var mlp_in = _t(mlp_in_h, [S2, Fmlp], ctx)
    var mlp_h = gelu(mlp_in, ctx)

    var out_in = concat(1, ctx, att_flat, mlp_h)               # [S2, D+Fmlp]
    var out_in_h_f32 = out_in.to_host(ctx)

    var b2 = Optional[Tensor](w.b2[].clone(ctx))
    var out_base = linear(out_in, w.w2[], b2, ctx).to_host(ctx)
    var out_h = flux_lora_apply(out_base, out_in_h_f32, lora.linear2, S2, ctx)
    var out_proj = _t(out_h, [S2, D], ctx)

    var result = residual_gate(x_t, gate_bf16, out_proj, ctx)  # per-sample [2,D]
...
# ── SINGLE block b2 backward ─────────────────────────────────────────────────
def single_block_lora_backward_b2[
    H: Int, Dh: Int, S: Int
](
    d_out: List[Float32],
    w: SingleBlockWeights, sgl_mod_flat2: List[Float32], lora: SingleBlockLora,
    ...
    D: Int, Fmlp: Int, eps: Float32,
    ctx: DeviceContext,
) raises -> SingleBlockLoraBackwardB2:
    var scale = Float32(1.0) / sqrt(Float32(Dh))
    var ones_t = _t(_ones(D), [D], ctx)
    var S2 = 2 * S
    var scale_bf16 = _mod2_bf16(sgl_mod_flat2, 3, 1, D, ctx)
    var gate_f32 = _mod2_f32(sgl_mod_flat2, 3, 2, D, ctx)

    var d_out_t = _tf32(d_out, [S2, D], ctx)
    var x_t = _tb16(saved.x.copy(), [S2, D], ctx)
    var out_in_t = _tb16(saved.out_in.copy(), [S2, D + Fmlp], ctx)
    ...
    for i in range(len(saved.out_in)):
        out_in_vals_f32.append(Float32(saved.out_in[i]))
    var out_y_h = flux_lora_apply(out_base, out_in_vals_f32, lora.linear2, S2, ctx)
    var out_y = _t(out_y_h, [S2, D], ctx)
    var grg = gate_residual_backward(d_out_t, x_t, gate_f32, out_y, ctx, compute_gate_grad=False)
    ...
    var cb = cat_backward(dx_w2_f32, D, Fmlp, 2, ctx)
    reshape_in_place(cb.d_0, [2, S, H, Dh])
    reshape_in_place(cb.d_1, [S2, Fmlp])
    var d_att_flat_b = _to_bf16(cb.d_0, ctx)
    var d_mlp_h_b = _to_bf16(cb.d_1, ctx)

    var mlp_in_t = _tb16(saved.mlp_in.copy(), [S2, Fmlp], ctx)
    var d_mlp_in = gelu_backward(d_mlp_h_b, mlp_in_t, ctx)

    var q_rope_t = _tb16(saved.q_rope.copy(), [2, S, H, Dh], ctx)
    var k_rope_t = _tb16(saved.k_rope.copy(), [2, S, H, Dh], ctx)
    var v_t = _tb16(saved.v.copy(), [2, S, H, Dh], ctx)
    var sb = sdpa_backward[2, S, H, Dh](q_rope_t, k_rope_t, v_t, d_att_flat_b, scale, ctx)

    var d_q_rms = rope_backward(_to_f32(sb.d_q, ctx), cos2, sin2, True, ctx)
    var d_k_rms = rope_backward(_to_f32(sb.d_k, ctx), cos2, sin2, True, ctx)

    var q_pre_t = _tb16(saved.q_pre.copy(), [2, S, H, Dh], ctx)
    var k_pre_t = _tb16(saved.k_pre.copy(), [2, S, H, Dh], ctx)
    var rb_q = rms_norm_backward(_to_bf16(d_q_rms, ctx), q_pre_t, w.q_norm[], eps, ctx)
    var rb_k = rms_norm_backward(_to_bf16(d_k_rms, ctx), k_pre_t, w.k_norm[], eps, ctx)

    reshape_in_place(rb_q.d_x, [S2, D])
    reshape_in_place(rb_k.d_x, [S2, D])
    reshape_in_place(sb.d_v, [S2, D])

    var d_qkv = concat(1, ctx, rb_q.d_x, rb_k.d_x, sb.d_v)     # [S2, 3D]
    var d_fused = concat(1, ctx, d_qkv, d_mlp_in)              # [S2, 3D+Fmlp]

    var norm_t = _tb16(saved.norm.copy(), [S2, D], ctx)
    var lb_w1 = linear_backward(d_fused, norm_t, w.w1[], S2, D, 3 * D + Fmlp, ctx)
    var d_norm = lb_w1.d_x.to_host(ctx)
    ...
    var d_mlp_in_h = d_mlp_in.to_host(ctx)
    if lora.to_q:
        var lg = flux_lora_bwd(d_q_h, norm_vals_f32, lora.to_q.value(), S2, ctx)
        d_a_slots[S_SQ] = lg.d_a.copy(); d_b_slots[S_SQ] = lg.d_b.copy()
        d_norm = _add_lists(d_norm, lg.d_x)
    if lora.to_k:
        var lg = flux_lora_bwd(d_k_h, norm_vals_f32, lora.to_k.value(), S2, ctx)
        d_a_slots[S_SK] = lg.d_a.copy(); d_b_slots[S_SK] = lg.d_b.copy()
        d_norm = _add_lists(d_norm, lg.d_x)
    if lora.to_v:
        var lg = flux_lora_bwd(d_v_h, norm_vals_f32, lora.to_v.value(), S2, ctx)
        d_a_slots[S_SV] = lg.d_a.copy(); d_b_slots[S_SV] = lg.d_b.copy()
        d_norm = _add_lists(d_norm, lg.d_x)
    if lora.proj_mlp:
        var lg = flux_lora_bwd(d_mlp_in_h, norm_vals_f32, lora.proj_mlp.value(), S2, ctx)
        d_a_slots[S_PMLP] = lg.d_a.copy(); d_b_slots[S_PMLP] = lg.d_b.copy()
        d_norm = _add_lists(d_norm, lg.d_x)

    var ln_t = _tb16(saved.ln.copy(), [S2, D], ctx)
    var mb = modulate_backward(_t(d_norm, [S2, D], ctx), ln_t, scale_bf16, ctx, compute_param_grads=False)
    var lnb_dx = layer_norm_backward_dx(mb.d_x, x_t, ones_t, eps, ctx)
    var d_x_res = grg.d_x.to_host(ctx)
    var d_x_norm = lnb_dx.to_host(ctx)
    var d_x = _add_lists(d_x_res, d_x_norm)
    return SingleBlockLoraBackwardB2(d_x^, SingleBlockLoraGrads(d_a_slots^, d_b_slots^))
