...
#
# Size support: the proven 512x512 path plus the finite shared seven-shape 1MP
# image ladder. Product-size decode tries a whole-frame VAE in a clean child
# first; the rectangular-safe overlap-blended tiled VAE is the loud OOM fallback.
#
...
from serenitymojo.models.vae.zimage_tiled_decode import (
    zimage_tiled_decode, zimage_tiled_decode_with_decoder,
)
...
from serenitymojo.serve.zimage_decode_subprocess import decode_whole_subprocess
...
comptime ZIMAGE_PRODUCT_EDGE_UNITS = 16


def _zimage_shape_supported(width: Int, height: Int) -> Bool:
    if width == 512 and height == 512:
        return True
    comptime for bi in range(DEFAULT_ASPECT_LADDER_LEN):
        comptime X100_BI = DEFAULT_ASPECT_LADDER_X100[bi]
        comptime LH_BI = aspect_lat_h_units(X100_BI, ZIMAGE_PRODUCT_EDGE_UNITS)
        comptime LW_BI = aspect_lat_w_units(X100_BI, ZIMAGE_PRODUCT_EDGE_UNITS)
        if width == LW_BI * 8 and height == LH_BI * 8:
            return True
    return False

# 16GB headroom gate: below this free-VRAM level the ~10.8 GB encoder child
# cannot run beside the resident DiT — evict the blocks first (see
# _evict_dit_blocks_for_encode). Mirrors zimage_encode_subprocess's preflight.
comptime _ENCODE_EVICT_MIN_FREE_BYTES = Int(10800) * 1024 * 1024
# VAE decode headroom gate: whole-image child decode preflights at 6144 MiB
# and the tiled fallback still OOMs beside the resident DiT on 16GB — evict
# the blocks when free VRAM is below this before decoding.
comptime _DECODE_EVICT_MIN_FREE_BYTES = Int(8192) * 1024 * 1024
...
struct ZImageBackend(GenBackend, Movable):
    ...
    def start(mut self, params: JobParams) raises:
        ...
        # Finite comptime dispatch: preserve the proven 512 square and admit the
        # exact shared seven-shape 1MP ladder used by the other image backends.
        if not _zimage_shape_supported(params.width, params.height):
            raise Error(
                String("zimage: unsupported size ") + String(params.width)
                + "x" + String(params.height)
                + " — supported sizes are 512x512 and the compiled 1MP aspect ladder."
            )
    ...
    def _evict_dit_blocks_for_encode(mut self) raises:
        """16GB card: the resident DiT (~11.5 GB) leaves ~2.3 GB free — the
        ~10.8 GB encoder child can never fork and the in-process fallback
        OOMs (the 24GB-authored failure). Drop ONLY the DiT block lists
        (aux/final_lin/pads survive; st_open is already closed post-load),
        trim the pool back to the OS, and let PHASE_LOAD re-chunk-load the
        blocks from page cache after the encode. Same weights reloaded —
        no numerics change; the cost is a few seconds per prompt change."""
        print("[zimage] 16GB headroom: evicting resident DiT blocks for the encoder child")
        self.nr_blocks = List[ZImageBlockWeights]()
        self.cr_blocks = List[ZImageBlockWeights]()
        self.main_blocks = List[ZImageBlockWeights]()
        self.load_cursor = 0
        ...
        cu_mempool_trim_current(0)
        self.ctx.synchronize()
        _print_vram("after DiT evict for encode")

    def _evict_all_dit_state_for_decode(mut self) raises:
        """Release every DiT-only allocation after the final denoise step.

        On the 16 GiB product card, retaining aux/final tensors in addition to
        the freed block pool moved the 512 VAE over the measured headroom by
        ~120 MiB. No transformer state is used by decode, and the worker reloads
        it for the next job, so keep only the final latent and conditioning-free
        output state before constructing the VAE.
        """
        print("[zimage] releasing all DiT state before VAE decode")
        self.nr_blocks = List[ZImageBlockWeights]()
        self.cr_blocks = List[ZImageBlockWeights]()
        self.main_blocks = List[ZImageBlockWeights]()
        self.st_open = List[ArcPointer[ShardedSafeTensors]]()
        ...
        cu_mempool_trim_current(0)
        self.ctx.synchronize()
        _print_vram("after full DiT-state release for decode")

    def _cap_cache_hit(self) -> Bool:
        """True when the conditioning cache already covers this job's prompts."""
        return (
    ...
    def _prepare_job(mut self) raises:
        ...
        if self._cap_cache_hit():
            ...
            # 16GB order flip: with the DiT resident there is never headroom
            # for the encoder (child needs ~10.8 GB; ~2.3 GB free). Evict the
            # blocks first — PHASE_LOAD re-chunk-loads them after the encode.
            if self.loaded and cu_mem_get_info().free_bytes < _ENCODE_EVICT_MIN_FREE_BYTES:
                self._evict_dit_blocks_for_encode()
    ...
    def _encode_init_latent_shape[LH_: Int, LW_: Int](
        mut self, image_t: Tensor
    ) raises -> List[Float32]:
        var enc = ZImageVaeEncoder[LH_, LW_].load(String(VAE_DIR), self.ctx)
    ...
    def _encode_init_latent(mut self) raises -> List[Float32]:
        ...
        if self.params.width == 512 and self.params.height == 512:
            var z512 = self._encode_init_latent_shape[64, 64](image_t)
        ...
        comptime for bi in range(DEFAULT_ASPECT_LADDER_LEN):
            ...
            comptime LH_BI = aspect_lat_h_units(X100_BI, ZIMAGE_PRODUCT_EDGE_UNITS)
            comptime LW_BI = aspect_lat_w_units(X100_BI, ZIMAGE_PRODUCT_EDGE_UNITS)
            if self.hl == LH_BI and self.wl == LW_BI:
                var z = self._encode_init_latent_shape[LH_BI, LW_BI](image_t)
                _print_vram("after init-image encode (encoder freed on return)")
                return z^
        raise Error("zimage: admitted init-image encoder shape was not compiled")
    ...
    def _decode_and_save_product_shape[LH_: Int, LW_: Int](mut self) raises -> String:
        ...
        self.lora_dev = List[ZImageLoraDeviceSet]()
        self.latent = List[TArc]()
        # Whole-image decode in a clean child first; tiled decode is only the
        # loud OOM/failure fallback because it measurably degrades output.
        try:
            var rgb_whole = decode_whole_subprocess(
                _cast(decode_latent, STDtype.BF16, self.ctx), LH_, LW_, self.ctx
            )
            print("[zimage] whole-image VAE decode via child process (no tiling)")
            rgb_whole = self._apply_lanpaint_mask_blend(rgb_whole^)
            _save_rgb_png_with_text(rgb_whole, png_path, self.params.params_json, self.ctx)
        except e:
            print("[zimage] whole-image child decode unavailable (", e,
                  ") → tiled VAE decode")
            var rgb: Tensor
            comptime if LH_ == 128 and LW_ == 128:
                if len(self.vae64) == 0:
                ...
                rgb = zimage_tiled_decode_with_decoder[128, 128, 64, 64](
                    _cast(decode_latent, STDtype.BF16, self.ctx), self.vae64[0][], self.ctx
                )
            else:
                print("[zimage] tiled VAE decode (rectangular 3x3 half-latent crops)")
                rgb = zimage_tiled_decode[LH_, LW_](
                    _cast(decode_latent, STDtype.BF16, self.ctx), String(VAE_DIR), self.ctx
                )
    ...
    def _decode_and_save(mut self) raises -> String:
        ...
        comptime for bi in range(DEFAULT_ASPECT_LADDER_LEN):
            ...
            comptime LW_BI = aspect_lat_w_units(X100_BI, ZIMAGE_PRODUCT_EDGE_UNITS)
            if self.hl == LH_BI and self.wl == LW_BI:
                return self._decode_and_save_product_shape[LH_BI, LW_BI]()
        raise Error("zimage: admitted decode shape was not compiled")
        ...
        try:
            if self.phase == PHASE_LOAD:
                ...
                if self._load_chunk():
                    ...
                    self.load_cursor == 0
                    and len(self.nr_blocks) == 0
                    and len(self.aux) == 1
                    and not self._cap_cache_hit()
                ):
                    # 16GB order flip: aux just landed (tick 1) and this job
                    # needs a fresh encode — run the ~10.8 GB encoder child
                    # BEFORE the 11.5 GB block load, while the card is free.
                    # PHASE_ENCODE routes back here for the block load.
                    self.phase = PHASE_ENCODE
            ...
            if self.phase == PHASE_ENCODE:
                ...
                if self.loaded:
                    ...
                    # — chunk-(re)load them; the conditioning cache makes the
                    # second ENCODE pass a no-op encode.
                    self.phase = PHASE_LOAD
                r.step = 0
                return r^
            if self.phase == PHASE_DENOISE:
                ...
                if self.hl == 64 and self.wl == 64:
                    ...
                    comptime for bi in range(DEFAULT_ASPECT_LADDER_LEN):
                        ...
                        comptime LW_BI = aspect_lat_w_units(X100_BI, ZIMAGE_PRODUCT_EDGE_UNITS)
                        if self.hl == LH_BI and self.wl == LW_BI:
                            self._denoise_one[LH_BI, LW_BI]()
                            dispatched = True
                    if not dispatched:
                        raise Error("zimage: admitted denoise shape was not compiled")
            ...
            # 24GB residency contract: keep the DiT resident through decode.
            # 16GB reality: DiT-resident (+~13.9 GB) leaves ~1.1 GB — even the
            # tiled decoder OOMs (measured job-0066). The blocks are dead
            # weight after the final denoise step: evict them and reload
            # per job (page-cache chunk reload), trading residency for a
            # worker that completes at all.
            if self.loaded and cu_mem_get_info().free_bytes < _DECODE_EVICT_MIN_FREE_BYTES:
                self._evict_all_dit_state_for_decode()
