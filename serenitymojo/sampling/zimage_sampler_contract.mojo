# sampling/zimage_sampler_contract.mojo - Z-Image SerenityTrainer sampler contract.
#
# This is a source contract, not a denoiser and not image or speed parity. It
# pins the local SerenityTrainer ZImageSampler/BaseZImageSetup essentials that a real
...
def validate_zimage_sampler_contract(contract: ZImageSamplerContract) raises:
    ...
    if contract.external_pack_latents or contract.external_unpack_latents:
        raise Error("Z-Image sampler contract: SerenityTrainer has no external latent pack/unpack in the sampler")
