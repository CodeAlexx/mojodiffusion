# tests/oft_serenity_trainer_backward_fd.mojo — finite-difference gate for the
# SerenityTrainer-OFT analytic backward (oft_serenity_backward). The forward is already
# verified vs SerenityTrainer (oft_serenity_trainer_parity.mojo cos~1.0), so FD against the
...
#     serenitymojo/training/tests/oft_serenity_trainer_backward_fd.mojo -o /tmp/oft_serenity_bwd_fd \
#   && /tmp/oft_serenity_bwd_fd
...
def main() raises:
    ...
    var ne = b * (b - 1) // 2
    print("[oft SerenityTrainer backward FD] IN=", IN, " OUT=", OUT, " M=", M, " b=", b, " r=", r)
    ...
    var g = oft_serenity_backward(d_y.copy(), x.copy(), vec.copy(), W.copy(), M, IN, OUT, b, r)
    ...
    for t in range(r * ne):
        ...
        var yp = oft_serenity_forward(x.copy(), vp, W.copy(), M, IN, OUT, b, r)
        var ym = oft_serenity_forward(x.copy(), lm, W.copy(), M, IN, OUT, b, r)
    ...
    for t in range(M * IN):
        ...
        var yp = oft_serenity_forward(xp, vec.copy(), W.copy(), M, IN, OUT, b, r)
        var ym = oft_serenity_forward(xm, vec.copy(), W.copy(), M, IN, OUT, b, r)
    ...
    if cv >= COS_BAR and rv <= REL_BAR and cx >= COS_BAR and rx <= REL_BAR:
        print("PASS — SerenityTrainer-OFT analytic backward matches FD (d_vec + d_x)")
    else:
