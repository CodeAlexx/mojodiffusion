#!/usr/bin/env python3
"""Static/header guard for SDXL SerenityTrainer LoRA key parity."""
...
TRAIN_REF_PREFIX = Path("/home/alex/serenity_trainer-mojo/parity/sdxl_train_ref_step000")

SERENITY_SETUP = SERENITY_TRAINER / "modules/modelSetup/StableDiffusionXLLoRASetup.py"
SERENITY_BASE_SETUP = SERENITY_TRAINER / "modules/modelSetup/BaseStableDiffusionXLSetup.py"
SERENITY_DATALOADER = SERENITY_TRAINER / "modules/dataLoader/StableDiffusionXLBaseDataLoader.py"
SERENITY_SAMPLER = SERENITY_TRAINER / "modules/modelSampler/StableDiffusionXLSampler.py"
SERENITY_TRAINER = SERENITY_TRAINER / "modules/trainer/GenericTrainer.py"
SERENITY_SAVER_MIXIN = SERENITY_TRAINER / "modules/modelSaver/mixin/LoRASaverMixin.py"
SERENITY_CONFIG = SERENITY_TRAINER / "configs/sdxl_100step_baseline.json"
SERENITY_METRICS = SERENITY_TRAINER / "output/sdxl_100step_baseline/metrics.json"
SERENITY_BASELINE = SERENITY_TRAINER / "output/sdxl_100step_baseline/lora_last.safetensors"
SERENITY_REPLAY_DUMP = SERENITY_TRAINER / "output/sdxl_100step_baseline/step000_replay.safetensors"
SERENITY_REPLAY_MANIFEST = SERENITY_TRAINER / "output/sdxl_100step_baseline/step000_replay_manifest.json"
...
NEXT_REPLAY_GATES = (
    ...
    (
        ...
        "must compare per-adapter SDXL LoRA dA/dB against SerenityTrainer on the same dumped step",
    ),
    (
        "AdamW update replay",
...
def require_block(
    ...
    print(f"[sdxl-lora-keys] SerenityTrainer {label}: PASS")
...
def check_baseline_config() -> dict[str, Any]:
    config = load_json(SERENITY_CONFIG)
    ...
    if mismatches:
        raise SystemExit(
            f"[sdxl-lora-keys] baseline config mismatch: {SERENITY_CONFIG}\n  "
            + "\n  ".join(mismatches)
        )
    ...
    if output_path != SERENITY_BASELINE:
        raise SystemExit(
            f"[sdxl-lora-keys] output_model_destination {output_path} != {SERENITY_BASELINE}"
        )
    ...
    print(
        "[sdxl-lora-keys] SerenityTrainer baseline config: PASS "
...
def check_baseline_metrics(config: dict[str, Any]) -> None:
    metrics = load_json(SERENITY_METRICS)
    ...
    if Path(str(metrics.get("output_model_destination", ""))) != SERENITY_BASELINE:
    ...
    print(
        "[sdxl-lora-keys] SerenityTrainer 100-step baseline metrics: PASS "
...
def check_serenity_trainer_sources() -> None:
    dataloader = read_text(SERENITY_DATALOADER)
    for needle in (
        ...
        require(dataloader, needle, "reference trainer SDXL cache/data output contract")
    print("[sdxl-lora-keys] SerenityTrainer SDXL cache/data source contract: PASS")

    setup = read_text(SERENITY_BASE_SETUP)
    for needle in (
        ...
        require(setup, needle, "reference trainer SDXL predict/loss contract")
    print("[sdxl-lora-keys] SerenityTrainer SDXL predict/loss source contract: PASS")

    sampler = read_text(SERENITY_SAMPLER)
    for needle in (
        ...
        require(sampler, needle, "reference trainer SDXL sampler contract")
    print("[sdxl-lora-keys] SerenityTrainer SDXL sampler source contract: PASS")

    saver = read_text(SERENITY_SAVER_MIXIN)
    for needle in (
        ...
        require(saver, needle, "reference trainer SDXL LoRA saver contract")
    print("[sdxl-lora-keys] SerenityTrainer LoRA saver source contract: PASS")
...
def validate_replay_dump(path: Path) -> None:
    ...
    print(f"[sdxl-lora-keys] SerenityTrainer replay dump header: PASS file={path}")


def validate_replay_manifest(path: Path) -> None:
    manifest = load_json(path)
    expected_config = str(SERENITY_CONFIG)
    if str(manifest.get("serenity_trainer_config")) != expected_config:
        raise SystemExit(
            f"[sdxl-lora-keys] replay manifest serenity_trainer_config={manifest.get('serenity_trainer_config')!r}, "
    ...
    print(f"[sdxl-lora-keys] SerenityTrainer replay dump manifest: PASS file={path}")
...
def validate_train_ref_dump(path: Path) -> None:
    ...
    print(f"[sdxl-lora-keys] SerenityTrainer train-ref step artifact: PASS file={path}")
...
def validate_train_ref_adapter_dump(path: Path) -> None:
    ...
    print(f"[sdxl-lora-keys] SerenityTrainer train-ref adapter artifact: PASS file={path}")
...
def report_replay_plan(dump_path: Path, manifest_path: Path, require_artifact: bool) -> None:
    setup = read_text(SERENITY_BASE_SETUP)
    has_dump_hook = "SERENITY_DUMP_STEP1_INPUTS" in setup and "safetensors.torch" in setup
    ...
    print(f"  source_anchor: {SERENITY_BASE_SETUP}::predict")
    print(f"  trainer_anchor: {SERENITY_TRAINER}::train")
    ...
    if not has_dump_hook:
        print("[sdxl-lora-keys] WARN SerenityTrainer SDXL predict has no SERENITY_DUMP_STEP1_INPUTS hook")
...
def check_serenity_source() -> None:
    ...
    require(setup, 'model.unet, "lora_unet"', "reference trainer UNet prefix")
    require(setup, "config.layer_filter.split", "reference trainer layer filter")
    print("[sdxl-lora-keys] SerenityTrainer source contract: PASS")
...
def check_saved_header(path: Path, expected_adapters: int | None, expected_rank: int | None) -> None:
    ...
    for prefix in SAMPLE_SERENITY_KEYS:
        for suffix in SAVE_SUFFIXES:
            key = f"{prefix}.{suffix}"
            if key not in header:
    ...
    print(
        "[sdxl-lora-keys] SerenityTrainer saved inventory: PASS "
...
def check_mojo_sources(strict_port: bool) -> None:
    ...
    require(stack, "def sdxl_lora_supported_unet_prefixes", "explicit supported reference trainer UNet surface")
    require(stack, "def sdxl_lora_unsupported_serenity_trainer_targets", "explicit unsupported reference trainer surface")
    ...
    if "save_lora_serenity_trainer" not in stack:
        blockers.append("Mojo SDXL save hook does not call save_lora_serenity_trainer")

    for prefix in IMPLEMENTED_ST_PREFIXES:
        require(stack, f'return "{prefix}"', f"reference trainer ST prefix mapping {prefix}")
    for suffix in IMPLEMENTED_ST_SUFFIXES:
        require(stack, f'return "{suffix}"', f"reference trainer ST suffix mapping {suffix}")
...
def main() -> int:
    ...
    parser.add_argument(
        "--replay-dump",
        type=Path,
        default=SERENITY_REPLAY_DUMP,
        help="expected local SerenityTrainer SDXL one-step replay safetensors artifact",
    )
    parser.add_argument(
        "--replay-manifest",
        type=Path,
        default=SERENITY_REPLAY_MANIFEST,
        help="expected local SerenityTrainer SDXL one-step replay manifest",
    )
    parser.add_argument(
        "--require-replay-dump",
        action="store_true",
    ...
    check_serenity_trainer_sources()
