#!/usr/bin/env python
"""Lens SerenityTrainer oracle — dumps parity references from SerenityTrainer's OWN deps.

Reference policy: SerenityTrainer ONLY. The `lens` package (lens/transformer.py) is
SerenityTrainer's actual dependency (modules/model/LensModel.py imports it), so it is
the 1:1 forward-math source. NO Rust / inference-flame.

Run with SerenityTrainer's venv:
  python serenitymojo/models/lens/parity_serenity/lens_oracle.py
