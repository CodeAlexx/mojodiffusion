...
the later SerenityTrainer step required to prove a nonzero AdamW/update oracle.
"""
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Inspect a SerenityTrainer adapter dump and report whether a later "
...
def required_update_artifacts(
    ...
    requirements = [
        {
            "name": "step dump",
            "requirement": f"SerenityTrainer tensor dump for {model} optimizer step 1",
            **path_info(paths["step_safetensors"]),
        },
        {
            "name": "adapter dump",
            "requirement": (
                "SerenityTrainer adapter_before/adapter_pre/adapter_post/adapter_after "
...
def positive_lr_dump_template(
    ...
    return {
        ...
        "note": (
            ...
            "as update-bearing SerenityTrainer oracle evidence until the referenced step001 "
