...
#
# Opens the real SerenityTrainer ZImage step-0 dump, patchifies the dumped BF16
...
def _require_tensor(
    ...
    _require(key in st.tensors, String("missing ZImage reference trainer tensor ") + key)
...
def main() raises:
    ...
    print(
        "[zimage-train-ref-device-loss] scope=real SerenityTrainer step0 dump through v5 device flow-MSE d_patches root; not transformer forward/backward parity"
    )
