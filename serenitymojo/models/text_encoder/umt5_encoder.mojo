...
def _umt5_canonical_key(name: String) raises -> String:
    """Map official Diffusers UMT5 names to the creator/Mojo schema.

    Already-canonical NAVA checkpoints pass through unchanged.
    """
    if name == "shared.weight":
        return String("token_embedding.weight")
    if name == "encoder.final_layer_norm.weight":
        return String("norm.weight")
    if name.startswith("blocks.") or name == "token_embedding.weight" or name == "norm.weight":
        return name
    if not name.startswith("encoder.block."):
        raise Error(String("unsupported UMT5 weight key: ") + name)
    var out = name.replace("encoder.block.", "blocks.")
    out = out.replace(
        ".layer.0.SelfAttention.relative_attention_bias.weight",
        ".pos_embedding.embedding.weight",
    )
    out = out.replace(".layer.0.SelfAttention.q.weight", ".attn.q.weight")
    out = out.replace(".layer.0.SelfAttention.k.weight", ".attn.k.weight")
    out = out.replace(".layer.0.SelfAttention.v.weight", ".attn.v.weight")
    out = out.replace(".layer.0.SelfAttention.o.weight", ".attn.o.weight")
    out = out.replace(".layer.0.layer_norm.weight", ".norm1.weight")
    out = out.replace(
    ...
    out = out.replace(".layer.1.layer_norm.weight", ".norm2.weight")
    if ".layer." in out or ".SelfAttention." in out or ".DenseReluDense." in out:
        raise Error(String("unmapped UMT5 weight key: ") + name)
    return out^
...
struct Umt5Encoder[S: Int = 42]:
    ...
    def load(
        ...
        for ref nm in sharded.names():
            ...
            var canonical = _umt5_canonical_key(String(nm))
            if canonical in name_to_idx:
                raise Error(String("duplicate canonical UMT5 key: ") + canonical)
    ...
    def _layer(
        ...
        var position_bias = add(
            self._layer_bias(layer_idx, bucket_ids, ctx), key_padding_bias, ctx
        )
    ...
    def encode(self, token_ids: List[Int], ctx: DeviceContext) raises -> Tensor:
        ...
        return self.encode_masked(token_ids, Self.S, ctx)

    def encode_masked(
        self, token_ids: List[Int], valid_tokens: Int, ctx: DeviceContext
    ) raises -> Tensor:
        """Encode with the creator UMT5 key-padding mask.

        `valid_tokens` leading positions are visible as attention keys; trailing
        positions receive a large negative additive score bias. Queries remain
        present, matching the official UMT5 encoder's `[B,S]` attention mask.
        """
        var cfg = self.config
        comptime SS = Self.S
        if len(token_ids) != SS:
        ...
        if valid_tokens < 0 or valid_tokens > SS:
            raise Error(
                String("Umt5Encoder.encode_masked: valid_tokens must be in [0,")
                + String(SS) + String("], got ") + String(valid_tokens)
            )
        ...
        # Official Wan UMT5 passes a `[B,S]` attention mask. Broadcast a tiny
        # `[1,1,1,S]` key-column bias over batch, heads, and query rows instead
        # of materializing another `[1,64,S,S]` tensor. -3e38 is finite BF16
        # and softmaxes to exactly zero for padded key columns.
        var key_mask_h = List[Float32](capacity=SS)
        for i in range(SS):
            key_mask_h.append(
                Float32(0.0) if i < valid_tokens else Float32(-3.0e38)
            )
        var key_mask = Tensor.from_host(
            key_mask_h,
            [1, 1, 1, SS],
            embed_w.dtype(),
            ctx,
        ...
        for i in range(cfg.num_layers):
            hidden = self._layer(i, hidden, bucket_ids, key_mask, ctx)

        # 4. Final layer norm, then enforce the Wan conditioning-cache
        # contract: valid rows followed by exact-zero padding.  The official
        # encoder still computes padded query rows, so the key-column mask
        # above is not sufficient by itself.
        var output = rms_norm(
            hidden, self._w(String("norm.weight")), cfg.layer_norm_eps, ctx
        )
        var output_mask_h = List[Float32](capacity=SS)
        for i in range(SS):
            output_mask_h.append(
                Float32(1.0) if i < valid_tokens else Float32(0.0)
            )
        var output_mask = Tensor.from_host(
            output_mask_h, [SS, 1], embed_w.dtype(), ctx
        )
        return mul(output, output_mask, ctx)
