# serenitymojo/models/anima/parity/anima_serenity_step_parity.mojo
#
# CHUNK-A RECIPE PARITY GATE for the SerenityTrainer-faithful Anima LoRA STEP.
#
# Reads the FIXED inputs written by anima_serenity_step_oracle.py (scaled_latent grid,
# noise grid, frozen context[1,512,1024]) and runs the SAME reference trainer recipe through the
...
comptime S_TXT = 512                    # delta 1: reference trainer context length
...
# ── 3D-RoPE (T,H,W) NTK tables (matches train_anima_serenity._rope_tables) ───
# REAL Anima 3-axis rope (anima_dit.build_anima_3d_rope / CosmosRotaryPosEmbed),
# rope_scale (t=1.0, h=4.0, w=4.0). Replaces the reference trainer-unfaithful single-axis table.
...
def main() raises:
    ...
    print("==== anima_serenity_step_parity (Chunk A reference trainer recipe vs torch) ====")
    ...
    var scaled = _in("serenity_scaled_bthwc")   # [B,1,Hd,Wd,C] flat
    var noise = _in("serenity_noise_bthwc")
    var context = _in("serenity_context")       # [B,512,1024]
    ...
    var ref_pred = _in("serenity_pred")
    var ref_loss_l = _in("serenity_loss")
