...
# (the serenity-trainer UI keys) are SerenityTrainer modules/util/loss/
...
# ── MAE / L1 (torch.nn.functional.l1_loss, mean reduction) ───────────────────
def mae_loss_grad(pred: List[Float32], target: List[Float32]) raises -> LossGrad:
...
# ── masked weighted losses (T1.E; see header for ST/SerenityTrainer citations) ────
def mask_weights(
    mask: List[Float32], unmasked_weight: Float32
) raises -> List[Float32]:
    """Per-element loss weights from a mask: w_i = clamp(mask_i, uw, 1).

    SerenityTrainer masked_loss.py:11 `torch.clamp(mask, unmasked_weight, 1)`
...
def _normalize_in_place(
    ...
    """Divide loss + grad by mean(weights) — SerenityTrainer masked_loss.py:15-16
...
def masked_mse_loss_grad(
    ...
    normalize per SerenityTrainer masked_loss.py:15-16. Existing mse_loss_grad is
...
def masked_mae_loss_grad(
    pred: List[Float32], target: List[Float32], weights: List[Float32],
    normalize: Bool,
) raises -> LossGrad:
    """loss = mean(w*|pred-target|) [/mean(w)]; d_pred = w*sign(d)/N [/mean(w)].

    Masked sibling of mae_loss_grad (torch l1_loss semantics, subgradient 0 at
    d==0). Same weighting/normalize contract as masked_mse_loss_grad — closes
    the LOSS_FN_MAE gap in the masked dispatch (skeptic D1, 2026-07-16)."""
    _check_inputs(pred, target, "masked_mae_loss_grad")
    _check_weights(pred, weights, "masked_mae_loss_grad")
    var n = len(pred)
    var inv_n = Float32(1.0) / Float32(n)
    var acc = 0.0
    var wsum = 0.0
    ...
    var loss64 = acc / Float64(n)
    if normalize:
        _normalize_in_place(loss64, d_pred, wsum, n, "masked_mae_loss_grad")
    return LossGrad(Float32(loss64), d_pred^)
