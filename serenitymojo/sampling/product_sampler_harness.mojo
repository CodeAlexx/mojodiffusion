# sampling/product_sampler_harness.mojo - SerenityTrainer product sampler lifecycle.
#
# This is a fail-loud harness contract, not a denoiser. SerenityTrainer samples run:
...
struct ProductSamplerRunContract(Copyable, Movable):
    var plan: SerenityTrainerSamplerPlan
...
struct SamplerProductMeasurements(Copyable, Movable):
    var serenity_baseline_seconds_per_step: Float64
    var serenity_peak_vram_mib: Int
...
def build_product_sampler_run_contract(
    ...
    var plan = serenity_sampler_plan_with_sample_overrides(
        model_type,
        prompt.width,
        prompt.height,
        prompt.steps,
...
def sampler_product_scaffold_status() -> SamplerProductStageStatus:
    return SamplerProductStageStatus(
        ...
        False,  # SerenityTrainer progress callback count is not proven here.
...
def validate_product_sampler_run_contract(run: ProductSamplerRunContract) raises:
    validate_serenity_sampler_plan(run.plan)
    ...
    if run.progress_callback_name != String("on_update_progress"):
        raise Error("product sampler harness: progress callback must mirror SerenityTrainer")
    if run.output_callback_name != String("on_sample"):
        raise Error("product sampler harness: output callback must mirror SerenityTrainer")


def sampler_speed_ratio(measurements: SamplerProductMeasurements) -> Float64:
    if measurements.serenity_baseline_seconds_per_step <= Float64(0.0):
        return Float64(0.0)
    return measurements.mojo_seconds_per_step / measurements.serenity_baseline_seconds_per_step
...
def validate_sampler_measurements(
    ...
    if measurements.serenity_baseline_seconds_per_step <= Float64(0.0):
        raise Error("product sampler harness: missing SerenityTrainer seconds/step baseline")
    if measurements.serenity_peak_vram_mib <= 0:
        raise Error("product sampler harness: missing SerenityTrainer peak VRAM baseline")
