...
comptime NT = 116          # eri2 prose caption w/ vrtlEri2 trigger (2026-07-02)
...
def main() raises:
    ...
    var lora_path = String(a[6]) if len(a) > 6 else String("-")
    ...
    if lora_path != "-":
        var nl = cond_w.load_lora(lora_path, ctx)
        print("LoRA overlay applied:", nl, "tensors from", lora_path)
