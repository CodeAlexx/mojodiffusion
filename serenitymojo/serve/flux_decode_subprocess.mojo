# serenitymojo.serve.flux_decode_subprocess — FLUX's parity-preserving 3x3
# tiled VAE decode in a fork+execv child process.
#
# The parent keeps its offloaded DiT handle, turbo loader, and RoPE resident
# across jobs. The child receives the exact packed latent through io.cap_cache,
# unpacks it with flux_sample_cli's shared helper, executes the existing gated
# flux_tiled_decode, writes RGB, and exits. Process death reclaims every VAE
...
from serenitymojo.pipeline.flux_sample_cli import _unpack_latent_shape
...
# Measured in-process tiled decode adds about 6.5 GiB above the live worker
# floor. Require 9 GiB device-global free for the fresh child context, decoder,
# activations, and workspace margin; failure falls back to release+decode.
comptime _DECODE_CHILD_MIN_FREE_BYTES = Int(9216) * 1024 * 1024
comptime _FLUX_PRODUCT_EDGE_UNITS = 16
...
def _decode_child_shape[LH_: Int, LW_: Int](
    packed: Tensor, rgb_path: String, vae_path: String, ctx: DeviceContext
) raises:
    # The denoiser stores packed tokens in an F32-compatible tensor. Reuse the
    # CLI's exact unpack transform before the already parity-gated tiled VAE.
    var packed_f32 = cast_tensor(packed, STDtype.F32, ctx)
    var latent = _unpack_latent_shape[LH_, LW_](packed_f32, ctx)
    var rgb = flux_tiled_decode[LH_, LW_](latent, vae_path, ctx)
...
def decode_child_run(
    ...
    var packed = load_tensor_bin(packed_path, ctx)
    comptime for bi in range(DEFAULT_ASPECT_LADDER_LEN):
        comptime X100_BI = DEFAULT_ASPECT_LADDER_X100[bi]
        comptime LH_BI = aspect_lat_h_units(X100_BI, _FLUX_PRODUCT_EDGE_UNITS)
        comptime LW_BI = aspect_lat_w_units(X100_BI, _FLUX_PRODUCT_EDGE_UNITS)
        if lh == LH_BI and lw == LW_BI:
            _decode_child_shape[LH_BI, LW_BI](
                packed, rgb_path, vae_path, ctx
            )
            print("[flux-decode-child] wrote tiled rgb", rgb_path,
                  "grid=", lh, "x", lw)
            return
    raise Error(
        String("flux decode-child: unsupported latent grid ")
...
def decode_tiled_subprocess(
    ...
    var prefix = String("/tmp/serenity_flux_decode_") + String(_getpid())
    var packed_path = prefix + String(".packed.bin")
    var rgb_path = prefix + String(".rgb.bin")

    try:
        cu_mempool_trim_current(0)
    except e:
        print("[flux] pre-decode pool trim failed (continuing):", e)
    var free_bytes = cu_mem_get_info().free_bytes
    if free_bytes < _DECODE_CHILD_MIN_FREE_BYTES:
        raise Error(
            String("flux decode child preflight failed: free VRAM ")
    ...
    print("[flux] tiled decode -> fork VAE child; offloaded DiT remains resident (parent pid",
          _getpid(), ")")
    var pid = sys_fork()
    if pid == 0:
        ...
        sys__exit(127)
    if pid < 0:
        _unlink_file(packed_path)
        raise Error(String("flux decode child fork failed: ") + errno_str())
    ...
    if reaped != pid:
        ...
        raise Error("flux decode child timed out or waitpid failed")
    var exited_ok = (status & 0x7F) == 0 and ((status >> 8) & 0xFF) == 0
    if not exited_ok:
        _unlink_file(packed_path)
        _unlink_file(rgb_path)
        raise Error(
            String("flux decode child abnormal exit status ") + String(status)
        )

    var rgb = load_tensor_bin(rgb_path, ctx)
    _unlink_file(packed_path)
    _unlink_file(rgb_path)
    print("[flux] decode child reaped; tiled RGB loaded and offloaded DiT retained")
    return rgb^
