# serenity_trainer_product_run_smoke.mojo - SerenityTrainer train.py entrypoint smoke.
#
# Run:
#   timeout 180 prlimit --as=16000000000 pixi run mojo run -I . serenitymojo/training/serenity_trainer_product_run_smoke.mojo
...
from serenitymojo.training.serenity_trainer_product_run import (
    ...
    validate_serenity_trainer_model_loop_direct_invocation_ready,
...
from serenitymojo.training.serenity_trainer_preset_catalog import (
    ...
    serenity_trainer_preset_config_path,
    serenity_trainer_preset_reference_path,
    serenity_trainer_preset_summary,
...
def _check(cond: Bool, msg: String) raises:
    if not cond:
        raise Error(String("serenity_trainer_product_run_smoke FAILED: ") + msg)


def _write_qwen_config(path: String, concept_path: String, sample_path: String) raises:
    var content = String("{")
    content += '"model_type":"qwenimage",'
    content += '"workspace_dir":"/tmp/reference-product-qwen-workspace",'
    content += '"cache_dir":"/tmp/reference-product-qwen-cache",'
    content += '"output_model_destination":"/tmp/reference-product-qwen/out.safetensors",'
...
def _write_min_product_config(path: String, model_type: String, concept_path: String) raises:
    ...
    content += '"workspace_dir":"/tmp/reference-product-all-workspace",'
    content += '"cache_dir":"/tmp/reference-product-all-cache",'
    content += '"concept_file_name":"' + concept_path + String('",')
    content += '"training_method":"LORA",'
...
def _check_target_dispatch(
    ...
    var config_path = (
        String("/tmp/serenity_product_dispatch_")
        + model_type
        + String(".json")
    )
...
def _gate_all_target_dispatch() raises:
    ...
    var concept_path = String("/tmp/serenity_product_all_concepts.json")
...
def _gate_good_entrypoint() raises:
    print("--- SerenityTrainer train.py product entrypoint ---")
    var config_path = String("/tmp/serenity_product_qwen_config.json")
    var concept_path = String("/tmp/serenity_product_concepts.json")
    var sample_path = String("/tmp/serenity_product_samples.json")
    _write_file(concept_path, String("[]"))
    ...
    _check(plan.workspace_config_dir == String("/tmp/reference-product-qwen-workspace/config"), "workspace config dir")
    ...
    _check(
        serenity_trainer_train_runner_command(plan)
        == String("pixi run mojo run -I . serenitymojo/training/train_qwenimage_real.mojo /tmp/serenity_product_qwen_config.json 123"),
        "runner command",
    )
...
def _gate_disabled_sampling() raises:
    ...
    var config_path = String("/tmp/serenity_product_no_sample_config.json")
    var concept_path = String("/tmp/serenity_product_no_sample_concepts.json")
    _write_file(concept_path, String("[]"))
    var content = String("{")
    content += '"model_type":"STABLE_DIFFUSION_35",'
    content += '"workspace_dir":"/tmp/reference-product-sd35-workspace",'
    content += '"cache_dir":"/tmp/reference-product-sd35-cache",'
    content += '"output_model_destination":"/tmp/reference-product-sd35/out.safetensors",'
    content += '"concept_file_name":"' + concept_path + String('",')
    content += '"training_method":"LORA",'
    content += '"sample_after":0,'
    content += '"sample_after_unit":"NEVER",'
...
def _check_named_preset_dispatch(
    ...
    _check(serenity_trainer_preset_config_path(preset) == entry.config_path, preset + String(" config path"))
    _check(
        serenity_trainer_preset_reference_path(preset) == entry.serenity_reference_path,
        preset + String(" reference path"),
    )
    var plan = create_serenity_trainer_train_entrypoint_plan_from_preset(
        preset,
        concept_path,
        sample_path,
    )
    ...
    _check(plan.preset_reference_path == entry.serenity_reference_path, preset + String(" reference path"))
    _check(plan.preset_reference_kind == entry.serenity_reference_kind, preset + String(" reference kind"))
    ...
    _check(
        plan.config_path == String("/tmp/mojo-reference-presets/") + entry.preset_id + String(".json"),
    ...
    print("  preset ", serenity_trainer_preset_summary(entry))
...
def _gate_named_preset_catalog() raises:
    print("--- SerenityTrainer-style named preset catalog ---")
    var concept_path = String("/tmp/serenity_product_named_concepts.json")
    var sample_path = String("/tmp/serenity_product_named_samples.json")
    _write_file(concept_path, String("[]"))
    _write_file(sample_path, String("[]"))
...
def _gate_zimage_product_readiness() raises:
    ...
    var concept_path = String("/tmp/serenity_product_zimage_concepts.json")
    var sample_path = String("/tmp/serenity_product_zimage_samples.json")
    _write_file(concept_path, String("[]"))
    _write_file(sample_path, String("[]"))
...
def _gate_resume_preflight() raises:
    ...
    var concept_path = String("/tmp/serenity_product_resume_concepts.json")
    _write_file(concept_path, String("[]"))

    var good_workspace = String("/tmp/reference-product-resume-good")
    ...
    var config_path = String("/tmp/serenity_product_resume_good_config.json")
    ...
    var empty_workspace = String("/tmp/reference-product-resume-empty")
    _ = sys_system(String("rm -rf ") + empty_workspace)
    _ = sys_system(String("mkdir -p ") + empty_workspace)
    config_path = String("/tmp/serenity_product_resume_empty_config.json")
    _write_resume_config(config_path, empty_workspace, concept_path, True)
    cfg = read_model_config(config_path)
    plan = create_serenity_trainer_train_entrypoint_plan(config_path, cfg)
    validate_serenity_trainer_train_entrypoint_plan(plan)
    ...
    var missing_workspace = String("/tmp/reference-product-resume-missing-optimizer")
    ...
    config_path = String("/tmp/serenity_product_resume_missing_optimizer_config.json")
...
def _gate_fail_loud() raises:
    ...
    var config_path = String("/tmp/serenity_product_bad_config.json")
    var concept_path = String("/tmp/serenity_product_bad_concepts.json")
    var sample_path = String("/tmp/serenity_product_missing_samples.json")
    _write_file(concept_path, String("[]"))
    _write_qwen_config(config_path, concept_path, sample_path)
    ...
    var flux2_dev_alias_path = String("/tmp/serenity_product_flux2_dev_alias.json")
    ...
    var flux2_dev_heads_path = String("/tmp/serenity_product_flux2_dev_heads.json")
    ...
    var plain_sd3_path = String("/tmp/serenity_product_plain_sd3.json")
    ...
    cfg = read_model_config(config_path)
    cfg.sample_after_unit = SAMPLE_UNIT_NEVER
    cfg.concept_file_name = String("/tmp/serenity_product_missing_concepts.json")
    _expect_plan_raises(String("missing concept file"), config_path, cfg)

    cfg = read_model_config(config_path)
    _expect_plan_raises(String("missing config path"), String("/tmp/does-not-exist-reference-product.json"), cfg)

    var direct_path = String("/tmp/serenity_product_direct_invocation_config.json")
    _write_min_product_config(direct_path, String("qwenimage"), concept_path)
    cfg = read_model_config(direct_path)
    var direct_plan = create_serenity_trainer_train_entrypoint_plan(direct_path, cfg)
    validate_serenity_trainer_train_entrypoint_plan(direct_plan)
    var direct_raised = False
    try:
        validate_serenity_trainer_model_loop_direct_invocation_ready(direct_plan)
    except e:
        direct_raised = True
        var msg = String(e)
        ...
        _check(
            msg.find(serenity_trainer_train_runner_command(direct_plan)) >= 0,
            "direct invocation blocker includes dry-run command",
        )
    _check(direct_raised, "direct model-loop invocation must fail loud")

    var only_cache_path = String("/tmp/serenity_product_flux_only_cache.json")
    var only_cache_content = String("{")
    only_cache_content += '"model_type":"flux",'
    only_cache_content += '"workspace_dir":"/tmp/reference-product-flux-workspace",'
    only_cache_content += '"cache_dir":"/tmp/reference-product-flux-cache",'
...
def main() raises:
    print("==== SerenityTrainer product-run entrypoint smoke ====")
    ...
    _gate_fail_loud()
    print("serenity_trainer_product_run_smoke PASS")
