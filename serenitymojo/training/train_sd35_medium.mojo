...
# Iterates the real SerenityTrainer cache (latent/text_embedding/pooled), fresh noise
...
# ── libc getenv: True iff the env var is set to exactly "1" (mirrors the
# grad_coverage / transfer_benchmark_smoke helper). Used only for the H2D
# copy-mode A/B knob below; default-unset → async overlap (production path).
...
def main() raises:
    ...
    # ── H2D copy-mode A/B knob (default OFF = production async overlap) ──────────
    # The sd35 block loop already drives double-buffered prefetch
    # (prefetch_with_ctx(0) → await_block(bi) → prefetch_next_with_ctx(bi) →
    # mark_active_block_done), which stages block i+1's H2D on the loader's
    # explicit copy_stream (cuMemcpyHtoDAsync_v2) while block i computes on the
    # default stream — real async overlap, and it is the DEFAULT. This knob does
    # NOT add overlap machinery (it already exists); it exposes the loader's
    # existing synchronous ablation so the async win can be measured end-to-end:
    #   default (unset)   → use_default_stream_copy=False → async copy-stream overlap
    #   SD35_SYNC_H2D=1   → use_default_stream_copy=True  → H2D on the default stream
    # Run both and compare s/step to quantify the overlap. Unset is byte-identical
    # to the prior TurboPlannedLoader.open() path (fill_block_store defaults True).
    var sync_h2d = _env_is_set(String("SD35_SYNC_H2D"))
    var loader = TurboPlannedLoader.open_with_copy_mode(
        CKPT, plan^, cfg, ctx, sync_h2d
    )
    if sync_h2d:
        print("[offload] H2D copy mode = SYNC (default-stream ablation, A/B baseline)")
    else:
        print("[offload] H2D copy mode = ASYNC (copy-stream overlap, production)")
