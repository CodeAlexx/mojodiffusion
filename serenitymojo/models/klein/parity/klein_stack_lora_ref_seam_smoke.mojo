# serenitymojo/models/klein/parity/klein_stack_lora_ref_seam_smoke.mojo
#
# DEVICE-SEAM GATE for the img-EDIT reference hook wired into the LIVE Klein
# device-turbo forward/backward (klein_stack_lora.mojo, task #4a). The primary
# device-turbo functions
...
# now carry an OPT-IN ref hook whose ONLY new device ops at the input-projection
# seam are:
...
# The block stack in between is untouched by the hook. This gate exercises those
# EXACT device ops in isolation (a full end-to-end device-turbo run needs the
# real FLUX.2 TurboPlannedLoader weights on disk — deferred) and cross-checks
# them against the ALREADY-PROVEN base compute unit img_in_ref_forward /
...
#
# Importing KleinLoraGrads from klein_stack_lora forces the edited file (both
# wired variants + the new struct field) to COMPILE as part of this gate.
#
...
# Force compile of the edited LIVE device-turbo file (wired fwd+bwd + new field).
from serenitymojo.models.klein.klein_stack_lora import KleinLoraGrads
...
def main() raises:
    ...
    print("==== klein_stack_lora_ref_seam_smoke (device seam vs proven base unit) ====")
    ...
    # ── DEVICE SEAM FORWARD (the exact ops the wired fwd now runs) ──
    ...
    print("")
    print("---- forward: device linear+add  vs  img_in_ref_forward.img_act ----")
    ...
    # ── FLAGS-OFF: no-ref device path is JUST linear(noise, img_in) == base ──
    print("")
    print("---- flags-off: device no-ref img (linear only) == base _linear_fwd ----")
    ...
    # ── DEVICE SEAM BACKWARD (the exact op the wired bwd now runs) ──
    ...
    print("")
    print("---- backward: device linear_backward_dw  vs  img_in_ref_backward.d_img_in_ref_w ----")
    ...
    print("")
    if allok:
        print("VERDICT: PASS — device ref seam (linear+add fwd, linear_backward_dw bwd) matches the")
        print("               proven img_in_ref unit (cos>=0.999), flags-off no-ref path is max_abs 0,")
        print("               and d_img_in_ref is non-zero. Edited klein_stack_lora.mojo COMPILED.")
    else:
