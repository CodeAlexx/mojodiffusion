#!/usr/bin/env python3
"""Static/header guard for ERNIE SerenityTrainer LoRA key parity.

Checks the local SerenityTrainer Ernie LoRA wrapper/saver source, the Mojo Ernie
SerenityTrainer save path, and an optional safetensors header. The saved-header path
...
SERENITY_SETUP = SERENITY_TRAINER / "modules/modelSetup/ErnieLoRASetup.py"
SERENITY_MODEL = SERENITY_TRAINER / "modules/model/ErnieModel.py"
SERENITY_SAVER = SERENITY_TRAINER / "modules/modelSaver/ernie/ErnieLoRASaver.py"
SERENITY_SAVER_MIXIN = SERENITY_TRAINER / "modules/modelSaver/mixin/LoRASaverMixin.py"
SERENITY_LORA_MODULE = SERENITY_TRAINER / "modules/module/LoRAModule.py"
SERENITY_CONFIG = SERENITY_TRAINER / "configs/ernie_eri2_100step_baseline.json"
SERENITY_BASELINE = SERENITY_TRAINER / "output/ernie_eri2_100step_baseline/lora.safetensors"

STACK = REPO / "serenitymojo/models/ernie/ernie_stack_lora.mojo"
LORA_SAVE = REPO / "serenitymojo/training/lora_save.mojo"
SMOKE = REPO / "serenitymojo/models/ernie/parity/ernie_lora_serenity_save_key_smoke.mojo"
...
def expected_prefixes(num_layers: int) -> list[str]:
    return [saved_prefix(layer, module) for layer in range(num_layers) for module in SERENITY_MODULES]
...
def check_serenity_source(config_path: Path | None) -> tuple[int | None, str | None]:
    ...
    require(setup, 'model.transformer, "transformer"', "reference trainer transformer wrapper prefix")
    require(setup, "config.layer_filter.split(\",\")", "reference trainer layer filter")
    require(setup, "model.text_encoder.requires_grad_(False)", "reference trainer text encoder frozen")
    require(setup, "model.vae.requires_grad_(False)", "reference trainer VAE frozen")
    require(model, "return [a for a in [\n            self.transformer_lora,", "reference trainer Ernie adapters list")
    ...
    require(lora_module, "lora_down.weight", "reference trainer lora_down suffix")
    require(lora_module, "lora_up.weight", "reference trainer lora_up suffix")

    require(saver, "return None", "reference trainer Ernie no legacy conversion keyset")
    require(saver, "state_dict |= model.transformer_lora.state_dict()", "reference trainer transformer LoRA save")
    require(saver, "state_dict |= model.lora_state_dict", "reference trainer carried LoRA state save")
    ...
    if config_path is not None and config_path.exists():
        ...
        print(
            "[ernie-lora-keys] SerenityTrainer baseline config: PASS "
            f"rank={config_rank or DEFAULT_RANK} dtype={config_dtype or DEFAULT_DTYPE} "
            "filter=self_attention,mlp"
        )

    print("[ernie-lora-keys] SerenityTrainer source contract: PASS")
...
def check_mojo_sources(strict_port: bool) -> None:
    ...
    save_body = function_body(lora_save, "save_lora_serenity_trainer")
    ...
    require(stack, "save_lora_serenity_trainer", "Mojo SerenityTrainer save call")
    ...
    require(save_body, ".lora_down.weight", "shared reference trainer down suffix")
    require(save_body, ".lora_up.weight", "shared reference trainer up suffix")
    require(save_body, ".alpha", "shared reference trainer alpha suffix")
    ...
    if "_f32_2d" in save_body or "STDtype.F32" in save_body:
        die("save_lora_serenity_trainer stores SerenityTrainer LoRA tensors through an F32 boundary")
    ...
    if ".lora_down.weight" not in save_body or ".lora_up.weight" not in save_body:
        gaps.append("shared saver does not emit raw SerenityTrainer lora_down/lora_up")
    if ".alpha" not in save_body:
        gaps.append("shared saver does not emit raw SerenityTrainer alpha")
    if "_f32_2d" in save_body or "STDtype.F32" in save_body:
        gaps.append("shared SerenityTrainer saver stores production LoRA tensors as F32")
    if gaps:
        for gap in gaps:
...
def check_saved_header(
    ...
    expected_adapter_count = num_layers * len(SERENITY_MODULES)
...
def main() -> int:
    ...
    parser.add_argument("--config", type=Path, default=SERENITY_CONFIG if SERENITY_CONFIG.exists() else None)
    ...
    config_rank, config_dtype = check_serenity_source(args.config)
