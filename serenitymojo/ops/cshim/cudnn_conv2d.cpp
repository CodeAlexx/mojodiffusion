...
// NHWC variant for the image decoders and the SDXL UNet, whose activations are
// already [N, H, W, C]. Same engine, same F32 accumulation, same heuristic;
// only the descriptor formats differ. The filter is expected in cuDNN's NHWC
// filter order KRSC = [Cout, Kh, Kw, Cin] (cudnnSetFilter4dDescriptor takes the
// logical K, C, R, S counts regardless of format). A null bias skips the add.
extern "C" int serenity_cudnn_conv2d_bf16_nhwc(
    const void* input,
    const void* filter,
    const void* bias,
    void* output,
    ...
    if (runtime_version / 10000 != 9 || runtime_version < 91002) {
        std::fprintf(
            stderr,
            "[serenity_cudnn_conv2d_nhwc] requires cuDNN 9.10.2 or newer cuDNN 9 "
            "runtime, loaded %zu\n",
            runtime_version);
        return -91002;
    }
    if (runtime_version != 91002) {
        ...
        std::call_once(version_warning, [runtime_version]() {
            std::fprintf(
                stderr,
                "[serenity_cudnn_conv2d_nhwc] using cuDNN %zu; Creator numeric "
                "acceptance remains gated against the pinned 9.10.2 oracle\n",
                runtime_version);
        });
    }
    ...
    do {
        ...
        status = cudnnSetTensor4dDescriptor(
            x_desc, CUDNN_TENSOR_NHWC, CUDNN_DATA_BFLOAT16, n, cin, hin, win);
        if (status != CUDNN_STATUS_SUCCESS) break;
        status = cudnnSetFilter4dDescriptor(
            w_desc, CUDNN_DATA_BFLOAT16, CUDNN_TENSOR_NHWC, cout, cin, kh, kw);
        if (status != CUDNN_STATUS_SUCCESS) break;
        status = cudnnSetConvolution2dDescriptor(
            conv_desc, pad_h, pad_w, stride_h, stride_w, 1, 1,
            CUDNN_CROSS_CORRELATION, CUDNN_DATA_FLOAT);
        ...
        if (status != CUDNN_STATUS_SUCCESS) break;
        status = cudnnSetTensor4dDescriptor(
            y_desc, CUDNN_TENSOR_NHWC, CUDNN_DATA_BFLOAT16, n, cout, hout, wout);
        if (status != CUDNN_STATUS_SUCCESS) break;
        ...
        if (std::getenv("SERENITY_CUDNN_CONV2D_TRACE")) {
            std::fprintf(
                stderr,
                "[serenity_cudnn_conv2d_nhwc] n=%d cin=%d hin=%d win=%d "
                "cout=%d kh=%d kw=%d algo=%d cudnn=%zu\n",
                n, cin, hin, win, cout, kh, kw, (int)algo,
                runtime_version);
        }
        ...
        if (bias) {
            if ((status = cudnnCreateTensorDescriptor(&b_desc)) != CUDNN_STATUS_SUCCESS) break;
            status = cudnnSetTensor4dDescriptor(
                b_desc, CUDNN_TENSOR_NHWC, CUDNN_DATA_BFLOAT16, 1, cout, 1, 1);
            if (status != CUDNN_STATUS_SUCCESS) break;
            const float one = 1.0f;
            status = cudnnAddTensor(
                g_conv2d_handle, &one, b_desc, bias, &one, y_desc, output);
